...
export PATH := $(ANDROID_NDK)/toolchains/llvm/prebuilt/$(android_host)/bin:$(PATH)

$(OUTDIR)/lib/libc++_shared.so: $(ANDROID_NDK)/toolchains/llvm/prebuilt/$(android_host)/sysroot/usr/lib/arm-linux-androideabi/libc++_shared.so
	$(CP) $< $@
...
CPP=armv7a-linux-androideabi$(ANDROID_API_LEVEL)-clang -E
LD=ld
RANLIB=llvm-ranlib
AR=llvm-ar
STRIP=llvm-strip -S
...
CONFIGURE_TARGET=--host armv7a-linux-androideabi$(ANDROID_API_LEVEL)
...
CFLAGS_generic:=-fPIC -fstack-protector-all -mfloat-abi=softfp -mfpu=neon -march=armv7-a -mtune=cortex-a8 -ftree-vectorize
CFLAGS_release:=-O3
CFLAGS_debug:=-g -O0
CXXFLAGS_generic:=$(CFLAGS_generic)
CXXFLAGS_release:=$(CFLAGS_release)
CXXFLAGS_debug:=$(CFLAGS_debug)
LDFLAGS_generic:=-static-libstdc++

# Per-package flags
kdu_PLATFORM=arm-eabi-clang
...
openssl_CONFIG=ANDROID_NDK_HOME=$(ANDROID_NDK) ./Configure android-arm -D__ANDROID_API__=$(ANDROID_API_LEVEL) no-shared no-module --prefix=$(OUTDIR_CYGSAFE) $(openssl_CFLAGS)
openssl_LDFLAGS=
