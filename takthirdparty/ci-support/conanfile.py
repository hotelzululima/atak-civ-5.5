...
class TTPDistConan(ConanFile):
    ...
    def set_version(self):
        content = load(self, os.path.join(self.recipe_folder, "gradle/versions.gradle"))
    ...
    def package(self):
        ...
        copy(self, "*", os.path.join(self.source_folder, "../" + source + "/include"), os.path.join(self.package_folder, source + "/include"))
        copy(self, "*", os.path.join(self.source_folder, "../" + source + "/bin"), os.path.join(self.package_folder, source + "/bin"))
        copy(self, "*", os.path.join(self.source_folder, "../" + source + "/debuglib"), os.path.join(self.package_folder, source + "/debuglib"))
        ...
        if self.settings.os == self.CONST_WINDOWS_OS and self.settings.arch == self.CONST_x64_ARCH:
            for lib in self.CONST_WIN_64_COMPILE_LIBS:
                copy(self, f"{lib}", os.path.join(self.source_folder, "../" + source + "/lib"), os.path.join(self.package_folder, source + "/lib"))
        # If we're not packaging ttp-dist for Win64, then simply copy everything from /lib
        else:
            copy(self, "*", os.path.join(self.source_folder, "../" + source + "/lib"), os.path.join(self.package_folder, source + "/lib"))
