...
public class Commo {
     ...
     * @throws CommoException if library loading fails for some reason
     *
    ...
    */
     ...
     * @throws CommoException if loading or initialization fails for
     *                       any reason
     *
     ...
     *                         transfers
     * @throws CommoException if simpleIO is null or if this method has
     ...
     *                         transactions
     * @throws CommoException if enrollmentIO is null or if this method has
     *        previously been called successfully for this Commo instance.
     */
    public void enableEnrollment(EnrollmentIO enrollmentIO)
                                        throws CommoException
    {
        if (!enableEnrollmentNative(nativePtr, enrollmentIO))
     ...
     * The provided CoTDetailExtender implementation will remain active
     ...
     * @throws IllegalArgumentException if extensionId is negative or any argument is null
     * @throws CommoException if another Extender has already been
     *         registered for the given extensionId or any other error occurs
     */
    public void registerCoTDetailExtender(int extensionId, String extensionDetailElement, CoTDetailExtender extender) throws CommoException
    {
        if (extensionId < 0 || extensionDetailElement == null || extender == null)
            throw new IllegalArgumentException();
        if (!registerCoTDetailExtenderNative(nativePtr, extensionId, extensionDetailElement, extender))
            throw new CommoException("Failed to register CoTDetailExtension for " + extensionDetailElement);
    }
    ...
    /**
      ...
      * @throws IllegalArgumentException if extensionId is negative
      * @throws CommoException no Extender has been registered for
      *         the given extensionId or any other error occurs.
      */
    public void deregisterCoTDetailExtender(int extensionId) throws CommoException
    {
        if (extensionId < 0)
            throw new IllegalArgumentException();
        if (!deregisterCoTDetailExtenderNative(nativePtr, extensionId))
     ...
     *        this behavior
     */
    public void setDestUidInsertionEnabled(boolean insertDestUid)
    {
        setDestUidInsertionEnabledNative(nativePtr, insertDestUid);
    }
    ...
    /**
     * Enables or disables (the default) the ability of datagram sockets created by
     ...
     * @throws CommoException if the web port is not valid/available or
     ...
     *                     listen on, or MPIO_LOCAL_PORT_DISABLE.
     *                     The port must be free at the time of invocation.
     * @param certificate certificate store, with key, in pkcs12 format
     * @param certPass password for certificate
     * @throws CommoException if the web port is not valid/available,
     *     setupMissionPackageIO has not yet been successfully invoked,
     ...
     * @throws CommoException if the port number is out of range
     */
     ...
     * @throws CommoException if the value is out of range
     */
     ...
     * @param certificat certificate store, with key, in pkcs12 format
     * @param certPass password for certificate
     * @return QuicInboundNetInterface object uniquely identifying the
     *         added interface.  Remains valid as long as it is
     *         not passed to removeQuicInboundInterface
     * @throws CommoException if any error occurs, including but not limited to
     *                        port out of range, invalid certificate,
     *                        invalid certificate password,
     *                        or the specified port is already in use.
     */
    public QuicInboundNetInterface addQuicInboundInterface(int port,
            byte[] certificate, String certPass)
                                           throws CommoException
    {
        if (certPass == null || certificate == null)
            throw new CommoException("certificate and certPass are required");
        QuicInboundNetInterface ret = addQuicInboundNative(nativePtr,
                                   port, certificate,
                                   certificate.length,
                                   certPass);
        if (ret == null)
     ...
     *                     was already removed
     */
    public void removeQuicInboundInterface(QuicInboundNetInterface iface) {
        long removeMe = iface.getNativePtr();
        if (removeMe == 0)
            throw new IllegalArgumentException("Interface already removed");
        if (!removeQuicInboundNative(nativePtr, iface.getNativePtr()))
            throw new IllegalArgumentException("Interface invalid");
    }
     ...
     * @deprecated replaced with addStreamingInterface(StreamingTransport, ...)
     *             overload
     */
    ...
    {
        StreamingTransport transport = StreamingTransport.TCP;
        if (clientCert != null)
            transport = StreamingTransport.SSL;
        
        return addStreamingInterface(transport, hostname, port, types,
                                     clientCert, caCert, certPassword,
                                     caCertPassword, username, password);
    }
     ...
     * For TCP, null MUST be specified for for clientCert, trustStore,
     * username, and password as these cannot be utilized on plain TCP
     * connections.
     * For SSL and QUIC, clientCert and trustStore must be valid/non-null. 
     * Additionally, with these transports, a non-null username and password
     * will result
     ...
     * Regardless of the type of connection, this Commo instance will attempt
     * to create and restore the server connection as needed until this
     * interface is removed or this Commo object is shutdown.
     * The returned interface is valid until removed via
     * removeStreamingInterfaceor this Commo object is shutdown.
     *  
     ...
     *                       or invalid combinations of arguments
     */
    public StreamingNetInterface addStreamingInterface(
            StreamingTransport transport, String hostname,
            int port, CoTMessageType[] types, byte[] clientCert,
            byte[] caCert, String certPassword, 
            String caCertPassword,
            String username, String password) throws CommoException
    {
        ...
        StreamingNetInterface ret = addStreamingNative(nativePtr,
                                 transport.getNativeVal(), hostname, port,
                                 typeInts,
     ...
     * @param cotMessage CoT message to send
     * @throws CommoException if cotMessage or streamId is not valid
     */
     ...
     * with the unique identifier returned.  The enrollment operation
     * will not commence processing until enrollmentStart() is invoked 
     * with the returned identifier.  Once started, updates on progress
     ...
     *             an identifier
     * @return integer identifier that uniquely identifies this enrollment
     *             operation
     * @throws CommoException if an error occurs, including but not limited
     *                       to caCert being invalid, invalid port number,
     *                       or missing/invalid arguments
     */
    public int enrollmentInit(String host, int port, boolean verifyHost,
                              String user, String password,
                              boolean useTokenAuth,
                              byte[] caCert, String caCertPassword,
                              String clientCertPassword,
                              String enrolledTrustPassword,
                              String keyPassword,
                              int keyLength,
                              String clientVersionInfo) throws CommoException
    {
        return enrollmentInitNative(nativePtr, host, port, verifyHost,
                                    user, password,
                                    useTokenAuth,
                                    caCert,
                                    caCert != null ? caCert.length : 0,
     ...
     * @param id identifier of the previously initialized enrollment operation
     * @throws CommoException if the supplied id does not identify an
     *        initialized (but not already started) enrollment operation
     */
    public void enrollmentStart(int id) throws CommoException
    {
        if (!enrollmentStartNative(nativePtr, id))
     ...
     * @throws CommoException if the supplied xferId does not
     *         identify an initialized (but not already started) transfer
     */
     ...
     * @deprecated Replaced with overload which receives security parameters.
     */
    @Deprecated
    ...
    {
        return sendMissionPackageInit(destinations, file, 
     ...
     * @param xferName the name for the transfer, used to generically refer
     *                 to the transfer both locally and remotely
     * @param access value to use for the access attribute of the event element
     *               when sending CoT associated with the transfer; supplied
     *               value is assumed to be valid for CoT and not internally
     *               checked. null or empty string results in "Undefined" as
     *               the value
     * @param caveats value to use for the caveats attribute of the event
     *               element when sending CoT associated with the transfer;
     *               supplied value is assumed to be valid for CoT and not
     *               internally checked
     * @param releasableTo value to use for the releasableTo attribute of the
     *               event element when sending CoT associated with the
     *               transfer; supplied value is assumed to be valid for CoT
     *               and not internally checked
                                  ...
                                  String releasableTo) throws CommoException
    {
        ...
        MPNativeResult ret = sendMPInitNative(nativePtr, uids,
                                          ...
                                          xferFileName, xferName,
                                          access, caveats, releasableTo);
        if (ret == null || ret.error != null)
            throw new CommoException(ret != null ? ret.error : "Invalid argument(s) for mission package send");
     ...
     * @param certPass the password used to protect the new certificate
     * @return the pkcs12 self-signed certificate and private key generated
     */
     ...
     * @throws CommoException if the source XML is not valid CoT,
     ...
     *         as an XML string.
     * @throws CommoException the protodata is not valid TAK protocol data,
    ...
    static native boolean enableSimpleFileIONative(long nativePtr,
                                   SimpleFileIO io);
    static native boolean enableEnrollmentNative(long nativePtr,
                                   EnrollmentIO io);
    ...
    static native boolean registerCoTDetailExtenderNative(long nativePtr, int extensionId, String extensionDetailElement, CoTDetailExtender extender);
    static native boolean deregisterCoTDetailExtenderNative(long nativePtr, int extensionId);
    static native void setDestUidInsertionEnabledNative(long nativePtr, boolean insertDestUid);
    ...
    static native boolean removeTcpInboundNative(long nativePtr,
                                                 long ifaceNativePtr);
    static native QuicInboundNetInterface
                  addQuicInboundNative(long nativePtr, int port,
                                       byte[] cert, int certLen,
                                       String certPass) throws CommoException;
    static native boolean removeQuicInboundNative(long nativePtr,
                                                  long ifaceNativePtr);
    ...
    static native boolean broadcastCoTNative(long nativePtr, String cot,
                                             int method);
    static native int enrollmentInitNative(long nativePtr,
                                         String host,
                                         int port,
                                         boolean verifyHost,
                                         String user,
                                         ...
                                         throws CommoException;
    static native boolean enrollmentStartNative(long nativePtr,
                                         int id);
