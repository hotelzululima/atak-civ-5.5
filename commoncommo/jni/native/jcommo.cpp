...
jmethodID EnrollmentIOJNI::jmethod_enrollmentUpdate = NULL;
    
jclass EnrollmentIOJNI::jclass_enrollmentIOUpdate = NULL;
jmethodID EnrollmentIOJNI::jmethod_enrollmentIOUpdateCtor = NULL;
jglobalobjectref EnrollmentIOJNI::nativeEnrollmentStepToJava[NUM_ENROLLMENT_STEPS];

EnrollmentIOJNI::EnrollmentIOJNI(JNIEnv *env, jobject jenrollmentio)
                        COMMO_THROW (int) : EnrollmentIO(), jenrollmentio(NULL)
{
    this->jenrollmentio = env->NewGlobalRef(jenrollmentio);
    if (!this->jenrollmentio)
        throw 1;
}


void EnrollmentIOJNI::destroy(JNIEnv *env, EnrollmentIOJNI *enrollmentio)
{
    if (enrollmentio->jenrollmentio) {
        env->DeleteGlobalRef(enrollmentio->jenrollmentio);
        enrollmentio->jenrollmentio = NULL;
    }
    delete enrollmentio;
}


void EnrollmentIOJNI::enrollmentUpdate(
                    const EnrollmentIOUpdate *update)
{
    JNIEnv *env = NULL;
    LocalJNIEnv localEnv(&env);
    if (!env)
    ...
    if (update->additionalInfo != NULL)
        jinfo = env->NewStringUTF((const char *)
                                     update->additionalInfo);
    jobject jstep = nativeEnrollmentStepToJava[update->step];
    ...
    if (update->totalBytesToTransfer <= INT64_MAX)
        total = (jlong)update->totalBytesToTransfer;
    jbyteArray jpriv = NULL;
    if (update->privResult) {
        jpriv = env->NewByteArray(update->privResultLen);
        if (!jpriv) {
            env->ExceptionClear();
            return;
        }
        env->SetByteArrayRegion(jpriv, 0, update->privResultLen, 
                                (const jbyte *)update->privResult);
    }
    jbyteArray jca = NULL;
    if (update->caResult) {
        jca = env->NewByteArray(update->caResultLen);
        if (!jca) {
            env->ExceptionClear();
            return;
        }
        env->SetByteArrayRegion(jca, 0, update->caResultLen, 
                                (const jbyte *)update->caResult);
    }
    
    
    jobject jupdate = env->NewObject(jclass_enrollmentIOUpdate, 
                                     jmethod_enrollmentIOUpdateCtor,
                                     jstep,
                                     update->xferid,
                                     jstat,
                                     jinfo,
    ...
    env->CallVoidMethod(jenrollmentio, jmethod_enrollmentUpdate, jupdate);
}


bool EnrollmentIOJNI::reflectionInit(JNIEnv *env)
{
    jclass class_enrollmentio;
    jclass class_enrollmentstep;
    jmethodID method_enrollmentstepValues;
    jmethodID method_enrollmentstepName;
    jmethodID method_enrollmentstepValue;
    jobjectArray jenrollmentstepValues;
    jsize numVals;
    bool ret = false;

    memset(nativeEnrollmentStepToJava, 0, sizeof(nativeEnrollmentStepToJava));

    LOOKUP_CLASS(class_enrollmentio, COMMO_PACKAGE "EnrollmentIO", true);
    LOOKUP_METHOD(jmethod_enrollmentUpdate, class_enrollmentio, 
                  "enrollmentUpdate",
                  "(L" COMMO_PACKAGE "EnrollmentIOUpdate;)V");
    
    LOOKUP_CLASS(jclass_enrollmentIOUpdate, 
                 COMMO_PACKAGE "EnrollmentIOUpdate", false);
    LOOKUP_METHOD(jmethod_enrollmentIOUpdateCtor, jclass_enrollmentIOUpdate,
                  "<init>",
                  "(L" COMMO_PACKAGE "EnrollmentStep;IL" COMMO_PACKAGE "SimpleFileIOStatus;Ljava/lang/String;JJ[B[B)V");
    
    // Build mapping of native file io status to java versions
    LOOKUP_CLASS(class_enrollmentstep, 
                 COMMO_PACKAGE "EnrollmentStep", true);
    LOOKUP_STATIC_METHOD(method_enrollmentstepValues, class_enrollmentstep,
                         "values",
                         "()[L" COMMO_PACKAGE "EnrollmentStep;");
    jenrollmentstepValues = (jobjectArray)env->CallStaticObjectMethod(
                         class_enrollmentstep, method_enrollmentstepValues);
    if (env->ExceptionOccurred())
        goto cleanup;
    
    numVals = env->GetArrayLength(jenrollmentstepValues);
    
    if (numVals != NUM_ENROLLMENT_STEPS)
        goto cleanup;
        
    LOOKUP_METHOD(method_enrollmentstepName, class_enrollmentstep, 
                  "name", "()Ljava/lang/String;");
    LOOKUP_METHOD(method_enrollmentstepValue, class_enrollmentstep,
                  "getNativeVal", "()I");

#define CHECK_ESTEP(val) checkEnum(env, method_enrollmentstepName,           \
                                      method_enrollmentstepValue,            \
                                      jenrollmentstepValues, numVals,        \
                                      #val,                                  \
                                      nativeEnrollmentStepToJava,            \
                                      PASTE(ENROLL_STEP_, val),              \
                                      PASTE(ENROLL_STEP_, val))              \

    ret = true;
    ret = ret && CHECK_ESTEP(KEYGEN);
    ret = ret && CHECK_ESTEP(CSR);
    ret = ret && CHECK_ESTEP(SIGN);
#undef CHECK_ESTEP
...
void EnrollmentIOJNI::reflectionRelease(JNIEnv *env)
{
    for (int i = 0; i < NUM_ENROLLMENT_STEPS; ++i) {
        if (nativeEnrollmentStepToJava[i])
            env->DeleteGlobalRef(nativeEnrollmentStepToJava[i]);
        nativeEnrollmentStepToJava[i] = NULL;
    }
    
    env->DeleteGlobalRef(jclass_enrollmentIOUpdate);
}

EnrollmentIOJNI::~EnrollmentIOJNI()
{
}
...
/**********************************************************************/
// CoTDetailExtenderJNI

jmethodID CoTDetailExtenderJNI::jmethod_encode = NULL;
jmethodID CoTDetailExtenderJNI::jmethod_decode = NULL;

CoTDetailExtenderJNI::CoTDetailExtenderJNI(JNIEnv *env, jobject jcotDetailExtender)
                                       COMMO_THROW (int) :
          JNIObjWrapper(), CoTDetailExtender(),
          jcotDetailExtender(NULL)
{
    this->jcotDetailExtender = env->NewGlobalRef(jcotDetailExtender);
    if (!this->jcotDetailExtender)
        throw 1;
}

void CoTDetailExtenderJNI::destroy(JNIEnv *env, CoTDetailExtenderJNI *extender)
{
    if (extender->jcotDetailExtender) {
        env->DeleteGlobalRef(extender->jcotDetailExtender);
        extender->jcotDetailExtender = NULL;
    }
    delete extender;
}

CommoResult CoTDetailExtenderJNI::encode(uint8_t **encodedExtension, size_t *encodedSize, const char *cotDetailElement)
{
    JNIEnv *env = NULL;
    LocalJNIEnv localEnv(&env);
    if (!env)
    ...
    jstring jcotDetailElement = env->NewStringUTF(cotDetailElement);
    if (env->ExceptionOccurred()) {
        env->ExceptionClear();
        return COMMO_ILLEGAL_ARGUMENT;
    }

    jbyteArray jencbytes = (jbyteArray)env->CallObjectMethod(jcotDetailExtender,
                        jmethod_encode, jcotDetailElement);
    
    if (!jencbytes || env->ExceptionOccurred()) {
        env->ExceptionClear();
        return COMMO_ILLEGAL_ARGUMENT;
    }
    
    jsize jEncodedSize = env->GetArrayLength(jencbytes);
    if (jEncodedSize < 0)
        return COMMO_ILLEGAL_ARGUMENT;
    
    size_t n = jEncodedSize;
    uint8_t *ret = new uint8_t[n];
    env->GetByteArrayRegion(jencbytes, 0, jEncodedSize, (jbyte *)ret);
    *encodedExtension = ret;
    *encodedSize = n;
    return COMMO_SUCCESS;
}

CommoResult CoTDetailExtenderJNI::decode(const char **cotDetailElement, const uint8_t *encodedExtension, size_t encodedSize)
{
    JNIEnv *env = NULL;
    LocalJNIEnv localEnv(&env);
    if (!env)
    ...
    jbyteArray jencodedExtension = env->NewByteArray(encodedSize);
    if (!jencodedExtension) {
        env->ExceptionClear();
        return COMMO_ILLEGAL_ARGUMENT;
    }
    env->SetByteArrayRegion(jencodedExtension, 0, encodedSize, (const jbyte *)encodedExtension);
    
    jstring jcotDetailElement = (jstring)env->CallObjectMethod(jcotDetailExtender, jmethod_decode, jencodedExtension);
    if (!jcotDetailElement || env->ExceptionOccurred()) {
        env->ExceptionClear();
        return COMMO_ILLEGAL_ARGUMENT;
    }

    jsize jn = env->GetStringUTFLength(jcotDetailElement);
    jsize jnchars = env->GetStringLength(jcotDetailElement);
    char *ret = new char[jn + 1];
    env->GetStringUTFRegion(jcotDetailElement, 0, jnchars, ret);
    ret[jn] = '\0';
    *cotDetailElement = ret;
    return COMMO_SUCCESS;
}

void CoTDetailExtenderJNI::releaseEncodeResult(const uint8_t *resultBuffer)
{
    delete[] resultBuffer;
}

void CoTDetailExtenderJNI::releaseDecodeResult(const char *resultBuffer)
{
    delete[] resultBuffer;
}

jglobalobjectref CoTDetailExtenderJNI::getWrappedRef() const
{
    return jcotDetailExtender;
}

bool CoTDetailExtenderJNI::reflectionInit(JNIEnv *env)
{
    bool ret = false;
    jclass class_cotDetailExtender = NULL;
    
    LOOKUP_CLASS(class_cotDetailExtender,
                 COMMO_PACKAGE "CoTDetailExtender", true);
    LOOKUP_METHOD(jmethod_encode, class_cotDetailExtender,
                  "encode",
                  "(Ljava/lang/String;)[B");
    LOOKUP_METHOD(jmethod_decode, class_cotDetailExtender,
                  "decode",
                  "([B)Ljava/lang/String;");
    ret = true;
...
void CoTDetailExtenderJNI::reflectionRelease(JNIEnv *env)
{
}

CoTDetailExtenderJNI::~CoTDetailExtenderJNI()
{
}
...
jclass CommoJNI::jclass_quicnetiface = NULL;
jmethodID CommoJNI::jmethod_quicnetifaceCtor = NULL;
...
CommoJNI::CommoJNI(JNIEnv *env, jobject jlogger,
        ...
        commo(NULL), mpio(NULL), fileio(NULL), enrollmentio(NULL),
        logger(NULL),
        jaddrMode(jaddrMode),
        extendersMutex(), extenders(),
...
void CommoJNI::destroy(JNIEnv *env, CommoJNI *commoJNI)
{
    ...
    if (commoJNI->enrollmentio)
        EnrollmentIOJNI::destroy(env, commoJNI->enrollmentio);
    ...
    for (ExtendersMap::iterator i = commoJNI->extenders.begin(); i != commoJNI->extenders.end(); ++i)
        CoTDetailExtenderJNI::destroy(env, i->second);
...
void CommoJNI::enableEnrollment(JNIEnv *env, jobject jenrollmentio) COMMO_THROW (int)
{
    if (enrollmentio)
        throw 1;
    EnrollmentIOJNI *newenrollio = new EnrollmentIOJNI(env, jenrollmentio);
    if (commo->enableEnrollment(newenrollio) != COMMO_SUCCESS) {
        EnrollmentIOJNI::destroy(env, newenrollio);
        throw 1;
    }
    enrollmentio = newenrollio;
}
...
bool CommoJNI::registerCoTDetailExtender(JNIEnv *env, jint jextId, 
                                         jstring jelement, jobject jextender)
{
    if (jextId < 0)
        return false;
    unsigned int extId = (unsigned)jextId;

    const char *element = env->GetStringUTFChars(jelement, NULL);
    if (!element)
        return false;

    CoTDetailExtenderJNI *newExtender = NULL;
    {
        CommoLock lock(extendersMutex);

        if (extenders.find(jextId) != extenders.end()) {
            env->ReleaseStringUTFChars(jelement, element);
            return false;
        }
        
        try {
            newExtender = new CoTDetailExtenderJNI(env,
                                                   jextender);
        } catch (int &) {
            env->ReleaseStringUTFChars(jelement, element);
            return false;
        }
        extenders.insert(ExtendersMap::value_type(jextId, newExtender));
    }
    // then add it
    CommoResult rc = commo->registerCoTDetailExtender(extId, 
                                                      element, newExtender);
    env->ReleaseStringUTFChars(jelement, element);
    if (rc != COMMO_SUCCESS) {
        CommoLock lock(extendersMutex);
        extenders.erase(jextId);
        CoTDetailExtenderJNI::destroy(env, newExtender);
        return false;
    }
    return true;
}

bool CommoJNI::deregisterCoTDetailExtender(JNIEnv *env, jint jextId)
{
    if (jextId < 0)
        return false;
    unsigned int extId = (unsigned)jextId;

    CoTDetailExtenderJNI *extender = NULL;
    {
        CommoLock lock(extendersMutex);

        ExtendersMap::iterator iter = extenders.find(jextId);
        if (iter == extenders.end())
            return false;
        
        extender = iter->second;
        extenders.erase(iter);
    }
    CommoResult rc = commo->deregisterCoTDetailExtender(extId);
    CoTDetailExtenderJNI::destroy(env, extender);
    return rc == COMMO_SUCCESS;
}
...
jobject CommoJNI::wrapQuicNetInterface(JNIEnv *env, 
                                      QuicInboundNetInterface *iface,
                                      jint port)
{
    // Wrap in global java obj
    jobject ret = env->NewObject(jclass_quicnetiface, jmethod_quicnetifaceCtor,
                                 PTR_TO_JLONG(iface),
                                 port);
    if (env->ExceptionOccurred())
        return NULL;
...
bool CommoJNI::reflectionInit(JNIEnv *env)
{
    ...
    jclass class_streamingTransport;
    jmethodID method_streamingTransportValues;
    jmethodID method_streamingTransportName;
    jmethodID method_streamingTransportValue;
    jobjectArray jstreamingTransportVals;
    jclass class_send;
    ...
    LOOKUP_CLASS(jclass_quicnetiface, COMMO_PACKAGE "QuicInboundNetInterface", false);
    ...
    LOOKUP_METHOD(jmethod_tcpnetifaceCtor, jclass_tcpnetiface, 
                  "<init>", "(JI)V");
    LOOKUP_METHOD(jmethod_quicnetifaceCtor, jclass_quicnetiface, 
                  "<init>", "(JI)V");
...
#undef CHECK_ADDRMODE
    ...
    // Check mapping of native streaming transport to java equivalent
    LOOKUP_CLASS(class_streamingTransport, COMMO_PACKAGE "StreamingTransport", true);
    LOOKUP_STATIC_METHOD(method_streamingTransportValues, class_streamingTransport, "values", "()[L" COMMO_PACKAGE "StreamingTransport;");
    jstreamingTransportVals = (jobjectArray)env->CallStaticObjectMethod(class_streamingTransport, method_streamingTransportValues);
    if (env->ExceptionOccurred())
        goto cleanup;
    
    numVals = env->GetArrayLength(jstreamingTransportVals);
    
    LOOKUP_METHOD(method_streamingTransportName, class_streamingTransport, "name", 
                  "()Ljava/lang/String;");
    LOOKUP_METHOD(method_streamingTransportValue, class_streamingTransport, "getNativeVal", "()I");


#define CHECK_TRANSPORT(i,streamingTransport) checkEnum(env,                 \
                                      method_streamingTransportName,         \
                                      method_streamingTransportValue,        \
                                      jstreamingTransportVals, numVals,      \
                                      #streamingTransport,                   \
                                      NULL,                                  \
                                      i,                                     \
                                      PASTE(netinterfaceenums::TRANSPORT_, streamingTransport)) \

    ret = ret && numVals == 3;
    ret = ret && CHECK_TRANSPORT(0, TCP);
    ret = ret && CHECK_TRANSPORT(1, SSL);
    ret = ret && CHECK_TRANSPORT(2, QUIC);
#undef CHECK_TRANSPORT
    ...
    ret = ret && EnrollmentIOJNI::reflectionInit(env);
    ...
    ret = ret && CoTDetailExtenderJNI::reflectionInit(env);
...
{
    CoTDetailExtenderJNI::reflectionRelease(env);
    ...
    EnrollmentIOJNI::reflectionRelease(env);
    ...
    env->DeleteGlobalRef(jclass_quicnetiface);
...
JNIEXPORT jboolean JNICALL
Java_com_atakmap_commoncommo_Commo_enableEnrollmentNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr,
     jobject jEnrollmentIO)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    bool ret = false;
    try {
        c->enableEnrollment(env, jEnrollmentIO);
        ret = true;
    } catch (int &) {
    }
    return ret;
...
JNIEXPORT jboolean JNICALL Java_com_atakmap_commoncommo_Commo_registerCoTDetailExtenderNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr, jint jextId, jstring jelement, jobject jextender)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    return c->registerCoTDetailExtender(env, jextId, jelement, jextender);
}


JNIEXPORT jboolean JNICALL Java_com_atakmap_commoncommo_Commo_deregisterCoTDetailExtenderNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr, jint jextId)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    return c->deregisterCoTDetailExtender(env, jextId);
}


JNIEXPORT void JNICALL
Java_com_atakmap_commoncommo_Commo_setDestUidInsertionEnabledNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr, jboolean jinsertUid)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    c->commo->setDestUidInsertionEnabled(jinsertUid == JNI_TRUE);
}
...
JNIEXPORT jobject JNICALL Java_com_atakmap_commoncommo_Commo_addQuicInboundNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr, jint port,
     jbyteArray jcert, jint certLen, jstring jcertPass)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    ...
    CommoResult r;
    QuicInboundNetInterface *quicIn = c->commo->addQuicInboundInterface(port,
            (uint8_t *)cert, certLen, certPass, &r);
    ...
    switch (r) {
    case COMMO_SUCCESS:
        ret = c->wrapQuicNetInterface(env, quicIn, port);
        if (!ret) {
            // can't wrap it, so don't orphan it - take it back down
            c->commo->removeQuicInboundInterface(quicIn);
            err = "Internal error";
        }
        break;
    case COMMO_ILLEGAL_ARGUMENT:
        err = "port or other argument invalid";
        break;
    case COMMO_INVALID_CERT:
        err = "certificate is invalid";
        break;
    case COMMO_INVALID_CERT_PASSWORD:
...
JNIEXPORT jboolean JNICALL Java_com_atakmap_commoncommo_Commo_removeQuicInboundNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr, jlong ifaceNativePtr)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    QuicInboundNetInterface *iface = JLONG_TO_PTR(QuicInboundNetInterface,
                                                  ifaceNativePtr);
    CommoResult res = c->commo->removeQuicInboundInterface(iface);
    if (res == COMMO_SUCCESS) {
        c->unwrapNetInterface(env, iface);
        return true;
    } else
...
Java_com_atakmap_commoncommo_Commo_addStreamingNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr, jint jtransport,
     jstring jhostname,
     ...
     jstring juser, jstring jpassword)
{
    ...
    StreamingNetInterface *stream = 
        c->commo->addStreamingInterface(
                (netinterfaceenums::StreamingTransport)jtransport,
                hostname,
                port,
                types,
                typesLen,
...
JNIEXPORT jint JNICALL Java_com_atakmap_commoncommo_Commo_enrollmentInitNative
  (JNIEnv *env, jclass selfCls, jlong nativePtr,
   jstring jhost, jint port, jboolean jverifyHost,
   jstring juser, jstring jpassword, jboolean juseTokenAuth,
   jbyteArray jcaCert, jint caCertLen, jstring jcaCertPassword,
   jstring jclientCertPassword, jstring jenrolledTrustPassword,
   jstring jkeyPassword, jint keyLen, jstring jclientVersionInfo)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);

    CommoResult result = COMMO_ILLEGAL_ARGUMENT;
    const char *err = NULL;
    int xferId = -1;
    ...
    jbyte *caCert = NULL;
    const char *caCertPassword = NULL;
    const char *clientCertPassword = NULL;
    const char *enrolledTrustPassword = NULL;
    const char *keyPassword = NULL;
    const char *clientVersionInfo = NULL;
    ...
    clientCertPassword = env->GetStringUTFChars(jclientCertPassword, NULL);
    if (!clientCertPassword)
        goto cleanup;

    enrolledTrustPassword = env->GetStringUTFChars(jenrolledTrustPassword, NULL);
    if (!enrolledTrustPassword)
        goto cleanup;

    keyPassword = env->GetStringUTFChars(jkeyPassword, NULL);
    if (!keyPassword)
        goto cleanup;

    if (jclientVersionInfo) {
        clientVersionInfo = env->GetStringUTFChars(jclientVersionInfo, NULL);
        if (!clientVersionInfo)
            goto cleanup;
    }

    result = c->commo->enrollmentInit(&xferId,
                                      host,
                                      port,
                                      jverifyHost == JNI_TRUE,
                                      user,
                                      password,
                                      juseTokenAuth == JNI_TRUE,
                                      (uint8_t *)caCert,
                                      caCertLen,
                                      caCertPassword,
                                      clientCertPassword,
    ...
    case COMMO_ILLEGAL_ARGUMENT:
        ...
        break;
    case COMMO_INVALID_CACERT:
        err = "Provided enrollment CA Certificate is invalid";
        break;
    case COMMO_INVALID_CACERT_PASSWORD:
        err = "Invalid password for provided enrollment CA Certificate";
        break;
    default:
    ...
    if (err) {
        env->ThrowNew(CommoJNI::jclass_commoexception, err);
        // proceed to cleanup now that we've queued error
    }
...
cleanup:
    ...
    if (caCertPassword)
        env->ReleaseStringUTFChars(jcaCertPassword, caCertPassword);
    if (clientCertPassword)
        env->ReleaseStringUTFChars(jclientCertPassword, clientCertPassword);
    if (enrolledTrustPassword)
        env->ReleaseStringUTFChars(jenrolledTrustPassword, enrolledTrustPassword);
    if (keyPassword)
        env->ReleaseStringUTFChars(jkeyPassword, keyPassword);
    if (clientVersionInfo)
        env->ReleaseStringUTFChars(jclientVersionInfo, clientVersionInfo);
        
    return xferId;
}


JNIEXPORT jboolean JNICALL Java_com_atakmap_commoncommo_Commo_enrollmentStartNative
  (JNIEnv *env, jclass selfCls, jlong nativePtr, jint id)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    
    if (c->commo->enrollmentStart(id) == COMMO_SUCCESS)
        return JNI_TRUE;
    else
        return JNI_FALSE;
}
...
Java_com_atakmap_commoncommo_Commo_sendMPInitNative
    (JNIEnv *env, jclass selfCls, jlong nativePtr, jobjectArray jcontactUIDs,
     ...
     jstring jaccess, jstring jcaveats, jstring jrelTo)
{
    CommoJNI *c = JLONG_TO_PTR(CommoJNI, nativePtr);
    ...
    if (jaccess) {
        access = env->GetStringUTFChars(jaccess, NULL);
        if (!access) {
            env->ReleaseStringUTFChars(jfilePath, filePath);
            env->ReleaseStringUTFChars(jxferFileName, xferFileName);
    ...
    const char *caveats = NULL;
    if (jcaveats) {
        caveats = env->GetStringUTFChars(jcaveats, NULL);
        if (!caveats) {
            env->ReleaseStringUTFChars(jfilePath, filePath);
            env->ReleaseStringUTFChars(jxferFileName, xferFileName);
            env->ReleaseStringUTFChars(jxferName, xferName);
            if (access)
                env->ReleaseStringUTFChars(jaccess, access);
            return NULL;
        }
    }
    const char *relTo = NULL;
    if (jrelTo) {
        relTo = env->GetStringUTFChars(jrelTo, NULL);
        if (!relTo) {
            env->ReleaseStringUTFChars(jfilePath, filePath);
            env->ReleaseStringUTFChars(jxferFileName, xferFileName);
            env->ReleaseStringUTFChars(jxferName, xferName);
            if (access)
                env->ReleaseStringUTFChars(jaccess, access);
            if (caveats)
                env->ReleaseStringUTFChars(jcaveats, caveats);
            return NULL;
        }
    }
    ...
    if (caveats)
        env->ReleaseStringUTFChars(jcaveats, caveats);
    if (relTo)
        env->ReleaseStringUTFChars(jrelTo, relTo);
    
    if (err) {
        // Either exception already thrown due to JNI failures
        // or we had ILLEGAL_ARGUMENT return
        return NULL;
    } else {
