...
#ifdef WIN32
    // windows implements recvmsg as a runtime-fetchable function pointer
    // that may or may not be available on a particular windows version
    // or winsock provider;  NULL if not available
...
void PlatformNet::staticInit(CommoLogger *logger)
{
#ifdef WIN32
    PlatformNet::SocketFD s = createSocket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    LPFN_WSARECVMSG ptr;
    DWORD n;
    GUID rxmsgGuid = WSAID_WSARECVMSG;
    if (s == BAD_SOCKET)
        return;
    if (WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER,
             &rxmsgGuid, sizeof(GUID),
             &ptr, sizeof(ptr),
             &n, NULL, NULL) == 0) {
        pWSARecvMsg = ptr;
    } else {
        InternalUtils::logprintf(logger, CommoLogger::LEVEL_ERROR, "Error retreiving winsock WSARecvMsg ptr - recvfrom(localaddr) will not be available");
    }
    closeSocket(s);
#endif
}
...
{
#ifdef WIN32
    PlatformNet::SocketFD newfd = WSASocket(addressFamily, type, protocol, NULL, 0, WSA_FLAG_OVERLAPPED | WSA_FLAG_NO_HANDLE_INHERIT);
#else
    PlatformNet::SocketFD newfd = ::socket(addressFamily, type | SOCK_CLOEXEC, protocol);
#endif
    return newfd;
}
...
void PlatformNet::setSocketEnablePktInfo(SocketFD fd)
{
#ifdef WIN32
    DWORD en;
#else
...
#endif
    en = 1;
    setsockopt(fd, IPPROTO_IP, IP_PKTINFO, SOCKOPT_CAST &en, sizeof(en));
}
...
PlatformNet::ErrorCode PlatformNet::socketReadFrom(SocketFD fd,
                        uint8_t *data, size_t *len,
                        struct sockaddr *srcAddr, socklen_t *srcAddrLen,
                        struct sockaddr *localAddr, socklen_t *localAddrLen,
                        bool *localAddrValid)
{
    socklen_t slen0 = *srcAddrLen;
#ifdef WIN32
    if (localAddr && pWSARecvMsg) {
        if (*len > ULONG_MAX) {
            *len = 0;
            return OTHER_ERROR;
        }
        DWORD msgLen = 0;
        ULONG lenULong = (ULONG)*len;
        ...
        wbuf.buf = (char *)data;
        wbuf.len = lenULong;
        char pktinfmsgbuf[WSA_CMSG_SPACE(sizeof(struct in_pktinfo))];
        ...
        msg.Control.len = sizeof(pktinfmsgbuf);
        msg.Control.buf = pktinfmsgbuf;
        
        int n = pWSARecvMsg(fd, &msg, &msgLen, NULL, NULL);
        if (n == SOCKET_ERROR || msgLen == 0) {
            *len = 0;
            if (n == SOCKET_ERROR && WSAGetLastError() == WSAEWOULDBLOCK)
                return IO_WOULD_BLOCK;
            else
        ...
        *srcAddrLen = msg.namelen;
        *localAddrValid = false;
        if (srcAddr->sa_family == AF_INET) {
            for (WSACMSGHDR *cmhdr = WSA_CMSG_FIRSTHDR(&msg); cmhdr; cmhdr = WSA_CMSG_NXTHDR(&msg, cmhdr)) {
                if (cmhdr->cmsg_level == IPPROTO_IP && cmhdr->cmsg_type == IP_PKTINFO) {
                    struct in_pktinfo pktinf;
                    memcpy(&pktinf, WSA_CMSG_DATA(cmhdr), sizeof(pktinf));
                    ...
                    *localAddrLen = sizeof(localAddr4);
                    // port to be set by caller
                    *localAddrValid = true;
                    break;
                }
            }
        ...
        *len = msgLen;
        ...
        int lenInt = (int)*len;
        int n = ::recvfrom(fd, (char *)data, lenInt, 0, srcAddr, srcAddrLen);
        if (n == SOCKET_ERROR || n == 0) {
            *len = 0;
            if (n == SOCKET_ERROR && WSAGetLastError() == WSAEWOULDBLOCK)
                return IO_WOULD_BLOCK;
    ...
    if (localAddr) {
        ...
        *len = (size_t)n;
        ...
        ssize_t n = ::recvfrom(fd, data, *len, 0, srcAddr, srcAddrLen);
        if (n < 0 || *srcAddrLen > slen0) {
            *len = 0;
            if (n < 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
                return IO_WOULD_BLOCK;
            else if (*srcAddrLen > slen0)
