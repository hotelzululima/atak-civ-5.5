...
typedef std::set<unsigned> ExtensionIdSet;
...
public:
    ...
    // true on success, false if already exists.  name is copied
    bool addExtension(unsigned id, const char *name, CoTDetailExtender *ext);
    
    // true if extension exists and is removed, false if did not exist
    bool removeExtension(unsigned id);
    
    // Lookup extension by name. The lock pointer, if not holding a lock,
    // is initialized to hold one before the lookup takes place.
    // If exists, returns id, impl.
    // True if looked up successfully, false otherwise.
    // The read lock is held open for use by caller regardless of return value.
    bool getByName(thread::ReadLockPtr &lock, unsigned *id, CoTDetailExtender **ext, const char *name);

    // Lookup extension by id. If exists, returns its name, impl.
    // The lock pointer, if not holding a lock,
    // is initialized to hold one before the lookup takes place.
    // True if looked up successfully, false otherwise.
    // The read lock is held open for use by caller regardless of return value.
    bool getById(thread::ReadLockPtr &lock, std::string &name, CoTDetailExtender **ext, unsigned id);
    
    // Gets set of extension ids that are currently registered.
    // Provided set is cleared on entry
    void getAllIds(ExtensionIdSet &ids);

    // Obtain reference to an empty ExtensionIdSet
    static const ExtensionIdSet &getEmptyIdSet();

private:
    // Protects map structures as well as usage of Extenders themselves
    thread::RWMutex mutex;
    typedef std::map<unsigned, std::pair<std::string, CoTDetailExtender *>> IdMap;
    typedef std::map<std::string, std::pair<unsigned, CoTDetailExtender *>> NameMap;
    IdMap idToName;
    NameMap nameToId;

    static ExtensionIdSet nullIdSet;
    static std::set<std::string> reservedElements;

    COMMO_DISALLOW_COPY(ExtensionRegistry);
};
...
public:
    ...
    static void logvprintf(CommoLogger* logger, CommoLogger::Level level, CommoLogger::Type type, void* detail, const char* format, va_list vl);
    ...
    static uint64_t uint64FromString(const char *s) COMMO_THROW (std::invalid_argument);

    // throws if ALL of s is not a string version of a single int,
    // if the string value is too big for int, or if s is empty string
    static unsigned int uintFromString(const char *s) COMMO_THROW (std::invalid_argument);

    // throws if ALL of s is not a string version of a single int,
    // if the string value is too big for int,
    // the converted int is less than min or greater than max,
    // or if s is empty string
    static uint64_t uint64FromString(const char *s, uint64_t min, uint64_t max) COMMO_THROW (std::invalid_argument);
    ...
    // Reading privKey and ca is optional; pass NULL if not needed.
    ...
    // our implementations of open/close sockets for curl uses
    static curl_socket_t curlOpenSock(void *opaque, curlsocktype purpose,
                                      struct curl_sockaddr *address);
    static int curlCloseSock(void *opaque, curl_socket_t sock);
