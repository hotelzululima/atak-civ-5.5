...
protected:
    enum { RX_THREADID, TX_THREADID, TX_UPLOAD_THREADID, SUSPENSION_THREADID,
           EVENT_THREADID };
...
private:
    ...
    struct WebserverFileContext{
        WebserverFileContext(MissionPackageManager *owner, FileHandle *handle,
                             std::shared_ptr<FileIOProvider> provider);
        WebserverFileContext(MissionPackageManager *owner,
                             MHD_Connection *connection,
                             QuicManagement::FileTransferBuffer *qTransferBuf);
        ~WebserverFileContext();

        MissionPackageManager *owner;

        // this group is quic transfers only
        MHD_Connection *connection;
        QuicManagement::FileTransferBuffer *qTransferBuf;
        
        // this group is file transfers only
        FileHandle* handle;
        std::shared_ptr<FileIOProvider> provider;
    private:
        COMMO_DISALLOW_COPY(WebserverFileContext);
    };
    ...
    // Set of active quic transfers' file contexts that are suspended due to
    // IO blocking.
    // Protected by quicMutex
    std::set<WebserverFileContext *> suspendedTransfers;
    // Shared mutex/cond with quic manager for shared I/O
    thread::Mutex *quicTransferMutex;
    thread::CondVar *quicTransferCond;

    // Mapping of txtransfer id to filename which is shared
    // with other components.
    // Shared to us. Added once transfer is active and removed
    ...
    void suspensionThreadProcess();
    void eventThreadProcess();
    
    void processSuspendedTransfers();
    ...
    int webThreadAccessHandlerCallback(
                                              ...
                                              size_t *upload_data_size,
                                              void **con_cls);
    static void webThreadReaderFree(void *opaque);
    static ssize_t webThreadReaderCallback(void *opaque, uint64_t pos,
                                           char *buf, size_t max);
