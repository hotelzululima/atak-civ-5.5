...
MissionPackageManager::MissionPackageManager(CommoLogger *logger,
        ...
        QuicManagement *quicMgr, HWIFScanner *hwIfScanner,
        ...
        COMMO_THROW (std::invalid_argument) :
                ThreadedHandler(5, THREAD_NAMES), DatagramListener(),
                PeerMessageListener(),
                ...
                txTransfers(), txTransfersMutex(), 
                suspendedTransfers(),
                quicTransferMutex(NULL),
                quicTransferCond(NULL),
                ...
                providerTracker(factory)
{
    ...
    quicMgr->getFileTransferSync(&quicTransferMutex, &quicTransferCond);    
...
void MissionPackageManager::peerMessageReceived(const NetAddress *sender,
                     ...
                     const char *relToAttrib)
{
    ...
    atakmap::commoncommo::CommoResult ret =
            sendFileInit(xferId, &vdest, filePath, fileName, name,
                         accessAttrib, caveatsAttrib, relToAttrib);
                     ...
                     const char *relToAttrib)
{
    ...
    if (!InternalUtils::computeSha256Hash(&hash, filePath, provider)) {
        InternalUtils::logprintf(logger, CommoLogger::LEVEL_ERROR, "MP Send init failed to read source file (%s) and compute hash", filePath);
    ...
    try {
        ...
        InternalUtils::logprintf(logger, CommoLogger::LEVEL_ERROR, "MP Send init failed get size of source file (%s)", filePath);
        return COMMO_ILLEGAL_ARGUMENT;
    }
    ...
    // Sort destinations by gone, tak servers, and local
    {
        ...
        if (!local.empty() || !streamToContacts.empty()) {
            TxTransferContext *txCtx = new TxTransferContext(this, 
                    ...
                    fileSize,
                    hash,
                    accessAttrib == NULL ? "" : accessAttrib,
                    caveatsAttrib == NULL ? "" : caveatsAttrib,
                    relToAttrib == NULL ? "" : relToAttrib);
            *xferId = txCtx->id;
                       ...
                       int xferId)
{
    ...
    if (!txCtx->uploadCtxs.empty() || !txCtx->outstandingAcks.empty()) {
        ...
        {
            Lock sharedTransferLock(*mpTransferFilesMutex);
            (*mpTransferFiles)[txCtx->id] = txCtx->fileToSend;
        }
    } else
                    ...
                    bool peerHosted)
{
    ...
    callsignMutex.lock();
    CoTFileTransferRequest filexfer(ctx->access,
                       ctx->caveats,
                       ctx->relTo,
                       ctx->sha256hash,
                       ctx->transferName,
        ...
        size_t threadNum)
{
    ...
    case SUSPENSION_THREADID:
    {
        Lock lock(*quicTransferMutex);
        quicTransferCond->broadcast(lock);
        break;
    }
    case EVENT_THREADID:
    {
            ...
            if (cmsg && cmsg->msg == CURLMSG_DONE) {
                ...
                if (upCtx->state == TxUploadContext::TOOLSET) {
                    ...
                    if (upCtx->state == TxUploadContext::UPLOAD) {
                        curl_mime_free(upCtx->uploadMultipart);
                        upCtx->uploadMultipart = NULL;
                        if (upCtx->localFile) {
...
#define CURL_CHECK(a) if ((a) != 0) throw std::invalid_argument("Failed to set curl option")
    ...
    try {
        ...
        case TxUploadContext::UPLOAD:
          {
            ...
            upCtx->uploadMultipart = curl_mime_init(upCtx->curlCtx);
            if (!upCtx->uploadMultipart)
                throw std::invalid_argument("failed to initialize upload multipart");

            curl_mimepart *uploadData = curl_mime_addpart(upCtx->uploadMultipart);
            if (!uploadData) {
                throw std::invalid_argument("failed to initialize upload data");
            }

            CURL_CHECK(curl_mime_data_cb(uploadData,
                    upCtx->owner->fileSize,
                    curlReadCallback,
                    NULL,
                    NULL,
                    upCtx));
            CURL_CHECK(curl_mime_name(uploadData, "assetfile"));
            CURL_CHECK(curl_mime_filename(uploadData, upCtx->owner->filename.c_str()));
            CURL_CHECK(curl_mime_type(uploadData, "application/x-zip-compressed"));
            ...
            CURL_CHECK(curl_easy_setopt(upCtx->curlCtx, CURLOPT_MIMEPOST, upCtx->uploadMultipart));
        ...
        CURL_CHECK(curl_easy_setopt(upCtx->curlCtx, CURLOPT_OPENSOCKETFUNCTION, InternalUtils::curlOpenSock));
        CURL_CHECK(curl_easy_setopt(upCtx->curlCtx, CURLOPT_CLOSESOCKETFUNCTION, InternalUtils::curlCloseSock));
...
void MissionPackageManager::receiveThreadProcess()
{
    ...
    while (!threadShouldStop(RX_THREADID)) {
        ...
        for (iter = rxTransfers.begin(); iter != rxTransfers.end(); ++iter) {
            ...
            int epPort;
            std::string epip = contactMgr->getActiveEndpointHost(
                                     ftc->request->senderuid, &epType, &epPort);
            ...
            bool logUrlAdjusted = false;
            if (webPort != MP_LOCAL_PORT_DISABLE && 
                    !ftc->senderURLIsTAKServer && !epip.empty() &&
                    epType == CoTEndpoint::QUIC) {
                // rewrite to our local proxy for quic transport
                InternalUtils::logprintf(logger, CommoLogger::LEVEL_DEBUG, 
                            "MP sender %s has active QUIC endpoint, attempting rewrite of URL to utilize quic proxy",
                             ftc->request->senderCallsign.c_str());
                bool ok = true;
                bool https = false;
                if (url.find("https://") == 0)
                ...
                size_t n = url.find("/getfile");
                if (ok && n != std::string::npos) {
                    url.replace(n, 8, "/qprox");
                    if (https)
                        url.erase(4, 1);
                    ...
                    InternalUtils::urlAppendParam(ftc->curlCtx, &url,
                                "port",
                                InternalUtils::intToString(epPort).c_str());
                    InternalUtils::urlReplaceHost(&url, "127.0.0.1");
                    InternalUtils::urlReplacePort(&url, webPort);
                    logUrlAdjusted = true;
                }
            }

            logUrlAdjusted |= ftc->request->peerHosted && 
                              ftc->request->httpsPort != MP_LOCAL_PORT_DISABLE;
            InternalUtils::logprintf(logger, CommoLogger::LEVEL_DEBUG, 
                            "Initiating download of %s from %s at %s URL %s%s%s%s%s%s "
                            "(attempt %d of %d)", ftc->request->name.c_str(),
                            ftc->request->senderCallsign.c_str(),
                            ftc->usingSenderURL ? "sender" : "alternate",
                            url.c_str(),
                            ftc->request->peerHosted || logUrlAdjusted ? 
                                " (" : "",
                            ftc->request->peerHosted ? "peer hosted" : "",
                            logUrlAdjusted ? 
                                " url adjusted, orig URL " : 
                                " url unchanged",
                            logUrlAdjusted ?
            ...
            curl_easy_setopt(ftc->curlCtx, CURLOPT_OPENSOCKETFUNCTION, InternalUtils::curlOpenSock);
            curl_easy_setopt(ftc->curlCtx, CURLOPT_CLOSESOCKETFUNCTION, InternalUtils::curlCloseSock);
            ftc->curlErrBuf[0] = 0;
                                          ...
                                          void **connectionContext)
{
    ...
    try {
        ...
        static const std::string quicSpecifier("/qprox");
        ...
        if (urlStr == infoSpecifier) {
            ...
            bool qprox = false;
            if (urlStr == quicSpecifier)
                qprox = true;
            else if (urlStr != fileSpecifier)
            ...
            uint64_t size = -1;
            if (qprox) {
                const char *qhost = MHD_lookup_connection_value(connection,
                    MHD_GET_ARGUMENT_KIND, "host");
                if (!qhost)
                    throw std::invalid_argument("No host specified for qprox");
                const char *qport = MHD_lookup_connection_value(connection,
                    MHD_GET_ARGUMENT_KIND, "port");
                if (!qport)
                    throw std::invalid_argument("No host specified for qprox");
                int port = InternalUtils::intFromString(qport);
                InternalUtils::logprintf(logger, CommoLogger::LEVEL_DEBUG,
                        "MP webserver: quicproxy connection for File ID %s "
                        "on %s:%s", file, qhost, qport);
                
                QuicManagement::FileTransferBuffer *qftbuf =
                    quicMgr->initFileTransfer(qhost, port, file);
                if (!qftbuf)
                    throw std::invalid_argument("Could not create qprox transfer buffer");

                context = new WebserverFileContext(this, connection, qftbuf);
            } else {
                ...
                auto provider(providerTracker->getCurrentProvider());
                size = provider->getSize(fileString.c_str());
                FileHandle *f = providerTracker->getCurrentProvider()->
                                          open(fileString.c_str(), "rb");
                if (!f) {
                    InternalUtils::logprintf(logger, CommoLogger::LEVEL_DEBUG,
                ...
                context = new WebserverFileContext(this, f, providerTracker->getCurrentProvider());
            }
            

            response = MHD_create_response_from_callback(size, 4096,
                    webThreadReaderCallback, context, webThreadReaderFree);
        }

        if (!response) {
            if (context)
                // Clean up context if response creation failed
                delete context;
...
void MissionPackageManager::webThreadReaderFree(void *opaque) {
    auto context ((WebserverFileContext *)opaque);
    delete context;
}

ssize_t MissionPackageManager::webThreadReaderCallback(void *opaque, uint64_t pos, char *buf, size_t max)
{
    auto context ((WebserverFileContext *)opaque);
    ssize_t ret;
    if (context->handle) {
        ...
        size_t n = provider->read(buf, 1, max, context->handle);
        if (n == 0) {
            if (provider->eof(context->handle)) {
        ...
        Lock lock(*context->owner->quicTransferMutex);
        size_t n = context->qTransferBuf->read((uint8_t *)buf, max);
        if (n == 0) {
            if (context->qTransferBuf->isAtEOF())
                ret = MHD_CONTENT_READER_END_OF_STREAM;
            else if (context->qTransferBuf->isWriteErrored())
                ret = MHD_CONTENT_READER_END_WITH_ERROR;
            else if (context->owner->terminal) {
                // bail out; we're going to shut down and we don't want
                // to suspend this connection
                ret = MHD_CONTENT_READER_END_WITH_ERROR;
                InternalUtils::logprintf(context->owner->logger,
                    CommoLogger::LEVEL_DEBUG,
                    "MP webserver: terminating so failing transfer");
            } else {
                ret = 0;
                context->owner->suspendedTransfers.insert(context);
                MHD_suspend_connection(context->connection);
            }
        } else {
            ret = n;
        }
...
void MissionPackageManager::stopWebserver()
{
    // MHD requires resumption of any suspended transfers BEFORE
    // stopping.
    {
        Lock lock(*quicTransferMutex);
        terminal = true;
        processSuspendedTransfers();
    }
...
/***************************************************************************/
// Suspension handler thread and related methods


void MissionPackageManager::suspensionThreadProcess()
{
    while (!threadShouldStop(SUSPENSION_THREADID)) {
        Lock lock(*quicTransferMutex);
        processSuspendedTransfers();
        quicTransferCond->wait(lock);
    }
}

// Assumes holding of quicTransferMutex
void MissionPackageManager::processSuspendedTransfers()
{
    std::set<WebserverFileContext *>::iterator iter = suspendedTransfers.begin();
    while (iter != suspendedTransfers.end()) {
        std::set<WebserverFileContext *>::iterator curIter = iter;
        iter++;
        
        WebserverFileContext *ctx = *curIter;
        if (terminal || ctx->qTransferBuf->isReadable()) {
            MHD_resume_connection(ctx->connection);
            suspendedTransfers.erase(curIter);
        }
    }
}
...
MissionPackageManager::WebserverFileContext::WebserverFileContext(
                MissionPackageManager *owner, 
                FileHandle *handle, std::shared_ptr<FileIOProvider> provider) :
                owner(owner),
                connection(NULL),
                qTransferBuf(NULL),
                handle(handle),
...
MissionPackageManager::WebserverFileContext::WebserverFileContext(
                MissionPackageManager *owner, MHD_Connection *connection,
                QuicManagement::FileTransferBuffer *qTransferBuf) :
                owner(owner),
                connection(connection),
                qTransferBuf(qTransferBuf),
                handle(NULL),
                provider(NULL)
{
}
