...
public:
    ...
    // Throws if the socket can't be created, or if connecting or binding fails.
    UdpSocket(const NetAddress *localAddr, bool isBlocking,
              bool reuseAddr, bool mcastLooping) COMMO_THROW (SocketException);

    // Creates local UDP socket connected to the given NetAddress.
    // The new socket will be configured
    // for blocking or non-blocking I/O depending on the isBlocking
    // argument. 
    // Throws if the socket can't be created, or if connecting or binding fails.
    UdpSocket(const NetAddress *remoteAddr, bool isBlocking) COMMO_THROW (SocketException);
    ~UdpSocket();
    ...
    // same as first form of recvfrom, but also obtains the local address
    // on which the message was received (if available) when localAddr is
    // given as non-NULL.
    // If localAddr is non-NULL, and the local address on which the message
    // was received is able to be determined, localAddr will be set to
    // a newly allocated NetAddress which the caller will own.  If the local
    // address is not available, *localAddr will be set to NULL.
    void recvfrom(NetAddress **localAddr, NetAddress **source, uint8_t *data, size_t *len) COMMO_THROW (SocketException);
...
private:
    ...
    void commonInit(bool forConnect, const NetAddress *addr,
