                                  ...
                                  extensionIds(NULL)
{
    if (includeVersion) {
        ...
        extensionIds = new ExtensionIdSet();
        extensionRegistry->getAllIds(*extensionIds);
    }
}
                                  ...
                                  extensionIds(NULL)
{
    ...
    if (tryXml)
        ...
        initFromProtobuf(data, len, extensions);
    else
...
size_t TakMessage::serializeAsProtobuf(int protoVersion,
                                       ...
                                       const ExtensionIdSet &remoteExtensionIds) const 
                                 COMMO_THROW (std::invalid_argument)
{
    ...
    if (useProtoInf) {
        ...
        for (ExtensionIdSet::iterator iter = extensionIds->begin(); iter != extensionIds->end(); iter++)
            c->add_extensionids(*iter);
    }
    
    if (useCot) {
        protobuf::v1::CotEvent *ev = takm.mutable_cotevent();
        cot->serializeAsProtobuf(ev, extensions, useAnyExtension, remoteExtensionIds);
    }
...
const ExtensionIdSet *TakMessage::getExtensions() const
{
    return extensionIds;
}
                        ...
                        COMMO_THROW (std::invalid_argument)
{
    ...
    if (takm.has_cotevent()) {
        ...
        ownedCot = new CoTMessage(logger, ev, extensions);
        cot = ownedCot;
    }
    ...
    if (takm.has_takcontrol()) {
        ...
        if (min == 0) min = 1;
        if (max == 0) max = 1;
        const int numExts = tc.extensionids_size();
        try {
        ...
        for (int i = 0; i < numExts; i++)
            extensionIds->insert(tc.extensionids(i));
