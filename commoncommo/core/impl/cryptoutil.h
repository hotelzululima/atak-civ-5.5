...
public:
    ...
    char *generateCSRCryptoString(const char **entryKeys,
                                  ...
                                  const char* pem,
                                  const char* password);
    bool generateCSR(std::string &csr,
    ...
    // generates binary p12
    // Always uses c++ array alloc, not cryptostring alloc
    size_t generateKeystore(uint8_t **p12, X509 *cert, EVP_PKEY *pkey, 
                              STACK_OF(X509) *caStack, 
                              const char *password, const char *friendlyName);
...
private:
    ...
    // if asCryptoString true, generates base64 cryptostring allocated
    // with cryptostring allocator and return is 0 (fail) or 1 (success)
    // if false, generates binary p12 with c++ array allocator and return is
    // 0 (fail) or length of data
    size_t generateKeystoreImpl(uint8_t **p12, X509 *cert, EVP_PKEY *pkey, 
                              STACK_OF(X509) *caStack, 
                              const char *password, const char *friendlyName,
                              bool asCryptoString);
