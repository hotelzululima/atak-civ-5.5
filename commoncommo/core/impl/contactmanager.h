...
class PeerCoTEndpoint : public CoTEndpoint
{
public:
    PeerCoTEndpoint(InterfaceType type, const std::string &ipString, int port) COMMO_THROW (std::invalid_argument);
    virtual ~PeerCoTEndpoint();
...
private:
    COMMO_DISALLOW_COPY(PeerCoTEndpoint);
...
struct ContactState
{
    ...
    PeerCoTEndpoint *tcpEndpoint;
    PeerCoTEndpoint *quicEndpoint;
    std::string callsign;
...
public:
    ...
    // Enable or disable the destination uid insertion on outbound messages
    void setDestUidInsertionEnabled(bool destUidInsertionEnabled);
    ...
    // If epType is non-NULL, populate with type of the active endpoint
    // (value of which only means something if non-empty string returned)
    // If port non-NULL, it is populated with the remote port for the active
    // endpoint or -1 if not known
    std::string getActiveEndpointHost(const ContactUID *contact,
                                      CoTEndpoint::InterfaceType *epType = NULL,
                                      int *port = NULL);
    ...
    CommoResult sendCoT(std::vector<const ContactUID *> *destinations, CoTMessage *cotMessage, CoTSendMethod sendMethod = SEND_ANY);
    CommoResult sendCoT(ContactList *destinations, CoTMessage *cotMessage, CoTSendMethod sendMethod = SEND_ANY);
...
private:
    ...
    int protoVersion;
    ExtensionIdSet protoExtensions;
    bool protoVDirty;
    
    bool preferStreaming;
    bool destUidInsertionEnabled;
    ...
    void protoVThreadFireExtSetChange(const ExtensionIdSet &extIds);
    ...
    bool updateStateDatagramEndpoint(ContactState *contact,
            ...
            int port);
    bool updateStatePeerEndpoint(ContactState *contact,
            PeerCoTEndpoint **stateEndpoint,
            CoTEndpoint::InterfaceType type,
            const std::string &endpoint, int port);
    bool updateStateQuicEndpoint(ContactState *contact,
            const std::string &endpoint, int port);
    ...
    // If protoInfOnly, then updates only protocol and extension info;
    //    impl must ignore all args EXCEPT protoInf, extIds, 
    //    knownEP, type, and uid
    ...
    CommoResult processContactUpdate(
            ...
            const std::string *streamSource,  // See processContactStateUpdate
            bool knownEP,
    ...
    bool processContactStateUpdate(ContactState *state,
            ...
            unsigned int msgVer,
            const std::string &callsign,
            const std::string *streamSource, // non-NULL means update from
                                             // identified stream source.
                                             // remainder of args should be
                                             // ignored and may be NULL
            bool knownEP,                    // if true, remainder of args
                                             // except endpoints ignored
                                             // and all endpoints *must*
                                             // have one and only one
                                             // endpoint. that single endpoint
                                             // MUST be UDP type
            const NetAddress *sender,        // source of message
            const TakProtoInfo *protoInf,
            const ExtensionIdSet *extIds,
            CoTMessage::EndpointList *endpoints);
    bool processContactStateEndpointsUpdate(ContactState *state,
            const NetAddress *sender,
            CoTMessage::EndpointList *endpoints);
    void msgRxImpl(unsigned int msgVer,
