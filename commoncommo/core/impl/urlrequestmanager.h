...
struct URLRequest
{
    ...
    URLRequest(URLRequestType type, const std::string &url,
               ...
               bool verifyHost = false) COMMO_THROW (CommoResult);
    ...
    // Repeatedly invoked to get data for upload when when type is
    // POST_BUFFER_UPLOAD. Fill up to len bytes at data, returning number
    // of bytes actually written.  Return 0 to signal end of data.
    virtual size_t dataForUpload(uint8_t *data, size_t len) { return 0; };
    // Invoked for POST_BUFFER_UPLOAD only - implementations should return
    // total expected number of bytes in the upload, or 0 if not known.
    virtual uint64_t getUploadLen() { return 0; }
};
