        ...
        CommoLogger::Level level, const char* format, ...)
{
    ...
    logvprintf(logger, level, CommoLogger::Type::TYPE_GENERAL, NULL,
                  format, args);
        ...
        void* detail, const char* format, ...)
{
    ...
    logvprintf(logger, level, type, detail, format, args);

    va_end(args);
}

void InternalUtils::logvprintf(CommoLogger *logger,
        CommoLogger::Level level, CommoLogger::Type type,
...
uint64_t InternalUtils::uint64FromString(const char *s) COMMO_THROW (std::invalid_argument)
{
    return (uint64_t)uint64FromString(s, 0, UINT64_MAX);
}

unsigned int InternalUtils::uintFromString(const char *s) COMMO_THROW (std::invalid_argument)
{
    return (unsigned int)uint64FromString(s, 0, UINT_MAX);
}

uint64_t InternalUtils::uint64FromString(const char *s, uint64_t min, uint64_t max) COMMO_THROW (std::invalid_argument)
{
    char *end;
    unsigned long long l = strtoull(s, &end, 10);
    if (l > max || l < min || *end != '\0' || *s == '\0')
         ...
         STACK_OF(X509) **pcaCerts, int *pnCaCerts) COMMO_THROW (SSLArgException)
{
    ...
    int ret = PKCS12_parse(pkcs12, certPassword, pprivKey ? &privKey : NULL,
                           &cert,
...
curl_socket_t InternalUtils::
curlOpenSock(void *opaque, curlsocktype purpose, struct curl_sockaddr *addr)
{
    PlatformNet::SocketFD sockRet = PlatformNet::createSocket(addr->family, addr->socktype, addr->protocol);
    if (sockRet == PlatformNet::BAD_SOCKET)
        return CURL_SOCKET_BAD;
    return sockRet;
}

int InternalUtils::curlCloseSock(void *opaque, curl_socket_t sock)
{
    PlatformNet::closeSocket(sock);
    return CURLE_OK;
}
...
ExtensionIdSet ExtensionRegistry::nullIdSet;
std::set<std::string> ExtensionRegistry::reservedElements;

ExtensionRegistry::ExtensionRegistry() : mutex(thread::RWMutex::Policy_Fair), 
                                         idToName(),
                                         nameToId()
{
    static bool reservedInitialized = false;
    if (!reservedInitialized) {
        reservedElements.insert("contact");
        reservedElements.insert("__group");
        reservedElements.insert("precisionlocation");
        reservedElements.insert("status");
        reservedElements.insert("takv");
        reservedElements.insert("track");
...
{
    thread::WriteLock wlock(mutex);
    idToName.clear();
    nameToId.clear();
}

bool ExtensionRegistry::addExtension(unsigned id, 
                                     const char *name, 
                                     CoTDetailExtender *ext)
{
    thread::WriteLock wlock(mutex);
    if (reservedElements.find(name) != reservedElements.end())
        return false;
    std::pair<IdMap::iterator, bool> idRet = idToName.insert(IdMap::value_type(id, IdMap::mapped_type(std::string(name), ext)));
    if (!idRet.second)
        return false;
    std::pair<NameMap::iterator, bool> nameRet = nameToId.insert(NameMap::value_type(name, NameMap::mapped_type(id, ext)));
    if (!nameRet.second) {
        idToName.erase(idRet.first);
        return false;
    }
    return true;
}

bool ExtensionRegistry::removeExtension(unsigned id)
{
    thread::WriteLock wlock(mutex);
    IdMap::iterator idRet = idToName.find(id);
    if (idRet == idToName.end())
        return false;
    nameToId.erase(idRet->second.first);
    idToName.erase(idRet);
    return true;
}

bool ExtensionRegistry::getByName(thread::ReadLockPtr &lock,
                                  unsigned *id,
                                  CoTDetailExtender **ext,
                                  const char *name)
{
    if (!lock)
        thread::ReadLock::create(lock, mutex);
    NameMap::iterator iter = nameToId.find(name);
    if (iter == nameToId.end())
        return false;
    *id = iter->second.first;
    *ext = iter->second.second;
    return true;
}

bool ExtensionRegistry::getById(thread::ReadLockPtr &lock, 
                                std::string &name,
                                CoTDetailExtender **ext,
                                unsigned id)
{
    if (!lock)
        thread::ReadLock::create(lock, mutex);
    IdMap::iterator iter = idToName.find(id);
    if (iter == idToName.end())
        return false;
...
void ExtensionRegistry::getAllIds(ExtensionIdSet &ret)
{
    thread::ReadLock lock(mutex);
    ret.clear();
    for (IdMap::const_iterator i = idToName.begin(); i != idToName.end(); i++)
        ret.insert(i->first);
}

const ExtensionIdSet &ExtensionRegistry::getEmptyIdSet()
{
    return nullIdSet;
}
