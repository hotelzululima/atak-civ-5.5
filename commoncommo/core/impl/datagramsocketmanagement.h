...
{
public:
    DatagramSocketManagement(CommoLogger *logger, ContactUID *ourUid, HWIFScanner *scanner, ExtensionRegistry *extensions);
    ...
    // Change of broadcast-level protocol support
    void protoLevelChange(int newVersion);
    // Change of extensions that are able to be used for broadcasting
    // This is the set commonly supported by known broadcast-reachable clients.
    // Actual extensions used may be less than this due to own local support
    // (or lack thereof)
    void protoExtSetChange(const ExtensionIdSet &extIds);

    // Force proto support announcement asap
    void forceProtoBroadcast();
    ...
    void sendDatagram(const NetAddress *dest, const CoTMessage *msg,
                      int protoVersion, const ExtensionIdSet &enabledExtensions)
    ...
    void setHasQuicInterface(bool en);
...
private:
    ...
    struct TxQueueItem
    {
        ...
        // a bcast message
        int protoVersion;
        ExtensionIdSet enabledExtensions;
    ...
    bool magtabEnabled;
    int protoVersion;
    ExtensionIdSet bcastExtensions;
