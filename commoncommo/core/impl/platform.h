...
struct PlatformNet {
    ...
    static void staticInit(CommoLogger *logger);
    ...
    static void setSocketEnablePktInfo(SocketFD fd);
    ...
    // srcAddr(Len) is required and populated on success return
    // localAddr(Len) optional; if non-NULL and success return, it *MAY* be
    // filled in with address and family of local address packet was sent to,
    // in which case localAddrValid is set to true.
    // Obtaining localAddr may not be possible, in
    // which case it is not changed and *localAddrValid is set to false.
    // On error, localAddrValid is not set at all as it has no meaning.
    // When localAddr is populated/valid, the caller is left to populate the
    // port portion of the address as this cannot be determined herein.
