...
namespace {
    ...
    struct DestInfo
    {
        ...
        std::vector<EndpointType> altEpTypes;
        std::vector<NetAddress *> dests;
        
        DestInfo(EndpointType epType,
                 bool useQuicAlts) :
            srcIpString(),
            srcEpType(epType),
            altEpTypes(),
        ...
        DestInfo() : srcIpString(), srcEpType(ENDPOINT_TCP),
                     altEpTypes(), dests()
        {
        }
        void setEndpointType(EndpointType epType,
                 bool useQuicAlts)
        {
            srcEpType = epType;
            if (useQuicAlts)
                   ...
                   bool useQuicAlts)
        {
            ...
            altEpTypes.clear();
            dests.clear();
            setEndpointType(epType, useQuicAlts);
        }
    private:
        void addQuicAlts()
        {
            ...
            case ENDPOINT_UDP:
            case ENDPOINT_TCP:
                altEpTypes.push_back(ENDPOINT_QUIC);
                break;
            case ENDPOINT_UDP_USESRC:
            case ENDPOINT_TCP_USESRC:
                altEpTypes.push_back(ENDPOINT_QUIC_USESRC);
                break;
            default:
                // none to add
                break;
...
void DatagramSocketManagement::protoExtSetChange(const ExtensionIdSet &extIds)
{
    thread::Lock txLock(txQueueMutex);
    bcastExtensions = extIds;
}


void DatagramSocketManagement::forceProtoBroadcast()
{
    thread::Lock txLock(txQueueMutex);
    nextBroadcastTime = CommoTime::ZERO_TIME;
    txQueueMonitor.broadcast(txLock);
...
void DatagramSocketManagement::setHasQuicInterface(bool en)
{
    hasQuic = en;
}
...
void DatagramSocketManagement::sendDatagram(
        ...
        int protoVersion,
        const ExtensionIdSet &enabledExtensions) COMMO_THROW (std::invalid_argument)
{
    thread::Lock qlock(txQueueMutex);
    txQueue.push_front(TxQueueItem(dest, msg, protoVersion, enabledExtensions));
            ...
            if (rxNeedsRebuild) {
                ...
                for (iter = socketContexts.begin(); iter != socketContexts.end(); ++iter) {
                    ...
                    if (!socketContext->socket) {
                        ...
                        try {
                            socketContext->socket = new UdpSocket(bindAddr, false, reuseAddress, false);
                            delete bindAddr;
    ...
    while (!threadShouldStop(RX_QUEUE_THREADID)) {
        ...
        if (qItem.generic) {
            ...
            try {
                ...
                TakMessage msg(logger, data, dataLen, extensions, true, true);
                thread::Lock listenerLock(listenerMutex);
                std::set<DatagramListener *>::iterator iter;
                for (iter = listeners.begin(); iter != listeners.end(); ++iter) {
                    DatagramListener *l = *iter;
        ...
        }
        {
            ...
            CoTMessageType cotType;
            {
                thread::ReadLock ifaceLock(interfaceMutex);
                {
                    ...
                    localBcastExts = bcastExtensions;
                }
                    ...
                    if (!fixedDest) {
                        ...
                        if (!curDestInfo.dests.empty()) {
                            if (checkTXSocket(NULL, &curDestInfo.srcIpString)) {
                                ...
                                curDestInfo.setEndpointType(epAsUdp ? 
                                                        ENDPOINT_UDP_USESRC :
                                                        ENDPOINT_TCP_USESRC,
                                                      hasQuic);
                                destPairs[NULL] = curDestInfo;
            ...
            TakMessage takmsg(logger, qitem.cotmsg, ourUid, extensions, true);
            const int sendVersion = qitem.protoVersion < 0 ? 
                            localProtoVersion : 
                            qitem.protoVersion;
            const ExtensionIdSet &sendExtIds = qitem.protoVersion < 0 ?
                            localBcastExts : qitem.enabledExtensions;
            ...
            for (iter = destPairs.begin(); iter != destPairs.end(); iter++) {
                ...
                if (qitem.cotmsg)
                    qitem.cotmsg->setEndpoints(iter->second.srcEpType,
                                               iter->second.srcIpString,
                                               &iter->second.altEpTypes);
