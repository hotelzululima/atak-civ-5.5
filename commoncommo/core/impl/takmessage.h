...
// Captures min/max tak protocol versions and any extensions supported
...
public:
    ...
    size_t serializeAsProtobuf(int protoVersion, uint8_t **buf, 
                               ...
                               const ExtensionIdSet &enabledExtensionIds) 
                               const COMMO_THROW (std::invalid_argument);
    
    // NULL if not present in message
    const TakProtoInfo *getProtoInfo() const;
    
    // NULL if not present in message; guaranteed (non-)NULL if getProtoInfo()
    // (non-)NULL
    const ExtensionIdSet *getExtensions() const;
...
private:
    ...
    // NULL state here follows protoInfo
    ExtensionIdSet *extensionIds;
};
