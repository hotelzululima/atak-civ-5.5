...
public:
    ...
    virtual void peerMessageReceived(const NetAddress *sender, 
                                    const std::string *endpointId,
...
struct CommoImpl
{
    CommoImpl(CommoLogger *logger, const ContactUID *ourUID,
            const char *ourCallsign, netinterfaceenums::NetInterfaceAddressMode addrMode) :
                ...
                extensions(NULL),
                scanner(NULL),
                cotSendFailListeners(),
                cotSendFailListenersMutex(),
                dgMgmt(NULL), quicMgmt(NULL), tcpMgmt(NULL), streamMgmt(NULL),
                ...
                mpTransferFilesMutex()
    {
        ...
        dgMgmt = new DatagramSocketManagement(logger, this->ourUID, scanner, extensions);
        quicMgmt = new QuicManagement(logger, this->ourUID, extensions,
                                      this->providerTracker,
                                      &cotSendFailListeners,
                                      &cotSendFailListenersMutex,
                                      &mpTransferFiles,
                                      &mpTransferFilesMutex);
        tcpMgmt = new TcpSocketManagement(logger, this->ourUID, extensions,
                                          &cotSendFailListeners,
                                          &cotSendFailListenersMutex);
        streamMgmt = new StreamingSocketManagement(logger, extensions,
                std::string((const char *)ourUID->contactUID,
                            ourUID->contactUIDLen));
        contactMgmt = new ContactManager(logger, dgMgmt, tcpMgmt, quicMgmt, streamMgmt);
        ...
        quicMgmt->addMessageReceiver(listenerMgmt);
        ...
        quicMgmt->addInterfaceStatusListener(listenerMgmt);
    ...
    ~CommoImpl()
    {
        ...
        if (missionPkgMgmt) {
            ...
            quicMgmt->removeMessageReceiver(missionPkgMgmt);
        }
        dgMgmt->removeInterfaceStatusListener(listenerMgmt);
        quicMgmt->removeInterfaceStatusListener(listenerMgmt);
        ...
        quicMgmt->removeMessageReceiver(listenerMgmt);
    ...
    void setupMPIO(MissionPackageIO *missionPackageIO) COMMO_THROW (std::invalid_argument) {
        ...
        quicMgmt->addMessageReceiver(missionPkgMgmt);
    ...
    void enableEnrollment(EnrollmentIO *enrollmentIO) COMMO_THROW (std::invalid_argument) {
        thread::Lock lock(mpMutex);
        if (enrollmentMgmt)
            throw std::invalid_argument("Already initialized");
        enrollmentMgmt = new EnrollmentManager(logger, urlMgmt, crypto, ourUID, enrollmentIO);
    }
    ...
    HWIFScanner *scanner;
    PeerMessagingBase::CoTSendFailureListenerSet cotSendFailListeners;
    thread::Mutex cotSendFailListenersMutex;
    ...
    EnrollmentManager *enrollmentMgmt;
        ...
        const char *ourCallsign, netinterfaceenums::NetInterfaceAddressMode addrMode) : impl(NULL)
{
    ...
    PlatformNet::staticInit(logger);
    CryptoUtil::staticInit(logger);
...
CommoResult Commo::enableEnrollment(EnrollmentIO *enrollIO)
{
    if (!enrollIO)
        return COMMO_ILLEGAL_ARGUMENT;

    try {
        impl->enableEnrollment(enrollIO);
    } catch (std::invalid_argument &e) {
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR, "Could not enable enrollment: %s", e.what());
        return COMMO_ILLEGAL_ARGUMENT;
    }
...
CommoResult Commo::registerCoTDetailExtender(unsigned int extensionId, const char *extensionDetailElement, CoTDetailExtender *extender)
{
    bool added = impl->extensions->addExtension(extensionId, extensionDetailElement, extender);
    if (added) {
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_INFO, "Added CoTDetailExtender for id %u and element name %s", extensionId, extensionDetailElement);
        impl->dgMgmt->forceProtoBroadcast();
        impl->streamMgmt->reconnectAll();
    }
    
    return added ? COMMO_SUCCESS : COMMO_ILLEGAL_ARGUMENT;
}

CommoResult Commo::deregisterCoTDetailExtender(unsigned int extensionId)
{
    bool removed = impl->extensions->removeExtension(extensionId);
    if (removed) {
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_INFO, "Removed CoTDetailExtender for id %u", extensionId);
        impl->dgMgmt->forceProtoBroadcast();
        impl->streamMgmt->reconnectAll();
    }

    return removed ? COMMO_SUCCESS : COMMO_ILLEGAL_ARGUMENT;
}
...
QuicInboundNetInterface *Commo::addQuicInboundInterface(int port, 
                                      const uint8_t *certData,
                                      size_t certLen, 
                                      const char *certPassword,
                                      CommoResult *result)
{
    QuicInboundNetInterface *ret = impl->quicMgmt->addInboundInterface(port,
                                               certData, certLen, 
                                               certPassword, result);
    if (ret)
        impl->dgMgmt->setHasQuicInterface(true);
    return ret;
}

CommoResult Commo::removeQuicInboundInterface(QuicInboundNetInterface *iface)
{
    CommoResult ret = impl->quicMgmt->removeInboundInterface(iface);
    if (ret == COMMO_SUCCESS)
        impl->dgMgmt->setHasQuicInterface(impl->quicMgmt->hasInboundInterface());
    return ret;
}
...
{
    impl->contactMgmt->setDestUidInsertionEnabled(insertDestUid);
    InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_INFO, "Dest UID insertion %s", insertDestUid ? "enabled" : "disabled");
}
        ...
        CommoResult *result)
{
    ...
    if (clientCert)
        transport = netinterfaceenums::TRANSPORT_SSL;
    return impl->streamMgmt->addStreamingInterface(transport,
            hostname, port, types,
...
CommoResult Commo::broadcastCoT(const char* cotMessage, CoTSendMethod sendMethod)
{
    try {
        ...
        if (len > maxUDPMessageSize) {
            InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                             CommoLogger::TYPE_GENERAL, nullptr,
                                             "broadcastCoT() message exceeds max length");
            return COMMO_ILLEGAL_ARGUMENT;
        }
        ...
        if (sendMethod & SEND_TAK_SERVER) {
            msg.setEndpoints(ENDPOINT_STREAMING, "", NULL);
        ...
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                 CommoLogger::TYPE_GENERAL, nullptr,
                                 "broadcastCoT() invalid argument: %s", ex.what() ? ex.what() : "");
        return COMMO_ILLEGAL_ARGUMENT;
    }
}
...
CommoResult Commo::sendCoT(ContactList *destinations, const char *cotMessage, CoTSendMethod sendMethod)
{
    try {
        ...
        if (len > maxUDPMessageSize) {
            InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                             CommoLogger::TYPE_GENERAL, nullptr,
                                             "sendCoT() message exceeds max length");
            return COMMO_ILLEGAL_ARGUMENT;
        }
        CoTMessage msg(impl->logger, (const uint8_t *)cotMessage,
                                     len);
        ...
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                         CommoLogger::TYPE_GENERAL, nullptr,
                                         "sendCoT() invalid argument: %s", ex.what() ? ex.what() : "");
        return COMMO_ILLEGAL_ARGUMENT;
    }
}
...
CommoResult Commo::sendCoTTcpDirect(const char *host, int port, const char *cotMessage)
{
    ...
    try {
        ...
        if (len > maxUDPMessageSize) {
            InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                             CommoLogger::TYPE_GENERAL, nullptr,
                                             "sendCoTTcpDirect() message exceeds max length");
            return COMMO_ILLEGAL_ARGUMENT;
        }
        CoTMessage msg(impl->logger,
                       (const uint8_t *)cotMessage, len);
        impl->tcpMgmt->sendMessage(hostStr, port, &msg, ExtensionRegistry::getEmptyIdSet(), 0);
        return COMMO_SUCCESS;
    } catch (std::invalid_argument &ex) {
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                         CommoLogger::TYPE_GENERAL, nullptr,
                                         "sendCoTTcpDirect() invalid argument: %s", ex.what() ? ex.what() : "");
        return COMMO_ILLEGAL_ARGUMENT;
    }
}
                                        ...
                                        const char *cotMessage)
{
    ...
    try {
        ...
        if (len > maxUDPMessageSize) {
            InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                             CommoLogger::TYPE_GENERAL, nullptr,
                                             "sendCoTServerControl() message exceeds max length");
            return COMMO_ILLEGAL_ARGUMENT;
        }
        CoTMessage msg(impl->logger,
                       (const uint8_t *)cotMessage, len);
        ...
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                         CommoLogger::TYPE_GENERAL, nullptr,
                                         "sendCoTServerControl() invalid argument: %s", ex.what() ? ex.what() : "");
        return COMMO_ILLEGAL_ARGUMENT;
    }
}
                                              ...
                                              const char *cotMessage)
{
    try {
        ...
        if (len > maxUDPMessageSize) {
            InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                             CommoLogger::TYPE_GENERAL, nullptr,
                                             "sendCoTToServerMissionDest() message exceeds max length");
            return COMMO_ILLEGAL_ARGUMENT;
        }
        CoTMessage msg(impl->logger,
                       (const uint8_t *)cotMessage, len);
        ...
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                         CommoLogger::TYPE_GENERAL, nullptr,
                                         "sendCoTToServerMissionDest() invalid argument: %s", ex.what() ? ex.what() : "");
        return COMMO_ILLEGAL_ARGUMENT;
    }
}
...
{
    thread::Lock lock(impl->cotSendFailListenersMutex);
    if (!impl->cotSendFailListeners.insert(listener).second)
        return COMMO_ILLEGAL_ARGUMENT;
    return COMMO_SUCCESS;
}
...
{
    thread::Lock lock(impl->cotSendFailListenersMutex);
    if (impl->cotSendFailListeners.erase(listener) != 1)
        return COMMO_ILLEGAL_ARGUMENT;
    return COMMO_SUCCESS;
}
                                  ...
                                  const char *clientVersionInfo)
{
    {
        thread::Lock lock(impl->mpMutex);
        if (!impl->enrollmentMgmt)
            return COMMO_ILLEGAL_ARGUMENT;
    }

    return impl->enrollmentMgmt->enrollmentInit(enrollmentId,
                                  host,
                                  port,
                                  verifyHost,
                                  user,
...
CommoResult Commo::enrollmentStart(int enrollmentId)
{
    ...
    return impl->enrollmentMgmt->enrollmentStart(enrollmentId);
}
...
CommoResult Commo::sendMissionPackageInit(int *xferId, ContactList *destinations,
                                      ...
                                      const char *name)
{
    return sendMissionPackageInit(xferId, destinations, filePath, fileName,
                                  name, NULL, NULL, NULL);
}
                                    ...
                                    const char *cotXml, int desiredVersion)
{
    try {
        ...
        TakMessage takmsg(impl->logger, &msg, NULL, impl->extensions, false);
        ExtensionIdSet unused;
        ...
        *dataLen = ret;
        ...
        InternalUtils::logprintf(impl->logger, CommoLogger::LEVEL_ERROR,
                                         CommoLogger::TYPE_GENERAL, nullptr,
                                         "cotXmlToTakproto() invalid argument: %s", ex.what() ? ex.what() : "");
        return COMMO_ILLEGAL_ARGUMENT;
    }
}
...
{
    try {
        TakMessage takmsg(impl->logger, (const uint8_t *)protodata, dataLen, 
                          impl->extensions, true, true);
