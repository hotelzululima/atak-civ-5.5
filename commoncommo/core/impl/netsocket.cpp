...
void UdpSocket::commonInit(bool forConnect, const NetAddress *addr,
                           bool isBlocking, bool reuseAddr)
                                   COMMO_THROW (SocketException)
{
    ...
    const struct sockaddr *saddr = addr->getSockAddr();
    ...
    if (forConnect) {
        if (PlatformNet::socketConnect(newfd, saddr,
                    addr->getSockAddrLen()) != PlatformNet::SUCCESS) {
        ...
        if (::bind(newfd, saddr, addr->getSockAddrLen()) != 0) {
    ...
    // Request recvmsg support info
    PlatformNet::setSocketEnablePktInfo(newfd);
...
{
    try {
        commonInit(false, localAddr, isBlocking, reuseAddr);

        if (PlatformNet::socketSetMcastLoop(fd, localAddr->family, mcastLooping) != PlatformNet::SUCCESS) {
            const char *msg = mcastLooping ? 
                  "mcast loop failed to set to on" : 
                  "mcast loop failed to set to off";
            throw SocketException(netinterfaceenums::ERR_OTHER, msg);
        }
...
UdpSocket::UdpSocket(const NetAddress *remoteAddr, bool isBlocking)
    COMMO_THROW (SocketException) :
        Socket(), boundAddr(NULL), currentTTL(1),
        outboundMcastIfSet(false)
{
    commonInit(true, remoteAddr, isBlocking, false);
}
...
{
    recvfrom(NULL, source, data, len);
}

void UdpSocket::recvfrom(NetAddress **localAddr, NetAddress **source,
        uint8_t *data, size_t *len) COMMO_THROW (SocketException)
{
    struct sockaddr_storage saddr;
    socklen_t saddr_len = sizeof(sockaddr_storage);
    struct sockaddr_storage laddr;
    socklen_t laddr_len = sizeof(sockaddr_storage);
    struct sockaddr *pladdr = localAddr ? (struct sockaddr *)&laddr : NULL;
    bool lValid = false;
    ...
    try {
        ...
        if (localAddr) {
            // we need to populate with port
#ifdef COMMO_USE_IPV6
# error fix me for ipv6
#endif
            if (lValid && pladdr->sa_family == AF_INET) {
                ((struct sockaddr_in *)pladdr)->sin_port = htons(boundAddr->getPort());
                *localAddr = NetAddress::create(pladdr);
            } else
                *localAddr = NULL;
