                                        ...
                                        const char *certPassword)
{
    ...
    // Check if provided cert is using legacy hashing algorithm
    // If so, reject
    int sigAlgoNID = X509_get_signature_nid(cert);
    if (sigAlgoNID == NID_sha1WithRSAEncryption) {
        InternalUtils::logprintf(logger, CommoLogger::LEVEL_ERROR, "https proxy - certificate uses legacy sha1 signature, rejecting");
        serverOk = false;
    }
        
    
    if (serverOk)
        serverOk = ioCtx->setServerParams(localPort, cert, privKey, caCerts);
        ...
        stopSignalled(false)
{
    ...
    if (!sslCtx) {
        ...
        InternalUtils::logprintf(logger, CommoLogger::LEVEL_ERROR,
            ...
            ebuf);
    } else if (!SSL_CTX_set_min_proto_version(sslCtx, TLS1_2_VERSION)) {
        InternalUtils::logprintf(logger, CommoLogger::LEVEL_ERROR,
            "Cannot configure SSL Context security settings! Https proxy will not be available");
        SSL_CTX_free(sslCtx);
        sslCtx = NULL;
    }
    try {
