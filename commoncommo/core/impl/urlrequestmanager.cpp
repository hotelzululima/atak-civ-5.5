                           ...
                           URLRequest *req)
{
    ...
    InternalUtils::logprintf(logger, CommoLogger::LEVEL_INFO,
        ...
        (req->type != URLRequest::BUFFER_DOWNLOAD && 
         req->type != URLRequest::POST_BUFFER_UPLOAD) ? 
             req->localFileName.c_str() : "mem");
...
size_t URLRequestManager::curlReadCallback(char *buf, size_t size, size_t nmemb, void *userCtx)
{
    IOContext *ioCtx = (IOContext *)userCtx;
    size_t n;
    if (ioCtx->request->type == URLRequest::FILE_UPLOAD) {
        n = ioCtx->provider->read(buf, size, nmemb, ioCtx->localFile);
        n *= size;
    } else
        n = ioCtx->request->dataForUpload((uint8_t *)buf, size * nmemb);
...
void URLRequestManager::ioThreadInitCtx(IOContext *ioCtx) COMMO_THROW (IOStatusException)
{
    InternalUtils::logprintf(logger, CommoLogger::LEVEL_DEBUG,
        ...
        (ioCtx->request->type == URLRequest::BUFFER_DOWNLOAD ||
         ioCtx->request->type == URLRequest::POST_BUFFER_UPLOAD) ? "mem" : ioCtx->request->localFileName.c_str());
    ...
    try {
        ...
        if (ioCtx->request->isSSL) {
            ...
            CURL_CHECK(curl_easy_setopt(ioCtx->curlCtx, CURLOPT_SSL_VERIFYHOST, 
                                        ioCtx->request->verifyHost ? 2L : 0L));
        ...
        if (ioCtx->isUpload) {
            ...
            if (ioCtx->request->type == URLRequest::POST_BUFFER_UPLOAD) {
                CURL_CHECK(curl_easy_setopt(ioCtx->curlCtx,
                                            CURLOPT_POST, 1L));
        ...
        // For verbose debugging; uses stderr output
        // CURL_CHECK(curl_easy_setopt(ioCtx->curlCtx,
        //                            CURLOPT_VERBOSE,
        //                            1L));
        ...
        CURL_CHECK(curl_easy_setopt(ioCtx->curlCtx, CURLOPT_OPENSOCKETFUNCTION, InternalUtils::curlOpenSock));
        CURL_CHECK(curl_easy_setopt(ioCtx->curlCtx, CURLOPT_CLOSESOCKETFUNCTION, InternalUtils::curlCloseSock));
...
{
    if (request->type == URLRequest::FILE_UPLOAD) {
        // Get file size
    ...
    } else if (request->type == URLRequest::POST_BUFFER_UPLOAD) {
        localFileLen = request->getUploadLen();
        // No actual local file needed/used
        return;
    } else {
...
URLRequest::URLRequest(URLRequestType type, const std::string &url,
               ...
               bool verifyHost) COMMO_THROW (CommoResult) :
                           type(type),
                           url(url),
