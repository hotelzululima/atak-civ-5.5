...
namespace TAK {
    namespace Commo {
        namespace impl {
            ...
            class CoTDetailExtenderImpl : public atakmap::commoncommo::CoTDetailExtender
            {
            public:
                CoTDetailExtenderImpl(TAK::Commo::ICoTDetailExtender ^extender);
                virtual ~CoTDetailExtenderImpl();

                virtual atakmap::commoncommo::CommoResult encode(uint8_t **encodedExtension, size_t *encodedSize, const char *cotDetailElement);
                virtual atakmap::commoncommo::CommoResult decode(const char **cotDetailElement, const uint8_t *encodedExtension, size_t encodedSize);
                virtual void releaseDecodeResult(const char *resultBuffer);
                virtual void releaseEncodeResult(const uint8_t *resultBuffer);

            private:
                gcroot<TAK::Commo::ICoTDetailExtender ^> extenderCLI;
            };
        }
    }
}
