...
namespace {
    ...
    array<CoTMessageType> ^getAllTypes() {
        return gcnew array<CoTMessageType> {
            CoTMessageType::Chat,
...
void test::CommoAppCLI::run(array<System::String ^> ^args)
{
    ...
    while (!quit) {
        ...
        if (curArg < args->Length && time(NULL) > nextCmdLineTime) {
            ...
            if (s == "quit") {
            ...
            else if (s->IndexOf("estream:") == 0) {
                array<String ^> ^params = s->Split(':');
                array<Byte> ^caCert = nullptr;
                String ^caPw = nullptr;
                String ^user = nullptr;
                String ^userPw = nullptr;
                String ^keyPw = nullptr;
                String ^enrollCaPw = nullptr;
                String ^versionInfo = nullptr;
                EnrollInfo ^enroll = gcnew EnrollInfo();
                int eport;
                bool hostCheck = true;
                ...
                if (enroll->host->Length > 0 && enroll->host[0] == '-') {
                    hostCheck = false;
                    enroll->host = enroll->host->Substring(1);
                }
                if (user->Length > 0 && user[0] == '-') {
                    useTokenAuth = true;
                    user = user->Substring(1);
                }
                eport = Convert::ToInt32(params[9]);
                enroll->port = Convert::ToInt32(params[10]);
                versionInfo = params[11];
                if (versionInfo->Length > 0 && versionInfo[0] == '-')
                    versionInfo = nullptr;

                if (enrollCaPw->Length == 0) {
                    enroll->useEnrollResultTrust = false;
                    enrollCaPw = "JunkNotUsed";
                    enroll->caPass = caPw;
                    enroll->trust = caCert;
                }
                else {
                    enroll->useEnrollResultTrust = true;
                    enroll->caPass = enrollCaPw;
                }

                if (commo->EnrollmentInit(id, enroll->host, eport, hostCheck, user, userPw, useTokenAuth, caCert, caPw, enroll->clientPass, enroll->caPass, keyPw, 4096, versionInfo) != CommoResult::CommoSuccess)
                {
                estreamerr:
                    msclr::interop::marshal_context mctx;
                ...
                if (params[0]->Equals("sstream") || params[0]->Equals("qstream")) {
                    ...
                    if (params[0][0] == 'q')
                        transport = StreamingTransport::TransportQuic;
                    else
                ...
                iface = commo->AddStreamingInterface(transport,
                    params[hostIdx], portNum, getAllTypes(),
                    cert,
...
void test::CommoAppCLI::EnrollmentUpdate(EnrollmentIOUpdate ^update)
{
    msclr::interop::marshal_context mctx;
    const char *msg = "no info";
    if (update->additionalInfo)
        msg = mctx.marshal_as<const char *>(update->additionalInfo);

    fprintf(logFile, "EnrollUpdate: step %d id %d status %d (%s) %d of %d priv %d ca %d\n",
        update->step,
        update->transferId, update->status,
        msg,
        (int)update->totalBytesTransferred, (int)update->totalBytesToTransfer,
        update->privResult != nullptr ? update->privResult->Length : 0,
        update->caResult != nullptr ? update->caResult->Length : 0);
    fflush(logFile);
    if (update->status == SimpleFileIOStatus::InProgress)
        return;
    if (update->status == SimpleFileIOStatus::Success && update->step != EnrollmentStep::Sign)
        return;

    // all remaining cases will result in termination of enrollment
    
    EnrollInfo ^e;
    if (!activeEnrollments.TryGetValue(update->transferId, e)) {
    ...
    if (update->status != SimpleFileIOStatus::Success) {
        ...
        StreamingNetInterface ^iface = commo->AddStreamingInterface(
            e->host, e->port, getAllTypes(),
            update->privResult,
            ca,
            e->clientPass, e->caPass, nullptr, nullptr, r);
    ...
    activeEnrollments.Remove(update->transferId);
}
