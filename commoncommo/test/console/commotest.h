...
struct EnrollInfo {
    std::vector<uint8_t> trust;
    std::string clientPass;
    std::string caPass;
    std::string host;
    int port;
    bool useEnrollResultTrust;
};
...
class CommoTest : public CoTDetailExtender,
                public CoTMessageListener, public GenericDataListener,
                ...
                public SimpleFileIO, public CloudIO, public EnrollmentIO
{
public:
    CommoTest();
    virtual ~CommoTest();
    ...
    virtual CommoResult encode(uint8_t **encoded, size_t *encodedSize, const char *cotDetail) override;
    virtual CommoResult decode(const char **cotDetail, const uint8_t *encoded, size_t encodedSize) override;
    virtual void releaseDecodeResult(const char *resultBuffer) override;
    virtual void releaseEncodeResult(const uint8_t *resultBuffer) override;
    ...
    virtual void enrollmentUpdate(const EnrollmentIOUpdate *update) override;
...
private:
    ...
    std::map<int, EnrollInfo *> activeEnrollments;
