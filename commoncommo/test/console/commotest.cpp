...
namespace {
    ...
    bool writeFile(const char *fileName, const uint8_t *data, const size_t len)
    {
...
"    quicin:<port>\n"
"        Add quic inbound interface on <port>\n"
...
"    qstream:<certfile>:<truststore>:<cert pass>:<ip or host>:<port>\n"
"    qstream:<certfile>:<truststore>:<cert pass>:<user>:<pass>:<ip or host>:<port>\n"
"        Same as sstream, but instead of TCP-based TLS/SSL, uses QUIC transport\n"
"    estream:<truststore>:<trust pass>:<keypass>:<clientpass>:<capass>:<user>:<pass>:<ip or host>:<eport>:<port>:<versioninfo>\n"
"        Enroll for a client certificate, then add streaming interface\n"
"        to TAK server at <ip or host> on <port>\n"
"        using TLS encryption. <ip or host> can be pre-pended with a \"-\" \n"
"        to suppress SSL hostname verification.\n"
"        <truststore> is used to check server during enrollment and, if\n"
"        <capass> is empty, for the streaming connection. Specify <truststore> as\n"
"        a single dash (\"-\") to skip checking of the server certificate.\n"
"        <trust pass> is the password encrypting <truststore> (ignored if\n"
"        <truststore> is \"-\"\n"
"        <keypass> is used to protect the private key generated.\n"
"        <clientpass> and <capass> protect the client certificate\n"
"        and CA/truststore obtained during enrollment. Leave <capass> blank\n"
"        to cause the post-enrollment streaming connection to use the\n"
"        enrollment trust (<truststore>) instead of that obtained during\n"
"        enrollment.\n"
"        <user> and <pass> are the credentials for enrollment.\n"
"        Prepending <user> with a \"-\" will result in <pass> being \n"
"        treated as a bearer token for authentication.\n"
"        <eport> specifies the port number on the server for enrollment\n"
"        <versioninfo> specifies version string to supply during enrollment.\n"
"        Supplying simply '-' will result in not supplying version info to server.\n"
"    mcast:<macaddr>:<port>\n"
...
"        (clients are numbered starting from 0\n"
"    proto:<cotxmlfile>:<takprotofile>\n"
"        convert cot in xml format to takproto\n"
"        (single message in input file)\n"
"\n"
...
void test::CommoTest::run(int argc, char *argv[])
{
    ...
    // commo->registerCoTDetailExtender(1, "commotest", this);
    //commo->setStreamMonitorEnabled(false);
...
//        sendSA(false, "d0580174-4919-45a7-888f-db8f14e274f1");
        ...
        if (curArg < argc && time(NULL) >= nextCmdLineTime) {
            ...
            if (s == "quit") {
                ...
                if (params.size() != 2) {
                    fprintf(logFile, "ERROR IN COMMAND LINE: wrong args for proto - needs <cotxmlfile>:<takprotofile>\n");
                } else {
                    xml = readFile(params[0].c_str(), &xmlLen);
                    if (!xml)
                        fprintf(logFile, "ERROR READING input xml file %s\n", params[0].c_str());
                    else if (xml[xmlLen] != '\0') {
                        uint8_t *nxml = new uint8_t[xmlLen + 1];
                        memcpy(nxml, xml, xmlLen);
                        nxml[xmlLen] = '\0';
                        delete[] xml;
                        xml = nxml;
                    }
                ...
                if (xml) {
                    char *proto = NULL;
                    size_t protoLen = 0;
                    CommoResult r = commo->cotXmlToTakproto(&proto, &protoLen, (char *)xml, 1);
                    if (r != COMMO_SUCCESS)
                        fprintf(logFile, "ERROR converting xml file %s to takproto - invalid input\n", params[0].c_str());
                    else {
                        fprintf(logFile, "takproto conversion from %d to %d bytes\n", (int)xmlLen, (int)protoLen);
                        writeFile(params[1].c_str(), (uint8_t *)proto, protoLen);
                        commo->takmessageFree(proto);
                    }
                }
...
//                commo->sendCoTTcpDirect("192.168.99.111", 1111, saString.c_str());
                    ...
                    for (std::vector<NetIfaceInfo>::iterator iter = v.begin(); iter != v.end(); ++iter) {
                        ...
                        switch (inf.type) {
                            ...
                            case NetIfaceInfo::IT_TCPIN:
                                ...
                                break;
                            case NetIfaceInfo::IT_QUICIN:
                                res = commo->removeQuicInboundInterface((QuicInboundNetInterface *)inf.iface);
                                break;
                ...
                int id;
                if (commo->sendMissionPackageInit(&id, &clist, fn.c_str(), fn.c_str(), fn.c_str(), NULL, NULL, NULL) != COMMO_SUCCESS)
            ...
            } else if (s.find("quicin:") <= 1) {
                size_t loc = s.find(':');
                std::string port = s.substr(loc + 1);
                int portNum = atoi(port.c_str());

                QuicInboundNetInterface *iface = commo->addQuicInboundInterface(portNum, httpsCertBuf, httpsCertLen, "atak99123", NULL);
                if (!iface)
                    fprintf(logFile, "Failed to add interface %s\n", s.c_str());
                else {
                    ifaceDescs[iface] = s;
                    ...
                    std::vector<NetIfaceInfo> v;
                    v.push_back(NetIfaceInfo(NetIfaceInfo::IT_QUICIN, iface));
                    ifaceList.push_back(v);
                }
            } else if (s.find("estream:") <= 1) {
                std::vector<std::string> params = split(s, ':');
                uint8_t *caCert = NULL;
                size_t caLen = 0;
                std::string caPw;
                ...
                std::string userPw;
                std::string keyPw;
                std::string enrollCaPw;
                std::string versionInfo;
                EnrollInfo *enroll = NULL;
                int eport;
                bool hostCheck = true;
                bool useTokenAuth = false;
                int id;
                bool hasCACert = false;
                bool useEnrollResultTrust = false;

                if (params.size() != 12) {
                    fprintf(logFile, "ERROR IN COMMAND LINE: need 12 args for estream, got %d\n", (int)params.size());
                    goto estreamerr;
                }
                enrollCaPw = params[5];
                hasCACert = params[1] != "-";
                useEnrollResultTrust = enrollCaPw.length() != 0;
                if (!hasCACert && !useEnrollResultTrust) {
                    fprintf(logFile, "ERROR IN COMMAND LINE: cannot enroll without CACert if not using enrollment trust response\n");
                    goto estreamerr;
                }
                
                enroll = new EnrollInfo();
                if (hasCACert)
                    caCert = readFile(params[1].c_str(), &caLen);
                caPw = params[2];
                keyPw = params[3];
                enroll->clientPass = params[4];
                user = params[6];
                userPw = params[7];
                ...
                enroll->host = params[8];
                if (enroll->host.length() > 0 && enroll->host[0] == '-') {
                    hostCheck = false;
                    enroll->host.erase(0, 1);
                }
                eport = atoi(params[9].c_str());
                enroll->port = atoi(params[10].c_str());
                versionInfo = params[11];
                if (versionInfo.length() > 0 && versionInfo[0] == '-')
                    versionInfo = "";
                enroll->useEnrollResultTrust = useEnrollResultTrust;
                if (!useEnrollResultTrust) {
                    enrollCaPw = "IDoNotMatterSinceIWontBeUsed!";
                    // Stash enrollment trust and pw for use by
                    // streaming connection after enrollment
                    enroll->caPass = caPw;
                    enroll->trust.insert(enroll->trust.end(), caCert, caCert + caLen);
                } else {
                    enroll->caPass = enrollCaPw;
                }
                
                
                if (commo->enrollmentInit(&id, enroll->host.c_str(),
                     eport, hostCheck, 
                     user.c_str(),
                     userPw.c_str(),
                     useTokenAuth,
                     caCert, caLen, caPw.c_str(),
                     enroll->clientPass.c_str(),
                     enroll->caPass.c_str(),
                     keyPw.c_str(),
                     4096,
                     versionInfo.empty() ? NULL : versionInfo.c_str()) != COMMO_SUCCESS) {
                    fprintf(logFile, "enrollmentInit failed for %s\n", desc.c_str());
estreamerr:
                    delete enroll;
                } else {
                    fprintf(logFile, "enrollmentInit successful - id is %d\n", id);
                    activeEnrollments[id] = enroll;
                ...
                netinterfaceenums::StreamingTransport transport =
                        netinterfaceenums::TRANSPORT_TCP;
                int portNum = 0;
                int hostIdx = 1;
                if (params.size() < 3) {
                ...
                if (params[0].compare("sstream") == 0 ||
                            params[0].compare("qstream") == 0) {
                    transport = params[0][0] == 'q' ? 
                                netinterfaceenums::TRANSPORT_QUIC :
                                netinterfaceenums::TRANSPORT_SSL;
                    if (params.size() != 6 && params.size() != 8) {
                        fprintf(logFile, "ERROR IN COMMAND LINE: need 5 or 7 args for sstream/qstream\n");
                        goto streamerr;
                    } else {
                        hostIdx = 4;
                ...
                iface = commo->addStreamingInterface(transport,
...
streamerr:
    ...
    if (httpsCertLen != 0)
        commo->freeSelfSignedCert(httpsCertBuf);
...
CommoResult test::CommoTest::encode(uint8_t **encoded, size_t *encodedSize, const char *cotDetail)
{
    uint8_t *encodedData = new uint8_t[10];
    memcpy(encodedData, "CommoTest", 10);
    *encoded = encodedData;
    *encodedSize = 10;
...
CommoResult test::CommoTest::decode(const char **cotDetail, const uint8_t *encoded, size_t encodedSize)
{
    if (encodedSize != 10 || memcmp("CommoTest", encoded, 10) != 0) {
	fprintf(logFile, "Invalid extension data received\n");
...
void test::CommoTest::releaseDecodeResult(const char *resultBuffer)
{
    // static alloc - nothing to do
}

void test::CommoTest::releaseEncodeResult(const uint8_t *resultBuffer)
{
    delete[] resultBuffer;
}
...
void test::CommoTest::enrollmentUpdate(const EnrollmentIOUpdate *update)
{
    fprintf(logFile, "EnrollUpdate: step %d id %d status %d (%s) %" PRIu64 " of %" PRIu64 " priv %d %p ca %d %p\n",
                      update->step,
                      update->xferid, update->status, 
                      update->additionalInfo ? update->additionalInfo : "no info",
                      update->bytesTransferred, update->totalBytesToTransfer,
                      (int)update->privResultLen, update->privResult,
                      (int)update->caResultLen, update->caResult);
    fflush(logFile);
    if (update->status == FILEIO_INPROGRESS)
        return;
    if (update->status == FILEIO_SUCCESS && update->step != ENROLL_STEP_SIGN)
        return;

    // all remaining cases will result in termination of enrollment
    auto iter = activeEnrollments.find(update->xferid);
    if (iter == activeEnrollments.end()) {
        fprintf(logFile, "Enrollment update for unknown enrollment transaction! %d\n", update->xferid);
        return;
    }
    EnrollInfo *e = iter->second;

    if (update->status != FILEIO_SUCCESS) {
        fprintf(logFile, "Failure status for enrollment %d, terminating\n", update->xferid);
    } else {
        // We know this to be SUCCESS+SIGN which is the end of road
        fprintf(logFile, "Completed status for enrollment %d, adding stream connection\n", update->xferid);
        
        const uint8_t *ca;
        size_t caLen;
        if (e->useEnrollResultTrust) {
            ca = update->caResult;
            caLen = update->caResultLen;
        } else {
            ca = e->trust.data();
            caLen = e->trust.size();
        }
        
        std::string certFn = baseDir + "/commo-enroll-cert.p12";
        FILE *certFile = fopen(certFn.c_str(), "wb");
        
        if (certFile) {
            fwrite(update->privResult, 1, update->privResultLen, certFile);
            fclose(certFile);
        }

        StreamingNetInterface *iface = commo->addStreamingInterface(
               netinterfaceenums::TRANSPORT_SSL,
               e->host.c_str(), e->port, allTypes,
               2,
               //e->cert.data(), e->cert.size(),
               //e->trust.data(), e->trust.size(),
               update->privResult, update->privResultLen,
               ca, caLen,
               e->clientPass.c_str(), e->caPass.c_str(), NULL, NULL);
        if (!iface)
            fprintf(logFile, "Adding post-enrollment %d connection failed\n", update->xferid);
        else {
            std::string d("enroll-");
            d += e->host;
            ifaceDescs[iface] = d;
