...
import com.atakmap.map.cache.CacheEventsService;
...
public final class EngineLibrary
{
    ...
    static
    {
        ...
        MapRendererFactory.registerSpi(new IMapRendererSpi()
        {
            ...
            public MapRenderer create(IGlobe globe, Object parent, Class<?> surfaceType)
            {
                ...
                if (!surfaceType.isAssignableFrom(GLMapSurface.class))
                    return null;
                ...
                if (parent instanceof AtakMapView && globe instanceof Globe)
                {
                    ...
                    return LegacyAdapters.adapt(context.getGLMapView());
                }
                return null;
            }
        });
    ...
    public static void initialize()
    {
        ...
        try
        {
            ...
            final String hostModel = Build.MODEL;
            if(hostModel != null && hostModel.toLowerCase().startsWith("mpu"))
                ConfigOptions.setOption("mpu", 1);
        } catch (Throwable t)
        {
        ...
        try
        {
            cacheEventsService = new CacheEventsService();
        } catch (Throwable t)
        {
            Log.w("EngineLibrary", "Failed initialize CacheEventsService", t);
        }
    }
    ...
    {
        // shutdown services
        if (cacheEventsService != null)
            cacheEventsService.shutdown();
    ...
    private static CacheEventsService cacheEventsService;
