...
namespace takenginetests {
    ...
    TEST_F(BitmapFactory2Tests, testBitmapEncode) {
        TAKErr code(TE_Ok);

        std::string resource = TAK::Engine::Tests::getResource("FLAG_B24.PNG");
        ASSERT_TRUE(TE_Ok == code);

        // cross platform bitmap
        BitmapPtr gdalBitmap(nullptr, nullptr);
        FileInput2 fileInput;

        code = fileInput.open(resource.c_str());
        std::size_t read;
        std::vector<uint8_t> original;
        auto len = fileInput.length();

        original.resize(len);
        fileInput.read(original.data(), &read, len);
        fileInput.seek(0);
        code = BitmapFactory2_decode(gdalBitmap, fileInput, nullptr);
        ASSERT_TRUE(TE_Ok == code);
        ASSERT_TRUE(gdalBitmap.get());
        fileInput.close();

        std::vector<uint8_t> bytes;
        bytes.resize(len* 2);
        MemoryOutput2 data;
        data.open(bytes.data(), len);
        BitmapFactory2_encode(data, *gdalBitmap, BitmapEncodeOptions());
        std::size_t remaining = 0;
        data.remaining(&remaining);

        MemoryInput2 roundTrip;
        roundTrip.open(bytes.data(), len - remaining);
        BitmapPtr roundTripBitmap(nullptr, nullptr);
        code = BitmapFactory2_decode(roundTripBitmap, roundTrip, nullptr);
            
        ASSERT_TRUE(roundTripBitmap->getWidth() == gdalBitmap->getWidth());
        ASSERT_TRUE(roundTripBitmap->getHeight() == gdalBitmap->getHeight());
        ASSERT_TRUE(roundTripBitmap->getFormat() == gdalBitmap->getFormat());
        ASSERT_TRUE(roundTripBitmap->getStride() == gdalBitmap->getStride());
        const int relates = memcmp(gdalBitmap->getData(), roundTripBitmap->getData(), roundTripBitmap->getWidth() * roundTripBitmap->getStride());

        ASSERT_TRUE(relates == 0);
