...
namespace takenginetests {
	...
	TEST(TileReaderFactory2Tests, testTileReaderPriorityTouchOrder)
	{
		...
		TAKErr code = TileReaderFactory2_register(context.addType("last", 0, TE_InvalidArg, TE_InvalidArg));
		ASSERT_TRUE(code == TE_Ok);

		code = TileReaderFactory2_register(context.addType("middle", 1, TE_InvalidArg, TE_InvalidArg));
		ASSERT_TRUE(code == TE_Ok);

		code = TileReaderFactory2_register(context.addType("first", 3, TE_InvalidArg, TE_InvalidArg));
		ASSERT_TRUE(code == TE_Ok);

		code = TileReaderFactory2_isSupported("foo");
		ASSERT_TRUE(code == TE_InvalidArg);
	...
	TEST(TileReaderFactory2Tests, testTileReaderFirstToFail)
	{
		...
		// Test for create and isSupported returning a non-TE_Ok or TE_InvalidArg error code and stopping
		// as this is considered a full failure

		TestTileReader context;

		TAKErr code = TileReaderFactory2_register(context.addType("a", 3, TE_InvalidArg, TE_InvalidArg));
		ASSERT_TRUE(code == TE_Ok);
