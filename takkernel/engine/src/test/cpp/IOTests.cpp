...
namespace takenginetests {
    ...
    TEST(IOTests, testGetParent) {
        const char* path = "/folder1/folder2/foo.zip";

        TAK::Engine::Port::String parent;
        TAK::Engine::Util::TAKErr code = TAK::Engine::Util::IO_getParentFile(parent, path);
        ASSERT_EQ(parent, "/folder1/folder2");

         code = TAK::Engine::Util::IO_getParentFile(parent, parent);
        ASSERT_EQ(parent, "/folder1");
    }

    TEST(IOTests, testIOFailures) {
        const char* badPath = "asdf";

        TAK::Engine::Port::STLVectorAdapter<TAK::Engine::Port::String> files;
        TAKErr code = IO_listFiles(files, badPath);
        ASSERT_NE(code, TE_Err);

        std::size_t size;
        code = IO_getFileCount(&size, badPath);
        ASSERT_NE(code, TE_Err);
