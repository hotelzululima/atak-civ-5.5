...
namespace takenginetests {
	namespace {
		...
		public:
			...
			virtual TAKErr create(TileContainerPtr &result, const char *name, const char *path, const TileMatrix *spec) const NOTHROWS
			{
				...
				return success ? TAK::Engine::Util::TE_Ok : TAK::Engine::Util::TE_InvalidArg;
			}
			...
			virtual TAKErr open(TileContainerPtr &result, const char *path, const TileMatrix *spec, bool readOnly) const NOTHROWS
			{
				TAKErr ret;
				if (spec)
					ret = spec->getSRID() == compatSRID ? TAK::Engine::Util::TE_Ok : TAK::Engine::Util::TE_InvalidArg;
				else
					ret = openRes;
	...
	TEST(TileContainerFactoryTests, testOpen)
	{
		...
		TAKErr code = TileContainerFactory_registerSpi(context.addSpi("spi50fail", "s50", 50, TAK::Engine::Util::TE_InvalidArg));
