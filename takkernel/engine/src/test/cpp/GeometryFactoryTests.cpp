...
namespace takenginetests {
    ...
    TEST_F(GeometryFactoryTests, wedgeCartesian) {
        TAKErr code(TE_Ok);
        ...
        Geometry2Ptr value(nullptr, nullptr);
        code = GeometryFactory_createWedge(value, center, 0, 90, 1.0, algo);
        ASSERT_TRUE(TE_Ok == code);

        ASSERT_TRUE(value->getClass() == TEGC_Polygon);

        const Feature::Polygon2& poly = static_cast<const Feature::Polygon2&>(*value);

        std::shared_ptr<LineString2> lineString;
        poly.getExteriorRing(lineString);

        ASSERT_TRUE(lineString->getNumPoints() == 94);

        TAK::Engine::Feature::Point2 start(0,0), end(0,0);
        lineString->get(&start, 0);
        lineString->get(&end, 93);

        TAK::Engine::Feature::Point2 north(0, 0), east(0, 0);
        lineString->get(&north, 1);
        lineString->get(&east, 92);

        ASSERT_TRUE(start.x == end.x);
        ASSERT_TRUE(start.y == end.y);
        ASSERT_TRUE(start.z == end.z);

        const double epsilon = 1e-6;

        ASSERT_TRUE(std::abs(north.x - 0) < epsilon);
        ASSERT_TRUE(std::abs(north.y - 1) < epsilon);
        ASSERT_TRUE(std::abs(north.z - 0) < epsilon);

        ASSERT_TRUE(std::abs(east.x - 1) < epsilon);
        ASSERT_TRUE(std::abs(east.y - 0) < epsilon);
        ASSERT_TRUE(std::abs(east.z - 0) < epsilon);
