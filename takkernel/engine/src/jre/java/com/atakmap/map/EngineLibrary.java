...
public final class EngineLibrary
{
    ...
    public static void initialize()
    {
        ...
        if (SystemUtils.isArch64())
        {
            key |= ARCH_x64;
        } else if (SystemUtils.isArch32())
        {
            key |= ARCH_x86;
        }
        ...
        try
        {
            ...
            if(!GraphicsEnvironment.isHeadless()) {
                final float scaleFactor = java.awt.Toolkit.getDefaultToolkit().getScreenResolution() / 96f;
                GLRenderGlobals.setRelativeScaling(scaleFactor);
            }
            DisplaySettings.setDpi(300f);

            // force GDAL static init and suppress logging
            {
