...
#ifdef MSVC
            struct ENGINE_API ThreadSpi
            {
                
            public:
                virtual ~ThreadSpi() {}
                virtual Util::TAKErr start(ThreadPtr& value, void* (*entry)(void*), void* opaque, const ThreadCreateParams params = ThreadCreateParams()) NOTHROWS = 0;
            };

            typedef std::unique_ptr<ThreadSpi, void(*)(const ThreadSpi*)> ThreadSpiPtr;

            ENGINE_API Util::TAKErr Thread_setDefaultSpi(ThreadSpiPtr ptr);
#endif
            /**
