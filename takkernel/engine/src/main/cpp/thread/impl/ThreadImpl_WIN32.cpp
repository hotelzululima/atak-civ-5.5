...
namespace
{
    ...
    {
        static ThreadSpiPtr defaultSpi;
    public:
        static void setDefault(ThreadSpiPtr ptr)
        {
            defaultSpi = std::move(ptr);
        }

        static void start(ThreadPtr& value, void* (*entry)(void*), void* opaque, const ThreadCreateParams params)
        {
            if(defaultSpi)
                defaultSpi->start(value, entry, opaque, params);
        }
    };
    ThreadSpiPtr ThreadRegistry::defaultSpi(nullptr, nullptr);
}

TAKErr TAK::Engine::Thread::Thread_setDefaultSpi(ThreadSpiPtr ptr)
{
    ThreadRegistry::setDefault(std::move(ptr));
    return TE_Ok;
}
...
TAKErr TAK::Engine::Thread::Thread_start(ThreadPtr &value, void *(*entry)(void *), void *opaque, const ThreadCreateParams params) NOTHROWS
{
    ...
    ThreadRegistry::start(value, entry, opaque, params);
    if (value)
        return code;
...
{
    thread_local ThreadID tid(std::move(ThreadIDImplPtr(new Windows32ThreadIDImpl(GetCurrentThreadId()), Memory_deleter_const<ThreadIDImpl, Windows32ThreadIDImpl>)));
    return tid;
}
...
inline TAKErr ThreadImpl::start(void *(*entry)(void *), void *opaque) NOTHROWS
{
    ...
    try {
        ...
        if (name_) {
#if _DEBUG
            // set the thread name via `SetThreadDescription` in debug mode. Alternative mechanisms
            // identified as more portable, such as those discussed here,
            // https://docs.microsoft.com/en-us/visualstudio/debugger/how-to-set-a-thread-name-in-native-code?view=vs-2022
            // are observed to result in deadlock and failure of debugger to attach in some
            // environments.
