...
{
    thread_local ThreadID tid(std::move(ThreadIDImplPtr(new PThreadsThreadIDImpl(pthread_self()), Memory_deleter_const<ThreadIDImpl, PThreadsThreadIDImpl>)));
    return tid;
}
...
namespace
{
    ...
    PThreadsMutexImpl::~PThreadsMutexImpl() NOTHROWS
    {
        mutex.reset();
        attr.reset();
    }
