...
namespace TAK {
	namespace Engine {
		namespace Model {
            ...
            TAKErr C3DT_parsePNTSScene(ScenePtr& scenePtr, SceneInfoPtr& sceneInfoPtr, DataInput2* input, const char* baseURI, const char* URI) NOTHROWS {
                TAKErr code = PNTS_parse(scenePtr, input, baseURI);
                TE_CHECKRETURN_CODE(code);
...
TAK::Engine::Util::TAKErr Cesium3DTilesSceneInfoSpi::create(TAK::Engine::Port::Collection<SceneInfoPtr>& scenes, const char* URI) NOTHROWS {
	...
	if (type == C3DTFileType_TilesetJSON) {
        ...
        SceneInfoPtr model;
        ScenePtr scenePtr(nullptr, nullptr);
        code = C3DT_parsePNTSScene(scenePtr, model, input.get(), baseURI, URI);
