...
namespace
{
    ...
    {
    public :
        typedef std::unique_ptr<ElementAccess<T>, void(*)(const ElementAccess<T> *)> Ptr;
    protected :
...
#undef NORMALIZED_ELEMENT_ACCESS_DECL
    ...
    public:
        ...
        ElementAccess<std::size_t>::Ptr indexAccess { ElementAccess<std::size_t>::Ptr(nullptr, nullptr) };
        std::vector<MemBuffer2> buffers;
        VoidPtr cleaner;

        ElementAccess<double>::Ptr position_access_ { ElementAccess<double>::Ptr(nullptr, nullptr) };
        std::map<VertexAttribute, ElementAccess<float>::Ptr> texCoordAccess;
        ElementAccess<float>::Ptr normal_access_ { ElementAccess<float>::Ptr(nullptr, nullptr) };
        ElementAccess<float>::Ptr color_access_ { ElementAccess<float>::Ptr(nullptr, nullptr) };
    ...
    public :
        TAKErr getVertices(const void **value, const std::size_t attr) const NOTHROWS override;
    public :
        MemBuffer2 positions_;
        std::map<VertexAttribute, MemBuffer2> texCoords;
        MemBuffer2 normals;
        MemBuffer2 colors;
    };

    const uint8_t EMPTY_BUFFER[1u] = {0u};

    TAKErr checkInitParams(const DrawMode &mode, const VertexDataLayout &layout, const DataType &indexType) NOTHROWS;
    TAKErr resize(MemBuffer2 &buf, const std::size_t newSize) NOTHROWS;
    template<class T>
    TAKErr createElementAccess(typename ElementAccess<T>::Ptr &value, const DataType &type, const bool normalized) NOTHROWS;
    template<class T>
...
#define COPY_VERTICES(teva, srcVa, dstVa) \
    ...
    COPY_VERTICES(TEVA_Normal, normals, normals);
    COPY_VERTICES(TEVA_Color, colors, colors);
#undef COPY_VERTICES
...
TAKErr TAK::Engine::Model::MeshBuilder_buildInterleavedMesh(MeshPtr &value, const DrawMode mode, const WindingOrder order, const VertexDataLayout &layout, const std::size_t numMaterials, const Material *materials, const Feature::Envelope2 &aabb, const std::size_t numVertices, const void *vertices, const Port::DataType indexType, const std::size_t numIndices, const void *indices, std::unique_ptr<void, void(*)(const void *)> &&cleaner) NOTHROWS
{
    ...
    impl.vertices = MemBuffer2(static_cast<const uint8_t *>(vertices), size);
    impl.vertexCount = numVertices;

    if (indices)
        impl.indices = MemBuffer2(static_cast<const uint8_t *>(indices), impl.indexAccess->transferSize(numIndices));
...
#define COPY_VERTICES(teva, srcVa, dstVa) \
    if(layout.attributes&teva) { \
        ...
        code = VertexDataLayout_requiredDataSize(&size, layout, teva, numVertices); \
        TE_CHECKRETURN_CODE(code); \
        impl.dstVa = MemBuffer2(static_cast<const uint8_t *>(srcVa), size); \
    }
...
TAKErr TAK::Engine::Model::MeshBuilder_buildInterleavedMesh(MeshPtr& value, const DrawMode mode, const WindingOrder order, const VertexDataLayout& layout, const std::size_t numMaterials, const Material* materials, const Feature::Envelope2& aabb, const std::size_t numVertices, std::unique_ptr<const void, void(*)(const void*)>&& vertices, const Port::DataType indexType, const std::size_t numIndices, std::unique_ptr<const void, void(*)(const void*)>&& indices,
    ...
    impl.vertices = MemBuffer2(std::move(vertices), size);
    impl.vertexCount = numVertices;

    if(indices)
        impl.indices = MemBuffer2(std::move(indices), impl.indexAccess->transferSize(numIndices));
...
#pragma warning(pop)
    ...
    void ModelImplBase::initAttributeAccess() NOTHROWS
    {
        if(layout_.attributes&TEVA_Position)
            createElementAccess<double>(position_access_, layout_.position.type, false);
        if(layout_.attributes&TEVA_TexCoord0) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord0.type, false);
            texCoordAccess.emplace(TEVA_TexCoord0, std::move(access));
        }
        if(layout_.attributes&TEVA_TexCoord1) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord1.type, false);
            texCoordAccess.emplace(TEVA_TexCoord1, std::move(access));
        }
        if(layout_.attributes&TEVA_TexCoord2) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord2.type, false);
            texCoordAccess.emplace(TEVA_TexCoord2, std::move(access));
        }
        if(layout_.attributes&TEVA_TexCoord3) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord3.type, false);
            texCoordAccess.emplace(TEVA_TexCoord3, std::move(access));
        }
        if(layout_.attributes&TEVA_TexCoord4) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord4.type, false);
            texCoordAccess.emplace(TEVA_TexCoord4, std::move(access));
        }
        if(layout_.attributes&TEVA_TexCoord5) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord5.type, false);
            texCoordAccess.emplace(TEVA_TexCoord5, std::move(access));
        }
        if(layout_.attributes&TEVA_TexCoord6) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord6.type, false);
            texCoordAccess.emplace(TEVA_TexCoord6, std::move(access));
        }
        if(layout_.attributes&TEVA_TexCoord7) {
            ElementAccess<float>::Ptr access(nullptr, nullptr);
            createElementAccess<float>(access, layout_.texCoord7.type, false);
            texCoordAccess.emplace(TEVA_TexCoord7, std::move(access));
        }
        if(layout_.attributes&TEVA_Normal)
            createElementAccess<float>(normal_access_, layout_.normal.type, false);
        if(layout_.attributes&TEVA_Color) {
            ...
            switch(layout_.color.type) {
                ...
                case TEDT_UInt16 :
                case TEDT_UInt32 :
                    createElementAccess<float>(color_access_, layout_.color.type, true);
                    break;
                case TEDT_Float32 :
                case TEDT_Float64 :
                    createElementAccess<float>(color_access_, layout_.color.type, false);
                    break;
                default :
                    createElementAccess<float>(color_access_, layout_.color.type, true);
                    break;
    ...
    TAKErr ModelImplBase::addBuffer(std::unique_ptr<const void, void(*)(const void*)>&& buffer, size_t bufferSize) NOTHROWS {
        TAKErr code(TE_Ok);
        TE_BEGIN_TRAP() {
            buffers.push_back(MemBuffer2(std::move(buffer), bufferSize));
        } TE_END_TRAP(code);
        return code;
    }
    ...
    TAKErr ModelImplBase::getTextureCoordinate(Point2<float> *value, const VertexAttribute texAttr, const std::size_t index) const NOTHROWS
    {
        ...
        TE_CHECKRETURN_CODE(code);
        std::map<VertexAttribute, ElementAccess<float>::Ptr>::const_iterator entry;
    ...
    {
        if(!vertexCount) {
            // pointer to at end of array
            *value = &EMPTY_BUFFER[1];
            return TE_Ok;
        }
        if(!vertices.size())
            return TE_IllegalState;
        switch(attr) {
            ...
            case TEVA_Reserved8 :
            case TEVA_Reserved9 :
            case TEVA_Reserved10 :
            case TEVA_Reserved11 :
            case TEVA_Reserved12 :
            case TEVA_Reserved13 :
            case TEVA_Reserved14 :
            case TEVA_Reserved15 :
            case TEVA_Reserved16 :
            case TEVA_Reserved17 :
            case TEVA_Reserved18 :
            case TEVA_Reserved19 :
            case TEVA_Reserved20 :
                break;
            default :
                return TE_InvalidArg;
        }
...
#define ADD_VERTEX_TEXCOORD(teva) \
    if(layout.attributes&teva) {  \
        code = texCoordAccess.find(teva)->second->put(vertices, texu, texv); \
        TE_CHECKRETURN_CODE(code); \
    }
...
#define ADD_VERTEX_TEXCOORD(teva, vao) \
    if(layout.attributes&teva) { \
        VertexArray texCoord = layout.vao; \
        const std::size_t required = texCoord.offset + (texCoord.stride*(vertexCount+1u)); \
        MemBuffer2 &texCoordsBuf = texCoords[teva]; \
        ...
        ElementAccess<float> &access = *texCoordAccess.find(teva)->second; \
...
#define CASE_TEXCOORD(teva) \
    case teva : \
    { \
        auto entry = texCoords.find(teva); \
...
#undef CASE_TEXCOORD
    ...
    TAKErr resize(MemBuffer2 &buf, const std::size_t newSize) NOTHROWS
    {
        ...
        if(!buf.get() || buf.size() < newSize) {
            ...
            TE_CHECKRETURN_CODE(code);
            MemBuffer2 resized(std::move(membuf), newSize);
            if(buf.get()) {
                buf.flip();
...
#define CASE_STMT(l, t) \
    case l : \
        if(normalized) \
            value = typename ElementAccess<T>::Ptr(new t##_NormalizedElementAccess<T>(), Memory_deleter_const<ElementAccess<T>, t##_NormalizedElementAccess<T>>); \
        else \
            value = typename ElementAccess<T>::Ptr(new t##_ElementAccess<T>(), Memory_deleter_const<ElementAccess<T>, t##_ElementAccess<T>>); \
        break;
...
#undef CASE_STMT
    ...
    bool isDefaultInterleave(const VertexDataLayout &layout) NOTHROWS
    {
        ...
        ElementAccess<double>::Ptr posAccess(nullptr, nullptr);
        if(createElementAccess<double>(posAccess, layout.position.type, false) != TE_Ok)
            return false;
...
#define CHECK_ATTR(teva, vao, t, elems, n) \
    if(layout.attributes&teva) { \
        ...
        if(layout.vao.stride != layout.position.stride) \
            return false; \
        ElementAccess<t>::Ptr access(nullptr, nullptr); \
        if(createElementAccess<t>(access, layout.vao.type, n) != TE_Ok) \
            return false; \
...
#define CONFIGURE_ATTR(teva, vao, eat, dt, elems, n) \
    if(layout.attributes&teva) { \
        ...
        layout.vao.type = dt; \
        typename ElementAccess<eat>::Ptr access(nullptr, nullptr); \
        if(createElementAccess<eat>(access, layout.vao.type, n) == TE_Ok) { \
            vertexSize += access->transferSize(elems); \
            access.reset(); \
        } \
    }
