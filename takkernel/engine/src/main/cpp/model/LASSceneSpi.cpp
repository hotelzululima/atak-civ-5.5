...
int LASSceneSPI::getPriority() const NOTHROWS {
    // LAS goes last as
    //  1. the library is "noisy"
    //  2. LAS gets converted on import at the application layer
    return -1;
}
...
TAKErr LASSceneSPI::create(ScenePtr &scene, const char *URI, ProcessingCallback *callbacks,
    ...
    static TAK::Engine::Thread::Mutex lasReaderMutex;
    TAK::Engine::Thread::Lock lock(lasReaderMutex);
    ...
    while ((point = LASReader_GetNextPoint(*reader, skipFactor)) != nullptr && (!callbacks || (cancel = *callbacks->cancelToken) == false)) {
