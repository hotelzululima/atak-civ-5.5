////============================================================================
////
////    FILE:           MutableFeatureDataStore.cpp
////
////    DESCRIPTION:    Implementation of the MutableFeatureDataStore class
////
...
#include "feature/MutableFeatureDataStore.h"
...
#define MEM_FN( fn ) "atakmap::feature::MutableFeatureDataStore::" fn ": "
...
MutableFeatureDataStore::MutableFeatureDataStore
    (const char* filePath)
  : FeatureDataStore (MODIFY_FEATURESET_INSERT
    ...
    URI (filePath),
    featureDB (FeatureDatabase::createDatabase (filePath)),
    inTransaction (false),
...
//                  (MEM_FN ("MutableFeatureDataStore")
    ...
    if (!featureDB.get ())
      {
        throw std::runtime_error
                  (MEM_FN ("MutableFeatureDataStore")
                   "Failed to create FeatureDatabase");
      }
  }


void
MutableFeatureDataStore::dispose ()
  {
    TAKErr code(TE_Ok);
...
Feature*
MutableFeatureDataStore::getFeature (int64_t featureID)
  {
...
FeatureSet*
MutableFeatureDataStore::getFeatureSet (int64_t featureSetID)
  {
...
bool
MutableFeatureDataStore::isAvailable ()
    const
  {
    TAKErr code(TE_Ok);
    LockPtr lock(NULL, NULL);
...
bool
MutableFeatureDataStore::isFeatureSetVisible (int64_t featureSetID)
    const
  {
    bool visible (false);
...
bool
MutableFeatureDataStore::isFeatureVisible (int64_t featureID)
    const
  {
    std::vector<const char*> queryArgs;
    std::ostringstream strm;
...
MutableFeatureDataStore::queryFeatures (const FeatureQueryParameters& params)
    const
  {
    enum
      {
    ...
    static const char* const featureSelect
        ("SELECT Geometry.id, Geometry.group_id, Geometry.version, "
         "Geometry.name, Geometry.geom, Style.style_rep, NULL");
...
std::size_t
MutableFeatureDataStore::queryFeaturesCount
...
MutableFeatureDataStore::queryFeatureSets
    ...
    const
  {
    ...
    return new FeatureSetCursorImpl (cursor.release (),
                                     ...
                                     *this, &MutableFeatureDataStore::adopt);
  }


std::size_t
MutableFeatureDataStore::queryFeatureSetsCount
    (const FeatureSetQueryParameters& params)
    const
  {
    std::ostringstream sqlStrm;
...
void
MutableFeatureDataStore::beginBulkModificationImpl ()
  {
    TAKErr code(TE_Ok);
    LockPtr lock(NULL, NULL);
    code = Lock_create(lock, getMutex());
    ...
    bool
    MutableFeatureDataStore::isInBulkModification() const {
        TAKErr code(TE_Ok);
        LockPtr lock(NULL, NULL);
        code = Lock_create(lock, getMutex());
        if (code != TE_Ok)
...
void
MutableFeatureDataStore::deleteAllFeatureSetsImpl ()
  {
    ...
    //
      {
        FeatureDatabase::Transaction trans (*featureDB);
...
void
MutableFeatureDataStore::deleteAllFeaturesImpl (int64_t featureSetID)
  {
    std::unique_ptr<db::Statement> stmt
...
void
MutableFeatureDataStore::deleteFeatureImpl (int64_t featureID)
  {
    TAKErr code(TE_Ok);
    LockPtr lock(NULL, NULL);
    code = Lock_create(lock, getMutex());
...
void
MutableFeatureDataStore::deleteFeatureSetImpl (int64_t featureSetID)
  {
    TAKErr code(TE_Ok);
    LockPtr lock(NULL, NULL);
    code = Lock_create(lock, getMutex());
...
void
MutableFeatureDataStore::endBulkModificationImpl (bool successful)
  {
    TAKErr code(TE_Ok);
    LockPtr lock(NULL, NULL);
    code = Lock_create(lock, getMutex());
...
Feature*
MutableFeatureDataStore::insertFeatureImpl (int64_t featureSetID,
                                            const char* name,
                                            Geometry* geometry,
                                            Style* style,
...
FeatureSet*
MutableFeatureDataStore::insertFeatureSetImpl (const char* provider,
                                               const char* type,
                                               const char* name,
                                               double minRes,
                                               double maxRes,
...
void
MutableFeatureDataStore::notifyContentListeners ()
    const
  {
    //
...
void
MutableFeatureDataStore::setFeatureSetVisibleImpl (int64_t featureSetID,
                                                   bool visible)
  {
    std::unique_ptr<db::Statement> stmt
...
void
MutableFeatureDataStore::setFeatureVisibleImpl (int64_t featureID,
                                                bool visible)
  {
    std::ostringstream strm;
    ...
    std::unique_ptr<db::Cursor> cursor
        (featureDB->query ("SELECT Geometry.group_id, groups.visible_version "
                           "FROM Geometry LEFT JOIN groups "
                           "ON Geometry.id = group.id WHERE Geometry.id = ?",
...
void
MutableFeatureDataStore::updateFeatureImpl (int64_t featureID,
                                            const util::AttributeSet& attrs)
  {
    //
    ...
    if (cursor->moveToNext ())
      {
        ...
        if (styleID)
        ...
        else
          {
            ...
            stmt->bind (1, featureName);
            stmt->bind (2, std::make_pair (blob, blob + blobString.size ()));
            stmt->bind (3, featureDB->addStyle (styleRep));
...
void
MutableFeatureDataStore::updateFeatureSetImpl (int64_t featureSetID,
