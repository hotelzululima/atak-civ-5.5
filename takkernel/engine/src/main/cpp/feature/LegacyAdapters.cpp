...
namespace
{
    ...
    }; // FeatureDefinitionAdapter

    class FeatureDefinitionAdapter3 : public FeatureDefinition3
    {
    public :
        typedef std::unique_ptr<FeatureDefinition2, void(*)(FeatureDefinition2 *)> LegacyPtr;
    public:
        FeatureDefinitionAdapter3() NOTHROWS;
        FeatureDefinitionAdapter3(LegacyPtr &&impl) NOTHROWS;
    public :
        void reset(LegacyPtr &&impl) NOTHROWS;
    public:
        TAK::Engine::Util::TAKErr getRawGeometry(RawData *value) NOTHROWS override;
        ...
        TAK::Engine::Util::TAKErr getAttributes(const atakmap::util::AttributeSet **value) NOTHROWS override;
        TAK::Engine::Util::TAKErr get(const Feature2 **feature) NOTHROWS override;
        Traits getTraits() NOTHROWS override;
        AltitudeMode getAltitudeMode() NOTHROWS override;
        double getExtrude() NOTHROWS override;

    private :
        LegacyPtr impl;
    }; // FeatureDefinitionAdapter3
...
#pragma warning(disable : 4250)
    ...
    class ENGINE_API FeatureCursor3Adapter : public FilterFeatureCursor2, public FeatureCursor3
    {
    public:
        FeatureCursor3Adapter(FeatureCursorPtr&& impl_);
    public:
        Traits getTraits() NOTHROWS override;
    }; // FeatureCursor3Adapter
...
ENGINE_API TAKErr TAK::Engine::Feature::LegacyAdapters_adapt(FeatureDefinition3Ptr &value, FeatureDefinition2 &legacy) NOTHROWS
{
    FeatureDefinitionAdapter3::LegacyPtr legacyPtr(&legacy, Memory_leaker<FeatureDefinition2>);
    value = FeatureDefinition3Ptr(new FeatureDefinitionAdapter3(std::move(legacyPtr)), Memory_deleter_const<FeatureDefinition3, FeatureDefinitionAdapter3>);
    return TE_Ok;
}
...
TAKErr TAK::Engine::Feature::LegacyAdapters_adapt(FeatureCursor3Ptr &value, FeatureCursorPtr &legacy) NOTHROWS
{
    value = FeatureCursor3Ptr(new FeatureCursor3Adapter(std::move(legacy)), Memory_deleter_const<FeatureCursor3, FeatureCursor3Adapter>);
    return TE_Ok;
}
...
namespace
{
    ...
    FeatureDefinitionAdapter3::FeatureDefinitionAdapter3() NOTHROWS :
            impl(nullptr, nullptr)
    {}

    FeatureDefinitionAdapter3::FeatureDefinitionAdapter3(FeatureDefinitionAdapter3::LegacyPtr &&impl_) NOTHROWS :
            impl(std::move(impl_))
    {}

    void FeatureDefinitionAdapter3::reset(FeatureDefinitionAdapter3::LegacyPtr &&impl_) NOTHROWS
    {
        impl = std::move(impl_);
    }

    TAKErr FeatureDefinitionAdapter3::getRawGeometry(FeatureDefinition3::RawData *value) NOTHROWS
    {
        return impl->getRawGeometry(value);
    }

    FeatureDefinition3::GeometryEncoding FeatureDefinitionAdapter3::getGeomCoding() NOTHROWS
    {
        return impl->getGeomCoding();
    }

    TAKErr FeatureDefinitionAdapter3::getName(const char **value) NOTHROWS
    {
        return impl->getName(value);
    }

    FeatureDefinition3::StyleEncoding FeatureDefinitionAdapter3::getStyleCoding() NOTHROWS
    {
        return impl->getStyleCoding();
    }

    TAKErr FeatureDefinitionAdapter3::getRawStyle(RawData *value) NOTHROWS
    {
        return impl->getRawStyle(value);
    }

    TAKErr FeatureDefinitionAdapter3::getAttributes(const atakmap::util::AttributeSet **value) NOTHROWS
    {
        return impl->getAttributes(value);
    }

    TAKErr FeatureDefinitionAdapter3::get(const Feature2 **feature) NOTHROWS
    {
        return impl->get(feature);
    }

    Traits FeatureDefinitionAdapter3::getTraits() NOTHROWS
    {
        return Traits{ impl->getAltitudeMode(), impl->getExtrude() };
    }

    AltitudeMode FeatureDefinitionAdapter3::getAltitudeMode() NOTHROWS
    {
        return impl->getAltitudeMode();
    }

    double FeatureDefinitionAdapter3::getExtrude() NOTHROWS
    {
        return impl->getExtrude();
    }

    FeatureCursor3Adapter::FeatureCursor3Adapter(FeatureCursorPtr&& impl_) :
            FilterFeatureCursor2(std::move(impl_))
    {}

    Traits FeatureCursor3Adapter::getTraits() NOTHROWS
    {
        return Traits { impl->getAltitudeMode(), impl->getExtrude() };
    }
