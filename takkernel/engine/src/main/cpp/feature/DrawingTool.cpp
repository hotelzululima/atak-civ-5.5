...
extractValue (const char* value,
  ...
  {
    char* endPtr;
    result = std::strtof(value, &endPtr);
...
const char*
extractValue (const char* value,
              feature::Pen::ExtrusionMode& result)
    {
      result = feature::Pen::VERTEX;
      if(strncmp(value, "no", 2) == 0) {
        result = feature::Pen::NONE;
      } else if(strncmp(value, "ep", 2) == 0) {
        result = feature::Pen::ENDPOINT;
      }
...
const char*
extractValue (const char* value,
              feature::Pen::ArrowHeadMode& result)
    {
      result = feature::Pen::ONLYLAST;
      if(strncmp(value, "pv", 2) == 0) {
        result = feature::Pen::PERVERTEX;
      }
      return value + strlen(value);
    }
              ...
              size_t size)
{
    ...
    size_t read;
    size_t count = 0;
    while(std::getline(ss, segment, ' ') && count < size)
    {
        float val = std::stof(segment, &read);
        if (read == 0) break;
        result[count++] = val;
    }
    ...
    if (count != 16) {
        std::ostringstream msg;
        msg << MEM_FN ("extractValue") <<
            "Invalid Symbol::Transform value: " <<
            value;
        throw std::invalid_argument(msg.str());
    }

    return value + ss.str().length();
}
...
Pen::Pen()
    ...
    priority(1),
    arrowRadius(0.f),
    extrusionMode(ExtrusionMode::VERTEX),
    arrowHeadMode(ArrowHeadMode::ONLYLAST),
    minLod(-1),
    maxLod(-1)
{ }
...
DrawingTool::parseOGR_Color (const char* color)
  {
    ...
    if ((len != 7 && len != 9)
        || *color != '#'
        || std::find_if (color + 1, color + len, [](const char c) { return !strchr("0123456789ABCDEFabcdef", c); })
           != color + len)
      {
          std::ostringstream msg;
               msg <<
...
const char* const Brush::BRUSH_MINIMUM_LOD ("nd");
const char* const Brush::BRUSH_MAXIMUM_LOD ("xd");
...
Brush::setParam (const char* key,
                 const char* value)
  {
    ...
    if (!std::strcmp (key, BRUSH_FORE_COLOR))
      ...
      // libtakengine additions
    else if (!std::strcmp (key, BRUSH_MINIMUM_LOD))
      {
        extractValue (value, minLod);
      }
    else if (!std::strcmp (key, BRUSH_MAXIMUM_LOD))
      {
        extractValue (value, maxLod);
      }
  }
...
const char* const Label::LABEL_MINIMUM_LOD ("nd");
const char* const Label::LABEL_MAXIMUM_LOD ("xd");
...
Label::setParam (const char* key,
                 const char* value)
  {
    ...
    else if (!std::strcmp (key, LABEL_MINIMUM_LOD))
      {
        extractValue (value, minLod);
      }
    else if (!std::strcmp (key, LABEL_MAXIMUM_LOD))
      {
        extractValue (value, maxLod);
      }
  }
...
const char* const Pen::PEN_ARROW_RADIUS ("ar");
const char* const Pen::PEN_PRIORITY_LEVEL ("l");
const char* const Pen::PEN_EXTRUSION_MODE ("ex");
// libtakengine additions
const char* const Pen::PEN_ARROW_HEAD_MODE ("ah");
const char* const Pen::PEN_MINIMUM_LOD ("nd");
const char* const Pen::PEN_MAXIMUM_LOD ("xd");
...
Pen::setParam (const char* key,
               const char* value)
  {
    ...
    else if (!std::strcmp (key, PEN_ARROW_RADIUS))
      {
        extractValue (value, arrowRadius);
      }
    else if (!std::strcmp (key, PEN_PRIORITY_LEVEL))
      {
        extractValue (value, priority);
      }
    else if (!std::strcmp (key, PEN_EXTRUSION_MODE))
      {
        extractValue (value, extrusionMode);
      }
    else if (!std::strcmp (key, PEN_ARROW_HEAD_MODE))
      {
        extractValue(value, arrowHeadMode);
      }
    // libtakengine additions
    else if (!std::strcmp (key, PEN_MINIMUM_LOD))
      {
        extractValue (value, minLod);
      }
    else if (!std::strcmp (key, PEN_MAXIMUM_LOD))
      {
        extractValue (value, maxLod);
      }
  }
...
const char* const Symbol::SYMBOL_MINIMUM_LOD ("nd");
const char* const Symbol::SYMBOL_MAXIMUM_LOD ("xd");
const char* const Symbol::SYMBOL_TRANSFORM ("tf");
...
Symbol::setParam (const char* key,
                  const char* value)
  {
    ...
    else if (!std::strcmp (key, SYMBOL_MINIMUM_LOD))
      {
        extractValue (value, minLod);
      }
    else if (!std::strcmp (key, SYMBOL_MAXIMUM_LOD))
      {
        extractValue (value, maxLod);
      }
    else if (!std::strcmp (key, SYMBOL_TRANSFORM))
    {
        extractValue (value, transform, 16);
    }
  }
