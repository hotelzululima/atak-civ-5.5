...
TAKErr TAK::Engine::Feature::FeatureDataSourceFactory_parse(FeatureDataSource2::ContentPtr &content, const char *path, const char *hint) NOTHROWS
{
    ...
    if (hint) {
        ...
        {
            // miss -- do a case insensitive lookup
            for(it = featureDataSourceMap.begin(); it != featureDataSourceMap.end(); it++)
            {
                if(TAK::Engine::Port::String_strcasecmp(hint, it->first) == 0)
                {
                    spi = it->second;
                    if (spi.get() != nullptr)
...
TAKErr TAK::Engine::Feature::FeatureDataSourceFactory_registerProvider(const std::shared_ptr<FeatureDataSource2> &provider, const int priority) NOTHROWS
{
    ...
    FeatureDataSourceMap &featureDataSourceMap = getProviderMap();
    {
        //if entry is overshadowed, remove the previous from the ProviderPriorityVector
        FeatureDataSourceMap::iterator entry = featureDataSourceMap.find(provider->getName());
        if(entry != featureDataSourceMap.end()) {
            const FeatureDataSource2 & prov = *entry->second.get();
			for (FeatureDataSourcePriorityVector::iterator it = priorityQueue.begin(); it != priorityQueue.end(); it++) {
                if (it->second.get() == entry->second.get()) {
