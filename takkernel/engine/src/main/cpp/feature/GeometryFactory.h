    ...
    namespace Engine {
        namespace Feature {
            ...
            */
            ...
            /**
             * @param cmd   The current instruction
             * @param type  If the command is `TEGF_StartGeometry`, the type of the new geometry
             * @param dim   If the command is `TEGF_StartGeometry`, the dimension of the new geometry
             * @param count If the command is `TEGF_StartGeometry`, the number of _components_ for the new geometry
             *              <UL>
             *                <LI>Number of child geometries for `TEGC_GeometryCollection`</LI>
             *                <LI>Number of rings for `TEGC_Polygon`</LI>
             *                <LI>Number of points for `TEGC_LineString`</LI>
             *              </UL>
             * @param xyz   If the command is `TEGF_Point`, the value of the point
             * @return  client specific; return code will be returned via enclosing function
             */
            typedef std::function<Util::TAKErr(const GeometryParseCommand cmd, const GeometryClass type, const std::size_t dim, const std::size_t count, const Feature::Point2 &xyz)> GeometryParseCallback;
            ...
            ENGINE_API Util::TAKErr GeometryFactory_fromSpatiaLiteBlob(Util::DataInput2 &src, int *srid, const GeometryParseCallback &cb) NOTHROWS;
            ENGINE_API Util::TAKErr GeometryFactory_fromWkb(Util::DataInput2 &src, const GeometryParseCallback &cb) NOTHROWS;

            ENGINE_API Util::TAKErr GeometryFactory_getWkbGeometryType(GeometryClass *type, std::size_t *dim, const uint8_t *srcData, const std::size_t srcDataLen) NOTHROWS;
            ENGINE_API Util::TAKErr GeometryFactory_getWkbGeometryType(GeometryClass *type, std::size_t *dim, Util::DataInput2 &src)
                NOTHROWS;

            ENGINE_API Util::TAKErr GeometryFactory_getSpatiaLiteBlobGeometryType(GeometryClass *type, std::size_t *dim, const uint8_t *srcData, const std::size_t srcDataLen) NOTHROWS;
            ENGINE_API Util::TAKErr GeometryFactory_getSpatiaLiteBlobGeometryType(GeometryClass *type, std::size_t *dim, Util::DataInput2 &src)
                NOTHROWS;
            ...
            **/
            ...
            /**
            * Creates a wedge geometry based on a center, start / end angle, and range.
            * @param value Returns the Wedge geometry on success
            * @param location  the center point of the wedge geometry
            * @param startAngle start angle of the wedge
            * @param endAngle end angle of the wedge
            * @param algo Distance, direction, and interpolation functions in chosen basis
            **/
            ENGINE_API Util::TAKErr  GeometryFactory_createWedge(Geometry2Ptr& value, const Math::Point2<double>& location, const double startAngle, const double endAngle, const double range, const Renderer::Algorithm& algo) NOTHROWS;

            /**
             * Create a geometry based on an Envelope
             * @param value Returns the generated geometry on success
             * @param e The envelope
             */
            ENGINE_API Util::TAKErr GeometryFactory_fromEnvelope(Geometry2Ptr &value, const Envelope2 &e) NOTHROWS;
        }
    }
}
