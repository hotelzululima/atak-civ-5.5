...
namespace
{
    ...
    TAKErr openDataStore(FeatureDataStore2Ptr& value, const char *file, const char* fdbpath, const char* type, const unsigned int modflags, const unsigned int visflags, bool readOnly) NOTHROWS
    {
        TAKErr code(TE_Ok);
        if (!fdbpath)
        ...
        // open as FDB
        if (!value) {
            std::unique_ptr<FeatureSetDatabase> fdb(new(std::nothrow) FeatureSetDatabase(modflags, visflags, readOnly));
            if (!fdb)
                return TE_OutOfMemory;
    ...
    {
        return openDataStore(value, file, fdbpath, type, modflags, visflags, false);
    }
...
PersistentDataSourceFeatureDataStore2::PersistentDataSourceFeatureDataStore2() NOTHROWS :
    PersistentDataSourceFeatureDataStore2(false)
{}

PersistentDataSourceFeatureDataStore2::PersistentDataSourceFeatureDataStore2(bool readOnly) NOTHROWS :
...

    {
        ...
        for (dbIter = dbs.begin(); dbIter != dbs.end(); dbIter++) {
            std::unique_ptr<FeatureSetDatabase, void (*)(const FeatureDataStore2 *)> fdbImpl(
                new FeatureSetDatabase(modificationFlags, visibilityFlags, read_only_), Memory_deleter_const<FeatureDataStore2, FeatureSetDatabase>);
...
TAKErr PersistentDataSourceFeatureDataStore2::queryFeatures(FeatureCursorPtr &result, const char *file) NOTHROWS
{
    ...
    TE_CHECKRETURN_CODE(code);
    if(params.featureSetIds->empty()) {
        code = params.featureSetIds->add(FEATURESET_ID_NONE);
        TE_CHECKRETURN_CODE(code);
    }
...
TAKErr PersistentDataSourceFeatureDataStore2::queryFeatureSets(FeatureSetCursorPtr &result, const char *file) NOTHROWS
{
    ...
    TE_CHECKRETURN_CODE(code);
    if(params.ids->empty()) {
        code = params.ids->add(FEATURESET_ID_NONE);
        TE_CHECKRETURN_CODE(code);
    }
...
TAKErr PersistentDataSourceFeatureDataStore2::insertFeatureSet(FeatureSetPtr_const *featureSet, const char *file, const char *name, const double minResolution, const double maxResolution) NOTHROWS
{
    ...
    if (entry == this->fsid_to_feature_db_.end()) {
        ...
        FeatureDataStore2Ptr fdbImpl(
                new FeatureSetDatabase(modificationFlags, visibilityFlags, read_only_),
...
TAKErr PersistentDataSourceFeatureDataStore2::setReadOnly(bool value) NOTHROWS
{
    TAKErr code(TE_Ok);
    read_only_ = value;
    return code;
