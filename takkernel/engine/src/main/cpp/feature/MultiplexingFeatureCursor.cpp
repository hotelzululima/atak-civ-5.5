...
TAKErr MultiplexingFeatureCursor::moveToNext() NOTHROWS
{
    ...
    const std::size_t invalidLimit = this->invalid.size();
    for(std::size_t i = 0u; i < invalidLimit; i++) {
        auto &cursor = *this->invalid[i];
    ...
    if (this->comp) {
        ...
        std::sort(this->pendingResults.begin(), this->pendingResults.end(), [&](FeatureCursor2* a, FeatureCursor2* b)
        {
            return !(*this->comp)(orderArg, *a, *b);
        });
    }
