    ...
    namespace Engine {
        namespace Feature {
            ...
            private :
                ...
                FDB(int modificationFlags, int visibilityFlags, bool readOnly) NOTHROWS;
            ...
            protected:
                ...
                virtual Util::TAKErr insertFeatureSetImpl(FeatureSetPtr_const *ref, const char *provider, const char *type, const char *name, const double minResolution, const double maxResolution, const bool readOnly) NOTHROWS;
            ...
            private :
                ...
                virtual Util::TAKErr insertFeatureSetImpl(const int64_t fsid, const char *provider, const char *type, const char *name, const double minResolution, const double maxResolution, const bool readOnly) NOTHROWS;
            protected :
            ...
            public :
                ...
                FeatureCursorImpl(FDB &owner, DB::QueryPtr &&filter, const int idCol, const int fsidCol, const int versionCol, const int nameCol, const int geomCol, const int originalGeomCol, const int styleCol, const int attribsCol, const int altitudeModeCol, const int extrudeCol) NOTHROWS;
            public: // FeatureCursor2
            ...
            private:
                struct LT_featureSetName
                {
                    bool operator()(const std::shared_ptr<const FeatureSetDefn> &a, const std::shared_ptr<const FeatureSetDefn> &b)
                    {
                         do {
                            if (a->name && b->name) {
                                const int cmp = TAK::Engine::Port::String_strcasecmp(a->name, b->name);
                                if(cmp)
                                    return (cmp<0);
                            } else if (a->name) {
                                return true;
                        ...
                        // names are equal, deconflict on FSID
