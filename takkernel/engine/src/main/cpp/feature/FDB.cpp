...
FDB::FDB(int modificationFlags, int visibilityFlags) NOTHROWS :
     FDB(modificationFlags, visibilityFlags, false)
{}

FDB::FDB(int modificationFlags, int visibilityFlags, bool readOnly) NOTHROWS :
...
TAKErr FDB::open(const char *path, int* dbVersion, bool buildIndices) NOTHROWS
{
    ...
    code = this->database_->query(journalCursor, "pragma journal_mode=wal");
    TE_CHECKRETURN_CODE(code);
...
TAKErr FDB::queryFeatures(FeatureCursorPtr &result, const FeatureQueryParameters &params) NOTHROWS
{
    ...
    if (!MathUtils_hasBits(ignoredFields, FeatureQueryParameters::GeometryField)) {
        if (!params.ops->empty()) {
            ...
            do {
                ...
                if (op.type == FeatureQueryParameters::SpatialOp::Simplify) {
                    insert(geomStr, "features.geometry, SimplifyPreserveTopology(");
                    geomStr << ", ?)";

                    args.push_back(BindArgument(op.args.simplify.distance));
                    originalGeomCol = extrasCol++;
        ...
        if (originalGeomCol < 0)
            originalGeomCol = geomCol;
    }
    ...
    if (!TE_ISNAN(params.minResolution) || !TE_ISNAN(params.maxResolution)) {
        this->validateInfo();
    ...
    if (!fsCheck.empty()) {
        ...
        for (fsCheckIter = fsCheck.begin(); fsCheckIter != fsCheck.end(); fsCheckIter++) {
            ...
            retval.push_back(new FeatureCursorImpl(*this, std::move(queryResult), idCol, fsidCol, versionCol, nameCol, geomCol, originalGeomCol, styleCol,
                                                   attribsCol, altitudeModeCol, extrudeCol));
            queryResult.reset();
        }
    }
...
TAKErr FDB::setFeaturesVisibleImpl(const FeatureQueryParameters &paramsRef, const bool visible) NOTHROWS
{
    ...
    if (!params ||
        ...
        TE_ISNAN(params->minResolution) &&
        TE_ISNAN(params->maxResolution))) {
...
{
    return this->insertFeatureSetImpl(returnRef, provider, type, name, minResolution, maxResolution, false);
}

TAKErr FDB::insertFeatureSetImpl(FeatureSetPtr_const *returnRef, const char *provider, const char *type, const char *name, const double minResolution, const double maxResolution, const bool readOnly) NOTHROWS
{
    using namespace atakmap::raster::osm;
    ...
    StatementPtr stmt(nullptr, nullptr);
    {
        ...
        TE_CHECKRETURN_CODE(code);
        code = stmt->bindInt(6, readOnly ? 1 : 0);
        TE_CHECKRETURN_CODE(code);
...
{
    return this->insertFeatureSetImpl(fsid, provider, type, name, minResolution, maxResolution, false);
}

TAKErr FDB::insertFeatureSetImpl(const int64_t fsid, const char *provider, const char *type, const char *name, const double minResolution, const double maxResolution, const bool readOnly) NOTHROWS
{
    using namespace atakmap::raster::osm;
    ...
    TE_CHECKRETURN_CODE(code);
    code = stmt->bindInt(7, readOnly ? 1 : 0);
    TE_CHECKRETURN_CODE(code);
...
TAKErr FDB::buildParamsWhereClauseNoCheck(bool *emptyResults, const FeatureQueryParameters &params, Collection<std::shared_ptr<const FeatureSetDefn>> &fs, WhereClauseBuilder2 &whereClause) NOTHROWS
{
    ...
    if (!TE_ISNAN(params.maxResolution) || !TE_ISNAN(params.minResolution)) {
        // handled upstream
    }
    if (params.visibleOnly) {
        // handled upstream
...
FDB::FeatureCursorImpl::FeatureCursorImpl(FDB& owner_, QueryPtr&& filter_, const int idCol_, const int fsidCol_, const int versionCol_,
    ...
    const int altitudeModeCol_, const int extrudeCol_) NOTHROWS
    : FeatureCursorImpl(owner_, std::move(filter_), idCol_, fsidCol_, versionCol_, nameCol_, geomCol_, geomCol_, styleCol_, attribsCol_, altitudeModeCol_, extrudeCol_) {}

FDB::FeatureCursorImpl::FeatureCursorImpl(FDB &owner_, QueryPtr &&filter_, const int idCol_, const int fsidCol_, const int versionCol_,
                                          const int nameCol_, const int geomCol_, const int originalGeomCol_, const int styleCol_, const int attribsCol_,
                                          const int altitudeModeCol_, const int extrudeCol_) NOTHROWS : CursorWrapper2(std::move(filter_)),
                                                                                                        ...
                                                                                                        nameCol(nameCol_),
                                                                                                        geomCol(geomCol_),
                                                                                                        originalGeomCol(originalGeomCol_),
                                                                                                        styleCol(styleCol_),
...
TAKErr FDB::FeatureCursorImpl::getRawGeometry(FeatureDefinition2::RawData *value) NOTHROWS
{
    ...
    return this->filter->getBlob(&value->binary.value, &value->binary.len,
             (getAltitudeMode() == TEAM_ClampToGround) ? this->geomCol : this->originalGeomCol);
}
...
{
    return this->insertFeatureSet(fsid, provider, type, name, minResolution, maxResolution, false);
}

TAKErr FDB::Builder::insertFeatureSet(int64_t *fsid, const char *provider, const char *type, const char *name, const double minResolution, const double maxResolution, const bool readOnly) NOTHROWS
{
    TAKErr code;
    FeatureSetPtr_const inserted(nullptr, nullptr);

    code = db.insertFeatureSetImpl(&inserted, provider, type, name, minResolution, maxResolution, readOnly);
    TE_CHECKRETURN_CODE(code);
...
{
    return this->insertFeatureSet(fsid, provider, type, name, false);
}

TAKErr FDB::Builder::insertFeatureSet(const int64_t fsid, const char *provider, const char *type, const char *name, const bool readOnly) NOTHROWS
{
    return db.insertFeatureSetImpl(fsid, provider, type, name, std::numeric_limits<double>::max(), 0.0, readOnly);
}
