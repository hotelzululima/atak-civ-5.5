...
namespace
{
    ...
    template<class Ht, class Pt, class It>
    TAKErr meshFromHeightmap(MeshPtr &value, const Ht *heightmap, const std::size_t tileWidth, const std::size_t tileHeight, const Envelope2 &tileBounds_, const HeightmapNoData noData, const TAK::Engine::Port::DataType posType) {
        ...
        switch(sizeof(It)) {
            case 1u :
                indexType = TEDT_UInt8;
                break;
            case 2u :
                indexType = TEDT_UInt16;
                break;
            case 4u :
                indexType = TEDT_UInt32;
                break;
            default :
                return TE_InvalidArg;
        }
        auto tileBounds(tileBounds_);
        const auto cellWidth = (tileBounds.maxX-tileBounds.minX) / (double)(tileWidth-1u);
        const auto cellHeight = (tileBounds.maxY-tileBounds.minY) / (double)(tileHeight-1u);

        const auto numVertices = tileWidth*tileHeight;

        VertexDataLayout layout;
        layout.attributes = VertexAttribute::TEVA_Position;
        layout.interleaved = false;
        layout.position.type = posType;
        layout.position.stride = TAK::Engine::Port::DataType_size(posType)*3;

        // vertex data
        std::vector<Pt> positions(numVertices*3u);
        auto xyz = positions.data();
        tileBounds.minZ = (double)heightmap[0];
        tileBounds.maxZ = tileBounds.minZ;
        for(std::size_t y = 0; y < tileHeight; y++) {
            for (std::size_t x = 0; x < tileWidth; x++) {
                auto height = (Pt)heightmap[(y * tileWidth) + x];
                if(TE_ISNAN(height))
                    height = (Pt)0;
                *xyz++ = (Pt)(tileBounds.minX + ((double)x * cellWidth));
                *xyz++ = (Pt)(tileBounds.maxY - ((double)y * cellHeight));
                *xyz++ = height;

                if(height < tileBounds.minZ)
                    tileBounds.minZ = height;
                else if(height > tileBounds.maxZ)
                    tileBounds.maxZ = height;
            }
        }

        // triangle indices
        std::vector<It> indices((tileWidth-1u)*(tileHeight-1u)*6u);
        auto idx = indices.data();
        std::size_t numIndices = 0u;
        for(int y = 0; y < tileHeight-1; y++) {
            for(int x = 0; x < tileWidth-1; x++) {
                // skip any triangles with NAN height

                // using CCW wind order

                // a---b
                // |\  |
                // | \ |
                // c---d
                const auto a = (y*tileWidth)+x;
                const auto b = (y*tileWidth)+(x+1);
                const auto c = ((y+1)*tileWidth)+x;
                const auto d = ((y+1)*tileWidth)+(x+1);
                if((noData == TEND_Zero) || (!TE_ISNAN(heightmap[a]) && !TE_ISNAN(heightmap[d]) && !TE_ISNAN(heightmap[c]))) {
                    *idx++ = (It)a;
                    *idx++ = (It)c;
                    *idx++ = (It)d;
                    numIndices += 3;
                }
                if((noData == TEND_Zero) || (!TE_ISNAN(heightmap[b]) && !TE_ISNAN(heightmap[d]) && !TE_ISNAN(heightmap[a]))) {
                    *idx++ = (It)a;
                    *idx++ = (It)d;
                    *idx++ = (It)b;
                }
        ...
        return MeshBuilder_buildNonInterleavedMesh(
                ...
                0u, nullptr,
                tileBounds,
                numVertices, positions.data(), nullptr, nullptr, nullptr,
                indexType, indices.size(), indices.data());
    }
    template<class T>
    TAKErr meshFromHeightmap(MeshPtr &value, const T *heightmap, const std::size_t tileWidth, const std::size_t tileHeight, const Envelope2 &tileBounds, const HeightmapNoData noData, const TAK::Engine::Port::DataType posType) NOTHROWS
    {
        const auto numVertices = tileWidth*tileHeight;
        switch(posType) {
            case TEDT_Float32 :
                return (numVertices <= 0xFFFFu) ?
                    meshFromHeightmap<T, float, uint16_t>(value, heightmap, tileWidth, tileHeight, tileBounds, noData, posType) :
                    meshFromHeightmap<T, float, uint32_t>(value, heightmap, tileWidth, tileHeight, tileBounds, noData, posType);
            case TEDT_Float64 :
                return (numVertices <= 0xFFFFu) ?
                       meshFromHeightmap<T, double, uint16_t>(value, heightmap, tileWidth, tileHeight, tileBounds, noData, posType) :
                       meshFromHeightmap<T, double, uint32_t>(value, heightmap, tileWidth, tileHeight, tileBounds, noData, posType);                break;
            default :
                return TE_InvalidArg;
        }
    }
...
TAKErr TAK::Engine::Elevation::ElevationChunkFactory_meshFromHeightmap(MeshPtr &value, const float *heightmap, const std::size_t tileWidth, const std::size_t tileHeight, const Envelope2 &tileBounds, const HeightmapNoData noData, const TAK::Engine::Port::DataType posType) NOTHROWS
{
    return meshFromHeightmap<float>(value, heightmap, tileWidth, tileHeight, tileBounds, noData, posType);
}
TAKErr TAK::Engine::Elevation::ElevationChunkFactory_meshFromHeightmap(MeshPtr &value, const double *heightmap, const std::size_t tileWidth, const std::size_t tileHeight, const Envelope2 &tileBounds, const HeightmapNoData noData, const TAK::Engine::Port::DataType posType) NOTHROWS
{
    return meshFromHeightmap<double>(value, heightmap, tileWidth, tileHeight, tileBounds, noData, posType);
}
