...
namespace
{
    ...
    {
        CursorComparator(std::vector<bool(*)(ElevationChunkCursor &, ElevationChunkCursor &) NOTHROWS> &impl_) :
            impl(impl_)
        {}
...
MultiplexingElevationChunkCursor::MultiplexingElevationChunkCursor(Collection<std::shared_ptr<ElevationChunkCursor>> &cursors_, bool(*order_)(ElevationChunkCursor &, ElevationChunkCursor &) NOTHROWS) NOTHROWS
{
    do {
        TAKErr code(TE_Ok);
        if (!order_) {
...
MultiplexingElevationChunkCursor::MultiplexingElevationChunkCursor(Collection<std::shared_ptr<ElevationChunkCursor>> &cursors_, Collection<bool(*)(ElevationChunkCursor &, ElevationChunkCursor &) NOTHROWS> &order_) NOTHROWS
{
    do {
        TAKErr code(TE_Ok);
        if (order_.empty()) {
            ...
            Collection<bool(*)(ElevationChunkCursor &, ElevationChunkCursor &) NOTHROWS>::IteratorPtr iter(nullptr, nullptr);
            code = order_.iterator(iter);
            TE_CHECKBREAK_CODE(code);

            do {
                bool (*element)(ElevationChunkCursor &, ElevationChunkCursor &) NOTHROWS;
                code = iter->get(element);
                TE_CHECKBREAK_CODE(code);
