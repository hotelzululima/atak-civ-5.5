...
namespace TAK {
    namespace Engine {
        namespace Renderer {
            ...
            template<class T>
            Util::TAKErr Tessellate_linestring(TessellateCallback &value, const VertexData &src, const std::size_t count, const double threshold, Algorithm &algorithm, const bool *cancelToken = nullptr) NOTHROWS
            {
                if(!src.data)
             ...
             */
            template<class T>
            Util::TAKErr Tessellate_linestring(VertexDataPtr& value, std::size_t* dstCount, const VertexData& src, const std::size_t count, const double threshold, Algorithm& algorithm, const bool *cancelToken = nullptr) NOTHROWS
            {
                Util::TAKErr code(Util::TE_Ok);
                *dstCount = count;
...
#define __TE_TESSELLATE_COMPUTE_DST_COUNT(axv, ayv, azv, bxv, byv, bzv) \
    for(std::size_t i = 1u; i < count; i++) {                           \
        if(cancelToken && *cancelToken) return Util::TE_Interrupted; \
        const T *srca = reinterpret_cast<const T *>(srcData + (src.stride*(i-1u))); \
...
#undef __TE_TESSELLATE_COMPUTE_DST_COUNT
                ...
                if (src.size == 2u) {
                    ...
                    return Tessellate_linestring<T>(cb, src, count, threshold, algorithm, cancelToken);
                } else if (src.size == 3u) {
            ...
            ENGINE_API Util::TAKErr Tessellate_polygon(VertexDataPtr &value, std::size_t *dstCount, const VertexData &src, const std::size_t count, const double threshold, Algorithm &algorithm, ReadVertexFn vertRead, WriteVertexFn vertWrite, const bool *cancelToken = nullptr) NOTHROWS;
            ENGINE_API Util::TAKErr Tessellate_polygon(TessellateCallback &value, const VertexData &src, const std::size_t count, const double threshold, Algorithm &algorithm, ReadVertexFn vertRead, const bool *cancelToken = nullptr) NOTHROWS;
            ENGINE_API Util::TAKErr Tessellate_polygon(VertexDataPtr &value, std::size_t *dstCount, const VertexData &src, const int *counts, const int *startIndices, const int numPolygons, const double threshold, Algorithm &algorithm, ReadVertexFn vertRead, WriteVertexFn vertWrite, const bool *cancelToken = nullptr) NOTHROWS;
            ENGINE_API Util::TAKErr Tessellate_polygon(TessellateCallback &value, const VertexData &src, const int *counts, const int *startIndices, const int numPolygons, const double threshold, Algorithm &algorithm, ReadVertexFn vertRead, const bool *cancelToken = nullptr) NOTHROWS;

            template<class T>
            Util::TAKErr Tessellate_polygon(TessellateCallback &value, const VertexData& src, const std::size_t count, const double threshold, Algorithm& algorithm, const bool *cancelToken) NOTHROWS
            {
                if(sizeof(T)*src.size > src.stride)
                    return Util::TE_InvalidArg;
                ...
                return Tessellate_polygon(value, src, count, threshold, algorithm, readV, cancelToken);
            }

            template<class T>
            Util::TAKErr Tessellate_polygon(VertexDataPtr &value, std::size_t *dstCount, const VertexData &src, const std::size_t count, const double threshold, Algorithm &algorithm, const bool *cancelToken = nullptr) NOTHROWS
            {
                ...
                code = Tessellate_polygon(countcb, src, count, threshold, algorithm, readV, cancelToken);
                TE_CHECKRETURN_CODE(code);
                ...
                code = Tessellate_polygon(assemblecb, src, count, threshold, algorithm, readV, cancelToken);
                TE_CHECKRETURN_CODE(code);
            ...
            template<class T>
            Util::TAKErr Tessellate_polygon(TessellateCallback &value, const VertexData &src, const int *counts, const int *startIndices, const int numPolygons, const double threshold, Algorithm &algorithm, const bool *cancelToken = nullptr) NOTHROWS
            {
                if(sizeof(T)*src.size > src.stride)
                    return Util::TE_InvalidArg;
                ...
                return Tessellate_polygon(value, src, counts, startIndices, numPolygons, threshold, algorithm, readV, cancelToken);
            }
            template<class T>
            Util::TAKErr Tessellate_polygon(VertexDataPtr &value, std::size_t *dstCount, const VertexData &src, const int *counts, const int *startIndices, const int numPolygons, const double threshold, Algorithm &algorithm, const bool *cancelToken = nullptr) NOTHROWS
            {
                Util::TAKErr code(Util::TE_Ok);
                struct Count : public TessellateCallback
                {
                ...
                code = Tessellate_polygon<T>(countcb, src, counts, startIndices, numPolygons, threshold, algorithm, cancelToken);
                TE_CHECKRETURN_CODE(code);
                ...
                code = Tessellate_polygon<T>(assemblecb, src, counts, startIndices, numPolygons, threshold, algorithm, cancelToken);
                TE_CHECKRETURN_CODE(code);
            ...
            Util::TAKErr Tessellate_polygon(TessellateCallback &callback, const VertexData *polygons, const int *polygonSizes, const int polygonCount, const size_t totalVertexCount, const double threshold, Algorithm &algorithm, ReadVertexFn vertRead, const bool *cancelToken);
             ...
             */
            template<typename T>
            Util::TAKErr Tessellate_polygon(TessellateCallback &callback, const VertexData *polygons, const int *polygonSizes, const int polygonCount, const double threshold, Algorithm &algorithm, const bool *cancelToken = nullptr) NOTHROWS
            {
                ...
                auto code = Tessellate_polygon(callback, polygons, polygonSizes, polygonCount, totalVertexCount, threshold, algorithm, readV, cancelToken);
                return code;
            }
        }
    }
