...
#define CHECKERRS()
        ...
        static TAK::Engine::Renderer::RenderContextAssociatedCache<GLES20FixedPipeline*>
            local_rc_singletons;
        ...
        GLES20FixedPipeline *GLES20FixedPipeline ::getInstance()
        {
            ...
            GLES20FixedPipeline*& result = local_rc_singletons.get(renderContext);
            if (!result) {
                result = new GLES20FixedPipeline();
            }
            return result;
        }
        ...
        GLES20FixedPipeline::~GLES20FixedPipeline()
        {
            ...
            for (iter = texturePrograms.begin(); iter != texturePrograms.end(); ++iter)
                delete iter->second;
            for (iter = pointPrograms.begin(); iter != pointPrograms.end(); ++iter)
                delete iter->second;
        }
