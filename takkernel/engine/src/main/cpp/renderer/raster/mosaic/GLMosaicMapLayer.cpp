...
namespace {
    ...
    struct MosaicPendingData : public GLAsynchronousMapRenderable3::QueryContext
    {
        ...
        ThreadPoolPtr frameLoaderWorkers{ nullptr, nullptr };
        ...
        struct {
            ...
            struct {
                unsigned count{ 0u };
                std::list<MosaicDatabase2::Frame> value;
            } requests;
        } frameLoadContext;

        MosaicPendingData(GLMosaicMapLayer &owner);
    ...
    void *frameLoaderThread(void* opaque) NOTHROWS;
}
...
GLMosaicMapLayer::GLMosaicMapLayer(TAK::Engine::Core::RenderContext *context, MosaicDatasetDescriptor *info) NOTHROWS
    : asyncio(nullptr),
      ...
      resolution_select_mode_(ImagerySelectionControl::Mode::MaximumResolution),
...
GLMosaicMapLayer::GLMosaicMapLayer(TAK::Engine::Core::MapRenderer *renderer, MosaicDatasetDescriptor *info) NOTHROWS :
    GLMosaicMapLayer(&static_cast<Core::GLGlobeBase *>(renderer)->context, info)
{}
...
Util::TAKErr GLMosaicMapLayer::createQueryContext(QueryContextPtr &value) NOTHROWS
{
    ...
    std::unique_ptr<MosaicPendingData, void (*)(const QueryContext *)> retval(new MosaicPendingData(*this),
...
TAKErr GLMosaicMapLayer::constructQueryParams(std::list<MosaicDatabase2::QueryParameters> *retval, const GLGlobeBase::State &localQuery) NOTHROWS
{
    ...
    int maxZoom = 0;
    std::map<int, TAK::Engine::Feature::Envelope2> regions;
    for (std::size_t i = 0u; i < target_state_.surfaceBounds.count; i++) {
        auto region = target_state_.surfaceBounds.value[i];
        if (!TAK::Engine::Math::Rectangle2_intersects(
            region.minX, region.minY,
            region.maxX, region.maxY,
            localQuery.upperLeft.longitude, localQuery.lowerRight.latitude,
            localQuery.lowerRight.longitude, localQuery.upperLeft.latitude,
            false)) {

            continue;
        }

        const auto zoom = (int)(log(180.0 / (region.maxY - region.minY)) / log(2.0));
        if (zoom > maxZoom)
            maxZoom = zoom;
        
        auto entry = regions.find(zoom);
        if(entry == regions.end()) {
            regions[zoom] = region;
        } else {
            entry->second = TAK::Engine::Feature::SpatialCalculator_union(entry->second, region);
        }
    }

    for (auto it = regions.rbegin(); it != regions.rend(); it++) {
        Feature::GeometryPtr geo(
            DatasetDescriptor::createSimpleCoverage(
                GeoPoint2(it->second.maxY, it->second.minX),
                GeoPoint2(it->second.maxY, it->second.maxX),
                GeoPoint2(it->second.minY, it->second.maxX),
                GeoPoint2(it->second.minY, it->second.minX)),
            atakmap::feature::destructGeometry);
        code = Feature::LegacyAdapters_adapt(base.spatialFilter, *geo);
        TE_CHECKRETURN_CODE(code);

        const auto surfaceTileGsd = atakmap::raster::osm::OSMUtils::mapnikTileResolution(it->first) / 8.0;
        const auto gsd = (it->first == maxZoom) ?
            std::max(surfaceTileGsd / 2.0, localQuery.drawMapResolution) :
            localQuery.drawMapResolution;
...
namespace
{
    class PFPSDbg {
    public :
        PFPSDbg(const char* msg) :
            _timerStart(TAK::Engine::Port::Platform_systime_millis()),
            _msg(msg)
        {
            Count = -1;
        }
        ~PFPSDbg()
        {
            static const bool dopfpsdb = TAK::Engine::Util::ConfigOptions_getIntOptionOrDefault("pfps.debug", 0) != 0;
            if (dopfpsdb) {
                const auto timerStop = TAK::Engine::Port::Platform_systime_millis();
                unsigned ms = (unsigned)(timerStop-_timerStart);
                if(Count >= 0)
                    TAK::Engine::Util::Logger_log(TAK::Engine::Util::TELL_Info, "%s {count=%d} in %ums", _msg, Count, ms);
                else
                    TAK::Engine::Util::Logger_log(TAK::Engine::Util::TELL_Info, "%s in %ums", _msg, ms);
            }
        }
    public:
        int Count;
...
TAKErr GLMosaicMapLayer::queryImpl(QueryContext &retvalqc, const GLGlobeBase::State &localQuery) NOTHROWS
{
    ...
    PFPSDbg pfps_dbg("GLMosaicMapLayer::queryImpl");
    ...
    assert(retval.frameLoadContext.requests.value.empty());
    assert(retval.frameLoadContext.requests.count == 0);
    ...
    do {
        ...
        for (MosaicDatabase2::QueryParameters &p : params) {
            ...
            while ((code = result->moveToNext()) == TE_Ok) {
                ...
                Port::String resolved;
                if (this->resolvePath(resolved, frame->path) == TE_Ok) {
                    const bool needsLoad = retval.loaded.find(resolved.get()) == retval.loaded.end();
                        
                    Monitor::Lock qlock(retval.frameLoadContext.monitor);
                    if (needsLoad) {
                        // queue frame for prefetch
                        retval.frameLoadContext.requests.count++;
                        retval.frameLoadContext.requests.value.push_back(*frame);
                        qlock.broadcast();
                    } else {
                        // push back empty to pick up already loaded node
    ...
    while(true) {
        Monitor::Lock qlock(retval.frameLoadContext.monitor);
        // if the query has been canceled and there are pending requests, evict
        if (cancelled_ && !retval.frameLoadContext.requests.value.empty()) {
            retval.frameLoadContext.requests.count -= (unsigned)retval.frameLoadContext.requests.value.size();
            retval.frameLoadContext.requests.value.clear();
        }
        // wait until all pending requests have been serviced
        if (!retval.frameLoadContext.requests.count)
            break;
        else
            qlock.wait(100);
    }
    ...
    pfps_dbg.Count = (int)retval.frames.size();
...
bool GLMosaicMapLayer::checkQueryInterrupt(const GLGlobeBase::State &queryState) NOTHROWS
{
    TAK::Engine::Thread::Monitor::Lock lock(monitor_);
    return !TAK::Engine::Math::Rectangle2_intersects(
            target_state_.westBound, target_state_.southBound, target_state_.eastBound, target_state_.northBound,
...
void *GLMosaicMapLayer::frameLoaderThread(void *opaque)
{
    auto &ctx = *static_cast<MosaicPendingData*>(opaque);
    while(true) {
        std::unique_ptr<Monitor::Lock> lock0(std::make_unique<Monitor::Lock>(ctx.frameLoadContext.monitor));
        if (ctx.frameLoadContext.shutdown)
            break;
        if (ctx.frameLoadContext.requests.value.empty()) {
            lock0->wait();
            continue;
        }
        auto frame = ctx.frameLoadContext.requests.value.front();
        ctx.frameLoadContext.requests.value.pop_front();
        lock0.reset();

        // create node outside of lock
        GLQuadTileNode3Ptr node(nullptr, nullptr);
        ctx.owner.createRootNode(node, frame);

        // insert node (if created) and signal
        Monitor::Lock lock1(ctx.frameLoadContext.monitor);
        if(node)
            ctx.frames.push_back(std::make_pair(frame, std::move(node)));
        assert(ctx.frameLoadContext.requests.count > 0);
        ctx.frameLoadContext.requests.count--;
        lock1.broadcast();
    }
    return nullptr;
}
...
namespace {
    ...
    MosaicPendingData::MosaicPendingData(GLMosaicMapLayer &owner_) :
        owner(owner_),
        frames(),
        spatialCalc(),
        loaded(),
    ...
    {
        ThreadPool_create(frameLoaderWorkers, 8u, GLMosaicMapLayer::frameLoaderThread, this);
    }
    MosaicPendingData::~MosaicPendingData()
    {
        {
            Monitor::Lock lock(frameLoadContext.monitor);
            frameLoadContext.shutdown = true;
            lock.broadcast();
        }
        frameLoaderWorkers.reset();
