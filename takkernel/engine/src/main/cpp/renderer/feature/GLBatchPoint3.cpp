...
namespace
{
    ...
    std::vector<TAK::Engine::Port::String> failed_icon_uris;
        ...
        if (point.iconLoader.get()) {
            ...
            if (bitmapFuture.isDone()) {
                ...
                if (bitmap.get() == nullptr) {
                    ...
                    if (std::find(failed_icon_uris.begin(), failed_icon_uris.end(), point.iconUri) == failed_icon_uris.end()) {
                        Logger::log(Logger::Warning, "GLBatchPoint3: Failed to load icon %s, loading default", point.iconUri.get());
                        failed_icon_uris.push_back(point.iconUri);
                    }
...
#else
        // Is non-windows int64_t not a long long?
        const int64_t settleDelta = std::max(std::min((view.animationLastTick - labelFadeTimer), (int64_t)1000), (int64_t)200);
#endif
