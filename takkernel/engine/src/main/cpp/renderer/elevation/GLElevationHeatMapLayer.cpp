...
void GLElevationHeatMapLayer::draw(const GLGlobeBase& view, const int renderPass) NOTHROWS
{
    ...
    const auto checkVersion = (getRenderPass()&GLGlobe::Sprites) ? pass.drawVersion : pass.renderPump;
    if(checkVersion != check_version_) {
        check_version_ = checkVersion;
        ...
        double minEl = NAN;
        double maxEl = NAN;
        for (std::size_t i = 0u; i < view.renderPasses[0].renderTiles.count; i++) {
            ...
            // don't include tiles significantly far away
...
TAKErr TAK::Engine::Renderer::Elevation::GLElevationHeatMapLayer_createSpi(Core::GLLayerSpi2Ptr& spi) NOTHROWS
{
