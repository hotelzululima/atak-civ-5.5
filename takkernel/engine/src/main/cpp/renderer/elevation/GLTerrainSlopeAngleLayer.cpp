...
GLTerrainSlopeAngleLayer::GLTerrainSlopeAngleLayer(TAK::Engine::Core::MapRenderer* ctx, TerrainSlopeAngleLayer &subject) NOTHROWS :
    renderer_(ctx),
    subject_(subject),
    alpha_(subject.getAlpha()),
    visible_(subject.isVisible()),
    legend_enabled_(ConfigOptions_getIntOptionOrDefault("terrain-slope.disable-legacy-legend", 0) == 0),
    texture_id_(0u),
    legend_vbo_(0u)
{}
GLTerrainSlopeAngleLayer::GLTerrainSlopeAngleLayer(TerrainSlopeAngleLayer &subject) NOTHROWS :
    GLTerrainSlopeAngleLayer(nullptr, subject)
{}
...
void GLTerrainSlopeAngleLayer::draw(const GLGlobeBase& view, const int renderPass) NOTHROWS
{
    ...
    if(renderPass&GLGlobeBase::Surface)
        drawSlopeAngle(view);
    if((renderPass&GLGlobeBase::UserInterface) != 0 && legend_enabled_)
        drawLegend(view);
}
...
#define LEGEND_TEXT \
    LEGEND_TEXT_LINE("27-29") \
    LEGEND_TEXT_LINE("30-31") \
    LEGEND_TEXT_LINE("32-34") \
    LEGEND_TEXT_LINE("35-45") \
    LEGEND_TEXT_LINE("46-50") \
    LEGEND_TEXT_LINE("51-59") \
    LEGEND_TEXT_LAST_LINE("60+")

static const int lineColors[7u] =
{(int)0xFFFFFF00,
 (int)0xFFFFC000,
 (int)0xFFFF7F00,
 (int)0xFFFF0000,
...
#define LEGEND_TEXT_LINE(l) l "\n"
#define LEGEND_TEXT_LAST_LINE(l) l
...
void GLTerrainSlopeAngleLayer::start() NOTHROWS
{
    ...
    if (this->renderer_) {
        this->renderer_->registerControl(this->subject_, GradientControl_getType(),
                                         static_cast<GradientControl *>(this));
    }
}
void GLTerrainSlopeAngleLayer::stop() NOTHROWS
{
    ...
    if (this->renderer_)
        this->renderer_->unregisterControl(this->subject_, GradientControl_getType(), static_cast<GradientControl *>(this));
}
...
TAKErr GLTerrainSlopeAngleLayer::getGradientColors(Port::Collection<uint32_t>& gradientColors) NOTHROWS {

    for (int i = 0; i < sizeof(lineColors) / sizeof(int); ++i) {
        gradientColors.add(static_cast<uint32_t>(lineColors[i]));
    }

    return TE_Ok;
}
TAKErr GLTerrainSlopeAngleLayer::getLineItemStrings(Port::Collection<Port::String>& itemStrings) NOTHROWS {

#undef LEGEND_TEXT_LINE
#undef LEGEND_TEXT_LAST_LINE
#define LEGEND_TEXT_LINE(l) itemStrings.add(l);
#define LEGEND_TEXT_LAST_LINE(l) itemStrings.add(l);

    // add text to textItems vector
    LEGEND_TEXT

    return TE_Ok;
}
TAKErr GLTerrainSlopeAngleLayer::getMode(int* mode) NOTHROWS {
...
TAKErr GLTerrainSlopeAngleLayer::setMode(int mode) NOTHROWS {

    // no-op, always absolute
    return TE_Ok;
}
TAKErr TAK::Engine::Renderer::Elevation::GLTerrainSlopeAngleLayer_createSpi(GLLayerSpi2Ptr& spi) NOTHROWS {
    class SpiImpl : public GLLayerSpi2
    {
    public :
        TAKErr create(GLLayer2Ptr &value, GLGlobeBase &renderer, Layer2 &subject) NOTHROWS override
        {
            auto impl = dynamic_cast<TerrainSlopeAngleLayer *>(&subject);
            int implIsNull = (impl == nullptr);
            if(!impl)
                return TE_InvalidArg;
            value = GLLayer2Ptr(new(std::nothrow) GLTerrainSlopeAngleLayer(&renderer, *impl), Memory_deleter_const<GLLayer2, GLTerrainSlopeAngleLayer>);
            if(!value)
                return TE_OutOfMemory;
            return TE_Ok;
        }
    };
    spi = GLLayerSpi2Ptr(new SpiImpl(), Memory_deleter_const<GLLayerSpi2, SpiImpl>);
    return TE_Ok;
}
