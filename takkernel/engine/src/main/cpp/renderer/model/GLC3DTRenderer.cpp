...
namespace {
    ...
    public:
        ...
        void start() NOTHROWS override {}
        void stop() NOTHROWS override {}
        virtual TAKErr gatherDepthSamplerDrawables(std::vector<GLDepthSamplerDrawable*>& result, int levelDepth, const TAK::Engine::Core::MapSceneModel2& sceneModel, float x, float y) NOTHROWS override;
...
namespace {
    TAKErr deleteTilesetTask(int&, const GLC3DTTileset* ts) {
        delete ts;
        return TE_Ok;
    }

    void deleteTileset(const GLMapRenderable2* impl) {
        // make sure this is on the glThread. On CLI environments, for example, this may be called from the GC finalizer thread
        const GLC3DTTileset *ts = static_cast<const GLC3DTTileset*>(impl);
        if (ts)
            Task_begin(GLWorkers_glThread(), deleteTilesetTask, ts);
    }
}
...
TAKErr GLC3DTRenderer::LoaderImpl::load(TAK::Engine::Renderer::Core::GLMapRenderable2Ptr& output, TAK::Engine::Core::RenderContext& ctx, const TAK::Engine::Port::String& URI) NOTHROWS {
...

    {
        ...
        if (code == TE_Ok) {
            if (type == C3DTFileType_TilesetJSON) {
                output = GLMapRenderable2Ptr(tileset.release(), deleteTileset);
            } else if (type == C3DTFileType_B3DM) {
