...
bool GLSceneNode::isLoaded(const GLGlobeBase &view) const NOTHROWS
{
    ...
    // choose LOD for view
    const int lodIdx = selectLodIndex(view, *subject, info, mbb.wgs84, true);
...
TAKErr GLSceneNode::prepareLoadContext(GLSceneNode::LoadContext* value, const TAK::Engine::Core::MapSceneModel2& scene, double drawMapResolution) const NOTHROWS {
    ...
    // choose LOD for view
    const int lod = selectLodIndex(scene, drawMapResolution, *subject, info, mbb.wgs84, true);
    const auto clampedLod = atakmap::math::clamp<std::size_t>(lod + 1u, 0u, subject->getNumLODs() - 1u);

    value->centroid = GeoPoint2((mbb.wgs84.minY+mbb.wgs84.maxY)/2.0, (mbb.wgs84.minX+mbb.wgs84.maxX)/2.0, (mbb.wgs84.minZ+mbb.wgs84.maxZ)/2.0, (info.altitudeMode==TEAM_Absolute) ? AltitudeReference::HAE: AltitudeReference::AGL);
...
GLSceneNode::RenderVisibility GLSceneNode::isRenderable(const GLGlobeBase &view) const NOTHROWS
{
    ...
    const int lodIdx = selectLodIndex(view, *subject, info, mbb.wgs84, false);
    if(lodIdx < -1)
        return RenderVisibility::None;
    ...
    if (info.altitudeMode != TEAM_Absolute) {
        ...
        view.getTerrainMeshElevation(&zOff, (mbb.wgs84.maxY + mbb.wgs84.minY) / 2.0, (mbb.wgs84.maxX + mbb.wgs84.minX) / 2.0);
    }

    if (mbb.proj.srid != view.renderPass->drawSrid) {
        mbb.proj.srid = view.renderPass->drawSrid;
        GeometryTransformer_transform(&mbb.proj.value, mbb.wgs84, 4326, mbb.proj.srid);
    }

    Frustum2 frustum(view.renderPass->scene.camera.projection, view.renderPass->scene.camera.modelView);
    AABB aabb(TAK::Engine::Math::Point2<double>(mbb.proj.value.minX, mbb.proj.value.minY, mbb.proj.value.minZ), TAK::Engine::Math::Point2<double>(mbb.proj.value.maxX, mbb.proj.value.maxY, mbb.proj.value.maxZ));
    if (mbb.proj.srid == 4326) {
        // translate AABB
        aabb.minZ += zOff;
        aabb.maxZ += zOff;
    } else {
        // expand radius for other projections
        aabb.minX -= zOff;
        aabb.minY -= zOff;
        aabb.minZ -= zOff;
        aabb.maxX += zOff;
    ...
    const double wx = (aabb.maxX-aabb.minX);
    const double wy = (aabb.maxY-aabb.minY);
    const double wz = (aabb.maxZ-aabb.minZ);
    ...
    return frustum.intersects(aabb) ? RenderVisibility::Prefetch : RenderVisibility::None;
}
...
{
    TAKErr code(TE_Done);
    const int lodIdx = selectLodIndex(sceneModel, sceneModel.gsd, *subject, info, mbb.wgs84, false);
...
TAKErr GLSceneNode::refreshAABB(const Envelope2 &aabb_mesh_local) NOTHROWS
{
    ...
    code = Mesh_transform(&mbb.wgs84, aabb_mesh_local, opts, MeshTransformOptions(4326));
    mbb.proj.srid = -1;
    TE_CHECKRETURN_CODE(code);
...
void GLSceneNode::LODMeshes::setData(std::vector<std::unique_ptr<GLMesh>> &mesh_data) NOTHROWS
{
    ...
    if (!mesh_data.empty()) {
        ...
        for (std::size_t i = 0u; i < mesh_data.size(); i++) {
            const auto& mesh = mesh_data[i]->getSubject();
            Matrix2 localFrame;
            if (mesh_data[i]->local_frame_)
                localFrame.set(*mesh_data[i]->local_frame_);
