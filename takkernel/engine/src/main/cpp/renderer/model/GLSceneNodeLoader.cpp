...
GLSceneNodeLoader::QueueNode::QueueNode(const std::shared_ptr<GLSceneNode> &node_, GLSceneNode::LoadContext &&ctx_, const int srid_) NOTHROWS 
    : node(node_),
    ctx(std::move(ctx_)),
    srid(srid_)
{}
...
TAKErr GLSceneNodeLoader::enqueue(const std::shared_ptr<GLSceneNode> &node, GLSceneNode::LoadContext &&ctx, const bool prefetch, const int srid) NOTHROWS
{
    ...
    if (queueTo)
        queueTo->push_back(QueueNode(node, std::move(ctx), srid));
...
void GLSceneNodeLoader::threadImpl() 
{
    ...
    while (true) {
        ...
        std::shared_ptr<GLSceneNode> node;
        ...
        
        {
            ...
            cancelToken = &executingNodes.insert(std::make_pair(node.get(), false)).first->second;
        }

        TAK::Engine::Core::RenderContext_setCurrent(&node->ctx);
