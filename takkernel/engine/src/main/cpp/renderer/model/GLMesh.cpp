...
TAKErr GLMesh::initMaterials() NOTHROWS
{
    ...
    // set the first material as opaque white for the replace
    {
        ...
        code = getShader(s, ctx_, replaceLayout, *glmat, this->subject_->getDrawMode());
        TE_CHECKRETURN_CODE(code);
    ...
    for(std::size_t i = 0; i < numMaterials; i++) {
        ...
        this->materials_.push_back(glmat);
        std::shared_ptr<const Shader> s;
        code = getShader(s, ctx_, vertexDataLayout, *glmat, this->subject_->getDrawMode());
        TE_CHECKBREAK_CODE(code);
        this->shader_.push_back(s);
...
TAKErr GLMesh::resolveMaterials(bool *value) NOTHROWS
{
    ...
    *value = true;
    for (std::size_t i = 0u; i < materials_.size(); i++) {
        ...
        std::shared_ptr<const Shader> s;
        code = getShader(s, ctx_, this->subject_->getVertexDataLayout(), *this->materials_[i], this->subject_->getDrawMode());
        TE_CHECKBREAK_CODE(code);
        this->shader_[i] = s;
...
TAKErr GLMesh::getShader(std::shared_ptr<const Shader> &value, RenderContext &ctx, const TAK::Engine::Model::VertexDataLayout &layout, GLMaterial &material, DrawMode drawMode) NOTHROWS
{
    ...
    attrs.points = drawMode == TEDM_Points;
