...
namespace {
    ...
    struct TextFormatParamsLess {
        bool operator()(const TextFormatParams &a, const TextFormatParams &b) const {
            ...
            if(a.underline != b.underline) return a.underline < b.underline;
            if(a.strikethrough != b.strikethrough) return a.strikethrough < b.strikethrough;
            if(a.outline != b.outline) return a.outline < b.outline;
            if(a.fontName != b.fontName) return a.fontName < b.fontName;
            return false; // ==
        }
    };
    
    typedef std::map<std::shared_ptr<TextFormat2>, std::unique_ptr<GLText2>> GLTextCacheMap;

    struct PerContextData {
        GLTextCacheMap textCache;
        std::unique_ptr<GLRenderBatch2> SPRITE_BATCH;
    };

    static RenderContextAssociatedCache<PerContextData> perContextDataCache;

    GLTextCacheMap& getGLTextCache() {
        TAK::Engine::Core::RenderContext* renderContext = nullptr;
        TAK::Engine::Core::RenderContext_getCurrent(&renderContext);
        PerContextData& data = perContextDataCache.get(renderContext);
        return data.textCache;
    }

    std::unique_ptr<GLRenderBatch2>& getSpriteBatch() {
        TAK::Engine::Core::RenderContext* renderContext = nullptr;
        TAK::Engine::Core::RenderContext_getCurrent(&renderContext);
        PerContextData& data = perContextDataCache.get(renderContext);
        return data.SPRITE_BATCH;
    }
    ...
    void begin(GLRenderBatch2 &impl) NOTHROWS
    {
        ...
        gl.readMatrix(atakmap::renderer::GLES20FixedPipeline::MatrixMode::MM_GL_PROJECTION, tmpMatrix);
        impl.setMatrix(GL_PROJECTION, tmpMatrix);
        gl.readMatrix(atakmap::renderer::GLES20FixedPipeline::MatrixMode::MM_GL_MODELVIEW, tmpMatrix);
...
TextFormatParams::TextFormatParams(const char *name_, const float size_) NOTHROWS :
    fontName(TAK::Engine::Port::String_intern(name_)),
    size(size_),
    bold(false),
    italic(false),
    underline(false),
...
GLText2 *TAK::Engine::Renderer::GLText2_intern(std::shared_ptr<TextFormat2> textFormat) NOTHROWS
{
    ...
    TAK::Engine::Core::RenderContext_getCurrent(&ctx);
...
GLText2 *TAK::Engine::Renderer::GLText2_intern(const TAK::Engine::Core::RenderContext &ctx, std::shared_ptr<TextFormat2> textFormat) NOTHROWS
{
    ...
    std::unique_ptr<GLRenderBatch2>& SPRITE_BATCH = getSpriteBatch();
    if (!SPRITE_BATCH.get())
        SPRITE_BATCH.reset(new GLRenderBatch2(GLTEXT2_CHAR_BATCH_SIZE*4u));

    GLTextCacheMap& glTextCache = getGLTextCache();
    do {
    ...
    glTextCache[textFormat] = std::move(retval);
...
GLText2 *TAK::Engine::Renderer::GLText2_intern(const TAK::Engine::Core::RenderContext &ctx, const TextFormatParams &params) NOTHROWS
{
    std::shared_ptr<TextFormat2> fmt;
    {
        ...
        if(!fmt)
        {
            ...
            if(!fmt) {
                TextFormat2_createDefaultSystemTextFormat(fmt2ptr, params.size);
                fmt = std::move(fmt2ptr);
                textFormatCache[params] = fmt;
            }
        }
    }
    return fmt ? GLText2_intern(ctx, fmt) : nullptr;
}
...
TAKErr TAK::Engine::Renderer::GLText2_invalidateCache() NOTHROWS {
    ...
    for (auto& entry : glTextCache) {
        GLText2_invalidate(entry.second.get());
    }
...
TAKErr GLText2::draw(const char *text, const int *colors, const std::size_t colorsCount) NOTHROWS
{
    ...
    std::unique_ptr<GLRenderBatch2>& SPRITE_BATCH = getSpriteBatch();
    begin(*SPRITE_BATCH);
    code = batch(*SPRITE_BATCH, text, 0.0f, 0.0f, 0.0f, colors, colorsCount);
    SPRITE_BATCH->end();
    ...
    const float scissorX0, const float scissorX1) NOTHROWS
{
    ...
    std::unique_ptr<GLRenderBatch2>& SPRITE_BATCH = getSpriteBatch();
    begin(*SPRITE_BATCH);
    code = batch(*SPRITE_BATCH, text, 0.0f, 0.0f, 0.0f, colors, colorsCount, scissorX0, scissorX1);
    SPRITE_BATCH->end();
...
TAKErr GLText2::draw(const char *text, const float r, const float g, const float b, const float a) NOTHROWS
{
    ...
    if(!SPRITE_BATCH)
        return TE_IllegalState;
    begin(*SPRITE_BATCH);
    code = batch(*SPRITE_BATCH, text, 0.0f, 0.0f, 0.0f, r, g, b, a);
    SPRITE_BATCH->end();
...
TAKErr GLText2::draw(const char *text, const float r, const float g, const float b, const float a, const float scissorX0, const float scissorX1) NOTHROWS
{
    ...
    std::unique_ptr<GLRenderBatch2>& SPRITE_BATCH = getSpriteBatch();
    begin(*SPRITE_BATCH);
    code = batch(*SPRITE_BATCH, text, 0.0f, 0.0f, 0.0f, r, g, b, a, scissorX0,
        scissorX1);
    SPRITE_BATCH->end();
...
{
    return this->batch(batch, text, x, y, z, r, g, b, a, 0.f, 0.f, 0.f, 0.f);
}
TAKErr GLText2::batch(GLRenderBatch2 &batch,
    const char *text,
    const float x, const float y, const float z,
...
TAKErr GLText2::batch(GLRenderBatch2 &batch,
    ...
    const float scissorX0, const float scissorX1) NOTHROWS
{
    return this->batch(batch, text, x, y, z, r, g, b, a, 0.f, 0.f, 0.f, 0.f, scissorX0, scissorX1);
}
TAKErr GLText2::batch(GLRenderBatch2 &batch,
    const char *text,
    const float x, const float y, const float z,
    const float r, const float g, const float b, const float a,
    const float outlineR, const float outlineG, const float outlineB, const float outlineA,
    const float scissorX0, const float scissorX1) NOTHROWS
{
    TAKErr code;
...
TAKErr GLText2::batchImpl(GLRenderBatch2 &batch,
    ...
    const float outlineR, const float outlineG, const float outlineB, const float outlineA) NOTHROWS
{
    return batchImpl(batch, tx, ty, tz, text, off, len, r, g, b, a, outlineR, outlineG, outlineB, outlineA, 0.0f, FLT_MAX);
}
