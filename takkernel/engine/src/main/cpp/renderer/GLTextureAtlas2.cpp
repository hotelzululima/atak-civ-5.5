...
TAKErr TAK::Engine::Renderer::GLTextureAtlas_create(std::unique_ptr<GLTextureAtlas2, void(*)(const GLTextureAtlas2 *)> &value, const Bitmap2 &spriteSheet, const char **uris, const TAK::Engine::Math::Rectangle2<float> *regions, const std::size_t numEntries) NOTHROWS
{
    const std::size_t texSize = std::max(spriteSheet.getWidth(), spriteSheet.getHeight());
    value = std::unique_ptr<GLTextureAtlas2, void(*)(const GLTextureAtlas2 *)>(new GLTextureAtlas2(texSize), Memory_deleter_const<GLTextureAtlas2>);

    BitmapPtr_const bitmap(&spriteSheet, Memory_leaker_const<Bitmap2>);
    if (bitmap->getFormat() != Bitmap2::RGBA32)
        bitmap = BitmapPtr_const(new Bitmap2(*bitmap, Bitmap2::RGBA32), Memory_deleter_const<Bitmap2>);

    // write spritesheet to a new texture
    GLuint id{0};

    glGenTextures(1, &id);
    if (id == 0) {
        GLenum err = glGetError();
        atakmap::util::Logger::log(atakmap::util::Logger::Error, "GLTextureAtlas2: failed to generate new texture for spritesheet err: %u", err);
        return TE_Err;
    }
    value->currentTexId = id;

    glBindTexture(GL_TEXTURE_2D, value->currentTexId);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
                 static_cast<GLsizei>(texSize), static_cast<GLsizei>(texSize), 0,
                 GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0u, 0u, static_cast<GLsizei>(bitmap->getWidth()), static_cast<GLsizei>(bitmap->getHeight()),
                    GL_RGBA, GL_UNSIGNED_BYTE, bitmap->getData());

    if (!value->currentSheetRects) {
        value->sheetRects.reset(new GLTextureAtlas2::SheetRects(value->currentTexId));
        value->currentSheetRects = value->sheetRects.get();
    } else {
        value->currentSheetRects->next.reset(new GLTextureAtlas2::SheetRects(value->currentTexId));
        value->currentSheetRects = value->currentSheetRects->next.get();
    }
    value->freeIndex = 0;
    for(std::size_t i = 0u; i < numEntries; i++) {
        int64_t retval = ((int64_t) value->currentTexId << 32L)
                         | ((int64_t) value->freeIndex & 0xFFFFFFFFL);

        // capture the icon region
        GLTextureAtlas2::Rect iconR((std::size_t)regions[i].x, (std::size_t)regions[i].y, (std::size_t)regions[i].width, (std::size_t)regions[i].height, value->splitFreeHorizontal);
        if (value->currentSheetRects->numRects < value->currentSheetRects->limit &&
            value->freeIndex == value->currentSheetRects->numRects) {
            // numRects and freeIndex must always be equal !!!
            value->currentSheetRects->rect[value->currentSheetRects->numRects++] = iconR;
        } else {
            // rect sheet is filled, put in the overflow
            value->keyToIconRectOverflow.insert(std::pair<int64_t, GLTextureAtlas2::Rect>(retval, iconR));
        }
        value->uriToKey[uris[i]] = retval;
        value->freeIndex++;
    }
    // emit sheet
    value->freeIndex = 0;
    value->currentTexId = 0;
