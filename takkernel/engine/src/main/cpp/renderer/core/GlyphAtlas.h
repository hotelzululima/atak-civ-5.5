    ...
    namespace Engine {
        namespace Renderer {
            ...
            namespace Core {
                ...
                struct LabelData {
                    LabelData(float color_r, float color_g, float color_b, float color_a,
                    ...
                    float x,y,z;
                    float anchorOffsets;
                    float rotRadsFontSize;
                    float bufferValues;
                    float xrayAlpha;

                    void insert(std::vector<float>& verts) const NOTHROWS{
                        verts.push_back(color);
                        verts.push_back(outlineColor);
                        verts.push_back(x);
                        verts.push_back(y);
                        verts.push_back(z);
                        verts.push_back(anchorOffsets);
                        verts.push_back(rotRadsFontSize);
                        verts.push_back(bufferValues);
                        verts.push_back(xrayAlpha);
                    }
                ...
                struct GlyphBuffersOpts {
                    ...
                    GlyphBuffersOpts(const GlyphBuffersOpts &other) NOTHROWS
                        : point_size(other.point_size), 
                        h_alignment(other.h_alignment),
                        break_on_end_of_line(other.break_on_end_of_line),
                        break_on_end_of_string(other.break_on_end_of_string),
                        fontName(TAK::Engine::Port::String_intern(other.fontName)),
                        style(TAK::Engine::Port::String_intern(other.style)),
                        font_size(other.font_size),
                        outline_weight(other.outline_weight),
                        underline(other.underline),
                        strikethrough(other.strikethrough),
                        fill(other.fill),
                        text_color_red(other.text_color_red),
                        text_color_green(other.text_color_green),
                        text_color_blue(other.text_color_blue),
                        text_color_alpha(other.text_color_alpha),
                        back_color_red(other.back_color_red),
                        back_color_green(other.back_color_green),
                        back_color_blue(other.back_color_blue),
                        back_color_alpha(other.back_color_alpha),
                        outline_color_red(other.outline_color_red),
                        outline_color_green(other.outline_color_green),
                        outline_color_blue(other.outline_color_blue),
                        outline_color_alpha(other.outline_color_alpha),
                        renderX(other.renderX), renderY(other.renderY), renderZ(other.renderZ),
                        anchorX(other.anchorX), anchorY(other.anchorY), anchorZ(other.anchorZ),
                        rotation(other.rotation),
                        xray_alpha(other.xray_alpha),
                        is_default_font(other.is_default_font),
                        bold(other.bold),
                        italic(other.italic)
                    ...
                    TAK::Engine::Port::String style;  //@deprecated, remove along with GlyphAtlas
                    float font_size;
                ...
                public:
                    ...
                    friend TAK::Engine::Util::TAKErr GlyphAtlas_open(std::unique_ptr<GlyphAtlas>& result, const char* URI, const char* font,
                        const char* style, const char* type, const GlyphAtlasOpts* opts) NOTHROWS;
                    ...
                    friend TAK::Engine::Util::TAKErr GlyphAtlas_loadBitmap(BitmapPtr& result, const std::unique_ptr<GlyphAtlas> &atlas,
                        const char* URI, const char* font, const char* style, const char *type, const GlyphAtlasOpts* opts) NOTHROWS;
                private :
                    /**
                ...
                TAK::Engine::Util::TAKErr GlyphAtlas_open(std::shared_ptr<GlyphAtlas>& result, const char* URI, const char* font,
                    const char* style, const char* type, const GlyphAtlasOpts* opts) NOTHROWS;
                TAK::Engine::Util::TAKErr GlyphAtlas_open(std::unique_ptr<GlyphAtlas>& result, const char* URI, const char* font,
                    const char* style, const char* type, const GlyphAtlasOpts* opts) NOTHROWS;

                /** @deprecated shared_ptr prevents assigning responsibility of releasing added resources */
                TAK::Engine::Util::TAKErr GlyphAtlas_loadBitmap(BitmapPtr& result, const std::shared_ptr<GlyphAtlas> &atlas,
                    const char* URI, const char* font, const char* style, const char *type, const GlyphAtlasOpts* opts) NOTHROWS;
                TAK::Engine::Util::TAKErr GlyphAtlas_loadBitmap(BitmapPtr& result, const char* URI, const char* font,
                    const char* style, const char *type, const GlyphAtlasOpts* opts) NOTHROWS;
            }
        }
    }
