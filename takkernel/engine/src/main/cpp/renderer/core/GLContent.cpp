...
GLContentContext::GLPendingContent::GLPendingContent(PendingNode_ &node) NOTHROWS :
    node_(node)
{}
...
GLContentContext::PendingNode_* GLContentContext::PendingNode_::fromPendingContent(GLPendingContent* content) NOTHROWS {
    return !!content ? &content->node_ : nullptr;
}
...
TAKErr GLContentContext::loadTask_(
    ...
    TAK::Engine::Core::RenderContext_setCurrent(ctx);
