...
LabelData::LabelData(float color_r, float color_g, float color_b, float color_a,
...
{
    auto temp = packColor(color_r, color_g, color_b, color_a);
    this->color = (reinterpret_cast<float&>(temp));
    temp = packColor(outline_r, outline_g, outline_b, outline_a);
    this->outlineColor = reinterpret_cast<float&>(temp);

    this->x = x;
    this->y = y;
    this->z = z;

    temp = packFloats(anchorXOffset, 1, anchorYOffset, 1);
    this->anchorOffsets = reinterpret_cast<float&>(temp);

    temp = packFloats(rotationRads, 4, fontSize, 1);
    this->rotRadsFontSize = reinterpret_cast<float&>(temp);

    temp = packFloats(buffer, 4, outlineBuffer, 4);
    this->bufferValues = reinterpret_cast<float&>(temp);
...
size_t GlyphAtlas::fillBuffers(std::vector<float>& pos, std::vector<float>& uvs, std::vector<uint16_t>& idxs,
    ...
    const uint32_t* codepoints, size_t num_codepoints, LabelData* labelData) NOTHROWS {

    return this->fillBuffersOrMeasure_(&pos, &uvs, &idxs, &decor_pos, cursor, opts, codepoints, num_codepoints, labelData);
}
...
size_t GlyphAtlas::measureCodepoints(
    ...
    return this->fillBuffersOrMeasure_(nullptr, nullptr, nullptr, nullptr, cursor, opts, codepoints, num_codepoints, nullptr);
}
...
TAKErr GetAtlasResource(AtlasResource& result, const char* URI, const char* font, const char* style, const char* type) NOTHROWS {
    ...
    StringBuilder fontResourcesUri;
    code = StringBuilder_combine(fontResourcesUri, URI, "/font_resources.json");
    TE_CHECKRETURN_CODE(code)

    std::unique_ptr<DataInput2, void (*)(const DataInput2*)> fontResourcesInput(nullptr, nullptr);
    code = openPathOrUri(fontResourcesInput, fontResourcesUri.c_str());
...
TAKErr TAK::Engine::Renderer::Core::GlyphAtlas_open(std::shared_ptr<GlyphAtlas>& result, const char* URI, const char* font,
    ...
    std::unique_ptr<GlyphAtlas> uniqueResult;
    TAKErr code = GlyphAtlas_open(uniqueResult, URI, font, style, type, opts);
    if (code == TE_Ok) {
        result = std::move(uniqueResult);
    }
    return code;
}
TAKErr TAK::Engine::Renderer::Core::GlyphAtlas_open(std::unique_ptr<GlyphAtlas>& result, const char* URI, const char* font,
    const char* style, const char* type, const GlyphAtlasOpts* opts) NOTHROWS {

    TAKErr code;

    std::unique_ptr<GlyphAtlas> atlas = std::unique_ptr<GlyphAtlas>(new GlyphAtlas());
    ...
    StringBuilder jsonUri;
    code = StringBuilder_combine(jsonUri, URI, "/", atlasResource.jsonFilename);
    TE_CHECKRETURN_CODE(code)

    std::unique_ptr<DataInput2, void(*)(const DataInput2*)> jsonInput(nullptr, nullptr);
    code = openPathOrUri(jsonInput, jsonUri.c_str());
    if (code == TE_InvalidArg)
        return code;
    TE_CHECKRETURN_CODE(code);
...
TAKErr TAK::Engine::Renderer::Core::GlyphAtlas_loadBitmap(BitmapPtr &result, const std::shared_ptr<GlyphAtlas> &atlas,
    ...
    return GlyphAtlas_loadBitmap(result, URI, font, style, type, opts);
}

TAKErr TAK::Engine::Renderer::Core::GlyphAtlas_loadBitmap(BitmapPtr &result, const char *URI, const char *font,
    const char* style, const char *type, const GlyphAtlasOpts *opts) NOTHROWS {
    ...
    StringBuilder pngUri;
    code = StringBuilder_combine(pngUri, URI, "/", atlasResource.pngFilename);
    TE_CHECKRETURN_CODE(code)

    std::unique_ptr<DataInput2, void(*)(const DataInput2*)> bitmapInput(nullptr, nullptr);
    code = openPathOrUri(bitmapInput, pngUri.c_str());
    if (code == TE_InvalidArg)
        return code;
    TE_CHECKRETURN_CODE(code);
...
namespace {
    ...
    TAKErr openPathOrUri(DataInput2Ptr &value, const char *pathOrUri) NOTHROWS
    {
        // open as URI (assumed preferred execution)
        if (ProtocolHandler_handleURI(value, pathOrUri) == TE_Ok)
            return TE_Ok;
        // fallback to open as path
        if (IO_openFileV(value, pathOrUri) == TE_Ok)
            return TE_Ok;
        // open failed...
        return TE_InvalidArg;
    }
