...
namespace
{
    ...
    private:
        void getCharMetrics(char_metrics_t *metrics, const unsigned int c) NOTHROWS;
        std::wstring getWideString(unsigned int c) NOTHROWS;
    private:
        int designUnitsToPixels(float value);
    };

    class GdiPlusTextFormatFactory : public TextFormatFactory
    {
    public:
        TAKErr createTextFormat(TextFormat2Ptr& value, const TextFormatParams& params) NOTHROWS override;
    };
...
bool TextFormatFactory_getDefault(std::shared_ptr<TextFormatFactory>& value) NOTHROWS
{
    std::unique_ptr<TextFormatFactory, void(*)(const TextFormatFactory*)> impl(new(std::nothrow) GdiPlusTextFormatFactory(), Memory_deleter_const<TextFormatFactory, GdiPlusTextFormatFactory>);
    if (!impl)
        return false;
    value = std::move(impl);
    return true;
...
namespace
{
    TAKErr GdiPlusTextFormatFactory::createTextFormat(TextFormat2Ptr& value, const TextFormatParams& params) NOTHROWS
    {
        if (params.size <= 0)
    ...
    void CLITextFormat2::getCharMetrics(char_metrics_t *metrics, const unsigned int c) NOTHROWS
    {
    ...
    std::wstring CLITextFormat2::getWideString(unsigned int c) NOTHROWS {
        char buf[5u];
        std::wstring wstring;
        try {
            if (c < 0xFF) {
