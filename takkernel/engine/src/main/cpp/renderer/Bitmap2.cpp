...
#undef CONVERSION_FN_DECL
    ...
    // indexed as (srcFmt*NUM_FORMATS) + dstFmt
...
TAKErr Bitmap2::subimage(BitmapPtr &value, const std::size_t x, const std::size_t y, const std::size_t w, const std::size_t h, const bool share) const NOTHROWS
{
    if ((x + w) > this->width)
...
TAKErr Bitmap2::getPixel(uint32_t *value, const std::size_t x, const std::size_t y) const NOTHROWS
{
    const static auto endianArgb = (TE_PlatformEndian == TE_BigEndian) ? Format::ARGB32 : Format::BGRA32;
    if(x >= width || y >= height)
        return TE_InvalidArg;

    const std::size_t dataOff = (y*stride) + (x*formatSize[format]);
    conversionFunctions[format*NUM_FORMATS + endianArgb](reinterpret_cast<uint8_t *>(value), 0, dataPtr.get() + dataOff, stride, 1u, 1u);
    return TE_Ok;
}
TAKErr Bitmap2::setPixel(const std::size_t x, const std::size_t y, const uint32_t argb) NOTHROWS
{
    const static auto endianArgb = (TE_PlatformEndian == TE_BigEndian) ? Format::ARGB32 : Format::BGRA32;
    if(x >= width || y >= height) {
        return TE_InvalidArg;
    ...
    const std::size_t dataOff = (y * stride) + (x * formatSize[format]);
    conversionFunctions[endianArgb*NUM_FORMATS + format](dataPtr.get() + dataOff, stride, reinterpret_cast<const uint8_t *>(&argb), 0, 1u, 1u);
    return TE_Ok;
}
