...
GLMegaTexture::TileIndex::TileIndex() NOTHROWS :
    TileIndex(0u, 0u, 0u)
{}
GLMegaTexture::TileIndex::TileIndex(const std::size_t level_, const std::size_t column_, const std::size_t row_) NOTHROWS :
    level(level_),
    column(column_),
    row(row_)
{}
...
{
    std::size_t sz;
    if(getTileSize(&sz, &sz, key) != TE_Ok)
        return tileSize;
    else
        return sz;
}
TAKErr GLMegaTexture::getTileSize(std::size_t *tileWidth, std::size_t *tileHeight, const TileIndex &key) const NOTHROWS
{
    *tileWidth = tileSize;
    *tileHeight = tileSize;
    ...
    *tileWidth = (std::size_t)entry->second.width;
    *tileHeight = (std::size_t)entry->second.height;
    return TE_Ok;
}
...
{
    return bindTile(key, tileSize>>mip, tileSize>>mip);
}
TAKErr GLMegaTexture::bindTile(const TileIndex &key, const std::size_t tileWidth, const std::size_t tileHeight) NOTHROWS
{
    auto entry = tiles.find(key);
    if (entry != tiles.end()) {
        do {
            ...
            if(entry->second.width == tileWidth && entry->second.height == tileHeight) {
                entry->second.bind();
                return TE_Ok;
            } else {
    ...
    if(!pool.empty() && tileWidth == tileSize && tileHeight == tileSize) {
        ...
        TAKErr code = GLOffscreenFramebuffer_create(&tile, (int)tileWidth, (int)tileHeight, opts);
