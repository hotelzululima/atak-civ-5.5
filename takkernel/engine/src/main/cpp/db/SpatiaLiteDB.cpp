...
class StatementImpl
  ...
  public:                               //                      PUBLIC        //
    ...
    void
    bindNULL (std::size_t index) override;

    void
    clearBindings () override;
...
void SpatiaLiteDB::init(const char *filePath) 
  {
      try {
          //
          ...
          std::unique_ptr<Cursor> cursor(query("PRAGMA busy_timeout"));
