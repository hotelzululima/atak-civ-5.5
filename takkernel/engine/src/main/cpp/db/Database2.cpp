...
TAK::Engine::Util::TAKErr Database2::getErrorMessage(TAK::Engine::Port::String &value) NOTHROWS {
    return TAK::Engine::Util::TE_NotImplemented;
}
...
namespace
{
    ...
    {
        friend TAK::Engine::Util::TAKErr TAK::Engine::DB::Databases_openDatabase(DatabasePtr &db, const char* databaseFilePath, const uint8_t *key, const std::size_t keylen, const bool readOnly) NOTHROWS;

    private :
        // Creator must check openError member; non-null return indicates DB could not be
        // opened and is unusable
        SpatiaLiteDB(const char* filePath,
        const uint8_t *key,
        const std::size_t keylen,
        bool readOnly) NOTHROWS;
    ...
    private :
        Mutex mutex;
        // nullptr if db opened successfully, else points to error details
        // If non-null, further use of object is invalid
        std::unique_ptr<DB_Error> openError;
    ...
    void
    createDB_Error(std::unique_ptr<DB_Error> &error,
                  int response,
...
{
    SpatiaLiteDB *sldb = new SpatiaLiteDB(databaseFilePath, key, keylen, readOnly);
    DB_Error *error = sldb->openError.get();
    if (error) {
        Logger::log(Logger::Error, "Failed to open database %s, message: %s", databaseFilePath, error->what());
        delete sldb;
        return TE_Err;
    }

    
    db = DatabasePtr(sldb, deleteImpl<Database2, SpatiaLiteDB>);
    return TE_Ok;
}
...
namespace
{
    ...
    #define TAKCIPHER_SHA256 1
#endif
    
    int setTAKCipherSettings(sqlite3 *connection, const char *dbname)
    {
#ifdef TAKCIPHER_SHA256
        std::string dbprefix;
        if (dbname) {
            dbprefix = dbname;
            dbprefix += ".";
        ...
        std::stringstream ss;
        ss << "PRAGMA " << dbprefix << "kdf_iter = 64000; PRAGMA " << dbprefix << "cipher_kdf_algorithm = PBKDF2_HMAC_SHA256;";
        std::string pragmastr = ss.str();
        return sqlite3_exec(connection, pragmastr.c_str(), NULL, NULL, NULL);
#else
        // Use default cipher settings for current version
        return SQLITE_OK;
#endif
    }
    
    SpatiaLiteDB::SpatiaLiteDB(const char* filePath, const uint8_t *key, const std::size_t keylen, const bool ro) NOTHROWS :
        ...
        readOnly(ro)
    {
        ...
        if (response != SQLITE_OK)
        {
            ...
            createDB_Error(openError, response,
                MEM_FN("SpatiaLiteDB"),
                msg.c_str());
            return;
        }
        ...
        if(key) {
            ...
            // Try to open with TAK-preferred cipher settings
            setTAKCipherSettings(connection, NULL);
            response = sqlite3_exec(connection, "SELECT count(*) FROM sqlite_master;", NULL, NULL, NULL);
            if (response != SQLITE_OK) {
                // This could be a legacy DB that needs cipher settings updated.
                // This should not be done universally
                // as it is expensive, so we do it only after failed open.
                // This operation requires CREATE flag, so only perform on files that exist
                // since non-existing files won't fail due to antiquated cipher settings.
                // This avoids us creating new db's when a read-only open is requested
                // for a non-existing file.
                bool exists = false;
                if (filePath && IO_exists(&exists, filePath) == TAKErr::TE_Ok && exists) {
                    Logger::log(Logger::Debug, MEM_FN("SpatiaLiteDB")
                            "Initial open of existing keyed database failed - attempting migration of cipher settings");

                    // Close our non-working connection first
                    closeConnection();
                    bool migrationSuccess = false;
                    
                    // Get parent directory of database file for use in temp file creation
                    String dbdir;
                    String migrateDBFile;
                    bool tempDirOk = false;
                    if (IO_getParentFile(dbdir, filePath) != TAKErr::TE_Ok)
                        Logger::log(Logger::Debug, MEM_FN("SpatiaLiteDB")
                                "Getting parent directory of source database file %s failed. Cannot create temp database for cipher migration", filePath);
                    else
                        tempDirOk = true;

#ifdef TAKCIPHER_SHA256
                    // On platforms where we want to use SHA256 settings (see setTAKCipherSettings())
                    // try to migrate from both current version defaults and version sqlcipher v3 defaults
                    const int numLegacyCiphersets = 2;
#else
                    // On all others just try migrating from old sqlcipher v3 (to current version default)
                    const int numLegacyCiphersets = 1;
#endif
                    int migrateVersion;
                    TAKErr migrateVersionValid = TAKErr::TE_Err;
                    ...
                    for (int i = 0; tempDirOk && i <= numLegacyCiphersets; ++i) {
                        ...
                        int updateResponse = sqlite3_open_v2(filePath,
                                                   &connection,
                                                   SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE,
                                                   nullptr);
                        if (updateResponse == SQLITE_OK) {
                            sqlite3_key(connection, key, (int) keylen);
                            
                            if (i == numLegacyCiphersets) {
                                // sqlcipher v3 defaults
                                sqlite3_exec(connection, "PRAGMA cipher_page_size = 1024; "
                            ...
                            updateResponse = sqlite3_exec(connection,
                                                          "SELECT count(*) FROM sqlite_master;", NULL,
                                                          NULL, NULL);
                            if (updateResponse == SQLITE_OK) {
                                // opened with this set of legacy parameters; now do migration
                                std::string keystring((const char *)key, keylen);
                                const char *ckeystring = keystring.c_str();
                                std::vector<const char *> migrateParams;
                                migrateParams.push_back(migrateDBFile);
                                migrateParams.push_back(keystring.c_str());
                                
                                bool attached = execute("ATTACH DATABASE ? AS migrationdb KEY ?", migrateParams.data(), 2) == TAKErr::TE_Ok; 

                                QueryPtr q(nullptr, nullptr);
                                int64_t migrateResult = -1;
                                if (attached && setTAKCipherSettings(connection, "migrationdb") == SQLITE_OK && 
                                        query(q, "SELECT sqlcipher_export('migrationdb')") == TE_Ok &&
                                        q->moveToNext() == TE_Ok &&
                                        q->getLong(&migrateResult, 0) == TE_Ok &&
                                        migrateResult == 0) {
                                    // Migration is good, now re-open as originally intended
                                    Logger::log(Logger::Debug, MEM_FN("SpatiaLiteDB")
                                                               "Migration of cipher settings complete, reopening");
                                    // Capture user version
                                    migrateVersionValid = getVersion(&migrateVersion);
                                    migrationSuccess = true;
                                } else {
                                    Logger::log(Logger::Debug, MEM_FN("SpatiaLiteDB")
                                                               "Migration of cipher settings failed");
                                }
                                if (attached)
                                    sqlite3_exec(connection, "DETACH DATABASE migrationdb", NULL, NULL, NULL);
                            }
                            closeConnection();
                            if (migrationSuccess)
                                // leave temp file to be consumed later
                                break;
                            else
                                // Clean up temp file
                                IO_delete(migrateDBFile);
                        }
                    }
                    
                    if (migrationSuccess) {
                        IO_copy(filePath, migrateDBFile);
                        IO_delete(migrateDBFile);

                        // Now retry the original open
                        response = sqlite3_open_v2(filePath,
                                                   &connection,
                                                   (readOnly ? SQLITE_OPEN_READONLY :
                                                    SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE),
                                                   nullptr);
                        ...
                        setTAKCipherSettings(connection, NULL);
                        response = sqlite3_exec(connection, "SELECT count(*) FROM sqlite_master;", NULL, NULL, NULL);
                        if (response == SQLITE_OK && migrateVersionValid == TAKErr::TE_Ok)
                           setVersion(migrateVersion);
                    } // else keep original failure
                }
            }
            if(response != SQLITE_OK) {
                createDB_Error(openError, response,
    ...
    void createDB_Error(std::unique_ptr<DB_Error> &error, int response, const char* header, const char* doing)
    {
		StringBuilder logStr;
        ...
        case SQLITE_BUSY:
            ...
            error = std::unique_ptr<DB_Error>(new DB_Busy(logStr.c_str()));
            break;

        case SQLITE_INTERRUPT:

            error = std::unique_ptr<DB_Error>(new DB_Interrupted(logStr.c_str()));
            break;

        default:

            error = std::unique_ptr<DB_Error>(new DB_Error(logStr.c_str()));
        }
    }
