...
namespace
{
    TAKErr isValidApp(bool *value, CatalogDatabase2::CatalogCursor &result, CatalogCurrency2 *currency)
    {
        *value = false;
        ...
        const char *u8file;
        code = result.getPath(&u8file);
        TE_CHECKRETURN_CODE(code);

#ifdef _MSC_VER
        std::wstring wfile;
        TE_BEGIN_TRAP() {
            wfile = std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t>{}.from_bytes(u8file);
        } TE_END_TRAP(code);
        TE_CHECKRETURN_CODE(code);
        std::error_code ec;
        if (!std::filesystem::exists(wfile.c_str()), ec)
            return code;
        if (ec)
            return TE_Err;
#else
        if (!atakmap::util::pathExists(u8file))
            return code;
#endif
