...
TerrainData::TerrainData(int level, int srid) NOTHROWS :
    totalSize(0), srid(srid), level(level), x(0), y(0),
    spacing(TileCoord_getSpacing(level, srid)), header(), vertexData(),
    indexData(), edgeIndices(), seamsResolved{ false, false, false, false }, skirts()
{
}

Util::TAKErr TAK::Engine::Formats::QuantizedMesh::Impl::TerrainData_deserialize(TerrainDataPtr &result, const char *filename, int level, int srid) NOTHROWS
{
    Util::TAKErr code = Util::TE_Ok;
    std::unique_ptr<TerrainData> value;
    code = TerrainData_deserialize(value, filename, level, srid);
    TE_CHECKRETURN_CODE(code);
    result = TerrainDataPtr(value.release(), Util::Memory_deleter_const<TerrainData>);
    return code;
}
Util::TAKErr TAK::Engine::Formats::QuantizedMesh::Impl::TerrainData_deserialize(std::unique_ptr<TerrainData> &result, const char *filename, int level, int srid) NOTHROWS
{
    Util::TAKErr code = Util::TE_Ok;
    ...
    TE_CHECKRETURN_CODE(code);
    TerrainDataPtr resultPtr = TerrainDataPtr (new TerrainData(level, srid), Util::Memory_deleter_const<TerrainData>);
    code = TerrainData::deserialize(*resultPtr, *input, level, srid, true);
    TE_CHECKRETURN_CODE(code);

    result.reset(resultPtr.release());
    return code;
}
Util::TAKErr TAK::Engine::Formats::QuantizedMesh::Impl::TerrainData_deserialize(TerrainDataPtr &result, const uint8_t *buf, const std::size_t len, int level, int srid) NOTHROWS
{
    Util::TAKErr code(Util::TE_Ok);
    Util::MemoryInput2 input;
    code = input.open(buf, len);
    TE_CHECKRETURN_CODE(code);
    return TerrainData_deserialize(result, input, level, srid);
}
Util::TAKErr TAK::Engine::Formats::QuantizedMesh::Impl::TerrainData_deserialize(TerrainDataPtr &result, Util::DataInput2 &input, int level, int srid) NOTHROWS
{
    result = TerrainDataPtr (new TerrainData(level, srid), Util::Memory_deleter_const<TerrainData>);
    return TerrainData::deserialize(*result, input, level, srid, false);
}
Util::TAKErr TerrainData::deserialize(TerrainData &result, Util::DataInput2 &input, const int level, const int srid, const bool sortEdges) NOTHROWS
{
    Util::TAKErr code = Util::TE_Ok;
    ...
    TE_CHECKRETURN_CODE(code);
    code = EdgeIndices_deserialize(result.edgeIndices, result.vertexData, result.indexData.is32Bit(), input, sortEdges);
    TE_CHECKRETURN_CODE(code);
    ...
    result.x = (int)TileCoord_getTileX(g.longitude, level, srid);
    result.y = (int)TileCoord_getTileY(g.latitude, level, srid);
...
Feature::Envelope2 TerrainData::getBounds() const NOTHROWS
{
    return Feature::Envelope2(
            TileCoord_getLongitude(x, level, srid),
            TileCoord_getLatitude(y+1, level, srid),
            0.0,
            TileCoord_getLongitude(x+1, level, srid),
            TileCoord_getLatitude(y, level, srid),
            0.0);
}
...
{
    double tileX = (srid == 4326) ? (lon + 180) / spacing : TileCoord_getTileX(lon, level, srid);
    double tileY = (srid == 4326) ? (lat + 90) / spacing : TileCoord_getTileY(lat, level, srid);
    ...
    Math::Vector4<double> *vec) NOTHROWS
{
    ...
    if (convertToHAE && !TE_ISNAN(altMSL)) {
        double offset;
...
double TerrainData::getElevationNoSync(const Math::Vector4<double> &vec, bool ignoreSkirts) NOTHROWS
{
    ...
    const auto pindices = indexData.getTriangleIndices(0, (int) vec.x, (int) vec.y);
    if (pindices) {
        // Triangle indices
        const auto &indices = *pindices;
        const auto limit = indices.size();
        for (std::size_t i = 0; i < limit; i++) {
            int tIndex = indices[i]*3;

            // Get the 3 vertices and build the test triangle
            const auto v0 = vertexData[indexData[tIndex]];
            const auto v1 = vertexData[indexData[tIndex+1]];
            const auto v2 = vertexData[indexData[tIndex+2]];
