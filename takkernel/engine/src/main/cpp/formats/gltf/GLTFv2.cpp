...
namespace {
    ...
    TAKErr buildMaterials(std::vector<Material>& mats, BuildState& state, const tinygltf::Model &model, const tinygltf::Material& mat) {
        ...
        if (!image.uri.empty()) {
            ...
            array_ptr<uint8_t> bufCopy(new uint8_t[bv.byteLength]);
            memcpy(bufCopy.get(), &b.data[0] + bv.byteOffset, bv.byteLength);
            state.meshBufferArgs.push_back(MemBufferArg{
                    std::unique_ptr<const void, void (*)(const void*)>(bufCopy.release(), Memory_void_array_deleter_const<uint8_t>),
                    bv.byteLength
                });
