...
C3DTTileset::C3DTTileset() 
    : geometricError(NAN)
{}
TAKErr TAK::Engine::Formats::Cesium3DTiles::C3DTTileset_writeJSON(const char* URI, const TAK::Engine::Feature::Envelope2& envelope, const char* contentURI) NOTHROWS {
    json obj;
...
namespace {
    ...
    TAKErr determineFileType(C3DTFileType* type, const char* name) NOTHROWS {
        ...
        if (isExt(name, ".pnts")) {
            if (type) *type = C3DTFileType_PNTS;
            return TE_Ok;
        }
        //TODO-- i3dm, .pnt
    ...
    bool isFileSystemTileset(
        ...
        bool isDir = false;
        TAKErr isDirCode = IO_isDirectoryV(&isDir, URI);
        if (isDirCode == TE_Done)
            isDirCode = TE_Ok;
        if (isDirCode != TE_Ok)
            return false;
        if (isDir) {
            ...
            if (IO_getName(name, URI) != TE_Ok)
                return false;
            C3DTFileType resultType = C3DTFileType_TilesetJSON;
            if (determineFileType(&resultType, name) != TE_Ok)
                return false;
            ...
            // only check for valid JSON if C3DTFileType_TilesetJSON detected
            if (resultType == C3DTFileType_TilesetJSON) {
                TAK::Engine::Port::StringBuilder sb;
    ...
    bool isStreamingTileset(C3DTFileType* type,
        ...
        bool isB3Dm = false;
        if (fileName) {
            isJson = String_strcasestr(fileName, ".json") == nullptr ? false : true;
            isB3Dm = String_endsWith(fileName, ".b3dm");
        }
        
        // Assume it is a base URI
        if (!fileName || (!isJson && !isB3Dm)) {
