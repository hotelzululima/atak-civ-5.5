...
namespace
{
    ...
    protected :
        CPLErr readData(void* data, int srcX, int srcY, int srcW, int srcH, int dstW, int dstH, int elemWidth);
    public :
    ...
    colorTable(nullptr)
{
    if (nullptr != this->dataset) {
        ...
        if ((alphaBand == 0 && numBands >= 3) || (alphaBand != 0 && numBands > 3)) {
            ...
            if (alphaBand != 0) {
                this->bandRequest.insert(this->bandRequest.end(), {1, 1, 1, alphaBand});
                this->format = Format::RGBA;
            }
            else {
                this->bandRequest.insert(this->bandRequest.end(), {1, 1, 1});
...
namespace
{
    ...
    CPLErr AbstractReaderImpl::readData(void* data, int srcX, int srcY, int srcW, int srcH, int dstW, int dstH, int elemWidth)
    {
        const int numDataElements = gdalReader->getNumDataElements();
        const int nPixelSpace = getPixelSpacing(dstW, dstH, numDataElements, elemWidth);
        const int nLineSpace = getLineSpacing(dstW, dstH, numDataElements, elemWidth);
        const int nBandSpace = getBandSpacing(dstW, dstH, numDataElements, elemWidth);

        return gdalReader->getDataset()->RasterIO(GF_Read,
            srcX, srcY, srcW, srcH, data,
            dstW, dstH,
            gdalReader->getDataset()->GetRasterBand(1)->GetRasterDataType(),
    ...
    CPLErr ByteReaderImpl::read(int srcX, int srcY, int srcW, int srcH, int dstW, int dstH,
        ...
        CPLErr success = readData(data, srcX, srcY, srcW, srcH, dstW, dstH, 1);
        // scale if necessary
        if (success == CE_None && this->abpp < this->nbpp)
            scaleABPP(data, dstW * dstH * gdalReader->getNumDataElements(), this->scaleMaxValue);
        return success;
    }
    ...
    CPLErr ShortReaderImpl::read(int srcX, int srcY, int srcW, int srcH, int dstW, int dstH,
        ...
        CPLErr success = readData(arr.data(), srcX, srcY, srcW, srcH, dstW, dstH, 2);
        if (success == CE_None) {
            if (this->abpp < this->nbpp)
                scaleABPP(arr.data(), data, numSamples, this->scaleMaxValue);
            else
                scale(arr.data(), data, numSamples);
        }
        return success;
    }
    ...
    CPLErr IntReaderImpl::read(int srcX, int srcY, int srcW, int srcH, int dstW, int dstH,
        ...
        CPLErr success = readData(arr.data(), srcX, srcY, srcW, srcH, dstW, dstH, 4);
    ...
    CPLErr DoubleReaderImpl::read(int srcX, int srcY, int srcW, int srcH, int dstW, int dstH,
        ...
        CPLErr success = readData(arr.data(), srcX, srcY, srcW, srcH, dstW, dstH, 8);
        if (success == CPLErr::CE_None)
            scale(arr.data(), data, numSamples);
        return success;
    }
