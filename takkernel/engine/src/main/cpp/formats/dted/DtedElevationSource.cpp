...
namespace
{
        ...
        cellFormat(nullptr, NAN, NAN, NAN, nullptr)
    {
        ...
        unsigned formatMask = 0u;
        constexpr unsigned dt3_mask = 0x8u;
        constexpr unsigned dt2_mask = 0x4u;
        constexpr unsigned dt1_mask = 0x2u;
        constexpr unsigned dt0_mask = 0x1u;
        ...
        if(formatMask&dt3_mask)
            formats.push_back(dt3);
        if(formatMask&dt2_mask)
            formats.push_back(dt2);
        if(formatMask&dt1_mask)
            formats.push_back(dt1);
        if(formatMask&dt0_mask)
    ...
    TAKErr CursorImpl::moveToNext() NOTHROWS
    {
        ...
        do
        {
            ...
            const auto formatIdx = (this->idx%this->numFormats);
            if(formatIdx == 0)
                filename = nullptr;
            if(!filename) {
            ...
            f.append(filename);
            f.append(this->formats[formatIdx].extension);
            ...
            this->cellFormat = this->formats[formatIdx];
            break;
        } while (true);
