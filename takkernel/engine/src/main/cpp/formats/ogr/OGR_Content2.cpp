...
namespace {
    ...
    bool isEmpty (const OGRGeometry* geometry) {
        bool empty = true;
            ...
            if(OGR_GT_IsSubClassOf(geometry->getGeometryType(), wkbGeometryCollection)) {
                ...
                if (collection) {
                    ...
                    for (int i (0); i < elementCount; ++i) {
                        empty &= isEmpty(collection->getGeometryRef (i));
                        if(!empty)
                            break;
                    }
                } else {
...
OGR_Content2::OGR_Content2(const char* filePath, char **openOptions, std::size_t areaThreshold)
    ...
    } else if(strcmp(driver->getDriverName(), "LIBKML") == 0) {
        layerNeedsLod = false;
    }
...
bool OGR_Content2::moveToNextFeatureImpl() NOTHROWS
{
    ...
    do {
        ...
        if (currentFeature.get ()) {
            ...
            // if the feature does not have any non-empty geometries, skip
            if(!currentGeometry || isEmpty(currentGeometry.get()))
                continue;
                ...
                case OFTInteger64List:
                    {
                        ...
                        const int64_t* start
// XXX--
#if defined(__APPLE__) || (defined(__ANDROID__) && (defined(__aarch64__) || defined(__x86_64__))) || defined(__LINUX__)
...
std::size_t OGR_Content2::preprocessDataset ()
{
    ...
    if(!layerNeedsLod) {
        // if the provider configures LODs, skip 
        levelOfDetail = 0;
    } else if (featureCount > 1 || featureCount > pointCount) {
        // auto compute LOD based on feature density heuristic
    ...
    } // else -- Don't calculate level of detail for no features or just a single point.
            ...
            if (index >= 0) {
                ...
                if (fieldDef && fieldDef->GetType () == OFTString) {
                    ...
                    if (!TAK::Engine::Port::String_trim(trimmed, currentFeatureName)) {
                        ...
                        const auto namelen = strlen(currentFeatureName);
                        for(int i = 0; i < namelen; i++)
                            if(currentFeatureName[i] == '\n')
                                currentFeatureName[i] = ' ';
                    }
                }
            }
        }
...
TAKErr OGR_Content2::createLegacyResult(std::unique_ptr<feature::FeatureDataSource::FeatureDefinition> &legacyResultOut, bool *vis) const {
    ...
    for (int i = 0; i < currentFeature->GetFieldCount(); ++i) {
        ...
        if (!TAK::Engine::Port::String_strcasecmp(name, "altitudeMode")) {
            ...
            if (!TAK::Engine::Port::String_strcasecmp(value, "clampToGround"))
                ...
                altitudeMode = 2;
        } else if (!TAK::Engine::Port::String_strcasecmp(name, "tessellate")) {
            tessellate = currentFeature->GetFieldAsInteger(i) != 0;
    ...
    if (styleString && styleString[0] == '@') {
        ...
        // XXX - this looks broken, do something with `tableStyle` ???
        if (styleTable) {
    ...
    // if the style could not be derived, do another attempt with the driver to pull a default style
    if(!styleString) {
        std::unique_ptr<OGRFeature, void(*)(OGRFeature *)> empty(currentFeature->Clone(), OGRFeature::DestroyFeature);
        empty->SetStyleString(nullptr);
        driver->getStyle(styleString, *empty, *currentGeometry);
    }
