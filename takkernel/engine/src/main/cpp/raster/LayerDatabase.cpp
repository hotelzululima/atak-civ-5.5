...
//    db.setTransactionSuccessfu l ();
  }

bool
LayerDatabase::removeLayer(const DatasetDescriptor &descriptor)
    NOTHROWS
  {
    Lock lock(getMutex());
    // verify that the descriptor is in the store and obtain the associated file
    TAK::Engine::Port::String file;
    std::vector<const char *> filev;
    {
        std::ostringstream strm;
        strm << descriptor.getLayerID();
        auto result = queryLayersInternal(std::vector<const char*>(1, COLUMN_LAYERS_INFO),
            COL_LAY_ID " = ?",
            std::vector<const char*>(1, strm.str().c_str()));
        if (!result.moveToNext())
            return false;
        if(*result.getLayerInfo() != descriptor)
            return false;
        file = result.getPath();
        filev.push_back(file.get());
    }
    // delete the layer
    db::Database& db (getDatabase ());
    std::unique_ptr<db::Statement> stmt(db.compileStatement("DELETE FROM layers WHERE id = ?"));
    stmt->bind(1, descriptor.getLayerID());
    stmt->execute();
    // if there are no remaining layers for the file, evict
    {
        std::ostringstream strm;
        strm << "SELECT 1 FROM " << TABLE_LAYERS << " WHERE " << COLUMN_LAYERS_PATH << " = ? LIMIT 1";
        std::unique_ptr<db::Cursor> result(db.query(strm.str().c_str(), filev));
        if (!result->moveToNext())
            deleteCatalogPath(file, false);
    }
