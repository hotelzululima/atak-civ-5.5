            ...
            namespace {
                bool initImpl(const char *gdalDir)
                {
                    if (gdalDir) {
                        ...
                        gdalDataVerPath = gdalDataVer.c_str();
#else
                        std::filesystem::path gdalDataVer(gdalDir);
                        gdalDataVer.append("gdal.version");
                        gdalDataVerPath = gdalDataVer.string().c_str();
#endif
                        if (util::pathExists(gdalDataVerPath)) {
                            util::FileInput input;
                            int devVersion = 0;
                            try {
                                input.open(gdalDataVerPath);
                                devVersion = input.readAsciiInt(util::getFileSize(gdalDataVerPath));
                            }
                            catch (util::IO_Error&) {
                            }
                            try { input.close(); }
