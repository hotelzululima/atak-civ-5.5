...
namespace atakmap {
    namespace raster{
        namespace osm {
            ...
            double OSMUtils::mapnikTileXd(int level, double lng)
            {
                int n = 1 << level;
                return (n * ((lng + 180.0) / 360.0));
            }
            ...
            double OSMUtils::mapnikTileYd(int level, double lat)
            {
                int n = 1 << level;
                double lat_rad = math::toRadians(lat);
                return (n * (1.0 - (log(tan(lat_rad) + (1.0 / cos(lat_rad))) / M_PI)) / 2.0);
            }
            ...
            double OSMUtils::mapnikTileResolution(double level, double lat)
            {
                if (level >= 32)
                    return 0.0;
                return 156543.034 * cos(lat*M_PI/180.0) / pow(2.0, level);
            }
