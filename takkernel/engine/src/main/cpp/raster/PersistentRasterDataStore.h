...
class ENGINE_API PersistentRasterDataStore
  ...
  public:                               //                      PUBLIC        //
    ...
    //
    // Removes a single dataset. If the dataset is currently available from the
    //  store, it will be removed and no longer available via future query
    // operations. If it is the only dataset associated with a given file, that
    // file will be removed from the store as well. If there are additional
    // datasets associated with the same file, those datasets as well as the
    // file will remain available from the store. In no case will removing a
    // dataset modify the underlying file.
    //

    bool
    removeDataset (const DatasetDescriptor &desc)
        NOTHROWS;
  ...
  private:                              //                      PRIVATE       //
    ...
    std::unique_ptr<LayerDatabase> layerDB;
