...
class FactoryRegistry
  ...
  private :
      struct Spi
      {
          const raster::DatasetDescriptor::Factory *value{nullptr};
          std::size_t insert{0u};
      };
      ...
      struct Spi_LT
      {
          bool operator()(const Spi &a, const Spi &b) const
          {
              ...
              else if (a.value->getPriority() > b.value->getPriority())
                  return true; // `a` is higher priority than `b`
              else if (a.value->getPriority() < b.value->getPriority())
                  return false; // `a` is lower priority than `b`
              else
                  return (a.insert < b.insert); // `a` inserted before `b`
          }
      };
                                        //====================================//
  public:                               //                      PUBLIC        //
    ...
    //
    const raster::DatasetDescriptor::Factory*
    getFactory (const char* strategy) const
      {
        ReadLock lock(_mutex);
        for (auto& spi : _spis)
        {
            if (strcmp(spi.value->getStrategy(), strategy) == 0)
                return spi.value;
        }
        return nullptr;
      }
      raster::DatasetDescriptor::DescriptorSet *
      create(const std::pair<const char *, const char *> &args, const char *strategy, raster::DatasetDescriptor::Factory::CallbackType *callback) const
      {
        ReadLock lock(_mutex);
        for (auto& spi : _spis)
        {

            if (!strategy || strcmp(spi.value->getStrategy(), strategy) == 0) {
                auto result = spi.value->create(args, strategy, callback);
                if (result)
                    return result;
            }
        }
      ...
      bool
      isSupported (const std::pair<const char*, const char *> &args, const int probeLimit)
      {
        struct ProbeCB : public raster::DatasetDescriptor::Factory::CallbackType
        {
            std::size_t getProbeLimit() const NOTHROWS override { return limit; }
            bool isCanceled() const override { return false; }
            bool isProbeOnly() const NOTHROWS override { return true; }
            void setError(const char *msg) override { }
            void setProbeResult(bool value) override { matched = value; }
        ...
        for (auto& spi : _spis)
        {
            spi.value->create(args, nullptr, &probe);
            if (probe.matched)
                return true;
        }
        return false;
      ...
      void
      registerProvider (const raster::DatasetDescriptor::Factory* factory)
        {
      ...
      void
      unregisterProvider (const raster::DatasetDescriptor::Factory* factory)
        {
          if (factory && factory->getStrategy ())
            {
              WriteLock lock(_mutex);
              for(auto it = _spis.begin(); it != _spis.end(); it++) {
                  if((*it).value == factory) {
                      _spis.erase(it);
                      break;
                  }
  ...
  private :
      ...
      std::set<Spi, Spi_LT> _spis;
      std::size_t _insert{ 0u };
  };
...
//
// Returns true if the string can be encoded as ASCII, false it if includes unicode
//
bool
isAsciiEncodeable(const char *str) NOTHROWS
  {
    if (!str)
        return true;
    const auto limit = strlen(str);
...
//
// Read a null terminated string of chars from the supplied stream.
//
TAK::Engine::Port::String
readCharStr(std::istream& strm)
  {
...
//
// Write the supplied null terminated string of chars to the supplied stream.
//
std::ostream&
writeCharStr(std::ostream& strm, 
    const char* begin)
  {
...
DatasetDescriptor::decode (std::istream& strm)
  {
    ...
    if (version == 0x07)
      ...
      {
        return decodeVersion10_11(strm, atakmap::util::readUTF);
      }
    else if (version == 0x0B)
      {
        return decodeVersion10_11(strm, readCharStr);
      }
    else
      {
        throw util::IO_Error (MEM_FN ("decode")
...
DatasetDescriptor::encode (int64_t layer_id,
                           std::ostream& strm)
  {
    ...
    // version 12+ will always use `writeCharStr`
    constexpr auto version10 = (char)0x0A;
    constexpr auto version11 = (char)0x0B;

    char encodeVersion = version10;
    if (!isAsciiEncodeable(name_))
        encodeVersion = (char)0x0B;
    if (!isAsciiEncodeable(uri_))
        encodeVersion = (char)0x0B;
    if (!isAsciiEncodeable(provider_))
        encodeVersion = (char)0x0B;
    if (!isAsciiEncodeable(dataset_type_))
        encodeVersion = (char)0x0B;
    if (!isAsciiEncodeable(working_directory_))
        encodeVersion = (char)0x0B;
    for (auto iter (imagery_types_.begin ());
         iter != imagery_types_.end ();
         ++iter)
      {
        const auto iType = *iter;
        if (!isAsciiEncodeable(iType))
            encodeVersion = (char)0x0B;
      }
    for (auto iter (extra_data_.begin ());
         iter != extra_data_.end ();
         ++iter)
      {
        if (!isAsciiEncodeable(iter->first))
            encodeVersion = (char)0x0B;
        if (!isAsciiEncodeable(iter->second))
            encodeVersion = (char)0x0B;
      }

    auto writeString = (encodeVersion == 0x0A) ? atakmap::util::writeUTF : writeCharStr;

    strm.put (static_cast<char>(0x00)).put (static_cast<char>(0x01)).put (static_cast<char>(0xFFu)).put (encodeVersion);   // Header & version.


    write<int> (strm, type_);
    write (strm, layer_id);
    writeString(strm, name_);
      
      TAK::Engine::Port::String URIStr = uri_.get();
      TAK::Engine::Util::File_getStoragePath(URIStr);
      uri_ = static_cast<const char *>(URIStr.get());
      
    writeString(strm, uri_);
    writeString (strm, provider_);
    writeString (strm, dataset_type_);
    ...
    if (working_directory_)
      {
        writeString (strm, working_directory_);
      }
    ...
    for (StringMap::const_iterator iter (extra_data_.begin ());
         ...
         ++iter)
      {
        writeString (strm, iter->first);
        writeString (strm, iter->second);
      }
...
DatasetDescriptor::decodeVersion9(std::istream& strm)
  ...
  DatasetDescriptor*
  DatasetDescriptor::decodeVersion10_11(std::istream& strm, TAK::Engine::Port::String(*readString)(std::istream &))
  {
      using namespace util;
      ...
      Type descriptorType(read<Type>(strm));
      auto layerID(read<int64_t>(strm));
      TAK::Engine::Port::String name(readString(strm));
      TAK::Engine::Port::String uri(readString(strm));
      TAK::Engine::Port::String provider(readString(strm));
      TAK::Engine::Port::String datasetType(readString(strm));
      std::size_t typeCount(read<uint32_t>(strm));
      StringVector imageryTypes(typeCount);
      ResolutionMap resolutions;
      CoverageMap coverages;
      ...
      for (std::size_t i(0); i < typeCount; ++i)
      {
          TAK::Engine::Port::String imageryType(readString(strm));
          auto minRes = read<double>(strm);
          auto maxRes = read<double>(strm);
      ...
      int referenceID(read<int32_t>(strm));
      bool isRemote(read<bool>(strm));
      TAK::Engine::Port::String workingDirectory(read<bool>(strm) ? readString(strm) : nullptr);
      std::size_t dataCount(read<uint32_t>(strm));
      StringMap extraData;

      for (std::size_t i(0); i < dataCount; ++i)
      {
          TAK::Engine::Port::String k(readString(strm));
          TAK::Engine::Port::String v(readString(strm));
      ...
      case MOSAIC:
      {
          TAK::Engine::Port::String mosaicFile(read<bool>(strm) ? readString(strm) : nullptr);
          TAK::Engine::Port::String mosaicProvider(read<bool>(strm)
              ? readString(strm)
              : nullptr);
