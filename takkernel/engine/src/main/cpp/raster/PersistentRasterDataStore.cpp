...
class LayerCursorImpl
  ...
  public:                               //                      PUBLIC        //
      ...
      LayerCursorImpl(DescriptorMap &descriptorMap_,
                     ...
                     std::set<std::string> availableMounts_,
      ...
      : DatasetDescriptorCursor (std::move(subject)),
        ...
        availableMounts(availableMounts_)
      {
      }
...
class ValidateCurrency
  ...
  private:                              //                      PRIVATE       //
    ...
    {
    public:
        StringResultIterator(const std::vector<std::string> &results);
            ...
            if (iter == descriptorMap.end())
            {
                try
                {
                    ...
                    dataset.reset(raster::DatasetDescriptor::decode
                        (getBlob(colInfo)));
#ifdef _MSC_VER
                    std::string path(getString(colPath));
                    bool exists = false;
                    IO_exists(&exists, path.c_str());
                    if (!exists) {
                        const auto mountSepIdx = path.find(':');
                        if (mountSepIdx == std::string::npos)
                            continue;
                        std::string mount = path.substr(0, mountSepIdx + 1u);
                        if (availableMounts.find(mount) != availableMounts.end())
                            continue;
                        // mount is not currently available, check other mounts for removable/network drives and see if path exists
                        for (const auto& mp : availableMounts) {
                            bool isNwDrive = false;
                            IO_isNetworkDirectory(&isNwDrive, mp.c_str());
                            if (!isNwDrive)
                                continue;
                            auto remappedPath =  mp + path.substr(mountSepIdx + 1u);
                            bool remappedExists = false;
                            IO_exists(&exists, remappedPath.c_str());
                            if (exists)
                                break;
                        }
                        if(!exists)
...
ValidateCurrency::isValid (const char* filePath,
    ...
    const
  {
    ...
    if (valid)
      {
        ...
        if (valid) {
            ...
            if (isDir && size < LARGE_DATASET_RECURSE_LIMIT) {
                std::size_t actual;
                if (is_network_drive)
    ...
    StringResultIterator::StringResultIterator(const std::vector<std::string> &impl_) :
        impl(impl_),
        idx(0)
    {}
...
bool
PersistentRasterDataStore::removeDataset(const DatasetDescriptor &dataset) NOTHROWS
  {
    TAK::Engine::Thread::Lock lock(getMutex());
    const bool removed = layerDB->removeLayer(dataset);
...
PersistentRasterDataStore::getDatasetNames ()
    const
  {
      std::unique_ptr<DatasetDescriptorCursor> result(queryDatasets(DatasetQueryParameters()));
      std::set<std::string> names;
      std::vector<std::string> names_v;
      while (result->moveToNext()) {
          const auto &desc = result->get();
          const char *cname = desc.getName();
          if (!cname)
              continue;
    ...
    return new StringResultIterator(names_v);
  }
...
PersistentRasterDataStore::getDatasetTypes ()
    const
  {
      ...
      while (result->moveToNext()) {
          const auto &desc = result->get();
          const char *cname = desc.getDatasetType();
          if (!cname)
              continue;
          std::string name(cname);
          if (!names.insert(name).second)
...
PersistentRasterDataStore::getImageryTypes ()
    const
  {
      ...
      while (result->moveToNext()) {
          const auto &desc = result->get();
          const auto &cnames = desc.getImageryTypes();
...
PersistentRasterDataStore::getProviders ()
    const
  {
      ...
      while (result->moveToNext()) {
          const auto &desc = result->get();
          const char *cname = desc.getProvider();
          if (!cname)
              continue;
          std::string name(cname);
          if (!names.insert(name).second)
...
PersistentRasterDataStore::queryDatasets (const DatasetQueryParameters& params)
    const
  {
    ...
    sqlStrm << "SELECT id, info, path FROM layers";
    if (!wheres.empty ())
      {
        if (whereClause)
            sqlStrm << " WHERE";
