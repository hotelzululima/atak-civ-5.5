...
{
    return fabs(range)*tan((vfov/2.0)*M_PI / 180.0)/((double)sceneHeightPixels/2.0);
}
...
{
    return (fabs(gsd)*((double)sceneHeightPixels / 2.0) / std::tan((vfov/2.0)*M_PI / 180.0));
}

double TAK::Engine::Core::MapSceneModel2_computeSSE(const MapSceneModel2& scene, const double distance, const double geometricError) NOTHROWS
{
    // project geometric error
    const double lambda = (scene.height / 2.0) / tan((scene.camera.fov / 2.0) / 180.0 * M_PI);
    return lambda * (geometricError / distance);
}
...
namespace
{
        ...
        const double nearMeters_, const double farMeters_, const MapCamera2::Mode cameraMode, bool fromGeo) NOTHROWS
    {
        ...
        if(!TE_ISNAN(focusGeo.altitude)) {
            offsetRange -= fabs(focusGeo.altitude);
        }
