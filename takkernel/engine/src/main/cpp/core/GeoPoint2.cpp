...
namespace
{
    ...
    GeoPoint2 greatCirclePointAtDistance(const double lat1, const double lng1, const double brng, const double d) NOTHROWS
    {
        ...
        double y = sin(brng / 180.0 * M_PI) * sin(d / R) * cos(rlat1);
        double x = cos(d / R) - sin(rlat1) * sin(rlat2);

        // correct for pole case where x and y should both be 0 but sin(M_PI) is a small epsilon
        // atan2(0, 0) is dramatically different than atan2(epsilon, 0)
        if (std::abs(y) < std::numeric_limits<double>::epsilon())
            y = 0;

        const double rlng2 = rlng1 + atan2(y, x);
    ...
    GeoPoint2 hae(const GeoPoint2 &p) NOTHROWS
    {
        ...
        if (TE_ISNAN(p.altitude) || p.altitudeRef == AltitudeReference::HAE) {
            // no-op
        } else {
            // AGL
            retval.altitude = NAN;
...
double TAK::Engine::Core::GeoPoint2_slantAngle(const GeoPoint2 &a, const GeoPoint2 &b, const bool quick) NOTHROWS
{
    ...
    if (quick)
    {
        if (TE_ISNAN(a.altitude) || TE_ISNAN(b.altitude))
            return NAN;
...
GeoPoint2 TAK::Engine::Core::GeoPoint2_pointAtDistance(const GeoPoint2& a, const double az, const double distance, const double inclination, const bool quick) NOTHROWS
{
    ...
    if (!TE_ISNAN(inclination) && inclination && !TE_ISNAN(retval.altitude))
    {
