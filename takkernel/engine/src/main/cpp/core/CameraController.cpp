...
{
    return CameraController_panBy(renderer, tx, ty, MapRenderer::DisplayOrigin::UpperLeft, collide, poleSmoothScroll, animate);
}
TAKErr TAK::Engine::Core::CameraController_panBy(MapRenderer2 &renderer, const float tx, const float ty, const MapRenderer::DisplayOrigin origin, const MapRenderer2::CameraCollision collide, const bool poleSmoothScroll, const bool animate) NOTHROWS
{
    TAKErr code(TE_Ok);
    MapSceneModel2 currentModel;
    code = renderer.getMapSceneModel(&currentModel, false, origin);
    TE_CHECKRETURN_CODE(code);
    RendererCamera cam(renderer);
    const float tyAdj = ty * ((origin == MapRenderer::DisplayOrigin::LowerLeft) ? -1.0f : 1.0f);
    if(currentModel.camera.mode == MapCamera2::Scale)
        return panByImpl_ortho(currentModel, tx, tyAdj, animate, cam);
    else
        return panByImpl_perspective(currentModel, tx, tyAdj, poleSmoothScroll, animate, cam);
}
...
{
    return CameraController_panTo(renderer, focus, x, y, MapRenderer::DisplayOrigin::UpperLeft, collide, poleSmoothScroll, animate);
}
TAKErr TAK::Engine::Core::CameraController_panTo(MapRenderer2 &renderer, const GeoPoint2 &focus, const float x, const float y, const MapRenderer::DisplayOrigin origin, const MapRenderer2::CameraCollision collide, const bool poleSmoothScroll, const bool animate) NOTHROWS
{
    TAKErr code(TE_Ok);
    MapSceneModel2 sm;
    code = renderer.getMapSceneModel(&sm, false, origin);
    TE_CHECKRETURN_CODE(code);
    GeoPoint2 camLLA;
    ...
    } else if(camLLA.altitude < focus.altitude) {
        // NOTE: interactive pan is intended for dragging points that are visible on screen to a
        // given location within the current viewport. Attempting to use this function with inputs
        // inconsistent will lead to poor results.

        // reset requested focus to scene focus
        return renderer.lookAt(focus,
                   MapSceneModel2_gsd(fabs(focus.altitude) + 0.1, sm.camera.fov, sm.height),
                   sm.camera.azimuth,
...
namespace {
                      ...
                      const bool animate) NOTHROWS
    {
        ...
        do {
            ...
            // `inverse` will produce bad result if camera is below the focus point so just skip GSD adjustment
            GeoPoint2 sm_cam;
            sm.projection->inverse(&sm_cam, sm.camera.location);
            if (sm_cam.altitude < focus.altitude)
                break;
            ...
            const double rangeAltAdj = TE_ISNAN(focus.altitude) ? 0.0 : fabs(focus.altitude);
    ...
    TAKErr panToImpl_perspective(const MapSceneModel2 &sm, const GeoPoint2 &focus_, const float x, const float y, const bool poleSmoothScroll, const bool animate, ICamera &camera) NOTHROWS
    {
        ...
        return camera.lookAt(
                newFocus,
                MapSceneModel2_gsd(rangeAtAlt+fabs(curFocusAtAlt.altitude), sm.camera.fov, sm.height),
                newAzimuth,
                90.0 + sm.camera.elevation,
                animate);
    }
