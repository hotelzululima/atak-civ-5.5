...
#define PROJ2_CLASS_DECL(clazz) \
    ...
    PROJ2_CLASS_DECL(WGS84_3DProjection);
    PROJ2_CLASS_DECL(WebMercatorProjection);
    PROJ2_CLASS_DECL(EcefWGS84);
    PROJ2_CLASS_DECL(WorldMercatorProjection);
...
namespace
{
    ...
    int WGS84_3DProjection::getSpatialReferenceID() const NOTHROWS
    {
        return 4979;
    }
    TAKErr WGS84_3DProjection::forward(Point2<double> *value, const GeoPoint2 &geo) const NOTHROWS
    {
        EquirectangularProjection impl;
        return impl.forward(value, geo);
    }
    TAKErr WGS84_3DProjection::inverse(GeoPoint2 *value, const Point2<double> &proj) const NOTHROWS
    {
        EquirectangularProjection impl;
        return impl.inverse(value, proj);
    }
    double WGS84_3DProjection::getMinLatitude() const NOTHROWS
    {
        EquirectangularProjection impl;
        return impl.getMinLatitude();
    }
    double WGS84_3DProjection::getMaxLatitude() const NOTHROWS
    {
        EquirectangularProjection impl;
        return impl.getMaxLatitude();
    }
    double WGS84_3DProjection::getMinLongitude() const NOTHROWS
    {
        EquirectangularProjection impl;
        return impl.getMinLongitude();
    }
    double WGS84_3DProjection::getMaxLongitude() const NOTHROWS
    {
        EquirectangularProjection impl;
        return impl.getMaxLongitude();
    }
    bool WGS84_3DProjection::is3D() const NOTHROWS
    {
        EquirectangularProjection impl;
        return impl.is3D();
    }
...
#undef TO_DEGREES
    ...
    int WorldMercatorProjection::getSpatialReferenceID() const NOTHROWS
    {
        return 3395;
    }
    TAKErr WorldMercatorProjection::forward(Point2<double> *value, const GeoPoint2 &geo) const NOTHROWS
    {
#define TO_RADIANS(x) ((x)*(M_PI/180.0))
        const double a = Datum2::WGS84.reference.semiMajorAxis;              // WGS84 semi-major axis
        const double b = Datum2::WGS84.reference.semiMinorAxis;         // WGS84 semi-minor axis
        const double e = sqrt(1.0 - ((b*b) / (a*a)));  // ellipsoid eccentricity

        const double c = pow((1.0 - e*sin(TO_RADIANS(geo.latitude))) / (1.0 + e*sin(TO_RADIANS((geo.latitude)))), e/2.0);

        value->y = a * log(tan(M_PI/4.0 + TO_RADIANS(geo.latitude)/2.0) * c);
        value->x = a * TO_RADIANS(geo.longitude);
#undef TO_RADIANS
        if (TE_ISNAN(geo.altitude)) {
            value->z = 0;
        } else if(geo.altitudeRef == AltitudeReference::HAE) {
    ...
    TAKErr WorldMercatorProjection::inverse(GeoPoint2 *value, const Point2<double> &proj) const NOTHROWS
    {
#define TO_DEGREES(x) ((x)*(180.0/M_PI))
#define TO_RADIANS(x) ((x)*(M_PI/180.0))
        const double a = Datum2::WGS84.reference.semiMajorAxis;              // WGS84 semi-major axis
        ...
        const double lng = TO_DEGREES(proj.x/a);

        const double t = exp(-1.0*proj.y/a);
        double lat = 90.0 - 2.0*TO_DEGREES(atan(t));
        double delta;
        int it = 0;
        do {
            const double sinLat = sin(TO_RADIANS(lat));
            double lat1 = 90.0 - 2.0*TO_DEGREES(atan(t*pow((1.0-e*sinLat)/(1.0+e*sinLat), e/2.0)));
            delta = fabs(lat1-lat);
            lat = lat1;
...
#undef TO_RADiANS
    ...
    double WorldMercatorProjection::getMinLatitude() const NOTHROWS
    {
        return -worldMercatorLatitudeLimit;
    }
    double WorldMercatorProjection::getMaxLatitude() const NOTHROWS
    {
        return worldMercatorLatitudeLimit;
    }
    double WorldMercatorProjection::getMinLongitude() const NOTHROWS
    {
        return -180;
    }
    double WorldMercatorProjection::getMaxLongitude() const NOTHROWS
    {
        return 180;
    }
    bool WorldMercatorProjection::is3D() const NOTHROWS
    {
        return false;
    }
    ...
    TAKErr InternalProjectionSpi::create(Projection2Ptr &value, int srid) NOTHROWS
    {
        ...
        case 4978:
            ...
            break;
        case 3395:
            value = Projection2Ptr(&proj3395, Memory_leaker_const<Projection2>);
            break;
        case 4979:
            value = Projection2Ptr(&proj4979, Memory_leaker_const<Projection2>);
            break;
        default:
            return TE_InvalidArg;
        }
