...
TAKErr BlockPoolAllocator::allocate(std::unique_ptr<void, void(*)(const void *)> &value, const bool heapAllocOnExhaust) NOTHROWS
{
    do {
        ...
        value = std::unique_ptr<void, void(*)(const void *)>(static_cast<unsigned char *>(block) + metadata.blockSize.reserved, BlockPoolAllocator::freePoolBlock);
        return TE_Ok;
    } while (false);
...
void BlockPoolAllocator::freePoolBlock(const void* blockData) NOTHROWS
{
