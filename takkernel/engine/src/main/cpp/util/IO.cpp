...
void ByteBufferInput::close() {
    this->buffer = nullptr;
}

size_t ByteBufferInput::read(uint8_t *buf, size_t len) {
    return this->buffer->get(buf, len);
}

uint8_t ByteBufferInput::readByte() {
    return this->buffer->get();
}

size_t ByteBufferInput::skip(size_t n)  {
...
void RewindDataInput::close() {
    innerInput.close();
    rewindCache.resize(0);
}

size_t RewindDataInput::read(uint8_t *buf, size_t len) {
...
uint8_t RewindDataInput::readByte() {
...
int atakmap::util::parseASCIIInt(const char *src) 
{
    char *end = nullptr;
...
double atakmap::util::parseASCIIDouble(const char *src) 
{
