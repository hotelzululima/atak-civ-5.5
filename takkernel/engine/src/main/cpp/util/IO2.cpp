...
TAKErr TAK::Engine::Util::IO_getAbsolutePath(Port::String &value, const char *path) NOTHROWS
{
    try {
        ...
        f = std::filesystem::absolute(path);

        value = f.string().c_str();
        return TE_Ok;
    }
    catch (...) {
        return TE_Err;
...
TAKErr TAK::Engine::Util::IO_getRelativePath(Port::String &value, const char *basePath, const char *filePath) NOTHROWS
{
    try {
        ...
        std::filesystem::path commonPath(std::string(baseStr, i));
        if (commonPath.empty()) {
            // Nothing in common
            value = filePath;
            return TE_Ok;
        ...
        size_t remains = file.string().size() - i;
        ...
        if (remains > 0)
            ret.append(std::string(fileStr + i, remains));

        value = ret.string().c_str();
        return TE_Ok;
    } catch (...) {
        return TE_Err;
    }
...
TAKErr TAK::Engine::Util::IO_isNetworkDirectory(bool *value, const char *path) NOTHROWS
{
    ...
    try {
        if (std::filesystem::is_directory(path)) {
            ...
            *value = *value = (::GetDriveTypeA(drive) == 4);
...
bool TAK::Engine::Util::IO_exists(const char *path) NOTHROWS
{
    bool exists = false;
    return (IO_exists(&exists, path) == TE_Ok) && exists;
}
...
TAKErr TAK::Engine::Util::IO_getZipComment(Port::String &out, const char *vpath) NOTHROWS {
    out = "";
    std::pair<std::string, std::string> split = splitVPath(vpath);
    if (split.second.length() > 0) {
        ZipFilePtr zipPtr(nullptr, nullptr);
        ...
        code = zipPtr->getGlobalComment(out);
        TE_CHECKRETURN_CODE(code);
...
namespace
{
    ...
    TAKErr getFileCountImpl_Windows(std::size_t *result, std::string path, const std::size_t limit) NOTHROWS
    {
        ...
        try {
            ...
            {
                // path is expected to be UTF-8
                std::filesystem::path stdPath = std::filesystem::u8path(path.c_str());
    ...
    TAKErr DefaultFilesystem::createTempFile(TAK::Engine::Port::String &value, const char *prefix, const char *suffix, const char *dir) NOTHROWS
    {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (suffix) {
                ...
                remove(tmpPath);
                if (std::filesystem::exists(suffixPath.c_str()))
                    continue;
                {
                    // create the file
        ...
        try {
            for (int i = 0; i < 20; i++) {
                ...
                if (suffix) {
                    ...
                    // suffix is expected to be UTF-8
                    std::filesystem::path stdSuffixPath = std::filesystem::u8path(suffixPath);
                    if (std::filesystem::exists(stdSuffixPath))
                        continue;

                    if (std::filesystem::create_directories(stdSuffixPath))
                    {
                        std::filesystem::remove_all(tmpPath.get());
    ...
    TAKErr DefaultFilesystem::listFiles(TAK::Engine::Port::Collection<TAK::Engine::Port::String>& value, const char* path) NOTHROWS
    {
        ...
        try {
            ...
            {
                value.add(dir_entry.path().string().c_str());
            }
        }
        catch (...)
        {
    ...
    TAKErr DefaultFilesystem::length(int64_t *value, const char* path) NOTHROWS
    {
        ...
        // path is expected to be UTF-8
        std::filesystem::path stdPath = std::filesystem::u8path(path);
        try {
            ...
            if (path)
            {
                if(std::filesystem::is_regular_file(stdPath))
                {
                    *value = std::filesystem::file_size(stdPath);
                }
                else if (std::filesystem::is_directory(stdPath))
                {
                    for (const std::filesystem::directory_entry& dir_entry :
                        std::filesystem::directory_iterator(stdPath))
                    {
                        int64_t fs;
                        if(length(&fs, dir_entry.path().string().c_str()) == TE_Ok)
                            *value += fs;
                    }
                }
            }
        ...
        try {
...
#else
                    auto epoch = std::filesystem::last_write_time(stdPath).time_since_epoch();
                    auto nano = std::chrono::duration_cast<std::chrono::milliseconds>(epoch).count();

                    int64_t fileTimeMS = static_cast<uint64_t> (nano);
                    if (fileTimeMS)
                ...
                else if (std::filesystem::is_directory(stdPath))
                {
                    *value = -1LL;
                    ...
                    {
                        int64_t lm;
                        if(getLastModified(&lm, dir_entry.path().string().c_str()) == TE_Ok)
    ...
    TAKErr DefaultFilesystem::isDirectory(bool *value, const char* path) NOTHROWS
    {
        ...
        try {
            // path is expected to be UTF-8
            std::filesystem::path stdPath = std::filesystem::u8path(path);
            *value = std::filesystem::is_directory(stdPath);
            return TE_Ok;
        } catch (...) {
            return TE_Err;
        }
    ...
    TAKErr DefaultFilesystem::exists(bool *value, const char *path) NOTHROWS
    {
        ...
        try {
            // path is expected to be UTF-8
            std::filesystem::path stdPath = std::filesystem::u8path(path);
            *value = std::filesystem::exists(stdPath);
            return TE_Ok;
        } catch (...) {
            return TE_Err;
        }
