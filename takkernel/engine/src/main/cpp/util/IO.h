...
public:
     ...
     */
    virtual size_t readFully(uint8_t *buf, size_t len);
    ...
    */
    double readAsciiDouble(size_t len);
...
ENGINE_API int parseASCIIInt(const char *src) ;

ENGINE_API double parseASCIIDouble(const char *src) ;
