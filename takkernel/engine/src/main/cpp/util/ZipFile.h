    ...
    namespace Engine {
        namespace Util {
            ...
            public:
                ...
                /**
                 * Seek the first file entry of a given path. This can be a folder if path ends with '/'. Not all zip utilities create
                 * entries for folders, but this function will succeed if the folder exists as determined by prefix of a file entry.
                 * In the case that the archive does NOT have entries for folders, calling this method with a folder path will result
                 * in the current entry being the first file entry discovered with that folder prefix. Call getCurrentEntryPath to
                 * determine if the found entry is a folder path or it is a file with the folder prefix.
                 */
