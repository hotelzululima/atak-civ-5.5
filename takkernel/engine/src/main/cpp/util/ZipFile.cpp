...
struct ZipFile::Impl {
    ...
    TAKErr gotoEntry(const char* path) NOTHROWS {
        ...
        bool isFolder = corrected.size() > 0 && corrected.back() == '/';

        std::vector<char> buffer;
        int res = libkml_unzGoToFirstFile(handle);
        while (res == TE_Ok) {

            unz_file_info fileInfo;
            res = libkml_unzGetCurrentFileInfo(handle,
                &fileInfo,
                nullptr,
                0,
                nullptr,
            ...
            // if this is a folder, find the last '/' and term the string there for the compare
            if (isFolder) {
                char* slash = strrchr(buffer.data(), '/');
                if (slash) {
                    slash[1] = '\0';
                }
            }

            // default file system case sensitivity
            if (libkml_unzStringFileNameCompare(corrected.c_str(), buffer.data(), 0) == 0) {
                return TE_Ok;
            }

            res = libkml_unzGoToNextFile(handle);
        }
