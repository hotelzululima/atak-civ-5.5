...
namespace TAK {
  ...
  || defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
		...
		/** returns `false` on error as well as _file-does-not-exist_ */
		ENGINE_API bool IO_exists(const char *path) NOTHROWS;
	    ENGINE_API TAKErr IO_delete(const char *path) NOTHROWS;
	    ENGINE_API TAKErr IO_mkdirs(const char* dirPath) NOTHROWS;
        ENGINE_API TAKErr IO_truncate(const char* path, std::size_t size) NOTHROWS;
            ...
            ENGINE_API TAKErr IO_getZipComment(Port::String &out, const char *vpath) NOTHROWS;
            ENGINE_API TAKErr IO_setZipComment(const char *vpath, const char *comment) NOTHROWS;
            ...
            /**
             * Check if vpath is a directory on the file system or a folder inside a zip file
             */
