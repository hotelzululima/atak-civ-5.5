...
JNIEXPORT void JNICALL Java_com_atakmap_map_CameraController_panBy
  (JNIEnv *env, jclass clazz, jlong ptr, jfloat tx, jfloat ty, jboolean llOrigin, jint mcollide, jboolean poleSmoothScroll, jboolean animate)
{
    ...
    CameraController_panBy(*renderer, tx, ty, llOrigin ? MapRenderer::LowerLeft : MapRenderer::UpperLeft, ccollide, poleSmoothScroll, animate);
}
JNIEXPORT void JNICALL Java_com_atakmap_map_CameraController_panTo__JDDDFFZIZZ
  (JNIEnv *env, jclass clazz, jlong ptr, jdouble lat, jdouble lng, jdouble alt, jfloat x, jfloat y, jboolean llOrigin, jint mcollide, jboolean poleSmoothScroll, jboolean animate)
{
    MapRenderer2 *renderer = JLONG_TO_INTPTR(MapRenderer2, ptr);
    if(!renderer) {
        ATAKMapEngineJNI_checkOrThrow(env, TE_InvalidArg);
    ...
    MapRenderer::CameraCollision ccollide;
    JCAMERA_CONTROLLER_MARSHAL_COLLIDE(mcollide, ccollide);
    CameraController_panTo(*renderer, GeoPoint2(lat, lng, alt, AltitudeReference::HAE), x, y, llOrigin ? MapRenderer::LowerLeft : MapRenderer::UpperLeft, ccollide, poleSmoothScroll, animate);
}
