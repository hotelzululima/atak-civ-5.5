...
namespace
{
    class NullDataOutput : public DataOutput2
    {
        public:
...
JNIEXPORT jobject JNICALL Java_com_atakmap_map_layer_feature_geometry_GeometryFactory_fromEnvelope
        (JNIEnv *env, jclass clazz, jdouble minX, jdouble minY, jdouble minZ, jdouble maxX, jdouble maxY, jdouble maxZ)
{
    ...
    Envelope2 envelope(minX, minY, minZ, maxX, maxY, maxZ);
    code = GeometryFactory_fromEnvelope(cgeom, envelope);
    if(code != TE_Ok)
        return NULL;
...
JNIEXPORT jobject JNICALL Java_com_atakmap_map_layer_feature_geometry_GeometryFactory_createWedge
  (JNIEnv *env, jclass clazz, jlong locationPtr, jdouble startAngle, jdouble endAngle, jdouble range, jint algoPtr)
{
    TAKErr code(TE_Ok);
    Geometry2Ptr cgeom2Value(nullptr, nullptr);
    ...
    code = GeometryFactory_createWedge(cgeom2Value, *cgeomlocationPtr, startAngle, endAngle, range, alg);
    if(code != TE_Ok)
        return NULL;
