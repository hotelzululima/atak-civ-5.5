...
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLMapView_setStarsLayer
        (JNIEnv *env, jclass clazz, jlong ptr, jobject mstarslayer)
{
    GLGlobeBase *cview = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    if(!cview) {
        ATAKMapEngineJNI_checkOrThrow(env, TE_InvalidArg);
    ...
    GLMapRenderable2Ptr cstarslayer(nullptr, nullptr);
    if(mstarslayer)
        Renderer::Core::Interop_marshal(cstarslayer, *env, mstarslayer);
    cview->setStarsLayer(std::move(cstarslayer));
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr, jint tedm)
{
    ...
    if(cglobe->getControl(&ctrl, SurfaceRendererControl_getType()) != TE_Ok)
        return false;
    SurfaceRendererControl *surfaceControl = static_cast<SurfaceRendererControl *>(ctrl);
    return surfaceControl->isDrawModeEnabled((TAK::Engine::Model::DrawMode)tedm);
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr, jobject msurfaceBounds)
{
    ...
    public :
        ...
        virtual TAKErr clear() NOTHROWS override
        {
            Java::JNILocalRef mmbb(*env, NULL);
        ...
        virtual bool empty() NOTHROWS override
        {
            return true;
        }
        virtual TAKErr iterator(IteratorPtr &iterator) NOTHROWS override
        {
            return TE_NotImplemented;
        }
    };
...
JNIEXPORT jlong JNICALL Java_com_atakmap_map_opengl_GLMapView_getElevationSourceControl
    (JNIEnv *env, jclass clazz, jlong ptr)
{
    GLGlobeBase *cglobe = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    if(!cglobe) {
    ...
    void *ctrl = nullptr;
    cglobe->getControl(&ctrl, ElevationSourceControl_getType());
    return INTPTR_TO_JLONG(ctrl);
}
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLMapView_setElevationSource
    (JNIEnv *env, jclass clazz, jlong ptr, jobject melsrc)
{
    TAKErr code(TE_Ok);
    GLGlobeBase *cglobe = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    if(!cglobe) {
    ...
    if(!ctrl)
        return;
    std::shared_ptr<ElevationSource> celsrc;
    if(melsrc) {
        code = TAKEngineJNI::Interop::Elevation::Interop_adapt(celsrc, env, melsrc, false);
        if (ATAKMapEngineJNI_checkOrThrow(env, code))
            return;
    }
    code = static_cast<ElevationSourceControl *>(ctrl)->setElevationSource(celsrc);
    if(ATAKMapEngineJNI_checkOrThrow(env, code))
        return;
}
JNIEXPORT jobject JNICALL Java_com_atakmap_map_opengl_GLMapView_getElevationSource
    (JNIEnv *env, jclass clazz, jlong ptr)
{
    TAKErr code(TE_Ok);
    GLGlobeBase *cglobe = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    ...
    if(!ctrl)
        return nullptr;
    std::shared_ptr<ElevationSource> celsrc;
    code = static_cast<ElevationSourceControl *>(ctrl)->getElevationSource(celsrc);
    if(ATAKMapEngineJNI_checkOrThrow(env, code))
        return nullptr;
    if(!celsrc)
        return nullptr;
    return TAKEngineJNI::Interop::Elevation::Interop_adapt(env, celsrc, false);
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr, jint type)
{
    ...
    if(type == com_atakmap_map_opengl_GLMapView_IMPL_IFACE) {
        ...
        GLMapView2 *cview = static_cast<GLMapView2 *>(cglobe);
        return NewReference<TAK::Engine::Renderer::Elevation::TerrainRenderService>(env, cview->getTerrainRenderService());
    } else if(type == com_atakmap_map_opengl_GLMapView_IMPL_V2) {
        GLGlobe *cview = static_cast<GLGlobe *>(cglobe);
        return NewReference<TAK::Engine::Renderer::Elevation::TerrainRenderService>(env, cview->getTerrainRenderService());
    } else {
...
JNIEXPORT jboolean JNICALL Java_com_atakmap_map_opengl_GLMapView_inverse
  (JNIEnv *env, jclass clazz, jlong ptr, jdouble x, jdouble y, jdouble z, jint mmode, jobject mlla, jboolean requireModeResult)
{
    GLGlobeBase *cglobe = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    if(!cglobe) {
        ATAKMapEngineJNI_checkOrThrow(env, TE_InvalidArg);
    ...
    switch(mmode) {
        ...
        case com_atakmap_map_opengl_GLMapView_INVERSE_MODE_MODEL :
            ...
            hints |= MapRenderer::IgnoreTerrainMesh|MapRenderer::IgnoreSurfaceMesh;
            break;
        case com_atakmap_map_opengl_GLMapView_INVERSE_MODE_SURFACE :
            cmode = MapRenderer::InverseMode::RayCast;
            hints |= MapRenderer::IgnoreTerrainMesh;
            break;
        case com_atakmap_map_opengl_GLMapView_INVERSE_MODE_TERRAIN :
            cmode = MapRenderer::InverseMode::RayCast;
            hints |= MapRenderer::IgnoreSurfaceMesh;
            break;
        default :
            return false;
    }
    ...
    if(result == MapRenderer::None)
        return false;
    if(requireModeResult) {
        // the caller requires that the result match the requested mode
        switch(result) {
            case MapRenderer::InverseResult::GeometryModel :
                if(mmode != com_atakmap_map_opengl_GLMapView_INVERSE_MODE_MODEL)
                    return false;
                break;
            case MapRenderer::InverseResult::TerrainMesh :
                if(mmode != com_atakmap_map_opengl_GLMapView_INVERSE_MODE_TERRAIN)
                    return false;
                break;
            case MapRenderer::InverseResult::SurfaceMesh :
                if(mmode != com_atakmap_map_opengl_GLMapView_INVERSE_MODE_SURFACE)
                    return false;
                break;
            case MapRenderer::InverseResult::Transformed :
                if(mmode != com_atakmap_map_opengl_GLMapView_INVERSE_MODE_ABSOLUTE)
                    return false;
                break;
            default :
                return false;
...
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLMapView_setStarsEnabled
        (JNIEnv *env, jclass clazz, jlong ptr, jboolean enabled, jint type)
{
    GLGlobeBase *cglobe = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    if(!cglobe) {
    ...
    if(type == com_atakmap_map_opengl_GLMapView_IMPL_IFACE) {
        ...
        GLGlobe *cview = static_cast<GLGlobe *>(cglobe);
        cview->setStarsEnabled(enabled);
    } else {
        // no-op
    }
}
JNIEXPORT jboolean JNICALL Java_com_atakmap_map_opengl_GLMapView_isStarsEnabled
        (JNIEnv *env, jclass clazz, jlong ptr, jint type)
{
    GLGlobeBase *cglobe = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    if(!cglobe) {
...
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLMapView_setClampToGroundAtNadir
  (JNIEnv *env, jclass clazz, jlong ptr, jboolean v)
{
    GLGlobeBase *cglobe = JLONG_TO_INTPTR(GLGlobeBase, ptr);
    if(!cglobe) {
    ...
    void* ctrl{ nullptr };
    cglobe->getControl(&ctrl, ClampToGroundControl_getType());
    if (ctrl)
        static_cast<ClampToGroundControl*>(ctrl)->setClampToGroundAtNadir(v);
}
