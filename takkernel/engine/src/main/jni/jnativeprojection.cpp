...
JNIEXPORT jboolean JNICALL Java_com_atakmap_map_projection_NativeProjection_forward
  (JNIEnv *env, jclass clazz, jlong ptr, jdouble latitude, jdouble longitude, jdouble hae, jobject mpoint)
{
    ...
    if(proj->forward(&cpoint, GeoPoint2(latitude, longitude, hae, AltitudeReference::HAE, NAN, NAN)) != TE_Ok)
        return false;
...
JNIEXPORT jboolean JNICALL Java_com_atakmap_map_projection_NativeProjection_inverse
  (JNIEnv *env, jclass clazz, jlong ptr, jdouble x, jdouble y, jdouble z, jobject mgeopoint)
{
    ...
    GeoPoint2 cgeopoint;
    if(proj->inverse(&cgeopoint, Point2<double>(x, y, z)) != TE_Ok)
        return false;
    TAKErr code = Interop_copy(mgeopoint, env, cgeopoint);
