  ...
  (JNIEnv *env, jclass clazz, jobject jpointer, jstring jkey)
{
    ...
    try {
        ...
        Java::JNILocalRef retval(*env, env->NewObjectArray(count, ATAKMapEngineJNI_findClass(env, "java/lang/String"), NULL));
        for(std::size_t i = 0u; i < count; i++) {
            Java::JNILocalRef melem(*env, env->NewStringUTF(blob.first[i]));
            env->SetObjectArrayElement((jobjectArray)retval.get(), i, melem.get());
        }
        return (jobjectArray)retval.release();
    } catch(...) {
        ATAKMapEngineJNI_checkOrThrow(env, TE_InvalidArg);
        return 0;
    }
  ...
  (JNIEnv *env, jclass clazz, jobject jpointer, jstring jkey)
{
    ...
    try {
        ...
        Java::JNILocalRef retval(*env, env->NewObjectArray(count, ATAKMapEngineJNI_findClass(env, "[B"), NULL));
        for(std::size_t i = 0u; i < count; i++) {
            AttributeSet::Blob blob = blobArray.first[i];
            if(!blob.first)
                continue;
            Java::JNILocalRef melem(*env, JNIByteArray_newByteArray(env, reinterpret_cast<const jbyte *>(blob.first), blob.second-blob.first));
            env->SetObjectArrayElement((jobjectArray)retval.get(), i, melem.get());
        }
        return (jobjectArray)retval.release();
    } catch(...) {
  ...
  (JNIEnv *env, jclass clazz, jobject jpointer)
{
    ...
    try {
        ...
        Java::JNILocalRef retval(*env, env->NewObjectArray(cnames.size(), ATAKMapEngineJNI_findClass(env, "java/lang/String"), NULL));
        for(std::size_t i = 0u; i < cnames.size(); i++) {
            Java::JNILocalRef melem(*env, env->NewStringUTF(cnames[i]));
            env->SetObjectArrayElement((jobjectArray)retval.get(), i, melem.get());
        }
        return (jobjectArray)retval.release();
    } catch(...) {
