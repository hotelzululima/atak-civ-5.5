...
JNIEXPORT jlong JNICALL Java_com_atakmap_opengl_GLText_intern
  (JNIEnv *env, jclass clazz, jlong ctxPtr, jobject mtextFormat, jobject mglyphRenderer)
{
    const RenderContext *ctx = JLONG_TO_INTPTR(RenderContext, ctxPtr);
    ...
    return INTPTR_TO_JLONG(GLText2_intern(*ctx, std::move(cfmt)));
}
...
{
    auto nativeFactory = std::make_shared<ManagedTextFormatFactory>(*env);
