...
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLLabelManager_setHorizontalAlignment
        (JNIEnv *env, jclass clazz, jlong ptr, jint id, jint alignment)
{
    GLLabelManager *glLabelManager = JLONG_TO_INTPTR(GLLabelManager, ptr);
    ...
    {
        switch(alignment) {
            case com_atakmap_map_opengl_GLLabelManager_HORIZONTAL_ALIGNMENT_LEFT :
                glLabelManager->setHorizontalAlignment(id, TEHA_Left);
                break;
            case com_atakmap_map_opengl_GLLabelManager_HORIZONTAL_ALIGNMENT_CENTER :
                glLabelManager->setHorizontalAlignment(id, TEHA_Center);
                break;
            case com_atakmap_map_opengl_GLLabelManager_HORIZONTAL_ALIGNMENT_RIGHT :
                glLabelManager->setHorizontalAlignment(id, TEHA_Right);
                break;
        }
    }
}
...
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLLabelManager_setPriorityLevel
        (JNIEnv *env, jclass clazz, jlong ptr, jint priority)
{
    GLLabelManager *glLabelManager = JLONG_TO_INTPTR(GLLabelManager, ptr);
    if (glLabelManager != NULL)
    {
        switch(priority) {
            case com_atakmap_map_opengl_GLLabelManager_PRIORITY_ALWAYS :
                glLabelManager->setPriorityLevel(TEP_Always);
                break;
            case com_atakmap_map_opengl_GLLabelManager_PRIORITY_HIGH :
                glLabelManager->setPriorityLevel(TEP_High);
                break;
            case com_atakmap_map_opengl_GLLabelManager_PRIORITY_STANDARD :
                glLabelManager->setPriorityLevel(TEP_Standard);
                break;
            case com_atakmap_map_opengl_GLLabelManager_PRIORITY_LOW :
                glLabelManager->setPriorityLevel(TEP_Low);
                break;
        }
    }
}
JNIEXPORT jint JNICALL Java_com_atakmap_map_opengl_GLLabelManager_getPriorityLevel
        (JNIEnv *env, jclass clazz, jlong ptr)
{
    GLLabelManager *glLabelManager = JLONG_TO_INTPTR(GLLabelManager, ptr);
    ...
    if (glLabelManager != NULL)
    {
        switch(glLabelManager->getPriorityLevel()) {
            case TEP_Always:
                return com_atakmap_map_opengl_GLLabelManager_PRIORITY_ALWAYS;
            case TEP_High:
                return com_atakmap_map_opengl_GLLabelManager_PRIORITY_HIGH;
            case TEP_Standard:
                return com_atakmap_map_opengl_GLLabelManager_PRIORITY_STANDARD;
            case TEP_Low:
                return com_atakmap_map_opengl_GLLabelManager_PRIORITY_LOW;
            default:
                return com_atakmap_map_opengl_GLLabelManager_PRIORITY_LOW;
        }
    } else {
...
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLLabelManager_setFloatWeight
        (JNIEnv *env, jclass clazz, jlong ptr, jint id, jfloat weight)
{
    GLLabelManager *glLabelManager = JLONG_TO_INTPTR(GLLabelManager, ptr);

    if (glLabelManager != NULL)
    {
        glLabelManager->setFloatWeight(id, weight);
    }
}
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLLabelManager_setLevelOfDetail
  (JNIEnv *env, jclass clazz, jlong ptr, jint id, jint minLod, jint maxLod)
{
    auto lblmgr = JLONG_TO_INTPTR(GLLabelManager, ptr);
    if(lblmgr)
        lblmgr->setLevelOfDetail(id, minLod, maxLod);
}
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLLabelManager_setHeightInMeters
        (JNIEnv *env, jclass clazz, jlong ptr, jint id, jdouble heightInMeters)
{
    GLLabelManager *glLabelManager = JLONG_TO_INTPTR(GLLabelManager, ptr);

    if (glLabelManager != NULL)
    {
        glLabelManager->setHeightInMeters(id, heightInMeters);
    }
}
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLLabelManager_setHitTestable
        (JNIEnv *env, jclass clazz, jlong ptr, jint id, jboolean hittestable)
{
    auto lblmgr = JLONG_TO_INTPTR(GLLabelManager, ptr);
    if(lblmgr)
        lblmgr->setHitTestable(id, hittestable);
}
JNIEXPORT void JNICALL Java_com_atakmap_map_opengl_GLLabelManager_hitTest
        (JNIEnv *env, jclass clazz, jlong ptr, jfloat viewportX, jfloat viewportY, jfloat radius, jobject mgeo, jint limit, jobject mhitids)
{
    auto lblmgr = JLONG_TO_INTPTR(GLLabelManager, ptr);

    TAKErr code(TE_Ok);
    atakmap::core::GeoPoint geoPoint;
    if (mgeo) {
        GeoPoint2 cgeo;
        code = Interop_copy(&cgeo, env, mgeo);
        if (ATAKMapEngineJNI_checkOrThrow(env, code))
            return;
        geoPoint = cgeo;
    }
    if(lblmgr) {
        std::vector<uint32_t> chitids;
        TAK::Engine::Port::STLVectorAdapter<uint32_t> chitids_a(chitids);
        lblmgr->hitTest(chitids_a, viewportX, viewportY, radius, mgeo ? &geoPoint : nullptr, limit);
        if(!chitids.empty()) {
            LocalJNIEnv env;
            for (const auto id : chitids) {
                auto mid = TAKEngineJNI::Interop::Java::Integer_valueOf(*env, id);
                TAKEngineJNI::Interop::Java::JNICollection_add(*env, mhitids, mid.get());
            }
        }
    }
}
...
JNIEXPORT jint JNICALL Java_com_atakmap_map_opengl_GLLabelManager_getHintSurface
        (JNIEnv *env, jclass clazz)
{
    return GLLabel::Hints::Surface;
}
