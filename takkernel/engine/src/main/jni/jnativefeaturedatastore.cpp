#include "com_atakmap_map_layer_feature_NativeFeatureDataStore2.h"
...
JNIEXPORT jobject JNICALL Java_com_atakmap_map_layer_feature_NativeFeatureDataStore2_wrap
  (JNIEnv *env, jclass clazz, jobject mdatastore)
{
    FeatureDataStore2Ptr cdatastore(new TAKEngineJNI::Interop::Feature::ManagedFeatureDataStore2(*env, mdatastore), Memory_deleter_const<FeatureDataStore2, TAKEngineJNI::Interop::Feature::ManagedFeatureDataStore2>);
...
#define ADAPT_JSTRING_ARRAY(jarr, carr) \
    if(jarr) { \
        ...
        for(std::size_t i = 0u; i < length; i++) { \
            Java::JNILocalRef jname(*env, env->GetObjectArrayElement(jarr, i)); \
  ...
  (JNIEnv *env, jclass clazz, jlong ptr, jlongArray jfidsArr, jobjectArray jnamesArr, jintArray jgeomTypesArr, jboolean visibleOnly, jlong spatialFilterPtr, jlong minimumTimestamp, jlong maximumTimestamp, jint ignoredFeatureProperties, jint numSpatialOps, jintArray spatialOpTypesArr, jdoubleArray spatialOpArgsArr, jint numOrders, jintArray orderTypesArr, jdoubleArray orderArgsArr, jint limit, jint offset, jlong timeout)
{
    ...
    if(jnamesArr) {
        ...
        for(std::size_t i = 0u; i < length; i++) {
            Java::JNILocalRef jname(*env, env->GetObjectArrayElement(jnamesArr, i));
            JNIStringUTF name(*env, jname);
