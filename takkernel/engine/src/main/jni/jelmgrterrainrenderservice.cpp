#include "com_atakmap_map_opengl_ElMgrTerrainRenderService.h"
...
JNIEXPORT jobject JNICALL Java_com_atakmap_map_opengl_ElMgrTerrainRenderService_createDefaultLoResElevationSource
  (JNIEnv *env, jclass clazz)
{
    ElevationSourcePtr cloresel(nullptr, nullptr);
    ElMgrTerrainRenderService_getDefaultLoResElevationSource(cloresel);
    if(!cloresel)
        return nullptr;
    Java::JNILocalRef mloresel(*env, nullptr);
    Elevation::Interop_marshal(mloresel, *env, std::move(cloresel));
    return mloresel.release();
}
