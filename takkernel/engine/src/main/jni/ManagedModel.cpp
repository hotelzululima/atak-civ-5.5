...
TAKErr ManagedModel::getPosition(Point2<double> *value, const std::size_t index) const NOTHROWS
{
    ...
    if(!jiface.valid)
        return TE_Err;
    Java::JNILocalRef jxyz(*env, env->NewObject(jiface.PointD_clazz, jiface.PointD_ctor__DDD, 0, 0, 0));
    if(env->ExceptionCheck())
        return TE_Err;
    if(!jxyz)
        return TE_Err;
    env->CallVoidMethod(jmodel, jiface.getPosition, (jint)index, jxyz.get());
    if(env->ExceptionCheck())
        return TE_Err;
...
TAKErr ManagedModel::getTextureCoordinate(Point2<float> *value, const VertexAttribute texCoord, const std::size_t index) const NOTHROWS
{
    ...
    if(!jxyz)
        return TE_Err;
    env->CallVoidMethod(jmodel, jiface.getTextureCoordinate, 1, (jint)index, jxyz.get());
    if(env->ExceptionCheck())
        return TE_Err;
    value->x = env->GetDoubleField(jxyz, pointD_x);
    value->y = env->GetDoubleField(jxyz, pointD_y);
...
TAKErr ManagedModel::getNormal(Point2<float> *value, const std::size_t index) const NOTHROWS
{
    ...
    if(!jxyz)
        return TE_Err;
    env->CallVoidMethod(jmodel, jiface.getNormal, (jint)index, jxyz.get());
    if(env->ExceptionCheck())
        return TE_Err;
    value->x = env->GetDoubleField(jxyz, pointD_x);
    value->y = env->GetDoubleField(jxyz, pointD_y);
