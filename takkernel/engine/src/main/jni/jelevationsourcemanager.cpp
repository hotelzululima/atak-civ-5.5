...
namespace
{
    ...
    TAKErr CallbackForwarder::onSourceAttached(const std::shared_ptr<ElevationSource> &csrc) NOTHROWS
    {
        ...
        LocalJNIEnv env;
        // look up the object
        Java::JNILocalRef msrc(*env, nullptr);
        code = Elevation::Interop_getObject(msrc, *env, *csrc);
        // if not previously interop'd, intern a new wrapper
        if(code != TE_Ok || !msrc)
            msrc = Java::JNILocalRef(*env, Elevation::Interop_adapt(env, csrc, false));
        if(!msrc)
            return TE_Done;
        if(env->ExceptionCheck())
            return TE_Err;
        env->CallVoidMethod(impl, ElevationSourceManager_OnSourcesChangedListener_class.onSourceAttached, msrc.get());
        if(env->ExceptionCheck())
            return TE_Err;
        return TE_Ok;
    }
    TAKErr CallbackForwarder::onSourceDetached(const ElevationSource &csrc) NOTHROWS
    {
        ...
        if(env->ExceptionCheck())
            return TE_Err;
        env->CallVoidMethod(impl, ElevationSourceManager_OnSourcesChangedListener_class.onSourceDetached, msrc.get());
        if(env->ExceptionCheck())
            return TE_Err;
        return TE_Ok;
    }
