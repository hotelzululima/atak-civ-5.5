...
JNIEXPORT jobject JNICALL Java_com_atakmap_map_layer_model_Models_simplify
        (JNIEnv *env, jclass clazz, jlong srcPtr, jdouble maxError)
{
    TAKErr code(TE_Ok);
    if(!srcPtr)
        return nullptr;
    const auto &src = *JLONG_TO_INTPTR(TAK::Engine::Model::Mesh, srcPtr);
    MeshPtr simplified(nullptr, nullptr);
    code = MeshSimplication_simplify(simplified, src, maxError);
    if(ATAKMapEngineJNI_checkOrThrow(env, code))
        return nullptr;
    return NewPointer(env, std::move(simplified));
}
