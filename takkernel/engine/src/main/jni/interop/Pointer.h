...
namespace TAKEngineJNI {
    namespace Interop {
        ...
        template<class T>
        jobject NewPointer(JNIEnv *env, T *ptr) NOTHROWS;
        template<class T>
        jobject NewReference(JNIEnv *env, T &ptr) NOTHROWS;
        template<class T>
        ...
        {
            typedef void(*deleter_fn)(const T *);
            deleter_fn d = TAK::Engine::Util::Memory_deleter_const<T>;
            return env->NewObject(Pointer_class.id,
                                  Pointer_class.ctor__JJIJ,
                                  INTPTR_TO_JLONG(ptr),
                                  INTPTR_TO_JLONG(ptr),
        ...
        template<class T>
        inline jobject NewReference(JNIEnv *env, T &ptr) NOTHROWS
        {
            typedef void(*deleter_fn)(const T *);
            deleter_fn d = TAK::Engine::Util::Memory_leaker_const<T>;
            return env->NewObject(Pointer_class.id,
                                  Pointer_class.ctor__JJIJ,
                                  INTPTR_TO_JLONG(&ptr),
                                  INTPTR_TO_JLONG(&ptr),
                                  com_atakmap_interop_Pointer_REFERENCE,
                                  INTPTR_TO_JLONG(d));
        }
