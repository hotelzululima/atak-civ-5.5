...
TAKErr ManagedElevationChunk::createData(ElevationChunkDataPtr &value) NOTHROWS
{
    ...
    cdata->srid = env->GetIntField(mdata, ElevationChunk_Data_class.srid);
...
TAKErr ManagedElevationChunk::sample(double *value, const std::size_t count, const double *srcLat, const double *srcLng, const std::size_t srcLatStride, const std::size_t srcLngStride, const std::size_t dstStride) NOTHROWS
{
    ...
    if(!value)
        return TE_InvalidArg;
    LocalJNIEnv env;
    JNILocalRef jarr(*env, env->NewDoubleArray(count*3));
    {
        JNIDoubleArray jarr_w(*env, (jdoubleArray)jarr.get(), 0);
    ...
    const bool done = env->CallBooleanMethod(impl, ElevationChunk_class.sample__1DII, (jdoubleArray)jarr.get(), 0, count);
    if(env->ExceptionCheck())
        return TE_Err;

    {
        JNIDoubleArray jarr_w(*env, (jdoubleArray)jarr.get(), JNI_ABORT);
        for (std::size_t i = 0; i < count; i++) {
