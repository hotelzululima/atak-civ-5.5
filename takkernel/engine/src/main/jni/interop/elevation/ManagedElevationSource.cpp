...
TAKErr ManagedElevationSource::query(ElevationChunkCursorPtr &value, const ElevationSource::QueryParameters &cparams) NOTHROWS
{
    ...
    if(cparams.spatialFilter.get()) {
        JNILocalRef mgeom(*env, Feature::Interop_create(env, *cparams.spatialFilter));
        env->SetObjectField(mparams, ElevationSource_QueryParameters_class.spatialFilter, mgeom);
    }
...
TAKErr ManagedElevationSource::addOnContentChangedListener(TAK::Engine::Elevation::ElevationSource::OnContentChangedListener *l) NOTHROWS
{
    ...
    JNILocalRef mlistenerPointer(*env, NewPointer(env, std::move(clistener)));
    JNILocalRef mlistener(*env, env->NewObject(NativeOnContentChangedListener_class.id, NativeOnContentChangedListener_class.ctor, mlistenerPointer.get(), INTPTR_TO_JLONG(this)));
    if(env->ExceptionCheck())
        return TE_Err;
