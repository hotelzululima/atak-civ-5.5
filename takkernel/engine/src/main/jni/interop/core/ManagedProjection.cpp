...
TAKErr ManagedProjection::forward(Point2<double> *proj, const GeoPoint2 &geo) const NOTHROWS
{
    ...
    Java::JNILocalRef mgeo(*env, Interop_create(env, geo));
    if(!mgeo)
        return TE_Err;
    Java::JNILocalRef mproj(*env, env->CallObjectMethod(impl, Projection_class.forward, mgeo.get(), NULL));
    if(!mproj)
        return TE_Err;
...
TAKErr ManagedProjection::inverse(GeoPoint2 *geo, const Point2<double> &proj) const NOTHROWS
{
    ...
    Java::JNILocalRef mproj(*env, env->NewObject(pointDClass, pointD_ctor__DDD, proj.x, proj.y, proj.z));
    if(!mproj)
        return TE_Err;
    Java::JNILocalRef mgeo(*env, env->CallObjectMethod(impl, Projection_class.inverse, mproj.get(), NULL));
    if(!mgeo)
        return TE_Err;
