...
namespace
{
    ...
    struct {
        jclass id;
        jmethodID getSurfaceWidth;
        jmethodID getSurfaceHeight;
    } RenderSurface2_class;
    ...
    impl(env_.NewWeakGlobalRef(impl_))
{
    ...
    isRenderSurface2 = env_.IsInstanceOf(impl_, RenderSurface2_class.id);
}
...
std::size_t ManagedRenderSurface::getWidth() const NOTHROWS
{
    LocalJNIEnv env;
    if (isRenderSurface2) {
        return env->CallIntMethod(impl, RenderSurface2_class.getSurfaceWidth);
    }
...
std::size_t ManagedRenderSurface::getHeight() const NOTHROWS
{
    LocalJNIEnv env;
    if (isRenderSurface2) {
        return env->CallIntMethod(impl, RenderSurface2_class.getSurfaceHeight);
    }
...
void ManagedRenderSurface::addOnSizeChangedListener(RenderSurface::OnSizeChangedListener *clistener) NOTHROWS
{
    ...
    if(listeners.find(clistener) != listeners.end())
        return;
    Java::JNILocalRef mlistenerPointer(*env, NewReference(env, *clistener));
    Java::JNILocalRef mlistener(*env, env->NewObject(NativeRenderSurfaceSizeChangedListener_class.id, NativeRenderSurfaceSizeChangedListener_class.init, INTPTR_TO_JLONG(static_cast<RenderSurface *>(this)), mlistenerPointer.get()));
...
namespace
{
    bool RenderSurface_init(JNIEnv &env) NOTHROWS {
        ...
        RenderSurface2_class.id = ATAKMapEngineJNI_findClass(&env, "gov/tak/api/engine/map/RenderSurface2");
        RenderSurface2_class.getSurfaceWidth = env.GetMethodID(RenderSurface2_class.id, "getSurfaceWidth", "()I");
        RenderSurface2_class.getSurfaceHeight = env.GetMethodID(RenderSurface2_class.id, "getSurfaceHeight", "()I");
