...
ManagedTextFormatFactory::ManagedTextFormatFactory(JNIEnv &env) NOTHROWS
{}
...
TAK::Engine::Util::TAKErr ManagedTextFormatFactory::createTextFormat(TAK::Engine::Renderer::TextFormat2Ptr& value, const TAK::Engine::Renderer::TextFormatParams& params) NOTHROWS
{
    ...
    Java::JNILocalRef mtextformat(*env, nullptr);
    Java::JNILocalRef mfontName(*env, env->NewStringUTF(params.fontName));
    if(env->ExceptionCheck()) {
        env->ExceptionClear();
        return TE_Err;
    }
    mtextformat = Java::JNILocalRef(*env, env->CallStaticObjectMethod(GLText_class.id, GLText_class.createTextFormat, mfontName.get(), params.bold,
                                                                      params.italic, (int)params.size, params.underline, params.strikethrough, params.outline));
...
namespace
{
    ...
    bool ManagedTextFormatFactory_init(JNIEnv &env) NOTHROWS
    {
        GLText_class.id = ATAKMapEngineJNI_findClass(&env, "com/atakmap/opengl/GLText");
        GLText_class.createTextFormat = env.GetStaticMethodID(GLText_class.id,
                "createTextFormat",
                "(Ljava/lang/String;ZZIZZZ)Lcom/atakmap/android/maps/MapTextFormat;");
        return true;
    }
}
