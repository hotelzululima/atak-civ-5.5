...
TAKErr TAKEngineJNI::Interop::Java::Interop_marshal(JNILocalRef &value, JNIEnv &env, void(*run)(void *), std::unique_ptr<void, void(*)(const void *)> &&opaque) NOTHROWS
{
    ...
    JNILocalRef mopaque(env, NewPointer(&env, std::move(opaque)));
    value = JNILocalRef(env, env.NewObject(NativeRunnable_class.id, NativeRunnable_class.ctor, mopaque.get(), INTPTR_TO_JLONG(run), NULL));
    return TE_Ok;
}
