...
TAK::Engine::Util::TAKErr TAKEngineJNI::Interop::Model::Interop_create(Java::JNILocalRef &value, JNIEnv *env, const SceneInfo &sceneInfo) NOTHROWS
{
    if(!env)
        return TE_InvalidArg;
    if(env->ExceptionCheck())
    ...
    value = Java::JNILocalRef(*env, env->NewObject(ModelInfo_class.id, ModelInfo_class.ctor));
    return Interop_marshal(value, env, sceneInfo);
}
...
namespace
{
    ...
    bool Interop_class_init(JNIEnv *env) NOTHROWS
    {
        ...
        ModelInfo_class.ctor = env->GetMethodID(ModelInfo_class.id, "<init>", "()V");
