...
namespace TAKEngineJNI {
    namespace Interop {
        ...
        private :
            std::map<std::weak_ptr<const T>, jweak, std::owner_less<std::weak_ptr<const T>>> nativeImplToManagedWrapper;
        ...
        inline TAK::Engine::Util::TAKErr ImplementationMarshalContext<T>::marshal(std::shared_ptr<T> &value, JNIEnv &env, jobject obj) NOTHROWS
        {
            ...
            do {
                ...
                auto it = nativeImplToManagedWrapper.begin();
                while (it != nativeImplToManagedWrapper.end())
                {
                    auto entry = it->first.lock();
                    if (!entry) {
                        env.DeleteWeakGlobalRef(it->second);
                        it = nativeImplToManagedWrapper.erase(it);
                    } else {
                        if (entry.get() == value.get()) return TAK::Engine::Util::TE_Ok;
                        ++it;
                    }
                }
                std::weak_ptr<T> weakValue = value;
                nativeImplToManagedWrapper[weakValue] = env.NewWeakGlobalRef(obj);
            ...
            do {
                ...
                while (it != nativeImplToManagedWrapper.end()) {
                    ...
                    if (!entry) {
                        ...
                        if (entry.get() == &obj) {
                            value = Java::JNILocalRef(env, env.NewLocalRef(it->second));
                            if(value) {
                                return code;
                            } else {
                                env.DeleteWeakGlobalRef(it->second);
