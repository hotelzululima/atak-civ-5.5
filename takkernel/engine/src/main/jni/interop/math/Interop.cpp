...
TAKErr TAKEngineJNI::Interop::Math::Interop_marshal(JNILocalRef &value, JNIEnv &env, const Matrix2 &cmatrix) NOTHROWS
{
    ...
    Java::JNILocalRef mmatrixPtr(env, NewPointer(&env, std::move(retval)));
    value = JNILocalRef(env, env.NewObject(Matrix_class.id, Matrix_class.ctor, mmatrixPtr.get(), NULL));
    return TE_Ok;
}
