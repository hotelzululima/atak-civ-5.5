...
bool ManagedGeometryModel::intersect(Point2<double> *value, const Ray2<double> &ray) const
{
    ...
    Java::JNILocalRef morg(*env, env->NewObject(pointDClass, pointD_ctor__DDD, ray.origin.x, ray.origin.y, ray.origin.z));
    Java::JNILocalRef mdir(*env, env->NewObject(Vector3D_class.id, Vector3D_class.ctor, ray.direction.x, ray.direction.y, ray.direction.z));
    Java::JNILocalRef mray(*env, env->NewObject(Ray_class.id, Ray_class.ctor, morg.get(), mdir.get()));

    Java::JNILocalRef mvalue(*env, env->CallObjectMethod(impl, GeometryModel_class.intersect, mray.get()));
    if(env->ExceptionCheck())
        return false;
    if(!mvalue)
        return false;
