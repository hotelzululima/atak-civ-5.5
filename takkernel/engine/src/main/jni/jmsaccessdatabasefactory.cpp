...
#include "com_atakmap_map_formats_msaccess_MsAccessDatabaseFactory.h"
...
Java_com_atakmap_map_formats_msaccess_MsAccessDatabaseFactory_createDatabaseNative(JNIEnv *env,
                                                                                   jclass clazz,
                                                                                   jstring jfilepath) {
    DatabasePtr db(NULL, NULL);
    JNIStringUTF filepath(*env, jfilepath);
    DatabaseInformation dbInf(filepath);

    if (MsAccessDatabaseFactory_create(db, dbInf) != TE_Ok)
        return nullptr;
    else
        return NewPointer(env, std::move(db));
}
...
JNIEXPORT void JNICALL Java_com_atakmap_map_layer_feature_falconview_FalconViewFeatureDataSource_registerProvider(
    ...
    //TAKErr code(TE_Ok);
    std::shared_ptr<DatabaseProvider> wrappedProvider;
    // Legacy provider first as we want to support user/app-registered provider during
    // deprecation period. Remove this once deprecation on the java-side of proxy methods
    // are removed/no longer supported.
    wrappedProvider = std::make_shared<FalconViewProviderFactoryProxy>(*env);
    MsAccessDatabaseFactory_registerProvider(wrappedProvider);
    // Native cross-platform implementation
    wrappedProvider = std::make_shared<MDBAccessDatabaseProvider>();
    MsAccessDatabaseFactory_registerProvider(wrappedProvider);
