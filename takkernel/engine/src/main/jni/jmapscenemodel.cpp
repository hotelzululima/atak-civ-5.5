  ...
  (JNIEnv *env, jclass clazz, jlong ptr)
{
    ...
    return NewReference(env, *model->earth);
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr)
{
    ...
    return NewReference(env, model->forwardTransform);
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr)
{
    ...
    return NewReference(env, model->inverseTransform);
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr)
{
    ...
    return NewReference(env, *model->displayModel);
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr)
{
    ...
    return NewReference(env, model->camera.projection);
}
  ...
  (JNIEnv *env, jclass clazz, jlong ptr)
{
    ...
    return NewReference(env, model->camera.modelView);
}
...
JNIEXPORT void JNICALL Java_com_atakmap_map_MapSceneModel_getCameraUp
  (JNIEnv *env, jclass clazz, jlong ptr, jobject mup)
{
    MapSceneModel2 *model = JLONG_TO_INTPTR(MapSceneModel2, ptr);
    if(!model) {
        ATAKMapEngineJNI_checkOrThrow(env, TE_InvalidArg);
    ...
    env->SetDoubleField(mup, pointD_x, model->camera.up.x);
    env->SetDoubleField(mup, pointD_y, model->camera.up.y);
    env->SetDoubleField(mup, pointD_z, model->camera.up.z);
}
