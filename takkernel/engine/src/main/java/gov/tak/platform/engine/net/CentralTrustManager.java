...
public final class CentralTrustManager implements X509TrustManager
{
    ...
    // Defaulted to false for backwards compatibility
    private boolean ignorePublicCAs = false;
    ...
    {
        this.ignorePublicCAs = ignorePublicCAs;
    }
            ...
            String authType) throws CertificateException
    {
        ...
        // only use the public CAs when configured to do so (which generally means
        // we are going to do hostname verification)
        X509TrustManager ltm = certificateManager.getLocalTrustManager(ignorePublicCAs);
        if (iss)
        {
            ...
            String authType) throws CertificateException
    {
        ...
        // only use the public CAs when configured to do so (which generally means
        // we are going to do hostname verification)
        X509TrustManager ltm;
        if (host != null)
            ltm = certificateManager.getLocalTrustManager(host);
        else
            ltm = certificateManager.getLocalTrustManager(ignorePublicCAs);

        if (ltm == null)
            throw new CertificateException("No trust anchors configured");
