...
public class CertificateManager
{
    ...
    private static final Map<String, ExtendedSSLSocketFactory> socketFactories = new ConcurrentHashMap<>();
    ...
    {
        return (_impl != null) ? _impl.getLocalTrustManager(excludePublicCAs) : null;
    }
    ...
    {
        return (_impl != null) ? _impl.getLocalTrustManager(host) : null;
    }
    ...
    public final static class CentralTrustManager implements X509TrustManager
    {
         ...
         * may be self signed.
         *
         * @param iss false to use the system TrustManager, true to only use the
         *            local TrustManager.
         */
        ...
        /**
         * Whether to use public certificate authorities when consulting the local trust store
         * @param ignorePublicCAs true to ignore public CAs when examining the local trust store, false
         *                        to allow public CAs
         */
        public void setIgnorePublicCAs(final boolean ignorePublicCAs)
        {
            this.impl.setIgnorePublicCAs(ignorePublicCAs);
        }

        /**
         * Restrict entries used in local trust store to those for the specified host. If null, no
         * restriction is used
         * @param host hostname to filter local trust store entries against
         */
        public void setServerHost(final String host)
        {
            this.impl.setServerHost(host);
        }
    ...
    {
        // XXX - temporary implementation detail
        static {
            MarshalManager.registerMarshal(new IMarshal() {
                ...
                public <T, V> T marshal(V in) {
                    if(in == null)
                        return null;
                    return (T) ((ExtendedSSLSocketFactory) in).centralTrustManager;
                }
            }, ExtendedSSLSocketFactory.class, X509TrustManager.class);
        }
        final SSLContext sslContext;
        // Allow all hostnames in server-presented certificates (don't hostname check)
        boolean allowAllHostnames = true;
        final CentralTrustManager centralTrustManager;
                ...
                KeyStoreException, UnrecoverableKeyException
        {
            ...
            centralTrustManager = new CentralTrustManager(
                    CertificateManager.getInstance(), this);
            centralTrustManager.setIgnoreSystemCerts(true);
            centralTrustManager.setIgnorePublicCAs(allowAllHostnames);
            String hostname = null;
            try {
                URI uri = new URI(server);
                hostname = uri.getHost();
            } catch (Exception ex) {
                Log.d(TAG, "Invalid URI encountered creating " + getClass().getName(), ex);
            }
            // When hostname checking is disabled and we know what server we are hitting, use
            // only the trust we have configured for that server
            if (allowAllHostnames && hostname != null)
                centralTrustManager.setServerHost(hostname);
                ...
                KeyStoreException, UnrecoverableKeyException
        {
            ...
            // When hostname checking is disabled and we know what server we are hitting, use
            // only the trust we have configured for that server
            if (allowAllHostnames && netConnectString.getHost() != null)
                centralTrustManager.setServerHost(netConnectString.getHost());
    ...
    /**
     * @param server base URL to a tak server
     * @param allowAllHostnames if true, hostname verification will not be applied to the 
     *                          server-presented certificates and, if server is non-null,
     *                          we only trust certificates we have configured for that server
     */
            ...
            boolean useCache, String server, boolean allowAllHostnames)
    {
        ...
        String cacheKey = makeCacheKey(server, allowAllHostnames);
        if (useCache)
        {
            socketFactory = socketFactories.get(cacheKey);
        ...
        try
        {
            ...
            Log.i(TAG, "creating socketFactory for : " + cacheKey);
            socketFactory = new ExtendedSSLSocketFactory(server,
                    allowAllHostnames);

            if (useCache)
            {
                socketFactories.put(cacheKey, socketFactory);
            }
    ...
    /**
     * @param allowAllHostnames if true, hostname verification will not be applied to the 
     *                          server-presented certificates and
     *                          we only trust certificates we have configured for the server
     *                          identified in netConnectString
     */
    public static ExtendedSSLSocketFactory getSockFactory(
            boolean useCache, NetConnectString netConnectString, boolean allowAllHostnames)
    {
        ...
        String cacheKey = makeCacheKey(netConnectString.toString(), allowAllHostnames);
        if (useCache)
        {
            socketFactory = socketFactories.get(cacheKey);
            if (socketFactory != null)
    ...
    private static String makeCacheKey(String host, boolean allowAllHostnames)
    {
        return host + "," + String.valueOf(allowAllHostnames);
    }
