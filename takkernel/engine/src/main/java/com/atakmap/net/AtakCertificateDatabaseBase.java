 ...
 *
 */
class AtakCertificateDatabaseBase
{
     ...
     *
     * @deprecated use the AtakCredentialDatabase directly.
     */
    @Deprecated
    ...
    public static synchronized ICertificateStore getStore()
    {
        return getAdapter().certDb;
    }
    ...
    {
        return gov.tak.platform.engine.net.CertificateManager.loadCertificate(p12, password);
    }
    ...
    /**
     * Check if certs in store are valid, and will be beyond the specified date
     *
    ...
    public static CertificateValidity checkValidity(byte[] keystore, String password)
    {
        ...
        for (X509Certificate certificate : certificates)
        {
            ...
            } catch (CertificateExpiredException cee)
            {
                ...
                if (ret == null || ret.certNotAfter.after(certificate.getNotAfter()))
                    ret = new CertificateValidity(certificate, false, cee.getMessage(), certificate.getNotAfter());
            } catch (CertificateNotYetValidException cnyve)
            {
                Log.e(TAG, "Found a not yet valid certificate!", cnyve);
                if (ret == null || ret.certNotAfter.after(certificate.getNotAfter()))
                    ret = new CertificateValidity(certificate, false, cnyve.getMessage(), certificate.getNotAfter());
            }

            if (ret == null || ret.certNotAfter.after(certificate.getNotAfter()))
                ret = new CertificateValidity(certificate, true, null, certificate.getNotAfter());
        }
