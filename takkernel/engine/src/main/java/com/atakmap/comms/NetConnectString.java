...
public class NetConnectString
{
            ...
            final String connectString)
    {
        ...
        try
        {
            ...
            {
                port = Parsers.parseInt(parts[1], port);
                host = "0.0.0.0";
            } else if (parts[1].contains("//"))
            {
                ...
                if (parts.length > 2)
                    port = Parsers.parseInt(parts[2], port);
            } else
            {
                port = Parsers.parseInt(parts[1], port);
                host = parts[0];
            ...
            // account for either *:-1:stcp or a valid port and host
            if ( (port == -1 && host.equals("*")) || (!host.equals("*") && port > 0)) {
            ...
            {
                Log.e(TAG, "Failed to parse net connect string: " + connectString);
            }
        } catch (Exception e)
        {
            Log.e(TAG, "Failed to parse net connect string: " + connectString, e);
