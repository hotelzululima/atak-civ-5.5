...
public class GLQuadTileNode2 implements GLResolvableMapRenderable
{
    ...
    public void draw(GLMapView view)
    {
        ...
        final double zoomScale = ((Globe.getMapScale(view.currentPass.scene.dpi, view.currentPass.scene.gsd) * Globe.getFullEquitorialExtentPixels(view.currentPass.scene.dpi)) / 360d);
    ...
    private void _releaseInvisibleChildren(GLMapView view, double halfLat, double halfLng,
                                           int drillLevel)
    {
        if (view.currentPass.eastBound < halfLng)
        {
            _releaseChild(_NE);
            _releaseChild(_SE);

        } else if (view.currentPass.westBound > halfLng)
        {
            _releaseChild(_NW);
            _releaseChild(_SW);
        }

        if (view.currentPass.southBound > halfLat)
        {
            _releaseChild(_SW);
            _releaseChild(_SE);
        } else if (view.currentPass.northBound < halfLat)
        {
            _releaseChild(_NW);
            _releaseChild(_NE);
        }
    ...
    private void _drawVisibleChildren(GLMapView view, double halfLat, double halfLng, int drillLevel)
    {
        ...
        {
            // east
            if (view.currentPass.northBound > halfLat)
            {
                // north-east
        ...
        if (view.currentPass.westBound < halfLng)
        {
            // west
            if (view.currentPass.northBound > halfLat)
            {
    ...
    private void drawTexture(GLMapView view, int texId)
    {
        ...
        } else
        {
            if (vertsIsProjected)
            {
                ...
                GLES20FixedPipeline.glLoadMatrixf(view.currentPass.sceneModelForwardMatrix, 0);
            }
    ...
    private List<GeoPoint> _getClipList(GeoPoint sw, GeoPoint nw, GeoPoint ne, GeoPoint se)
    {
        ...
        {
            if (cur.getLongitude() >= this.boundsSWNE[1])
            { // inside
        ...
        for (GeoPoint cur : inputList)
        {
            if (cur.getLatitude() <= this.boundsSWNE[2])
            { // inside
        ...
        for (GeoPoint cur : inputList)
        {
            if (cur.getLongitude() <= this.boundsSWNE[3])
            { // inside
        ...
        for (GeoPoint cur : inputList)
        {
            if (cur.getLatitude() >= this.boundsSWNE[0])
            { // inside
    ...
    private void _projectVerts(GLMapView view)
    {
        ...
        final boolean hardwareTransforms = Options.hardwareTransforms &&
                !view.currentPass.crossesIDL &&
                (view.currentPass.drawMapResolution > view.hardwareTransformResolutionThreshold);
        if (hardwareTransforms)
        {
            if (this.vertsDirty || view.currentPass.drawSrid != this.vertsSrid || !vertsIsProjected)
            {
                int idx = 0;
                for (int i = 0; i < _pointCount; ++i)
                {
                ...
                this.vertsSrid = view.currentPass.drawSrid;
            }
        } else if ((vertsDirty) || (view.currentPass.drawVersion != vertsDrawVersion) || (view.currentPass.drawSrid
                != vertsSrid) || vertsIsProjected)
        {
            ...
            for (int i = 0; i < _pointCount; ++i)
            {
                ...
                view.scratch.geo.set(_points[y], _points[x]);
                view.currentPass.scene.mapProjection.forward(view.scratch.geo, view.scratch.pointD);
                view.currentPass.scene.forward.transform(view.scratch.pointD, view.scratch.pointD);
            ...
            vertsDirty = false;
            vertsDrawVersion = view.currentPass.drawVersion;
            vertsSrid = view.currentPass.drawSrid;
        }
