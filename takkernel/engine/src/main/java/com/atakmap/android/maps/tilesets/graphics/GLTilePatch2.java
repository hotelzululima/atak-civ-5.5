...
public class GLTilePatch2 implements GLMapLayer3, RasterDataAccess2, GLResolvableMapRenderable
{
    ...
    private synchronized void validateGrid(GLMapView view)
    {
        ...
        int southIndex = this.uriResolver.getTileZeroY(view.currentPass.southBound, _gridY, _gridHeight);
        southIndex = Math.max(0, Math.min(southIndex, _gridHeight - 1));

        int westIndex = this.uriResolver.getTileZeroX(view.currentPass.westBound, _gridX, _gridWidth);
        westIndex = Math.max(0, Math.min(westIndex, _gridWidth - 1));

        int northIndex = this.uriResolver.getTileZeroY(view.currentPass.northBound, _gridY, _gridHeight);
        northIndex = Math.max(0, Math.min(northIndex, _gridHeight - 1));

        int eastIndex = this.uriResolver.getTileZeroX(view.currentPass.eastBound, _gridX, _gridWidth);
