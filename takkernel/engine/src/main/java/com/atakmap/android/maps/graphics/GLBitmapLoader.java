...
public class GLBitmapLoader
{
    ...
    private static class LoaderSpec implements Comparable<LoaderSpec>
    {
        ...
        public LoaderSpec(LoaderSpi loader, QueueType queue, long ptr)
        {
            this.loader = loader;
            this.queue = queue;
            this.pointer = ptr;
        ...
        public boolean equals(Object o)
        {
            ...
            if (insert != that.insert)
                return false;
            if (!Objects.equals(loader, that.loader))
                return false;
                          ...
                          final int threadPriority)
    {
        ...
        else if (GLRenderGlobals.appContext != null)
            ctx = GLRenderGlobals.appContext;
        else
            Log.w(TAG, "Failed to obtain Context; asset and resource decoding not enabled");

        synchronized(_loaders) {
            // register off-the-shelf loaders
            if(_loaders.isEmpty()) {
                ...
                this.registerLoader("jar", ZipBitmapLoaderSpi.INSTANCE, QueueType.LOCAL_COMPRESSED);
    ...
    /** @deprecated  use {@link #registerBitmapLoader(String, LoaderSpi, QueueType)} */
    @Deprecated
    ...
    {
        registerBitmapLoader(scheme, loader, queue);
    }
     ...
     */
    public static void registerBitmapLoader(String scheme, LoaderSpi loader, QueueType queue)
    {
        synchronized (_loaders)
        {
            final long ptr = (loader instanceof AbstractInputStreamLoaderSpi) ?
                    registerProtocolHandler(scheme, (AbstractInputStreamLoaderSpi) loader, (queue != null) ? queue.name : null) : 0L;
            _loaders.put(scheme, new LoaderSpec(loader, queue, ptr));
        }
    }

    /** @deprecated  use {@link #unregisterBitmapLoader(LoaderSpi)} */
    @Deprecated
    @DeprecatedApi(since = "5.3", forRemoval = true, removeAt = "5.6")

    public void unregisterLoader(LoaderSpi loader)
    {
        unregisterBitmapLoader(loader);
    }
     ...
     * @param loader the loader to call
     */
    public static void unregisterBitmapLoader(LoaderSpi loader)
    {
        synchronized (_loaders)
        {
            Iterator<Map.Entry<String, LoaderSpec>> iter = _loaders.entrySet().iterator();
            ...
            while (iter.hasNext())
            {
                ...
                if (entry.getValue().loader == loader) {
                    // evict the loader
                    iter.remove();
                    // perform native de-registration
                    if (entry.getValue().pointer != 0L) {
                        unregisterProtocolHandler(entry.getValue().pointer);
                        // the loader may be registered against multiple schemes; only effect native
                        // cleanup once
                        entry.getValue().pointer = 0L;
                    }
                }
            }
        }
    ...
    public void shutdown()
    {
        ...
        for (ReferenceCount<ZipCacheEntry> value : _zipCache.values())
        {
            try
            {
                value.value.zipFile.close();
                                         ...
                                         final BitmapFactory.Options opts)
    {
        ...
        final LoaderSpec s = _loaders.get(scheme);
        if (s != null)
        {
    ...
    {
        mountArchiveImpl(arcPath, false);
    }

    /**
     * Mount a zip archive for use.  If it is already mounted and force is false, the mount reference
     * count will be increased by one.  If it is already mounted and force is true, 
     * a the file is mounted anew and the existing cached entry is abandoned.
     * The mounted zip file is made available by a call to
     * getMountedArchive and guaranteed to remain open until the reference count goes to zero.
     *
     * @param arcPath the archive to mount in zip format.
     */
    static private void mountArchiveImpl(final String arcPath, final boolean force)
    {
        boolean addRefresher = false;
        synchronized (_zipCache)
        {
            ReferenceCount<ZipCacheEntry> zipCacheEntry = _zipCache.get(arcPath);
            if (force || zipCacheEntry == null || !validZip(zipCacheEntry.value.zipFile))
            {
                try
                {
                    long mtime = 0;
                    ...
                    if (!f.exists())
                        f = new File(URLDecoder.decode(arcPath, "UTF-8"));
                    mtime = f.lastModified();
                    zipCacheEntry = new ReferenceCount<>(new ZipCacheEntry(new ZipFile(f.getAbsolutePath()), f, mtime));
                    Log.d(TAG, "archive: " + arcPath + " add to cache.");
                    _zipCache.put(arcPath, zipCacheEntry);
    ...
    static public void unmountArchive(final String arcPath)
    {
        ...
        synchronized (_zipCache)
        {
            ...
            try
            {
                if (zipCacheEntry != null)
                {
                    zipCacheEntry.dereference();
                    if (!zipCacheEntry.isReferenced())
                    {
                        Log.d(TAG, "archive: " + arcPath + " remove from cache.");
                        _zipCache.remove(arcPath);
                        zipCacheEntry.value.zipFile.close();
    ...
    static private ZipFile getMountedArchive(String path)
    {
        synchronized (_zipCache)
        {
            ReferenceCount<ZipCacheEntry> zipCacheEntry = _zipCache.get(path);
            if (zipCacheEntry != null) {
                File f = new File(path);
                try
                {
                    if (!f.exists())
                        f = new File(URLDecoder.decode(path, StandardCharsets.UTF_8.name()));
                } catch (UnsupportedEncodingException e) { }
                if (f.lastModified() == zipCacheEntry.value.mtime)
                    return zipCacheEntry.value.zipFile;
                else
                    return null;
            } else
                return null;
    ...
    private static class ZipBitmapLoaderSpi extends AbstractInputStreamLoaderSpi
    {
        ...
        protected InputStream openStream(String uriStr) throws Exception
        {
            ...
            if (uri.getScheme().equals("jar"))
            {
                try
                {
                    ...
                    if (SystemUtils.isOsWindows()) {
                        // URI.create() balks at spaces on desktop platforms so go straight to URL
                        url = new URL(uriStr);
                    } else
                    {
                        //jar URLs are best parsed by JarURLConnection
                        url = URI.create(uriStr).toURL();
                    }

                    JarURLConnection connection = (JarURLConnection) url.openConnection();

                    arcPath = connection.getJarFileURL().getPath();
                    entryPath = connection.getEntryName();
                } catch (Exception ex)
                {
                    Log.e(TAG, "error: ", ex);
                    return null;
            ...
            } else
            {
                ...
                arcPath = path.substring(0, entryPathIdx);
                entryPath = path.substring(entryPathIdx + 2);
            }

            ZipFile zip = getMountedArchive(arcPath);
            if (zip == null)
            {
                // Force remount since we know it is either not mounted or invalid/stale
                mountArchiveImpl(arcPath, true);
    ...
    private static class FileBitmapLoaderSpi implements LoaderSpi
    {
        ...
        public Bitmap loadBitmap(String uri, BitmapFactory.Options opts) throws Exception
        {
            ...
            do
            {
                ...
                f = new File(uri.substring((scheme != null) ? scheme.length() + 3 : 0));
                if (f.exists())
                    break;
    ...
    private class UrlBitmapLoaderSpi extends AbstractInputStreamLoaderSpi
    {
        ...
        @Override
        protected InputStream openStream(String url) throws Exception
        {
            final boolean cache;
                ...
                if (GLBitmapLoader.this.urlIconCacheDatabase != null)
                {
                    ...
                    try
                    {
                        ...
                        do
                        {
                            ...
                            if (fd != null)
                                return new ByteArrayInputStream(fd);
                        } while (false);
                    } finally
                    {
            ...
            try
            {
                ...
                File cacheFile = IOProviderFactory.createTempFile("icon", ".cache", null);
                ...
                // caching is desired
                final int maxCacheWidth = 128;
                final int maxCacheHeight = 128;

                // crude heuristic based on file size for a compressed image
                if(cacheFile.length() < (maxCacheWidth*maxCacheHeight*4)) {
                    final BitmapFactory.Options opts = new BitmapFactory.Options();
                    opts.inJustDecodeBounds = true;
                    final Bitmap urlBitmap;
                    try (InputStream bmpbnds = IOProviderFactory.getInputStream(cacheFile)) {
                        urlBitmap = BitmapFactory.decodeStream(bmpbnds, null, opts);
                    }

                    if ((opts.outWidth*opts.outHeight > 0) && opts.outWidth < maxCacheWidth
                            && opts.outHeight < maxCacheHeight) {
                        synchronized (GLBitmapLoader.this) {
                            if (GLBitmapLoader.this.urlIconCacheDatabase != null) {
                ...
                return new DeleteOnCloseInputStream(IOProviderFactory.getInputStream(cacheFile), cacheFile);
            } catch (Exception e)
            {
    ...
    final static class DeleteOnCloseInputStream extends FilterInputStream
    {
        final File file;

        DeleteOnCloseInputStream(InputStream in, File f)
        {
            super(in);
            file = f;
        }
    ...
    private static class ZipCacheEntry {
        public final ZipFile zipFile;
        public final long mtime;
        public final File file;

        ZipCacheEntry(ZipFile zipFile, File file, long mtime) {
    ...
    static native long registerProtocolHandler(String scheme, AbstractInputStreamLoaderSpi handler, String queueHint);
    static native void unregisterProtocolHandler(long handler);

    private static final String TAG = "GLBitmapLoader";
    private static final HashMap<String, ReferenceCount<ZipCacheEntry>> _zipCache = new HashMap<>();
    ...
    private final static Map<String, LoaderSpec> _loaders = new HashMap<>();
