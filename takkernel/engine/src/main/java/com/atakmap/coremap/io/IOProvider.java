...
public abstract class IOProvider
{
    ...
    /**
     * If toggled, when `delete` is invoked on a directory, the directory
     * and all of its contents will be recursively deleted.
     */
    public static final int RECURSIVE = 0x02;

    /**
     * If toggled, when `delete` is invoked on a directory, only the
     * children of the directory will be deleted; the directory itself
     * will not be deleted. If the {@link #RECURSIVE} bit is also toggled,
     * the entire directory structure will be preserved, but all files
     * will be deleted.
     */
    public static final int CONTENTS_ONLY = 0x04;
