...
public class Vector2D
{
     ...
     * BooleanUtils classes.   Please note that if the array is empty, then consider this
     * vacuously true which mimics the impl for all false or all true.
     */
    ...
    {
        ArrayList<Integer> changed = new ArrayList<>();
        if (_areAllFalse(arrayV))
        { // Don't worry about a false positive
            for (int i = 0; i < array.length; i++)
            {
                int j = (i == array.length - 1) ? 0 : i + 1;
