...
public class GeoBounds
{
    ...
    {
        return contains(p.getLatitude(), p.getLongitude(), p.getAltitude());
    }

    public boolean contains(double latitude, double longitude, double altitude)
    {
        if (Double.isNaN(_maxAltitude) || Double.isNaN(_minAltitude) ||
        ...
        {
            if (_maxAltitude < altitude || _minAltitude > altitude)
                return false;
        }

        if (latitude < _south || latitude > _north)
            return false;
        if (crossesIDL())
        {
            return longitude >= _east
                    && longitude <= _west + 360
                    || longitude >= _east - 360
                    && longitude <= _west;
        }
        return longitude >= _west &&
                longitude <= _east;
    }
