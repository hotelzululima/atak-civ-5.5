...
public final class OSMDroidInfo
{
    ...
    /** Tile content type, one of {@code "imagery"}, {@code "vector"} or {@code "terrain"} */
    ...
    public static OSMDroidInfo get(DatabaseIface database, BoundsDiscovery bounds)
    {
        try
        {
            ...
            if (Databases.getColumnNames(database, "ATAK_metadata") != null)
            {
                ...
                {
                    result = database.query(
                            "SELECT value FROM ATAK_metadata WHERE key = \'content\'", null);
                    if (result.moveToNext())
                    {
                        info.content = result.getString(0);
                    }
                } catch (Exception ignored)
                {
                    // quietly ignore
