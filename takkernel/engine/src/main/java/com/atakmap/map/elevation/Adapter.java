...
{
    static WeakHashMap<MosaicDatabase2, WeakReference<ElevationSource>> _mosaicdbCache = new WeakHashMap<>();
    ...
    {
        synchronized (_mosaicdbCache) {
            do {
                final WeakReference<ElevationSource> adaptedRef = _mosaicdbCache.get(source);
                if (adaptedRef == null)
                    break;
                final ElevationSource adapted = adaptedRef.get();
                if (adapted != null)
                    return adapted;
            } while(false);
            final ElevationSource adapted = new MosaicDatabaseSource(name, authoritative, source);
            _mosaicdbCache.put(source, new WeakReference<>(adapted));
            return adapted;
        }
    }
    ...
    final static class MosaicDatabaseSource extends ElevationSource implements Controls
    {
        private String name;
        private boolean authoritative;
        private MosaicDatabase2 source;
        private Set<OnContentChangedListener> listeners = Collections.newSetFromMap(new ConcurrentHashMap<>());
        ...
        MosaicDatabaseSource(String name, boolean authoritative, MosaicDatabase2 source)
        {
            ...
            if(this.source instanceof Controls) {
                final Observable observable = ((Controls) this.source).getControl(Observable.class);
                observable.addObserver(new Observer() {
                    @Override
                    public void update(Observable observable, Object o) {
                        for(OnContentChangedListener l : listeners)
                            l.onContentChanged(MosaicDatabaseSource.this);
                    }
                });
            }
        }
        ...
        public <T> T getControl(Class<T> controlClazz) {
            if(MosaicDatabase2.class.isAssignableFrom(controlClazz))
                return (T) source;
            return null;
        }
