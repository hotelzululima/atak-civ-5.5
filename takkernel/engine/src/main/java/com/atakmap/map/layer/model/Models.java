...
public final class Models
{
    ...
    public static Mesh simplify(Mesh src, double maxError) {
        if(!src.isIndexed()) {
            // XXX - support unindexed
            Log.w("Models", "Models.simplify requires an indexed mesh, returning unsimplified clone of source Mesh");
            return transform(src, src.getVertexDataLayout());
        }

        if (src instanceof NativeMesh)
        {
            Pointer simplified = simplify(((NativeMesh) src).pointer.raw, maxError);
            return (simplified == null || simplified.raw == 0L) ? null : new NativeMesh(simplified);
        } else
        {
            NativeMesh adapted = null;
            try
            {
                ...
                return simplify(adapted, maxError);
            } finally
            {
                if (adapted != null)
                    adapted.dispose();
    ...
    static native Pointer simplify(long srcModel, double maxError);
