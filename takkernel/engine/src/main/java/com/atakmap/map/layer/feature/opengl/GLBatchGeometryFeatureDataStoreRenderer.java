...
/** @deprecated class implementation will be removed, leaving only {@link #SPI} field */
@Deprecated
        ...
        HitTestControl
{
    ...
    public final static GLLayerSpi2 SPI = new GLLayerSpi2()
    {
        ...
        {
            do {
                final GLLayer2 glv2 = GLBatchGeometryFeatureDataStoreRenderer2.SPI.create(object);
                if (glv2 != null)
                    return glv2;
            } while(false);
            ...
            Collection<GLBatchGeometry> result)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                final boolean isFeatureDefinition3 = (cursor instanceof FeatureDefinition3);
                while (cursor.moveToNext())
                {
                    ...
                    if (glitem != null)
                    {
                        ...
                        if (glitem.version == version &&
                            ((glitem.lod == lod) || (glitem.lod < 0)))
                        {
                    ...
                    int geomLod = -1;
                    do {
                        // point geometries are not simplified
                        if((type%1000) == 1)
                            break;
                        // Relative and Absolute geometries are not simplified
                        if(isFeatureDefinition3 && ((FeatureDefinition3)cursor).getAltitudeMode() != Feature.AltitudeMode.ClampToGround)
                            break;
                        geomLod = lod;
                    } while(false);
                    ...
                    if (useBlobGeom)
                    {
                        glitem.setGeometry(blob, type, geomLod);
                    } else
                    {
                        switch (type % 1000)
                        {
                            case 1:
                                if (!(glitem instanceof GLBatchPoint))
                                    continue;
                                glitem.setGeometry(geom, geomLod);
                                break;
                            case 2:
