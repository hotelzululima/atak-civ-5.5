...
public abstract class Geometry implements Disposable
{
    ...
    {
        // Java contract for equals says two differing types can be equal as long as there is
        // symmetry. Simplifies API wrappers to do this
        final Geometry other = castGeom(o);
        if (other == null)
            return false;
    ...
    // Marshal IMPL -> API
    // API -> IMPL is handled in gov.tak.api.engine.map.features.Geometry because an instance
    // of the class is required for static initializer to run.
    private static class ToApiMarshal implements IMarshal {
        @Override
        public <T, V> T marshal(V in) {
            Geometry impl = (Geometry)in;
            if (impl == null)
                return null;
            return (T)gov.tak.api.engine.map.features.Geometry.create(impl);
        }
    }

    static {
        MarshalManager.registerMarshal(new ToApiMarshal(), Geometry.class, gov.tak.api.engine.map.features.Geometry.class);
    }

    // Helper for unwrapping to IMPL Geometry instances for equality testing
    static Geometry castGeom(Object o) {
        if (o instanceof Geometry)
            return (Geometry)o;
        if (o instanceof gov.tak.api.engine.map.features.Geometry)
            return MarshalManager.marshal((gov.tak.api.engine.map.features.Geometry)o,
                    gov.tak.api.engine.map.features.Geometry.class, Geometry.class);
        return null;
    }
