...
public class GLDatasetRasterLayer3 extends GLAbstractDataStoreRasterLayer3
{
     ...
     */
    final static Comparator<DatasetDescriptor> MULTIPLEX_DATASET_COMP = new Comparator<DatasetDescriptor>()
    {
        @Override
        public int compare(DatasetDescriptor lhs, DatasetDescriptor rhs)
        {
            Map<String, String> extra0 = lhs.getExtraData();
            Map<String, String> extra1 = rhs.getExtraData();
            if(extra0 != null && extra1 != null) {
                final boolean vector0 = Objects.equals(extra0.get("contentType"), "vector") || Objects.equals(extra0.get("_imageExt"), "pbf");
                final boolean vector1 = Objects.equals(extra1.get("contentType"), "vector") || Objects.equals(extra1.get("_imageExt"), "pbf");
                if(vector0 && !vector1)
                    return 1;
                else if(!vector0 && vector1)
                    return -1;
            }
    ...
    public static final String TAG = "GLDatasetRasterLayer3";
    ...
    private final OnlineImageryHandler onlineImagery;

    public GLDatasetRasterLayer3(MapRenderer surface, DatasetRasterLayer2 subject)
    {
        ...
        this.onlineImagery = new OnlineImageryHandler();
    }
    ...
    public synchronized void start()
    {
        ...
        {
            this.offlineOnlyService.addCacheRefreshListener(this.onlineImagery);
            this.offlineOnlyService.addOnOfflineOnlyModeChangedListener(this.onlineImagery);
    ...
    public synchronized void stop()
    {
        ...
        if (this.offlineOnlyService != null)
        {
            this.offlineOnlyService.removeCacheRefreshListener(this.onlineImagery);
            this.offlineOnlyService.removeOnOfflineOnlyModeChangedListener(this.onlineImagery);
    ...
    @Override
    protected void updateTransparency(GLMapRenderable2 renderer)
    {
        if(!(renderer instanceof Controls))
            return;

        final Controls controls = (Controls)renderer;

        // apply alpha value
        ColorControl ctrl = controls.getControl(ColorControl.class);
        if (ctrl != null)
        {
            // Also handle color modulation here (specified by KML)
            final DatasetDescriptor desc = controls.getControl(DatasetDescriptor.class);
    ...
    {
        final boolean retval = super.updateRenderList(state, pendingData);
        if (retval)
        {
            for (GLMapRenderable2 renderer : this.renderable.values())
            {
                ...
                // proper mode
                TileClientControl svc = ((Controls)renderer).getControl(TileClientControl.class);
                if (svc != null)
                {
    ...
    protected void query(ViewState state, LinkedList<DatasetDescriptor> retval)
    {
        ...
        Set<Pair<String, String>> attrs = new HashSet<Pair<String, String>>();
    ...
    final class OnlineImageryHandler implements OnlineImageryExtension.OnOfflineOnlyModeChangedListener, OnlineImageryExtension.CacheRefreshListener {
        @Override
        public void onOfflineOnlyModeChanged(OnlineImageryExtension ext, boolean offlineOnly)
        {
            GLDatasetRasterLayer3.this.offlineOnly.offlineOnly = ext.isOfflineOnlyMode();
            GLDatasetRasterLayer3.this.invalidateNoSync();
        }

        @Override
        public void onManualRefreshRequested(OnlineImageryExtension ext) {
            synchronized (this) {
                Collection<GLMapRenderable2> renderables = GLDatasetRasterLayer3.this.getRenderList();
                for (GLMapRenderable2 renderer : renderables) {
                    if (!(renderer instanceof Controls))
                        continue;
                    TileClientControl ctrl = ((Controls) renderer).getControl(TileClientControl.class);
                    if (ctrl != null)
                        ctrl.refreshCache();
                }
            }
        ...
        public void onAutoRefreshIntervalChanged(OnlineImageryExtension ext, long millis) {
            GLDatasetRasterLayer3.this.offlineOnly.refreshInterval = millis;
            GLDatasetRasterLayer3.this.invalidateNoSync();
        }
    }
    ...
    protected class State extends GLAbstractDataStoreRasterLayer3.State
    {
        ...
        public void set(GLMapView view)
        {
            ...
            if (((DatasetRasterLayer2) GLDatasetRasterLayer3.this.getSubject()).getDatasetLimit() > 0)
            {
                for (DatasetDescriptor info : GLDatasetRasterLayer3.this.renderable.keySet()) {
                    this.renderables.add(info);
                }
            }
        }
