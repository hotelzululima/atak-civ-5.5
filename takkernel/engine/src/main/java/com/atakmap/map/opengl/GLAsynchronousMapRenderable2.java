 ...
 * @author Developer
 */
public abstract class GLAsynchronousMapRenderable2<Pending>
        implements GLMapRenderable2,
        GLResolvable
{
     ...
     */
    protected GLAsynchronousMapRenderable2()
    {
    ...
    /**
     * Constructs the render list based on the supplied query result.
     * <p>
     ...
     *
     * @param state       The state that produced the results
     ...
     */
    protected abstract boolean updateRenderList(ViewState state, Pending pendingData);
    ...
    {
        this.invalidateNoSync(null);
    }

    protected void invalidateNoSync(Envelope surfaceRegion)
    {
        this.invalid = true;
        if (surfaceRegion != null)
        {
            ...
            if (ctrl != null)
                ctrl.markDirty(surfaceRegion, false);
        }
    ...
    protected final synchronized void invalidate(Envelope surfaceRegion)
    {
        this.invalidateNoSync(surfaceRegion);
    }
    ...
    public synchronized void draw(GLMapView view, int renderPass)
    {
        ...
        final Collection<? extends GLMapRenderable2> renderList = this.getRenderList();
        for (GLMapRenderable2 r : renderList)
            if ((r.getRenderPass() & renderPass) != 0)
    ...
    {
        for (GLMapRenderable2 r : this.getRenderList())
            r.release();
    }
    ...
    public synchronized State getState()
    {
        ...
        else if (!this.initialized || this.checkState())
            return State.UNRESOLVED;

        // the scene is up to date, check the renderables for resolution state
        Collection<? extends GLMapRenderable2> renderList = this.getRenderList();
        for (GLMapRenderable2 r : renderList)
        {
            if (r instanceof GLResolvable)
            {
                switch (((GLResolvable) r).getState())
                {
                    case UNRESOLVED:
                    case RESOLVING:
    ...
    protected static class ViewState
    {
         ...
         * to the
         * {@link GLAsynchronousMapRenderable2#query(ViewState, Object) query}
        ...
        public void set(GLMapView view)
        {
            ...
            this.drawTilt = view.currentScene.drawTilt;
            ...
            this.renderPump = view.currentScene.renderPump;
            this.scene = new MapSceneModel(view.currentScene.scene);
            ...
            if (ctrl != null)
            {
                ...
                double maxSurfaceGsd = Double.MAX_VALUE;
                for (Envelope region : this.surfaceRegions)
                {
                    final double gsd = OSMUtils.mapnikTileResolution((int) (Math.log(180.0 / (region.maxY - region.minY)) / Math.log(2.0))) / 8.0;
                    if (gsd < maxSurfaceGsd)
                        maxSurfaceGsd = gsd;
                }
                this.drawMapResolution = Math.max(this.drawMapResolution, maxSurfaceGsd);
            }

            this.drawMapScale = Globe.getMapScale(view.currentScene.scene.dpi, this.drawMapResolution);
        }
        ...
        public void copy(ViewState view)
        {
            ...
            // assign by reference -- this method is used for transfer
    ...
    protected class WorkerThread implements Runnable
    {
        ...
        public void run()
        {
            GLAsynchronousMapRenderable2.this.backgroundThreadEntry();

            Pending pendingData = null;
            try
            {
                pendingData = GLAsynchronousMapRenderable2.this.createPendingData();
                ViewState queryState = GLAsynchronousMapRenderable2.this.newViewStateInstance();
                while (true)
                {
                    synchronized (GLAsynchronousMapRenderable2.this)
                    {
                        if (GLAsynchronousMapRenderable2.this.checkQueryThreadAbort())
                            break;

                        GLAsynchronousMapRenderable2.this.dirtyRegions.clear();
                        if (queryState.crossesIDL)
                        {
                            GLAsynchronousMapRenderable2.this.dirtyRegions.add(
                                    new Envelope(
                                            queryState.westBound, queryState.southBound, 0d,
                                            180d, queryState.northBound, 0d));
                            GLAsynchronousMapRenderable2.this.dirtyRegions.add(
                                    new Envelope(
                                            -180d, queryState.southBound, 0d,
                                            queryState.eastBound, queryState.northBound, 0d));
                        } else
                        {
                            GLAsynchronousMapRenderable2.this.dirtyRegions.add(
                                    new Envelope(
                        ...
                        if (GLAsynchronousMapRenderable2.this.servicingRequest
                                && GLAsynchronousMapRenderable2.this
                                .updateRenderList(queryState, pendingData))
                        {
                            GLAsynchronousMapRenderable2.this.preparedState.copy(queryState);
                            if (GLAsynchronousMapRenderable2.this.renderCtx != null)
                                GLAsynchronousMapRenderable2.this.renderCtx.requestRefresh();
                            if (GLAsynchronousMapRenderable2.this.surfaceControl != null &&
                                    !GLAsynchronousMapRenderable2.this.dirtyRegions.isEmpty())
                            {

                                final Collection<Envelope> dirty = new ArrayList<>(GLAsynchronousMapRenderable2.this.dirtyRegions);
                                GLAsynchronousMapRenderable2.this.renderCtx.queueEvent(new Runnable()
                                {
                                    public void run()
                                    {
                                        for (Envelope region : dirty)
                                            GLAsynchronousMapRenderable2.this.surfaceControl.markDirty(region, false);
                                    }
                                });
                            }
                        }
                        GLAsynchronousMapRenderable2.this.resetPendingData(pendingData);
                        GLAsynchronousMapRenderable2.this.servicingRequest = false;

                        // check the state and wait if appropriate
                        if (!GLAsynchronousMapRenderable2.this.isDirty())
                        {
                            try
                            {
                                GLAsynchronousMapRenderable2.this.wait();
                            } catch (InterruptedException ignored)
                            {
                            }
                            continue;
                        ...
                        // synchronized block
                        queryState.copy(GLAsynchronousMapRenderable2.this.targetState);
                        GLAsynchronousMapRenderable2.this.invalid = false;
                        GLAsynchronousMapRenderable2.this.servicingRequest = true;
                    }

                    GLAsynchronousMapRenderable2.this.query(queryState, pendingData);
                }
            } finally
            {
                if (pendingData != null)
                    GLAsynchronousMapRenderable2.this.releasePendingData(pendingData);
                GLAsynchronousMapRenderable2.this.servicingRequest = false;

                GLAsynchronousMapRenderable2.this.backgroundThreadExit();
            }
        }
    }
}
