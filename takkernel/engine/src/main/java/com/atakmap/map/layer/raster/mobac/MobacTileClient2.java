...
public class MobacTileClient2 implements TileClient, TileClientControl
{
    ...
    public final static long ONE_WEEK_MILLIS = TimeUnit.DAYS.toMillis(7);
    ...
    private static Map<String, ReferenceCount<TileContainer>> caches = new HashMap<>();
    ...
    public byte[] getTileData(int zoom, int x, int y, Throwable[] error)
    {
                ...
                && System.currentTimeMillis() > expiration)
        {
            try
            {
                ...
                {
                    retval = tile.data;
                    expiration = Math.max(tile.expiration, (System.currentTimeMillis() + ONE_WEEK_MILLIS));

                    // update cache with downloaded data
                    if (this.offlineCache != null) this.offlineCache.setTile(zoom, x, y, tile.data, expiration);
                }
            } catch (IOException e)
            {
                ...
                if (!suppressLog)
                {
                    final String reason = suppressible ? errorType.name() : "IO Error";
                    Log.e(TAG, String.format("%s during tile download, %s (%d, %d, %d)", reason, getName(), x, y, zoom), suppressible ? null : e);
                    if (SuppressibleErrorTracker.suppressDuplicateLogs() && suppressible)
                        Log.e(TAG, "Suppressing additional " + reason + " error logs for " + getName());
                }
                if (error != null)
                    error[0] = e;
            } catch (Throwable t)
