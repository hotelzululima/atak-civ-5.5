...
@DontObfuscate final class NativeElevationSource extends ElevationSource implements Disposable, Controls
{
    ...
    private NativeElevationSource(Pointer pointer, Object owner)
    {
        ...
        this.controls = Collections2.newIdentityHashSet();
    ...
    @DontObfuscate final static class CallbackForwarder implements NotifyCallback
    {
    ...
    @DontObfuscate final static class NativeOnContentChangedListener implements OnContentChangedListener
    {
        final ReadWriteLock rwlock = new ReadWriteLock();
        Pointer pointer;
        long sourcePtr;
    ...
    static void transferControls(ElevationSource[] sources, ElevationSource sink) {
        if(!(sink instanceof NativeElevationSource))
            return;

        // collect controls
        Map<Class<?>, Collection> controls = new HashMap<>();
        for(ElevationSource source : sources) {
            if (source instanceof Controls) {
                for (Class<?> transferControlType : transferControlTypes) {
                    final Object ctrl = ((Controls) source).getControl(transferControlType);
                    if (ctrl != null) {
                        Collection c = controls.get(transferControlType);
                        if(c == null)
                            controls.put(transferControlType, c=new ArrayList(sources.length));
                        c.add(ctrl);
                    }
                }
            }
        ...
        // aggregate controls
        final Collection tileClientControls = controls.get(TileClientControl.class);
        if(tileClientControls != null) {
            if(tileClientControls.size() == 1)
                ((NativeElevationSource)sink).controls.add(Collections2.first(tileClientControls));
            else
                ((NativeElevationSource)sink).controls.add(new TileClientControlAggregator(tileClientControls));
        }
        final Collection spatialFilterControls = controls.get(SpatialFilterControl.class);
        if(spatialFilterControls != null) {
            if(spatialFilterControls.size() == 1)
                ((NativeElevationSource)sink).controls.add(Collections2.first(spatialFilterControls));
            else
                ((NativeElevationSource)sink).controls.add(new SpatialFilterControlAggregator(spatialFilterControls));
        }
    }

    static void transferControls(ElevationSource source, ElevationSource sink) {
        if(!(sink instanceof NativeElevationSource))
            return;
        ...
        if(source instanceof Controls) {
            for (Class<?> transferControlType : transferControlTypes) {
                final Object ctrl = ((Controls) source).getControl(transferControlType);
                if(ctrl != null)
                    ((NativeElevationSource)sink).controls.add(ctrl);
            }
        }
    }
    ...
    private static native ElevationSource multiplex(String name, ElevationSource[] sources);
    static ElevationSource multiplexSources(String name, ElevationSource[] sources) {
        final ElevationSource result = multiplex(name, sources);
        transferControls(sources, result);
        return result;
    }
    private static native ElevationSource cascade(String name, ElevationSource[] sources);
    static ElevationSource cascadeSources(String name, ElevationSource[] sources) {
        final ElevationSource result = cascade(name, sources);
        transferControls(sources, result);
        return result;
    }
    private static native ElevationSource constrainResolution(ElevationSource impl, double minResolution, double maxResolution);
    static ElevationSource constrainResolutionSource(ElevationSource impl, double minResolution, double maxResolution) {
        final ElevationSource result = constrainResolution(impl, minResolution, maxResolution);
        transferControls(impl, result);
        return result;
    }
    private static native ElevationSource clod(ElevationSource impl, ElevationManager.HeightmapStrategy strategy, boolean sampleOnly);
    static ElevationSource clodSource(ElevationSource impl, ElevationManager.HeightmapStrategy strategy, boolean sampleOnly) {
        final ElevationSource result = clod(impl, strategy, sampleOnly);
        transferControls(impl, result);
        return result;
    }
    private static native ElevationSource tile(ElevationSource impl, boolean pointSampler, int srid, ElevationManager.HeightmapStrategy strategy, ElevationManager.HeightmapParams params, int overlap, String cachePath, int cacheLimit, boolean async);
    static ElevationSource tileSource(ElevationSource impl, boolean pointSampler, int srid, ElevationManager.HeightmapStrategy strategy, ElevationManager.HeightmapParams params, int overlap, String cachePath, int cacheLimit, boolean async) {
        final ElevationSource result = tile(impl, pointSampler, srid, strategy, params, overlap, cachePath, cacheLimit, async);
        transferControls(impl, result);
        return result;
    }

    final static class TileClientControlAggregator implements TileClientControl {

        final Collection<TileClientControl> impl;

        TileClientControlAggregator(Collection<TileClientControl> impl) {
            this.impl = impl;
            if(this.impl.isEmpty())
                throw new IllegalArgumentException();

            // synchronize state across all controls
            boolean offlineOnly = false;
            long cacheRefreshInterval = 0L;
            for(TileClientControl ctrl : this.impl) {
                offlineOnly |= ctrl.isOfflineOnlyMode();
                final long ctrlRefresh = ctrl.getCacheAutoRefreshInterval();
                if(cacheRefreshInterval == 0L || (ctrlRefresh != 0L && ctrlRefresh < cacheRefreshInterval))
                    cacheRefreshInterval = ctrlRefresh;
            }
            setOfflineOnlyMode(offlineOnly);
            setCacheAutoRefreshInterval(cacheRefreshInterval);
        }

        @Override
        public void setOfflineOnlyMode(boolean offlineOnly) {
            for(TileClientControl ctrl : impl)
                ctrl.setOfflineOnlyMode(offlineOnly);
        }

        @Override
        public boolean isOfflineOnlyMode() {
            return Collections2.first(impl).isOfflineOnlyMode();
        }
        ...
        public void setCacheAutoRefreshInterval(long milliseconds) {
            for(TileClientControl ctrl : impl)
                ctrl.setCacheAutoRefreshInterval(milliseconds);
        }

        @Override
        public long getCacheAutoRefreshInterval() {
            return Collections2.first(impl).getCacheAutoRefreshInterval();
        }
    }

    final static class SpatialFilterControlAggregator implements SpatialFilterControl {
        final Collection<SpatialFilterControl> impl;

        SpatialFilterControlAggregator(Collection<SpatialFilterControl> impl) {
            this.impl = impl;
        }

        @Override
        public void setFilter(EnvelopeFilter filter) {
            for(SpatialFilterControl ctrl : impl)
