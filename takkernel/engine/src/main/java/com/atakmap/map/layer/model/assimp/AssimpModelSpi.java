...
public class AssimpModelSpi implements ModelSpi
{
    ...
    public Model create(ModelInfo modelInfo, Callback callback)
    {
        ...
        try
        {
            ...
            Set<AiPostProcessSteps> steps = accumAnyNeededPrimitiveTypeProcessing(scene);
            if(steps.size() > 0)
            {
                postSteps.addAll(steps);
                scene = Jassimp.importFile(file.getPath(), postSteps, iosys);
            }
    ...
    private static void addMesh(ModelInfo modelInfo, File file, Map<String, int[]> meshVertCounts, Map<String, MeshBuilder> builders, ModelBuilder models, AiScene scene, int meshId, AiMesh mesh, Matrix accumulated, int[] meshProgress, boolean isInstance, ModelSpi.Callback callback)
    {
        ...
        if (builder == null)
        {
            ...
            builder = new MeshBuilder(vertexAttr, false, drawModeFromAssimpPType(mesh.getPrimitiveTypes()));
            if (!isInstance)
...
*/
    ...
    static Set<AiPostProcessSteps> accumAnyNeededPrimitiveTypeProcessing(AiScene scene) {

        List<AiMesh> meshes = scene.getMeshes();
        HashSet<AiPostProcessSteps> steps = new HashSet<>();
;
        for (AiMesh mesh : meshes) {
            if(mesh.getPrimitiveTypes().size() > 1)
                steps.add(AiPostProcessSteps.SORT_BY_PTYPE);

            if (mesh.getPrimitiveTypes().contains(AiPrimitiveType.POLYGON)) {
    ...
    static Mesh.DrawMode drawModeFromAssimpPType(Set<AiPrimitiveType> ptype) {
        if(ptype.contains(AiPrimitiveType.POINT))
            return Mesh.DrawMode.Points;
        if(ptype.contains(AiPrimitiveType.LINE))
            return Mesh.DrawMode.Lines;
        if(ptype.contains(AiPrimitiveType.TRIANGLE))
            return Mesh.DrawMode.Triangles;
    ...
    private static Envelope getAabb(FloatBuffer buf, int numVerts)
    {
        ...
        for (int i = 1; i < numVerts; i++)
        {
            ...
            if (x < retval.minX)
                retval.minX = x;
            else if (x > retval.maxX)
                retval.maxX = x;
            if (y < retval.minY)
                retval.minY = y;
            else if (y > retval.maxY)
                retval.maxY = y;
            if (z < retval.minZ)
                retval.minZ = z;
            else if (z > retval.maxZ)
                retval.maxZ = z;
        }
        return retval;
    }
