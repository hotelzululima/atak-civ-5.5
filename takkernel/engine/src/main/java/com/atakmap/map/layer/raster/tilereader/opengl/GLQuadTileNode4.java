        ...
        Disposable
{
    ...
    private static final int HINT_RECEIVED_UPDATE = 1;
    private static final int HINT_PARTIAL_RESULT = 1 << 1;
    ...
    private final boolean suppressDuplicateLogs;
    private static boolean loggedAtLeastOnce = false;
     ...
     * @param opts       The render configuration {@link NodeOptions}. May be
     *                   <code>null</code>.
    ...
    private GLQuadTileNode4(GLQuadTileNode4 parent, int idx, NodeCore core)
    {
        ...
        {
            // root is at "max" reduced resolution level for tilereader
            this.set(0, 0, this.core.tileReader.getMaxNumResolutionLevels()-1);
        }
        ...
        this.suppressDuplicateLogs = SuppressibleErrorTracker.suppressDuplicateLogs() || ctrl != null;
    }
    ...
    private Envelope computeBounds(long srcX, long srcY, long srcW, long srcH)
    {
        ...
        if (scratchG.getLatitude() < mbb.minY)
            mbb.minY = scratchG.getLatitude();
        if (scratchG.getLatitude() > mbb.maxY)
            mbb.maxY = scratchG.getLatitude();
        if (scratchG.getLongitude() < mbb.minX)
            mbb.minX = scratchG.getLongitude();
        if (scratchG.getLongitude() > mbb.maxX)
            mbb.maxX = scratchG.getLongitude();
    ...
    private void set(long tileColumn, long tileRow, int level)
    {
        ...
        Envelope bounds = this.core.getNodeBounds(level, tileColumn, tileRow);
        minLng = bounds.minX;
        minLat = bounds.minY;
        maxLng = bounds.maxX;
        maxLat = bounds.maxY;
    ...
    private void releaseTexture()
    {
        ...
        this.hints &= ~HINT_RECEIVED_UPDATE;
        this.hints &= ~HINT_PARTIAL_RESULT;
        this.derivedUnresolvableData = false;
    }
    ...
    public void draw(GLMapView view, int renderPass)
    {
        ...
        if (this.core.shader == null)
        {
            ...
            core.scaleControl = view.getControl(ImageryRelativeScaleControl.class);
        }

        double levelTransitionAdjustment = this.core.options.levelTransitionAdjustment;
        if(this.core.scaleControl != null) {
            levelTransitionAdjustment += Math.log(this.core.scaleControl.getRelativeScale()) / Math.log(2.0);
        }

        // update the surface bounds filter
//        if(core.spatialFilter != null && core.surfaceControl != null)
//            core.spatialFilter.update(view, core.surfaceControl);

        if (this.lastTouch != view.currentPass.renderPump)
        {
            ...
            if(this.core.imprecise.groundToImage(view.scratch.geo, view.scratch.pointD))
                this.core.asyncio.setCameraLocation(view.scratch.geo);

            if (core.cacheControl != null)
                core.cacheControl.prioritize(view.scratch.geo);

            core.refreshUpdateList();

            // compile prefetch list
            final boolean checkLevel = (view.getDisplayMode() != IMapRendererEnums.DisplayMode.Flat);
            ...
            for (RequestRegion region : regions)
            {
                ...
                if (checkLevel)
                {
                    final Envelope envelope = region.bounds;
                    // apply some adjustment when we get close to the poles
                    int osmlevel = (int) (Math.log(180.0 / (envelope.maxY - envelope.minY)) / Math.log(2.0)) + 3;
                    double minY = MathUtils.clamp(envelope.minY, -85.0511, 85.0511);
                    double maxY = MathUtils.clamp(envelope.maxY, -85.0511, 85.0511);
                    // shift window within slippy tile bounds
                    if (minY >= 85.0511)
                        minY = maxY - (envelope.maxY - envelope.minY);
                    else if (maxY <= -85.0511)
                        maxY = minY + (envelope.maxY - envelope.minY);

                    final int y0 = OSMUtils.mapnikTileY(osmlevel, maxY);
                    final int y1 = OSMUtils.mapnikTileY(osmlevel, minY);
                    final int x0 = OSMUtils.mapnikTileX(osmlevel, envelope.minX);
                    final int x1 = OSMUtils.mapnikTileX(osmlevel, envelope.maxX);
                    ...
                    final int subsample = (int) (Math.log((double) (y1 - y0) / (double) (x1 - x0)) / Math.log(2.0)) / 2;
                    region.level = MathUtils.clamp(region.level + Math.min(subsample, 7), region.level, root.tileIndex.z);
                }
        ...
        {
            gov.tak.platform.commons.opengl.Matrix.orthoM(view.scratch.matrixF, 0, view.currentPass.left, view.currentPass.right, view.currentPass.bottom, view.currentPass.top, (float) view.currentPass.scene.camera.near, (float) view.currentPass.scene.camera.far);
            for (int i = 0; i < 16; i++)
                core.xproj.set(i % 4, i / 4, view.scratch.matrixF[i]);
        }
        core.mvp.set(core.xproj);
        core.mvp.concatenate(view.currentPass.scene.forward);
        ...
        if (!shouldDraw(view.scratch.envelope, minLng, minLat, maxLng, maxLat))
        {
            ...
            {
                cull(view.currentPass.renderPump);
        ...
        final double scale = (this.core.gsd / view.currentPass.drawMapResolution);

        // XXX - tune level calculation -- it may look better to swap to the
        // next level before we actually cross the threshold
        final int level = (int) Math.ceil(Math.max((Math.log(1.0d / scale) / Math.log(2.0)) + levelTransitionAdjustment, 0d));
            ...
            try
            {
                ...
                for (Pair<RequestRegion, Collection<GLMapRenderable2>> entry : core.requestRegions)
                {
                    ...
                    if (entry.first.bounds.equals(view.scratch.envelope))
                    {
                        for (GLMapRenderable2 r : entry.second)
                            ((GLQuadTileNode4) r).drawImpl(view, entry.first.level, MathUtils.hasBits(renderPass, GLMapView.RENDER_PASS_SURFACE_PREFETCH));
                ...
                if (!drawnFromPrefetch)
                {
                    ...
                    this.drawImpl(
                            ...
                            MathUtils.hasBits(renderPass, GLMapView.RENDER_PASS_SURFACE_PREFETCH));
                }
            } finally
            {
    ...
    private boolean shouldResolve()
    {
        ...
        for (int i = 0; i < this.children.length; i++)
            resolveUnresolvable |= (this.children[i] != null) &&
                    MathUtils.hasBits(this.children[i].hints, HINT_RECEIVED_UPDATE);

        return (this.state == State.UNRESOLVABLE && resolveUnresolvable) ||
                ((this.state == State.UNRESOLVED) && (this.currentRequest == null));
    }

    private void resolveTexture(boolean fetch, boolean isPrefetchRequest)
    {
        if (!this.core.tileReader.isMultiResolution() && this.core.textureCache != null)
        {
            final String key = core.uri + "&x=" + tileIndex.x + "&y=" + tileIndex.y + "&z=" + tileIndex.z;
                ...
                !offscreenFboFailed)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (this.children[i] != null
                        && ((this.children[i].state == State.RESOLVED) ||
                        MathUtils.hasBits(this.children[i].hints, HINT_RECEIVED_UPDATE)))
                {
                    hasChildData = true;
                }
            ...
            if (hasChildData)
            {
                ...
                do
                {
                    ...
                    if(frameBuffer[0] == GLES30.GL_NONE)
                        break;
                ...
                if (fboCreated)
                {
                    ...
                    gov.tak.platform.commons.opengl.Matrix.orthoM(mx, 0, 0, this.tileWidth, 0, this.tileHeight, 1, -1);
                    GLES30.glUniformMatrix4fv(core.shader.uMVP, 1, false, mx, 0);

                    core.resources.coordStreamBufferF.clear();
                    FloatBuffer xyuv = core.resources.coordStreamBufferF;
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        if (this.children[i] != null
                                ...
                                && ((this.children[i].state == State.RESOLVED) ||
                                MathUtils.hasBits(this.children[i].hints,
                                        HINT_RECEIVED_UPDATE)))
                        {
                            tx = i % 2;
                            ty = i / 2;
                ...
                // XXX - observing failure to construct the derived tile in some cases. borrowing
                //       will be disabled based on state, resulting in a tile appearing as empty
                //       temporarily. In the case where the texture is marked as unresolved, mark
                //       as partial as a temporary workaround

                // set hint if we only have partial results from the children
                if (this.state == State.UNRESOLVED /*&& parts < numChildren*/)
                    this.hints |= HINT_PARTIAL_RESULT;
        ...
        if (this.state == State.UNRESOLVED && fetch)
        {
            ...
            final TileReader.ReadRequest request = new TileReader.ReadRequest(this.core.tileReader, this.tileIndex.z, this.tileIndex.x, this.tileIndex.y, this);
            request.isPrefetch = isPrefetchRequest;
            this.core.asyncio.asyncRead(
                    request,
                    new Envelope(this.minLng, this.minLat, 0d, this.maxLng, this.maxLat, 0d));
        }
    }
    ...
    private void recurse(GLMapView view, RequestRegion region, Collection<GLMapRenderable2> nodes)
    {
        ...
        // all nodes touched during recurse are part of the visible set, update prefetch flag
        if (this.currentRequest != null)
            this.currentRequest.isPrefetch = false;
        ...
        } else if (region.level == this.tileIndex.z)
        {
            ...
            if (shouldResolve() || needsRefresh())
                this.resolveTexture(true, false);
            nodes.add(this);
        } else
        {
                    ...
                    ((this.state == State.UNRESOLVED) || (this.state == State.UNRESOLVABLE && needsRefresh())))
            {
                ...
                this.resolveTexture(core.tileReader.isMultiResolution(), false);
                this.touched = true;
                view.requestRefresh();
            }
     ...
     */
    private boolean drawImpl(GLMapView view, int level, boolean isPrefetchPass)
    {
        ...
        {
            // draw self
            this.drawImpl(view, true, isPrefetchPass);
        ...
        } else
        {
                    ...
                    ((this.state == State.UNRESOLVED) || (this.state == State.UNRESOLVABLE && needsRefresh())))
            {
                ...
                if (state == State.UNRESOLVABLE)
                    state = State.UNRESOLVED;
                this.resolveTexture(core.tileReader.isMultiResolution(), isPrefetchPass);
                this.touched = true;
                view.requestRefresh();
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                if (!drawChild[i])
                    continue;
                this.children[i].verticesInvalid |= this.verticesInvalid;
                descendantsRequestDraw |= this.children[i].drawImpl(view, level, isPrefetchPass);
            }
            this.verticesInvalid = false;
    ...
    /**
     * Invokes {@link #super_draw(GLMapView, boolean, boolean)}.
     *
     * @param view The view
     */
    private final void drawImpl(GLMapView view, boolean resolve, boolean isPrefetchPass)
    {
        ...
        super_draw(view, resolve, isPrefetchPass);
    ...
    private void super_draw(GLMapView view, boolean resolve, boolean isPrefetchPass)
    {
        ...
        if (resolve)
        {
                    ...
                    needsRefresh())
            {
                ...
                if (this.state == State.UNRESOLVABLE)
                    this.state = State.UNRESOLVED;
                this.resolveTexture(resolve, isPrefetchPass);
            }
        ...
        boolean receivedUpdate = MathUtils.hasBits(this.hints, HINT_RECEIVED_UPDATE);
        boolean partialResult = MathUtils.hasBits(this.hints, HINT_PARTIAL_RESULT);
        if (!this.core.options.adaptiveTileLod)
        {
            // Only draw borrowed ancestor tile if we aren't resolved and we have no pre-existing
            // data or only partial pre-existing data.
            if (this.state != State.RESOLVED && (!receivedUpdate || partialResult))
            {
    ...
    private GLTexture tryBorrow()
    {
        ...
        while (borrowFrom != this.borrowingFrom)
        {
            if (borrowFrom.state == State.RESOLVED)
                break;
            else if (MathUtils.hasBits(borrowFrom.hints, HINT_RECEIVED_UPDATE) &&
                    updatedAncestor == null)
...
*/
        ...
        final float stater = Color.red(stateColor) / 255f;
        final float stateg = Color.green(stateColor) / 255f;
        final float stateb = Color.blue(stateColor) / 255f;
        final float statea = Color.alpha(stateColor) / 255f;

        // XXX - tints tile based on state
        if (true)
        {
            r = Color.red(0x3FFFFFFF & stateColor) / 255f;
            g = Color.green(0x3FFFFFFF & stateColor) / 255f;
            b = Color.blue(0x3FFFFFFF & stateColor) / 255f;
            a = Color.alpha(0x3FFFFFFF & stateColor) / 255f;
            if (r != core.renderState.r || g != core.renderState.g || b != core.renderState.b || a != core.renderState.a)
            {
                GLES30.glUniform4f(core.shader.uColor, r, g, b, a);
                core.renderState.r = r;
        ...
        view.scratch.pointD.x = this.tileSrcX + 16;
        view.scratch.pointD.y = this.tileSrcY + this.tileSrcHeight / 4f;
        ...
        _titleText.draw((new java.io.File(core.tileReader.getUri())).getName(), stater, stateg, stateb, statea);
        com.atakmap.opengl.GLES20FixedPipeline.glTranslatef(0f, -_titleText.getBaselineSpacing(), 0);
        _titleText.draw(this.state.name(), stater, stateg, stateb, statea);
        com.atakmap.opengl.GLES20FixedPipeline.glTranslatef(0f, -_titleText.getBaselineSpacing(), 0);
        _titleText.draw(this.tileIndex.x + "," + this.tileIndex.y + "," + this.tileIndex.z, stater, stateg, stateb, statea);
        ...
        core.context.queueEvent(new Runnable()
        {
            ...
            public void run()
            {
                ...
                try
                {
                    ...
                    GLQuadTileNode4.this.hints |= HINT_RECEIVED_UPDATE;
                } finally
                {
                ...
                if (core.surfaceControl != null)
                    core.surfaceControl.markDirty(new Envelope(minLng, minLat, 0.0, maxLng, maxLat, 0.0), false);
            }
        });
    }
    ...
    public void requestCompleted(final int id)
    {
        ...
        core.context.queueEvent(new Runnable()
        {
            ...
            public void run()
            {
                ...
                GLQuadTileNode4.this.hints &= ~HINT_PARTIAL_RESULT;
            ...
            public void run()
            {
                                                ...
                                                tileIndex.z, tileIndex.x, tileIndex.y))))
                {
                            ...
                            ((GLQuadTileNode4.this.parent.state == State.UNRESOLVABLE && needsRefresh())))
                    {
                        ...
                        GLQuadTileNode4.this.parent.resolveTexture(true, current.isPrefetch);
