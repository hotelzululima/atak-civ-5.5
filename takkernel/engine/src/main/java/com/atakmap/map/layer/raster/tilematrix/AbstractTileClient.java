...
public abstract class AbstractTileClient implements TileClient
{
    ...
    public byte[] getTileData(int zoom, int x, int y, Throwable[] error)
    {
        ...
        final boolean tileExpired = expiration != -1 && System.currentTimeMillis() > expiration;

        // We have a non-expired tile, so try to load the cached version
        if (!tileExpired)
        {
            retval = getOfflineTileData(zoom, x, y, error);
        }

        // if data is null or expiration exceeded, load from URL
        if (!this.offlineMode && (tileExpired || retval == null))
        {
            try
            {
                byte[] sourceTileData = this.getTileDataImpl(zoom, x, y);
                if (sourceTileData != null)
                {
                    retval = sourceTileData;
                ...
                {
                    // If we didn't get anything from the source, try the offline cache
                    retval = getOfflineTileData(zoom, x, y, error);
                }
            } catch (IOException e)
            {
    ...
    private byte[] getOfflineTileData(int zoom, int x, int y, Throwable[] error)
    {
        byte[] retval = null;
