...
public final class ModelInfoFactory
{
    ...
    public static Set<ModelInfo> create(String path, String hint)
    {
        ...
        if (retval == null) {
            // fall back on `SceneInfoFactory` native impls
            return SceneInfoFactory.create(path, hint);
        }
        for (ModelInfo info : retval)
        {
            if (info.metadata != null) info.metadata.setAttribute("legacyModelInfoFactory", "");
            if (info.srid != -1)
                continue;
            rwlock.acquireRead();
            try
    ...
    public static boolean isSupported(final String uri, String hint)
    {
        ...
        if(!supported[0])
            supported[0] = SceneInfoFactory.isSupported(uri, hint);
        return supported[0];
    }
