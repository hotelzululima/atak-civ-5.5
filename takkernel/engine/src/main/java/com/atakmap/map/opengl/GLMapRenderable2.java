 ...
 * {@link #draw(GLMapView, int)} followed by a single invocation of
 ...
 * {@link #draw(GLMapView, int)} method is invoked when it is in the
 ...
 * from its {@link #draw(GLMapView, int)} method while in the
 * <I>initializing</I> state.
 *
 * <P>Once in the <I>initialized</I> state, the renderable should render the
 * appropriate content during each invocation of the
 * {@link #draw(GLMapView, int)} method. In the event that construction of the
 * scene for the current {@link GLMapView} state cannot be completed within
 * frame-rate time, scene construction should be offloaded to a background
 * thread.
 *
 * <H3>Release</H3>
 *
 ...
 * re-initialization via a subsequent invocation of
 * {@link #draw(GLMapView, int)}.
 *
 * @author Developer
 */
@DontObfuscate
public interface GLMapRenderable2 extends Releasable
{
    /**
     ...
     * @param renderPass The render pass being executed during this
     *                   invocation. The value will be the bitwise-OR of any
     *                   of the flags:
     *                   <UL>
     *                   <LI>{@link GLMapView#RENDER_PASS_SURFACE}
     *                   <LI>{@link GLMapView#RENDER_PASS_SPRITES}
     *                   </UL>
     *                   Only content valid for the specified pass should be
     *                   rendered.
     */
    ...
    /**
     * Returns a flag indicating the render passes that this renderable contains
     * content for. The returned value should be the bitwise-OR of any of the
     * following flags:
     * <UL>
     * <LI>{@link GLMapView#RENDER_PASS_SURFACE}
     * <LI>{@link GLMapView#RENDER_PASS_SPRITES}
     * </UL>
     *
     * @return A flag indicating the render passes that this renderable
     * contains content for
     */
