...
public abstract class GdalDatasetProjection2 implements DatasetProjection2
{
    ...
    private static final double[] IDENTITY_GEOTRANSFORM = {0, 1, 0, 0, 0, 1};
    ...
    void image2projected(double[] p)
    {
        PointD xy = image2projected(new PointD(p[0], p[1]));
        p[0] = xy.x;
        p[1] = xy.y;
    }

    void projected2image(double[] p)
    {
        PointD xy = projected2image(new PointD(p[0], p[1]));
        p[0] = xy.x;
        p[1] = xy.y;
    }
    ...
    {
        final double[] proj;
        if(nativeSpatialReferenceID != 4326) {
            proj = this.geo2proj.TransformPoint(g.getLongitude(), g.getLatitude());
        } else {
            proj = new double[]{g.getLongitude(), g.getLatitude()};
        }
        this.projected2image(proj);
        p.x = proj[0];
        p.y = proj[1];
        return true;
    }
    ...
    {
        double[] proj = new double[] {p.x, p.y};
        this.image2projected(proj);

        if(nativeSpatialReferenceID != 4326) {
            final double[] geo = this.proj2geo.TransformPoint(proj[0], proj[1]);
            g.set(geo[1], geo[0]);
        } else {
            g.set(proj[1], proj[0]);
        }
    ...
    public static GdalDatasetProjection2 getInstance(Dataset dataset)
    {
        ...
        if (isUsableGeoTransform(dataset.GetGeoTransform()))
        {
            ...
            } else
            {
                ...
                return new GeoTransform(dataset, spatialReference);
            }
    ...
    public static boolean isUsableGeoTransform(double[] geoTransform)
    {
        if (geoTransform == null)
            return false;
        if (Arrays.equals(geoTransform, IDENTITY_GEOTRANSFORM))
            return false;
    ...
    private static class GeoTransform extends GdalDatasetProjection2
    {
        ...
        // NOTE: the geotransform maps the UL coordinate to the UL corner of pixel 0,0 and the LR
        // coordinate to the LR corner of pixel width,height. As such, pixel scaling is applied to
        // map within the addressable bounds of the image.
        ...
        private GeoTransform(Dataset dataset, SpatialReference spatialReference)
        {
            ...
            final double width = dataset.GetRasterXSize();
            final double height = dataset.GetRasterYSize();
            this.pixelScaleX = (width > 1d) ? (width-1d) / width : 1d;;
            this.pixelScaleY = (height > 1d) ? (height-1d) / height : 1d;
        }
        ...
        protected PointD projected2image(PointD p)
        {
            ...
            return new PointD(imgX*pixelScaleX, imgY*pixelScaleY);
        }

        @Override
        protected PointD image2projected(PointD p)
        {
            final double px = p.x / pixelScaleX;
            final double py = p.y / pixelScaleY;
            final double projX = this.img2proj[0] + px * this.img2proj[1] + py * this.img2proj[2];
            final double projY = this.img2proj[3] + px * this.img2proj[4] + py * this.img2proj[5];

            return new PointD(projX, projY);
        }

        @Override
        protected void projected2image(double[] p)
        {
            final double imgX = this.proj2img[0] + p[0] * this.proj2img[1] + p[1]* this.proj2img[2];
            final double imgY = this.proj2img[3] + p[0] * this.proj2img[4] + p[1] * this.proj2img[5];
            p[0] = imgX*pixelScaleX;
            p[1] = imgY*pixelScaleY;
        }

        @Override
        protected void image2projected(double[] p)
        {
            final double px = p[0] / pixelScaleX;
            final double py = p[1] / pixelScaleY;
            final double projX = this.img2proj[0] + px * this.img2proj[1] + py * this.img2proj[2];
            final double projY = this.img2proj[3] + px * this.img2proj[4] + py * this.img2proj[5];
            p[0] = projX;
            p[1] = projY;
    ...
    {
        private final double[] img2proj = new double[16];
        private final double[] proj2img = new double[16];

        private final double p2i_x_x;
        private final double p2i_x_y;
        private final double p2i_x_c;
        private final double p2i_y_x;
        private final double p2i_y_y;
        private final double p2i_y_c;
        private final double p2i_w_x;
        private final double p2i_w_y;
        private final double p2i_w_c;
                                    ...
                                    ICHIPB ichipb) throws NoninvertibleTransformException
        {
            ...
            Matrix img2proj = Matrix.mapQuads(src2dst[0][0], src2dst[1][0], src2dst[2][0],
                    src2dst[3][0],
                    src2dst[0][1], src2dst[1][1], src2dst[2][1], src2dst[3][1]);

            if (ichipb != null)
                img2proj.concatenate(ichipb.getTransform());
            img2proj.get(this.img2proj);

            Matrix proj2img = img2proj.createInverse();
            proj2img.get(this.proj2img);

            p2i_x_x = this.proj2img[0];
            p2i_x_y = this.proj2img[1];
            p2i_x_c = this.proj2img[3];

            p2i_y_x = this.proj2img[4];
            p2i_y_y = this.proj2img[5];
            p2i_y_c = this.proj2img[7];

            p2i_w_x = this.proj2img[12];
            p2i_w_y = this.proj2img[13];
            p2i_w_c = this.proj2img[15];
        }

        @Override
        protected PointD projected2image(PointD p)
        {
            final double w = p.x*p2i_w_x + p.y*p2i_w_y + p2i_w_c;
            return new PointD(
                    (p.x*p2i_x_x + p.y*p2i_x_y + p2i_x_c)/w,
                    (p.x*p2i_y_x + p.y*p2i_y_y + p2i_y_c)/w);
        }

        @Override
        protected PointD image2projected(PointD p)
        {
            final double w = p.x*img2proj[12] + p.y*img2proj[13] + img2proj[15];
            return new PointD(
                    (p.x*img2proj[0]+p.y*img2proj[1]+img2proj[3])/w,
                    (p.x*img2proj[4]+p.y*img2proj[5]+img2proj[7])/w);
        }
        ...
        protected void projected2image(double[] p)
        {
            ...
            final double w = x*p2i_w_x + y*p2i_w_y + p2i_w_c;
            p[0] = (x*p2i_x_x + y*p2i_x_y + p2i_x_c)/w;
            p[1] = (x*p2i_y_x + y*p2i_y_y + p2i_y_c)/w;
        }
        ...
        protected void image2projected(double[] p)
        {
            ...
            final double w = x*img2proj[12] + y*img2proj[13] + img2proj[15];
            p[0] = (x*img2proj[0]+y*img2proj[1]+img2proj[3])/w;
            p[1] = (x*img2proj[4]+y*img2proj[5]+img2proj[7])/w;
        }
    }
