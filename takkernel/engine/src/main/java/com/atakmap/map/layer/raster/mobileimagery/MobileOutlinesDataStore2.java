...
public class MobileOutlinesDataStore2 extends OutlinesFeatureDataStore2
{

    private static final String TAG = "MobileOutlinesDataStore2";
    private boolean union = true;

    public MobileOutlinesDataStore2(RasterLayer2 layer,
                                    int strokeColor, FeatureDataStore2 dataStore)
    {
        super(layer, strokeColor, false, dataStore);
    }
    ...
    protected void refreshImpl()
    {
        ...
        try
        {
            ...
            if (doUnion)
            {
                ...
                try
                {
                    result = queryFeatures(params);
                } catch (DataStoreException dataStoreException)
                {
                    Log.e(TAG, "Failed to queryFeatures", dataStoreException);
                }
            } else
            {
            ...
            try {
                ...
                Log.w(TAG, "Failed to generate coverage unions", t);
            } finally
            {
                if (calc != null) {
                    try {
        ...
        synchronized (this)
        {
            try
            {
                ...
                for (Map.Entry<Long, Geometry> entry : unions.entrySet())
                {
                    ...
                    try
                    {
                        if (Utils.getFeature(this, entry.getKey()) == null)
                            continue;
                        this.updateFeature(entry.getKey(), PROPERTY_FEATURE_GEOMETRY, null, entry.getValue(),
                                null, null, 0);
                    } catch (DataStoreException dataStoreException)
                    {
                        Log.e(TAG, "MobileOutlinesDataStore2 failed to update feature", dataStoreException);
                    }
                }
            } catch (InterruptedException interruptedException)
            {
                Log.e(TAG, "MobileOutlinesDataStore2 could not aquire modify lock", interruptedException);
            } finally
            {
