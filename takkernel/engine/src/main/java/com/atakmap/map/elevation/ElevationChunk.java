...
public interface ElevationChunk extends Disposable
{
    ...
    public final static class Factory
    {
        ...
        final static PriorityServiceProviderRegistry2<ElevationChunk, Pair<ByteBuffer, ElevationChunkSpi.Hints>, SpiImpl> spis = new PriorityServiceProviderRegistry2<>();
        ...
        public static void regsiterSpi(ElevationChunkSpi spi, int priority) {
            spis.register(new SpiImpl(spi), priority);
        }

        public static void unregsiterSpi(final ElevationChunkSpi spi) {
            spis.visitProviders(new Visitor<Iterator<SpiImpl>>() {
                @Override
                public void visit(Iterator<SpiImpl> iter) {
                    while(iter.hasNext()) {
                        final SpiImpl i = iter.next();
                        if(i.impl == spi) {
                            spis.unregister(i);
                            break;
                        }
        ...
        public static ElevationChunkSpi getSpi(String mimeType) {
            final ElevationChunkSpi[] spi = new ElevationChunkSpi[1];
            spis.visitProviders(new Visitor<Iterator<SpiImpl>>() {
                @Override
                public void visit(Iterator<SpiImpl> iter) {
                    while(iter.hasNext()) {
                        final SpiImpl c = iter.next();
                        if(mimeType.equals(c.impl.getMimeType())) {
                            spi[0] = c.impl;
                            break;
                        }
                    }
         ...
         * Returns one of
         * <UL>
         *     <LI>{@link ElevationChunk.Factory.DataLoader}
         *     <LI>{@link ElevationChunk.Factory.Sampler}
         *     <LI>{@link ElevationChunk}
         *     <LI>{@code null}
         * </UL>
         * The caller must check the instance type and coerce as necessary.
         *
         * @param data      The data blob
         * @param mimeType  The MIME type of the data, may be {@link null} if unknown
         * @return
         */
        public static ElevationChunk create(final ByteBuffer data, final ElevationChunkSpi.Hints hints, String mimeType) {
            if(mimeType == null)
                return spis.create(Pair.create(data, hints));
            final ElevationChunk[] chunk = new ElevationChunk[1];
            spis.visitProviders(new Visitor<Iterator<SpiImpl>>() {
                @Override
                public void visit(Iterator<SpiImpl> iter) {
                    while(iter.hasNext()) {
                        final ElevationChunkSpi c = iter.next().impl;
                        if(mimeType.equals(c.getMimeType())) {
                            chunk[0] = c.create(data, hints);
                            if(chunk[0] != null)
                                break;
                        }
                    }
        ...
        final static class SpiImpl implements ServiceProvider<ElevationChunk, Pair<ByteBuffer, ElevationChunkSpi.Hints>>
        {
            final ElevationChunkSpi impl;

            SpiImpl(ElevationChunkSpi impl) {
                this.impl = impl;
            }

            @Override
            public ElevationChunk create(Pair<ByteBuffer, ElevationChunkSpi.Hints> object) {
                return impl.create(object.first, object.second);
            }
        }

        public static Mesh meshFromHeightmap(float[] heightmap, int tileSize, Envelope tileBounds, boolean discardNoDataPosts, Class<?> positionStorage) {
            return meshFromHeightmap(heightmap, tileSize, tileSize, tileBounds, discardNoDataPosts, positionStorage);
        }
        public static Mesh meshFromHeightmap(float[] heightmap, int tileWidth, int tileHeight, Envelope tileBounds, boolean discardNoDataPosts, Class<?> positionStorage) {
            final Pointer pointer = NativeElevationChunk.ElevationChunkFactory_meshFromHeightmap(heightmap, tileWidth, tileHeight, tileBounds.minX, tileBounds.minY, tileBounds.maxX, tileBounds.maxY, discardNoDataPosts, DataType.convert(positionStorage, true));
            return (pointer == null || pointer.raw == 0L) ? null : Mesh_interop.create(pointer);
        }

        public static Mesh meshFromHeightmap(double[] heightmap, int tileSize, Envelope tileBounds, boolean discardNoDataPosts, Class<?> positionStorage) {
            return meshFromHeightmap(heightmap, tileSize, tileSize, tileBounds, discardNoDataPosts, positionStorage);
        }
        public static Mesh meshFromHeightmap(double[] heightmap, int tileWidth, int tileHeight, Envelope tileBounds, boolean discardNoDataPosts, Class<?> positionStorage) {
            final Pointer pointer = NativeElevationChunk.ElevationChunkFactory_meshFromHeightmap(heightmap, tileWidth, tileHeight, tileBounds.minX, tileBounds.minY, tileBounds.maxX, tileBounds.maxY, discardNoDataPosts, DataType.convert(positionStorage, true));
            return (pointer == null || pointer.raw == 0L) ? null : Mesh_interop.create(pointer);
        }
    }
    ...
    final static class DataElevationChunk extends AbstractElevationChunk
    {
        ...
        public synchronized double sample(double latitude, double longitude)
        {
            ...
            if (this.proj != null)
            {
                rayOrg = proj.forward(new GeoPoint(latitude, longitude, 30000d), null);
                rayTgt = proj.forward(new GeoPoint(latitude, longitude), null);
            } else
            {
    ...
    final static class SampledElevationChunk extends AbstractElevationChunk
    {
        ...
        public synchronized Data createData()
        {
            if (this.data == null)
            {
                ...
                {
                    final double dx1 = GeoCalculations.distanceTo(new GeoPoint(bounds.getY(0), bounds.getX(0)), new GeoPoint(bounds.getY(1), bounds.getX(1)));
                    final double dx2 = GeoCalculations.distanceTo(new GeoPoint(bounds.getY(2), bounds.getX(2)), new GeoPoint(bounds.getY(3), bounds.getX(3)));
                    final double dy1 = GeoCalculations.distanceTo(new GeoPoint(bounds.getY(1), bounds.getX(1)), new GeoPoint(bounds.getY(2), bounds.getX(2)));
                    final double dy2 = GeoCalculations.distanceTo(new GeoPoint(bounds.getY(3), bounds.getX(3)), new GeoPoint(bounds.getY(0), bounds.getX(0)));
