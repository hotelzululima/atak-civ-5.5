...
public class MultiplexingFeatureCursor implements FeatureCursor
{
    ...
    public MultiplexingFeatureCursor(Collection<FeatureCursor> cursors)
    {
        this(cursors, (List<FeatureDataStore2.FeatureQueryParameters.Order>)null);
    }

    /** @deprecated use {@link #MultiplexingFeatureCursor(Collection, List)} */
    @Deprecated
    ...
    {
        this(cursors, adapt(sort));
    }

    public MultiplexingFeatureCursor(Collection<FeatureCursor> cursors, List<FeatureDataStore2.FeatureQueryParameters.Order> sort)
    {
        ...
        if (sort != null)
        {
            ...
            for (FeatureDataStore2.FeatureQueryParameters.Order order : sort)
            {
    ...
    private static Comparator<FeatureCursor> orderToComparator(FeatureDataStore2.FeatureQueryParameters.Order order)
    {
        if (order instanceof FeatureDataStore2.FeatureQueryParameters.Order.Distance)
        {
            // XXX - 
            return null;
        } else if (order instanceof FeatureDataStore2.FeatureQueryParameters.Order.ID)
        {
            return FID_COMPARATOR;
        } else if (order instanceof FeatureDataStore2.FeatureQueryParameters.Order.Name)
        {
    ...
    private static List<FeatureDataStore2.FeatureQueryParameters.Order> adapt(Collection<FeatureDataStore.FeatureQueryParameters.Order> lorder)
    {
        if(lorder == null)
            return null;
        ...
        lparams.order = lorder;
        Collection<FeatureDataStore2.FeatureQueryParameters.Order> order = Adapters.adapt(lparams, null).order;
