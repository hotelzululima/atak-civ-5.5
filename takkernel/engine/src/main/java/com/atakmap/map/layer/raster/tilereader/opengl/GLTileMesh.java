...
public class GLTileMesh implements Releasable
{
    ...
    public void drawMesh(GLMapView view, int texId, float r, float g, float b, float a)
    {
        ...
        if (this.meshVertsDrawVersion != view.currentPass.drawSrid)
        {
            view.scratch.geo.set(GeoPoint.UNKNOWN);
            if (this.useLCS)
            {
                view.currentPass.scene.mapProjection.forward(this.centroid, this.centroidProj);
            } else
            {
            ...
            this.meshVertsDrawVersion = view.currentPass.drawSrid;
        }

        GLES20FixedPipeline.glPushMatrix();

        setLCS(view, (view.currentPass.drawLng * centroid.getLongitude()) >= 0 || view.currentPass.drawSrid != 4326);
