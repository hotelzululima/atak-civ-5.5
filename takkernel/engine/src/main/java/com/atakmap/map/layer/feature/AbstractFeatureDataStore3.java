...
public abstract class AbstractFeatureDataStore3 implements FeatureDataStore4
{
    ...
    /**
     * Implementations that support Traits will need to implement this method.
     */
    protected Pair<Long, Long> insertFeatureImpl(long fsid,
                                                          long fid,
                                                          String name,
                                                          ...
                                                          long version) throws DataStoreException
    {
        return insertFeatureImpl(fsid, fid, name, geomCoding, rawGeom, styleCoding, rawStyle, attributes, traits.altitudeMode, traits.extrude, timestamp, version);
    }
    ...
    /**
     * Must be overridden if a datastore supports Traits.
     */
    protected void updateFeatureImpl(long fid,
                                              int updatePropertyMask,
                                              String name,
                                              ...
                                              AttributeSet attributes,
                                              int attrUpdateType,
                                              Feature.Traits traits) throws DataStoreException
    {
        this.updateFeatureImpl(fid, updatePropertyMask, name, geometry, style, attributes, traits.altitudeMode, traits.extrude, attrUpdateType);
    }
                                    ...
                                    Style style, AttributeSet attributes, Feature.AltitudeMode altitudeMode, double extrude, int attrUpdateType) throws DataStoreException
    {
        ...
        this.updateFeature(fid, updatePropertyMask, name, geometry, style, attributes, attrUpdateType, traits);
    }

    @Override
    public final void updateFeature(long fid, int updatePropertyMask, String name, Geometry geometry,
                                    Style style, AttributeSet attributes, int attrUpdateType, Feature.Traits traits) throws DataStoreException
    {
    ...
    public void acquireModifyLock(boolean bulk) throws InterruptedException
    {
        synchronized (this.modifyLockSync)
        {
            ...
            if (this.modifyLockHolder == currentThread)
            {
                ...
                onAcquireModifyLock(bulkModification, modifyLocks);
                return;
            }
    ...
    public void releaseModifyLock()
    {
        synchronized (this.modifyLockSync)
        {
            ...
            {
                onReleaseModifyLock(batchedModifications, modifyLocks);
                return;
            }
    ...
    {
        if (def instanceof FeatureDefinition4)
            return ((FeatureDefinition4) def).getTraits().altitudeMode;
        else if (def instanceof FeatureDefinition3)
    ...
    {
        if (def instanceof FeatureDefinition4)
            return ((FeatureDefinition4) def).getTraits().extrude;
        else if (def instanceof FeatureDefinition3)
     ...
     * copied since this FDB2 will be transitioned to native in the near future.
     */
    final protected static Feature.Traits getTraits(FeatureDefinition def)
    {
        if (def instanceof FeatureDefinition4)
            return ((FeatureDefinition4) def).getTraits();
        else {
            Feature.Traits traits = new Feature.Traits();
            traits.altitudeMode = getAltitudeMode(def);
            traits.extrude = getExtrude(def);
            traits.lineMode = getLineMode(def);
            return traits;
        }
    }
     ...
     * copied since this FDB2 will be transitioned to native in the near future.
     */
    final protected static Feature.LineMode getLineMode(FeatureDefinition def)
    {
        if (def instanceof FeatureDefinition4)
            return ((FeatureDefinition4) def).getTraits().lineMode;
        else
