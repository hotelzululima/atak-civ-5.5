...
public final class TileScraper
{
    ...
    private static class ScrapeContext
    {
        ...
        public ScrapeContext(TileMatrix client, TileContainer container, CacheRequest request)
        {
            ...
            // no zoom levels exist between min<>max; find closest
            if (lvlArray.size() == 0) {
                if(request.minResolution < this.zooms[this.zooms.length-1].resolution)
                    lvlArray.append(this.zooms[this.zooms.length - 1].level, true);
                else if(request.maxResolution > this.zooms[0].resolution)
                    lvlArray.append(this.zooms[0].level, true);
                else
                    lvlArray.append(TileGrid.findZoomLevel(this.zooms, request.maxResolution, 0), true);
            }
    ...
    private class MultiThreadDownloader extends Downloader
    {
        ...
        protected void onDownloadExit(ScrapeContext context, int jobStatus)
        {
            ...
            try
            {
                downloadService.awaitTermination(10, TimeUnit.SECONDS);
