...
public final class OSMDroidTileContainer implements TileContainer, Controls, TilesMetadataControl
{
    ...
    public final static TileContainerSpi SPI = new TileContainerSpi()
    {
        ...
        public TileContainer create(String name, String path, TileMatrix spec)
        {
            ...
            do {
                ...
                final TilesMetadataControl ctrl = ctrls.getControl(TilesMetadataControl.class);
                if(ctrl == null)
                    break;
            ...
            try
            {
                ...
                final OSMDroidTileContainer retval = new OSMDroidTileContainer(f, name, spec.getSRID(), db, true);
                if(metadata != null)
                    retval.initControl.setMetadata(metadata);
                db = null;
                return retval;
            } finally
            {
        ...
        {
            File f = new File(path);
            if (!IOProviderFactory.exists(f) || !Databases.isSQLiteDatabase(path))
                return null;
            ...
            try
            {
                ...
                final TileContainer retval = new OSMDroidTileContainer(f, info.provider, info.srid, db, false);
                db = null;
                return retval;
            } finally
            {
    ...
    private static final String TAG = "OSMDroidTileContainer";
    ...
    private OSMDroidTileContainer(File file, String name, int srid, DatabaseIface db, boolean initContainer)
    {
        this.file = file;
        this.name = name;
        switch (srid)
                    ...
                    try {
                        ...
                        for (Map.Entry<String, Object> entry : metadata.entrySet()) {
                            ...
                            if (v == null) {
                                ...
                                stmt.bind(2, ((Integer) v).intValue());
                            } else if (v instanceof Long) {
                                stmt.bind(2, ((Long) v).longValue());
                            } else if (v instanceof Double) {
                                stmt.bind(2, ((Double) v).doubleValue());
                            } else if (v instanceof byte[]) {
                                stmt.bind(2, (byte[]) v);
                            } else {
                                Log.w(TAG, "Skipping container initialization metadata [" + entry.getKey() + "]; unsupported value type: " + v.getClass());
                                continue;
                            }
                            stmt.bind(1, entry.getKey());
                            stmt.execute();

                            OSMDroidTileContainer.this.metadata.put(entry.getKey(), entry.getValue());
                        }
                    } finally {
                        if (stmt != null)
                            stmt.close();
        ...
        } else if(hasAtakMetadata) {
            this.metadata = new HashMap<>();
            try(QueryIface query = this.db.compileQuery("SELECT key, value FROM ATAK_metadata")) {
                while(query.moveToNext()) {
                    ...
                    if(value != null)
                        this.metadata.put(query.getString(0), value);
                }
            }
        }
    }
    ...
    public static boolean isCompatible(TileMatrix spec)
    {
        ...
        final ZoomLevel specMaxZoom = zoomLevels[Math.min(specLevels.length-1, limit)];
        if (Math.abs(spec.getOriginX()-bnds.minX) > specMaxZoom.pixelSizeX || Math.abs(spec.getOriginY()-bnds.maxY) > specMaxZoom.pixelSizeY)
            return false;
        ...
        if(Objects.equals(contentType, "vector"))
            return false;
    ...
    public static TileContainer openOrCreate(String path, String provider, int srid)
    {
        ...
        try
        {
            ...
            final TileContainer retval = new OSMDroidTileContainer(f, provider, srid, db, true);
            db = null;
            return retval;
        } finally
        {
