...
public final class DefaultGeopackageFeatureCursor extends RowIteratorWrapper implements GeoPackageFeatureCursor
{
    ...
    /** @deprecated use {@link  #query(GeoPackage, String, String, String, int, boolean, FeatureDataStore2.FeatureQueryParameters)} */
    @Deprecated
    @DeprecatedApi(since = "5.3", forRemoval = true, removeAt = "5.6")
    public static GeoPackageFeatureCursor query(GeoPackage gpkg, String tableName, String nameColumn, String geometryColumn, int geometrySrid, boolean autoAttributes, FeatureDataStore.FeatureQueryParameters params)
    {
        return query(gpkg, tableName, nameColumn, geometryColumn, geometrySrid, null, autoAttributes, Adapters.adapt(params, null));
    }

    public static GeoPackageFeatureCursor query(GeoPackage gpkg, String tableName, String nameColumn, String geometryColumn, int geometrySrid, boolean autoAttributes, FeatureDataStore2.FeatureQueryParameters params)
    {
        return query(gpkg, tableName, nameColumn, geometryColumn, geometrySrid, null, autoAttributes, params);
    }

    /** @deprecated use {@link  #query(GeoPackage, String, String, String, int, String, boolean, FeatureDataStore2.FeatureQueryParameters)} */
    @Deprecated
    @DeprecatedApi(since = "5.3", forRemoval = true, removeAt = "5.6")
    public static GeoPackageFeatureCursor query(GeoPackage gpkg, String tableName, String nameColumn, String geometryColumn, int geometrySrid, String rtreeIndex, boolean autoAttributes, FeatureDataStore.FeatureQueryParameters params)
    {
        return query(gpkg, tableName, nameColumn, geometryColumn, geometrySrid, rtreeIndex, autoAttributes, Adapters.adapt(params, null));
    }

    public static GeoPackageFeatureCursor query(GeoPackage gpkg, String tableName, String nameColumn, String geometryColumn, int geometrySrid, String rtreeIndex, boolean autoAttributes, FeatureDataStore2.FeatureQueryParameters params)
    {
        Set<String> attributeColumns = null;
        if (autoAttributes && ((params == null) || !MathUtils.hasBits(params.ignoredFeatureProperties, FeatureDataStore2.PROPERTY_FEATURE_ATTRIBUTES)))
        {
    ...
    /** @deprecated use {@link  #query(GeoPackage, String, String, String, int, String, Set, FeatureDataStore2.FeatureQueryParameters)} */
    @Deprecated
    @DeprecatedApi(since = "5.3", forRemoval = true, removeAt = "5.6")
    public static GeoPackageFeatureCursor query(GeoPackage gpkg, String tableName, String nameColumn, String geometryColumn, int geometrySrid, String rtreeTableName, Set<String> attributeColumns, FeatureDataStore.FeatureQueryParameters params)
    {
        return query(gpkg, tableName, nameColumn, geometryColumn, geometrySrid, rtreeTableName, attributeColumns, Adapters.adapt(params, null));
    }

    public static GeoPackageFeatureCursor query(GeoPackage gpkg, String tableName, String nameColumn, String geometryColumn, int geometrySrid, String rtreeTableName, Set<String> attributeColumns, FeatureDataStore2.FeatureQueryParameters params)
    {
        if (params == null)
        ...
        args.add(params.featureSetFilter == null || Double.isNaN(params.featureSetFilter.minResolution) ? new BindArgument() : new BindArgument(params.featureSetFilter.minResolution));
        args.add(params.featureSetFilter == null || Double.isNaN(params.featureSetFilter.maxResolution) ? new BindArgument() : new BindArgument(params.featureSetFilter.maxResolution));
        ...
        // name
        if (nameColumn != null && !MathUtils.hasBits(params.ignoredFeatureProperties, FeatureDataStore2.PROPERTY_FEATURE_NAME))
        {
            sql.append(", ");
            sql.append(tableName);
            sql.append(".");
        ...
        // geometry
        if (!MathUtils.hasBits(params.ignoredFeatureProperties, FeatureDataStore2.PROPERTY_FEATURE_GEOMETRY))
        {
            ...
            if (params.spatialOps != null && layerProjection != null)
            {
                for (FeatureDataStore2.FeatureQueryParameters.SpatialOp op : params.spatialOps)
                {
                    if (op instanceof FeatureDataStore2.FeatureQueryParameters.SpatialOp.Simplify)
                    {
                        geom.insert(0, "SimplifyPreserveTopology(");
                        geom.append(", ?)");

                        args.add(new BindArgument(getProjectionUnitsDistance(layerProjection, ((FeatureDataStore2.FeatureQueryParameters.SpatialOp.Simplify) op).distance)));
                    } else if (op instanceof FeatureDataStore2.FeatureQueryParameters.SpatialOp.Buffer)
                    {
                        geom.insert(0, "Buffer(");
                        geom.append(", ?)");

                        args.add(new BindArgument(getProjectionUnitsDistance(layerProjection, ((FeatureDataStore2.FeatureQueryParameters.SpatialOp.Buffer) op).distance)));
                    }
                }
            }
        ...
        // attributes
        if (!MathUtils.hasBits(params.ignoredFeatureProperties, FeatureDataStore2.PROPERTY_FEATURE_ATTRIBUTES))
        {
            int columnsUsed = 0;
        ...
        if (params.ids != null)
        {
            ...
            where.appendIn(tableName + ".ROWID", params.ids.size());
            for (Long fid : params.ids)
                where.addArg(fid.longValue());
        }
        if (nameColumn != null && params.names != null)
        {
            where.beginCondition();
            where.appendIn(tableName + "." + nameColumn, params.names);
        }
        ...
        {
            Envelope region = params.spatialFilter.getEnvelope();
            ...
            if (layerProjection != null)
            {
                if (rtreeTableName != null)
                {
                    ...
                    PointD ul = layerProjection.forward(new GeoPoint(region.maxY, region.minX), null);
                    PointD lr = layerProjection.forward(new GeoPoint(region.minY, region.maxX), null);
