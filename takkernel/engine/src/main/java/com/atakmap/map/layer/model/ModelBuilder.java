...
public final class ModelBuilder
{
    ...
    public static Model build(Mesh[] meshes, Matrix[] transforms)
    {
        ...
        for (int i = 0; i < meshes.length; i++)
        {
            ...
            if (meshAabb.minX < dstAabb.minX)
                dstAabb.minX = meshAabb.minX;
            if (meshAabb.minY < dstAabb.minY)
                dstAabb.minY = meshAabb.minY;
            if (meshAabb.minZ < dstAabb.minZ)
                dstAabb.minZ = meshAabb.minZ;
            if (meshAabb.maxX > dstAabb.maxX)
                dstAabb.maxX = meshAabb.maxX;
            if (meshAabb.maxY > dstAabb.maxY)
                dstAabb.maxY = meshAabb.maxY;
            if (meshAabb.maxZ > dstAabb.maxZ)
                dstAabb.maxZ = meshAabb.maxZ;
        }
