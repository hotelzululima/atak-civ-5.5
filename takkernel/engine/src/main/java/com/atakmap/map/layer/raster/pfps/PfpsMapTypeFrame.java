...
public class PfpsMapTypeFrame
{
    ...
    public static String getRpfPrettyName(File f)
    {
        ...
        if (len == RPF_FILENAME_LENGTH)
            return getRpfPrettyName(name.substring(9, 11).toUpperCase(LocaleUtil.getCurrent()));
        else if (len == ECRG_FILENAME_LENGTH)
            return getRpfPrettyName(name.substring(15, 17).toUpperCase(LocaleUtil.getCurrent()));
        else
            return null;
    }
