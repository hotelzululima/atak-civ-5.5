...
public class GLBatchLineString extends GLBatchGeometry
{
    ...
    boolean projectVertices(GLMapView view, int vertices)
    {
        ...
        switch (vertices)
        {
            case GLGeometry.VERTICES_PIXEL:
                if (this.verticesDrawVersion != view.currentPass.drawVersion
                        || this.vertexType != vertices
                        || !terrainValid)
                {
                ...
                this.verticesDrawVersion = view.currentPass.drawVersion;
            ...
            case GLGeometry.VERTICES_BATCH:
                if (view.currentPass.drawSrid != this.projectedVerticesSrid
                        || this.vertexType != vertices
                        || !terrainValid)
                {
                    ...
                    this.projectedVerticesSrid = view.currentPass.drawSrid;
                    retval = true;
                }
                break;
            default:
    ...
    {
        if (ortho.currentPass.drawTilt > 0d || ortho.currentPass.scene.camera.perspective)
        {
                           ...
                           int vertices, int size, FloatBuffer v, RenderState rs)
    {
        ...
        if(_hasInnerRings) {
            FloatBuffer vertsView = v.duplicate();

            for(int i=0;i < _startIndices.length;++i)
            {
                int position = _startIndices[i] * size;
                int count = _numVertices[i];

                // create a duplicate view of the vertex buffer over the current ring
                vertsView.position(position);
                vertsView.limit(position+count*size);
                // batch the ring
                batch.batch(-1,
    ...
    {
        if (getAltitudeMode() == AltitudeMode.ClampToGround)
            return SystemUtils.isOsAndroid() || !hasFill() ? surfaceHitTest(params) : surfaceInteriorHitTest(params);
        else
            return screenHitTest(params);
    }

    /**
     * Hit test against the interior of the rendered surface
     *
     * @param params Hit test parameters
     * @return Result if hit, null otherwise
     */
    private HitTestResult surfaceInteriorHitTest(HitTestQueryParameters params)
    {
        if (!_hitBounds.intersects(params.bounds))
            return null;

        int vertexCount = numRenderPoints;
        Vector2D[] vectors = new Vector2D[vertexCount];
        for (int a = 0; a < vertexCount; a++){
            GeoPoint renderPoint = getRenderPoint(a);
            vectors[a] = new Vector2D(renderPoint.getLongitude(), renderPoint.getLatitude());
        }

        boolean contains = Vector2D.polygonContainsPoint(new Vector2D(params.geo.getLongitude(), params.geo.getLatitude()), vectors);
        return contains ? new HitTestResult(featureId, params.geo) : null;
    }
