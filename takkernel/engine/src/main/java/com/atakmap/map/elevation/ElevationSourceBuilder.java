...
public final class ElevationSourceBuilder
{
    ...
    @DontObfuscate
    public static ElevationSource multiplex(String name, ElevationSource[] sources)
    {
        return NativeElevationSource.multiplexSources(name, sources);
    }

    @DontObfuscate
    public static ElevationSource cascade(String name, ElevationSource[] sources)
    {
        return NativeElevationSource.cascadeSources(name, sources);
    }

    @DontObfuscate
    public static ElevationSource constrainResolution(ElevationSource impl, double minResolution, double maxResolution)
    {
        return NativeElevationSource.constrainResolutionSource(impl, minResolution, maxResolution);
    }

    /**
     * Utility to generate a <I>Continuous Level of Detail (CLOD)</I> representation from an existing {@link ElevationSource} that presents a single level of detail per coverage.
     */
    public final static class CLOD {
        private ElevationSource _impl;
        private ElevationManager.HeightmapStrategy _strategy = ElevationManager.HeightmapStrategy.LowFillHoles;
        private int _srid = -1;
        private boolean _pointSampler = false;
        private File _cacheFile = null;
        private int _cacheLimit = 0;
        private boolean _cacheAsync;
        private ElevationManager.HeightmapParams _params = null;

        public CLOD(ElevationSource impl) {
            _impl = impl;
        }

        public CLOD setPointSampler(boolean forcePointSample) {
            _pointSampler = forcePointSample;
            return this;
        }
        public CLOD setStrategy(@NonNull ElevationManager.HeightmapStrategy strategy) {
            _strategy = strategy;
            return this;
        }
        public CLOD enableCaching(File cacheFile, int cacheLimit, boolean async) {
            _cacheFile = cacheFile;
            _cacheLimit = cacheLimit;
            _cacheAsync = async;
            return this;
        }
        public CLOD optimizeForTileGrid(int srid, @NonNull ElevationManager.HeightmapParams params) {
            _srid = srid;
            _params = params;
            return this;
        }

        public ElevationSource build() {
            return NativeElevationSource.tileSource(
                    _impl,
                    _pointSampler,
                    _srid,
                    _strategy,
                    _params, 1, (_cacheFile != null) ? _cacheFile.getAbsolutePath() : null,
                    _cacheLimit,
                    _cacheAsync);
        }
    }
