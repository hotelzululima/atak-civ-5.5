...
public final class Imagery
{
    ...
    {
        return createTiledImageryLayer(name, url, minZoom, maxZoom, 3857, cacheDir);
    }

    public static Layer createTiledImageryLayer(String name, String url, int minZoom, int maxZoom, int srid, File cacheDir) throws IOException
    {
        if (url == null)
            return null;
        url = url.replace("&", "&amp;");
        ...
        sb.append("    <coordinatesystem>EPSG:" + srid + "</coordinatesystem>");
        sb.append("</customMapSource>");
