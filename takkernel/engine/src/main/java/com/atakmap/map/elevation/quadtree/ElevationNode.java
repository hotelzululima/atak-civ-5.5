 ...
 * @deprecated use {@link com.atakmap.map.opengl.TerrainRenderService} API
 */
...
public class ElevationNode implements Comparable<ElevationNode>
{
     ...
     * @param minLatitude  The minimum latitude contained in the node
     * @param maxLatitude  The maximum latitude contained in the node
     * @param minLongitude The minimum longitude contained in the node
     * @param maxLongitude The maximum longitude contained in the node
     */
     ...
     *
     * @param point The point to see if it is contained in this node
     ...
     *
     * @param latitude  The latitude that needs to be contained for the node
     * @param longitude The longitude that needs to be contained by the node
     * @param maxLevel  The maximum level to go to
     ...
     *
     * @param latitude  The latitude of the child node to get
     * @param longitude The longitude of the child node to get
     ...
     *
     * @param latitude    The longitude of the point to get the elevation for
     * @param longitude   The longitude of the point to get the elevation for
     * @param interpolate true if interpolation should be preformed to improve the results, but
     ...
     *
     * @param lastRequestTime    The last time the node was requested in milliseconds since the epoch
     * @param lastTerrainVersion The last terrain version the node was requested for
     */
     ...
     * from the tree
     *
     * @param pruneTime The time where only nodes updated after this time should be kept
    ...
    public boolean equals(Object o)
    {
        ...
        if (level != that.level)
            return false;
        if (Double.compare(that.minLatitude, minLatitude) != 0)
            return false;
        if (Double.compare(that.maxLatitude, maxLatitude) != 0)
            return false;
        if (Double.compare(that.minLongitude, minLongitude) != 0)
            return false;
        if (Double.compare(that.maxLongitude, maxLongitude) != 0)
            return false;
        if (resolved != that.resolved)
            return false;
        if (lastRequestTime != that.lastRequestTime)
            return false;
        if (lastTerrainVersion != that.lastTerrainVersion)
            return false;
