...
import com.atakmap.map.layer.raster.tilereader.opengl.NodeInitializer;
import com.atakmap.map.layer.raster.tilereader.opengl.NodeOptions;
...
public class GLMosaicMapLayer2 extends
        GLAsynchronousMapRenderable2<MosaicPendingData> implements GLMapLayer3
{
    ...
    public final static GLMapLayerSpi3 SPI = new GLMapLayerSpi3()
    {
        ...
        public GLMapLayer3 create(Pair<MapRenderer, DatasetDescriptor> arg)
        {
            ...
            if (dbFile == null || !IOProviderFactory.exists(dbFile) || (IOProviderFactory.isFile(dbFile) && IOProviderFactory.length(dbFile) < 1))
            {
            ...
            return new GLMosaicMapLayer2(surface, mosaic);
        }
    ...
    private final static Comparator<Frame> FRAME_STORAGE_COMPARATOR = new Comparator<Frame>()
    {
        @Override
     ...
     */
    private Collection<GLMapRenderable2> resurrectedFrames;
    ;

    private Collection<GLMapRenderable2> renderList;
    ...
    private ImageryRelativeScaleControl scaleControl;
    private double lastRelativeScale = 1f;

    private ImagerySelectionControl.Mode resolutionSelectMode;
    private Set<String> imageryTypeFilter;
    private String selectedType;

    public GLMosaicMapLayer2(MapRenderer surface, MosaicDatasetDescriptor info)
    {
        this.surface = surface;
        this.info = info;

        this.relativePaths = DatasetDescriptor.getExtraData(info, "relativePaths", "true").equals("true");
        this.visibleFrames = new ConcurrentSkipListMap<>(FRAME_STORAGE_COMPARATOR);
        this.zombieFrames = new TreeMap<>(FRAME_STORAGE_COMPARATOR);

        this.renderList = new MultiCollection<>(Arrays.asList(this.zombieFrames.values(), this.visibleFrames.values()));
        ...
        this.controls.add(new LockOnTypeControl());

        if(surface instanceof MapRenderer3) {
            this.scaleControl = ((MapRenderer3)surface).getControl(ImageryRelativeScaleControl.class);
        } else {
            surface.visitControl(null, new Visitor<ImageryRelativeScaleControl>() {
                @Override
                public void visit(ImageryRelativeScaleControl object) {
                    scaleControl = object;
                }
    ...
    public synchronized void draw(GLMapView view, int renderPass)
    {
        ...
        if (!this.zombieFrames.isEmpty())
        {
            ...
            for (Map.Entry<Frame, GLMapRenderable2> entry : visibleFrames.entrySet())
            {
                // check if resolved
                final GLMapRenderable2 r = entry.getValue();
                if(r instanceof GLResolvable)
                    allResolved &= (((GLResolvable)r).getState() == State.RESOLVED);
            }

            Iterator<Map.Entry<Frame, GLMapRenderable2>> zombieIter = this.zombieFrames.entrySet().iterator();
            while (zombieIter.hasNext())
            {
                final Map.Entry<Frame, GLMapRenderable2> entry = zombieIter.next();
                final Frame frame = entry.getKey();
                final GLMapRenderable2 resolvable = entry.getValue();
                if (allResolved ||
                        !intersects(this.preparedState, frame))
                {
                    ...
                    zombieIter.remove();
                    resolvable.release();
                    if(resolvable instanceof Disposable)
                        ((Disposable)resolvable).dispose();
                } else if (resolvable instanceof GLResolvable && ((GLResolvable)resolvable).getState() == State.RESOLVING)
                {
                    ((GLResolvable)resolvable).suspend();
                }
            }
        }
        ...
        // resume
        for (GLMapRenderable2 resurrected : this.resurrectedFrames) {
            if(resurrected instanceof GLResolvable)
                ((GLResolvable)resurrected).resume();
        }
        ...
        if(this.scaleControl != null) {
            final float relativeScale = this.scaleControl.getRelativeScale();
            if(this.lastRelativeScale != relativeScale) {
                this.lastRelativeScale = relativeScale;
                this.invalid = true;
            }
        }
    ...
    public int getRenderPass() {
        return GLMapView.RENDER_PASS_SURFACE|GLMapView.RENDER_PASS_SURFACE_PREFETCH;
    }
    ...
    protected void resetPendingData(MosaicPendingData pendingData)
    {
        ...
        for (Frame frame : this.visibleFrames.keySet())
    ...
    private synchronized boolean checkQueryInterrupt(ViewState queryState)
    {
        return !Rectangle.intersects(
                targetState.westBound, targetState.southBound, targetState.eastBound, targetState.northBound,
                queryState.westBound, queryState.southBound, queryState.eastBound, queryState.northBound);
    }

    @Override
    protected boolean updateRenderList(ViewState state, MosaicPendingData pendingData)
    {
        ...
        Map<Frame, GLMapRenderable2> toRelease = this.visibleFrames;
        final String st = this.selectedType;
        if(st != null) {
            // compare biased on _selected type_, then based on GSD and type
            this.visibleFrames = new ConcurrentSkipListMap<>( new Comparator<Frame>() {
                public int compare(Frame a, Frame b) {
                    final boolean aSelected = a.type.equals(st);
                    final boolean bSelected = b.type.equals(st);
                    return (aSelected == bSelected) ?
                            FRAME_STORAGE_COMPARATOR.compare(a, b) :
                            (aSelected ? 1 : -1);
                }
            });
        } else {
            // compare strictly based on GSD and type
            this.visibleFrames = new ConcurrentSkipListMap<>(FRAME_STORAGE_COMPARATOR);
        }
        this.visibleFrames.putAll(toRelease);
        this.renderList = new MultiCollection<>(Arrays.asList(this.zombieFrames.values(), this.visibleFrames.values()));

        // move any previously visible nodes into our nowVisible map
        for (Frame frame : pendingData.frames)
        {
            // visible, do not release
            toRelease.remove(frame);

            GLMapRenderable2 renderable = this.visibleFrames.get(frame);
            if (renderable != null) {
        ...
        Iterator<Map.Entry<Frame, GLMapRenderable2>> releaseIter = toRelease
                .entrySet().iterator();
        while (releaseIter.hasNext())
        {
            final Map.Entry<Frame, GLMapRenderable2> entry = releaseIter.next();
            final Frame frame = entry.getKey();
            final GLMapRenderable2 mapRenderable = entry.getValue();
        ...
        final Collection<GLMapRenderable2> releaseList = toRelease.values();
        for (Frame frame : toRelease.keySet())
        ...
        this.surface.queueEvent(new Runnable()
        {
            ...
            public void run()
            {
                for (GLMapRenderable2 renderable : releaseList)
                {
                    renderable.release();
    ...
    private List<MosaicDatabase2.QueryParameters> constructQueryParams(ViewState localQuery)
    {
        ...
        final double drawMapResolution = localQuery.drawMapResolution *
                ((this.scaleControl != null) ?
                    this.scaleControl.getRelativeScale() : 1f);

        List<MosaicDatabase2.QueryParameters> retval = new LinkedList<>();
        ...
        switch (resSelectMode)
        {
            ...
            case MinimumResolution:
                ...
                if(st != null) {
                    ...
                    if(asc.types == null || asc.types.contains(st))
                        asc.types = Collections.singleton(st);
                    else
                        asc.types = Collections.emptySet();
                    asc.maxGsd = drawMapResolution;
                    ...
                    if(desc.types == null || desc.types.contains(st))
                        desc.types = Collections.singleton(st);
                    else
                        desc.types = Collections.emptySet();
                    desc.minGsd = drawMapResolution;
                    desc.minGsdCompare = MosaicDatabase2.QueryParameters.GsdCompare.MaximumGsd;
                    desc.maxGsd = drawMapResolution / 8d;
                    // XXX - typo, but has been an implementation detail for so long should not be
                    //       modified without thorough analysis of behavior
                    asc.maxGsdCompare = MosaicDatabase2.QueryParameters.GsdCompare.MinimumGsd;
                    desc.order = MosaicDatabase2.QueryParameters.Order.MaxGsdAsc;
                    retval.add(desc);
                }
                if(st == null || ConfigOptions.getOption("mosaic-lock-v2", 1) != 0) {
                    // query all other imagery as background when locked
                    if(base.types != null && st != null) {
                        base.types = new HashSet<>(base.types);
                        base.types.remove(st);
                    }
                    base.maxGsd = drawMapResolution;
                    base.maxGsdCompare = MosaicDatabase2.QueryParameters.GsdCompare.MaximumGsd;
                    retval.add(base);
    ...
    private void queryImpl(ViewState localQuery, MosaicPendingData retval)
    {
            ...
            for (MosaicDatabase2.QueryParameters p : params)
            {
                try
                {
                    ...
                    while (result.moveToNext())
                    {
                        if (this.checkQueryThreadAbort())
                            break outer;
                        if (this.checkQueryInterrupt(localQuery))
                        {
                            break outer;
                        }
        ...
        //Log.d(TAG, "Queried " + retval.frames.size() + " in " + (es-ss) + "ms");

        //long ts = System.currentTimeMillis();
        int iii = retval.waitForPreload();
        //long te = System.currentTimeMillis();
        //Log.d(TAG, "Preload " + retval.frames.size() + " in " + (te-ts) + "ms [" + retval.preloadsWaiting + "/" + iii + "]");
    }
    ...
    GLMapRenderable2 createRootNode(Frame frame)
    {
        ...
        final NodeOptions glopts = new NodeOptions();
        glopts.levelTransitionAdjustment = 0.5d;
        // XXX - avoid cast
        if (GLMosaicMapLayer2.this.textureCacheEnabled)
            glopts.textureCache = GLRenderGlobals.get(GLMosaicMapLayer2.this.surface).getTextureCache();

        try
        {
            return new GLQuadTileNode4(LegacyAdapters.getRenderContext(this.surface), frame, opts, glopts, this.nodeInitializer);
        } catch (Throwable t)
        {
            return null;
        }
    ...
    private NodeInitializer nodeInitializer = new NodeInitializer()
    {
        @Override
        public Result init(ImageInfo info, TileReaderFactory.Options opts)
        {
            Frame frame = (Frame) info;
            ...
            if (frame.type.equals("kmz") && frame.path.endsWith(".jpg"))
            {
                ...
                {
                    String kmzPath;
                    if (GLMosaicMapLayer2.this.relativePaths)
                        kmzPath = Uri.parse(GLMosaicMapLayer2.this.baseUri + "/" + frame.path).getPath();
                    else
            ...
            // try to create a TileReader for the frame
            retval.reader = null;
            do
            {
            ...
            {
                dataset = GdalLibrary.openDatasetFromPath(GLMosaicMapLayer2.this.resolvePath(frame.path));
                if (dataset == null)
                {
            ...
            if (frame.precisionImagery)
            {
                try
                {
                    PrecisionImagery p = PrecisionImageryFactory.create(GLMosaicMapLayer2.this.resolvePath(frame.path));
                    if (p == null)
     ...
     * @return
     */
    final String resolvePath(String path)
    {
    ...
    private static boolean intersects(ViewState state, Frame frame)
    {
    ...
    private static DatasetProjection2 getDatasetProjection(Frame frame, Dataset dataset)
    {
    ...
    private class RasterDataAccessControlImpl implements RasterDataAccessControl
    {
        ...
        {
            synchronized (GLMosaicMapLayer2.this)
            {
                final double latitude = p.getLatitude();
                final double longitude = p.getLongitude();

                final NavigableMap<Frame, GLMapRenderable2> reverse = GLMosaicMapLayer2.this.visibleFrames.descendingMap();
                Frame frame;
                GLMapRenderable2 r;
                RasterDataAccess2 data;
                PointD img = new PointD(0d, 0d);
                for (Map.Entry<Frame, GLMapRenderable2> entry : reverse.entrySet())
                {
                    frame = entry.getKey();
    ...
    private class ColorControlImpl implements ColorControl, MosaicFrameColorControl
    {
        ...
        private Map<Filter<Frame>, Integer> filters;
        ...
        public ColorControlImpl()
        {
            ...
            this.filters = new IdentityHashMap<Filter<Frame>, Integer>();
        }
        ...
        void apply()
        {
            if (!this.dirty)
                return;
            for (Map.Entry<Frame, GLMapRenderable2> entry : GLMosaicMapLayer2.this.visibleFrames.entrySet())
         ...
         * must hold lock on <code>GLMosaicMapLayer.this</code>
         */
        void apply(Frame frame, GLMapRenderable2 r)
        {
            ...
            if (r instanceof GLQuadTileNode4)
            {
                ((GLQuadTileNode4) r).setColor(color);
            } else if (r instanceof Controls)
            {
        ...
        @Override
        public void addFilter(Filter<Frame> filter, int color)
        {
            synchronized (GLMosaicMapLayer2.this)
            {
        ...
        @Override
        public void removeFilter(Filter<Frame> filter)
        {
            synchronized (GLMosaicMapLayer2.this)
            {
        ...
        {
            for (Map.Entry<Filter<Frame>, Integer> entry : this.filters.entrySet())
            {
    ...
    final class SelectControlImpl implements ImagerySelectionControl
    {
        ...
        public void setResolutionSelectMode(Mode mode)
        {
            if (mode == null)
                throw new NullPointerException();
            GLMosaicMapLayer2.this.resolutionSelectMode = mode;
            GLMosaicMapLayer2.this.invalidateNoSync();
        }

        @Override
        public Mode getResolutionSelectMode()
        {
            return GLMosaicMapLayer2.this.resolutionSelectMode;
        }
        ...
        public void setFilter(Set<String> filter)
        {
            ...
            GLMosaicMapLayer2.this.imageryTypeFilter = filter;
            GLMosaicMapLayer2.this.invalidateNoSync();
        }
    }

    final class LockOnTypeControl implements MapControl, RasterLayer2.OnSelectionChangedListener {

        @Override
        public void onSelectionChanged(RasterLayer2 layer) {
            GLMosaicMapLayer2.this.selectedType = layer.isAutoSelect() ? null : layer.getSelection();
        }
    }
}
