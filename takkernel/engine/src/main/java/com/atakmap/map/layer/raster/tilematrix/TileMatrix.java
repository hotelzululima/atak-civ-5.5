...
public interface TileMatrix extends Disposable
{
    ...
    public final class ZoomLevel
    {
        ...
        public final static class Builder {
            ...
            public Builder() {
                this.zoom = new TileMatrix.ZoomLevel();
            }
            ...
            public Builder setTileSize(int width, int height) {
                zoom.tileWidth = width;
                zoom.tileHeight = height;
                return this;
            }
            public Builder setTileSize(int size) {
                return setTileSize(size, size);
            }
            public Builder setPixelSize(double x, double y) {
                zoom.pixelSizeX = x;
                zoom.pixelSizeY = y;
                return this;
            }
            public Builder setPixelSize(double size) {
                return setPixelSize(size, size);
            }
            public Builder setResolution(double resolution) {
                zoom.resolution = resolution;
                return this;
            }
            public TileMatrix.ZoomLevel build() {
                return zoom;
            }
        }
    }
    ...
    public final static class Util
    {
        ...
        public static Envelope getTileBounds(ZoomLevel zoom, double originX, double originY, int tileX, int tileY)
        {
            ...
            point = getTilePoint(originX, originY, zoom, tileX, tileY, 0, zoom.tileHeight);
            retval.minX = point.x;
            retval.minY = point.y;
            point = getTilePoint(originX, originY, zoom, tileX, tileY, zoom.tileWidth, 0);
            retval.maxX = point.x;
            retval.maxY = point.y;
        ...
        {
            return getTilePoint(matrix.getOriginX(), matrix.getOriginY(), findLevelOrThrow(matrix, level), tileX, tileY, pixelX, pixelY);
        }

        public static PointD getTilePoint(double originX, double originY, ZoomLevel zoom, int tileX, int tileY, int pixelX, int pixelY)
        {
        ...
        public static boolean isQuadtreeable(TileMatrix spec)
        {
            // check compatibility of tiling
            TileMatrix.ZoomLevel[] specLevels = spec.getZoomLevel();
            TileMatrix.ZoomLevel prevLevel = specLevels[0];
            for (int i = 1; i < specLevels.length; i++)
            {
                // NOTE: resolution is only 'informative' so we aren't going to check it

                // check for consecutive levels
                if (specLevels[i].level != (prevLevel.level+1))
                    return false;

                // ensure tiles for all levels share pixel width/height
                if (specLevels[i].tileWidth != prevLevel.tileWidth ||
                        specLevels[i].tileHeight != prevLevel.tileHeight)
                {
                    return false;
                }

                // ensure pixel sizes are within tolerance of one
                if(Math.abs((specLevels[i].pixelSizeX*specLevels[i].tileWidth)-(prevLevel.pixelSizeX*prevLevel.tileWidth*0.5d)) > 1)
                    return false;
                if(Math.abs((specLevels[i].pixelSizeY*specLevels[i].tileHeight)-(prevLevel.pixelSizeY*prevLevel.tileHeight*0.5d)) > 1)
                    return false;
                prevLevel = specLevels[i];
            }
            return true;
        }
