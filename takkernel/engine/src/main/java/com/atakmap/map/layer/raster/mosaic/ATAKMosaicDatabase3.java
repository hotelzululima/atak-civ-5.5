...
public class ATAKMosaicDatabase3 implements MosaicDatabase2
{
    ...
    private void open(Pair<DatabaseIface, Map<String, DatabaseIface>> opened)
    {
        if (this.indexDatabase != null)
            throw new IllegalStateException();

        this.indexDatabase = opened.first;

        this.typeDbs = new HashMap<>(opened.second);
        ...
        try
        {
            ...
            while (cursor.moveToNext())
            {
                ...
                this.coverages.put(cursor.getString(cursor.getColumnIndex(COLUMN_TYPE)), c);
            }
        } finally
        {
            if (cursor != null)
    ...
    {
        for(String t : typeDbs.keySet())
            retval.put(t, getCoverage(t));
    }
    ...
    public MosaicDatabase2.Coverage getCoverage(String type)
    {
        if (this.indexDatabase == null)
            throw new IllegalStateException();
        if(type != null && type.equals(".deferred")) {
            File dir = new File(Databases.getDatabaseFile(indexDatabase)).getParentFile();
            if(new File(dir, ".deferred.coverage").exists()) {
                return coverage;
            } else {
                // coverages are complete, nothing is deferred
                return new MosaicDatabase2.Coverage(null, Double.NaN, Double.NaN);
            }
        }

        try(CursorIface cursor = this.indexDatabase.query("SELECT * FROM " + TABLE_COVERAGE + " WHERE " + COLUMN_TYPE + " = ? LIMIT 1", new String[] {type}))
        {
    ...
    public void close()
    {
        ...
        final File dir = new File(Databases.getDatabaseFile(this.indexDatabase)).getParentFile();
        Builder.closedb(dir);
    ...
    public MosaicDatabase2.Cursor query(QueryParameters params)
    {
        ...
        if (params.spatialFilter != null)
        {
            ...
            sb.append("MbrIntersects(");
            sb.append(COLUMN_COVERAGE_BLOB);
            sb.append(", BuildMbr(");
            sb.append(roi.minX);
            sb.append(", ");
            sb.append(roi.minY);
    ...
    {
        private static Map<File, ReferenceCount<Pair<DatabaseIface, Map<String, DatabaseIface>>>> dbs = new HashMap<>();
        private static ExecutorService deferredCoverageResolverThreadPool = new ThreadPoolExecutor(
                    4, // allow all threads to exit when idled
                    4, // up to 4 concurrent resolvers
                    1000L, TimeUnit.MILLISECONDS, // threads exit after 1s idle
                    new LinkedBlockingQueue<>(),
                    new NamedThreadFactory("mosaic-db-deferred-coverage-resolver") {
                        @Override
        ...
        static {
            ((ThreadPoolExecutor)deferredCoverageResolverThreadPool).allowCoreThreadTimeOut(true);
        }

        synchronized static Pair<DatabaseIface, Map<String, DatabaseIface>> opendb(final File f)
        {
            ReferenceCount<Pair<DatabaseIface, Map<String, DatabaseIface>>> dbref = dbs.get(f);
            if(dbref == null || !dbref.isReferenced()) {
                final DatabaseIface indexDatabase = IOProviderFactory.createDatabase(new File(f, INDEX_DB_FILENAME),
                        0);
                Map<String, DatabaseIface> typeDbs = new HashMap<>();
                dbref = new ReferenceCount<Pair<DatabaseIface, Map<String, DatabaseIface>>>(Pair.create(indexDatabase, typeDbs), true) {
                    @Override
                    protected void onDereferenced() {
                        value.first.close();
                        for(DatabaseIface typedb : value.second.values())
                            typedb.close();
                        super.onDereferenced();
                    }
                };

                final ReferenceCount<File> deferredCoverageSignal = new ReferenceCount<File>(new File(f, ".deferred.coverage"), true) {
                    @Override
                    protected void onDereferenced() {
                        value.delete();
                        super.onDereferenced();
                ...
                try
                {
                    cursor = indexDatabase.query("SELECT " + COLUMN_TYPE + ", " + COLUMN_PATH + ", " + COLUMN_MIN_GSD + " FROM " + TABLE_COVERAGE, null);
                    while (cursor.moveToNext())
                    {
                        final String type = cursor.getString(0);
                        final String typedbPath = cursor.getString(1);
                        final double mingsd = cursor.getDouble(2);
                        if (typedbPath != null)
                        {
                            try
                            {
                                File databaseFile = new File(typedbPath);
                                final DatabaseIface database = IOProviderFactory.createDatabase(
                                        new DatabaseInformation(Uri.fromFile(databaseFile),
                                                DatabaseInformation.OPTION_READONLY));
                                typeDbs.put(type, database);

                                // check for deferred coverage and start resolve if needed
                                final File typedbDeferredCov = new File(f, ".deferred.coverage." + type);
                                if(typedbDeferredCov.exists()) {
                                    // compute deferred coverage for the type in background thread
                                    deferredCoverageResolverThreadPool.submit(
                                            new DeferredCoverageComputer(
                                                type,
                                                dbref,
                                                mingsd,
                                                typedbDeferredCov,
                                                deferredCoverageSignal));
                                }
                            } catch (SQLiteException e)
                            {
                                Log.e(TAG, "error: ", e);
                ...
                dbs.put(f, dbref);
                // unmark check for deferred coverage if there are no longer any deferred types
                deferredCoverageSignal.dereference();
            } else {
                dbref.reference();
            }
            return dbref.value;
        ...
        synchronized static void closedb(File f)
        {
            ReferenceCount<Pair<DatabaseIface, Map<String, DatabaseIface>>> dbref = dbs.get(f);
            if(dbref == null)
                return;
            dbref.dereference();
            if(!dbref.isReferenced())
                dbs.remove(f);
        }
        ...
        public void close()
        {
            ...
            try
            {
                ...
                if (!this.typeDbs.isEmpty())
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            for (Map.Entry<String, TypeDbSpec> entry : this.typeDbs.entrySet())
                            {
                                ...
                                typedb.coverage.blob = GeometryFactory.toSpatiaLiteBlob(
                                        GeometryFactory.polygonFromQuad(
                                                typedb.coverage.minLon, typedb.coverage.minLat,
                                                typedb.coverage.minLon, typedb.coverage.maxLat,
                                                typedb.coverage.maxLon, typedb.coverage.maxLat,
                                                typedb.coverage.maxLon, typedb.coverage.minLat),
                                        4326);

                                // _touch_ `.deferred.coverage.<type>` to indicate that coverage
                                // computation was deferred
                                File f = new File(this.dir, ".deferred.coverage." + entry.getKey());
                                try {
                                    f.createNewFile();
                                ...
                                insertStmt.bind(idx++, typedb.coverage.maxLat);
                                insertStmt.bind(idx++, typedb.coverage.maxLon);
                                insertStmt.bind(idx++, typedb.coverage.minGSD);
                                insertStmt.bind(idx++, typedb.coverage.maxGSD);
                                insertStmt.bind(idx++, typedb.path);
                    ...
                    // _touch_ `.deferred.coverage` to indicate that coverage
                    // computation was deferred
                    File f = new File(this.dir, ".deferred.coverage");
                    try {
                        f.createNewFile();
                    } catch(IOException ignored) {}
                }
        ...
        static byte[] computeCoverage(DatabaseIface typedb, double minGSD)
        {
            CursorIface result;
            ...
            try
            {
                ...
                result = typedb.query("SELECT Buffer(UnaryUnion(Collect(" + TABLE_MOSAIC_DATA + "." + COLUMN_COVERAGE_BLOB + ")), (? / 111000) * 3) FROM " + TABLE_MOSAIC_DATA, new String[] {String.valueOf(minGSD)});
                return result.moveToNext() ? result.getBlob(0) : null;
            } finally
            {
                if (result != null)
                    result.close();
        ...
        final static class DeferredCoverageComputer implements Runnable
        {
            String type;
            ReferenceCount<Pair<DatabaseIface, Map<String, DatabaseIface>>> dbref;
            double minGsd;
            File typeCovSignal;
            ReferenceCount<?> deferredCovSignal;

            DeferredCoverageComputer(String type, ReferenceCount<Pair<DatabaseIface, Map<String, DatabaseIface>>> dbref, double minGsd, File typeCovSignal, ReferenceCount<?> deferredCovSignal) {
                this.type = type;
                this.dbref = dbref;
                this.minGsd = minGsd;
                this.typeCovSignal = typeCovSignal;
                this.deferredCovSignal = deferredCovSignal;

                this.dbref.reference();
                this.deferredCovSignal.reference();
            }

            public void run() {
                try {
                    final DatabaseIface indexDatabase = dbref.value.first;
                    final DatabaseIface database = dbref.value.second.get(type);
                    byte[] cov = computeCoverage(database, minGsd);
                    StatementIface stmt = null;
                    try {
                        if (cov != null) {
                            stmt = indexDatabase.compileStatement("UPDATE " + TABLE_COVERAGE + " SET " +
                                    COLUMN_COVERAGE_BLOB + " = ? WHERE " +
                                    COLUMN_TYPE + " = ?");
                            stmt.bind(1, cov);
                            stmt.bind(2, type);
                            stmt.execute();
                        }
                        typeCovSignal.delete();
                        deferredCovSignal.dereference();
                    } catch (Exception e) {
                        Log.w(TAG, "Failed to compute coverage for type " + type);
                    } finally {
                        if (stmt != null)
                            stmt.close();
                    }
