...
public final class GeometryFactory
{
    ...
    /**
     * Returns a 2D {@link Geometry} constructed from the specified minimum
     * bounding region.
     *
     * @param mbb the minimum bounding region
     * @return the calculated geometry, or null if there was an error
     */
    public static Geometry fromEnvelope(Envelope mbb)
    {
        return fromEnvelope(mbb.minX, mbb.minY, mbb.minZ, mbb.maxX, mbb.maxY, mbb.maxZ);
    }
    ...
    public static byte[] toSpatiaLiteBlob(Geometry geom, int srid)
    {
        final int size = computeSpatiaLiteBlobSize(geom);
        byte[] blob = new byte[size];
        toSpatiaLiteBlob(geom.pointer.raw, srid, blob, 0, size, true);
        return blob;
    }
    ...
    public static Geometry createWedge(Geometry location, double startAngle, double endAngle, double range, int algorithm)
    {
        if (location == null)
            throw new NullPointerException();
        return createWedge(location.pointer.raw, startAngle, endAngle, range, algorithm);
    }
    ...
    static native Geometry fromEnvelope(double minX, double minY, double minZ, double maxX, double maxY, double maxZ);
    ...
    static native Geometry createWedge(long locationPtr, double startAngle, double endAngle, double range, int algoPtr);
