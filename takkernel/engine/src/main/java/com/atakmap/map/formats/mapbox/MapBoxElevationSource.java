...
public final class MapBoxElevationSource extends TiledElevationSource
{
    ...
    public MapBoxElevationSource(String token, File cacheFile)
    {
        ...
        cache = new Cache(cacheFile, 3857);
            ...
            public void run()
            {
                try
                {
                    ...
                    try (UriFactory.OpenResult result = handler.handleURI(uri + "?access_token=" + token))
                    {
                        ...
                        final File f = getFile(cache, zoom, x, y);
                        if (!f.getParentFile().exists())
                            f.getParentFile().mkdirs();
                        final File downloadFile = new File(f.getParentFile(), f.getName() + ".download");
                        FileSystemUtils.copyStream(result.inputStream, false, new FileOutputStream(downloadFile), true);
        ...
        public ElevationChunk get(int zoom, int x, int y)
        {
            ...
            try
            {
                ...
                return ElevationChunk.Factory.create(
                                ...
                                Double.NaN,
                        false,
                                new TerrainRGB(data, srid, true, x, y, zoom, null));
            } catch (Throwable t)
            {
                return null;
            }
