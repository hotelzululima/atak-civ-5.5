...
@DontObfuscate final class NativeElevationChunk implements ElevationChunk
{
    ...
    public double sample(double latitude, double longitude)
    {
        ...
        try
        {
            return (this.pointer.raw == 0L) ? Double.NaN : sample(this.pointer.raw, latitude, longitude);
        } finally
        {
            this.rwlock.releaseRead();
        }
    ...
    public boolean sample(double[] lla, int off, int len)
    {
        ...
        try
        {
            return (this.pointer.raw == 0L) ? false : sample(this.pointer.raw, lla, off, len);
        } finally
        {
            this.rwlock.releaseRead();
        }
    ...
    {
        return (pointer != null) ? new NativeElevationChunk(pointer, owner) : null;
    }

    static native void destruct(Pointer pointer);

    static long getPointer(ElevationChunk object)
    {
        if (object instanceof NativeElevationChunk)
            return ((NativeElevationChunk) object).pointer.raw;
        else
            return 0L;
    }
    ...
    static native double sample(long pointer, double latitude, double longitude);

    static native boolean sample(long pointer, double[] lla, int off, int len);

    static native Pointer ElevationChunkFactory_meshFromHeightmap(double[] heightmap, int tileWidth, int tileHeight, double tileMinX, double tileMinY, double tileMaxX, double tileMaxY, boolean discardNoDataPosts, int positionStorage);
    static native Pointer ElevationChunkFactory_meshFromHeightmap(float[] heightmap, int tileWidth, int tileHeight, double tileMinX, double tileMinY, double tileMaxX, double tileMaxY, boolean discardNoDataPosts, int positionStorage);
}
