 ...
 * @author Developer
 * @deprecated use {@link GLAsynchronousMapRenderable2} API
 */
@Deprecated
        ...
        GLResolvableMapRenderable
{
    ...
    protected final void init(GLMapView view)
    {
    ...
    protected static class ViewState
    {
        ...
        public void set(GLMapView view)
        {
            ...
            if (ctrl != null)
            {
                // compute GSD based on camera range
                final double camRangeGsd = MapSceneModel.gsd(
                        MathUtils.distance(
                                view.currentScene.scene.camera.location.x * view.currentScene.scene.displayModel.projectionXToNominalMeters,
                                view.currentScene.scene.camera.location.y * view.currentScene.scene.displayModel.projectionYToNominalMeters,
                                view.currentScene.scene.camera.location.z * view.currentScene.scene.displayModel.projectionZToNominalMeters,
                                view.currentScene.scene.camera.target.x * view.currentScene.scene.displayModel.projectionXToNominalMeters,
                                view.currentScene.scene.camera.target.y * view.currentScene.scene.displayModel.projectionYToNominalMeters,
                                view.currentScene.scene.camera.target.z * view.currentScene.scene.displayModel.projectionZToNominalMeters
                        ),
                        view.currentScene.scene.camera.fov,
                        view.currentScene.scene.height
                );
                // favor higher GSD per nominal vs camera line-of-sight. If LOS
                // is very high GSD, it will be subsequently capped when
                // compared against terrain tiles
                this.drawMapResolution = Math.min(camRangeGsd, this.drawMapResolution);
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    synchronized (GLAsynchronousMapRenderable.this)
                    {
                        ...
                        GLAsynchronousMapRenderable.this.dirtyRegions.clear();
                        if (queryState.crossesIDL)
                        {
                            GLAsynchronousMapRenderable.this.dirtyRegions.add(
                                    new Envelope(
                                ...
                                .updateRenderableReleaseLists(pendingData))
                        {
                            ...
                            if (GLAsynchronousMapRenderable.this.surfaceControl != null &&
                                    !GLAsynchronousMapRenderable.this.dirtyRegions.isEmpty())
                            {

                                final Collection<Envelope> dirty = new ArrayList<>(GLAsynchronousMapRenderable.this.dirtyRegions);
                                GLAsynchronousMapRenderable.this.renderCtx.queueEvent(new Runnable()
                                {
                                    public void run()
                                    {
                                        // abort if renderable was released
                                        if (GLAsynchronousMapRenderable.this.surfaceControl == null)
                                            return;
                                        // mark the dirty regions
                                        for (Envelope region : dirty)
                                            GLAsynchronousMapRenderable.this.surfaceControl.markDirty(region, false);
                                    }
                                });
                            }
                        }
