...
import com.atakmap.map.layer.control.StarsControl;
        ...
        Disposable
{
    ...
    final static NativePeerManager.Cleaner CLEANER = new NativePeerManager.Cleaner()
    {
        ...
        protected void run(Pointer pointer, Object opaque)
        {
            ...
            globeReferences.remove(pointer);
            destruct(pointer);
        }
    };
    ...
    private final static int INVERSE_MODE_SURFACE = 3;
    ...
    public final static int RENDER_PASS_SURFACE_PREFETCH = 0x10;

    //! @deprecated use RENDER_PASS_SURFACE_PREFETCH
    @Deprecated
    public final static int RENDER_PASS_SURFACE2 = RENDER_PASS_SURFACE_PREFETCH;
    ...
    private StarsControl starsControl;
    private IlluminationControl2 illuminationControl2;
    private ImageryRelativeScaleControl imageryRelativeScaleControl;
    private SurfaceBoundsFilter surfaceBoundsFilter;
    private ClampToGroundControl clampToGroundControl;
    ...
    private Set<OnCameraChangedListener2> _cameraChangedListeners;
    private long[] cameraChangedForwarderPtr = new long[1];

    private final ThreadLocal<NativeModelHitTest> _nativeModelHitTest = new ThreadLocal<NativeModelHitTest>() {
        @Override
        protected NativeModelHitTest initialValue() {
            return new NativeModelHitTest();
        }
    };
    ...
    protected GLMapView(RenderContext context, Globe globe, int left, int bottom, int right, int top, boolean legacy)
    {
        ...
        gov.tak.api.engine.map.RenderContext.setCurrent(getRenderContext());

        // XXX -
        GLRenderGlobals.get(_context).getBitmapLoader();
        ...
        globeReferences.put(pointer, globe);
        ...
        if (impl == IMPL_V2)
        {
            ...
            starsControl = new StarsControlImpl();
            this.registerControl(null, starsControl);

            illuminationControl2 = new IlluminationControlImpl2();
            this.registerControl(null, illuminationControl2);

            imageryRelativeScaleControl = new ImageryRelativeScaleControlImpl();
            this.registerControl(null, imageryRelativeScaleControl);

            surfaceBoundsFilter = new SurfaceBoundsFilter();
        ...
        if(getElevationSourceControl(pointer.raw) != 0L)
            this.registerControl(null, new ElevationSourceControlImpl());
        ...
        // XXX - provider logic should be sensibly refactored
        GLRenderGlobals.get(_context).getBitmapLoader();
    }
    ...
    public void setStarsLayer(final GLMapRenderable2 starsLayer)
    {
        rwlock.acquireRead();
        try
        {
            if (this.pointer.raw != 0L)
                setStarsLayer(this.pointer.raw, starsLayer);
        } finally
        {
            rwlock.releaseRead();
        }
    ...
    @Override
    public void addOnCameraChangedListener(MapRenderer2.OnCameraChangedListener2 l)
    {
    ...
    @Override
    public void removeOnCameraChangedListener(MapRenderer2.OnCameraChangedListener2 l)
    {
        synchronized (_cameraChangedListeners)
        {
            _cameraChangedListeners.remove(l);
        }
    }
    ...
    private InverseResult inverseImpl(PointD xyz, GeoPoint lla, InverseMode mode, int hints, DisplayOrigin origin)
    {
        ...
        final double y = (origin == DisplayOrigin.UpperLeft) ?
                getSurfaceHeight() - xyz.y : // flip origin if necessary
                xyz.y;
        ...
        {
            case Transform:
                if (inverse(this.pointer.raw, x, y, z, INVERSE_MODE_ABSOLUTE, lla, false))
            ...
            case RayCast:
                ...
                if (!MathUtils.hasBits(hints, HINT_RAYCAST_IGNORE_SURFACE_MESH))
                {
                    ...
                    // Add native results, using renderer display origin coords
                    NativeModelHitTest nativeModelHitTest = _nativeModelHitTest.get();
                    nativeModelHitTest.x = x;
                    nativeModelHitTest.y = y;
                    nativeModelHitTest.z = z;
                    controls.add(nativeModelHitTest);

                    // perform model hit test
                    final float px = (float) x;
                    final float py = (origin == DisplayOrigin.UpperLeft) ? (float) xyz.y : (float) (getSurfaceHeight() - xyz.y);
                ...
                if (!MathUtils.hasBits(hints, HINT_RAYCAST_IGNORE_TERRAIN_MESH))
                {
                    ...
                    if (inverse(this.pointer.raw, x, y, z, INVERSE_MODE_TERRAIN, lla, true))
                    {
                        // a hit occurred -- check distance to camera
                        proj.forward(lla, candidatexyz);
                        final double dx = (candidatexyz.x - cam.x) * sx;
                ...
                if (inverse(this.pointer.raw, x, y, z, INVERSE_MODE_MODEL, lla, false))
    ...
    public void inverse(FloatBuffer src, FloatBuffer dst)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo);
    ...
    private void sync()
    {
        ...
        {
            // corners
            this.internal.pointF.x = currentPass.left;
            this.internal.pointF.y = currentPass.top;
            this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo, true);
            final double ullng = this.internal.geo.getLongitude();

            this.internal.pointF.x = currentPass.right;
            this.internal.pointF.y = currentPass.top;
            this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo, true);
            final double urlng = this.internal.geo.getLongitude();

            this.internal.pointF.x = currentPass.right;
            this.internal.pointF.y = currentPass.bottom;
            this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo, true);
    ...
    public void render()
    {
        ...
        gov.tak.api.engine.map.RenderContext.setCurrent(this.getRenderContext());
        ...
        // XXX - pass through _clamp-to-ground_ setting from injected control
        if(clampToGroundControl != null)
            setClampToGroundAtNadir(this.pointer.raw, clampToGroundControl.getClampToGroundAtNadir());

        render(this.pointer.raw);
        sync();
        this.syncPass = 0;

        if (this.currentPass.drawTilt > 0 && this.terrainBlendEnabled)
    ...
    protected void drawRenderables()
    {
        ...
        if (this.currentPass.drawTilt > 0 && this.syncPass == RENDER_PASS_SURFACE)
            this.syncPass = 0;
    }

    public int getLeft()
    {
        return currentPass.left;
    }

    public int getRight()
    {
        if (currentPass.right == MATCH_SURFACE)
        {
            currentPass.right = getSurfaceWidth();
        }
        return currentPass.right;
    }

    public int getTop()
    {
        if (currentPass.top == MATCH_SURFACE)
        {
            currentPass.top = getSurfaceHeight();
        }
        return currentPass.top;
    }

    public int getBottom()
    {
        return currentPass.bottom;
    }

    private int getSurfaceWidth()
    {
        return _surface instanceof RenderSurface2 ? ((RenderSurface2)_surface).getSurfaceWidth() : _surface.getWidth();
    }

    private int getSurfaceHeight()
    {
        return _surface instanceof RenderSurface2 ? ((RenderSurface2)_surface).getSurfaceHeight() : _surface.getHeight();
    }
    ...
    protected void updateBounds()
    {
        ...
        this.internal.pointF.y = currentPass.top;
        this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo, true);
        this.currentPass.upperLeft.set(this.internal.geo);
        ...
        this.internal.pointF.y = currentPass.top;
        this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo, true);
        this.currentPass.upperRight.set(this.internal.geo);
        ...
        this.internal.pointF.y = currentPass.bottom;
        this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo, true);
        this.currentPass.lowerRight.set(this.internal.geo);
        ...
        this.internal.pointF.y = currentPass.bottom;
        this.currentPass.scene.inverse(this.internal.pointF, this.internal.geo, true);
        this.currentPass.lowerLeft.set(this.internal.geo);
        ...
        if (this.currentPass.scene.mapProjection.is3D())
        {
            this.UpdateLatLonAABBoxEllipsoid();

            wrapCorner(this.currentPass.upperLeft);
            wrapCorner(this.currentPass.upperRight);
            wrapCorner(this.currentPass.lowerRight);
            wrapCorner(this.currentPass.lowerLeft);
    ...
    {
        int w = (currentPass.right - currentPass.left), hw = w >> 1, h = (currentPass.top - currentPass.bottom), hh = h >> 1;
        ...
        this.internal.pointF.x = currentPass.left;
        this.internal.pointF.y = currentPass.bottom + h;
        points[idx] = this.currentPass.scene.inverse(this.internal.pointF, null, false);
        if (points[idx] != null)
            idx++;
        //GetEllipsoidPosition((float)hw,0,true),
        this.internal.pointF.x = currentPass.left + hw;
        this.internal.pointF.y = currentPass.bottom + h;
        points[idx] = this.currentPass.scene.inverse(this.internal.pointF, null, false);
        if (points[idx] != null)
            idx++;
        //GetEllipsoidPosition( (float)w,0,true),
        this.internal.pointF.x = currentPass.left + w;
        this.internal.pointF.y = currentPass.bottom + h;
        points[idx] = this.currentPass.scene.inverse(this.internal.pointF, null, false);
        if (points[idx] != null)
            idx++;
        ...
        this.internal.pointF.x = currentPass.left + w;
        this.internal.pointF.y = currentPass.bottom + hh;
        points[idx] = this.currentPass.scene.inverse(this.internal.pointF, null, false);
        if (points[idx] != null)
            idx++;
        ...
        this.internal.pointF.x = this.currentPass.left;
        this.internal.pointF.y = this.currentPass.bottom;
        points[idx] = this.currentPass.scene.inverse(this.internal.pointF, null, false);
        if (points[idx] != null)
            idx++;
        ...
        this.internal.geo.set(this.currentPass.drawLat, this.drawLng);

        boolean horizonInView = (idx < points.length);
        boolean poleInView = (idx == 0 && currentPass.drawLat != 0d);
        if (idx > 0)
        {
            north = points[0].getLatitude();
            south = points[0].getLatitude();
            double furthestDsq = distanceSquared(currentPass.scene.camera.location, currentPass.scene.mapProjection.forward(points[0], internal.pointD));
            for (int i = 1; i < idx; i++)
            {
                ...
                double dsq = distanceSquared(currentPass.scene.camera.location, currentPass.scene.mapProjection.forward(points[i], internal.pointD));
                if (dsq > furthestDsq)
                    furthestDsq = dsq;
            }

            poleInView = (this.currentPass.drawLat < south || this.currentPass.drawLat > north) ||
                    distanceSquared(currentPass.scene.camera.location, currentPass.scene.mapProjection.forward(internal.geo.set(90d, 0d), internal.pointD)) < furthestDsq ||
                    distanceSquared(currentPass.scene.camera.location, currentPass.scene.mapProjection.forward(internal.geo.set(-90d, 0d), internal.pointD)) < furthestDsq;
        }
        ...
        if (poleInView)
        {
            ...
            if (idx == 0)
            {
                north = Math.min(this.currentPass.drawLat + 90d, 90d);
                south = Math.max(this.currentPass.drawLat - 90d, -90d);
            }

            if (this.currentPass.drawLat > 0d)
            {
                north = 90d;
                if (idx < 8 && this.currentPass.drawTilt == 0d)
                    south = this.currentPass.drawLat - 90d;
            } else if (this.currentPass.drawLat < 0d)
            {
                south = -90d;
                if (idx < 8 && this.currentPass.drawTilt == 0d)
                    north = this.currentPass.drawLat + 90d;
            } else
            {
                // XXX -
            }
            ...
            this.currentPass.upperLeft.set(north, GeoCalculations.wrapLongitude(west));
            this.currentPass.upperRight.set(north, GeoCalculations.wrapLongitude(east));
            this.currentPass.lowerRight.set(south, GeoCalculations.wrapLongitude(east));
            this.currentPass.lowerLeft.set(south, GeoCalculations.wrapLongitude(west));
...
/*
    ...
    public void onMapProjectionChanged(AtakMapView view)
    {
        ...
        _context.queueEvent(new Runnable()
        {
            ...
            public void run()
            {
                if (GLMapView.this.currentPass.drawSrid != srid)
                {
                    GLMapView.this.currentPass.drawSrid = srid;
                    GLMapView.this.drawProjection = ProjectionFactory.getProjection(GLMapView.this.currentPass.drawSrid);
    ...
    {
        this.validateSceneModelImpl(getSurfaceWidth(), getSurfaceHeight());
    }
    ...
    protected final void validateSceneModelImpl(int width, int height)
    {
        if (this.sceneModelVersion != this.currentPass.drawVersion)
        {
            ...
            this.internal.geo.set(this.currentPass.drawLat, this.drawLng, 0d);

            if (this.drawProjection == null || this.drawProjection.getSpatialReferenceID() != this.currentPass.drawSrid)
                this.drawProjection = ProjectionFactory.getProjection(this.currentPass.drawSrid);

            this.currentPass.scene = new MapSceneModel(_surface.getDpi(),
                    width, height,
                    this.drawProjection,
                    this.internal.geo,
                    this.currentPass.focusx, this.currentPass.focusy,
                    this.currentPass.drawRotation, this.currentPass.drawTilt,
                    this.currentPass.drawMapResolution,
                    this.continuousScrollEnabled);

            // account for flipping of y-axis for OpenGL coordinate space
            this.currentPass.scene.inverse.concatenate(GLMapView.this.verticalFlipTranslate);
            this.currentPass.scene.inverse.concatenate(XFORM_VERTICAL_FLIP_SCALE);

            this.currentPass.scene.forward.preConcatenate(XFORM_VERTICAL_FLIP_SCALE);
            this.currentPass.scene.forward.preConcatenate(GLMapView.this.verticalFlipTranslate);

            this.currentPass.scene.forward.get(internal.matrixD);
            for (int i = 0; i < 16; i++)
                this.currentPass.sceneModelForwardMatrix[i] = (float) internal.matrixD[i];
    ...
    protected class Animator
    {
         ...
         * view's {@link GLMapView#currentPass}.{@link State#drawVersion drawVersion} will be bumped as a result of
        ...
        public void updateView()
        {
            ...
            GLMapView.this.currentPass.drawLat = _drawLat;
            GLMapView.this.currentPass.drawLng = _drawLng;
            GLMapView.this.drawLng = _drawLng;
            GLMapView.this.currentPass.drawRotation = _drawRotation;
            GLMapView.this.currentPass.drawTilt = _drawTilt;
            GLMapView.this.currentPass.drawMapResolution = Globe.getMapResolution(_surface.getDpi(), _drawMapScale);

            GLMapView.this.currentPass.focusx = _focusx;
            GLMapView.this.currentPass.focusy = _focusy;
    ...
    public final static class ScratchPad
    {
        ...
        public final double[] matrixD = new double[48];
        public final float[] matrixF = new float[48];
    ...
    {
        // XXX - `ClampToGroundControl` should be provided, not injected
        if(control instanceof ClampToGroundControl)
            clampToGroundControl = (ClampToGroundControl) control;
    ...
    {
        if(control == clampToGroundControl)
            clampToGroundControl = null;
        rendererControlsLock.acquireWrite();
        try
        {
    ...
    @Override
    public <T extends MapControl> boolean visitControl(final Layer2 layer, final Visitor<T> visitor, final Class<T> ctrlClazz)
    {
        if (layer != null)
        {
            synchronized (this)
            {
                ...
                visited[0] = visitImpl(visitor, ctrlClazz, this.controls.get(layer));
                if(!visited[0]) {
                    // no managed control was found. try to visit native controls
                    NativeControl.visitNativeControls(this, layer, ctrlClazz,
                        new NativeControl.Visitor<T>() {
                            @Override
                            public void onVisitNativeControl(T control) {
    ...
    @Override
    public boolean visitControls(Layer2 layer, final Visitor<Iterator<MapControl>> visitor)
    {
        if (layer != null)
        {
            ...
            synchronized (this)
            {
                // obtain the managed controls
                final Collection<MapControl> ctrls = this.controls.get(layer);

                // Native visit controls has the following contract detail:
                //   * Client code should only interact with the controls during the
                ...
                //     results.
                //
                // We will visit the (marshaled) native controls and issue the callback within its
                // scope, concatenating any managed controls
                NativeControl.visitNativeControls(this, layer,
                        new NativeControl.Visitor<Iterator<MapControl>>() {
                            @Override
                            public void onVisitNativeControl(Iterator<MapControl> nativeControl) {
                                if(ctrls == null)
                                    visitor.visit(nativeControl);
                                else
                                    visitor.visit(Collections2.concatenate(ctrls.iterator(), nativeControl));
                                result[0] = true;
                            }
                        });
                // if no native controls were visited, dispatch the managed controls
                if (!result[0] && ctrls != null) {
    ...
    public synchronized void visitControls(Visitor<Iterator<Map.Entry<Layer2, Collection<MapControl>>>> visitor)
    {
        boolean[] result = {false};
        // obtain the managed controls
        final Collection<Map.Entry<Layer2, Collection<MapControl>>> ctrls = this.controls.entrySet();
        ...
        // We will visit the (marshaled) native controls and issue the callback within its
        // scope, concatenating any managed controls
        NativeControl.visitNativeControls(this, this.controls.keySet(),
                new NativeControl.Visitor<Iterator<Map.Entry<Layer2, Collection<MapControl>>>>() {
                    @Override
                    public void onVisitNativeControl(Iterator<Map.Entry<Layer2, Collection<MapControl>>> nativeControl) {
                        if(ctrls == null)
                            visitor.visit(nativeControl);
                        else
                            visitor.visit(Collections2.concatenate(ctrls.iterator(), nativeControl));
    ...
    public final static class State
    {
        ...
        /** Copies from {@code view} to {@code this} */
        ...
        /** Copies from {@code this} to {@code view} */
    ...
    final class IlluminationControlImpl2 implements IlluminationControl2
    {
        ...
        {
            final Calendar d = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
    ...
    final class StarsControlImpl implements StarsControl
    {
        @Override
        public boolean isStarsEnabled()
        {
            ...
            try
            {
                if (pointer.raw == 0L)
                    return false;
                return GLMapView.isStarsEnabled(pointer.raw, impl);
            } finally
            {
                rwlock.releaseRead();
            }
        ...
        public void setStarsEnabled(boolean enabled)
        {
            ...
            {
                if (pointer.raw != 0L)
                    GLMapView.setStarsEnabled(pointer.raw, enabled, impl);
            } finally
            {
                rwlock.releaseRead();
            }
    ...
    final class ElevationSourceControlImpl implements ElevationSourceControl
    {
        @Override
        public void setElevationSource(ElevationSource source)
        {
            if (surfaceBoundsFilter != null) {
                do {
                    // XXX - multi-globe support on desktop may share the same `ElevationSource`
                    //       instance
                    ...
                    if (!(source instanceof Controls))
                        break;
                    final SpatialFilterControl sfc = ((Controls) source).getControl(SpatialFilterControl.class);
                    if (sfc != null)
                        sfc.setFilter(surfaceBoundsFilter);
                } while (false);
            }
            rwlock.acquireRead();
            try
            {
                if (pointer.raw != 0L)
                    GLMapView.setElevationSource(pointer.raw, source);
            } finally
            {
                rwlock.releaseRead();
            }
        ...
        public ElevationSource getElevationSource()
        {
            ...
            try
            {
                return (pointer.raw != 0L) ? GLMapView.getElevationSource(pointer.raw) : null;
            } finally
            {
                rwlock.releaseRead();
            }
    ...
    final class ImageryRelativeScaleControlImpl implements ImageryRelativeScaleControl {

        float _relativeScale = 1f;
        ...
        public void setRelativeScale(float factor) {
            if(factor <= 0f)
                throw new IllegalArgumentException();
            factor = MathUtils.clamp(factor, 0.25f, 4f);
            if(factor != _relativeScale) {
                _relativeScale = factor;
    ...
    final class NativeModelHitTest implements ModelHitTestControl {
        double x;
        double y;
        double z;

        @Override
        public boolean hitTest(float screenX, float screenY, GeoPoint llaresult) {
            return inverse(pointer.raw, x, y, z, INVERSE_MODE_SURFACE, llaresult, true);
        }
    }
    ...
    {
        // update the surface bounds filter -- this is the earliest place in the render pump that
        // we can dispatch
        if(surfaceBoundsFilter != null)
        ...
        synchronized (_cameraChangedListeners)
        {
            for (OnCameraChangedListener2 l : _cameraChangedListeners)
            {
    ...
    private void dispatchCameraChangeRequested()
    {
        synchronized (_cameraChangedListeners)
        {
            for (OnCameraChangedListener2 l : _cameraChangedListeners)
            {
                l.onCameraChangeRequested(GLMapView.this);
            }
        }
    }
    ...
    static native void setStarsLayer(long ptr, GLMapRenderable2 basemap);
    ...
    // elevation source control
    static native long getElevationSourceControl(long ptr);
    static native void setElevationSource(long ptr, ElevationSource source);
    static native ElevationSource getElevationSource(long ptr);
    ...
    static native boolean inverse(long ptr, double x, double y, double z, int mode, GeoPoint result, boolean requireModeResult);
    ...
    static native void setStarsEnabled(long ptr, boolean enabled, int impl);

    static native boolean isStarsEnabled(long ptr, int impl);

    static native void setClampToGroundAtNadir(long ptr, boolean v);
}
