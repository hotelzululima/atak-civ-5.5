...
/** @deprecated deferred to C++ implementation by way of {@link com.atakmap.map.layer.feature.opengl.GLBatchGeometryFeatureDataStoreRenderer#SPI} */
@Deprecated
        ...
        GLMapRenderable2, HitTestControl
{
     ...
     */
    private final static int LINES_VERTEX_SIZE = (12 + 12 + 4 + 4 + 1 + 1 + 2); // 36
    ...
    public void draw(GLMapView view, int renderPass)
    {
        ...
        if (surface && batchSurfaceSrid != view.currentPass.drawSrid)
        {
            // reset relative to center
            batchSurfaceCentroid.set(view.currentScene.drawLat, view.currentScene.drawLng);
            view.currentPass.scene.mapProjection.forward(batchSurfaceCentroid, batchSurfaceCentroidProj);
            batchSurfaceSrid = view.currentPass.drawSrid;
        ...
        if (sprites && batchSpritesSrid != view.currentPass.drawSrid)
        {
            // reset relative to center
            batchSpritesCentroid.set(view.currentScene.drawLat, view.currentScene.drawLng);
            view.currentPass.scene.mapProjection.forward(batchSpritesCentroid, batchSpritesCentroidProj);
            batchSpritesSrid = view.currentPass.drawSrid;
    ...
    private void renderSurface(GLMapView view)
    {
        ...
        if (!this.surfaceLines.isEmpty())
        {
            if (MathUtils.hasBits(rebuildBatchBuffers, GLMapView.RENDER_PASS_SURFACE))
            {
                ...
                this.buildLineBuffers(surfaceLineBuffers, view, surfaceLines, batchSurfaceCentroidProj, true, true);
            }

            this.drawLineBuffers(view, surfaceLineBuffers, batchSurfaceCentroidProj, true);
    ...
    private void renderLines(GLMapView view, Collection<? extends GLBatchLineString> lines, int renderPass)
    {
        ...
        if (!(vertType == GLGeometry.VERTICES_PROJECTED && view.currentPass.scene.mapProjection.is3D()))
    ...
    private void renderSprites(GLMapView view)
    {
        ...
        sortInfo.order = (view.currentPass.drawTilt > 0d || view.currentPass.drawSrid == 4978) ? SortInfo.DEPTH : SortInfo.FID;
        sortInfo.centerLat = view.currentPass.drawLat;
        sortInfo.centerLng = view.currentPass.drawLng;

        view.scratch.geo.set(view.currentPass.drawLat, view.currentPass.drawLng);
        GeoPoint bottomCenter = GeoCalculations.midPointWGS84(view.currentPass.lowerLeft, view.currentPass.lowerRight);
        ...
        if (!this.spriteLines.isEmpty())
        {
            ...
            if (MathUtils.hasBits(rebuildBatchBuffers, GLMapView.RENDER_PASS_SPRITES))
            {
                ...
                this.buildLineBuffers(spriteLineBuffers0, view, spriteLines, batchSpritesCentroidProj, true, false);
            }

            this.drawLineBuffers(view, spriteLineBuffers0, batchSpritesCentroidProj, false);
        ...
        if (!this.spritePolys.isEmpty())
        {
            ...
            if (MathUtils.hasBits(rebuildBatchBuffers, GLMapView.RENDER_PASS_SPRITES))
            {
                ...
                this.buildLineBuffers(spriteLineBuffers1, view, spritePolys, batchSpritesCentroidProj, false, false);
            }
            ...
            // draw outlines
            this.drawLineBuffers(view, spriteLineBuffers1, batchSpritesCentroidProj, false);
    ...
    private static void bls3_vertex(ByteBuffer vbuf, GLBatchLineString.RenderState state, float relativeScale, PointD v1, PointD v2, int acap, int bcap)
    {
        ...
        vbuf.put((isMpu && ((state.pattern&0xFFFF) == 0xFFFF)) ?
                (byte)0x00 :
                (byte) MathUtils.clamp(state.factor, 1, 255));
        vbuf.put((byte) Math.min(state.strokeWidth / relativeScale / 2.0f, 255.0f));
        vbuf.put((byte)acap);
        vbuf.put((byte)bcap);
    }

    void buildLineBuffers(Collection<PrimitiveBuffer> linesBuf, GLMapView view, Collection<? extends GLBatchLineString> lines, PointD centroidProj, boolean requiresProject, boolean surface)
    {
        ...
        for (GLBatchLineString line : lines)
        {
            ...
            for (int i = 0; i < rs.length; i++) {
                ...
                final int numLines = line._hasInnerRings ? line._numPolygons : 1;
                for (int lineIdx = 0; lineIdx < numLines; lineIdx++) {
                    final int numSegs;
                    final int step;
                    final int numVerts = (numLines > 1) ? line._numVertices[lineIdx] : line.numRenderPoints;
                    ...
                    for (int j = 0; j < numSegs; j++) {
                        ...
                        final int aidx = ((j+line._startIndices[lineIdx]) * step);
                        ...
                        final int bcap = (j < (numSegs - 1)) ? acap * -1 : acap;
                        bls3_vertex(vbuf, rs[i], relativeScale, p0, p1, acap, bcap);
                    }
                }
            }
        }
    ...
    void drawLineBuffers(GLMapView view, Collection<PrimitiveBuffer> buf, PointD centroidProj, boolean surface)
    {
        ...
        if (this.lineShader == null)
        {
            ...
            lineShader.handle = GLAntiAliasedLine.getAntiAliasedLinesShader(view.getRenderContext());
            ...
            lineShader.a_cap = GLES30.glGetAttribLocation(lineShader.handle, "a_cap");
            lineShader.a_outlineColor = GLES30.glGetAttribLocation(lineShader.handle, "a_outlineColor");
            lineShader.a_outlineWidth = GLES30.glGetAttribLocation(lineShader.handle, "a_outlineWidth");
        ...
        // MVP
        {
            ...
            // model-view
            view.scratch.matrix.concatenate(view.currentPass.scene.forward);
        ...
        GLES30.glEnableVertexAttribArray(lineShader.a_cap);

        // instance geometry
        GLES30.glVertexAttribDivisor(lineShader.a_normal, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_dir, 0);
        // per-instance attributes
        GLES30.glVertexAttribDivisor(lineShader.a_vertexCoord0, 1);
        GLES30.glVertexAttribDivisor(lineShader.a_vertexCoord1, 1);
        GLES30.glVertexAttribDivisor(lineShader.a_color, 1);
        GLES30.glVertexAttribDivisor(lineShader.a_pattern, 1);
        GLES30.glVertexAttribDivisor(lineShader.a_factor, 1);
        GLES30.glVertexAttribDivisor(lineShader.a_halfStrokeWidth, 1);
        GLES30.glVertexAttribDivisor(lineShader.a_cap, 1);

        // constants
        GLES30.glDisableVertexAttribArray(lineShader.a_outlineWidth);
        GLES30.glDisableVertexAttribArray(lineShader.a_outlineColor);
        GLES30.glVertexAttrib1f(lineShader.a_outlineWidth, 0f);
        GLES30.glVertexAttrib4f(lineShader.a_outlineColor, 0f, 0f, 0f, 0f);

        GLES30.glEnable(GLES30.GL_BLEND);
        GLES30.glBlendFunc(GLES30.GL_SRC_ALPHA, GLES30.GL_ONE_MINUS_SRC_ALPHA);

        GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, GLES30.GL_NONE);
        GLES30.glVertexAttribPointer(lineShader.a_normal, 1, GLES30.GL_UNSIGNED_SHORT, true, 4, segmentGeom.position(0));
        GLES30.glVertexAttribPointer(lineShader.a_dir, 1, GLES30.GL_UNSIGNED_SHORT, true, 4, segmentGeom.position(1));
        ...
        for (PrimitiveBuffer it : buf)
        {
            ...
            // pattern
            if(isMpu)
                GLES30.glVertexAttribPointer(lineShader.a_pattern, 1, GLES30.GL_INT, false, LINES_VERTEX_SIZE, 28);
            else
                GLES30.glVertexAttribIPointer(lineShader.a_pattern, 1, GLES30.GL_INT, LINES_VERTEX_SIZE, 28);
            GLES30.glVertexAttribPointer(lineShader.a_factor, 1, GLES30.GL_UNSIGNED_BYTE, false, LINES_VERTEX_SIZE, 32);

            GLES30.glVertexAttribPointer(lineShader.a_halfStrokeWidth, 1, GLES30.GL_UNSIGNED_BYTE, false, LINES_VERTEX_SIZE, 33);
            // cap/join
            GLES30.glVertexAttribPointer(lineShader.a_cap, 2, GLES30.GL_BYTE, false, LINES_VERTEX_SIZE, 34);

            GLES30.glDrawArraysInstanced(it.mode, 0, 6, it.count);
            ...
            if(!surface)
            {
                ...
                GLES30.glDisableVertexAttribArray(lineShader.a_cap);
                // enable joins on all segments
                GLES30.glVertexAttrib2f(lineShader.a_cap, 1.f, 1.f);
                // draw lines
                GLES30.glDrawArraysInstanced(it.mode, 0, 6, it.count);
        ...
        GLES30.glVertexAttribDivisor(lineShader.a_vertexCoord0, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_vertexCoord1, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_color, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_normal, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_halfStrokeWidth, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_pattern, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_factor, 0);
        GLES30.glVertexAttribDivisor(lineShader.a_cap, 0);
    ...
    private void batchDrawPoints(GLMapView view)
    {
        ...
        if (view.currentPass.drawTilt > 0d || view.currentPass.drawSrid == 4978)
        {
        ...
        for (GLBatchGeometry geom : this.batchPoints2)
        {
            ...
            } else
            {
                ...
                if (view.currentPass.drawTilt > 0d)
                {
                    ...
                    adjustedAlt += view.currentPass.drawMapResolution * (point.iconHeight / 2d);

                    // move up ~5 pixels from surface
                    adjustedAlt += view.currentPass.drawMapResolution * 10d * GLRenderGlobals.getRelativeScaling();
