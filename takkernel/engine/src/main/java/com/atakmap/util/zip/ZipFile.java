 ...
 * consider {@link java.util.zip.ZipInputStream}, which provides stream-like read access to a zip file and
...
public class ZipFile implements Closeable, ZipConstants
{
    ...
    public ZipFile(File file, int mode) throws IOException
    {
        ...
        raf = IOProviderFactory.getRandomAccessFile(new File(filename), "r");
     ...
     * need random-access entry lookup by name, you should probably use {@link java.util.zip.ZipInputStream}
    ...
    public InputStream getInputStream(ZipEntry entry) throws IOException
    {
        ...
        synchronized (localRaf)
        {
            ...
            RAFStream rafStream = new RAFStream(localRaf,
                    entry.localHeaderRelOffset);
            ...
            } else
            {
                ...
                return new ZipInflaterInputStream(rafStream, new Inflater(true),
                        bufSize, entry);
            }
        }
    }
    ...
    private void readCentralDir() throws IOException
    {
        ...
        final long zip64EocdRecordOffset = Zip64
                .parseZip64EocdRecordLocator(raf, eocdOffset);
        ...
        EocdRecord record = parseEocdRecord(raf, eocdOffset + 4,
        ...
        if (record.commentLength > 0)
        {
            ...
            comment = new String(commentBytes, 0, commentBytes.length,
                    StandardCharsets.UTF_8);
        }
        // We have a zip64 eocd record : use that for getting the information we need.
        if (zip64EocdRecordOffset != -1)
        {
            record = Zip64.parseZip64EocdRecord(raf, zip64EocdRecordOffset,
                    record.commentLength);
        }
        ...
        BufferedInputStream bufferedStream = new BufferedInputStream(rafStream,
                4096);
    ...
    private static EocdRecord parseEocdRecord(RandomAccessFile raf, long offset,
                                              boolean isZip64) throws IOException
    {
        raf.seek(offset);
        ...
        raf.readFully(eocd);
        BufferIterator it = HeapBufferIterator.iterator(eocd, 0, eocd.length,
    ...
    {
        final String hexString = IntegralToString.intToHexString(magic, true,
                8);
     ...
     * @hide
     */
    final static class RAFStream extends InputStream
    {
        ...
        public int fill(Inflater inflater, int nativeEndBufSize)
                throws IOException
        {
            synchronized (sharedRaf)
            {
                ...
                int cnt = sharedRaf.read(buf);
                if (cnt > 0)
                    inflater.setInput(buf, 0, cnt);
                return len;
            }
        }
    }
    ...
    /** @hide */
    final static class ZipInflaterInputStream extends InflaterInputStream
    {
        private final ZipEntry entry;
        private long bytesRead = 0;
        private boolean closed;

        public ZipInflaterInputStream(InputStream is, Inflater inf, int bsize,
                                      ZipEntry entry)
        {
            super(is, inf, bsize);
            this.entry = entry;
                ...
                throws IOException
        {
            ...
            {
                throw new IOException("Error reading data for "
                        + entry.getName() + " near offset "
                        + bytesRead, e);
            }
            if (i == -1)
            {
                ...
                {
                    throw new IOException("Size mismatch on inflated file: "
                            + bytesRead + " vs "
                            + entry.size);
                }
            } else
