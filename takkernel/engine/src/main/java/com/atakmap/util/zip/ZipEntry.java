 ...
 * https://android.googlesource.com/platform/libcore/+/android-6.0.1_r21/luni/src/main/java/java/util/zip/ZipEntry.java
 * <p>
 * In support of
 ...
 * with {@link ZipFile#getInputStream}. The class documentation for {@link java.util.zip.ZipInputStream} and
 * {@link java.util.zip.ZipOutputStream} shows how {@code ZipEntry} is used in conjunction with those two classes.
 */
public class ZipEntry implements ZipConstants, Cloneable
{
    ...
    public ZipEntry(String name, String comment, long crc, long compressedSize,
                    long size, int compressionMethod, int time, int modDate,
                    byte[] extra,
     ...
     */
    ZipEntry(byte[] cdeHdrBuf, InputStream cdStream, Charset defaultCharset,
             boolean isZip64) throws IOException
    {
        Streams.readFully(cdStream, cdeHdrBuf, 0, cdeHdrBuf.length);
        BufferIterator it = HeapBufferIterator.iterator(cdeHdrBuf, 0,
                cdeHdrBuf.length,
        ...
        {
            throw new ZipException("Filename contains NUL byte: "
        ...
        if (isZip64)
        {
            Zip64.parseZip64ExtendedInfo(this,
                    true /* from central directory */);
        }
    }
    ...
    /**
     * Returns a java.util.zip.ZipEntry object populated with this object's information
     * @return Populated java.util.zip.ZipEntry object
     */
    public java.util.zip.ZipEntry toJavaZipEntry()
    {
        java.util.zip.ZipEntry returnZipEntry = new java.util.zip.ZipEntry(
                name);

        //Copy all of the data into the java.util.zip.ZipEntry object
        if (comment != null)
        {
            returnZipEntry.setComment(comment);
        }
        if (compressedSize != -1)
        {
            returnZipEntry.setCompressedSize(compressedSize);
        }
        if (crc != -1)
        {
            returnZipEntry.setCrc(crc);
        }
        if (extra != null)
        {
            returnZipEntry.setExtra(extra);
        }
        if (compressionMethod != -1)
        {
            if (compressionMethod == java.util.zip.ZipEntry.STORED)
            {
                returnZipEntry.setMethod(java.util.zip.ZipEntry.STORED);
            } else if (compressionMethod == java.util.zip.ZipEntry.DEFLATED)
            {
                returnZipEntry.setMethod(java.util.zip.ZipEntry.DEFLATED);
            }
        }
        if (size != -1)
        {
            returnZipEntry.setSize(size);
        }
        if (time != -1)
        {
            returnZipEntry.setTime(time);
        }
