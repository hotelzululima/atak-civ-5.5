...
class ServiceProviderRegistryBase<T, V, S extends ServiceProvider<T, V>, U>
{
    ...
    protected T create(V object, U strategy)
    {
        rwlock.acquireRead();
        try {
    ...
    protected void register(S provider, U strategy, boolean strategyOnly, int priority)
    {
        rwlock.acquireWrite();
        try {
    ...
    public final void unregister(S provider)
    {
        rwlock.acquireWrite();
        try {
    ...
    protected void visitProviders(Visitor<Iterator<S>> visitor, U hint)
    {
        rwlock.acquireRead();
        try {
    ...
    protected static class SpiWrapper<T, V, S extends ServiceProvider<T, V>, U> implements Comparable<SpiWrapper<T, V, S, U>>
    {
        ...
        public boolean equals(Object o)
        {
            ...
            if (insert != that.insert)
                return false;
            if (spi != null ? !spi.equals(that.spi) : that.spi != null)
                return false;
