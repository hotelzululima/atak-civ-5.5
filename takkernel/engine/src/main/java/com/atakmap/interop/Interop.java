...
public abstract class Interop<T>
{
     ...
     *
     * @param object The object
     * @param doIntern If {@code true}, interns the pointer for subsequent lookups via {@link #find(T)}
     * @return A {@link Pointer} to a native implementation of the
     * corresponding base interface/class whose calls are serviced
     * via the specified Java object.
     */
    public Pointer wrap(T object, boolean doIntern) {
        final Pointer retval = this.wrap(object);
        if(doIntern)
    ...
    public synchronized static <T> Interop<T> findInterop(Class<T> clazz)
    {
        ...
        do
        {
            ...
            try
            {
                ...
                {
                    retval = new ReflectInterop<T>(clazz, nativeClazz);
                    break;
                }
            } catch (Throwable ignored) {}
        } while (false);
    ...
    public synchronized static <T> boolean registerInterop(Class<T> clazz, Class<?> interopProvider)
    {
        try
        {
            final Interop<T> interop = new ReflectInterop<>(clazz, interopProvider);
            registry.put(clazz, interop);
            return true;
        } catch (Throwable t) {
            Log.e(TAG, interopProvider + " is not a suitable interop implementation for " + clazz);
            return false;
        }
    }
