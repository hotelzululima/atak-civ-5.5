...
public final class DatabaseImpl implements DatabaseIface
{
    ...
    {
        if (this.pointer.raw != 0L)
            Log.w("DatabaseImpl", "Database leaked");
    }
    ...
    // interop impl
    @DontObfuscate
    static long getPointer(DatabaseIface object) {
        return (object instanceof DatabaseImpl) ? ((DatabaseImpl)object).pointer.raw : 0L;
    }
    @DontObfuscate
    static DatabaseIface create(Pointer pointer, Object owner) {
        return (pointer != null && pointer.raw != 0L) ? new DatabaseImpl(pointer, owner) : null;
    }
    @DontObfuscate
    static boolean hasPointer(DatabaseIface object) {
        return (object instanceof DatabaseImpl);
    }
