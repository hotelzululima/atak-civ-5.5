...
public final class GLSLUtil
{
    ...
    {
        if(context == null) {
            // not the ideal location, but it will have to do
            ConfigOptions.setOption("app.dirs.files", ctx.getFilesDir().getAbsolutePath());
            ConfigOptions.setOption("app.dirs.cache", ctx.getCacheDir().getAbsolutePath());
        }
        context = ctx;
    }
