...
public class GLTextureCache
{
    ...
    private void trimToSize()
    {
        ...
        int [] releaseArray = new int[25];
        int releaseIndex = 0;

        while (this.size > this.maxSize * .75 && this.hash.size() > 1)
        {
            ...
            this.head.previous = null;
            this.count--;
            releaseArray[releaseIndex++] = toRelease.getTexId();
            if(releaseIndex == releaseArray.length)
            {
                GLES20FixedPipeline.glDeleteTextures(releaseIndex, releaseArray, 0);
                releaseIndex = 0;
            }
