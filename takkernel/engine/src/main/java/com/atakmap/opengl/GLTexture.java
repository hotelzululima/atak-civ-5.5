        ...
        if (_id == 0 && x == 0 && y == 0 && bitmap.getWidth() == _width && bitmap.getHeight() == _height)
        {
            ...
            do {
                try {
                    ...
                    // `GLUtils.texImage2D` may fail with an invalid bitmap format. Attempt to
                    // convert to ARGB
                    if(bitmap.getConfig() != Bitmap.Config.ARGB_8888 && _format != GLES30.GL_RGBA) {
                        bitmap = bitmap.copy(Bitmap.Config.ARGB_8888, true);
                        if(bitmap != null) {
                            _format = GLES30.GL_RGBA;
                            _type = GLES30.GL_UNSIGNED_BYTE;
                            continue;
                        }
                    }
                }
