...
public final class GLText
{
    ...
    {
        return getInstance(RenderContext.getCurrent(), textFormat);
    }
    ...
    private GLText(RenderContext ctx, MapTextFormat mapTextFormat, Typeface typeface, float densityAdjustedTextSize)
    {
        final int glyphRenderFontSize = findBestFontSize(mapTextFormat.getFontSize());

        //if(false) pointer = intern(mapTextFormat, null); else
        final Font baseFont = MarshalManager.marshal(mapTextFormat, MapTextFormat.class, TextFormat.class).getFont();
        final Font font = new Font(baseFont.getFamilyName(), baseFont.getStyle(), glyphRenderFontSize);
        final TextFormat textFormat = new TextFormat(font, mapTextFormat.isOutlined() ? TextFormat.STYLE_OUTLINED : 0);
        pointer = intern(RenderContext_interop.getPointer(ctx), mapTextFormat, new MapTextFormat(textFormat));
    ...
    static native void setTextFormatFactory();

    @DontObfuscate
    static MapTextFormat createTextFormat(String face, boolean isBold, boolean isItalic, int fontSize, boolean isUnderline, boolean isStrikethrough, boolean outline)
    {
        final Font.Style fontStyle;
        if(isBold && isItalic)
            fontStyle = Font.Style.BoldItalic;
        else if(isBold)
        ...
        int style = 0;
        if(isUnderline)
            style |= TextFormat.STYLE_UNDERLINE;
        if(isStrikethrough)
            style |= TextFormat.STYLE_STRIKETHROUGH;
        if(outline)
            style |= TextFormat.STYLE_OUTLINED;

        TextFormat fmt = new TextFormat(
                new Font(face, fontStyle, fontSize),
                style);


        return new MapTextFormat(fmt);
    }
}
