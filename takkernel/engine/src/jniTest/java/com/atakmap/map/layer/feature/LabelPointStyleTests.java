...
public class LabelPointStyleTests extends KernelJniTest {
    @Test
    ...
    public void LabelPointStyle_constructor_2_align_CC_roundtrip() {
        ...
        final double minRenderResolution = 1 + (RandomUtils.rng().nextDouble() * 9);
        final float scale = 1 + (RandomUtils.rng().nextFloat() * 9);
        final int offsetX = 50 - RandomUtils.rng().nextInt(100);
        final int offsetY = 50 - RandomUtils.rng().nextInt(100);
        LabelPointStyle style = new LabelPointStyle(text, textColor, bgColor,
                mode, textSize, offsetX, offsetY, alignX, alignY, rotation,
                rotationAbsolute, minRenderResolution, scale);
        Assert.assertEquals(style.getText(), text);
        Assert.assertEquals(style.getTextColor(), textColor);
        Assert.assertEquals(style.getBackgroundColor(), bgColor);
        ...
        Assert.assertEquals(minRenderResolution, style.getLabelMinRenderResolution(), 0.0);
        Assert.assertEquals(scale, style.getLabelScale(), 0.0);
        Assert.assertEquals(offsetX, style.getOffsetX(), 0.0);
        Assert.assertEquals(offsetY, style.getOffsetY(), 0.0);
    }
    ...
    static LabelPointStyle random() {
        ...
        return new LabelPointStyle(text, textColor, bgColor, mode, textSize,
                offsetX, offsetY, alignX, alignY, rotation, rotationAbsolute, minRenderResolution, scale);
    }
}
