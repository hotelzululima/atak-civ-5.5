...
import gov.tak.platform.contact.ContactNotification.ContactNotificationType;
...
/**
 * Responsible for storing and managing {@link IContact}s for the Contacts API. See {@link ContactStore}
...
class DefaultContactStore extends ContactStore
{
    ...
    DefaultContactStore(ExecutorService executor)
    {
    ...
    public void addContact(@NonNull IContact contact)
    {
        ...
        final String contactUid = contact.getUid();
        ...
        notifyListeners(contact, ContactNotificationType.ADD);
    }
    ...
    public void updateContact(@NonNull IContact contact)
    {
        ...
        notifyListeners(contact, ContactNotificationType.UPDATE);
    }
    ...
    public void removeContact(@NonNull String uniqueContactId)
    {
        ...
        notifyListeners(removedContact, ContactNotificationType.REMOVE);
    }
    ...
    public boolean containsContact(@NonNull IContact contact)
    {
        ...
        return allContacts.containsKey(contact.getUid());
    }

    @Override
    public IContact getContact(@NonNull String contactUid)
    {
        Objects.requireNonNull(contactUid, "Attempted to get contact with null UID.");

        return allContacts.get(contactUid);
    }
    ...
    public void registerContactListener(@NonNull IContactListener contactListener)
    {
        ...
        {
            executorService.submit(new ContactNotification(contactListener, contact, ContactNotificationType.ADD));
        }
    }
    ...
    private void notifyListeners(IContact contact, ContactNotificationType notificationType)
    {
        for (IContactListener listener : contactListeners)
        {
            executorService.submit(new ContactNotification(listener, contact, notificationType));
        }
    }
}
