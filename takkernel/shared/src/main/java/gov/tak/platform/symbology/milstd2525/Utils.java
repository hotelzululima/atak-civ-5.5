...
final class Utils {
    ...
    {
        return Vector2D.polygonContainsPoint(new Vector2D(point.getLatitude(), point.getLongitude()),
                bounds);
    }

    static boolean isFullyInsideBounds(IGeoPoint[]controlPoints, Vector2D[] bounds)
    {
        for(int i=0;i < controlPoints.length;++i)
        {
            if(!isInsideBounds(controlPoints[i], bounds))
                return false;
        }
        return true;
    }

    public static ArrayList<ArrayList<IGeoPoint>> clipPoints(IGeoPoint[] controlPoints, Envelope screenBounds)
    {
        ...
        {
            cp[i] = new Vector2D(controlPoints[i].getLatitude(), controlPoints[i].getLongitude());
        }
        ...
        for (int i=0; i < controlPoints.length - 1; ++i)
        {
            ...
            else
            {
                ...
                for(int j=0;j < 4;++j)
                {
                    com.atakmap.coremap.maps.coords.GeoPoint intersection =
                            GeoCalculations.findIntersection(
                                    MarshalManager.marshal(segment[0], IGeoPoint.class, com.atakmap.coremap.maps.coords.GeoPoint.class),
                                    MarshalManager.marshal(segment[1], IGeoPoint.class, com.atakmap.coremap.maps.coords.GeoPoint.class),
                                    MarshalManager.marshal(bounds[j], IGeoPoint.class, com.atakmap.coremap.maps.coords.GeoPoint.class),
                                    MarshalManager.marshal(bounds[j+1], IGeoPoint.class, com.atakmap.coremap.maps.coords.GeoPoint.class));
