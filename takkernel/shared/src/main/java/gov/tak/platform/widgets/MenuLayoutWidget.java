        ...
        IParentWidget.OnWidgetListChangedListener
{
    ...
    private static PointF getLocalPoint(IMapWidget widget, PointF p) {
        if(widget == null)
            return p;
        p.x -= widget.getPointX();
        p.y -= widget.getPointY();
        return getLocalPoint(widget.getParent(), p);
    }

    private static boolean isExpanderClick(IMapMenuButtonWidget button, MotionEvent event)
    {
        if(!(button instanceof IRadialButtonWidget))
            return false;
        else if(SystemUtils.isOsAndroid())
            return false;

        final PointF xy = getLocalPoint(button, new PointF(event.getX(), event.getY()));
        return RadialButtonWidget.isExpanderHit((IRadialButtonWidget) button, xy.x, xy.y);
    }
    ...
    protected void onMenuClick(IMapMenuButtonWidget button, IMapMenuWidget menuWidget, MotionEvent event)
    {
        ...
        if(button.isDisabled())
            return;
        final IMapMenuWidget submenuWidget = button.getSubmenu();
        final boolean hasClickAction = (null != button.getOnButtonClickHandler() && button.getOnButtonClickHandler().isSupported(_mapItem));
        if(submenuWidget != null && (!hasClickAction || isExpanderClick(button, event))) {
            // if button does not have action or click is on expander, show submenu
