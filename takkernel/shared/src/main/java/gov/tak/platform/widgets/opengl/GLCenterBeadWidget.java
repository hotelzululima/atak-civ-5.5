...
public class GLCenterBeadWidget extends GLShapeWidget
{
    ...
    public void drawWidgetContent(DrawState drawState)
    {
        ...
        RenderSurface renderSurface = getRenderContext().getRenderSurface();
        float density = (float) (renderSurface.getDpi() / 240d);
        float radius = 50f * density;
        float diameter = radius * 2;
        float d8 = diameter / 8, d32 = diameter / 32;

        float left = drawState.scene.focusx - radius;
        int height = renderSurface instanceof RenderSurface2 ? ((RenderSurface2)renderSurface).getSurfaceHeight() : renderSurface.getHeight();
