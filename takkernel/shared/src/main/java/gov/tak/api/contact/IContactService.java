...
/**
 * Contract for service that maintains an application-wide collection of {@link IContact Contacts}.
 *
 * The Contact service allows core applications and plugins to manage {@link IContact}s through the API, as well as
 ...
 * <pre>
 *     IContactService contactStore = Contacts.getDefaultContactStore();
...
public interface IContactService
{
     ...
     * that of the given contact, as decided by {@link IContact#getUid()}. Note that the contact must have already
     ...
     */
    @Nullable
    IContact getContact(@NonNull String contactUid);

    /**
     * Returns all the contacts currently in the store.
     *
     * @return A list containing all the contacts currently in the store.
     */
    List<IContact> getAllContacts();

    /**
     * Obtain a collection of all contacts of the given type.
     *
     * @param contactType Type of contact to return, must be a subtype of {@link IContact}
     * @return Collection of contacts of the given type, may be empty, never null
     * @param <T> Inferred type
     */
    @NonNull
    default <T extends IContact> List<T> getAllContactsOfType(@NonNull Class<T> contactType)
    {
        final List<T> contacts = new ArrayList<>();

        for (IContact contact : getAllContacts()) {
            if (contactType.isInstance(contact)) contacts.add(contactType.cast(contact));
        }
