...
/**
 * Created by lqd0532 on 8/7/2017.
 */
//Todo: I wasn't able to get this idling resource to work with toasts, it may work for other views but I didn't get a chance to check that
public class ViewVisibilityIdlingResource implements IdlingResource {
     ...
     */
    public ViewVisibilityIdlingResource(@NonNull Activity activity,
            @IdRes int viewId, int visibility) {
    ...
    public ViewVisibilityIdlingResource(@NonNull View view, int visibility) {
        ...
        mName = "View Visibility for view " + view.getId() + "(@"
                + System.identityHashCode(mView) + ")";
    }
    ...
    public boolean isIdleNow() {
        View view = mView.get();
        final boolean isIdle = view == null
                || view.getVisibility() == mVisibility;
        if (isIdle) {
