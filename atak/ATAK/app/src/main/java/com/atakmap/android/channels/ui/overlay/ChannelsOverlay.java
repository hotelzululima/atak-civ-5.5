...
public class ChannelsOverlay extends AbstractMapOverlay2 implements
    ...
    private final Map<String, Map<String, DistinguishedNameHierarchyListItem>> distinguishedNameHierarchyListItemCache = new HashMap<>();
    ...
    private void createDNHierarchyListItem(String server, String name) {
        Map<String, DistinguishedNameHierarchyListItem> serverDnMap =
                distinguishedNameHierarchyListItemCache.get(server);
        DistinguishedNameHierarchyListItem listItem = null;

        if (serverDnMap == null) {
            serverDnMap = new HashMap<>();
            distinguishedNameHierarchyListItemCache.put(server, serverDnMap);
        } else {
            listItem = serverDnMap.get(name);
        }

        if (listItem == null) {
            listItem = new DistinguishedNameHierarchyListItem(
                    context, this, server, name);
            serverDnMap.put(name, listItem);
        }
    }

    private void addDistinguishedName(String server, String distinguishedName) {
        Log.d(TAG, "addDistinguishedName " + server + ", " + distinguishedName);
        createDNHierarchyListItem(server, distinguishedName);
        int firstComma = distinguishedName.indexOf(",");
        if (firstComma != -1) {
            addDistinguishedName(server, distinguishedName.substring(firstComma+1));
        }
    }

    public List<DistinguishedNameHierarchyListItem> getOuRoots(String server) {
        List<DistinguishedNameHierarchyListItem> ouRoots = new ArrayList<>();
        ...
        for (DistinguishedNameHierarchyListItem dnListItem :
                serverDnMap.values()) {
            if ((dnListItem.getDistinguishedName().toLowerCase().startsWith("ou=") &&
                    dnListItem.getDistinguishedName().toLowerCase()
                            .split("ou=", -1).length == 2)) {
                ouRoots.add(dnListItem);
            }
        }
    ...
    public int getChildCount(String server, String distinguishedName) {
        List<ServerGroupHierarchyListItem> serverGroups = getServerGroups(server);
        ...
        if (distinguishedName == null) {
            ...
            int count = 0;
            for (ServerGroupHierarchyListItem serverGroupHierarchyListItem : serverGroups) {
                if (serverGroupHierarchyListItem.getUserObject().getDistinguishedName()
                        .endsWith(distinguishedName)) {
                    count++;
                }
            }
            return count;
    ...
    public List<DistinguishedNameHierarchyListItem> getChildren(String server, String distinguishedName) {
        List<DistinguishedNameHierarchyListItem> children = new ArrayList<>();
        ...
        for (DistinguishedNameHierarchyListItem dnListItem :
                serverDnMap.values()) {
            String nextDn = dnListItem.getDistinguishedName();
            if (nextDn.endsWith(distinguishedName)
                    && (commaCount(nextDn) == commaCount(distinguishedName) + 1)
                    && !processed.contains(nextDn)) {
                children.add(dnListItem);
                processed.add(nextDn);
            }
        }
        return children;
    }
    ...
    public void onGetServerGroups(String server,
        ...
        for (ServerGroup serverGroup : serverGroups) {
            ...
            boolean enableChannelsHierarchyForHost = AtakPreferences.getInstance(context).get(
                    ChannelsMapComponent.PREFERENCE_ENABLE_CHANNELS_HIERARCHY_HOST_KEY
                            + "-" +
                            server,
                    "false").equals("true");

            if (enableChannelsHierarchyForHost &&
                    serverGroup.getDistinguishedName() != null &&
                    !serverGroup.getDistinguishedName().isEmpty()) {
                addDistinguishedName(server, serverGroup.getDistinguishedName());
            }
    ...
    public ServerGroupHierarchyListItem getServerGroup(String server, String distinguishedName) {
        List<ServerGroupHierarchyListItem> serverGroupHierarchyListItems = serverGroupHierarchyListItemCache
                .get(server);
        if (serverGroupHierarchyListItems == null) {
            Log.d(TAG,
        ...
        List<ServerGroupHierarchyListItem> results = new LinkedList<>();
        for (ServerGroupHierarchyListItem serverGroupHierarchyListItem : serverGroupHierarchyListItems) {
            if (((ServerGroup)serverGroupHierarchyListItem.getUserObject())
                    .getDistinguishedName().equals(distinguishedName)) {
                return serverGroupHierarchyListItem;
            }
        }
    ...
    public static Intent getListIntent(Context context) {
        try {
            ...
            AtakPreferences sharedPreferences = AtakPreferences
                    .getInstance(context);
            ...
            for (TAKServer takServer : TAKServerListener.getInstance()
                ...
                boolean enableChannelsForHost = sharedPreferences.get(
    ...
    public void onCotEvent(CotEvent event, Bundle extra) {
        ...
        if (event.getUID() != null
                && event.getUID().contains(MapView.getDeviceUid())) {
            return;
        }
