...
public class ElevationOverlaysMapComponent extends AbstractMapComponent
    ...
    public static final String PREFERENCE_HEATMAP_VISIBLE_KEY = "prefs_dted_visible";
    ...
    public void onCreate(Context context, Intent intent, MapView view) {
        ...
        prefs.registerListener(_prefsListener);

        heatMapOverlay = new HeatMapOverlay();

        // don't want the legacy legend to show on terrain slope layer
        ConfigOptions.setOption("terrain-slope.disable-legacy-legend", 1);
        ...
        overlayLayer.setVisible(
                prefs.get(PREFERENCE_VISIBLE_KEY, false));
        ...
        gradientWidget.setName(ELEVATION_WIDGET);
        gradientWidget.setPadding(12f, 12f, 12f, 12f);
        gradientWidget.setMargins(16f, 0f, 0f, 16f);

        _mapView.addOnMapViewResizedListener(this);
        onMapViewResized(_mapView);

        layoutH.addChildWidgetAt(0, _keyWidget);
        layoutH.addChildWidgetAt(1, gradientWidget);
        ...
        final boolean heatmapviz = prefs.get(PREFERENCE_HEATMAP_VISIBLE_KEY,
                false);
        setHeatMapVisibleState(heatmapviz);
    ...
    private void pollGradientWidgetData() {
        _mapView.getRenderer3().visitControls(terrainSlopeOverlay,
                new Visitor<Iterator<MapControl>>() {
                    @Override
                    public void visit(Iterator<MapControl> iterator) {
                        if (iterator.hasNext()) {
                            MapControl control = iterator.next();
                            if (control instanceof GradientControl) {
                                GradientControl gradientControl = ((GradientControl) control);
                                gradientWidget.setLegend(
                                        gradientControl.getGradientColors(),
                                        gradientControl.getLineItemStrings());
                            }
                        }
                    }
                });
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        prefs.unregisterListener(_prefsListener);
    ...
    public void onMapViewResized(AtakMapView view) {
        ...
        _keyWidget.setBarSize(_mapView.getWidth() * 0.01f,
                _mapView.getHeight() * 0.175f);
        gradientWidget.setBarSize(_mapView.getWidth() * 0.01f,
                _mapView.getHeight() * 0.175f);
    }
    ...
    private void toggleHeatmapLegend() {
        ...
        gradientWidget.setVisible(overlayLayer.isVisible()
                && overlayLayer.get() == terrainSlopeOverlay);
    }

    private synchronized void toggleOverlay() {
        final boolean state = !overlayLayer.isVisible();
        setHeatMapVisibleState(!state);
    }

    private void setHeatMapVisibleState(boolean state) {
            ...
            public void run() {
                ...
                if (state) {
                    _mapView.addLayer(MapView.RenderStack.MAP_SURFACE_OVERLAYS,
                            overlayLayer);
                    pollGradientWidgetData();
                }

                Layer currentOverlay = overlayLayer.get();
                _keyWidget
                        .setVisible(state && currentOverlay == heatMapOverlay);
                gradientWidget.setVisible(
                        state && currentOverlay == terrainSlopeOverlay);

                // record the new visibility
                prefs.set(PREFERENCE_HEATMAP_VISIBLE_KEY, state);
