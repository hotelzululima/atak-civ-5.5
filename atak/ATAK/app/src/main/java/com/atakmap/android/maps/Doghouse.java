...
public final class Doghouse extends Polyline implements
    ...
    private static final String TAG = "Doghouse";
    ...
    Doghouse(int turnpointId,
        ...
        _isTablet = ATAKUtilities.isTablet();
        ...
        for (int i = 0; i < MAX_FIELDS; i++) {
            ...
            String value = _prefs.get(prefKey,
        ...
        _noseShift = _prefs.get(DoghouseReceiver.PERCENT_ALONG_LEG, 0.5f);
        _nose = computeNosePosition(_noseShift);

        _strokeColor = _prefs.get(DoghouseReceiver.STROKE_COLOR,
                Color.BLACK);

        int color = _prefs.get(DoghouseReceiver.SHADE_COLOR, 0xBFFFFFFF);
        ...
        color = _prefs.get(DoghouseReceiver.TEXT_COLOR, Color.RED);
        ...
        _distanceFromLeg = _prefs.get(DoghouseReceiver.DISTANCE_FROM_LEG,
                100);

        _strokeWidth = _prefs.get(DoghouseReceiver.STROKE_WIDTH, 2);

        _sizeSegment = _prefs.get(DoghouseReceiver.SIZE_SEGMENT, 120);

        _maxScale = _prefs.get(DoghouseReceiver.MAX_SCALE_VISIBLE, 200f);
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
        ...
        switch (key) {
            ...
            case DoghouseReceiver.SHADE_COLOR:
                int sColor = prefs.getInt(key, -1);
                _shadeColor[0] = Color.alpha(sColor);
                _shadeColor[1] = Color.red(sColor);
                _shadeColor[2] = Color.green(sColor);
                _shadeColor[3] = Color.blue(sColor);
                break;
            case DoghouseReceiver.TEXT_COLOR:
                int tColor = prefs.getInt(key, -1);
                _textColor[0] = Color.alpha(tColor);
                _textColor[1] = Color.red(tColor);
                _textColor[2] = Color.green(tColor);
                _textColor[3] = Color.blue(tColor);
                break;
    ...
    private String formatBearingString(double bearing) {
        ...
        String formattedBearing = AngleUtilities.format(bearing, Angle.DEGREE,
                decimalPlaces) + type;
        double wrappedBearing = AngleUtilities.wrapDeg(bearing);
        //if bearing is less than 10(1 digit) then append 2 zeroes to start to show "00X"
        if (wrappedBearing < 10) {
            formattedBearing = "00" + formattedBearing;
            //if bearing is less than 100 (2 digit) then append 1 zero to start to show "0XX"
        } else if (wrappedBearing < 100) {
            formattedBearing = "0" + formattedBearing;
        }
        return formattedBearing;
    }
