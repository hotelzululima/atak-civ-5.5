        ...
        for (File file : downloads) {
            ...
            for (ImportResolver sorter : sorters) {
                ...
                if (sorter.match(file)) {
                    // XXX - much of the below logic is related to legacy _sorters_ copying/moving
                    //       files around. current impl leaves things in place so legacy
                    //       check-if-updated/check-if-exists logic deletes the file. the logic is
                    //       left in place as it would be good in the future for refresh/update
                    //       flows.
                    ...
                    /*
                      Comment out the below code( if (IOProviderFactory.exists(destPath)) { )
                      to make the network kml always process the file even if its not changed when update tick happens -SPA
                    */
                    if (!destPath.equals(file)
                            && IOProviderFactory.exists(destPath)) {
