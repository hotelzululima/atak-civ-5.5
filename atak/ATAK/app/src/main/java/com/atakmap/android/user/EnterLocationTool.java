...
public class EnterLocationTool extends Tool implements
    ...
    public EnterLocationTool(MapView mapView) {
        ...
        _prefs = AtakPreferences.getInstance(con);
    }
    ...
    protected void onToolEnd() {
        ...
        MetricsUtils.record(MetricsUtils.CATEGORY_TOOL,
                MetricsUtils.EVENT_CLICKED,
                "EnterLocationTool", "EnterLocationTool",
    ...
    private void _requestAddMapListener() {
        if (!_listeningToMap) {
            ...
            _prefs.set("point_pick_center_designator", true);
    ...
    private void _requestRemoveMapListener() {
        if (_listeningToMap) {
            ...
            _prefs.set("point_pick_center_designator", false);
            _listeningToMap = false;
        }
    }
    ...
    private void addPoint(GeoPointMetaData p) {
        ...
        if (marker != null) {
            ...
            MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                    MetricsUtils.EVENT_MAPITEM_ADDED, "EnterLocationTool",
                    marker, MetricsUtils.EVENT_STATUS_SUCCESS);
        }
    }

    private void launchEnter() {
        if (_prefs.get("rapid_mgrs_dialog", false)) {
            ...
            _mapView.post(new Runnable() {
                @Override
                public void run() {
                    CoordinateEntryCapability.getInstance(con).showDialog(
                            "",
                            null,
                            true, _mapView.getPoint(), null, false,
