...
public abstract class LayerSelectionAdapter extends BaseAdapter implements
    ...
    public interface SelectionAliasFilter {
        boolean matches(LayerSelection selection);
    }
    ...
    LayersManagerBroadcastReceiver autoSelectCallback;
    LayersManagerView view;
    final FeatureDataStore2 outlinesDatastore;

    private final Set<OnItemSelectedListener> listeners;

    Map<String, SelectionAliasFilter> aliasFilters = new ConcurrentHashMap<>();
    ...
    public LayerSelectionAdapter(RasterLayer2 layer,
        ...
        _context = context;
        _mapView = mapView;
        _prefs = AtakPreferences.getInstance(_mapView.getContext());
    ...
    public void addSelectionAlias(String alias, SelectionAliasFilter filter) {
        aliasFilters.put(alias, filter);
        invalidate(true);
    }

    public void removeSelectionAlias(String alias) {
        aliasFilters.remove(alias);
        invalidate(true);
    }
    ...
    public final synchronized void setLocked(boolean isLocked) {
        ...
        if (autoSelectCallback != null) {
            ...
            _mapView.post(new Runnable() {
                @Override
                public void run() {
                    autoSelectCallback.updateAutoLayer();
                }
            });
        }
    }
    ...
    public final synchronized int selectedIndex() {
        ...
        for (LayerSelection layer : _selections) {
            if (matchesSelection(selection, layer))
                return i;
            i++;
        }
        return -1;
    ...
    boolean matchesSelection(String selection, LayerSelection ls) {
        return ls.getName().equals(selection);
    }
    ...
    final void setAutoSelectCallback(LayersManagerBroadcastReceiver lmbr) {
        this.autoSelectCallback = lmbr;
    }
