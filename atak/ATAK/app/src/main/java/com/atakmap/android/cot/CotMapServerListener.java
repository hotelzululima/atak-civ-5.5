...
public class CotMapServerListener extends CotStreamListener implements
        ...
        if (connected) {
            ...
            if (!bHaveContactList) {
                ...
                try {
                    ...
                    getProfileUpdates(ncs.getHost(), ncs.toString());
                } catch (Exception e) {
            ...
            getToolProfileUpdates(ncs.getHost(), ncs.toString());
        }
    }
    ...
    private void getToolProfileUpdates(String host,
        ...
        if (!_prefs.get("deviceProfileEnableOnConnect", false)) {
            return;
        }
        ...
        synchronized (toolProfileRequests) {
            for (String tool : toolProfileRequests) {
                ...
                if (!client.hasProfileBeenReceived(tool)) {
                    if (!client.getProfile(_context, host, serverConnectString,
                            tool)) {
                        Log.e(TAG, "unable to get a profile");
                    }
                } else {
     ...
     * @param callback
     * @throws JSONException if parsing the json results in an error
     */
        ...
        if (request
            ...
            if (FileSystemUtils.isEmpty(response)) {
                ...
                if (version != null && version.isValid()
                    ...
                    TAKServer input = findExistingStream(
                            connectString);
                    if (input != null) {
    ...
    private void getProfileUpdates(final String server,
        ...
        final long lastSyncTime = _prefs.get(
                "deviceProfileOnConnectSyncTime" + server, 0L);
        ...
        DeviceProfileClient.getInstance().getProfile(_context,
                server, serverConnectString, null, null, true, false, true,
