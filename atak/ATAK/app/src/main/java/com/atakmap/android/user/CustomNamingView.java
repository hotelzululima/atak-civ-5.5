...
public class CustomNamingView
    ...
    public void onClick(View v) {
        if (v == _resetButton) {
            ...
            if ("123".equals(_swapButton.getTag())) {
                _swapButton.setImageResource(R.drawable.swap_ab);
                _swapButton.setTag("ABC");
                _startNumberET
                        .setInputType(
                ...
                _startNumberET.setText("");
                _startNumberET.setEnabled(true);
            } else if ("ABC".equals(_swapButton.getTag())) {
                _swapButton.setImageResource(R.drawable.nav_empty);
                _swapButton.setTag("");
                _startNumberET.setEnabled(false);
            } else {
    ...
    public void incrementStartIndex() {
        ...
        if (_swapButton.getTag().equals("ABC")) {
            _startNumberET
    ...
    public String genCallsign() {
        if (_prefixET.getText().toString().isEmpty()) {
            if (_startNumberET.getText().toString().isEmpty()) {
                return "";
            } else {
            ...
            if (idx.isEmpty() && "123".equals(_swapButton.getTag())) {
                idx = "1";
                _startNumberET.setText("1");
            } else if (idx.isEmpty() && "ABC".equals(_swapButton.getTag())) {
                idx = "A";
                _startNumberET.setText("A");
            }

            if ("".equals(_swapButton.getTag()))
                return prefix;
            else
                return prefix + " " + idx;
        }
    ...
    /**
     * If the custom naming is enabled, go ahead and set up the marker creater
     * appropriately.
     * @param mc the marker creator to impact.
     * @return true if the custom callsign was set, otherwise false.
     */
    public boolean setCallsignIfEnabled(PlacePointTool.MarkerCreator mc) {
        final String callsign = genCallsign();
        if (!callsign.isEmpty()) {
            mc.setAllowDuplicativeNames(true);
