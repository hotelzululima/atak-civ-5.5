...
public class GenericPointDetailsView extends GenericDetailsView implements
    ...
    public void setPoint(MapView mapView, Marker point) {
        ...
        _init();
        formatAddress();
        setClassification(_point, marking);
        ...
        CoordinateEntryCapability.getInstance(getContext()).showDialog(
                ...
                new CoordinateEntryCapability.ResultCallback() {
                    ...
                    public void onResultCallback(String pane,
                        ...
                        GenericPointDetailsView.setAddress(point, _point,
                                _nameEdit);
                        CameraController.Programmatic.panTo(
     ...
     */
    public static void setAddress(GeoPointMetaData gp,
            PointMapItem _point, EditText _nameEdit) {

        if (_point == null)
            return;

        final String usagehint = (String) gp.getMetaData("address_usage_hint");
        final String address = (String) gp.getMetaData("address_text");
        final String address_src = (String) gp.getMetaData("address_geocoder");
        final String address_lookuptime = (String) gp
                .getMetaData("address_lookuptime");
        if (usagehint != null && usagehint.equals("title")) {
            if (address != null && !address.isEmpty()) {
                _point.setMetaString("callsign", address);
                if (_point instanceof Marker)
                    _point.setTitle(address);

                if (_nameEdit != null)
                    _nameEdit.setText(address);
            }
        }
        if (address != null && !address.isEmpty()) {
            ...
            _point.setMetaString("address_geopoint",
                    AddressDetailHandler.locationHash(gp.get()));
            _point.setMetaString("address_text", address);

            String src = address_src;
            if (src == null)
                src = "unknown";
            _point.setMetaString("address_geocoder", src);
            _point.setMetaString("address_lookuptime", address_lookuptime);
        }
        final MapView mapView = MapView.getMapView();
        if (mapView != null) {
            _point.persist(mapView.getMapEventDispatcher(),
                    new Bundle(),
                    GenericPointDetailsView.class);
        }
    }
