...
public class RouteImporter extends EditablePolylineImporter {
    ...
    public ImportResult importData(CotEvent event, Bundle extras) {
        ...
        final MapItem existing = findItem(event);

        super.importData(event, extras);

        final MapItem newItem = findItem(event);
    ...
    protected ImportResult importMapItem(MapItem existing, CotEvent event,
        ...
        if (existing != null && !(existing instanceof Route)) {
            Log.e(TAG, "route import error from event: " + event);
            Log.e(TAG,
                    "attempting to import a route with uid: "
                            + existing.getUID() +
                            " but the type is: " + existing.getType() + " "
                            + existing.getClass() +
                            " " + existing.getTitle()
                            + " and the class type is wrong, this is an error",
                    new IllegalStateException("wrong map item type"));
            return ImportResult.FAILURE;
        }
        ...
        if (linkAttr != null) {
            ...
            if (linkAttr.getAttribute("style") != null) {
                try {
                    route.setStrokeStyle(Integer.parseInt(linkAttr
                            .getAttribute("style")));
                } catch (Exception e) {
                    Log.e(TAG, "error: ", e);
                }
