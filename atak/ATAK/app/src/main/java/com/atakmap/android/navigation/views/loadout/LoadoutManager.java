...
public class LoadoutManager implements OnSharedPreferenceChangeListener {
    ...
    public static final String PREF_LANDSCAPE_EXTENDED_BUTTONS = "landscape_extended_buttons";
    ...
    public LoadoutManager(Context appContext) {
        _context = appContext;
        _prefs = AtakPreferences.getInstance(appContext);
    ...
    public String serializeToXML(LoadoutItemModel loadout) {
        ...
        // if the developer of the custom loadout was in extended landscape mode, it is likely going
        // to be that the developer wants the receiver to be able to view all of the icons.
        // only include this if it is true.
        if (_prefs.get(PREF_LANDSCAPE_EXTENDED_BUTTONS, false)) {
            sb.append(
                    "<entry key=\"landscape_extended_buttons\" class=\"class java.lang.Boolean\">true</entry>\n");
        }
    ...
    private LoadoutItemModel createDefaultLoadout() {
        ...
        // for extended landscape visualization
        def.setButton(5, "contacts");
        def.setButton(6, "goto");
        def.setButton(7, "rangeandbearing");
        def.setButton(8, "importmanager");
