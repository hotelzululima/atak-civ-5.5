...
import com.atakmap.android.missionpackage.event.MissionPackageEventProcessor2;
...
public class MissionPackageMapOverlay extends AbstractMapOverlay2 implements
    ...
    private final MissionPackageEventProcessor2 _eventProcessor;
    ...
    private boolean dpkgRootVisibilityToggle = true;
    ...
    public MissionPackageMapOverlay(MapView view,
        ...
        _eventProcessor = new MissionPackageEventProcessor2(_context,
                _view.getRootGroup());
        ...
        onSharedPreferenceChanged(_prefs.getSharedPrefs(),
                "dpgkRootVisibilityToggle");
        init();
    }

    private void init() {
        FileInfoPersistanceHelper.initialize(_context);
        ...
        for (MissionPackageListGroup group: _groups) {
    ...
    void importFile(final MissionPackageListGroup group, final File file,
        ...
        if (h == null) {
            ...
            EnumSet<ImportResolver.SortFlags> flags = hidden
                    ? EnumSet.of(ImportResolver.SortFlags.HIDE_FILE)
                    : EnumSet.noneOf(ImportResolver.SortFlags.class);

            final List<ImportResolver> matchingSorters = new ArrayList<>();
            List<ImportResolver> sorters = ImportFilesTask.GetKernelSorters(_context, false);
            for (ImportResolver sorter : sorters) {

                // Ignore APK installer - does not produce expected behavior
                // when adding to a data package
                if (autoImport && sorter instanceof ImportAPKResolver)
                    continue;
                ...
                if (sorter.match(file)) {
                    ...
                    if (p != null && contentType != null
                        ...
                        try {
                            ...
                            Log.e(TAG, "error importing the file " + file, e);
                        }
                    }
            ...
            for (ImportResolver sorter : matchingSorters)
                d.addButton(MarshalManager.marshal(sorter.getIcon(), gov.tak.api.commons.graphics.Drawable.class, Drawable.class), sorter.getDisplayableName());
    ...
    private void importFile(MissionPackageContent content, File file,
        ...
        resolver.beginImport(file, EnumSet.noneOf(gov.tak.api.importfiles.ImportResolver.SortFlags.class));
    }
    ...
    private final int START = 62222;
    private final int END = 62226;
    private int next = START;

    void toast(final String str) {
        if (!_disposed) {
            NotificationUtil.getInstance().postNotification(next++,
                    R.drawable.missionpackage_sent,
                    _context.getString(R.string.mission_package_name),  str,
                    new Intent("com.atakmap.android.missionpackage.MISSIONPACKAGE"),
                    true);
            if (next > END)
                next = START;
        }
    ...
    void promptExtractContent(final MissionPackageListGroup group,
        ...
        if (item instanceof MissionPackageListMapItem) {
            ...
            String cotXml = MissionPackageExtractor.ExtractCoT(
                    new File(group.getManifest().getPath()),
                    group.getManifest(),
                ...
                if (event != null && event.isValid()) {
                    ...
                    if (cp != null) {
                        CoordinateFormat cf = CoordinateFormat.find(
                                _prefs.get("coord_display_pref", _context
                                        .getString(
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
        ...
        if (key.equals("dpgkRootVisibilityToggle")) {
            dpkgRootVisibilityToggle = sharedPreferences.getBoolean(key,
                    dpkgRootVisibilityToggle);
        }
    }
}
