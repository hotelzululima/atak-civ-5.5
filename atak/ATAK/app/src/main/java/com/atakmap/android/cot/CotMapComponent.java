...
import com.atakmap.android.contact.AltTelephoneConnector;
...
import com.atakmap.android.cot.importer.CustomRoleImporter;
import com.atakmap.android.cot.importer.CustomRoleMarshal;
...
public class CotMapComponent extends AbstractMapComponent implements
    ...
    private static final CotDispatcher parallelInternalDispatcher = new CotDispatcher(
            CotDispatcher.getSharedConcurrentInternalDispatchTrackExecutor());
    private static final CotDispatcher parallelWaitInternalDispatcher = new CotDispatcher(
            CotDispatcher.getSharedConcurrentInternalDispatchTrackExecutor());
    ...
    private CotCacheManager _cotCacheManager;
    ...
    private RadialMenuSendReceiver radialItemSendReceiver;
    ...
    private String saAlternativeNumber;
    ...
    public void onCreate(Context context, Intent intent, final MapView view) {
        ...
        mapData.setMetaString("uniqueCotInstance", instanceUID.toString());

        phoneNumber = LocationMapComponent._fetchTelephonyLine1Number(context);
        updateSAContact(_prefs.getSharedPrefs());
        ...
        ImporterManager.registerImporter(
                new CustomRoleImporter(context, rolesManager));
        MarshalManager.registerMarshal(CustomRoleMarshal.INSTANCE);

        ImportExportMapComponent.getInstance().addImporterClass(
                ImportInPlaceResolver.fromMarshal(CustomRoleMarshal.INSTANCE));
        ...
        DocumentedIntentFilter onStartUriFilter = new DocumentedIntentFilter();
        onStartUriFilter.addAction(ATAKActivity.ONSTART_URI);
        AtakBroadcast.getInstance().registerReceiver(
                onStartUriReceiver, onStartUriFilter);

        DocumentedIntentFilter filter = new DocumentedIntentFilter();
        filter.addAction(RadialMenuSendReceiver.SEND_RADIAL_INTENT);
        AtakBroadcast.getInstance().registerReceiver(
                radialItemSendReceiver = new RadialMenuSendReceiver(view),
                filter);

        _updateCotPreferences(_prefs.getSharedPrefs());

        _reportingRate = new ReportingRate(this, _prefs.getSharedPrefs());

        _startListening(_prefs.getSharedPrefs());

        _setServerPorts(_prefs.getSharedPrefs());
        _prefs.registerListener(
        ...
        _cotCacheManager = new CotCacheManager();
        view.getMapEventDispatcher().addMapEventListener(MapEvent.ITEM_IMPORTED,
                _cotCacheManager);
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        try {
            ...
            if (locationMarker != null) {
                ...
                if (!ClearContentTask.isClearContent(_prefs.getSharedPrefs())) {
                    ...
                    _prefs.set("lastRecordedLocation", "" + agp.toString());
                }
            } else {
                Log.v(TAG,
        ...
        AtakBroadcast.getInstance().unregisterReceiver(radialItemSendReceiver);
        AtakBroadcast.getInstance().unregisterReceiver(onStartUriReceiver);
    ...
    public ImportResult processCotEvent(CotEvent event, Bundle extra) {
        ...
        if (event.getUID().equals(myUID)) {
            ...
            String mockingOption = _prefs.get("mockingOption", "WRGPS");
    ...
    private void updateSAContact(SharedPreferences preferences) {
        ...
        saAlternativeNumber = preferences.getString("saAlternativeNumber",
                null);
        urn = preferences.getString("saURN", null);
    }

    void addTadilJ(CotDetail detail, MetaDataHolder2 mapData) {
        if (mapData.hasMetaValue("tadiljId")) {
            if (mapData.hasMetaValue("tadiljSelfPositionType") &&
                    mapData.getMetaString("tadiljSelfPositionType", "")
                            .equals("J3.5")) {
                CotDetail tadilj = new CotDetail("uid");
                detail.addChild(tadilj);
                tadilj.setAttribute("tadilj",
                        mapData.getMetaString("tadiljId", null));
            } else {
                CotDetail tadilj = new CotDetail("__jtids");
                detail.addChild(tadilj);
                tadilj.setAttribute("jstn",
                        mapData.getMetaString("tadiljId", null));
            }
        }
    }
    ...
    private final TimerTask _checkStaleTask = new TimerTask() {
        @Override
        public void run() {
            _adapter.getCotMarkerSet().checkStaleTeams(_context,
                    _prefs.getSharedPrefs());
        }
    };
    ...
    private CotEvent getSelfEvent(final int offset) {
        ...
        final MetaDataHolder2 mapData = _mapView.getMapData();
        ...
        final String myType = mapData.getMetaString("deviceType", null);
        ...
        if (LocationManager.getInstance()
                .getPreferredLocationProvider() == null) {
            if (ATAKUtilities.findSelf(_mapView) != null)
                cotEvent.setHow(mapData.getMetaString("how", "h-e"));
            else
                cotEvent.setHow(mapData.getMetaString("how", "h-g-i-g-o"));
        } else {
            cotEvent.setHow(mapData.getMetaString("how", "m-g"));
        }

        AccessUtils.setAccessDefault(cotEvent, self);
        ...
        final String userRemarks = _prefs.get("userRemarks", "").trim();
        if (!FileSystemUtils.isEmpty(userRemarks)) {
            remarks.setInnerText(userRemarks.trim());
        ...
        // in this case sort the phone number order
        boolean preferAltPhone = _prefs.get("preferAltNumber", false);

        if (saHasPhoneNumber
                && LocationMapComponent.isValidTelephoneNumber(phoneNumber)) {
            contact.setAttribute(preferAltPhone ? "altphone" : "phone",
                    phoneNumber);
        }

        if (saHasPhoneNumber
                && LocationMapComponent
                        .isValidTelephoneNumber(saAlternativeNumber)) {
            contact.setAttribute(preferAltPhone ? "phone" : "altphone",
                    saAlternativeNumber);
        }
        ...
        final AttributeSet __group = new AttributeSet();
        final String deviceTeam = mapData.getMetaString("deviceTeam", null);
        if (deviceTeam != null)
            __group.setAttribute("team", deviceTeam);
        __group.setAttribute("atakRoleType",
                _prefs.get("atakRoleType", "Team Member"));
        final String customRole = _prefs.get("customRole", null);
        if (customRole != null)
            __group.setAttribute("customRole",
                    customRole);
        final String customRoleAbbreviation = _prefs
                .get("customRoleAbbreviation", null);
        if (customRoleAbbreviation != null)
            __group.setAttribute("customRoleAbbreviation",
                    customRoleAbbreviation);
        if (_prefs.get("customRoleUseTypeIcon", false))
            __group.setAttribute("displayAsSymbologyGraphic",
                    true);
        detail.addChild(gov.tak.platform.marshal.MarshalManager.marshal(
                GroupDetailHandler.toCotDetail(__group),
        ...
        final String readiness = mapData.getMetaString("readiness", null);
        if (readiness != null) {
            status.setAttribute("readiness", readiness);
        }
        ...
        if (mapData.hasMetaValue("archive")
                && mapData.getMetaBoolean("archive", false)) {
            CotDetail archive = new CotDetail("archive");
            detail.addChild(archive);
        }

        String mockLocationParentUID = mapData
                .getMetaString("mockLocationParentUID", null);
        String mockLocationParentType = mapData
                .getMetaString("mockLocationParentType", null);
    ...
    private RolesManager initializeRoles() {
        // load predefined foles
        RolesManager rolesManager = new RolesManager();
        File rolesdb = new File(_context.getCacheDir(), "roles.rb");
        ByteArrayOutputStream baos = new ByteArrayOutputStream(16 * 1024);
        if (FileSystemUtils.copyFromAssets(_context, "roles/roles.sqlite",
                baos)) {
            try (FileOutputStream fos = new FileOutputStream(rolesdb)) {
                fos.write(baos.toByteArray());
            } catch (IOException e) {
                Log.e(TAG, "Failed to write db", e);
            }
            DatabaseIface db = null;
            try {
                db = Databases.openDatabase(rolesdb.getAbsolutePath(), true);
                try (CursorIface result = db
                        .query("SELECT name, value FROM roles", null)) {
                    while (result.moveToNext()) {
                        try {
                            JSONObject rolesJson = new JSONObject(
                                    result.getString(1));
                            rolesManager.addRoles(Role.fromJSON(rolesJson));
                        } catch (Throwable t) {
                            Log.e(TAG, "Failed to parse [" + result.getString(0)
                                    + "] roles JSON", t);
                        }
                    }
                }
            } catch (Throwable t) {
                Log.e(TAG, "Failed to load roles database", t);
            } finally {
                if (db != null)
                    db.close();
            }
        }
        FileSystemUtils.delete(rolesdb);

        // load imported roles
        try {
            JSONArray importIndex = new JSONArray(
                    AtakPreferences.getInstance(_context)
                            .get(CustomRoleImporter.IMPORT_INDEX_KEY, "[]"));
            List<Integer> invalidIndices = new ArrayList<>(
                    importIndex.length());
            for (int i = importIndex.length() - 1; i >= 0; i--) {
                final File roleFile = new File(importIndex.getString(i));
                boolean imported = false;
                if (roleFile.exists()) {
                    try (FileInputStream strm = IOProviderFactory
                            .getInputStream(roleFile)) {
                        imported = rolesManager.importRoles(strm);
                    } catch (Exception e) {
                        Log.w(TAG, "Failed to import custom role file "
                                + roleFile.getName(), e);
                    }
                }
                if (!imported)
            ...
            if (!invalidIndices.isEmpty()) {
                for (Integer i : invalidIndices)
                    importIndex.remove(i);
                AtakPreferences.getInstance(_context).set(
                        CustomRoleImporter.IMPORT_INDEX_KEY,
                        importIndex.toString());
            }
        } catch (JSONException e) {
            AtakPreferences.getInstance(_context)
                    .remove(CustomRoleImporter.IMPORT_INDEX_KEY);
        }
    ...
    public IndividualContact getSelfContact(boolean bDeep) {
        ...
        if (saHasPhoneNumber
                && LocationMapComponent
                        .isValidTelephoneNumber(saAlternativeNumber)) {
            self.addConnector(new AltTelephoneConnector(saAlternativeNumber));
            //self.addConnector(new SmsConnector(saAlternativeNumber));
        }
     ...
     * @param detailName the name of the detail to remove
     * @return the detail that was removed or null if no detail was removed
     */
    ...
    private SharedPreferences.OnSharedPreferenceChangeListener _cotPrefsChangedListener = new SharedPreferences.OnSharedPreferenceChangeListener() {
        ...
        public void onSharedPreferenceChanged(
            ...
            if (key.equals("dispatchLocationCotExternal") ||
                    ...
                    key.equals("dispatchTAKVersionNumber")) {
            ...
            if (key.equals("listenPort") ||
                ...
                Thread t = new Thread("startListeningExternalGps") {
                    public void run() {
            ...
            if (key.equals("saSipAddressAssignment")
                    ...
                    || key.equals("saAlternativeNumber")
    ...
    synchronized private void _startListening(SharedPreferences cotPrefs) {
        ...
        /**
         * start up the mocking socket. the decision to use the data can be done independently of
         * receipt of the data.
         */
    ...
    private void _updateCotPreferences(SharedPreferences cotPrefs) {
        ...
        _hideLocation = cotPrefs.getBoolean("dispatchLocationHidden",
                false);
        _publishVersion = cotPrefs.getBoolean("dispatchTAKVersionNumber", true);
        Marker locationMarker = ATAKUtilities.findSelf(_mapView);
        if (locationMarker != null) {
    ...
    /**
     * Get a CotDispatcher configured to dispatch internal CoT on the parallel processing pipeline.
     * The method returns immediately after dispatch and does not wait until processing is finished.
     *
     * @return CotDispatcher instance for parallel CoT pipeline
     */
    public static CotDispatcher getParallelInternalDispatcher() {
        return parallelInternalDispatcher;
    }

    /**
     * Get a CotDispatcher configured to dispatch internal CoT on the parallel processing pipeline.
     * The method blocks until the dispatched CoT is processed.
     *
     * @return CotDispatcher instance for parallel CoT pipeline
     */
    public static CotDispatcher getParallelWaitInternalDispatcher() {
        return parallelWaitInternalDispatcher;
    }
    ...
    void setCotRemote(CotServiceRemote remote) {
        ...
        parallelInternalDispatcher.setDispatchFlags(
                DispatchFlags.INTERNAL | DispatchFlags.NO_WAIT);
        parallelInternalDispatcher.setDispatchFlags(DispatchFlags.INTERNAL);
        _adapter.getCotMarkerSet().setCotRemote(externalDispatcher);
    }

    public void stale(String[] uid) {
        if (_adapter != null
                && _prefs.get("staleRemoteDisconnects", true)) {
     ...
     * @param connected
     */
    void connected(TAKServer port, boolean connected) {
    ...
    private boolean _publishVersion;
    private final BroadcastReceiver onStartUriReceiver = new BroadcastReceiver() {
        @NonNull
        public String toString() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ATAKActivity.ONSTART_URI)) {
                ...
                if (u != null && "com.atakmap.app/enroll".equals(
                        u.getHost() + u.getPath())) {
                    ...
                    if (host == null || username == null || token == null)
                        return;

                    AlertDialog.Builder ad = new AlertDialog.Builder(_context);
                    ad.setTitle(R.string.enroll_client_message);
                    ad.setCancelable(false);
                    ad.setMessage(String.format(LocaleUtil.getCurrent(),
                            _context.getString(
                                    R.string.quick_enrollment_question),
                            username, host));
