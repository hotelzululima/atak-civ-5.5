...
public class OnlineLayersDownloadManager {
    ...
    public interface Callback {
        void receiveDownloadArea();

        void cancelDownloadSelect();
    }
    ...
    private LayerDownloader.RequestBuilder downloadRequest;

    private final String callbackAlias;

    OnlineLayersDownloadManager(MapView mv,
            LayersManagerBroadcastReceiver lmbr,
            View progressView, View mobileTools) {
        this(mv, new CallbackAdapter(lmbr), progressView, mobileTools);
    }
    ...
    OnlineLayersDownloadManager(MapView mv,
        ...
        this.downloader = LayerDownloader.getInstance(mv.getContext());
        callback = lmbr;
        ...
        downloadRequest = new LayerDownloader.RequestBuilder();
        callbackAlias = UUID.randomUUID().toString();
    }
            ...
            public void run() {
                try {
                    if (visible) {
                        ...
                        queueProg.setText(String.format(Locale.getDefault(), context.getString(R.string.num_queued_tilesets), queue));
                    } else
    ...
    void promptSelectRegion() {
        ...
        i.putExtra(DownloadAndCacheService.JOB_UID, callbackAlias);
        Bundle b = new Bundle();
    ...
    public void finishRegionSelect(Intent i) {
        if (!selectingRegion)
            return;
        ...
        if (!(mi instanceof Shape)) {
            callback.cancelDownloadSelect();
            return;
        }
        shape = (Shape) mi;
        downloadRequest.setShape(shape);
        downloadRequest
                .setExpandDistance(i.getDoubleExtra("expandDistance", 0));
    ...
    void startDownload(String title, String cacheUri,
            ...
            String importMime, String importContent) {

        downloadRequest.setTitle(title);
        downloadRequest.setResolution(minRes, maxRes);
        downloadRequest.setCacheURI(cacheUri);
        downloadRequest.setDefaultDownloadDirectory(defaultDownloadDir);
        downloadRequest.setSourceURI(
                ((MobileImagerySpec) layerSelection.getTag()).desc.getUri());
        if (importMime != null || importContent != null)
            downloadRequest.setImportParams(importMime, importContent);
        downloadRequest.setCallback(callbackAlias);
        if (downloader.startDownload(downloadRequest)) {
            resetProgressBar();
            downloadPB.setMax(downloadRequest.calculateTileCount());
        }
    }

    public boolean isDownloading() {
        return DownloadAndCacheService.getInstance().isDownloading();
    }
    ...
    public void setCallback(LayerDownloader.Callback cb) {
        downloader.setCallback(callbackAlias, cb);
    }
    ...
    int getTilesOverArea2(String layerUri, double minRes, double maxRes) {
        downloadRequest.setSourceURI(layerUri);
        downloadRequest.setResolution(minRes, maxRes);
        return downloadRequest.calculateTileCount();
    }
    ...
    double estimateMinimumResolution(TileMatrix tiles, double maxResolution) {
        ...
        GeoPoint ul = downloadRequest.getUpperLeft();
        GeoPoint lr = downloadRequest.getLowerRight();
        if (ul == null || lr == null)
        ...
        int maxResZoom = zoomLevels.length - 1;
        for (int i = 0; i < zoomLevels.length; i++) {
            ...
            // check if the remaining zoom levels higher resolution than
            // requeseted max resolution
        ...
        if (zoomLevels[maxResZoom].resolution > maxResolution)
            return zoomLevels[maxResZoom].resolution;

        // assume 3 LODs
        return zoomLevels[Math.max(maxResZoom - 3, 0)].resolution;
    }
    ...
    final static class CallbackAdapter implements Callback {

        final LayersManagerBroadcastReceiver lmbr;

        CallbackAdapter(final LayersManagerBroadcastReceiver lmbr) {
            this.lmbr = lmbr;
        }

        @Override
        public void receiveDownloadArea() {
            lmbr.receiveDownloadArea();
        }

        @Override
        public void cancelDownloadSelect() {
            lmbr.onKey(null, KeyEvent.KEYCODE_BACK, null);
        }
    }
}
