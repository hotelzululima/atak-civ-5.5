...
class NativeLayerSelectionAdapter extends LayerSelectionAdapter
    ...
    private View getFileView(LayerSelection selection, DatasetDescriptor desc,
        ...
        if (h == null) {
            ...
            h.send = row.findViewById(R.id.file_send);
            h.send.setTag(h);
        ...
        File f = URIHelper.getFile(desc.getUri());
        if (f == null)
            f = new File(desc.getUri());
        if (f.exists())
            h.send.setVisibility(View.VISIBLE);
        else
            h.send.setVisibility(View.INVISIBLE);
                ...
                b.setPositiveButton(R.string.yes,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                String fPath = URIHelper.getContent((desc.getUri().startsWith(URIScheme.ZIP)) ? URIScheme.ZIP : URIScheme.FILE, desc.getUri());
                                if (null == fPath)
                                    fPath = desc.getUri();
                                else
                                    fPath = Uri.decode(fPath);
                                FileSystemUtils.delete(new File(fPath));
