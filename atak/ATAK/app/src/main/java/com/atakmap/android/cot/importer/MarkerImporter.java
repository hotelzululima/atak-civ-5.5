...
public class MarkerImporter extends MapItemImporter {
    ...
    protected ImportResult importMapItem(MapItem existing, CotEvent event,
        ...
        AccessUtils.setAccessFromCot(event, marker);
        ...
        String milSymBefore = marker.getMetaString("milsym", null);
        ...
        // Add the marker to the map if isn't already. Supports lookup by handlers that require a reference to them
        ...
        if (!FileSystemUtils.isEquals(teamBefore, teamAfter)) {
            // since the first addition to the map triggered filtering, there was a
            // filterOverlayId assignment - we need to re-trigger filtering by removing the
            // assignment.
            marker.removeMetaData("filterOverlayId");
            needsRefresh = true;
        }
        ...
        String milSymAfter = marker.getMetaString("milsym", null);
        if (!FileSystemUtils.isEquals(milSymBefore, milSymAfter)) {
            needsRefresh = true;
        }
        ...
        if (needsRefresh) {
            ...
            if (prefs.get("showTadiljIds", false)) {
                marker.setSummaryFlag(true);
            } else {
                marker.setSummaryFlag(false);
            }

            // if the marker is newly created, skip icon refresh
            final Boolean adapt_marker_icon = (existing == null &&
                    marker.hasMetaValue("adapt_marker_icon"))
                            ? marker.getMetaBoolean("adapt_marker_icon", true)
                            : null;
            if (existing == null)
                marker.getMetaBoolean("adapt_marker_icon", false);
            marker.refresh(_mapView.getMapEventDispatcher(), extras,
                    this.getClass()); // notify listeners of the change
            if (existing == null) {
                if (adapt_marker_icon != null)
                    marker.setMetaBoolean("adapt_marker_icon",
                            adapt_marker_icon);
                else
                    marker.removeMetaData("adapt_marker_icon");
            }
