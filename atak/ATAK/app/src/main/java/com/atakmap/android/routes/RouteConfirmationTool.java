...
import com.atakmap.android.user.geocode.GeocodeConverter;
    ...
    public void onMapEvent(MapEvent event) {
        ...
        if (t.equals(MapEvent.ITEM_CLICK) || t.equals(MapEvent.MAP_CLICK)
            ...
            Thread lookup = new Thread(new Runnable() {
                @Override
                public void run() {
                    final GeocodeManager gm = GeocodeManager
                            .getInstance(_context);
                    List<Address> l = gm.getSelectedGeocoder()
                            .getLocation(point.get());
                    if (FileSystemUtils.isEmpty(l)
                            || _marker.getGroup() == null)
                        return;
                    _address = GeocodeConverter.getAddressString(l.get(0));
