...
class MobileLayerSelectionAdapter extends LayerSelectionAdapter
    ...
    private static final String DEFAULT_PREF_SELECTED = TAG + ".selected";
    private static final String DEFAULT_PREF_OFFLINE_ONLY = TAG
            + ".isOfflineOnly";
    ...
    private OnlineLayersDownloadManager _downloader;

    private final String _prefSelected;
    private final String _prefOfflineOnly;
    ...
    public MobileLayerSelectionAdapter(CardLayer cardLayer,
            MobileImageryRasterLayer2 layer,
            FeatureDataStore2 outlinesDataStore,
            MapView mapView, Context context,
            boolean showVisibilityToggle,
            String prefSelected,
            String prefOfflineOnly) {

        super(layer, outlinesDataStore, mapView, context);

        this.cardLayer = cardLayer;
        this.layer = layer;
        this.showVisibilityToggle = showVisibilityToggle;
        _prefSelected = (prefSelected == null) ? DEFAULT_PREF_SELECTED
                : prefSelected;
        _prefOfflineOnly = (prefOfflineOnly == null) ? DEFAULT_PREF_OFFLINE_ONLY
                : prefOfflineOnly;

        final boolean offlineOnly = _prefs.get(_prefOfflineOnly, false);
        ...
        final String selected = _prefs.get(_prefSelected, null);
    ...
    void setDownloader(OnlineLayersDownloadManager downloader) {
    ...
    public void dispose() {
        ...
        _prefs.set(_prefOfflineOnly, isOfflineOnly());
        calc.dispose();
    ...
    public void onSharedPreferenceChanged(SharedPreferences p, String key) {
        ...
        if (key.equals(_prefSelected)) {
            String selected = p.getString(_prefSelected, null);
        ...
        if (rebuild || offline) {
            ...
            for (final LayerSelection sel : _selections) {
                ...
                try {
                    ...
                    // don't double count the cache
                    if (spec.cache != null && localFiles.remove(spec.cache))
                        spec.count--;
                    if (spec.count > 0) {
                        ...
                        for (File localFile : localFiles) {
                            if (IOProviderFactory.exists(localFile))
            ...
            if (offline) {
                ...
                Collections.sort(_selections, getSortComparator());
            }
        }
    ...
    int getTileNum2(String layerUri, double minRes, double maxRes) {
        if (_downloader == null)
            return 0;
        long tiles = _downloader.getTilesOverArea2(layerUri, minRes, maxRes);
        if (tiles > LayersManagerBroadcastReceiver.TILE_DOWNLOAD_LIMIT
                || tiles < 0)
            return LayersManagerBroadcastReceiver.TILE_DOWNLOAD_LIMIT + 1;
        else
            return (int) tiles;
    }
    ...
    protected int getTileCount2(MobileImagerySpec spec, double minRes,
        ...
        if (res != null && maxRes < res.defaultMinRes) {
            ...
            int tileApprox = getTileNum2(spec.desc.getUri(), dmr, dmr);
        ...
        return getTileNum2(spec.desc.getUri(), minRes, maxRes);
    }
    ...
    protected View getViewImpl(final LayerSelection tsInfo, final int position,
        ...
        if (!aliasFilters.isEmpty()) {
            for (Map.Entry<String, SelectionAliasFilter> alias : aliasFilters
                    .entrySet()) {
                if (alias.getValue().matches(h.selection)) {
                    title = alias.getKey();
                    break;
                }
            }
        ...
        h.vizBtn.setVisibility(showVisibilityToggle ? View.VISIBLE : View.GONE);
        h.vizBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                layer.setVisible(h.selection.getName(),
                        !layer.isVisible(h.selection.getName()));
                notifyDataSetChanged();
            }
        });
        h.vizIcon.setVisible(layer.isVisible(h.selection.getName()));
        ...
        if (isSelection && expandedLayout && downloadable) {
                ...
                if (res == null) {
                    ...
                    try {
                        client = TileClientFactory.create(spec.desc.getUri(),
                                null, null);
                        if (client != null) {
                            res.defaultMinRes = _downloader
                                    .estimateMinimumResolution(client,
                                            res.maxAvailableRes);
            ...
            h.outlineBorder.setVisibility(
                    (outlinesDatastore != null) ? View.VISIBLE : View.GONE);
    ...
    protected Comparator<LayerSelection> getSortComparator() {
        if (layer.getExtension(LayerZOrderControl.class) != null) {
            final LayerZOrderControl zorder = layer
                    .getExtension(LayerZOrderControl.class);
            return new Comparator<LayerSelection>() {
                String getName(LayerSelection ls) {
                    do {
                        if (!(ls.getTag() instanceof MobileImagerySpec))
                            break;
                        final MobileImagerySpec spec = (MobileImagerySpec) ls
                                .getTag();
                        if (spec.parent == null)
                            break;
                        return spec.parent.getName();
                    } while (false);
                    return ls.getName();
                }

                @Override
                public int compare(LayerSelection a, LayerSelection b) {
                    int z = zorder.getPosition(getName(a))
                            - zorder.getPosition(getName(b));
                    if (z != 0)
                        return z;
    ...
    @Override
    boolean matchesSelection(String selection, LayerSelection ls) {
        if (super.matchesSelection(selection, ls))
            return true;
        if (!(ls.getTag() instanceof MobileImagerySpec))
            return false;
        final MobileImagerySpec spec = (MobileImagerySpec) ls.getTag();
        if (spec.parent == null || spec.parent == ls)
            return false;
        return matchesSelection(selection, spec.parent);
    }
    ...
    public void setSelected(final String sstr) {
        for (LayerSelection selection : _selections) {
            ...
            if (selection.getName().equalsIgnoreCase(sstr)) {
                ...
                _prefs.set(_prefSelected, curr.toString());
                return;
            }
        }
    }
    ...
    public boolean isOfflineOnly() {
        return isOfflineOnly(this.layer
                .getExtension(OnlineImageryExtension.class));
    }

    private boolean isOfflineOnly(OnlineImageryExtension svc) {
    ...
    public void setOfflineOnly(boolean offline) {
        OnlineImageryExtension svc = this.layer
                .getExtension(OnlineImageryExtension.class);
        if (offline == this.isOfflineOnly(svc))
            return;
        if (svc != null)
            svc.setOfflineOnlyMode(offline);

        setOnlyViewportDisplay(!offline);
        invalidate(true);
        ...
        _prefs.set(_prefOfflineOnly, offline);
    }
