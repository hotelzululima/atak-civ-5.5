...
public class RouteEditTool extends EditablePolylineEditTool implements
    ...
    public boolean onToolBegin(Bundle extras) {
        ...
        // send end nav here and in routecreationtool, send endtool in the handler for startnav; in
        // order to bridge between tools -> [navigation] mode
        // since they're at least currently two very separate concepts. nav mode could become a tool
        // at some point maybe?
    ...
    @Override
    protected void initEditMapClick() {
        // do not do anything
    }
            ...
            switch (e) {
                ...
                case MapEvent.ITEM_LONG_PRESS: {
                    ...
                    if (!(mi instanceof PointMapItem)) {
                        if (!_route.getUID().equals(mi.getUID()))
                            addWayPoint(event, _route.getNumPoints());
                    } else {
                        if (!_route.hasMarker((PointMapItem) mi)) {
                            addWayPoint(
                                    ((PointMapItem) mi).getGeoPointMetaData(),
                                    _route.getNumPoints());
                        }
                    }
                    break;
    ...
    private boolean addControlPoint(GeoPointMetaData gp, int index) {
        if (gp == null)
            return false;
        MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                MetricsUtils.EVENT_MAPITEM_CHANGED, "RouteEditTool",
                _route, MetricsUtils.EVENT_STATUS_SUCCESS);
        ...
        public void onMapEvent(MapEvent event) {
            ...
            if (e.equals(MapEvent.MAP_DRAW)
                ...
                if (_drawShape == null) {
                    ...
                    _drawShape.setStrokeStyle(_route.getStrokeStyle());
            ...
            // Finish drawing route segment
                ...
                if (simplified != null) {
                    for (GeoPoint gp : simplified) {
                        double hae = ATAKUtilities.getElevation(
