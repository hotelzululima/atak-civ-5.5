...
public class GLPolyline extends GLShape2 implements
    ...
    private AttributeSet segmentLabels;
    ...
    /** when a closed shape is extruded, this is the surface pass render of the base, so that terrain
     * clipping is visualized, but shape still extruded */
    private final GLBatchPolygon extrudedBase;
    ...
    public GLPolyline(MapRenderer surface, Polyline subject) {
        ...
        // Base rendered when the shape is extruded
        extrudedBase = new GLBatchPolygon(surface);
        extrudedBase.setAltitudeMode(AltitudeMode.ClampToGround);
        extrudedBase.setTesselationThreshold(threshold);
    ...
    private void refreshStyle() {
        ...
        if (MathUtils.hasBits(_subject.getRenderHints(),
        ...
        if (basicStyle == Polyline.BASIC_LINE_STYLE_DASHED)
            ...
            BasicStrokeStyle bg = new BasicStrokeStyle(
                    0xFF000000 & renderColor, this.strokeWeight + 2f);
            s = new CompositeStyle(new Style[] {
                    bg,
                    new BasicStrokeStyle(renderColor, this.strokeWeight)
            });
        } else {
            s = new BasicStrokeStyle(renderColor, this.strokeWeight);
        }
        ...
        BasicFillStyle extrudedBaseStyle = new BasicFillStyle(
                getExtrudedPolyColor());
        ...
        runOnGLThread(new Runnable() {
            ...
            public void run() {
                ...
                extrudedBase.setStyle(extrudedBaseStyle);
    ...
    protected void updatePointsImpl(GeoPoint center, GeoPoint[] points) {
        ...
        if (points.length > 0 && _closed) {
            ls.addPoint(points[0].getLongitude(), points[0].getLatitude(),
                    Double.isNaN(points[0].getAltitude()) ? centerPtAlt
        ...
        // XXX-- trying to use the same linestring for extrudedBase
        // causes issues with native management
        LineString extrudedBaseLs = (LineString) ls.clone();
        ...
        runOnGLThread(new Runnable() {
            ...
            public void run() {
                ...
                // really only needed if closed
                extrudedBase.setGeometry(new Polygon(extrudedBaseLs));
    ...
    public void onLabelsChanged(Polyline polyline) {
        final AttributeSet labels = polyline.getLabels();
        runOnGLThread(new Runnable() {
            @Override
            public void run() {
    ...
    public void draw(GLMapView ortho, int renderPass) {
        ...
        if (_hasHeight && numPoints > 0 && !_nadirClamp) {
            // draw the bottom of the extruded poly in surface mode to get around terrain clipping
            if (surface && _closed) {
                validateImpl(altitudeMode);
                extrudedBase.draw(ortho);
                ...
                if (renderPolygon) {
                    ...
                    if (rebuildExtrusionVertices) {
                        _3dPoints.clear();
                        for (int i = 0; i < _3dPointsPreForward
                                .limit(); i += 3) {
                    ...
                    int color = getExtrudedPolyColor();
    ...
    private void validateImpl(AltitudeMode altitudeMode) {
        if (implInvalid) {
            updatePointsImpl();
            impl.setAltitudeMode(altitudeMode);
            implInvalid = false;
    ...
    private int getExtrudedPolyColor() {
        ...
        // so extrudedBase + side + top is discernible
        if (_closed)
            a /= 2;
    ...
    private SegmentLabel buildTextLabel(
        ...
        ortho.getLabelManager().setText(lbl.id, text);
    ...
    private void validateSegmentLabels(GLMapView ortho) {
        ...
        if (segmentLabels != null) {
            ...
            double minGSD;
            for (String key : segmentLabels.getAttributeNames()) {
                labelBundle = segmentLabels.getAttributeSetAttribute(key);
    ...
    protected static RectF getDefaultWidgetViewF(MapRenderer ctx) {
        ...
        float right = ((GLMapView) ctx).currentPass.focusx * 2;
        // Could be in portrait mode as well, so change the bottom accordingly
        float top = ((GLMapView) ctx).currentPass.focusy * 2;
    ...
    private int findMiddleVisibleSegment(GLMapView ortho) {
        ...
        if (points.length > 0) {
            ...
            PointF endPt1 = ortho.currentPass.scene.forward(points[middle],
                    ortho.scratch.pointF);
            PointF endPt2 = ortho.currentPass.scene.forward(points[middle + 1],
                    ortho.scratch.pointF);
        ...
        if (points.length > 3) {
            ...
            PointF endPt1 = ortho.currentPass.scene.forward(points[midmid],
                    ortho.scratch.pointF);
            PointF endPt2 = ortho.currentPass.scene.forward(points[midmid + 1],
                    ortho.scratch.pointF);
            boolean contained = segContained(endPt1, endPt2, visibleView);
            if (contained)
                return midmid;

            endPt1 = ortho.currentPass.scene.forward(points[midmid * 3 - 1],
                    ortho.scratch.pointF);
            endPt2 = ortho.currentPass.scene
        ...
        for (int i = 0; i < points.length - 1; i++) {
            ...
            PointF endPt1 = ortho.currentPass.scene.forward(points[i],
                    ortho.scratch.pointF);
            PointF endPt2 = ortho.currentPass.scene.forward(points[i + 1],
                    ortho.scratch.pointF);
            boolean contained = segContained(endPt1, endPt2, visibleView);
            if (contained)
                return i;
