...
public class RouteElevationChart extends GraphicalView implements
    ...
    private double _startHeight = Double.NaN;
    private double _endHeight = Double.NaN;
    ...
    public RouteElevationChart(Context context, LineChart lineChart) {
        ...
        initPanListener();
        initZoomListener();
        AtakPreferences.getInstance(context).registerListener(this);
        renderLine = AtakPreferences.getInstance(getContext())
                .get("elevation_chart_lineofsight_visible", false);
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
        ...
        if (s == null)
            return;
        if (s.equals("elevation_chart_lineofsight_visible")) {
            renderLine = AtakPreferences.getInstance(getContext()).get(s,
                    false);
            repaint();
        }
    }
    ...
    /**
     * Sets the start and end heights for a "hi-lo" threshold line to be
     * rendered over the elevation profile. The line is only rendered if both
     * {@code startHeight} and {@code endHeight} are not equal to
     * {@link Double#NaN}.
     *
     * <P>Heights <B>MUST</B> be specified in the same units/reference as the
     * data series!!!
     *
     * @param startHeight   The start height or {@link Double#NaN} if not used
     * @param endHeight     The end height or {@link Double#NaN} if not used
     */
    void setHighLow(double startHeight, double endHeight) {
    ...
    private void setupLinePaint() {
        ...
        _hiLoPaint = new Paint();
        _hiLoPaint.setStyle(Paint.Style.STROKE);
        _hiLoPaint.setColor(this.getContext().getResources()
                .getColor(android.R.color.holo_red_light));
        _hiLoPaint.setStrokeWidth(2f);
    }
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        // XXX -
        if (renderLine && !Double.isNaN(_startHeight)
                && !Double.isNaN(_endHeight)) {
            scratch[0] = getMinXFromSeries();
            scratch[1] = _startHeight;
            final double[] sxy = toScreenPoint(scratch);

            scratch[0] = getMaxXFromSeries();
            scratch[1] = _endHeight;
            final double[] exy = toScreenPoint(scratch);
            canvas.drawLine((float) sxy[0], (float) sxy[1], (float) exy[0],
                    (float) exy[1], _hiLoPaint);
        }
    }
