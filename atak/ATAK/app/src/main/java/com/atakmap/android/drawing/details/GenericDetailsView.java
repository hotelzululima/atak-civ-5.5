...
public abstract class GenericDetailsView extends RelativeLayout implements
    ...
    protected CheckBox _cpVisibilityCB;
    ...
    protected TextView marking;

    protected View cotAuthorLayout;
    protected ImageView cotAuthorIconButton;
    protected ImageView cotAuthorPanButton;
    protected TextView _authorText;
    protected TextView _productionTimeText;

    /**
     * Helper function used to set the classification banner under additional information
     * @param _item the point map item to use
     * @param _marking the marking field that might be set to the address depending on the checkbox.
     */
    public static void setClassification(MapItem _item, TextView _marking) {
        ...
        final String access = _item.getMetaString("access", "Undefined");
        final String caveat = _item.getMetaString("caveat", null);
        final String releasableTo = _item.getMetaString("releasableTo", null);

        StringBuilder form = new StringBuilder();

        if (access.equals("Unclassified") && caveat != null
                && caveat.equals("CUI"))
            form.append("CUI");
        else {
            form.append(access);
            if (!FileSystemUtils.isEmpty(caveat))
                form.append("//").append(caveat);
        }

        if (!FileSystemUtils.isEmpty(caveat) && caveat.contains("REL TO")) {
            if (!FileSystemUtils.isEmpty(releasableTo))
                form.append("//").append(releasableTo);
        }

        _marking.setText(form.toString());
    ...
    protected void onFinishInflate() {
        ...
        urlUiHandler.update(_item);

        marking = this.findViewById(R.id.markingField);
        ...
        _cpVisibilityCB = findViewById(R.id.cpVisibilityCB);
        if (_cpVisibilityCB != null) {
            _cpVisibilityCB
                    .setOnCheckedChangeListener(new OnCheckedChangeListener() {
                        @Override
                        public void onCheckedChanged(
                                CompoundButton compoundButton, boolean b) {
                            if (_item instanceof CenterPointControl)
                                ((CenterPointControl) _item)
                                        .setCenterPointVisible(b);

                            _drawPrefs.set(
                                    "drawing.centerpointvisible.laststate", b);
                        }
                    });
    ...
    public boolean setItem(MapView mapView, MapItem item) {
        ...
        setClassification(_item, marking);

        if (_cpVisibilityCB != null) {
            if (_item instanceof CenterPointControl)
                _cpVisibilityCB.setChecked(
                        ((CenterPointControl) _item).isCenterPointVisible());
        }

        urlUiHandler.update(_item);

        long lastSeenTime = _item.getMetaLong("lastUpdateTime", 0);
        if (lastSeenTime > 0) {
