...
public class CamLockerReceiver extends BroadcastReceiver implements
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(LOCK_CAM)) {
            ...
            final boolean bCenter = intent.getBooleanExtra("center", true);
            final boolean bDisplayRelock = intent
                    .getBooleanExtra("displayRelock", true);
            unlockItem(bCenter);
            if (bDisplayRelock)
                displayRelockWidget();
        ...
        if (!suppressSnapback()) {
            ...
            if (forceCenter) {
                ...
                if (!disableFloatToBottom && trackupMode
                    ...
                    final float focusAdjustWeightTilt = 1f - (float) ((MathUtils
                            .clamp(90d + sm.camera.elevation, 15d, 30d) - 15d)
                            / 15d);
                    final float focusAdjustWeightZoom = (float) ((MathUtils
                            .clamp(OSMUtils.mapnikTileLevel(sm.gsd), 6d, 10d)
                            - 6d) / 4d);
                    final float focusAdjustWeight = Math
                            .min(focusAdjustWeightTilt, focusAdjustWeightZoom);

                    final float focusAdj = (sm.height *
                            (_mapView.isPortrait()
                                    ? (float) BOTTOM_PORTRAIT_PERCENT
                                    : (float) BOTTOM_LANDSCAPE_PERCENT))
                            - focusy;

                    focusy = focusy + focusAdjustWeight * focusAdj;
                }
            }

            if (focusy == sm.focusy) {
                // focus is _center_, do simple pan
                CameraController.Programmatic.panTo(
                        renderer,
                        focuslla,
                        !forceCenter);
            } else {
                // use interactive pan to attempt to align the focus point with
                // the adjusted focus screen position
    ...
    public void onMapEvent(MapEvent event) {
        ...
        // Stop lock on when target is removed
            ...
            final Intent toggle = new Intent(UNLOCK_CAM);
            toggle.putExtra("displayRelock", false);
