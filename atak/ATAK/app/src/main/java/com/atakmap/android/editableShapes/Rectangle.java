...
import com.atakmap.android.maps.CenterPointControl;
...
public abstract class Rectangle extends AssociationSet
    ...
    // if the center point that is auto generated should be visible
    private boolean centerPointVisible = true;
    ...
    protected Rectangle(MapGroup mapGroup, String uid) {
        ...
        _unitPrefs = new UnitPreferences(mv.getContext());
     ...
     */
    static private GeoPointMetaData fillAltitude(final GeoPointMetaData gpm) {
        GeoPoint gp = gpm.get();
        return ATAKUtilities.getElevationMetadata(gp.getLatitude(),
                gp.getLongitude());
    }
    ...
    public void setLength(double length) {
        if (_center == null)
            throw new IllegalStateException(
                    "center cannot be null when calling setHeight");
    ...
    public void setWidth(double width) {
        if (_center == null)
            throw new IllegalStateException(
                    "center cannot be null when calling setWidth");
    ...
    public void setCenterMarker(Marker marker) {
        ...
        if (marker == null) {
            ...
            marker.setVisible(centerPointVisible && getVisible());
        }
    ...
    /**
     * Allow for toggling the visibility of the center point in the rectangle.
    ...
    public void setCenterPointVisible(boolean visible) {
        centerPointVisible = visible;
        final Marker m = getCenterMarker();
        if (m != null)
            m.setVisible(visible && getVisible());
    }

    /**
     * Returns the current state of the center point visibility
    ...
    @Override
    public void setCenterPointLabelVisible(boolean visible) {
    ...
    public void setBasicLineStyle(int basicLineStyle) {
        ...
        if (_filledShape != null)
            _filledShape.setBasicLineStyle(basicLineStyle);
    ...
    public void setFilled(boolean filled) {
        if (filled) {
            ...
            if (points.length < 2)
                return;
            final boolean create = (_filledShape == null);
            if (create) {
                _filledShape = new SimpleRectangle(
                        UUID.randomUUID().toString());
                _filledShape.setMetaBoolean("nevercot", true);
                _filledShape.setMetaBoolean("addToObjList", false);
                _filledShape.setClickable(getClickable());
                _filledShape.setEditing(getEditing());
                _filledShape.setRadialMenu(getMenuPath());
                _filledShape.setMetaBoolean("editable",
                        getMetaBoolean("editable", true));
                _filledShape.setMetaString("shapeUID", getUID());
                _filledShape.setTitle(getTitle());
            }
            final GeoPoint center = getCenter().get();
            GeoPoint[] corners = new GeoPoint[Math.min(4, points.length)];
            for (int i = 0; i < corners.length; i++) {
                corners[i] = new GeoPoint(
                        points[i].get().getLatitude(),
                        points[i].get().getLongitude(),
                        center.getAltitude());
            }
            ((SimpleRectangle) _filledShape).setCenter(getCenter());
            _filledShape.setPoints(corners);
            ...
            _filledShape.setStrokeColor(getStrokeColor());
            setStyle(getStyle() | Shape.STYLE_FILLED_MASK);
            _filledShape.setZOrder(getZOrder() + Z_ORDER_FILL);
            _filledShape.setHeight(getHeight());
            if (!Double.isNaN(getHeight()) && getHeight() != 0d)
                _filledShape.setHeightStyle(Polyline.HEIGHT_STYLE_POLYGON
                        | Polyline.HEIGHT_STYLE_OUTLINE);
            else
                _filledShape.setHeightStyle(Polyline.HEIGHT_STYLE_NONE);
            _filledShape.setVisible(getVisible());
            if (create)
                _childMapGroup.addItem(_filledShape);
        } else {
            if (_filledShape != null) {
                _filledShape.removeFromGroup();
                _filledShape = null;
            }
    ...
    protected void onVisibleChanged() {
        ...
        if (_center != null) {
            _center.setVisible(centerPointVisible && visible);
        }
    ...
    public void setMetaBoolean(String key, boolean value) {
        ...
        if (key.equals("editable")) {
            for (Association a : _lines) {
                a.getMarker().setMetaBoolean(key, value);
                a.setMetaBoolean(key, value);
            }
            if (_center != null)
                _center.setMetaBoolean(key, value);
        } else if (key.equals("editing")) {
            for (Association a : _lines) {
                a.getMarker().setEditing(value);
                a.setEditing(value);
            }
            for (MapItem p : _points)
                p.setEditing(value);
            if (_center != null)
                _center.setEditing(value);
            if (_filledShape != null)
                _filledShape.setEditing(value);
        }
    }
    ...
    protected void onGroupChanged(boolean added, MapGroup group) {
        if (added) {
            ...
            for (PointMapItem pointMapItem : _points)
                _childMapGroup.addItem(pointMapItem);
            for (Association association : _lines)
                _childMapGroup.addItem(association);
            if (_filledShape != null)
                _childMapGroup.addItem(_filledShape);
            if (isCenterShapeMarker())
                _childMapGroup.addItem(_center);
            group.addGroup(_childMapGroup);
    ...
    protected void onPointsChanged() {
        ...
        for (int i = 0; i < corners; i++) {
            GeoPoint p = gp[i].get();
            double alt = ATAKUtilities.getElevation(p.getLatitude(),
                    p.getLongitude(), null);
        ...
        if (_filledShape != null) {
            final GeoPoint center = getCenter().get();
            GeoPoint[] points = new GeoPoint[Math.min(4, gp.length)];
            for (int i = 0; i < points.length; i++) {
                points[i] = new GeoPoint(
                        gp[i].get().getLatitude(),
                        gp[i].get().getLongitude(),
                        center.getAltitude());
            }
            ((SimpleRectangle) _filledShape).setCenter(getCenter());
            _filledShape.setPoints(points);
        }
    ...
    private void _onCornerChanged(PointMapItem corner) {
        ...
        double[] rbCC = DistanceCalculations.computeDirection(center.get(),
                corner.getPoint());
        double ang = calculateAngleDifference(azi, rbCC[1]);
        _ignore += 2;
        setPoint(ca,
                GeoPointMetaData
    ...
    private Association _createAssociation(PointMapItem firstItem,
        ...
        a.setMetaBoolean("nevercot", true);
    ...
    public static class Builder implements Undoable {
        ...
        protected Builder(Rectangle r, Mode mode) {
            ...
            _editHandlesVisible = true;
        }

        public Builder setEditHandlesVisible(boolean v) {
            _editHandlesVisible = v;
            return this;
        }
        ...
        public Builder setThirdPoint(GeoPointMetaData p) {
            ...
            double ang = calculateAngleDifference(rb0[1],
                    rb1[1]) * 0.5;
            double azi = rbcb[1];
        ...
        private void _addSideMarkers(Association[] assocs) {
            for (Association a : assocs) {
                ...
                s.setEditing(_r.getEditing());
                s.setVisible(_editHandlesVisible);
    ...
    // refactored single usage from `DistanceCalculations`
    static double calculateAngleDifference(double init, double fina) {
