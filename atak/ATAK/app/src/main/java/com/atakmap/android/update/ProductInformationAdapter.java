...
public class ProductInformationAdapter extends BaseAdapter
    ...
    private static String getString(int id) {
        Context a = ATAKApplication.getCurrentActivity();
        if (a != null)
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (_mode != Mode.All) {
            ...
            holder.loadPluginChk.setEnabled(!app.isProductType(
                    ProductInformation.ProductType.systemplugin));
            if (AtakPluginRegistry.isMandatoryLoad(_context, app.getPackageName())) {
                holder.loadPluginChk.setEnabled(false);
            } else
                holder.loadPluginChk.setEnabled(true);

            holder.loadPluginChk
                    .setText(productStatus.current == CURRENT_STATUS.Loaded
                            ? R.string.app_mgmt_filter_loaded
                            : R.string.app_mgmt_filter_NotLoaded);
        ...
        if ((app.isProductType(ProductInformation.ProductType.plugin) ||
                app.isProductType(ProductInformation.ProductType.systemplugin))
                &&
                AppMgmtUtils.isInstalled(_context, app.getPackageName())) {
            final int message;

            if (app.isProductType(ProductInformation.ProductType.systemplugin)
                    && SystemPluginValidator.checkCoreSignature(_context,
                            app.getPackageName())) {
                holder.trusted.setImageDrawable(
                        _context.getDrawable(R.drawable.trusted));
                message = R.string.officially_signed;
            } else if (AtakPluginRegistry.verifyTrust(_context,
                    app.packageName)) {
                holder.trusted.setImageDrawable(
                        _context.getDrawable(R.drawable.trusted));
                message = R.string.officially_signed;
    ...
    private void loadPlugin(final ProductInformation plugin) {
        String warning = "";
        String pluginAbiVersion = AtakPluginRegistry.get().getAbiVersion(_context, plugin.getPackageName());
        String coreAbiVersion = ATAKConstants.getAbiVersion();

        // only compare the major and minor and do not worry about the 3rd play (submajor)
        int i = AtakPluginRegistry.compareVersions(pluginAbiVersion, coreAbiVersion, 2);
        if (i < 0)
            warning = String.format(LocaleUtil.getCurrent(),
                    _context.getString(R.string.warning_plugin_abi_version),
                    pluginAbiVersion, coreAbiVersion);
        final AlertDialog.Builder dialog = new AlertDialog.Builder(_context)
                .setTitle(String.format(LocaleUtil.getCurrent(),
                                _context.getString(R.string.load_s), plugin.getSimpleName()))
                .setMessage(String.format(LocaleUtil.getCurrent(),
                                _context.getString(R.string.plugin_already_installed_load),
                        plugin.getSimpleName(), _context.getString(R.string.app_name), warning));
                        ...
                        if (!SideloadedPluginProvider.installProduct(plugin)) {
                            Toast.makeText(_context,
                                    String.format(LocaleUtil.getCurrent(),
                                            _context.getString(
                                                    R.string.failed_to_load_plugin),
    ...
    private void showAppOverview(final ProductInformation app,
        ...
        if (app == null || !app.isValid()) {
            ...
            Toast.makeText(_context,
                    _context.getString(R.string.invalid_app),
                    Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void showAppDetails(final ProductInformation app,
        ...
        if (app.isInstalled()) {
            ...
            if (FileSystemUtils.isEmpty(currentTakRequirement)) {
                ...
                if (app.isSystemPlugin() && app.packageName
                        .startsWith("com.atakmap.app.flavor.")) {
                    currentTakRequirement = currentTakRequirement.substring(0,
                            currentTakRequirement.lastIndexOf('.')) + ".CIV";
                }
        ...
        TextView buildTypeReason = detailView
                .findViewById(R.id.app_mgmt_product_build_type_compatible);
        ImageView buildTypeIcon = detailView
                    .findViewById(R.id.app_mgmt_product_build_type_icon);

        detailView.findViewById(R.id.app_mgmt_product_build_type).setVisibility(
                app.isInstalled() && app.isPlugin()?View.VISIBLE:View.GONE);

        if (app.isInstalled() && app.isPlugin()) {
            boolean debugCore = AppMgmtUtils.isDebuggable(_context, _context.getPackageName());
            boolean debugPlugin = AppMgmtUtils.isDebuggable(_context, app.getPackageName());
            setTextView(buildTypeReason,
                    String.format(LocaleUtil.getCurrent(), 
                    _context.getString(R.string.atak_core_s_plugin_s_s),
                            debugOrRelease(_context, debugCore),
                            debugOrRelease(_context, debugPlugin),
                            ((debugPlugin != debugCore)?_context.getString(R.string.may_be_incompatible_due_to_mismatch):"")));
            buildTypeIcon.setImageResource(
                            debugCore == debugPlugin
                                    ? R.drawable.importmgr_status_green
                                    : R.drawable.importmgr_status_yellow);
    ...
    private static String debugOrRelease(Context c, boolean debug) { 
        if (debug)
            return c.getString(R.string.debug);
        else 
            return c.getString(R.string.release);
    }
        ...
        if (provider == null) {
            ...
            Toast.makeText(_context,
                    String.format(LocaleUtil.getCurrent(),
                            String.format(LocaleUtil.getCurrent(),
                                    _context.getString(
                                            R.string.failed_to_install_app),
                                    app.getSimpleName())),
                    Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void uninstall(final ProductInformation app) {
        ...
        if (provider == null) {
            ...
            Toast.makeText(_context,
                    String.format(LocaleUtil.getCurrent(),
                            _context.getString(
                                    R.string.failed_to_uninstall_app),
                            app.getSimpleName()),
                    Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        AlertDialog.Builder dialog = new AlertDialog.Builder(_context)
                .setTitle(String.format(LocaleUtil.getCurrent(),
                        _context.getString(R.string.uninstall_app),
                        (app.isPlugin() ? "Plugin" : "App")))
                .setMessage(String.format(LocaleUtil.getCurrent(),
                        _context.getString(R.string.uninstall_app_question),
                        app.getSimpleName()));
