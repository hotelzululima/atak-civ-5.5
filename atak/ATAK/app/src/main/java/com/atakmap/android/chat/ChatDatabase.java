...
public class ChatDatabase {
    ...
    private static final String RAW_COT_EVENT = "rawCotEvent";
    private static final String COT_EVENT_UID = "cotEventUid";
    ...
    private static final DBColumn[] CHAT_COLS = {
            ...
            new DBColumn(RAW_COT_EVENT, TEXT_COL_TYPE),
            new DBColumn(COT_EVENT_UID, TEXT_COL_TYPE)
    };
    ...
    private final ConcurrentLinkedQueue<ChatDatabaseListener> chatDatabaseListeners = new ConcurrentLinkedQueue<>();

    interface ChatDatabaseListener {
        /**
         * Callback for when the database is cleared
         */
        void onDatabaseCleared();

        /**
         * Callback for when a conversation has been changed and if available will
         * provide the chat bundle
         * @param conversationId the conversation identifier
         * @param bundle the bundle included if this was a result of an addition
         */
        void onConversationChanged(String conversationId, Bundle bundle);
    }

    /**
     * Get an instance of the ChatDatabase for search, retrieval and archive of chat messages.
    ...
    private void initDatabase() {
        ...
        DatabaseIface newChatDb = ATAKDatabaseHelper
                .getPrefetchDb(CHAT_DB_FILE2);
    ...
    private void onUpgrade(DatabaseIface db, int oldVersion, int newVersion) {
        ...
        switch (oldVersion) {
            ...
            case 8:
                // Add raw cot event column to chat
                db.execute("ALTER TABLE " + TABLE_CHAT + " ADD COLUMN "
                        + RAW_COT_EVENT + " " + TEXT_COL_TYPE, null);
            case 9:
                // Add cot event column to chat
                db.execute("ALTER TABLE " + TABLE_CHAT + " ADD COLUMN "
                        + COT_EVENT_UID + " " + TEXT_COL_TYPE, null);
                break;
            default:
    ...
    public List<Long> addChat(Bundle chatMessage) {
        ...
        for (ChatDatabaseListener listener : chatDatabaseListeners)
            listener.onConversationChanged(convId, chatMessage);
    ...
    // there's a static function on BindArgument that takes a collection of them, compiles a statement, binds the arguments and executes it
    ...
    private class HierarchyParser {
        ...
        private HierarchyParser(Bundle msg) {
            ...
            _sending = (_senderUID != null) && _senderUID.equals(_selfUID);
    ...
    /**
     * Removes the conversation from the device.   If this is a group, the persisted group id
     * is removed and the conversation.   If this is just a conversation, then the conversation
     * is removed
    ...
    public void removeGroup(final String conversationId) {
        ...
        try {
            ...
            db.execute("DELETE FROM " + TABLE_GROUPS + " WHERE " +
                    CONVO_ID_COL_NAME + " = ?", new String[] {
                            conversationId
            });

            // in addition remove all conversations for that specific conversation id
            db.execute(
                    "DELETE FROM " + TABLE_CHAT + " WHERE " + CONVO_ID_COL_NAME
                            + " = ?",
                    new String[] {
                            conversationId
                    });
    ...
    public List<Bundle> getHistory(final String conversationId) {
        return getHistory(conversationId, -1, true);
    }

    /**
     * Returns the history of a given conversation as a list of bundles.
     * @param conversationId the chat conversation identification
     * @param limit limit the conversation return to the last n conversations or -1
     *              for all conversations.
     * @param legacy toast an error message which is legacy behavior
     * @return return an array of bundles that represent each line in the
     * chat history where the bundle is a representation of the cursor return or an
     * empty array if there are conversations.   Note the return can be less than the
     * limit.
     */
    public List<Bundle> getHistory(final String conversationId, int limit,
            boolean legacy) {
        ...
        try {
            ...
            if (cursor.moveToNext()) {
                do {
                    ret.add(0, cursorToBundle(cursor));
                } while (cursor.moveToNext());
            }
        } catch (IllegalStateException e) {
            Contact contact = Contacts.getInstance().getContactByUuid(
                    conversationId);
            final String name = contact != null ? contact.getName()
                    : "<UNKNOWN>";
            if (legacy) {
                ...
                Log.e(TAG, "error retrieving chat history for " + name + "!",
                        e);
            }
        } finally {
            if (cursor != null)
     ...
     * @return the conversation identifiers for the system that have been persisted.
     */
     ...
     * @return the group information in list form.
     */
    ...
    /**
     * Given a specific cot event uid, retrieve the chat message associated.
     * @param eventUID the cot event uid the message
     * @return the bundle that represents the chat message.
     */
    public Bundle getChatMessageFromEventUID(final String eventUID) {
        Bundle ret = null;
        DatabaseIface db;

        // in the case that the eventUID is null
        if (eventUID == null)
            return null;
        ...
        try {
            ...
            query = db.compileQuery("SELECT * FROM " + TABLE_CHAT
                    + " WHERE " + COT_EVENT_UID + "=? LIMIT 1");
            query.bind(1, eventUID);
            if (query.moveToNext())
     ...
     * @return true if the deletion was successful, false if the deletion failed.
     */
    ...
    /**
     * Registers a chat database change listener.
     * @param listener listener for the chat database changes
     */
    public void registerChatDatabaseListener(
            ChatDatabaseListener listener) {
        if (!chatDatabaseListeners.contains(listener))
            chatDatabaseListeners.add(listener);
    }
     ...
     * @param listener listener for the chat database changes
     */
    public void unregisterChatDatabaseListener(
            ChatDatabaseListener listener) {
        chatDatabaseListeners.remove(listener);
    }
