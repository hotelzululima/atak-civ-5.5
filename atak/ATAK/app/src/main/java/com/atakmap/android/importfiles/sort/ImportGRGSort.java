 ...
 *
 * @deprecated use {@link ImportGRGResolver}
 */
@Deprecated
@DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
public class ImportGRGSort extends ImportResolver {

    private static final String TAG = "ImportGRGSort";

    // Max Length 256MB
    private static final int MAX_GDAL_LENGTH = 256 * 1024 * 1024;
    ...
    public boolean match(File file) {
        if (IOProviderFactory.isDirectory(file)) {
            ...
            String lcFileName = fname.toLowerCase(LocaleUtil.getCurrent());
            if (lcFileName.endsWith(".ovr.sqlite") || lcFileName.endsWith(".ovr.mbtiles"))
                return true;
    ...
    static boolean checkDataset(Dataset dataset) {
        if (dataset != null
                && !FileSystemUtils.isEmpty(dataset.GetProjectionRef()) &&
        ...
        try {
            ...
            while (subdatasetIter.hasNext()) {
                ...
                if (entry.getKey().matches("SUBDATASET\\_\\d+\\_NAME")) {
                    ...
                    try {
                        subds = GdalLibrary
                                .openDatasetFromPath(entry.getValue());
