...
public class MissionSpecificPalletFragment extends Fragment implements
    ...
    private static final int[] SLOTS = {
            ...
            R.id.slot4, R.id.slot5, R.id.slot6, R.id.slot7, R.id.slot8,
            R.id.slot9
    };

    /**
     * This interface describes a capability that can be installed as part of slots 1,2,3,4 of the
     * Mission package fragment.   See the replacement Slot2
     * @deprecated
     */
    @Deprecated
    public interface Slot {
        ...
        /**
         * Returns the icon to be placed in the slot
         * @return the drawable to be used
         */
        Drawable getIcon();

        /**
         * The textual label for the slot
         * @return the text locale translated
         */
        String getLabel();

        /**
         * The type to be used when placing the marker
         * @return the type
         */
        String getType();

        /**
         * The action to be called after PlacePointTool.MarkerCreator::placeMarker is called
         * @param m the resulting marker
         */
        void afterAction(Marker m);
    }

    public interface Slot2 extends Slot {
        /**
         * The action to be called before PlacePointTool.MarkerCreator::placeMarker is called
         * @param c the marker creator
         */
        void beforeAction(PlacePointTool.MarkerCreator c);
    }
    ...
    /**
     * Allow for other utilities to register up to 9 additional point dropper
     * capabilities within the mission tab.
     * @param slotOffset the current slot to register a pallet icon [0..8]
     * @param slot the image to place in the slot
     */
    synchronized static public void registerSlot(int slotOffset, Slot slot) {
        if (slotOffset < 0 || slotOffset >= SLOTS.length)
            return;
        int res = SLOTS[slotOffset];
    ...
    private void setSelected(View v) {
        ...
        if (id == R.id.contact_point) {
            ...
            showHABP(false);
            checkedPosition = 9;
        } else if (id == R.id.slot5) {
            showHABP(false);
            checkedPosition = 10;
        } else if (id == R.id.slot6) {
            showHABP(false);
            checkedPosition = 11;
        } else if (id == R.id.slot7) {
            showHABP(false);
            checkedPosition = 12;
        } else if (id == R.id.slot8) {
            showHABP(false);
            checkedPosition = 13;
        } else if (id == R.id.slot9) {
            showHABP(false);
            checkedPosition = 14;
        } else {
            showHABP(false);
        ...
        if (MetricsApi.shouldRecordMetric()) {
            ...
            b.putString(MetricsUtils.FIELD_MAPITEM_TYPE, selectedType);
    ...
    public MapItem getPointPlacedIntent(GeoPointMetaData point,
        ...
        if (selectedButton == null || selectedType.isEmpty())
        ...
        if (selectedType.equals("b-m-p-s-p-loc")) {
            ...
            DrawingRectangle r;
            final String callsign = _customNamingView.genCallsign();
            if (!callsign.isEmpty()) {
                r = BPHARectangleCreator.drawRectangle(point, _BPHA,
                        MapView.getMapView(), callsign);
        ...
        final String callsign = _customNamingView.genCallsign();
        if (!callsign.isEmpty()) {
            mc = mc.setCallsign(callsign);
            _customNamingView.incrementStartIndex();
        }

        final Slot slot = _slotMap.get(selectedButton.getId());

        if (slot instanceof Slot2) {
            ((Slot2) slot).beforeAction(mc);
        }
