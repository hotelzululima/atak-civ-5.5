...
import com.atakmap.android.missionpackage.event.MissionPackageShapefileHandler2;
...
public abstract class ExportFileMarshal extends ExportMarshal {
    ...
    public void execute(final List<Exportable> exports) {
        ...
        String defaultName = filePrefix() + timestamp();
        if (exports.size() == 1) {
    ...
    public void postMarshal() {
        ...
        final View v = LayoutInflater.from(context)
                .inflate(R.layout.export_file_dialog_content, null);
        ((TextView) v.findViewById(R.id.exportFileMessage))
                .setText(context.getString(R.string.importmgr_exported_file,
                        FileSystemUtils.prettyPrint(file)));
        final CheckBox reimport = v.findViewById(R.id.exportFileReimport);
        final CheckBox promptOnMatch = v
                .findViewById(R.id.exportFileReimportPrompt);
        b.setView(v);
                                    ...
                                    .sendBroadcast(new Intent(
                                            ImportExportMapComponent.USER_HANDLE_IMPORT_FILE_ACTION)
                                                    .putExtra("filepath", file
                                                            .getAbsolutePath())
                                                    .putExtra(
                                                            "promptOnMultipleMatch",
                                                            true)
                                                    .putExtra("importInPlace",
                                                            true));
                        }
                    }
                });
        b.setNegativeButton(R.string.done,
                new DialogInterface.OnClickListener() {
                    @Override
    ...
    public static boolean sendFile(Context context, String contentType,
        ...
        if (ShapefileSpatialDb.SHP_CONTENT_TYPE.equalsIgnoreCase(contentType)
            ...
            if (!MissionPackageShapefileHandler2.add(context, manifest, file)
                    || manifest.isEmpty()) {
