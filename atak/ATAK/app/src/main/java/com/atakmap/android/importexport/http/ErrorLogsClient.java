 ...
 *
 */
public class ErrorLogsClient
        implements RequestManager.RequestListener, Disposable {
    ...
    public void sendLogsToServer(File logz, String server, boolean background,
        ...
        post(
                logz,
                NetConnectString.fromString(server),
                background,
                uid,
                callsign,
                versionName,
    ...
    public void post(File errorFile, NetConnectString serverConnectString,
            boolean background,
            String uid,
            String callsign,
            String versionName, String versionCode) {
        String serverUrl = ServerListDialog.getBaseUrl(serverConnectString);
        postImpl(errorFile, serverConnectString, serverUrl, background, uid,
                callsign, versionName, versionCode);
    }

    private void postImpl(File errorFile, NetConnectString serverConnectString,
            String serverUrl,
            boolean background,
            String uid,
            String callsign,
            String versionName, String versionCode) {
        try {
            PostErrorLogsRequest request = new PostErrorLogsRequest(
                    serverConnectString, serverUrl,
                    background,
            ...
            Log.d(TAG,
                    "Error log post request created for: "
                            + request + ", " + errorFile.getAbsolutePath());
    ...
    public void onRequestFinished(Request request, Bundle resultData) {
        ...
        try {
            ...
            if (request
                ...
                Log.d(TAG,
                        "Error Logs Posted: " + initialRequest.getExportFile());
