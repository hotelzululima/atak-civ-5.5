...
public class BreadcrumbReceiver extends BroadcastReceiver implements
    ...
    private final List<CrumbTrail> crumbTrailList = new ArrayList<>();
    ...
    public BreadcrumbReceiver(MapView view, MapGroup trackGroup) {
        ...
        logCrumbs = prefs.get("toggle_log_tracks", true);
        bPersistentSelfTrack = prefs.get("track_infinite", false);

        // save crumbs from last execution of ATAK out to new track segment KML
        createTrackSegment(getTrackTitle(prefs.getSharedPrefs()),
                this.view.getDeviceCallsign(),
    ...
    public void onMapEvent(MapEvent event) {
        ...
        if (item instanceof CrumbTrail) {
            CrumbTrail trail = (CrumbTrail) item;
            synchronized (crumbTrailList) {
                if (type.equals(MapEvent.ITEM_ADDED)) {
                    crumbTrailList.add(trail);
                } else if (type.equals(MapEvent.ITEM_REMOVED)) {
                    crumbTrailList.remove(trail);
                }
            }
        }
    }
    ...
    private final TimerTask _crumbTrailTask = new TimerTask() {
        ...
        public void run() {
            ...
            List<CrumbTrail> trails;
            synchronized (crumbTrailList) {
                trails = new ArrayList<>(crumbTrailList);
            }
            ...
            if (log) {
                ...
                if (trail == null) {
                    Log.d(TAG, "START LOGGING NEW CRUMB TRAIL for self UID: "
                            + pointItem.getUID());
                    trail = new CrumbTrail(view, pointItem,
                            prefs.getSharedPrefs(), UUID
                                    .randomUUID().toString());
    ...
    private void toggleBreadcrumbs(String uid) {
        ...
        if (trail == null) {
            ...
            trail = new CrumbTrail(view, pointItem, prefs.getSharedPrefs(), UUID
                    .randomUUID().toString());
            pointItem.setCrumbTrail(trail);
            pointItem.getGroup().addItem(trail);
    ...
    private void promptCrumbColor(final PointMapItem item) {
        ...
        palette.setColor(item.getMetaInteger("crumbColor", Color.BLACK));
        b.setView(palette);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(TOGGLE_BREAD)) {
            ...
            if (!locationInit) {
                Log.d(TAG, "location is now initialized and logCrumbs: "
                        + logCrumbs);
                logSelfCrumbs(logCrumbs);
                locationInit = true;
            }
            ...
            if (name != null && !name.isEmpty() && crumbDatabase != null) {
                crumbDatabase.setTrackName(track_dbid, name);
            }

            if (color != null && !color.isEmpty() && crumbDatabase != null) {
                crumbDatabase.setTrackColor(track_dbid, color);
            }

            if (style != null && !style.isEmpty() && crumbDatabase != null) {
    ...
    private void createTrackSegment(String trackTitle, String userTitle,
        ...
        crumbDatabase.createSegment(new CoordinatedTime().getMilliseconds(),
                getNextColor(prefs.getSharedPrefs()),
    ...
    public void dispose() {
        ...
        view.getMapEventDispatcher().removeMapEventListener(MapEvent.ITEM_ADDED,
                this);
