...
public class ActionMenuData implements Parcelable {
     ...
     * @return the action menu data for the provided identifier
     */
     ...
     * @return the associated drawable
     * @throws NotFoundException if the icon cannot be found
     */
    public Drawable getIcon(Context context) throws NotFoundException {
        ...
        //for custom action bar items passing the icon embedded directly
        if (currIconPath.startsWith("base64://")) {
            Bitmap bitmap = IconUtilities.decodeBitmap(currIconPath);
            Drawable drawRet = new BitmapDrawable(context.getResources(),
                    bitmap);
            return drawRet;
        }
        ...
        if (drawRet == null) {
            ...
            Drawable ld = context.getResources()
                    .getDrawable(resID);
            if (ld instanceof LayerDrawable) {
                ...
                if (count > 0) {
                    inst.setBadgeCount((LayerDrawable) ld, count);
                }
            }
    ...
    public boolean hasClickData() {
        if (pam.actionClickData != null && !pam.actionClickData.isEmpty()) {
            for (ActionClickData data : pam.actionClickData) {
                if (!data.isValid())
