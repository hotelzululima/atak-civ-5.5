...
public class WaveRelayControlLite
    ...
    private WaveRelayControlLite(MapView mapView) {
        ...
        boolean wrRedirect = prefs.get("waveRelayRedirect", false);
        if (wrRedirect) {
    ...
    public synchronized void start() {
        ...
        try {
            listenPort = Integer
                    .parseInt(prefs.get("listenPort", DEFAULT_WRGPS_PORT
                            + ""));
        ...
        int requestRate;
        try {
            requestRate = Integer.parseInt(prefs.get("requestDelay", "5000"));
        } catch (NumberFormatException e) {
            requestRate = 5000;
        }
                ...
                try {
                    synchronized (lock) {
                        if (_waverelay_InetAddress != null) {
                            DatagramSocket redirectSendSocket = null;
                            try {
                                redirectSendSocket = new DatagramSocket();
                                ...
                                IoUtils.close(redirectSendSocket);
                            }
                        }
                    }
                } catch (Exception e) {
