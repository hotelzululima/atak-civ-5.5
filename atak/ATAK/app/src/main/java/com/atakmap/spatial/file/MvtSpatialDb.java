...
public class MvtSpatialDb extends SpatialDbContentSource {
    ...
    public static final String MVT_FILE_MIME_TYPE = "application/vnd.mapbox-vector-tile";
    ...
    /** @deprecated use {@link #MvtSpatialDb(FeatureDataStore2)} */
    @Deprecated
    ...
    public MvtSpatialDb(FeatureDataStore2 spatialDb) {
        super(spatialDb, GROUP_NAME, MVT_TYPE);
    }
    ...
    public int processAccept(File file, int depth) {
        if (IOProviderFactory.isFile(file) && IOProviderFactory.canRead(file)) {
            ...
            if (lc.endsWith(".mvt") || lc.endsWith(".mbtiles"))
    ...
    public MapOverlay createOverlay(Context context,
        ...
        return new MvtMapOverlay(context, datastore, query);
    }
}
