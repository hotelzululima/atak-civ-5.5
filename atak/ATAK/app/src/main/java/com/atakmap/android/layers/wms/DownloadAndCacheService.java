...
public class DownloadAndCacheService {
     ...
     * will automatically be created in the default system directory on the same storage as the
     * source.
     */
    public static final String CACHE_URI = "cacheUri";
    /**
     * Key used to optionally define the download directory for the download cache. If not
     * specified, a new cache will automatically be created in the specified directory (using the
     * value associated with {@link #TITLE} on the same storage as the source.
     */
    ...
    /**
     * Key used to enable clients to deconflict events on a per-job basis. The
     * {@code JOB_UID} specified when starting a download will be included in
     * all events braodcast during that job's execution.
     */
    public static final String JOB_UID = "jobUid";
    /**
     * Key used to define an {@code Intent} to be broadcast when the download
     * is complete. When not specified, a default action to re-scan imagery
     * layers will be broadcast.
     */
    public final static String ON_COMPLETE_CALLBACK = "onCompleteCallback";
    /**
    ...
    private Intent onCompleteCallback = null;
    ...
    private static DownloadAndCacheService _instance;

    private DownloadAndCacheService() {
    }

    public static synchronized DownloadAndCacheService getInstance() {
        if (_instance == null)
            _instance = new DownloadAndCacheService();
        return _instance;
    }
    ...
    public int startService(Intent intent) {
        if (intent != null) {
            ...
            // Mark download queued
                ...
                asyncContext.execute(new Runnable() {
                    @Override
                    public void run() {
    ...
    private void queueDownload(Intent intent) {
        ...
        if (extras != null) {
            ...
            jobUid = extras.getString(JOB_UID);

            onCompleteCallback = extras.getParcelable(ON_COMPLETE_CALLBACK);
        }
        ...
        download(
                ...
                intent.getStringExtra(CACHE_DIR),
                uri,
                minRes,
                maxRes);
    }
     ...
     */
    private void download(String cacheUri, String cacheDir,
        ...
        try {
            ...
            if (cacheUri == null) {
                ...
                } else if (title.endsWith(".ovr")) {
                    dir = FileSystemUtils.getItem("grg");
                } else {
                    dir = FileSystemUtils.getItem("imagery");
                }
            ...
            cacheFile = this.currentRequest.cacheFile;
            ...
            client.cache(this.currentRequest, new CacheRequestListener() {
                ...
                public boolean onRequestError(Throwable t, String message,
                    ...
                    if (queuedDownloads <= 1) {
                        rescanForDownloadedLayer(title, cacheFile);
                    }
    ...
    protected void reportDownloadStatus(Long time, String tile, String layer) {
        ...
        if (jobUid != null)
            localIntent.putExtra(JOB_UID, jobUid);
    ...
    private void rescanForDownloadedLayer(final String title, File cacheFile) {
        if (onCompleteCallback != null) {
            Bundle props = onCompleteCallback.getExtras();
            if (cacheFile != null) {
                for (String key : props.keySet()) {
                    ...
                    onCompleteCallback.putExtra(key, s);
                }
            }
            AtakBroadcast.getInstance().sendBroadcast(onCompleteCallback);
        } else if (title.endsWith(".ovr")) {
            File f = new File(FileSystemUtils.getItem("grg"),
                    title + ".sqlite");
            Uri grgURI = Uri.fromFile(f);
            Importer grgImporter = ImporterManager.findImporter(
                    GRGMapComponent.IMPORTER_CONTENT_TYPE,
                    KmlFileSpatialDb.KMZ_FILE_MIME_TYPE);
            String mime = KmlFileSpatialDb.KMZ_FILE_MIME_TYPE;
            try {
                CommsMapComponent.ImportResult result = grgImporter
                        .importData(grgURI, mime, null);
                if (result == CommsMapComponent.ImportResult.SUCCESS)
                    AtakBroadcast.getInstance().sendBroadcast(new Intent(
                            HierarchyListReceiver.REFRESH_HIERARCHY));
                else
                    Log.e(TAG, "could not import the file: " + f);
            } catch (Exception e) {
                Log.e(TAG, "could not import the file: " + f, e);
            }
    ...
    private final Executor asyncContext = Executors.newSingleThreadExecutor(
            new NamedThreadFactory("downloadAndCacheService"));
