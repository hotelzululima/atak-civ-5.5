...
public abstract class CotDetailHandler {
    ...
    static {
        // benign force marshals registration
    ...
    public static boolean toCotDetail(
            gov.tak.api.cot.detail.ICotDetailHandler2 impl, MapItem item,
            CotEvent event, CotDetail detail) {
        return toCotDetail(
                impl,
                item,
                MarshalManager.marshal(item, MetaDataHolder2.class,
                        AttributeSet.class),
                event,
                detail);
    }
    ...
    public static boolean toCotDetail(
            gov.tak.api.cot.detail.ICotDetailHandler2 impl, MapItem item,
            AttributeSet attrs, CotEvent event, CotDetail detail) {
        gov.tak.api.cot.event.CotDetail root = new gov.tak.api.cot.event.CotDetail();
        final boolean result = impl.toCotDetail(
                item,
                attrs,
                MarshalManager.marshal(event, CotEvent.class,
                        gov.tak.api.cot.event.CotEvent.class),
                root);
        if (result) {
            for (gov.tak.api.cot.event.CotDetail child : root.getChildren()) {
                detail.addChild(
    ...
    public static ImportResult toItemMetadata(
            gov.tak.api.cot.detail.ICotDetailHandler2 impl,
            MapItem item, CotEvent event,
            CotDetail detail) {
        final AttributeSet attrs = (item != null) ? MarshalManager.marshal(
                item, MetaDataHolder2.class, AttributeSet.class)
                : new AttributeSet();
        final ICotDetailHandler.ImportResult result = impl.toItemMetadata(
                item,
                attrs,
                MarshalManager.marshal(event, CotEvent.class,
                        gov.tak.api.cot.event.CotEvent.class),
                MarshalManager.marshal(detail, CotDetail.class,
                        gov.tak.api.cot.event.CotDetail.class));
        switch (result) {
            case SUCCESS:
