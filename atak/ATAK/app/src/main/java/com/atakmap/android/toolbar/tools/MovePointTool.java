...
public class MovePointTool extends Tool implements OnVisibleChangedListener {
    ...
    protected boolean onToolBegin(Bundle extras) {
        ...
        if (active != null
            ...
            for (String freezeType : freezeMoveWhenActive.keySet())
                if (itemToMove.getType().startsWith(freezeType)) {
                    ...
                    final String tool = freezeMoveWhenActive.get(freezeType);
                    if (tool != null
                            && active.getIdentifier().contentEquals(tool)) {
        ...
        if (prompt) {
            ...
            if (MetricsApi.shouldRecordMetric()) {
                ...
                b.putString("point", item.getUID());
                b.putString("action", "move_initiated");
                MetricsUtils.record(
                        MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_CHANGED,
                        "MovePointTool", item, b);
            }
    ...
    protected void onToolEnd() {
        ...
        if (MetricsApi.shouldRecordMetric()) {
            final Bundle b = new Bundle();
            b.putString("point", itemToMove.getUID());
            b.putString("action", "move_ended");
            MetricsUtils.record(
                    MetricsUtils.CATEGORY_MAPITEM,
                    MetricsUtils.EVENT_MAPITEM_CHANGED,
                    "MovePointTool", itemToMove, b);
        }
        ...
        public void onMapEvent(MapEvent event) {
            ...
            if (type.startsWith("b-m-p-j-dip")) {
                ...
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_MOVED, "MovePointTool",
                        itemToMove, MetricsUtils.EVENT_STATUS_SUCCESS);
