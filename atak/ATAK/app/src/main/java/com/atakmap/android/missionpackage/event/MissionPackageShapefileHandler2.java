 ...
 */
public class MissionPackageShapefileHandler2 implements
        IMissionPackageEventHandler2 {

    private static final String TAG = "MissionPackageShapefileHandler2";

//    private final ImportSHPSort _sort;
    /**
     * Import Manager sort to match and import
     */
    private final ImportSHPResolver _sort;
    private final Context _context;

    public MissionPackageShapefileHandler2(Context context) {
        _context = context;
        _sort = new ImportSHPResolver(context.getString(R.string.mvt_file),
                FileSystemUtils.getItem(FileSystemUtils.OVERLAYS_DIRECTORY),
                MarshalManager.marshal(context.getDrawable(R.drawable.ic_mvt),
                        Drawable.class, gov.tak.api.commons.graphics.Drawable.class));
    }
    ...
    public boolean add(MissionPackageListGroup group, File file) {
        ...
        List<File> files = ImportSHPResolver.getShapefileMemberFiles(file);
    ...
    public static boolean add(Context context, MissionPackageManifest manifest,
        ...
        ImportSHPResolver sort = new ImportSHPResolver(context.getString(R.string.shapefile),
                FileSystemUtils.getItem(FileSystemUtils.OVERLAYS_DIRECTORY),
                ResUtils.getDrawable(context, R.drawable.ic_shapefile));
        if (!sort.match(file))
            return false;
    ...
    public boolean extract(MissionPackageManifest manifest,
        ...
        if (!_sort.beginImport(shpFile, EnumSet.noneOf(gov.tak.api.importfiles.ImportResolver.SortFlags.class))) {
