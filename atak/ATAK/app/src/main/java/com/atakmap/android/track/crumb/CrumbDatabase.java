...
public class CrumbDatabase {
    ...
    private void initDatabase() {
        ...
        DatabaseIface newCrumbDb = ATAKDatabaseHelper
                .getPrefetchDb(CRUMB_DB_FILE2);
        ...
        if (newCrumbDb == null) {
            ...
            String sql = "SELECT name FROM sqlite_master WHERE type='table' AND name='" + BREADCRUMB_TABLE_NAME2 + "'";
            CursorIface results = null;
            try {
                results = newCrumbDb.query(sql, null);
    ...
    private CrumbDatabase() {
        final File pFile = CRUMB_DB_FILE2.getParentFile();
        if (!IOProviderFactory.exists(pFile))
            if (!IOProviderFactory.mkdirs(pFile)) {
                Log.e(TAG, "Failed to make Directory at " + pFile);
            }
    ...
    public void deleteAll() {
        deleteAll(crumbdb);
        crumbdb.close();
        IOProviderFactory.delete(CrumbDatabase.CRUMB_DB_FILE2,
                IOProvider.SECURE_DELETE);
        crumbdb = null;
    }
    ...
    class Handler implements Runnable {
        ...
        private final StatementIface insertStmt;
