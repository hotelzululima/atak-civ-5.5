...
public class DropDownManager extends BroadcastReceiver {
    ...
    synchronized public boolean isTopDropDown(DropDownReceiver dropDown) {
        DropDownReceiver rightSideTop = rightSide;

        // deal with a transient drop down such as the overflow menu which should not
        // count for the purposes of determining if something is on top.
        if (rightSide != null && rightSide.isTransient()
                && !rightSideStack.isEmpty())
            rightSideTop = rightSideStack.get(0);
        return rightSideTop != null && rightSideTop == dropDown;
    }
                ...
                if ((rightSide != null)
                    ...
                    synchronized (ddrListsLock) {
                        Log.d(TAG, "retaining the drop down on the stack: "
                                + rightSide);
                        setFragmentRetain(rightSide, true);
    ...
    private void setFragmentRetain(DropDownReceiver ddr, boolean b) {
        Fragment f = ddr.getDropDown().getFragment();
        if (f instanceof GenericFragmentAdapter)
            ((GenericFragmentAdapter) f).setRetain(b);
    }
    ...
    private void discardChangeDialog(DropDownReceiver rs) {
        ...
        rs.getMapView().postOnActive(new Runnable() {
            @Override
            public void run() {
        ...
        if (leftSide != null && leftSide.getDropDown() == dd) {
            ...
            for (int i = 0; i < rightSideStack.size() && remove == null; ++i) {
                if (rightSideStack.get(i).getDropDown() == dd) {
                    ...
                    setFragmentRetain(remove, false);
        ...
        synchronized (ddrListsLock) {
            ...
            if (rightSide != null
                ...
                if (completely) {
                    ...
                    for (int i = 0; i < bsc; ++i) {
                        Log.d(TAG,
                                "draining the back stack for " + rightSide + ": "
                                        + rightSide.getBackStackCount());
                    }
                } else {

                }
                rightSide.clearFragment();
