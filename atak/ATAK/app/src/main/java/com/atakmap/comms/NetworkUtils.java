...
public class NetworkUtils {
    ...
    */
     ...
     * be multiple valid IP address on a phone. (Code is not equipped to deal with multiple valid IP
     * addresses at a higher level).
     */
    synchronized private static void getIP_helperFunction() {
        ...
        // SharedPreferences preferences = AtakPreferences.getInstance(_context);
                ...
                if (inter.isUp() && !inter.getDisplayName().equals("ppp0")) {
                    for (InterfaceAddress ifaceAddress : inter
                        ...
                        if (!address.isLoopbackAddress()
                            ...
                            // force preferred addresses such as vpn addresses
                            final String name = inter.getName();

                            // Since Java does not allow for determining if the address
                            // is primary or secondary, we need to use the MPU's getProps to
                            // make sure we do not return the cloud relay address.
                            // ATAK-20038 ATAK may report CR IP on MPU5
                            if (name.compareTo("waverelay") == 0) {

                                // this method rate limits itself
                                scanMpuDhcpSpoofIpAddress();
                                if (dhcp_spoof_ipaddress != null) {
                                    lastAddress = dhcp_spoof_ipaddress;
                                    Log.d(TAG, "found preferential interface "
                                            + name + " address: " + lastAddress);
                                    return;
                                }
                            }

                            // need to blacklist the backdoor address set on the waverelay interface
                            if (name.compareTo("waverelay") == 0 &&
                                    !"10.3.1.254".equals(lastAddress)) {
                                Log.d(TAG, "found preferential interface "
                                        + name + " address: " + lastAddress);
                                return;
                            }

                            if (name.compareTo("cscotun0") == 0 ||
                                    name.compareTo("tun0") == 0) {
                                Log.d(TAG, "found preferential interface "
                                        + name + " address: " + lastAddress);
                                return;
                            } else if (name.compareTo("wlan0") == 0) {
                                //keep iterating through but use wifi if it exists
    ...
    /**
     * Trigger a scan of of the system properties for the key "dhcp.spoof.ipaddress"
     * This scan is rate limited at once every 30 seconds because this is a very heavy
     * weight call.   Given that the "dhcp.spoof.ipaddress" could change at any time, it
     * is unfortunate that we cannot utilize the regular java networking code to determine
     * a primary or secondary address.
     */
    private synchronized static void scanMpuDhcpSpoofIpAddress() {
        long currTime = SystemClock.elapsedRealtime();
        if (last_dhcp_spoof_ipaddress_scan + SCAN_PERIOD > currTime) {
            return;
        }

        // the cache for the call has expired, run the full mechanism
        last_dhcp_spoof_ipaddress_scan = currTime;
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (key.equals("dhcp.spoof.ipaddress")) {
                    dhcp_spoof_ipaddress = value;
                    //Log.e(TAG, "found: " + dhcp_spoof_ipaddress);
                    return;
                }
            }
        } catch (Exception e) {
        ...
        //Log.e(TAG, "could not find dhcp_spoof_address, setting to null");
        dhcp_spoof_ipaddress = null;
    }

    private static String dhcp_spoof_ipaddress = null;
    private static long last_dhcp_spoof_ipaddress_scan = -1;
