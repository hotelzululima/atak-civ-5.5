...
public abstract class CotPortListActivity extends MetricActivity {
    ...
    private static class CotPortAdapter extends BaseAdapter {
        ...
        public void refresh(ArrayList<TAKServer> list) {
            this.portDestinations = list;
        ...
        public Object getItem(int position) {
            return portDestinations.get(position);
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            final TAKServer destination = portDestinations
                    .get(position);
            ...
            if (!FileSystemUtils.isEmpty(version)) {
                ...
                holder.connectVersion.setVisibility(View.INVISIBLE);
            }
            ...
            holder.checkbox.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    try {
                        ...
                        Toast.makeText(
                                listActivity,
                                R.string.invalid_configuration_please_delete_it,
                                Toast.LENGTH_LONG)
                                .show();
                        Log.e(TAG, "error", e);
                    }
    ...
    private void showDeleteDialog(final TAKServer cotPort) {
    ...
    private TAKServer currentlyEditingCotPort = null;

    private void showEdit(final TAKServer cotPort) {
        ...
        editNetInfoIntent.putExtra("enrollUseTrust",
                cotPort.enrollUseTrust());
    ...
    private void saveConnection(Intent data) {
        ...
        String connectString = data
                .getStringExtra(TAKServer.CONNECT_STRING_KEY);
        if (inputData == null) {
            Log.e(TAG,
                    "error saving credentials for old server: "
                            + currentlyEditingCotPort.getConnectString(),
                    new Exception());
            return;
        }
        inputData.putString(TAKServer.CONNECT_STRING_KEY,
                connectString);
        TAKServer newConnection = new TAKServer(inputData);
    ...
    protected void enrollForCertificate(Bundle inputData,
        ...
        boolean enrollForCertificateWithTrust = inputData
                .getBoolean(TAKServer.ENROLL_FOR_CERT_KEY, false);
        if (!enrollForCertificateWithTrust) {
            return;
        }
        boolean enrollUseTrust = inputData
                .getBoolean(TAKServer.ENROLL_USE_TRUST_KEY, true);

        String connectString = inputData
                .getString(TAKServer.CONNECT_STRING_KEY);
        NetConnectString newNcs = NetConnectString.fromString(connectString);
        if (newNcs == null) {
            Log.e(TAG, "error enrolling for: " + connectString,
                    new Exception());
            return;
        }
        String newServer = newNcs.getHost();
        ...
        String username = inputData.getString(TAKServer.USERNAME_KEY);
        String password = inputData.getString(TAKServer.PASSWORD_KEY);
        String cacheCreds = inputData.getString(TAKServer.CACHECREDS_KEY);
        String description = inputData.getString(TAKServer.DESCRIPTION_KEY);
        Long expiration = inputData.getLong(TAKServer.EXPIRATION_KEY);
    ...
    private void sort() {
        try {
            ...
            Collections.sort(_portList, new Comparator<TAKServer>() {
                @Override
                public int compare(TAKServer lhs, TAKServer rhs) {
                    if (lhs == null || rhs == null ||
                            lhs.getDescription() == null
                            || rhs.getDescription() == null) {
                        Log.e(TAG, "null TAKServer or description in compare!");
                        return 0;
                    }
    ...
    protected void removeAllCotPorts() {
        ...
        while (_portAdapter.getCount() > 0) {
            TAKServer item = (TAKServer) _portAdapter.getItem(0);
    ...
    protected final ArrayList<TAKServer> _portList = new ArrayList<>();
    protected CotPortAdapter _portAdapter;
    protected CotServiceRemote _remote;

    abstract String getPortType();

    abstract void enabledFlagSet(TAKServer port);
    ...
    protected final CotServiceRemote.OutputsChangedListener _outputsChangedListener = new CotServiceRemote.OutputsChangedListener() {
        ...
        public void onCotOutputRemoved(Bundle descBundle) {
            ...
            TAKServer port = new TAKServer(descBundle);
        ...
        public void onCotOutputUpdated(Bundle descBundle) {
            ...
            _putItem(new TAKServer(descBundle));
        }
    };
    ...
    protected final CotServiceRemote.InputsChangedListener _inputsChangedListener = new CotServiceRemote.InputsChangedListener() {
        ...
        public void onCotInputAdded(Bundle descBundle) {
            Log.v(TAG,
                    "Received ADD message for "
                            + descBundle.getString(TAKServer.DESCRIPTION_KEY)
                            + ": enabled="
                            + descBundle.getBoolean(TAKServer.ENABLED_KEY,
                                    true));
            _putItem(new TAKServer(descBundle));
        }
    };
     ...
     */
    protected boolean _putItem(TAKServer port) {
        boolean added = false;
        ...
        if (indexToAdd < 0) {
            ...
            // port.isEnabled());
            ListIterator<TAKServer> itr = _portList.listIterator(indexToAdd);
            itr.next();
            itr.set(port);
        }
        _portAdapter.refresh(_portList);
     ...
     */
    protected boolean _removeItem(TAKServer port) {
