...
public class TAKServerSender extends MissionPackageSender implements
    ...
    private boolean promptForServer(ServerListDialog.Callback callback) {
        ...
        final TAKServerListener takServerListener = TAKServerListener
                .getInstance();

        if (takServerListener == null)
            return false;

        if (_prefs.get("filesharingAllServers", false))
            servers = takServerListener.getServers();
        else
            servers = takServerListener.getConnectedServers();

        final ServerListDialog d = new ServerListDialog(_mapView);
    ...
    private class UploadRequest implements ServerListDialog.Callback,
        ...
        void upload() {
            ...
            if (this.mpm != null) {
                ...
                client.post(this.video,
                        NetConnectString
                                .fromString(this.server.getConnectString()),
                        this.server.getURL(false), this);
            }
        }
