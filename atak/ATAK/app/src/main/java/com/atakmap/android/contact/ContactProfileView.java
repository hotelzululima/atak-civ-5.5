...
public class ContactProfileView extends ContactDetailView {
    ...
    private RemarksLayout _contactRemarks;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        _contactRemarks = v
                .findViewById(R.id.remarksLayout);
        _contactRemarks.setEditable(false);
    ...
    protected void refresh() {
        ...
        if (_marker != null) {
            ...
            String team = _marker.getMetaString("team", "");
            //see ATAK-16816 the team is marked Unknown is team color for contact is not one of the
            //colors supported by TAK see teamColors preference for listing. Do the check here as i don't want to
            //change Pink as that value for the metadata on the contact marker is used for the map marker. -SPA
            String s = team.equals("Pink")
                    ? getContext().getString(R.string.unknown)
                    : TeamGroup.getLocaleTeamName(team);
            final String role;
            if (bSelf) {
                Role r = CotMapComponent.getInstance().getRolesManager()
                        .getRole(
                                _prefs.getString("customRole", null),
                                _prefs.getString("customRoleAbbreviation",
                                        null));
                role = (r != null) ? r.getName()
                        : _prefs.getString("atakRoleType", "Team Member");
            } else {
                Role r = CotMapComponent.getInstance().getRolesManager()
                        .getRole(
                                _marker.getMetaString("customRole", null),
                                _marker.getMetaString("customRoleAbbreviation",
                                        null));
                role = (r != null) ? r.getName()
                        : _marker.getMetaString("atakRoleType", null);
            }
            if (role != null)
                s += " " + RoleGroup.getLocaleRoleName(role);
            ...
            if (bSelf) {
                ...
                String userRemarks = _prefs.getString("userRemarks", "").trim();
                _contactRemarks.setText(userRemarks);
            } else {
                s = TakVersionDetailHandler.getVersion(_marker);
                _contactRemarks.setText(_marker.getRemarks());
            }
            if (FileSystemUtils.isEmpty(s)
                    || FileSystemUtils.isEmpty(s.trim())) {
            ...
            if (!FileSystemUtils.isEmpty(profiles)) {
                ...
                if (obj instanceof ActionBroadcastData) {
                    ...
                    //TODO hack to return icon, should probably instead better associate the
