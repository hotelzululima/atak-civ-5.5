...
public class SensorDetailHandler extends CotDetailHandler {
    ...
    public static final String RANGE_LINES_STROKE_COLOR = "rangeLineStrokeColor";
    public static final String RANGE_LINES_STROKE_WEIGHT = "rangeLineStrokeWeight";
    ...
    public boolean toCotDetail(MapItem item, CotEvent event, CotDetail detail) {
        ...
        int range = item.getMetaInteger(RANGE_ATTRIBUTE, 100);
        int elevation = item.getMetaInteger(ELEVATION_ATTRIBUTE, 0);

        double slantRange = range / Math.cos(Math.toRadians(elevation));

        sensor.setAttribute(RANGE_ATTRIBUTE, String.valueOf((int) Math.round(slantRange)));
        sensor.setAttribute(ELEVATION_ATTRIBUTE, String.valueOf(elevation));
        ...
        sensor.setAttribute(RANGE_LINES_STROKE_COLOR, String.valueOf(
                item.getMetaInteger(RANGE_LINES_STROKE_COLOR, Color.LTGRAY)));
        sensor.setAttribute(RANGE_LINES_STROKE_WEIGHT, String.valueOf(
                item.getMetaDouble(RANGE_LINES_STROKE_WEIGHT, 0)));
    ...
    public ImportResult toItemMetadata(MapItem item, CotEvent event,
        ...
        double groundRange = Double.NaN;
        if (!Double.isNaN(range)) {
            groundRange = range * Math.cos(Math.toRadians(elevation));
            item.setMetaInteger(RANGE_ATTRIBUTE, (int) Math.round(groundRange));
        }
        ...
        int rangeLineStrokeColor = parseInt(
                detail.getAttribute(RANGE_LINES_STROKE_COLOR),
                Color.LTGRAY);
        double rangeLineStrokeWeight = parseDouble(
                detail.getAttribute(RANGE_LINES_STROKE_WEIGHT),
                0);
        ...
        item.setMetaInteger(RANGE_LINES_STROKE_COLOR, rangeLineStrokeColor);
        item.setMetaDouble(RANGE_LINES_STROKE_WEIGHT, rangeLineStrokeWeight);
        ...
        SensorFOV sens = addFovToMap(marker, azimuth, fov, groundRange, color,
                visible, bLabels, rangeLines, elevation);
        if (sens == null)
        ...
        sens.setRangeLineStrokeColor(rangeLineStrokeColor);
        sens.setRangeLineStrokeWeight(rangeLineStrokeWeight);
    ...
    static SensorFOV getOrAddSensorFov(final MapGroup group,
        ...
        marker.addOnGroupChangedListener(new MapItem.OnGroupChangedListener() {
            ...
            public void onItemRemoved(MapItem item, MapGroup markerGroup) {
                ...
                Log.d(TAG, "Item " + (item == null ? null : item.getUID())
    ...
    public static SensorFOV addFovToMap(final PointMapItem marker,
            double azimuth,
    ...
    public static SensorFOV addFovToMap(final PointMapItem marker,
        ...
        return addFovToMap(marker, azimuth, fov, range, color, visible, bLabels, rangeLines, 0.0d);
    }

    /**
     * When setting the metrics that define the sensor, please pay attention to the
     * units of the various items:
     * @param marker the marker to attach the sensor field of view to
     * @param azimuth  the azimuth is in degrees true
     * @param fov field of view in angular degrees
     * @param range in meters bounded between Integer.MAX_VALUE and Integer.MIN_VALUE.   If the value
     *               exceeds the bounds it will be capped at the MAX or MIN value.   Please take care
     *               to note that the the extent is the ground range and not the slant range.
     * @param color the floating point values for the [ R, G, B ]
     * @param visible if the sensor field of view is visible
     * @param bLabels display FoV edges/angles as labels
     * @param rangeLines spacing for range lines in meters
     * @param elevation the angle from the sensor
     */
    public static SensorFOV addFovToMap(final PointMapItem marker,
            double azimuth,
            double fov, double range, float[] color,
            boolean visible, boolean bLabels, double rangeLines, double elevation) {
        ...
        if (mapGroup != null) {
            ...
            fovItem.setMetrics((float) azimuth, (float) fov, (float) range,
                    bLabels, (float) rangeLines, (float)elevation);

            //set the meta data for the FOV on its Marker
            //TODO what is missing that should be set? WinTAK has not implemented range lines yet
            marker.setMetaDouble(SensorDetailHandler.FOV_ALPHA, color[3]);
            marker.setMetaDouble(FOV_RED, color[0]);
            marker.setMetaDouble(FOV_GREEN, color[1]);
            marker.setMetaDouble(FOV_BLUE, color[2]);
            marker.setMetaInteger(STROKE_COLOR, fovItem.getStrokeColor());
            marker.setMetaDouble(STROKE_WEIGHT, fovItem.getStrokeWeight());
            marker.setMetaInteger(RANGE_LINES_STROKE_COLOR,
                    fovItem.getRangeLineStrokeColor());
            marker.setMetaDouble(RANGE_LINES_STROKE_WEIGHT,
                    fovItem.getRangeLineStrokeWeight());
            marker.setMetaInteger(AZIMUTH_ATTRIBUTE, (int) Math.round(azimuth));
            marker.setMetaInteger(FOV_ATTRIBUTE, (int) Math.round(fov));
            marker.setMetaInteger(RANGE_ATTRIBUTE, (int) Math.round(range));
            marker.setMetaInteger(RANGE_LINES_ATTRIBUTE,
                    (int) Math.round(rangeLines));
            return fovItem;
        } else {
    ...
    public static void selectFOVEndPoint(final PointMapItem m,
