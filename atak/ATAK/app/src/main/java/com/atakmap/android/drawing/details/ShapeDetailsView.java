...
public class ShapeDetailsView extends GenericDetailsView implements
    ...
    private CheckBox _cpVisibilityCB;
    ...
    private void _init() {
        ...
        findViewById(R.id.drawingShapeEditButton)
                .setEnabled(_shape.getMetaBoolean("editable", true));
    ...
    public void refresh() {
        ...
        _cpVisibilityCB.setOnCheckedChangeListener(null);
        _cpVisibilityCB.setChecked(_shape.isCenterPointVisible());
        _cpVisibilityCB.setOnCheckedChangeListener(this);
        ...
        CoTInfoView.refreshAuthorLayout(_mapView, _shape, _authorText,
                _productionTimeText, cotAuthorLayout, cotAuthorIconButton,
                cotAuthorPanButton);

        // Attachments button
        attachmentManager.refresh();
        _shape.persist(MapView.getMapView().getMapEventDispatcher(), null,
                this.getClass());
    }
    ...
    private void _onCenterSelected() {
        ...
        CoordinateEntryCapability.getInstance(getContext()).showDialog(
                "Enter Shape Center Point: ",
                _shape.getAnchorItem().getGeoPointMetaData(),
                _shape.getAnchorItem().getMovable(),
                _mapView.getPoint(), _cFormat, false,
                new CoordinateEntryCapability.ResultCallback() {
                    ...
                    public void onResultCallback(String pane,
                        ...
                        _shape.persist(_mapView.getMapEventDispatcher(),
                                null, getClass());
    ...
    private void onPointSelected(final boolean start) {
        ...
        GeoPointMetaData shapePoint = _shape.getPoint(index);
        if (shapePoint == null)
            return;

        CoordinateEntryCapability.getInstance(getContext()).showDialog(
                getContext().getString(
                        start ? R.string.start_point : R.string.end_point),
                shapePoint,
                _shape.getMovable(),
                _mapView.getPoint(), _cFormat, false,
                new CoordinateEntryCapability.ResultCallback() {
                    ...
                    public void onResultCallback(String pane,
                        ...
                        if (cf.getDisplayName().equals(pane)
                                && cf != CoordinateFormat.ADDRESS)
                            _cFormat = cf;
                        _shape.setPoint(index, point, true);
                        refresh();
                    }
                });
    }
    ...
    public void onCheckedChanged(CompoundButton cb, boolean checked) {
        if (cb == _cpVisibilityCB) {
            _shape.setCenterPointVisible(checked);
            _drawPrefs.set("drawing.centerpointvisible.laststate", checked);
        } else if (cb == _closedCB) {
