...
class BroadcastIntentMapAction implements MapAction,
    ...
    public void performAction(MapView mapView, MapItem mapItem) {
        ...
        params.setResolver('^', _TIME_RESOLVER);
        if (mapItem != null) {
            params.setResolver('$', new PhraseParser.BundleResolver(mapItem));
        }

        for (_Extra ex : _extras) {
            String resolvedValue = PhraseParser.expandPhrase(ex.value, params);
            switch (ex.type) {
                case _EXTRA_STRING:
        ...
        if (_dataUri != null) {
            String resolvedUri = PhraseParser.expandPhrase(_dataUri, params);
            if (_dataType != null) {
        ...
        intent.putExtra(MetricsUtils.INTENT_EXTRA_CLASS,
                "BroadcastIntentMapAction");
        intent.putExtra(MetricsUtils.INTENT_EXTRA_ELEMENT, "RadialMenu");
        intent.putExtra(MetricsUtils.INTENT_EXTRA_CATEGORY,
                MetricsUtils.CATEGORY_MAPWIDGET);
