...
public class GLQuadtreeNode2 extends
    ...
    public void insertItem(final GLMapItem2 item) {
        synchronized (this.items) {
            if (item.getOpaque() instanceof ItemStore) {
                Log.w(TAG,
    ...
    public void removeItem(final GLMapItem2 item) {
        synchronized (this.items) {
            ...
            if (!(opaque instanceof ItemStore)) {
    ...
    public void draw(GLMapView view, int renderPass) {
        // record depth state on entry
        DepthState depth = new DepthState();

        super.draw(view, renderPass);

        List<Collection<GLMapRenderable2>> toRelease;
        synchronized (this) {
            // swap versus copy for minimal lock contention
            if (!releasables.isEmpty()) {
                toRelease = releasables;
                releasables = new ArrayList<>();
            } else {
                toRelease = Collections.EMPTY_LIST;
            }
        }

        for (Collection<GLMapRenderable2> c : toRelease) {
            for (GLMapRenderable2 i : c)
                i.release();
        }

        // restore the depth state on exit in case modified by renderables
        depth.set();
    }
    ...
    protected void releaseImpl() {
        ...
        for (Collection<GLMapRenderable2> c : releasables) {
            for (GLMapRenderable2 i : c)
                i.release();
        }
        releasables.clear();
    ...
    protected boolean updateRenderList(ViewState state,
        ...
        Collection<GLMapRenderable2> queryReleasables = new HashSet<>(
                this.renderables);
        ...
        if (!queryReleasables.isEmpty()) {
            // minimize lock contention by adding reference to collection versus copy
            releasables.add(queryReleasables);
        }

        queryResultsTransferred = true;
        return true;
    }
    ...
    protected void query(
        ...
        AbstractGLMapItem2.debugHitBoxes = ConfigOptions
                .getOption("debug-hit-boxes", 0) != 0;
    ...
    private void queryImpl(
        ...
        synchronized (this.items) {
            this.items.collectDrawables(scene, bnds, resolution,
                    result);
        }
    }
    ...
    public void hitTest(MapRenderer3 renderer, HitTestQueryParameters params,
        ...
        LayerHitTestControl.Default.hitTest(this, renderer, params, results);
    }
    ...
    final class ItemStore implements GLMapItem2.OnBoundsChangedListener {
        ...
        public final GLMapRenderable2 entry = new GLMapRenderable2() {
            ...
            public void draw(GLMapView view, int renderPass) {
                if (!MathUtils.hasBits(renderPass,
                        GLMapView.RENDER_PASS_SURFACE))
                    return;
                if (view.currentPass.drawSrid != 4978)
                    return;
                ...
                view.currentPass.scene.forward.transform(view.scratch.pointD,
                        view.scratch.pointD);
                ...
                try {
                    ...
                    int idx = 0;
                    fb.put(idx++, view.currentPass.bottom); // lower-left
                    fb.put(idx++, view.currentPass.left);
                    fb.put(idx++, (float) view.scratch.pointD.z);
                    fb.put(idx++, view.currentPass.top); // upper-left
                    fb.put(idx++, view.currentPass.left);
                    fb.put(idx++, (float) view.scratch.pointD.z);
                    fb.put(idx++, view.currentPass.bottom); // lower-right
                    fb.put(idx++, view.currentPass.right);
                    fb.put(idx++, (float) view.scratch.pointD.z);
                    fb.put(idx++, view.currentPass.top); // upper-right
                    fb.put(idx++, view.currentPass.right);
                    fb.put(idx++, (float) view.scratch.pointD.z);
    ...
    private class GLBatchRenderer
        ...
        @Override
        public void hitTest(MapRenderer3 renderer,
                HitTestQueryParameters params,
                Collection<HitTestResult> results) {
            LayerHitTestControl.Default.hitTest(this, renderer, params,
                    results);
        }
