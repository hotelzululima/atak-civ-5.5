...
public class SettingsActivity extends MetricPreferenceActivity implements
    ...
    private AtakPreferences mainControlPrefs;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AtakPreferenceFragment.setLocale(SettingsActivity.this);

        setPreferenceContext();

        AtakPreferenceFragment.setSoftKeyIllumination(this);

        mainControlPrefs = AtakPreferences.getInstance(this);
        ...
        if (toolPreference != null) {
            ...
            //Log.d(TAG, "'" + statePref + "'");
            if (!mainControlPrefs.get(statePref, true)) {
                toolPreference = null;
            }
        }
    ...
    protected void onDestroy() {
        try {
            ...
            // ATAK-17994 Playstore Crash: IllegalStateException SettingActivity
        }
    ...
    private void handleNightVision() {
        AtakPreferences prefs = AtakPreferences.getInstance(this);
        if (prefs.get("loc_notification", false)
                && prefs.get("night_vision_widget", false)) {
        ...
        for (int i = 0; i < la.getCount(); ++i) {
            if (la.getItem(i) == p) {
                ...
                v.post(new Runnable() {
                    ...
                    public void run() {
                        lv.setSelection(fi);
                        // do not blink a preference category - it ends up looking strange
    ...
    private boolean isLegacyPreferenceLayout() {
        ...
        return mainControlPrefs.get("legacy_settings", false);
    }
