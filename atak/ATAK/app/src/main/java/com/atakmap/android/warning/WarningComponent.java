...
public class WarningComponent extends AbstractWidgetMapComponent implements
    ...
    private static void alert(final Alert alert) {
        ...
        if (_prefs.get("alert_notification", true) &&
                _prefs.get("individual_alert_notification", false)) {
            ...
            boolean vibrate = _prefs.get("alert_vibration", true);
            boolean chime = _prefs.get("alert_audible", false);
            boolean blink = chime;
        ...
        if (_prefs.get("alert_notification", true)) {
            if (FileSystemUtils.isEmpty(message)) {
                ...
                boolean vibrate = verbose
                        && _prefs.get("alert_vibration", true);
                boolean chime = verbose
                        && _prefs.get("alert_audible", false);
                boolean blink = chime;
     ...
     */
    static private boolean addAlerts(Collection<? extends Alert> list) {
    ...
    public static void addAlerts(Class<? extends Alert> clazz,
        ...
        if (!_active && FileSystemUtils.isEmpty(list))
            return;

        boolean changed = false;

        //first add/update alerts
        changed = addAlerts(list);
    ...
    private static void updateOverlay(Alert alert) {
        ...
        MapOverlay overlay = _overlayParent.get("AlertMapOverlay/" + groupName);
        if (overlay != null)
            _overlayParent.remove(overlay);
