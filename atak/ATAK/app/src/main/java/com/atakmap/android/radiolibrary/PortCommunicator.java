...
abstract class PortCommunicator {
    ...
    String read(final UsbSerialPort port) {
        ...
        do {
            ...
            try {
                ...
                if (bread > 0)
                    returnString.append(new String(buf, 0, bread,
    ...
    void write(final UsbSerialPort port, String command) throws IOException {
        ...
        byte[] ascii = command.getBytes(StandardCharsets.UTF_8);
        int amtWritten;
        try {
            port.write(ascii, 1000);
            Log.d(TAG, "port.write success, amtWritten=" + ascii.length);
        } catch (SerialTimeoutException ste) {
            Log.d(TAG, "error writing to port: ", ste);
        }
    }
}
