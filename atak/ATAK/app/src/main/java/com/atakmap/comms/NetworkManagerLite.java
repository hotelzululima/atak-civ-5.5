...
public class NetworkManagerLite extends BroadcastReceiver {
    ...
    public static class NetworkDevice {
        ...
        /**
         * Gets the current state of the network adapter (manually disabled)
         * @return true if it is manually disabled by the user
         */
        ...
        public NetworkDevice(String label) {
            this(label, new Type[] {
                    ...
                    label, null, null, null, false, false);
        }
         ...
         * @param disabled if the network device has been manually disabled by the user
         */
        public NetworkDevice(@NonNull String label, @NonNull Type[] types,
                ...
                boolean prefCreateDefaultRoute, boolean disabled) {
                    ...
                    try {
                        ...
                        switch (c) {
                            ...
                            case NONE:
                                ...
                                break;
                            case STATIC:
                                disabled = arr.length > 12
                                        && Boolean.parseBoolean(arr[12]);
                                nd = new NetworkDevice(arr[1], types,
    ...
    static private void refreshFilter() {
        ...
        if (!filtersDropped) {
            ...
            try {
                ...
                Process p;
                p = execCmd("ip", "rule flush");
                wait(p);
                p = execCmd("ip", "rule add from all table main");
                wait(p);
                p = execCmd("ip", "rule add from all table default");
                wait(p);
                p = execCmd("ip", "rule add from all lookup wlan0");
                wait(p);
            } catch (Exception ioe) {
                Log.e(TAG, "error: ", ioe);
            }
    ...
    /**
     * Send an intent (com.atakmap.exec) to execute the command.
     * @param command the commands { ip, dhcptool, netcfg, ndc }
     * @param args the arguments which have been validated prior to this call because they are command 
     *             dependent.
     */
    static private Process execCmd(final String command,
            final String args) {
        ...
        Log.d(TAG, "Sending command: " + command + " " + args);
    ...
    static public boolean unconfigure(final String ifaceNameRaw) {
        ...
        try {
            ...
            proc = execCmd("ip", "link set dev " + ifaceName + " down");
            wait(proc);
            return true;
        } catch (Exception e) {
            Log.e(TAG, "error: ", e);
    ...
    static public boolean configure(final String ifaceNameRaw,
        ...
        try {
            ...
            proc = execCmd("ip", "link set dev " + ifaceName + " down");
            wait(proc);
            proc = execCmd("ip", "addr flush dev " + ifaceName);
            wait(proc);
            proc = execCmd("ip", "addr add " + address + "/"
                    + toShortNetmask(netmask) + " dev "
                    + ifaceName);
            wait(proc);
            proc = execCmd("ip", "link set " + ifaceName + " up");
            wait(proc);
            if ((gateway != null) && (gateway.length() > 3)
                    && !gateway.equals("0.0.0.0")) {
                if (createDefaultRoute) {
                    proc = execCmd("ip", "route delete default");
                    wait(proc);
                    proc = execCmd("ip", "route add default via " + gateway);
                } else {
                    String masked = maskedAddress(address, netmask);
                    proc = execCmd("ip", "route change " +
                            masked + "/" + toShortNetmask(netmask)
                            + " via " + gateway);
                }
                wait(proc);
                proc = execCmd("ip", "route replace 224.0.0.0/4 dev "
                        + ifaceName);
                wait(proc);
            }
            ...
            if ((dns != null) && (dns.length() > 3)) {
                // android 5, 6, 7
                proc = execCmd("ndc", "resolver setnetdns " + ifaceName
                        + " \"\" " + dns + " 8.8.8.8");
                wait(proc);
    ...
    static public boolean configure(final String ifaceNameRaw) {
        ...
        try {
            ...
            if (IOProviderFactory.exists(dhcptool)) {
                ...
                proc = execCmd("dhcptool", ifaceName);
            } else {
                proc = execCmd("netcfg", ifaceName + " dhcp");
            }
