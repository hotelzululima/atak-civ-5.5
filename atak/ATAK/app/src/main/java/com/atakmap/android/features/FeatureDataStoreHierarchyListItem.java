...
public class FeatureDataStoreHierarchyListItem extends
    ...
    /** @deprecated use {@link #FeatureDataStoreHierarchyListItem(FeatureDataStore2, String, String, String, String, Context, BaseAdapter, HierarchyListFilter)} */
    @Deprecated
    ...
    private int queryDescendants() {
        ...
        try {
            FeatureDataStore2.FeatureQueryParameters params = buildQueryParams2(
                    filter);
            if (this.typeFilter != null) {
                if (params.featureSetFilter == null)
                    params.featureSetFilter = new FeatureSetQueryParameters();
                params.featureSetFilter.types = Collections
                        .singleton(this.typeFilter);
            }
            return this.spatialDb
                    .queryFeaturesCount(params);
        } catch (DataStoreException dse) {
            Log.e(TAG,
    ...
    public boolean setVisible(boolean visible) {
        ...
        try {
            this.spatialDb.setFeatureSetsVisible(params,
                    visible);
        } catch (DataStoreException dse) {
            Log.e(TAG,
                    "error occurred querying datastore: " + spatialDb.getUri());
    ...
    public Set<HierarchyListItem> find(String terms) {
        ...
        params.order
                .add(new FeatureDataStore2.FeatureQueryParameters.Order.Name());
    ...
    /** @deprecated subclasses should use {@link #buildQueryParams2(HierarchyListFilter)} */
    @Deprecated
    @DeprecatedApi(since = "5.3", forRemoval = true, removeAt = "5.6")
    private FeatureDataStore.FeatureQueryParameters buildQueryParams() {
    ...
    static boolean isEmpty(FeatureDataStore2 spatialDb,
        ...
        FeatureDataStore2.FeatureQueryParameters params = buildQueryParams2(
                filter);
        params.featureSetFilter = new FeatureSetQueryParameters();
        if (entry.fsid != FeatureDataStore2.FEATURESET_ID_NONE
            ...
            params.featureSetFilter.names.add(entry.folder);
            params.featureSetFilter.names.add(entry.folder + "/%");
            params.featureSetFilter.ids = entry.childFsids;
            if (entry.fsid != FeatureDataStore2.FEATURESET_ID_NONE) {
                params.featureSetFilter.ids = new HashSet<>(
                        params.featureSetFilter.ids);
                params.featureSetFilter.ids.add(entry.fsid);
            }
        }
        params.limit = 1;
        try {
