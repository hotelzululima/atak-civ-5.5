...
import com.atakmap.android.image.action.ImageActionManager;
import com.atakmap.android.image.action.ImageActionManager.ImageActionProvider;
import com.atakmap.android.image.action.ImageActionManager.ImageActionCallback;
...
public class ImageDropDownReceiver
    ...
    class LegacyImageResize implements ImageActionProvider {

        private final int desired;
        private final String name;

        LegacyImageResize(int desired, String name) {
        ...
        @Override
        public void performAction(String file, ImageActionCallback callback) {
            File f = resizeImageFile(new File(file), desired);
            if (f == null)
                callback.onComplete(file, null,
                        ImageActionCallback.ACTION_FAILURE, "unknown");
            else
                callback.onComplete(file, f.getAbsolutePath(),
                        ImageActionCallback.ACTION_SUCCESS, null);
        }
    }
    ...
    public static final FilenameFilter VideoFileFilter = new FilenameFilter() {
        ...
        public boolean accept(File dir, String filename) {
            ...
            return VideoFileWatcher.VIDEO_FILTER.accept(new File(
    ...
    ImageDropDownReceiver(MapView mapView) {
        ...
        String[] legacyCapabilities = new String[] {
        ...
        for (int i = 0; i < legacyCapabilities.length; ++i) {
            ImageActionManager.getInstance().registerImageActionProvider(
                    new LegacyImageResize(i, legacyCapabilities[i]));
        }
            ...
            if (action.equals(IMAGE_REFRESH) && newUID != null
                ...
                if (imageURI != null && imageURI.getScheme() != null &&
                    ...
                    inflateDropDown(_context, imageURI.toString(), null,
                            noFunctionality, title, null, false);
                } else if (_uid != null) {
                    inflateDropDown(_context, intent, false);
                }
                ...
                boolean shouldPromptToRename = intent.getBooleanExtra("promptRename",false);
                ...
                if (isVisible() && ic != null && (scheme != null
                    ...
                    if (scheme != null) {
                        inflateDropDown(_context, imageURI, imageURIs,
                                noFunctionality, title, titles, shouldPromptToRename);
                    } else if (_uid != null) {
                        // show all images for UID
                        inflateDropDown(_context, intent, shouldPromptToRename);
                    }
                }
                ...
                if (FileSystemUtils.isEmpty(contactUIDs)
                        && !CommsProviderFactory.getProvider().hasFeature(
                                CommsProvider.CommsFeature.BROADCAST_DATAPACKAGE_CAPABLE)) {
    ...
    public void onDropDownClose() {
        ...
        if (icPending != null) {
            ...
            if (newIC instanceof ImageFileContainer)
                inflateDropDown(_context, (ImageFileContainer) newIC, false, false);
            else if (newIC instanceof ImageBlobContainer)
                inflateDropDown((ImageBlobContainer) newIC);
        } else {
     ...
     * @param sender Content sender interface
     */
    private void confirmImageResize(final File file,
        ...
        final ImageActionProvider[] providers = ImageActionManager.getInstance()
                .getProviders();

        final List<String> names = new ArrayList<>();
        for (ImageActionProvider provider : providers) {
            names.add(provider.getName());
        }
        final String[] sizeValues = names.toArray(new String[0]);
        ...
        final String iapUid = prefs.get("selected.imageactionprovider", null);
        if (iapUid != null) {
            for (int i = 0; i < providers.length; ++i) {
                if (providers[i].getUID().equals(iapUid))
                ...
                .setPositiveButton(R.string.ok,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                final int pos = spinner
                                        .getSelectedItemPosition();
                                prefs.set("selected.imageactionprovider",
                                        providers[pos].getUID());
                                providers[pos].performAction(
                                        file.getAbsolutePath(),
                                        new ImageActionCallback() {
                                            @Override
                                            public void onComplete(
                                                    String originalFile,
                                                    String newFile, int status,
                                                    String reason) {
                                                _mapView.post(new Runnable() {
                                                    @Override
                                                    public void run() {
                                                        if (status == ImageActionCallback.ACTION_FAILURE
                                                                ||
                                                                status == ImageActionCallback.ACTION_CANCELLED) {
                                                            Toast.makeText(
                                                                    _context,
    ...
    private void inflateDropDown(ImageBlobContainer ibc) {
        if (ic != null) {
            icPending = ibc;
            closeDropDown();
            return;
    ...
    private void inflateDropDown(final Context context,
            final ImageFileContainer ifc, final boolean noFunctionality, boolean requireRename) {
        if (ic != null) {
            icPending = ifc;
            closeDropDown();
            return;
        ...
        final ImageButton renameImageButton = view.findViewById(R.id.renameImageFileButton);

        renameImageButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                promptForRename(ifc.getCurrentImageFile().getPath());
            }
        });
        ...
        editImageButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String team = AtakPreferences.getInstance(context)
                        .get("locationTeam", "Cyan");
    ...
    /**
     * Creates an instance to the ImageRenameDialog class
     * that generates a user based name to apply to this image file
     */
    private void promptForRename(String path) {

        //get current mapitem associated with this uid
        MapItem mapItem = _mapView.getRootGroup().deepFindUID(_uid);
        if(mapItem != null) {
            AttachmentManager m = new AttachmentManager(_mapView,new ImageButton(_mapView.getContext()));
            m.setMapItem(mapItem);
            FileRenameDialog ird = new FileRenameDialog(_mapView, path, new FileRenameDialog.FileRenamed() {
                @Override
                public void onFileRenamed(File oldFile, File newFile) {

                    //tell the attachment manager the image file changed and to refresh itself
                    AttachmentManager.notifyAttachmentChange(_uid);

                    //send an update notification so we
                    Intent i = new Intent(ImageGalleryReceiver.UPDATE_ITEM);
                    i.putExtra("oldUri",oldFile.toString());
                    i.putExtra("newUri",newFile.toString());
                    i.putExtra("uid",_uid);
     ...
     */
    private void inflateDropDown(final Context context,
            Intent intent, boolean requireRename) {
        ...
        if (!files.isEmpty()) {
            ...
            String curr = intent.getStringExtra("selectedImageURI");
            if (curr == null)
                curr = imageURIs[0];

            inflateDropDown(context, curr, imageURIs,
                    noFunctionality, title, titles, requireRename);
        } else {
    ...
    private void inflateDropDown(final Context context,
        ...
        if ("file".equals(scheme)) {
            inflateDropDown(context,
                    ...
                    noFunctionality, requireRename);
        } else if ("sqlite".equals(scheme)) {
            inflateDropDown(new ImageBlobContainer(context, _mapView,
                    _uid, imageURI, imageURIs, titles));
        }
    }
    ...
    private boolean sendImage(File file, @Nullable MapItem item,
        ...
        if (sender != null) {
            ...
            if (FileSystemUtils.isEmpty(contacts)
                    && CommsProviderFactory.getProvider().hasFeature(
                            CommsProvider.CommsFeature.BROADCAST_DATAPACKAGE_CAPABLE)) {
                // if alt comms, empty contacts means broadcast
                MissionPackageBuilder mb = new MissionPackageBuilder(null,
                        manifest, getMapView().getRootGroup());
                String pack = mb.build();
                File mp = new File(pack);
                try {
                    int id = CommsProviderFactory.getProvider()
                            .sendMissionPackageInit(new ArrayList<>(), mp,
                                    mp.getName(), mp.getName());
                    CommsProviderFactory.getProvider()
                            .startMissionPackageSend(id);
                    return true;
                } catch (Exception e) {
                    Log.w(TAG,
                            "failed to send mission package " + file.getName(),
                            e);
                    return false;
                }
            }
