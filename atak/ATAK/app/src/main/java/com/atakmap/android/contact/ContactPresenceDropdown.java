...
public class ContactPresenceDropdown extends DropDownReceiver
    ...
    private static final String CONTACT_SORT_MODE_PREF = "contactpresence_sort_mode";
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        _sortSpinner.setOnItemSelectedListener(
                new SimpleItemSelectedListener() {
                    ...
                    public void onItemSelected(AdapterView<?> parent,
                        ...
                        if (sort != null) {
                            _prefs.set(CONTACT_SORT_MODE_PREF, sort.getTitle());
                            adapter.sort(sort);
                            setViewToMode();
                        }
                    }
        ...
        int contactSortPos = 0;
        final String lastSortMode = _prefs.get(CONTACT_SORT_MODE_PREF, "");
        for (int i = 0; i < sorts.size(); ++i) {
            Sort s = sorts.get(i);
            if (s.getTitle().equals(lastSortMode))
                contactSortPos = i;
        }
        _sortSpinner.setSelection(contactSortPos);
        ...
        boolean showAll = !_prefs.get(FOVFilter.PREF, false);
    ...
    private boolean prepareContactList(final View root, final Intent intent) {
        ...
        boolean broadcastDisabled = _prefs.get("broadcast_disable", false);
        // check if it's invalid to broadcast first
        if (req.filename != null || req.imageFilename != null
            ...
            // then defer to prefs
        } else if (!broadcastDisabled) {
            // pref: broadcasting is enabled
            sendBroadcast.setVisibility(View.VISIBLE);
        } else if (!Arrays.asList(
                TAKServerListener.getInstance().getConnectedServers())
                .isEmpty()) {
            // pref: broadcasting is disabled if connected to TAK server
            sendBroadcast.setEnabled(false);
        }
    ...
    private static class SendRequest {
        ...
        SendRequest(Intent intent) {
            ...
            if (intent.hasExtra("sendCallback")) {
                Bundle extras = intent.getExtras();
                if (extras != null) {
                    Object o = extras.get("sendCallback");
                    if (o instanceof String)
                        sendCallback = new Intent((String) o);
                    else if (o instanceof Parcelable)
                        sendCallback = intent
                                .getParcelableExtra("sendCallback");
                }
            }
    ...
    private void sendBroadcast(SendRequest req) {
        if (req.sendCallback != null) {
            ...
            if (CommsProviderFactory.getProvider().hasFeature(
                    CommsProvider.CommsFeature.BROADCAST_DATAPACKAGE_CAPABLE)) {
                // if alt comms allow broadcast for Misison packages
                broadcastFile(req);
            } else {
                AtakBroadcast.getInstance().sendBroadcast(cb);
            }
            ...
            if (CommsProviderFactory.getProvider().hasFeature(
                    CommsProvider.CommsFeature.BROADCAST_DATAPACKAGE_CAPABLE)) {
                // if alt comms allow broadcast of files
                broadcastFile(req);
            } else {
    ...
    private boolean allFilesAreImages(MissionPackageManifest man) {
        if (!man.hasFiles())
            return false;
        for (MissionPackageContent files : man.getFiles()) {
            ResourceFile.MIMEType mime = ResourceFile
                    .getMIMETypeForFile(files.getManifestUid());
            if (mime != ResourceFile.MIMEType.JPEG &&
                    mime != ResourceFile.MIMEType.JPG &&
                    mime != ResourceFile.MIMEType.PNG) {
                return false;
            }
        }
        return true;
    ...
    private void setAttachmentPaths(MissionPackageManifest man) {
        if (man.hasFiles()) {
            for (MissionPackageContent file : man.getFiles()) {
                NameValuePair localPath = file.getParameter(
                        MissionPackageContent.PARAMETER_LOCALPATH);
                if (localPath != null)
                    continue;
                String name = file.getManifestUid()
                        .substring(file.getManifestUid().indexOf("/") + 1);
                File f = searchFile(FileSystemUtils.getItem("attachments"),
                        name);
                if (f != null) {
                    file.setParameter(MissionPackageContent.PARAMETER_LOCALPATH,
                            f.getAbsolutePath());
                }
    ...
    /**
     * Broadcast Mission packages/ files
     * @param req - request with Mission package intent, filepath, and callback
     */
    private void broadcastFile(SendRequest req) {
        if (req.sendCallback != null) {
            MissionPackageManifest man = req.intent.getExtras() != null
                    ? (MissionPackageManifest) req.intent.getExtras()
                            .get("MissionPackageManifest")
                    : null;
            if (man == null || !man.isValid()) {
                ...
                // resize image files
                if (Objects.equals(req.sendCallback.getAction(),
                        "com.atakmap.android.importexport.send.SELECT_RECIPIENTS")) {
                    setAttachmentPaths(man);
                    if (man.hasFiles()
                            && man.getFileCount() == man.getLocalFiles().size()
                            && allFilesAreImages(man)) {
                        for (MissionPackageContent file : man.getFiles()) {
                            String filePath = file.getParameter(
                                    MissionPackageContent.PARAMETER_LOCALPATH)
                                    .getValue();
                            Intent imageResize = new Intent(
                                    ImageDropDownReceiver.IMAGE_SELECT_RESOLUTION);
                            imageResize.putExtra("filepath", filePath);
                            imageResize.putExtra("uid", file.getParameter(
                                    MissionPackageConfiguration.PARAMETER_UID)
                                    .getValue());
                            AtakBroadcast.getInstance()
                                    .sendBroadcast(imageResize);
                        }
                    } else {
                        MissionPackageBuilder mb = new MissionPackageBuilder(
                                null,
                                man, getMapView().getRootGroup());
                        String build = mb.build();
                        File file = new File(build);
                        try {
                            int id = CommsProviderFactory.getProvider()
                                    .sendMissionPackageInit(new ArrayList<>(),
                                            file,
                                            file.getName(), file.getName());
                            CommsProviderFactory.getProvider()
                                    .startMissionPackageSend(id);
                        } catch (Exception e) {
                            Log.w(TAG, "failed to send mission package " + man,
                                    e);
                        }
                    }
                } else if (Objects.equals(req.sendCallback.getAction(),
                    ...
                    try {
                        int id = CommsProviderFactory.getProvider()
                                .sendMissionPackageInit(new ArrayList<>(), file,
                                        file.getName(), file.getName());
                        CommsProviderFactory.getProvider()
                                .startMissionPackageSend(id);
                    } catch (Exception e) {
                        Log.w(TAG, "failed to send mission package " + man, e);
                    }
                }
            }
        } else if (req.filename != null) {
            File file = new File(req.filename);
            MissionPackageManifest man = MissionPackageApi
                    .CreateTempManifest(file.getName(), true, true, null);
            man.addFile(file, null);
            MissionPackageBuilder mb = new MissionPackageBuilder(null, man,
                    getMapView().getRootGroup());
            String build = mb.build();
            File built = new File(build);
            try {
                int id = CommsProviderFactory.getProvider()
                        .sendMissionPackageInit(new ArrayList<>(), built,
                                built.getName(), built.getName());
                CommsProviderFactory.getProvider().startMissionPackageSend(id);
            } catch (Exception e) {
                Log.w(TAG, "failed to broadcast file " + file.getName(), e);
            }
    ...
    private void onRowSelected(Contact contact) {
        ...
        if (GroupContact.isGroup(contact)) {
            ...
            if (!((IndividualContact) contact)
                    .onSelected(_prefs.getSharedPrefs())) {
    ...
    public boolean showUnreadOnly() {
        return baseMode != ViewMode.SEND_LIST &&
                _prefs.get(UnreadFilter.PREF, false);
    }
    ...
    private void sendCot(String[] uids, IndividualContact[] netContacts) {
        ...
        for (String uid : uids) {
            ...
            if (target.getType() != null && target.getType().equals("self")) {
                ...
                CotDispatcher external = CotMapComponent
                        .getExternalDispatcher();
                if (external != null) {
                    try {
                        external.dispatch(
                                CotEventFactory.createCotEvent(target),
                                persistExtras);
                    } catch (IllegalArgumentException e) {
                        Log.e(TAG, "failed to dispatch marker", e);
                    }
                }
                ...
                if (target instanceof Marker) {
                    persistExtras.putString("deprecatedSince", "5.5");
                    persistExtras.putString("removalAt", "5.8");
                    persistExtras.putBoolean("dontSend", true);
    ...
    private Intent createIntentForPlugin(Intent fromIntent, Contact contact,
        ...
        if (i != null) {
            ...
            Bundle metaData = toBundle(i, null);
     ...
     * @param netContacts the array of network contacts to send to.
     */
    ...
    private Bundle toBundle(final MapItem item, AttributeSet data) {
        if (data == null) {
            data = item.getAttributes();
        }
        Bundle b = new Bundle();
        for (String key : data.getAttributeNames()) {
            Class type = data.getAttributeValueType(key);
            if (type == String.class) {
                b.putString(key, data.getStringAttribute(key));
            } else if (type == Integer.class) {
                b.putInt(key, data.getIntAttribute(key));
            } else if (type == Double.class) {
                b.putDouble(key, data.getDoubleAttribute(key));
            } else if (type == AttributeSet.class) {
                b.putBundle(key,
                        toBundle(item, data.getAttributeSetAttribute(key)));
            }
        }
        return b;
    }
