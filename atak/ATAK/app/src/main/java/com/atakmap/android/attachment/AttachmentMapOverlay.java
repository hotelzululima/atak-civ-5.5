...
public class AttachmentMapOverlay extends AbstractMapOverlay2 {
    ...
    public class AttachmentOverlayListModel extends AbstractHierarchyListItem2
        ...
        public void refreshImpl() {
            ...
            Map<MapItem, File[]> items = AttachmentManager.findAttachmentItems2(
                    _view.getRootGroup());
            for (Map.Entry<MapItem, File[]> entry : items.entrySet()) {
                MapItem m = entry.getKey();
        ...
        public Folder toKml(ExportFilters filters)
            ...
            for (HierarchyListItem item : children) {
                if (item instanceof Exportable
                    ...
                    if (itemFolder != null) {
                        List<Feature> featureList = itemFolder.getFeatureList();
                        if (!FileSystemUtils.isEmpty(featureList))
        ...
        public boolean goTo(boolean select) {
            ...
            if (ImageDropDownReceiver.ImageFileFilter.accept(null,
                ...
                intents.add(new Intent(ImageDropDownReceiver.IMAGE_DISPLAY)
                        .putExtra("uid", uid)
                        .putExtra("selectedImageURI",
                                Uri.fromFile(_attachment.getAttachment())
                                        .toString()));
    ...
    public static void deleteAttachmentMarker(MapItem item) {
        if (item != null && QuickPicReceiver.QUICK_PIC_IMAGE_TYPE
            ...
            AttachmentManager.notifyAttachmentChange(item.getUID());
            // get the number of attachments and if 0, remove from the group
            if (AttachmentManager.getNumberOfAttachments(item.getUID()) == 0
