...
public class GLDt2OutlineOverlay extends GLAbstractLayer2
    ...
    private GLDt2OutlineOverlay(MapRenderer surface, Instance subject) {
        ...
        _subject = subject;
        _dtedWatcher = ElevationMapComponent.getDtedWatcher();
    ...
    public void drawImpl(GLMapView ortho, int renderPass) {
        ...
        ortho.currentPass.scene.inverse(s.pointF, _corners[0]);

        s.pointF.x = ortho.getRight();
        ortho.currentPass.scene.inverse(s.pointF, _corners[1]);

        s.pointF.y = ortho.getBottom();
        ortho.currentPass.scene.inverse(s.pointF, _corners[2]);

        s.pointF.x = ortho.getLeft();
        ortho.currentPass.scene.inverse(s.pointF, _corners[3]);
