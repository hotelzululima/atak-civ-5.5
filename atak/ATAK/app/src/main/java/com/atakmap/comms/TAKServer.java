...
public class TAKServer {
    ...
    public TAKServer(Bundle bundle) throws IllegalArgumentException {
        ...
        if (connectString == null || connectString.trim().isEmpty()) {
            throw new IllegalArgumentException(
                    "Cannot construct TAKServer with empty/null connnectString");
        }
    ...
    public String getURL(boolean includePort) {
        try {
            ...
            if (proto.equalsIgnoreCase(CotServiceRemote.Proto.ssl.toString())
                    || proto.equalsIgnoreCase(
                            CotServiceRemote.Proto.quic.toString()))
                proto = "https";
            else
                proto = "http";
    ...
    public boolean enrollUseTrust() {
        return this.data.getBoolean(ENROLL_USE_TRUST_KEY, true);
    }
