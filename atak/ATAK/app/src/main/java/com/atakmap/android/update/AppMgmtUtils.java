...
public class AppMgmtUtils {
    ...
    /**
     * Obtains the apps min sdk version from an application described by the package name.
     * Note that if this is running on a device with Android 23 or less, it will return the
     * targetSdkVersion.
     * @param context the context to use
     * @param pkg the package name to resolve
     * @return the min sdk version unless running on a device < 24 then it will return the
     * target sdk version
     */
    static int getMinSdkVersion(Context context, String pkg) {
        PackageManager manager = context.getPackageManager();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            try {
                return manager.getApplicationInfo(pkg, 0).minSdkVersion;
            } catch (PackageManager.NameNotFoundException e) {
                Log.w(TAG, "Min SDK not found: " + pkg);
            }
        } else {
            return getTargetSdkVersion(context, pkg);
        }
    ...
    public static void uninstall(final Context ctx, final String packageName) {
        if (AtakPluginRegistry.isMandatoryLoad(ctx, packageName)) {
            Log.d(TAG, "Uninstalling is not allowed for: " + packageName);
            return;
        }
        try {
    ...
    /**
     * Returns true if the application is debuggable, false otherwise
     * @param context the ATAK context
     * @param packageName the package name
     * @return true if the package is debugable
     */
    public static boolean isDebuggable(final Context context,
        ...
        try {
            ...
            // debuggable will remain false if exception occurs
        }
        return debuggable;
    }
