...
public class AtakDocumentationPreferenceFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        configureDocumentPreference("atakIconGlossary",
                FileSystemUtils.SUPPORT_DIRECTORY + File.separatorChar +
                        "docs" + File.separatorChar + "ATAK_Icon_Glossary.pdf",
                false);
        ...
        if (SystemComponentLoader.getFlavorProvider() == null) {
            final File userDocumentTranslated = new File(
                    FileSystemUtils.getRoot(),
                    FileSystemUtils.SUPPORT_DIRECTORY + File.separatorChar
                            + "docs"
                            + File.separatorChar
                            + "ATAK_User_Guide_Translated.pdf");
            if (userDocumentTranslated.exists())
                IOProviderFactory.delete(userDocumentTranslated);
        }
        configureDocumentPreference("atakDocumentationTranslated",
                FileSystemUtils.SUPPORT_DIRECTORY + File.separatorChar +
                        "docs" + File.separatorChar
                        + "ATAK_User_Guide_Translated.pdf",
                true);
