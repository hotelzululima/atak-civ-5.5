    ...
    public CommsMapComponent.ImportResult importData(Uri uri, String mime,
        ...
        try {
            ...
            final String path = IOProviderFactory
                    .exists(new File(uriPath))
                            ? uriPath
                            : uri.toString();
            FeatureSetQueryParameters params = new FeatureSetQueryParameters();
            params.names = Collections.singleton(path);
            params.limit = 1;
            final boolean hasDataset = (modelDataStore
                    .queryFeatureSetsCount(params) > 0);
            if (hasDataset && b != null && b.getBoolean("ignoreExisting")) {
                return CommsMapComponent.ImportResult.IGNORE;
            }

            final Set<ModelInfo> models = ModelInfoFactory.create(path);
            final boolean importSuccess = (models != null && !models.isEmpty());
            if (importSuccess || (hasDataset
                    && (b == null || !b.getBoolean("retainOnImportFailure")))) {
                try {
                    params.limit = 0;

                    // delete existing to make way for import or evict on failure
            ...
            if (b != null && b.getBoolean(
                    ImportReceiver.EXTRA_SHOW_NOTIFICATIONS, true)) {
                NotificationUtil
                        .getInstance()
                        .postNotification(
                                notificationId,
            ...
            try {
                ...
                fs = new FeatureSet(fsid, first.type, first.type, path,
                        Double.MAX_VALUE, 5d,
                        FeatureDataStore2.FEATURESET_VERSION_NONE);
                ...
                for (final ModelInfo model : models) {
                    ...
                    final long refreshInterval = b.getLong("refreshInterval",
                            0L);
                    if (refreshInterval > 0L) {
                        if (model.metadata == null)
                            model.metadata = new AttributeSet();
                        model.metadata.setAttribute("refreshInterval",
                                refreshInterval);
                    }
                    if (b.getBoolean("streaming", false)) {
                        if (model.metadata == null)
                            model.metadata = new AttributeSet();
                        model.metadata.setAttribute("streaming", 1);
                    }
                    if (model.metadata != null) {
                    ...
                    // use AABB if available
                    if (model.metadata != null
                            && model.metadata.containsAttribute("aabb")) {
                        double[] aabb = model.metadata
                                .getDoubleArrayAttribute("aabb");
                        Envelope e = new Envelope(aabb[0], aabb[1], aabb[2],
                                aabb[3], aabb[4], aabb[5]);
                        geometry = GeometryFactory.fromEnvelope(e);
                        bounds.add(e);
                    }
                    ...
                    Feature f = new Feature(fsid,
                            ...
                            (models.size() == 1 && b != null)
                                    ? b.getString("datasetName", model.name)
                                    : model.name,
                            geometry,
                            style,
                            attributes,
            ...
            if (b != null && b.getBoolean(
                    ImportReceiver.EXTRA_SHOW_NOTIFICATIONS, true)) {
                Intent i = new Intent(
                        ImportExportMapComponent.ZOOM_TO_FILE_ACTION);
            ...
            Log.e(TAG, "Failed to import model: " + uri.getPath(), e);
            return CommsMapComponent.ImportResult.FAILURE;
        }
    }
