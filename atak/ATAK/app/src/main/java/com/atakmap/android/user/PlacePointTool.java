...
public class PlacePointTool {
    ...
    public static class MarkerCreator {
        ...
        private boolean allowDuplicativeNames = false;
        ...
        /**
         * When set, the PlacePointTool will allow for dropping of duplicative names
         * on the screen
         * @param duplicativeNames true if duplicative names will be allowed
         * @return the MarkerCreator for reuse in a chained call
         */
        public MarkerCreator setAllowDuplicativeNames(
                boolean duplicativeNames) {
            this.allowDuplicativeNames = duplicativeNames;
            return this;
        }
        ...
        public Marker placePoint() {
            ...
            ATAKUtilities.setAuthorInformation(marker);
            ...
            if (!allowDuplicativeNames) {
                if (mapGroup != null) {
                    if (prefix != null) {
                        ...
                        int count = 0;
                        List<MapItem> items = mapGroup.deepFindItems("type",
                                type);
            ...
            if (addToGroup && mapGroup != null) {
                // add the marker to the appropriate group. flag the marker as _editing_
                // around the add to hint for an interactive experience
                final boolean editing = marker.getEditing();
                if (!editing)
                    marker.setEditing(true);
                mapGroup.addItem(marker);
                if (!editing)
                    marker.setEditing(editing);
            }
    ...
    public static String genCallsign(final String type) {
        ...
        boolean legacyPointDropNaming = prefs.get(
