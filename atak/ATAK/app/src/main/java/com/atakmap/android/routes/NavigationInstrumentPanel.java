...
public class NavigationInstrumentPanel implements
    ...
    private static final String TAG = "NavigationInstrumentPanel";
    ...
    private final RoutePanelViewModel stateManager;
    ...
    public void onSharedPreferenceChanged(SharedPreferences p, String key) {
        ...
        if (key.equals(VOICE_CUE_PREFERENCE_KEY)) {
            boolean voice_cue = _prefs.get(key, true);
    ...
    private void addWidgets() {
        ...
        if (mapView.isPortrait()) {
            if (_prefs.get("nav_orientation_right", true))
        ...
        if (totalRenderWidth > minLayoutSize) {
            ...
            Log.d(TAG, "Setting scale to " + scale);
    ...
    private final float textFadeSpeed = 1;
    private final float textSlideSpeed = 1.9f;
    private final float textLayoutOpacitySpeed = 1;
    ...
    private LayoutWidget buildInstrumentCluster(int height,
        ...
        stateManager.setUnits(
                Integer.parseInt(_prefs.get("speed_unit_pref", "0")));

        final MapWidget.OnClickListener mocl = new MapWidget.OnClickListener() {
            @Override
            public void onMapWidgetClick(MapWidget widget, MotionEvent event) {
        ...
        backdrop.addOnClickListener(mocl);
        speedLayoutWidget.addOnClickListener(mocl);
        speedTextWidget.addOnClickListener(mocl);
        speedUnitsTextWidget.addOnClickListener(mocl);
        distanceTextWidget.addOnClickListener(mocl);
    ...
    private void updateBillboardWidget() {
        if (billboardWidget != null) {
            boolean enabled = _prefs.get("route_billboard_enabled",
                    true);
    ...
    private LayoutWidget buildSpeedWidget(int height,
        ...
        billboardWidget.addOnClickListener(new MapWidget.OnClickListener() {
            @Override
            public void onMapWidgetClick(MapWidget widget, MotionEvent event) {
                _prefs.get("route_billboard_enabled",
                        !_prefs.get("route_billboard_enabled", true));
            }
        });
        ...
        //-------------------- Build Volume Widget ---------------------------
        boolean voice_cue = AtakPreferences.getInstance(mapView.getContext())
                .get(VOICE_CUE_PREFERENCE_KEY, true);
        ...
        volumeWidget.addOnClickListener(new MapWidget.OnClickListener() {
            @Override
            public void onMapWidgetClick(MapWidget widget, MotionEvent event) {
                boolean voice_cue = AtakPreferences
                ...
                AtakPreferences.getInstance(mapView.getContext())
                        .set(VOICE_CUE_PREFERENCE_KEY, !voice_cue);
            }
        });
