...
public abstract class MapActivity extends MetricFragmentActivity {
    ...
    protected void onDestroyComponents() {
        final MapView view = getMapView();
        final MapComponent[] list = _observers.toArray(new MapComponent[0]);

        for (int i = list.length; i > 0; i--) {
            final MapComponent observer = list[i - 1];
            try {
                //Log.e(TAG, "calling onDestroy for: " + list[i-1].getClass());
                observer.onDestroy(this, view);

                // since the MapActivity is being destroyed, remove the observer from the
                // list of observers since it is no longer valid past onDestroy.
                if (!_observers.remove(observer))
                    Log.e(TAG, "error removing: " + observer);
            } catch (Exception e) {
                Log.e(TAG, "error calling onDestroy for: " + observer, e);
            }
        }
        _observers.clear();
        ...
        synchronized (lifecycleTransitionLock) {
            ...
            if (observer != null) {
                ...
                if (BuildConfig.DEBUG) {
                    Log.d(TAG,
                            "initialization of: " + observer.getClass()
                                    + " took= "
                                    + (SystemClock.elapsedRealtime() - s) + "ms"
                                    + (getMainLooper().getThread() == Thread
                                            .currentThread() ? " [ui]"
                                                    : " [async]"));
                }
            }
    ...
    /**
     * Remove a MapComponent from the mapping engine
     *
    ...
    protected void asyncLoadAssets(Runnable thenRun) {
        ...
        MapComponentLoader.asyncLoadMapComponents(this, new Runnable() {
            public void run() {
                try {
                    ConfigEnvironment.Builder configBuilder = new ConfigEnvironment.Builder();
                    ConfigEnvironment config = configBuilder
                            .setMapAssets(getMapAssets())
                            .build();
                    loadListeners("map_lngpress", "actions/map_click.xml",
                            config);
                    loadListeners("item_click", "actions/item_click.xml",
                            config);
