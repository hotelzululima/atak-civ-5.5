...
public class URIHelper {
     ...
     * @return File object or null if unable to create the file object
     */
    public static File getFile(String fileURI) {
        ...
        String path = decodedCache.get(fileURI);

        // cache miss
        if (path == null) {

            // dump the cache if it has exceeded the max cache size
            if (decodedCache.size() > MAX_CACHE_SIZE)
                decodedCache.clear();

            path = getContent(URIScheme.FILE, fileURI);
            if (FileSystemUtils.isEmpty(path))
                return null;
            path = Uri.decode(path);
            decodedCache.put(fileURI, path);
        }
    ...
    private static final Map<String, String> decodedCache = new ConcurrentHashMap<>();

    /**
     * Convert a file path to a file URI
     *
     * @param file File object
     * @return File URI string or null
     */
     ...
     * @return the uri for the mission package
     */
