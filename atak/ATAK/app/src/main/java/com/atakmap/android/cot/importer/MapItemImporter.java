...
public abstract class MapItemImporter extends CotEventTypeImporter {
    ...
    protected final boolean ignoreLateCoTEvents;

    protected final TimeTrackingProcessService timeTrackingService = new TimeTrackingProcessService();
    ...
    protected MapItemImporter(MapView mapView, MapGroup group,
        ...
        ignoreLateCoTEvents = (DeveloperOptions
                .getIntOption("mapitemimporter.ignore-late-cot", 0) != 0);
    }
    ...
    // No type filtering
    protected MapItemImporter(MapView mapView, MapGroup group) {
        this(mapView, group, new HashSet<>());
    }
    ...
    public ImportResult importData(CotEvent event, Bundle extras) {
        ...
        TimeTrackingProcessService.PendingToken processToken = null;

        if (ignoreLateCoTEvents) {
            processToken = timeTrackingService.begin(event.getUID(),
                    event.getTime());

            // the event time is not the latest, so ignore
            if (processToken == null)
                return ImportResult.IGNORE;
        }

        boolean commitProcess = false;
        try {
            ...
            if (res == ImportResult.SUCCESS) {
                ...
                if (item != null) {
                    try {
                        ...
                        Log.e(TAG,
                                "Error when notifying " + MapEvent.ITEM_IMPORTED
                                        + " listeners for: " + existing,
                                e);
                    }
                } else {
                    final String type = event.getType();
                    if (type != null && type.startsWith("b-a-o")) {
                        // do nothing   
                    } else {
                        Log.w(TAG,
                                "Import successful but item is null: " + type,
                                new Throwable());
                    }
                }
                commitProcess = true;
            ...
            if (ignoreLateCoTEvents) {
                if (commitProcess)
                    timeTrackingService.commit(processToken);
                else
                    timeTrackingService.cancel(processToken);
            }
        }
    }
    ...
    protected void postNotification(MapItem item) {
        ...
        if (point != null)
            msg = CoordinateFormatUtilities.formatToString(point,
                    CoordinateFormat.find(_prefs.getSharedPrefs()));
