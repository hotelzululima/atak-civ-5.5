...
public class MapTouchController implements OnTouchListener,
        ...
        OnDoubleTapListener, Disposable {
    ...
    protected void onItemLongPress(MapItem item, MotionEvent event) {
        ...
        eventBuilder
                .setPoint(new PointF((int) event.getX(), (int) event.getY()))
                .setItem(item);
    ...
    protected void onMapLongPress(MotionEvent event) {
        ...
        eventBuilder.setPoint(
                new PointF((int) event.getX(), (int) event.getY()));
        eventDispatcher.dispatch(eventBuilder.build());
    }
    ...
    protected void onMapPress(MotionEvent event, GeoPoint touchPoint) {
        ...
        PointF screenTouchPoint = new PointF((int) event.getX(),
                (int) event.getY());

        _mapView.getMapData().setMetaString("touchPoint",
                touchPoint.toString());
    ...
    public boolean onDown(MotionEvent event) {
        ...
        //ArrayList<MapItem> unfiltered = hitItems != null ? new ArrayList<>(hitItems) : null;

        hitItems = filterItems(hitItems, false);
        if (hitItems != null && !hitItems.isEmpty())
            _downHitItem = hitItems.first();

        //Log.i(TAG, "HIT ITEMS: " + unfiltered + " filtered " + hitItems);
    ...
    @Override
    public boolean onFling(@Nullable MotionEvent arg0,
            @NonNull MotionEvent arg1, float arg2,
            float arg3) {
        return false;
    }

    @Override
    public void onLongPress(@NonNull
    ...
    private SortedSet<MapItem> filterItems(final SortedSet<MapItem> hitItems,
        ...
        if (hitItems != null && hitItems.size() > 1) {
            ...
            iterLoop: while (iter.hasNext()) {
                ...
                // if this item is found in the list of items, discard all other hits and
                // return this item only.   Should be used with care and is only used to
                // model the expected behavior with the blue seeker bar
                if (hitItem.getMetaBoolean("force_only_touch", false)) {
                    // singleton sorted set
                    final TreeSet<MapItem> mi = new TreeSet<>(
                            new Comparator<MapItem>() {
                                @Override
                ...
                if (move && (!(hitItem instanceof PointMapItem)
                        ...
                        && !((hitItem instanceof EditablePolyline)
                                && hitItem.getEditable())
                        || !hitItem.getMovable()
    ...
    public static boolean goTo(MapItem item, boolean select) {
        return goTo(item, select, false);
    }

    public static boolean goTo(MapItem item, boolean select,
            boolean allowNullGroup) {
        // Do nothing if the item no longer exists
        if (item == null || (!allowNullGroup && item.getGroup() == null))
            return true;
        ...
        if (!hitItems.isEmpty()) {
            ...
            for (int i = 0; i < tracks.size(); i++) {
                ...
                if (item instanceof Shape)
                    touchPoints.put(i, item.getClickPoint());
            }
    ...
    public boolean onScroll(@Nullable MotionEvent e1, @NonNull MotionEvent e2,
        ...
        if ((_itemPressed != null) && (_itemDragged != null)) {
            ...
            MapEvent.Builder eventBuilder = new MapEvent.Builder(
                    MapEvent.MAP_SCROLL);
            eventBuilder
                    .setPoint(new PointF((int) dx, (int) dy));
            ...
            MapEvent.Builder drawEventBuilder = new MapEvent.Builder(
                    MapEvent.MAP_DRAW);
            drawEventBuilder
                    .setPoint(new PointF((int) e2.getX(), (int) e2.getY()));
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        if (sm == null) {
            // ATAK-18808 Playstore Crash: NullPointerException MapTouchController.isPoleInView
            return true;
        }
        ...
        if (_tiltLockState == STATE_TILT_ENABLED || isFreeForm3DEnabled()) {
            ...
            eb.setPoint(new PointF((int) e1.getX(), (int) e1.getY()));
            eb.setExtras(b);
            MapEvent evt = eb.build();
            _mapView.getMapEventDispatcher().dispatch(evt);
            if (Double.compare(tiltBy, 0) != 0 && evt.getExtras()
                ...
                if (ffPoint != null) {
                    _glglobe.lookAt(focusPoint, sm.gsd, sm.camera.azimuth,
                            newTilt, collide, false);
                } else {
    ...
    private void updateNadirClamp() {
        ...
        if (Double.compare(_mapView.getMapTilt(), 0) == 0) {
            GLMapSurface surface = _mapView.getGLSurface();
            if (surface != null) {
                GLMapView glMapView = surface.getGLMapView();
                if (glMapView != null) {
                    glMapView.visitControl(null,
    ...
    private void onGestureStart(float gestureFocusX, float gestureFocusY,
        ...
        if (sm == null) {
            // scene model can be null
            // ATAK-18807 Playstore Crash: NullPointerException MapTouchController
            return;
        }
        ...
        if (Double.isNaN(_originalAgl))
            _originalAgl = 0d;
        final double localel = (_glglobe instanceof GLMapView)
                ? ((GLMapView) _glglobe).getTerrainMeshElevation(
                        cam.getLatitude(), cam.getLongitude())
                : ElevationManager.getElevation(cam.getLatitude(),
