...
public class Wedge extends Polyline {
    ...
    public Wedge(long serialId, MetaDataHolder2 metadata, String uid) {
    ...
    public Wedge setAngle(double angleXprime,
        ...
        recomputeWedge();
        AttributeSet labels = this.getMetaAttributeSet("labels");
        if (labels == null) {
            labels = new AttributeSet();
            this.setMetaAttributeSet("labels", labels);
        }

        AttributeSet seg0 = labels.getAttributeSetAttribute("seg0");
        AttributeSet seg1 = labels.getAttributeSetAttribute("seg1");
        AttributeSet seg2 = labels.getAttributeSetAttribute("seg2");
        labels.clear();

        if (label0 != null) {
            if (seg0 == null)
                seg0 = new AttributeSet();
            seg0.setAttribute("segment", 0);
            seg0.setAttribute("text", label0);
            labels.setAttribute("seg0", seg0);
        }

        if (label1 != null) {
            if (seg1 == null)
                seg1 = new AttributeSet();
            seg1.setAttribute("segment",
                    (int) Math.ceil(NUM_SEGMENTS / 2.0));
            seg1.setAttribute("text", label1);
            labels.setAttribute("seg1", seg1);
        }

        if (label2 != null) {
            if (seg2 == null)
                seg2 = new AttributeSet();
            seg2.setAttribute("segment", NUM_SEGMENTS + 1);
            seg2.setAttribute("text", label2);
