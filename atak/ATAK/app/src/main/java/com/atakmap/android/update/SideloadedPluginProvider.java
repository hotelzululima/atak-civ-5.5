    ...
    public ProductRepository parseRepo(final String repoType,
        ...
        if (repo != null && repo.isValid()) {
            ...
            for (ProductInformation plugin : repo.getProducts()) {
                ProductInformation slp = null;

                if (plugin.isPlugin()) {
                    slp = new SideloadedPluginInformation(
                            plugin, _context);
                    ...
                    try {
                        ApplicationInfo appInfo = _context.getPackageManager()
                                .getApplicationInfo(plugin.getPackageName(), 0);
                        slp = new SideloadedAppInformation(plugin.getParent(),
                                _context, appInfo);
                    } catch (Exception e) {
                        Log.e(TAG, "error finding app information for: "
                        ...
                        new AtakPluginRegistry.PluginLoadingProgressCallback() {
                            ...
                            public void onComplete(
                                ...
                                if (discovered != null
                                    ...
                                    for (ApplicationInfo applicationInfo : pluginRegistry
                                            .getAssociatedApplications()) {
                                        ProductInformation product = new SideloadedAppInformation(
                                                repo, context, applicationInfo);
                                        if (product.isValid())
                                            products.add(product);
                                    }
