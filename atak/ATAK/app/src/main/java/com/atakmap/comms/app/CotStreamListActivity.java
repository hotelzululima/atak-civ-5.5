...
public class CotStreamListActivity extends CotPortListActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        MenuItem quickConnect = menu.findItem(R.id.add_menu_quickconnect);
        if (quickConnect != null)
            quickConnect.setVisible(true);
        MenuItem datapackage = menu.findItem(R.id.add_menu_datapackage);
        if (datapackage != null)
            datapackage.setVisible(true);
        return retval;
    }


    /**
     * gets an array of descriptions for each TAKServer in the list
     */
    private ArrayList<String> getDescriptionArray() {
        ArrayList<String> ret = new ArrayList<>(_portList.size());
        for (TAKServer p : _portList)
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        } else if (item.getItemId() == R.id.add_menu_quickconnect) {
            CertificateEnrollmentClient.getInstance().enroll(
                    this,
                    null, null, null, null,
                    null, true);
        } else if (item.getItemId() == R.id.add_menu_datapackage) {
            ImportMissionPackageSort
                    .importMissionPackage(this);
        }
        return result;
    }
        ...
        switch (requestCode) {
            case _REQUEST_ADD_PORT:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    if (connectString != null && inputData != null) {
                        try {
                            _remote.addStream(connectString, inputData);
                            inputData.putString(TAKServer.CONNECT_STRING_KEY,
                                    connectString);
                            _putItem(new TAKServer(inputData));
                ...
                builder.setPositiveButton(R.string.yes,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                _remote.removeStream(connectString);
                                _removeItem(new TAKServer(bundle));
                                removeDialog(id);
                            }
                        });
    ...
    @Override
    protected void enabledFlagSet(TAKServer port) {
