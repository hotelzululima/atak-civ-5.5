...
public class RoutePlannerView extends LinearLayout implements
    ...
    private RoutePlannerInterface2 _planner;
    ...
    protected ImageButton _strokeWeightButton, _strokeStyleButton;
    ...
    private static final double ROUTE_MAX_STROKE_WEIGHT = 24d;
    ...
    public DropDownReceiver init(MapView mapView, Route route,
            RouteMapReceiver receiver, RoutePlannerInterface2 autoPlan) {
        _mapView = mapView;
        ...
        _strokeStyleButton = findViewById(R.id.route_style);
        _strokeStyleButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {

                AlertDialog.Builder b = new AlertDialog.Builder(getContext());
                b.setTitle(R.string.line_style);
                b.setSingleChoiceItems(R.array.line_styles,
                        route.getStrokeStyle(),
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                if (style < 0)
                                    return;
                                route.setStrokeStyle(style);
                                DrawingPreferences prefs = new DrawingPreferences(
                                        mapView);
                                prefs.setStrokeStyle(style);
                                dialog.dismiss();
                            }
                        });
        ...
        _strokeWeightButton = findViewById(R.id.route_stroke);
        _strokeWeightButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                final double oldStrokeWeight = route.getStrokeWeight();
                final AlertDialog.Builder popDialog = new AlertDialog.Builder(
                        _context);
                final View v = LayoutInflater.from(_context)
                        .inflate(R.layout.stroke_seekbar, null);
                final SeekBar sb = v.findViewById(R.id.strokeSeek);
                sb.setProgress((int) (oldStrokeWeight / ROUTE_MAX_STROKE_WEIGHT
                        * sb.getMax()));
                popDialog.setView(v);
                ...
                sb.setOnSeekBarChangeListener(
                        new SeekBar.OnSeekBarChangeListener() {
                            ...
                            public void onProgressChanged(SeekBar seekBar,
                                    int i, boolean b) {
                                route.setStrokeWeight((sb.getProgress()
                                        / (double) sb.getMax())
                                        * ROUTE_MAX_STROKE_WEIGHT);
                                _prefs.set("routeLastUserThickness",
                                        route.getStrokeWeight());
                            }
                ...
                popDialog.setNegativeButton(R.string.cancel,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface,
                                    int i) {
                                route.setStrokeWeight(oldStrokeWeight);
                            }
                        });
                popDialog.setPositiveButton(R.string.ok, null);
                popDialog.show();
            }
        });
        ...
        _routeName.addTextChangedListener(new AfterTextChangedWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (!s.toString().equals(_route.getTitle())) {
                    if (MetricsApi.shouldRecordMetric()) {
                        final Bundle b = new Bundle();
                        b.putString("old_name", _route.getTitle());
                        b.putString("new_name", s.toString());
                        MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                                MetricsUtils.EVENT_MAPITEM_CHANGED,
                                "RoutePlannerView", _route, b);
                    }
                    updateName();
                }
            }
        ...
        _remarksLayout.addTextChangedListener(new AfterTextChangedWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (route != null && remarks != null
                        && !FileSystemUtils.isEquals(remarks,
                                _route.getRemarks()))
        ...
        if (autoPlan != null && route.getNumWaypoint() >= 2) {
            ...
            if (origin != null && dest != null) {
                ...
                //pull the waypoints if any exist other then SP/VDO
                if (route.getNumWaypoint() > 2) {
                    for (int i = 1; i < route.getNumWaypoint() - 1; i++) {
                        PointMapItem pmi = route.getPointMapItem(i);
                        if (pmi != null) {
                            byWayOf.add(pmi.getPoint());
                        }
                    }
                }
                _routeGenHandler = new RouteGenerationHandler(_mapView,
                        origin, dest, _route, byWayOf);
                RouteGenerationPackage routeGen = new RouteGenerationPackage(
                        _prefs.getSharedPrefs(), origin.getPoint(),
                        dest.getPoint(), byWayOf);
    ...
    private void onClose() {
        if (_active) {
            ...
            if (_route.hasMetaValue("archive")) {
                ...
                if (alpha < 255)
                    _route.setRenderHints(0);
                _route.persist(_mapView.getMapEventDispatcher(), null,
                        RoutePlannerView.class);
                if (alpha < 255)
                    _route.setRenderHints(Polyline.RENDER_FLAG_FADE_ITEM);
            }
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.undoButton) {
            ...
            if (MetricsApi.shouldRecordMetric()) {
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_ACTION, "RouteEditTool",
                        _route, "undo");
            }
        ...
        // Toggle draw mode
            ...
            if (tool != null) {
                ...
                if (MetricsApi.shouldRecordMetric()) {
                    ...
                    details.putString(MetricsUtils.FIELD_INFO,
                            tool.inDrawMode() ? "drawing" : "standard");
                    MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                            MetricsUtils.EVENT_MAPITEM_ACTION, "RouteEditTool",
                            _route, details);
                }
        ...
        // Start route navigation
            ...
            if (MetricsApi.shouldRecordMetric()) {
                Bundle b = new Bundle();
                b.putString(MetricsUtils.FIELD_ACTION_IDENTIFIER, "start_nav");
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
        ...
        // Plan route using interface such as VNS
            ...
            if (MetricsApi.shouldRecordMetric()) {
                Bundle b = new Bundle();
                b.putString(MetricsUtils.FIELD_ACTION_IDENTIFIER,
                        "start_route_planner");
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_ACTION,
                        "RoutePlannerView", _route, b);
        ...
        // Toggle route edit mode
            ...
            if (MetricsApi.shouldRecordMetric()) {
                Bundle b = new Bundle();
                b.putString(MetricsUtils.FIELD_ACTION_IDENTIFIER, "edit_route");
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_ACTION,
                        "RoutePlannerView", _route, b);
            }
            if (!findViewById(R.id.edit_route).isSelected())
                startEdit();
            else
                endTool();
        }
    ...
    protected void refresh() {
        ...
        _mapView.post(new Runnable() {
            ...
            public void run() {
                ...
                if (_route.getMetaBoolean("editable", true)) {
                    _editRoute.setEnabled(true);
                } else {
                    _editRoute.setEnabled(false);
                }
    ...
    protected class ColorClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            palette.setShowAlpha(true);
            palette.setColor(_route.getColor());
            b.setView(palette);
            b.setPositiveButton(R.string.ok,
                    new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialogInterface,
                            ...
                            refresh();
                            _prefs.set("route_last_selected_color", color);
                        }
                    });
            b.setNegativeButton(R.string.cancel, null);
    ...
    private class ContactPointAdapter extends BaseAdapter {
        ...
        void refresh() {
            ...
            _distFmt = Integer.parseInt(_prefs.get("rab_rng_units_pref",
    ...
    private class RowListener implements View.OnClickListener {
                ...
                b.setPositiveButton(R.string.done,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface d, int w) {
                                ...
                                final String prevName = cp.getTitle();
                                ...
                                if (MetricsApi.shouldRecordMetric()) {
                                    final Bundle b = new Bundle();
                                    b.putString("old_name", prevName);
                                    b.putString("new_name", name);
                                    b.putString("point", cp.getUID());
                                    MetricsUtils.record(
        ...
        private void promptRemoveWaypoint() {
            ...
            if (MetricsApi.shouldRecordMetric()) {
                ...
                bundle.putString("item", cp.getUID());
                bundle.putString("action", "selected");
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_CHANGED,
                        "RoutePlannerView", _route, bundle);
            }
        }
    ...
    static class PlanRouteAction extends EditAction {
        ...
        private final List<GeoPoint> _requiredWaypoints = new ArrayList<>();

        PlanRouteAction(Route route, PointMapItem origin,
                PointMapItem dest, RoutePointPackage rpp,
                List<GeoPoint> requiredWaypoints) {
            _route = route;
            _origin = origin;
            _dest = dest;
            _routePointPackage = rpp;
            _requiredWaypoints.addAll(requiredWaypoints);
        }

        /**
         * Runs the PlanRouteAction. By default, this will take the points and navigation cues
         * contained by the {@code RoutePointPackage} this class was constructed with and replace
         * points in the {@code route} between a starting index and ending index this class was
         * constructed with. The starting index is identified by searching for the {@code origin} in
         * the route. If the origin is not found, the startig index will be defaulted 0. Likewise,
         * the ending index will be identified by searching for the {@code dest} in the route, and
         * if it cannot be found, the ending index will be defaulted to the last position in the
         * route.
         *
         * @return Indicates if the action was able to be successfully run
         */
        @Override
        public boolean run() {
            ...
            if (_indexOfOrigin == -1) {
                Log.d(TAG, "Defaulting origin to index 0");
                _indexOfOrigin = 0;
            }

            if (indexOfDest == -1) {
                int indexOfLastPoint = _route.getNumPoints();
                Log.d(TAG, "Defaulting dest to index " + indexOfLastPoint);
                indexOfDest = indexOfLastPoint;
            }
            ...
            for (GeoPoint reqWpt : _requiredWaypoints) {
                PointMapItem item = Collections.min(_route.getPointMapItems(),
                        new Comparator<PointMapItem>() {
                            @Override
                            public int compare(PointMapItem pmi1,
                                    PointMapItem pmi2) {
                                if (pmi1.getType().equals(Route.WAYPOINT_TYPE)
                                        &&
                                        pmi2.getType()
                                                .equals(Route.WAYPOINT_TYPE)) {
                                    // find distance from pmi1 from reqWypt
                                    // find distance from pmi2 from reqWypt
                                    double distance1 = pmi1.getPoint()
                                            .distanceTo(reqWpt);
                                    double distance2 = pmi2.getPoint()
                                            .distanceTo(reqWpt);
                ...
                if (item != null) {
                    item.setMetaBoolean(
                            RouteRequiredWayPointHandler.IS_WAY_POINT_REQUIRED,
                            true);
                }
            }
            ...
            if (!Thread.currentThread().isInterrupted()) {
                ...
                if (mv != null) {
                    _route.refresh(mv.getMapEventDispatcher(), null,
                            getClass());
                }
            }
        ...
        /**
         * Replaces the points in the route between {@code indexOfOrigin} and {@code indexOfDest}
         * with {@code points} and append the navigation cues with {@code cues}.
         *
         * @param indexOfOrigin start index, inclusive
         * @param indexOfDest ending index, EXCLUSIVE
         * @param points replacement points
         * @param cues navigation cues that will be appended
         * @return the old points that have now been replaced
         */
    ...
    protected void onDetachedFromWindow() {
        ...
        final ProgressDialog dialog = _routeGenHandler.getDialog();
