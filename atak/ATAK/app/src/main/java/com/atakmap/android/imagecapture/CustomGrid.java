...
public class CustomGrid extends AbstractLayer implements Capturable {
    ...
    synchronized DoubleBuffer getPointBuffer() {
        ...
        if (_points == null) {
            ...
            setColor(prefs.get(CustomGrid.COLOR_PREF, Color.WHITE));
            setSpacing(prefs.get(CustomGrid.SPACING_PREF, 100f));
            setShowLabels(prefs.get(CustomGrid.LABELS_PREF, true));
            ...
            IGeoPoint iTopRight = (IGeoPoint) MarshalManager.marshal(_topRight,
                    GeoPoint.class,
                    gov.tak.api.engine.map.coords.GeoPoint.class);
            double xDist = gov.tak.api.engine.map.coords.GeoCalculations
