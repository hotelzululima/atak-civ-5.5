...
public class DrawingToolsToolbar extends DropDownReceiver implements
    ...
    public void toggleEditButtons(boolean edit) {
        ...
        if (edit) {
            final String s = ToolbarBroadcastReceiver.getInstance().getActive();
            if (!DrawingToolsToolbar.TOOLBAR_IDENTIFIER.equals(s)) {
                closeToolbar = true;
        ...
        // defer until after the toolbar is set up and displayed
        getMapView().postDelayed(new Runnable() {
            @Override
            public void run() {
                toggleMilSymVisibility(edit ? View.GONE : View.VISIBLE);
            }
        }, 50);
    ...
    public Drawable getToolbarIcon() {
        return _context.getDrawable(R.drawable.nav_draw);
    }
    ...
    private void toggleMilSymVisibility(int visibility) {

        final ActionBarView abv = getToolbarView();
        if (abv == null)
            return;

        final LinearLayout ll = (LinearLayout) abv.getParent();
        if (ll == null)
            return;
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if ("milsym".equals(c.getTag(R.id.name)))
