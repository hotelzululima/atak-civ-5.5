...
public class CircleEditTool extends ButtonTool implements Undoable,
    ...
    public boolean onToolBegin(Bundle extras) {
        ...
        _circle.setEditing(true);
        final MapItem m = _circle.getCenterMarker();
        if (m != null)
            m.setEditing(true);
        ...
        final GeoBounds bounds = _circle.getBounds(null);
        if (!bounds.intersects(_mapView.getBounds()))
            ATAKUtilities.scaleToFit(_circle);
    ...
    public void onMapEvent(final MapEvent event) {
        ...
        if (item == _circle.getCenterMarker()) {
            ...
            if (point == null)
                return;
            // examine the hit type. if the hit type is `Type.FILL`, ignore.
            final HitTestResult.Type hit_type = (HitTestResult.Type) point
                    .getMetaData(
                            "hit_type");
            if (hit_type == HitTestResult.Type.FILL)
                return;
