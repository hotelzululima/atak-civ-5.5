...
public class KLV {
    ...
    /** 
     * Produce a byte array representing the int32 value from which was passed in.
     */
    static public byte[] int32ToByteArray(final int val) {
        return ByteBuffer.allocate(4).putInt(val).array();
    }

    /** 
     * Produce a byte array representing the byte value from which was passed in.
     */
    static public byte[] int8ToByteArray(final byte val) {
        return new byte[] {
                val
        };
    }
    ...
    /**
     * Produce a byte array representing the short value which was passed in.
     */
    static public byte[] int16ToByteArray(final short val) {
        return ByteBuffer.allocate(2).putShort(val).array();
    }

    /**
     * Produce a byte array representing the long value which was passed in.
     */
    static public byte[] int64ToByteArray(final long val) {
        return ByteBuffer.allocate(8).putLong(val).array();
    }

    /**
     * Produce a byte array representing the String value which was passed in.
     */
    static public byte[] stringToByteArray(final String val) {
        return val.getBytes();
    }
     ...
     */
    public KLV(byte[] theBytes, KeyLength keyLength,
     ...
     */
    public KLV(byte[] theBytes, int offset, KeyLength keyLength,
            LengthEncoding lengthEncoding) {
        ...
        int valueOffset = setLength(theBytes, offset + keyLength.value(),
                lengthEncoding);
        ...
        System.arraycopy(theBytes, valueOffset, this.value, 0,
                this.value.length);
     ...
     */
    public KLV(int shortKey, KeyLength keyLength,
            LengthEncoding lengthFieldEncoding, byte[] value) {
        this(shortKey, keyLength, lengthFieldEncoding, value, 0,
     ...
     * then a zero-length value is assumed.
     */
    public KLV(int shortKey, KeyLength keyLength, LengthEncoding lengthEncoding,
     ...
     * then a zero-length value is assumed.
     */
    public KLV(byte[] key, LengthEncoding lengthEncoding, byte[] value,
        ...
        if (lengthEncoding == null)
            throw new NullPointerException("Length encoding must not be null.");
        if (!(key.length == 1 || key.length == 2 || key.length == 4
                || key.length == 16))
            throw new IllegalArgumentException(
                    "Key length must be 1, 2, 4, or 16 bytes, not "
                            + key.length);
        if (value != null) {
    ...
    public byte[] toBytes() {
        ...
        byte[] lengthField = KLV.makeLengthField(this.lengthEncoding,
                this.value.length);
        byte[] bytes = new byte[key.length + lengthField.length
                + this.value.length];

        System.arraycopy(key, 0, bytes, 0, key.length);
        System.arraycopy(lengthField, 0, bytes, key.length, lengthField.length);
        System.arraycopy(this.value, 0, bytes, key.length + lengthField.length,
                this.value.length);
     ...
     */
    public List<KLV> getSubKLVList(KeyLength keyLength,
            LengthEncoding lengthEncoding) {
     ...
     * 
     */
    public Map<Integer, KLV> getSubKLVMap(KeyLength keyLength,
            LengthEncoding lengthEncoding) {
    ...
    public int getValueAs16bitSignedInt() {
        ...
        int shortLen = Math.min(length, 2);
    ...
    public int getValueAs32bitInt() {
        ...
        int value = 0;
        int length = bytes.length;
        int shortLen = Math.min(length, 4);
        for (int i = 0; i < shortLen; i++)
            value |= (bytes[i] & 0xFF) << (shortLen * 8 - i * 8 - 8);
        return value;
    ...
    public long getValueAs64bitLong() {
        ...
        long value = 0;
        int length = bytes.length;
        int shortLen = Math.min(length, 8);
        for (int i = 0; i < shortLen; i++)
     ...
     * @throws ArrayIndexOutOfBoundsException   If offset is invalid
     */
    public int setLength(byte[] inTheseBytes, int offset,
            LengthEncoding lengthEncoding) {
            ...
            case BER:
                ...
                // Else, use following bytes to determine length
                    ...
                    for (int i = 0; i < following; i++) {
                        length |= (inTheseBytes[offset + 1 + i]
                                & 0xFF) << (following - 1 - i) * 8;
                    }
                    setLength(length, lengthEncoding);
    ...
    public KLV setLength(int length, LengthEncoding lengthEncoding) {
        ...
        if (this.value != null) {
            System.arraycopy(value, 0, bytes, 0,
                    Math.min(length, this.value.length));
        } // end if: value exists
        this.value = bytes;
    ...
    public KLV addSubKLV(int key, String subValue) {
        if (subValue == null) {
            ...
            try {
                return addSubKLV(key,
                        subValue.getBytes(KLV.DEFAULT_CHARSET_NAME));
            } catch (java.io.UnsupportedEncodingException exc) {
     ...
     * @param offset   offset within <code>extraBytes</code>
     * @param length   length of <code>extraBytes</code> to use
     * @return              <tt>this</tt>, to aid in stringing commands together.
     */
    ...
    public String toString() {
        ...
        if (this.keyLength.value() <= 4)
            ...
            for (byte b : longKey) {
                hexChar = Long.toHexString(b & 0xFF);
                if (hexChar.length() < 2) {
                    hexChar = "0" + hexChar; // pad with leading zero if needed
                }
    ...
    public static List<KLV> bytesToList(
        ...
        while (currentPos < offset + length) {
            try {
                ...
                System.err.println(
                        "Stopped parsing with exception: " + exc.getMessage());
                break;
            } // end catch
    ...
    public static Map<Integer, KLV> bytesToMap(
        ...
        Map<Integer, KLV> map = new HashMap<>();

        for (KLV klv : KLV.bytesToList(bytes, offset, length, keyLength,
                lengthEncoding)) {
     ...
     */
    protected static byte[] makeLengthField(LengthEncoding lengthEncoding,
            int payloadLength) {
        ...
        switch (lengthEncoding) {
            ...
            case OneByte:
                if (payloadLength > 255)
                    throw new IllegalArgumentException(
                            String.format(
                                    "Too much data (%d bytes) for one-byte length field encoding.",
                                    payloadLength));
                bytes = new byte[] {
                        (byte) payloadLength
                };
            ...
            case TwoBytes:
                if (payloadLength > 65535)
                    throw new IllegalArgumentException(
                            String.format(
                                    "Too much data (%d bytes) for two-byte length field encoding.",
                                    payloadLength));
                bytes = new byte[] {
            ...
            case BER:
                if (payloadLength <= 127) {
                    ...
                    if (payloadLength <= 255) { // One byte
                        bytes = new byte[] {
                                (byte) 0x81, (byte) payloadLength
                        };
                    } else if (payloadLength <= 65535) { // Two bytes
                        bytes = new byte[] {
                                (byte) 0x82, (byte) (payloadLength >> 8),
                                (byte) payloadLength
                        };
                    } else { // Four bytes
                        bytes = new byte[] {
            ...
            default:
                throw new IllegalStateException(
                        "Unknown length field encoding flag: "
                                + lengthEncoding);
        } // end switch
        return bytes;
    } // end makeLengthField
