...
public class GLViewShed2 extends GLHeatMap implements
    ...
    private int opacitySeen;
    private int opacityUnseen;

    private boolean showCircle = true;

    private double calculatedRadius = 0;
    private GeoPoint calculatedPOI = null;
    private int calculatedOpacityUnseen = -1;
    private int calculatedOpacitySeen = -1;
    ...
    public GLViewShed2(MapRenderer surface, ViewShedLayer2 subject) {
        ...
        this.opacitySeen = subject.getOpacitySeen();
        this.opacityUnseen = subject.getOpacityUnseen();
    }
    ...
    public void onOpacityChanged(ViewShedLayer2 layer) {
        final int valueSeen = layer.getOpacitySeen();
        final int valueUnseen = layer.getOpacityUnseen();
        if (this.renderContext.isRenderThread()) {
            this.opacitySeen = valueSeen;
            this.opacityUnseen = valueUnseen;
            this.invalidate();
        } else {
            this.renderContext.queueEvent(new Runnable() {
                @Override
                public void run() {
                    GLViewShed2.this.opacitySeen = valueSeen;
                    GLViewShed2.this.opacityUnseen = valueUnseen;
                    GLViewShed2.this.invalidate();
                }
            });
        }
    ...
    protected void updateRGBA(HeatMapState s, HeatMapParams result) {
        ...
        calculatedOpacitySeen = opacitySeen;
        calculatedOpacityUnseen = opacityUnseen;
    ...
    private float getSlope(final int x, final int y) {
        ...
        // the tested point elevation needs to take into account the curvature of the earth
        pointElev -= HorizonUtilities.getCurvatureDrop(run);
    ...
    private void testPoint(final boolean xAxis, final int x, final int y,
        ...
        final int seen = SEEN | (0x000000FF & opacitySeen);
        final int unseen = UNSEEN | (0x000000FF & opacityUnseen);
    ...
    protected void query(ViewState s, HeatMapParams result) {
        ...
        if (state.pointOfInterest == null || Double.isNaN(state.radius)) {
            ...
            double transPercentageSeen = ((ViewShedLayer2) subject)
                    .getOpacitySeen()
                    / 100d;
            double transPercentageUnseen = ((ViewShedLayer2) subject)
                    .getOpacityUnseen()
                    / 100d;

            opacitySeen = (int) Math.round(255d * transPercentageSeen)
                    & (byte) 0xFF;
            opacityUnseen = (int) Math.round(255d * transPercentageUnseen)
                    & (byte) 0xFF;
            ...
            if (state.elevationModel == result.elevationModel) {
                if (!rbgaUpdateNeeded && (opacitySeen != calculatedOpacitySeen
                        || opacityUnseen != calculatedOpacityUnseen)) {
                    for (int i = 3; i < result.rgbaData.length; i += 4) {
                        // 0x00 is missing and should not be used as an alpha value
                        if (result.rgbaData[i] != (byte) 0x00) {
                            if (result.rgbaData[i - 3] == (byte) 0xFF)
                                result.rgbaData[i] = (byte) opacityUnseen;
                            else
                                result.rgbaData[i] = (byte) opacitySeen;
                        }
                    }
                    return;
                }
            ...
            result.elevationModel = this.elevationModel;
