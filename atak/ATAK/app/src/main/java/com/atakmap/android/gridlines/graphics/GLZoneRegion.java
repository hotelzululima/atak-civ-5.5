...
class GLZoneRegion {
        ...
        if (mapScaleReciprocal < (2100000 * 2 * Math.PI)) {
            if (MathUtils.hasBits(renderPass, GLMapView.RENDER_PASS_SURFACE)
                ...
                RunnableFuture<GLGridTile[][]> loadJob = new RunnableFuture<GLGridTile[][]>() {
                    ...
                    @Override
                    public synchronized GLGridTile[][] get(long l,
                            TimeUnit timeUnit) throws ExecutionException,
                            InterruptedException, TimeoutException {
                        if (!this.isDone()) {
                            this.wait(
                                    TimeUnit.MILLISECONDS.convert(l, timeUnit));
                            if (!this.isDone())
                    ...
                    public synchronized void run() {
                        if (isDone())
                            return;
                        try {
                            result = _genTileGrid(_south, _west, _north, _east,
                                    cancelToken,
                            ...
                            this.notifyAll();
                            final SurfaceRendererControl ctrl = surface
                                    .getGLMapView()
                                    .getControl(SurfaceRendererControl.class);
                            if (ctrl != null) {
                                // mark the surface as dirty
                                ctrl.markDirty(
                                        new Envelope(_west, _south, 0d, _east,
                                                _north, 0d),
                                        false);
                            } else {
    ...
    private static GLGridTile[][] _genTileGrid(double south, double west,
            double north, double east, AtomicBoolean cancelToken,
        ...
        while (lat < north) {
            ...
            row = _genTileRow(new MutableMGRSPoint(minRef), west, east, south,
                    north, cancelToken, genTileGridExecutor,
                    segmentLabelsExecutor);
            if (row.length == 0) {
                break;
            }
            grid.add(row);
