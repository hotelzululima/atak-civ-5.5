...
public class NavView extends RelativeLayout implements View.OnClickListener,
    ...
    public static final float DEFAULT_SCALE_FACTOR = 1.33f;
    ...
    private final AtakPreferences _prefs = AtakPreferences
    ...
    private boolean _landscapeExtendedButtons;

    private final static int LANDSCAPE_MAXIMUM_BUTTONS = 10;
    ...
    private float userIconScale = 1.0f;
    ...
    private NavSelectedReceiver navSelectedReceiver;
    ...
    private void init() {
        ...
        _landscapeExtendedButtons = _prefs
                .get(LoadoutManager.PREF_LANDSCAPE_EXTENDED_BUTTONS, false);

        navSelectedReceiver = new NavSelectedReceiver();
        ...
        AtakBroadcast.getInstance().registerReceiver(navSelectedReceiver,
     ...
     * @param r The behavior to invoke when the GPS lock is dismissed.
     *
     */
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        if (_landscapeExtendedButtons)
            rebuildNavView();
    }

    private void rebuildNavView() {
        if (_mapView == null)
            return;
        ...
        _mapView.postOnActive(new Runnable() {
            @Override
            public void run() {
                userIconScale = DEFAULT_SCALE_FACTOR;
    ...
    private int getMaxButtons() {
        ...
        final int btnSize = (int) Math
                .ceil((r.getDimensionPixelSize(R.dimen.nav_button_size)) *
                        userIconScale);

        // determine the minimum width of the physical display
        final int minSize = Math.min(dm.widthPixels, dm.heightPixels);

        // account for the overflow menu and the compass
        final int numButtons = (minSize / btnSize) - 2;

        int maxButtons = 6;
        // apply the user icon scale
        maxButtons = (int) Math.floor(maxButtons / userIconScale);

        if (_landscapeExtendedButtons && !_mapView.isPortrait()) {
            // the maximum number of buttons that can fit into landscape mode not to exceed
            // the value set by LANDSCAPE_MAXIMUM_BUTTONS
            final int maxSize = Math.max(dm.widthPixels, dm.heightPixels);
            maxButtons = Math.min(LANDSCAPE_MAXIMUM_BUTTONS,
                    (maxSize / btnSize) - 2);
        }

        return Math.max(maxButtons, numButtons);
    }

    private void removeView(int i) {
        // first time these could be the NavButtons specified in the view
        ...
        existing = findViewWithTag(
                ATAKConstants.getPackageName() + ":id/" + id);
        if (existing != null)
            removeView(existing);
    ...
    private void initLoadoutButtons() {
        ...
        // clean up when contracting the toolbar (if needed)
        for (int i = numButtons; i < LANDSCAPE_MAXIMUM_BUTTONS; i++) {
            removeView(i);
        }
    ...
    private void updateButtonLayout() {
        ...
        float iconScale = DEFAULT_SCALE_FACTOR;
        if (!_prefs.get("largeActionBar", false))
            iconScale = 1.0f;
    ...
    private void removeChildView() {
        ...
        if (_prefs.get("tilt_rotation_menu_auto_close", false))
            removeCompassChildView();
    }

    private void refreshCompassButtonChildView() {
        if (_prefs.get("tilt_rotation_menu_auto_close", false))
            return;

        if (compassButtonChildView != null &&
                compassButtonChildView.getVisibility() == View.VISIBLE) {
            post(new Runnable() {
                @Override
                public void run() {
                    removeCompassChildView();
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
        ...
        switch (key) {
            ...
            case "dexControls":
                ...
                updateDexControls();
                break;
            case LoadoutManager.PREF_LANDSCAPE_EXTENDED_BUTTONS:
                _landscapeExtendedButtons = _prefs.get(key, false);
                rebuildNavView();
                break;
    ...
    private void toggleLock(boolean bSelf) {
        ...
        if (bSelf) {
            ...
            // above will hear it immediately. AS.
    ...
    private void addChildView(NavButton button) {
        Drawable icon = button.getModel().getImage();
        childView = new NavButtonChildView(this.getContext(), icon);
    ...
    public void dispose() {
        if (_mapView != null) {
            ...
            _buttonManager.removeModelListChangedListener(this);
            _buttonManager.removeModelChangedListener(this);
            _loadouts.removeListener(this);
        }

        AtakBroadcast.getInstance().unregisterReceiver(_buttonReceiver);
        AtakBroadcast.getInstance().unregisterReceiver(orientationReceiver);
        AtakBroadcast.getInstance().unregisterReceiver(lockBroadcastReceiver);
        AtakBroadcast.getInstance().unregisterReceiver(navSelectedReceiver);
