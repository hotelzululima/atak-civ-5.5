...
public class ModelMapComponent extends AbstractMapComponent {
    ...
    private final static Set<String> supportedExtensions = new HashSet<>();

    public ModelMapComponent() {
        super(true);
        supportedExtensions.add(".dae");
        supportedExtensions.add(".obj");
        supportedExtensions.add(".zip");
        supportedExtensions.add(".json");
        supportedExtensions.add(".3tz");
    }
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        for (String ext : supportedExtensions) {
            ImportFilesTask.unregisterExtension(ext);
        }
    ...
    public void onCreate(final Context context, Intent intent, MapView view) {
        ...
        modelImportResolver = MarshalManager.fromMarshal(
                ModelMarshal.INSTANCE, ResUtils.getDrawable(context, R.drawable.ic_model_building));
        ...
        streamingMeshImportResolver = MarshalManager
                .fromMarshal(StreamingMeshServiceImportUtil.marshal);
        ImportExportMapComponent.getInstance().addImporterClass(
                streamingMeshImportResolver);

        for (String ext : supportedExtensions) {
            ImportFilesTask.registerExtension(ext);
        }
            ...
            private void loadFiles(File dir, Bundle b) {
                ...
                for (final File f : listing) {
                    if (IOProviderFactory.isDirectory(f)) {
                        ...
                        if (ModelInfoFactory.isSupported(f.getAbsolutePath()))
