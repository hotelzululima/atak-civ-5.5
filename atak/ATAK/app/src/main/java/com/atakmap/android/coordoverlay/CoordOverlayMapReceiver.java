...
public class CoordOverlayMapReceiver extends BroadcastReceiver implements
    ...
    final MenuLayoutWidget mw = MapMenuReceiver.getMenuWidget();
    ...
    public CoordOverlayMapReceiver(final MapView mapView) {
        ...
        boolean b = _prefs.get("detail.enlarged", false);
            ...
            selfCoordText.addOnTextChangedListener(
                    selfCoordChangedListener = new TextWidget.OnTextChangedListener() {
                        ...
                        public void onTextWidgetTextChanged(TextWidget widget) {
                            if (_activeMarker == self && self != null)
    ...
    protected void initUnits() {
        ...
        rangeSystem = Integer
                .parseInt(_prefs.get("rab_rng_units_pref", String.valueOf(1)));

        bearingUnits = Angle.findFromValue(
                Integer.parseInt(
                        _prefs.get("rab_brg_units_pref", String.valueOf(0))));

        northRef = NorthReference.findFromValue(
                Integer.parseInt(_prefs.get("rab_north_ref_pref",
                        String.valueOf(NorthReference.MAGNETIC.getValue()))));

        areaRef = Area
                .findFromValue(_prefs.get("area_display_pref", Area.METRIC));

        switch (Integer
                .parseInt(_prefs.get("alt_unit_pref", "0"))) {
            case 0:
                altUnits = Span.FOOT;
                break;
            case 1:
    ...
    protected String constructCoordText(final String label,
        ...
        if (point.isAltitudeValid()) {
            if (el.doNotDisplayAgl) {
                ...
                String altRef = _prefs.get("alt_display_pref", "MSL");
                ...
                altString = AltitudeUtilities.format(point,
        ...
        if (!areaStr.isEmpty() || !heightStr.isEmpty()) {
            String haline = "";
            if (!areaStr.isEmpty())
                haline = areaStr;
        ...
        if (speed > 0 || teamMate) {
            ...
            try {
                speedIndex = Integer.parseInt(
                        _prefs.get("speed_unit_pref", String.valueOf(0)));
    ...
    protected void _updatePoint(final GeoPointMetaData point,
        ...
        displayRB = displayRB || label.equals("Red X")
                || label.startsWith("Local SPI");
        if (self != null && _activeMarker != null
                && self.getUID()
                        .equals(_activeMarker.getMetaString("parent_uid", ""))
                && _activeMarker.getType().equals("b-m-p-s-p-i")) {
            displayRB = true;
        }
    ...
    private String getCoords(GeoPoint point) {
        ...
        String coordFmt = _prefs.get("coord_display_pref",
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (action) {
                case COORD_COPY:
                    ...
                    // ATAK-19235 Copying MGRS coordinates include Unicode specifically the U+200E LEFT-TO-RIGHT MARK
                    String clean = coordString.replaceAll("\\u200E", "");
                    ATAKUtilities.copyClipboard("coordinates", clean,
                            true);
                ...
                case COORD_ENLARGE:
                    ...
                    // toggle the variable
                    boolean def = !b;
                    _prefs.set("detail.enlarged", def);
                    b = !b;
