...
public class MissionPackageManifest implements Parcelable {
    ...
    public void setManifestLocation(String manifestLocation) {
        if (manifestLocation == null)
            ...
            baseDirectory = manifestLocation
                    .replaceAll(MissionPackageBuilder.MANIFEST_XML, "");
    }
    ...
    public boolean addContent(MissionPackageContent content) {
        ...
        boolean retval = _contents.setContent(content);
        fireManifestChanged();
        return retval;
    }
    ...
    public boolean removeContent(MissionPackageContent content) {
        if (content == null || !content.isValid())
            return false;
        boolean retval = _contents.removeContent(content);
        fireManifestChanged();
        return retval;
    }
    ...
    public long getTotalSize() {
        if (totalSizeCache < 0) {
            ...
            totalSizeCache = totalSizeInBytes;
        }
    ...
    static MissionPackageManifest fromXml(File file) {
        ...
        try (FileInputStream fis = IOProviderFactory.getInputStream(file)) {
            MissionPackageManifest manifest = serializer.read(
                    MissionPackageManifest.class, fis);
            removeDuplicateContents(manifest);
    ...
    /**
     * Remove any duplicative entries that will causes issues with processing
     * the mission package contents.
     * @param manifest the manifest to process
     */
    private static void removeDuplicateContents(
            final MissionPackageManifest manifest) {
        final List<MissionPackageContent> nContents = new ArrayList<>();
        final List<MissionPackageContent> contents = manifest.getContents()
                .getContents();
        for (MissionPackageContent c : contents) {
            if (!nContents.contains(c)) {
                nContents.add(c);
            }
        }
        manifest.getContents().setContents(nContents);
    }
