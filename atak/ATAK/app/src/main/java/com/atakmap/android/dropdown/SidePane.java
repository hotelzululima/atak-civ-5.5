...
public class SidePane implements OnTouchListener,
    ...
    private final View selfCoordinateBottomBar;
    ...
    private static final String PREF_HANDLE_SHADE = "sidepane_handle_shade";
    private String handleShadePref;
    ...
    public SidePane(MapView view) {
        ...
        bottomBorder = act.findViewById(R.id.bottom_drop_down_border);
        selfCoordinateBottomBar = act.findViewById(R.id.self_coordinate_bar);
        ...
        handleShadePref = getHandleShadePref();
    }
    ...
    public void onSharedPreferenceChanged(SharedPreferences p, String key) {
        ...
        if (key.equals(NavView.PREF_NAV_ORIENTATION_RIGHT))
            resize();
        if (key.equals(PREF_HANDLE_SHADE))
            handleShadePref = getHandleShadePref();
        showHandle();
    }
    ...
    public void open() {
        ...
        final boolean navRight = prefs.get(NavView.PREF_NAV_ORIENTATION_RIGHT,
                true);
        final boolean menuVisible = NavView.getInstance().buttonsVisible();
        ...
        if (!layoutIsPortrait) {
            ...
            updateBottomBorderVisibility(true);
        }
    ...
    public void close() {
        ...
        showHandle();
        updatePaneVisibility();
        updateBottomBorderVisibility(false);
    }
    ...
    protected void showHandle() {
        if (dd == null) {
            ...
            if (bottomHandle instanceof ImageView) {
                ((ImageView) bottomHandle)
                        .setImageDrawable(getHorizontalHandleDrawable());
            }
        } else {
            bottomHandle.setVisibility(View.GONE);
            sideHandle.setVisibility(View.VISIBLE);
            if (sideHandle instanceof ImageView) {
                ((ImageView) sideHandle)
                        .setImageDrawable(getVerticalHandleDrawable());
            }
        }
    }
    ...
    private void updateBottomBorderVisibility(Boolean show) {
        bottomBorder.setVisibility(show
                && selfCoordinateBottomBar.getVisibility() == View.VISIBLE
                        ? View.VISIBLE
                        : View.GONE);
    }
    ...
    private final ExecutorService pool = Executors
            .newSingleThreadExecutor(new NamedThreadFactory("sidePaneThread"));
    ...
    private Drawable getVerticalHandleDrawable() {
        Context context = mapView.getContext();

        if (handleShadePref.equals("Dark")) {
            if (isOpen()) {
                return context.getDrawable(
                        R.drawable.ic_side_menu_trigger_open_legacy);
            } else {
                return context.getDrawable(
                        R.drawable.ic_side_menu_trigger_close_legacy);
            }
        } else {
            if (isOpen()) {
                return context
                        .getDrawable(R.drawable.ic_side_menu_trigger_open);
            } else {
                return context
                        .getDrawable(R.drawable.ic_side_menu_trigger_close);
            }
        }
    }

    private Drawable getHorizontalHandleDrawable() {
        Context context = mapView.getContext();
        ...
        if (handleShadePref.equals("Dark")) {
            if (isOpen()) {
                ...
                return context.getDrawable(
                        R.drawable.ic_side_menu_trigger_bottom_close_legacy);
            }
        } else {
            if (isOpen()) {
                return context.getDrawable(
                        R.drawable.ic_side_menu_trigger_bottom_open);
            } else {
                return context.getDrawable(
    ...
    private String getHandleShadePref() {
        return prefs.get(PREF_HANDLE_SHADE, "Dark");
    }
}
