...
class GLLayerOutlinesLayer extends
    ...
    private final Collection<Long> features;

    private Collection<GLMapRenderable> renderList;

    private GLBatchGeometryRenderer renderer;

    private final Collection<PrimitiveBuffer> lineBuffers = new LinkedList<>();
    ...
    protected boolean updateRenderableReleaseLists(
        ...
        features.clear();
        features.addAll(pendingData.features);
        pendingData.features.clear();
    ...
    public void hitTest(MapRenderer3 renderer, HitTestQueryParameters params,
        ...
        if (params.resultFilter != null
                && !params.resultFilter.acceptClass(Long.class))
            return;
        final MapSceneModel model = renderer.getMapSceneModel(true,
                renderer.getDisplayOrigin());
        final double radius = MathUtils.distance(params.rect.left,
                params.rect.top, params.rect.right, params.rect.bottom) / 2d;
        Feature f;
        for (Long fid : this.features) {
            try {
                f = Utils.getFeature(this.dataStore, fid);
            } catch (DataStoreException ignored) {
                continue;
            }

            if (f != null) {
                if (GLFeatureLayer.hitTest(f.getGeometry(), params.geo, radius
                        * model.gsd)) {
                    results.add(new HitTestResult(f.getId()));
