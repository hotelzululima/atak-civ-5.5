...
public class ImageryCapturePP extends CapturePP {
    ...
    public ImageryCapturePP(MapView mapView, TileCapture tc,
        ...
        ATAKUtilities.getElevation(_center.getLatitude(),
                _center.getLongitude(), gpm);
    ...
    public Span getDisplayUnit() {
        String units = _prefs.get("rab_rng_units_pref",
     ...
     * @apiNote set CapturePrefs.PREF_SHOW_IMAGERY to false to hide imagery with color specified in
     * CapturePrefs.PREF_IMAGERY_HIDING_COLOR
     */
    @Override
    public boolean drawElements(Canvas can) {
        ...
        if (!CapturePrefs.get(CapturePrefs.PREF_SHOW_IMAGERY, true))
            _can.drawColor(CapturePrefs
                    .get(CapturePrefs.PREF_IMAGERY_HIDING_COLOR, Color.WHITE));
    ...
    public void writeKML(GeoPoint[] c, String fName, File docKml) {
        ...
        try (OutputStream os = IOProviderFactory.getOutputStream(docKml);
                OutputStreamWriter osw = new OutputStreamWriter(
                        os, FileSystemUtils.UTF8_CHARSET);
