...
public class CoTInfoView extends RelativeLayout
    ...
    private final ModifierInfoView modifierView = new ModifierInfoView(
            MapView.getMapView());
    ...
    private TextView _markingField;
    ...
    private Switch _sensorFOVSwitch;
    private ImageButton _sensorFOVModify;
        ...
        switch (key) {
                    ...
                    public void run() {
                        try {
                            if (_prefs.get("hostileUpdateDelay", "60")
                                ...
                                _autoBroadcastButton.setEnabled(true);
                            }
            ...
            case "broadcast_disable":
                post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            if (_prefs.get("broadcast_disable", false)
                                    && !Arrays
                                            .asList(TAKServerListener
                                                    .getInstance()
                                                    .getConnectedServers())
                                            .isEmpty()) {
                                _autoBroadcastButton.setVisibility(VISIBLE);
                                _autoBroadcastButton.setEnabled(false);
                            } else {
                                _autoBroadcastButton.setVisibility(VISIBLE);
                                _autoBroadcastButton.setEnabled(true);
                            }
    ...
    protected void setup() {
        ...
        _markingField = this.findViewById(R.id.markingField);
        ...
        _addressInfoText = this
                .findViewById(R.id.dgInfoAddressInfo);
        ...
        _sensorFOVSwitch = findViewById(R.id.sensorFoVSwitch);
        _sensorFOVModify = findViewById(R.id.sensorFoVModify);
        _sensorFOVSwitch.setOnCheckedChangeListener(
                new CompoundButton.OnCheckedChangeListener() {
                    @Override
                    public void onCheckedChanged(CompoundButton buttonView,
                            boolean isChecked) {
                        final boolean checked = _sensorFOVSwitch.isChecked();
                        _sensorFOVModify.setEnabled(checked);
                        if (checked) {
                            addSensorFieldOfView(_marker);
                        } else {
                            removeSensorFieldOfView(_marker);
                        }
                    }
                });
        _sensorFOVModify.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                modifySensorFieldOfView(_marker);
            }
        });
                ...
                group.setOnCheckedChangeListener(
                        new RadioGroup.OnCheckedChangeListener() {
                            ...
                            public void onCheckedChanged(RadioGroup arg0,
                                ...
                                final String oldtype = _marker.getType();
                                ...
                                b.putString("old_type", oldtype);
                                b.putString("new_type", _marker.getType());
                                MetricsUtils.record(
                                        MetricsUtils.CATEGORY_MAPITEM,
                                        MetricsUtils.EVENT_MAPITEM_CHANGED,
                                        "CoTInfoView", _marker, b);

                                // if there is no `milsym` refresh the marker
                                if(_marker.getMetaString("milsym", null) == null)
                                    _marker.refresh(
                                ...
                                Intent intentRadial = new Intent(
                                        "com.atakmap.android.maps.REFRESH_MENU");
                                intentRadial.putExtra("uid", _marker.getUID());
                                AtakBroadcast.getInstance()
                                        .sendBroadcast(intentRadial);
        ...
        _panButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (_marker != null) {
                    ...
                    double elev = ATAKUtilities
                            .getElevation(gp.getLatitude(), gp.getLongitude(),
                                    null);
        ...
        _coordButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                CoordinateEntryCapability.getInstance(getContext()).showDialog(
                        getContext().getString(R.string.rb_coord_title),
                        ...
                        new CoordinateEntryCapability.ResultCallback() {
                            ...
                            public void onResultCallback(String pane,
                                ...
                                onPointChanged(m);
                                m.persist(mapView.getMapEventDispatcher(),
                                        null, this.getClass());

                                com.atakmap.android.drawing.details.GenericPointDetailsView
                                        .setAddress(point, m, _nameEdit);
                                ...
                                MetricsUtils.record(
                                        MetricsUtils.CATEGORY_MAPITEM,
                                        MetricsUtils.EVENT_MAPITEM_MOVED,
                                        "CoTInfoView",
                                        m, MetricsUtils.EVENT_STATUS_SUCCESS);
                            }
                        });
            }
        });
        ...
        _cotButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (_marker == null || !_cotType.startsWith("a-")) {
                    ...
                    selector.show(_marker, (CoTSelector.TypeChangedListener2) null);
                }
            }
        });
    ...
    protected void _onColorSelected() {
        ...
        palette.setColor(_marker.getMetaInteger("color", Color.BLACK));
        b.setView(palette);
        ...
        OnColorSelectedListener l = new OnColorSelectedListener() {
            ...
            public void onColorSelected(int color, String label) {
                ...
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_CHANGED, "CoTInfoView",
                        _marker, b);
    ...
    protected void _updateName() {
        ...
        if (!name.equals(_prevName) && _marker != null
            ...
            if (MetricsApi.shouldRecordMetric()) {
                final Bundle b = new Bundle();
                b.putString("old_name", _prevName);
    ...
    public void setType(String type) {
        ...
        String targ = getTypeLabel(getContext(), _marker);
    ...
    /**
     * Method to provide for the type label displayed to the user
     * @param context the context to use
     * @param type the type to be used
     * @return the string that corresponds with the type
     * @deprecated
     */
    @Deprecated
    ...
    public static String getTypeLabel(Context context, String type) {
        ...
        String _25b25bName = CotUtils.mil2525cFromCotType(type);
     ...
     * @return the string that corresponds with the type
     */
    public static String getTypeLabel(Context context, @NonNull PointMapItem marker) {
        String type = marker.getMetaString("milsym", null);
        if (type == null)
            type = marker.getType();
        ...
        // get the CoT name from some library
        if (type.startsWith("a-"))
            type = CotUtils.mil2525cFromCotType(type);


        String targ = context.getString(R.string.not_recognized);
        boolean includeType = true;
        if (type.equals(QUICK_NAV_COT) || type.equals(WAYPOINT_COT))
            targ = context.getString(R.string.waypoint);
        else if (type.equals(INITIAL_POINT_COT))
            targ = context.getString(R.string.initial_point);
        else if (type.equals(CONTACT_POINT_COT))
            ...
            targ = CotDescriptions.getHumanName(context, type);
            includeType = targ.equals(context.getString(
                    R.string.not_recognized));
        }
    ...
    protected void refreshMarker() {
        ...
        urlUiHandler.update(_marker);

        cotAuthorLayout.setVisibility(View.GONE);
        cotAuthorLayout.setOnClickListener(null);

        if (_marker instanceof Marker) {
            Marker m = (Marker) _marker;

            refreshAuthorLayout(mapView, m,
                    cotAuthorLayout);
            ...
            if (_prefs.get("hostileUpdateDelay", "60").equals("0")) {
                _autoBroadcastButton.setVisibility(INVISIBLE);
            } else if (_prefs.get("broadcast_disable", false)
                    && !Arrays.asList(
                            TAKServerListener.getInstance()
                                    .getConnectedServers())
                            .isEmpty()) {
                _autoBroadcastButton.setEnabled(false);
            }
        ...
        GenericDetailsView.setClassification(_marker, _markingField);
        ...
        final boolean hasSensor = hasSensorFieldOfView(_marker);
        _sensorFOVSwitch.setChecked(hasSensor);
        _sensorFOVModify.setEnabled(hasSensor);
    }
    ...
    @Deprecated
    @DeprecatedApi(since = "5.4", removeAt = "5.7", forRemoval = true)
    public static void refreshAuthorLayout(final MapView mapView,
            final MapItem m, final TextView _authorText,
        ...
        refreshAuthorLayout(mapView, m, cotAuthorLayout);
    }
     ...
     */
    public static void refreshAuthorLayout(final MapView mapView,
            final MapItem m, final View cotAuthorLayout) {

        // if the marker is null for whatever reason, just exit out
        if (m == null || cotAuthorLayout == null)
            return;

        final TextView _authorText = cotAuthorLayout
                .findViewById(R.id.dgInfoAuthor);
        final TextView _productionTimeText = cotAuthorLayout
                .findViewById(R.id.dgInfoProductionTime);
        final ImageView cotAuthorIconButton = cotAuthorLayout
                .findViewById(R.id.dgAuthorIconButton);
        final ImageView cotAuthorPanButton = cotAuthorLayout
                .findViewById(R.id.dgAuthorPanButton);

        cotAuthorPanButton.setVisibility(View.INVISIBLE);
        cotAuthorIconButton.setVisibility(View.INVISIBLE);
        cotAuthorLayout.setVisibility(View.GONE);
        ...
        if (author_uid != null) {
            ...
            if (author_item != null) {
                ...
                if (author_item instanceof Marker) {
                    ...
                    cotAuthorIconButton.setVisibility(View.VISIBLE);
        ...
        if (author_name != null) {
            ...
            _authorText.setVisibility(VISIBLE);
            _authorText.setText(R.string.unknown);
            cotAuthorLayout.setOnClickListener(null);
        }
    ...
    private boolean hasSensorFieldOfView(MapItem item) {
        return item.getMetaBoolean(SensorDetailHandler.SENSOR_FOV, false);
    }

    private synchronized void addSensorFieldOfView(MapItem item) {
        String spiuid = item.getUID() + SensorDetailHandler.UID_POSTFIX;
        MapItem existingspi = mapView.getMapItem(spiuid);
        if (existingspi == null) {
            item.setMetaBoolean(SensorDetailHandler.SENSOR_FOV, true);
            if (item instanceof PointMapItem) {
                SensorFOV fov = SensorDetailHandler.addFovToMap(
                        (PointMapItem) item, 0, 10, 500,
                        new float[] {
                                1f, 1f, .85f, .5f
                        },
                        true, false, 100d);
    ...
    public void modifySensorFieldOfView(MapItem item) {
        Intent i = new Intent("com.atakmap.android.cotdetails.SENSORDETAILS");
        i.putExtra("targetUID", item.getUID());
        i.putExtra("disable_send", true);
        AtakBroadcast.getInstance().sendBroadcast(i);
    }

    public synchronized void removeSensorFieldOfView(MapItem item) {
        String spiuid = item.getUID() + SensorDetailHandler.UID_POSTFIX;
        MapItem existingspi = mapView.getMapItem(spiuid);
        if (existingspi != null) {
            existingspi.removeFromGroup();
        }
        item.removeMetaData(SensorDetailHandler.SENSOR_FOV);

    }

    public ExtendedInfoView getModifierView() {
        return modifierView.getModifierView(_marker);
    }
}
