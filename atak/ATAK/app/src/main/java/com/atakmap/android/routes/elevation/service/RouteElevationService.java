...
public class RouteElevationService {
    ...
    public static GeoPointMetaData closestSegmentPoint(GeoPoint source,
        ...
        final double v = (x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4);
        final double px = ((x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2)
                * (x3 * y4 - y3 * x4))
                / v;
        final double py = ((x1 * y2 - y1 * x2) * (y3 - y4) - (y1 - y2)
                * (x3 * y4 - y3 * x4))
                / v;
    ...
    public static SegmentData expandSegment(final GeoPointMetaData source,
        ...
        double totalDistance = GeoCalculations.distanceTo(newSource.get(),
                newTarget.get());
        double totalAltChange = EGM96.getMSL(targAlt.get())
                - EGM96.getMSL(sourceAlt.get());
        ...
        if (totalDistance > 0 && totalDistance > incrementInMeters) {
            ...
            ElevationSource.QueryParameters queryParams = new ElevationSource.QueryParameters();

            double[] els = new double[geoPointVec.size()];
            Arrays.fill(els, Double.NaN);
            ElevationManager.getElevation(
                    new TransmuteIterator<GeoPointMetaData, GeoPoint>(
                            geoPointVec.iterator()) {
                        @Override
                        protected GeoPoint transmute(GeoPointMetaData arg) {
                            return arg.get();
                        }
                    }, els, null);
            ...
            for (int j = 0; j < els.length; j++) {
                ...
                newPoint = geoPointVec.get(j).get();
                double alt = els[j];
                //see if we can interpolate if the altitude is not defined
                if (bInterpolateAltitudes && Double.isNaN(alt)
                        && !Double.isNaN(totalAltChange)) {
                    double currentDistance = distVec.get(j);
                    double currentDistanceFraction = currentDistance
                            / distVec.get(distVec.size() - 1);
                ...
                if (GeoPoint.isAltitudeValid(alt)) {
                    newPoint = new GeoPoint(newPoint.getLatitude(),
                            ...
                            alt);
                    geoPointVec.get(j).set(newPoint);
                }
            }
