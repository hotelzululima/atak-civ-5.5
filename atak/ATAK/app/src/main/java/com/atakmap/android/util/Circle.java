...
public class Circle extends Polyline {
    ...
    private AttributeSet _labels = new AttributeSet();
    private AttributeSet _segment = new AttributeSet();
    ...
    public void setLabel(final String text) {
        ...
        if (_labels == null)
            _labels = new AttributeSet();
        if (_segment == null)
            _segment = new AttributeSet();

        _segment.setAttribute("segment", res / 2);
        if (_text == null)
            _segment.removeAttribute("text");
        else
            _segment.setAttribute("text", _text);

        _labels.setAttribute("segment", _segment);

        this.setMetaAttributeSet("labels", _labels);
    ...
    private void calculateCircle() {
        if (_radius >= MAX_RADIUS || _center == null || _radius < 0) {
            ...
            Log.w(TAG, "Failed to calculate Circle");
            return;
        }
    ...
    public void drawCanvas(CapturePP cap, Bundle data) {
        ...
        if (FileSystemUtils.isEmpty(label)) {
            ...
            Set<String> names = labels.getAttributeNames();
            for (String name : names) {
                labelBundle = getMetaAttributeSet(name);
            }
            if (labelBundle != null)
                label = (String) labelBundle.getStringAttribute("text");
        }
