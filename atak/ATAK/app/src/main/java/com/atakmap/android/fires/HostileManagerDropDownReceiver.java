...
public class HostileManagerDropDownReceiver extends DropDownReceiver implements
    ...
    private final PointF screenSpace = new PointF();
    ...
    private volatile boolean reopenOnTop = false;
    ...
    private static class MapItemHolder {
        ...
        public boolean equals(final Object comp) {
            if (comp instanceof MapItemHolder) {
                ...
                return holder.uid.equals(this.uid);
            }
            return false;
        }
    ...
    synchronized public void init() {
        if (content == null) {
                    ...
                    new CompoundButton.OnCheckedChangeListener() {
                        ...
                        public void onCheckedChanged(CompoundButton buttonView,
                            ...
                            synchronized (lock) {
                                ...
                                prefs.set("HostileListOnlyInViewport",
                                        flipChecked);
                                calc.exec();
                            }
                        }
            ...
            final boolean b = prefs.get("HostileListOnlyInViewport", false);
    ...
    public void onDropDownClose() {
        if (reopenOnTop) {
            ...
            getMapView().post(new Runnable() {
                @Override
                public void run() {
                    onReceive(getMapView().getContext(), null);
                }
            });
        }
    }
    ...
    private void updateDisplayList() {
        synchronized (lock) {
            ...
            final MapSceneModel model = mv.getSceneModel();

            for (Marker marker : hostilesListBase) {
                if (marker != null) {
                    if (filterCb != null && !filterCb.isChecked()) {
                        model.forward(marker.getPoint(), screenSpace);
                        final double x = screenSpace.x;
                        final double y = screenSpace.y;
                        if (x > 0 && x < mv.getWidth() && y > 0
                                && y < mv.getHeight())
    ...
    public void onMapEvent(MapEvent event) {
        ...
        Marker hostileItem = (Marker) item;

        switch (eventType) {
            case MapEvent.ITEM_ADDED:

                synchronized (lock) {
                    if (!hostilesListBase.contains(hostileItem)) {
                        addHostile(hostileItem);
                        updateDisplayList();
                    }
                }
                break;
            case MapEvent.ITEM_GROUP_CHANGED:
                synchronized (lock) {
                    if (!hostilesListBase.contains(hostileItem)) {
                        addHostile(hostileItem);
                        updateDisplayList();
                    } else {
                        if (hostilesListBase.remove(hostileItem)) {
                            updateDisplayList();
                            adapter.notifyDataSetChanged();
                        }
                    }
