...
public class TLSUtils {
     ...
     */
    public static List<TAKServer> getServersWithCredentials(
            final boolean bRequireUseAuth) {
        final List<TAKServer> serversWithCred = new ArrayList<>();
    ...
    public static AtakCertificateDatabase.CertificateValidity validateCert(
        ...
        return validateCertImpl(hostname, port,
                AtakCertificateDatabaseIFace.TYPE_CLIENT_CERTIFICATE,
                AtakAuthenticationCredentials.TYPE_clientPassword);
    }

    public static AtakCertificateDatabase.CertificateValidity validateTrust(
            final String hostname, final int port) {
        return validateCertImpl(hostname, port,
                AtakCertificateDatabaseIFace.TYPE_TRUST_STORE_CA,
                AtakAuthenticationCredentials.TYPE_caPassword);
    ...
    private static AtakCertificateDatabase.CertificateValidity validateCertImpl(
            final String hostname, final int port, String certType,
            String passType) {
        if (FileSystemUtils.isEmpty(hostname)) {
            Log.w(TAG, "validateCert invalid hostname");
            return null;
        ...
        byte[] certStore = AtakCertificateDatabase
        ...
        if (certStore == null) {
            certStore = AtakCertificateDatabase
                    .getCertificate(
                            certType);
            clientCertCredentials = AtakAuthenticationDatabase
                    .getCredentials(passType);
        }
        ...
        if (certStore != null
            ...
            return AtakCertificateDatabase.checkValidity(certStore,
                    clientCertCredentials.password);
        } else {
            Log.w(TAG, "Cert not found for: " + hostname);
            return null;
