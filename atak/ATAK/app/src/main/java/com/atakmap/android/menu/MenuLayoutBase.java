...
public class MenuLayoutBase extends LayoutWidget implements
    ...
    interface OnButtonClickDispatcher {
        void dispatch(IMapMenuButtonWidget.OnButtonClickHandler mapAction, Object mapItem);
    }
    ...
    private final Set<MenuHandler> _handlers;
    private final AtakPreferences _prefs;
    protected OnButtonClickDispatcher _mapActionDispatcher;

    MenuLayoutBase(final AtakMapView atakMapView, final MetaDataHolder2 mapData,
            final MapAssets assets, final MenuMapAdapter adapter,
            final OnButtonClickDispatcher mapActionDispatcher) {
        ...
        _handlers = new ConcurrentSkipListSet<>();

        _prefs = AtakPreferences.getInstance(_atakMapView
                .getContext());
    }
    ...
    private PointF pointFromItem(MapItem item) {
        ...
        // bizarre world.   shame shame shame.
        ...
        for (int i = 0; i < submenuWidget.getChildWidgetCount(); ++i) {
            ...
            childButton.addOnClickListener(new IMapWidget.OnClickListener() {
                ...
                public void onMapWidgetClick(IMapWidget widget,
                    ...
                    IMapMenuButtonWidget.OnButtonClickHandler clickAction =
                            childButton.getOnButtonClickHandler();
                    ...
                    if (clickAction != null || prefsValid) {
                        ...
                        if (clickAction != null) {
                            AttributeSet itemmap = _mapItem
                                    .getMetaAttributeSet("submenu_map");
                            if (itemmap == null)
                                itemmap = new AttributeSet();
                            _mapItem.setMetaAttributeSet("submenu_map",
                                    itemmap);
                            _mapActionDispatcher.dispatch(clickAction,
                                    _mapItem);
                        } else {
                            _clearAllMenus();
                        }
        ...
        for (IMapWidget childWidget : root.getChildren()) {
            if (childWidget instanceof MapMenuWidget) {
                ...
                if (widget == submenuWidget)
                    return buttonWidget;
                return getSubmenuButton(widget, (MapMenuWidget)submenuWidget);
            }
        }
        return null;
    }
    ...
    MenuHandler findHandler(MapMenuHandler handler) {
        for(MenuHandler wrapper : _handlers)
            if(wrapper._impl == handler)
                return wrapper;
        return null;
    }

    boolean registerMapMenuHandler(MapMenuHandler handler, int priority) {
        return (findHandler(handler) == null) ?
                _handlers.add(new MenuHandler(handler, priority)) :
                false;
    }

    boolean unregisterMapMenuHandler(MapMenuHandler handler) {
        final MenuHandler registered = findHandler(handler);
        return (registered != null) ? _handlers.remove(registered) : false;
    }
    ...
    private MapMenuWidget createMapMenuWidget(MapItem mapItem) {
        ...
        for (MapMenuFactory factory : _factories) {
            if (factory != null) {
                ...
                if (null != menuWidget) {
                    for(MenuHandler handler : _handlers)
                        handler._impl.updateMenu(mapItem, menuWidget);
    ...
    final static class MenuHandler implements Comparable<MenuHandler> {
        final static AtomicInteger _nextInsert = new AtomicInteger(0);
        final MapMenuHandler _impl;
        final int _priority;
        final int _insert;

        MenuHandler(MapMenuHandler impl, int priority) {
            _impl = impl;
            _priority = priority;
            _insert = _nextInsert.incrementAndGet();
        }

        @Override
        public int compareTo(MenuHandler other) {
            if(_priority < other._priority)
                return -1;
            else if(_priority > other._priority)
                return 1;
            else
                return _insert-other._insert;
        }
    }
}
