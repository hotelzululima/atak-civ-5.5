...
public class GLZonesOverlay implements GLMapRenderable, GLMapRenderable2 {
    ...
    public void draw(GLMapView map, int renderPass) {
        ...
        if (MathUtils.hasBits(renderPass, GLMapView.RENDER_PASS_SURFACE)) {
            if (map.currentPass.crossesIDL) {
                _drawLatLines(map,
                        map.eastBoundUnwrapped,
                        map.westBoundUnwrapped,
                        map.currentPass.southBound,
                        map.currentPass.northBound,
                        alpha, renderPass);
                // west of IDL
                _drawLngLines(map,
                        map.currentPass.southBound,
                        map.currentPass.northBound,
                        map.currentPass.westBound,
                        180d,
                        alpha, renderPass);
                // east of IDL
                _drawLngLines(map,
    ...
    private void _markZones(GLMapView map) {
        ...
        if (map.currentPass.crossesIDL) {
            // west of IDL
            _markZonesImpl(map.currentPass.northBound,
                    map.currentPass.westBound,
                    map.currentPass.southBound,
                    180d);
            // east of IDL
    ...
    private void _drawZones(GLMapSurface surface, GLMapView ortho,
        ...
        for (GLZoneRegion z = zoneDrawList; z != null; z = z.next) {
            if (Rectangle.intersects(ortho.currentPass.westBound,
                    ortho.currentPass.southBound,
                    ortho.currentPass.eastBound, ortho.currentPass.northBound,
                    z._west, z._south,
                    z._east, z._north,
                    false)) {
