...
public class EditablePolyline extends Polyline implements AnchoredMapItem,
    ...
    public EditablePolyline(MapView mapView, long serialId,
            MetaDataHolder2 metadata, String uid) {
    ...
    protected void setShapeMarker(Marker marker) {
        ...
        _shapeMarker
                .setVisible(centerPointVisible && getVisible() && isClosed());
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (item != null) {
                    ...
                    AttributeSetUtils.putAll(as, p.getMetaData(), true);
                    item.putAll(as);
                }
            }
     ...
     *
     * @return the list of all of the point map items
     */
     ...
     * Gets a defensive copy of all of the PointMapItems contained in the line.
     *
     * @return the array of all of the point map items
     */
    ...
    // TODO: deprecate / remove these (rely on polyline style / basiclinestyle instead)
    ...
    public GeoPointMetaData getCenter() {
        ...
        if (_shapeMarker != null) {
            ...
            if (prev.getAltitudeSource().equals(GeoPointMetaData.USER)) {
                gpm = GeoPointMetaData.wrap(
                        new GeoPoint(gp.getLatitude(),
                                gp.getLongitude(),
                                prev.get().getAltitude(), gp.getCE(),
                                gp.getLE()),
                        GeoPointMetaData.CALCULATED,
                        prev.getAltitudeSource());
            }
        }
        if (gpm == null)
            gpm = ATAKUtilities.getElevationMetadata(gp.getLatitude(),
                    gp.getLongitude());
    ...
    public void setCenterPointVisible(boolean visible) {
        ...
        if (_shapeMarker != null)
            _shapeMarker.setVisible(visible && getVisible() && isClosed());
    }
    ...
    protected void onVisibleChanged() {
        ...
        synchronized (this) {
            ...
            if (_shapeMarker != null)
                _shapeMarker.setVisible(visible && centerPointVisible);
        }
    ...
    @Override
    public void setRenderHints(int renderHints) {
        super.setRenderHints(renderHints);
        int renderColor = _linkColor;
        if (MathUtils.hasBits(renderHints, Polyline.RENDER_FLAG_FADE_ITEM)) {
            renderColor = 0x32FFFFFF & _linkColor;
        }
    ...
    public synchronized boolean addMarkers(int pointIndex,
        ...
        if (indexToMarker.isEmpty() && pointIndex == 0) {
    ...
    protected boolean addPointNoSync(int index, GeoPointMetaData point) {
        ...
        try {
            ...
            Log.w(TAG,
                    "attempted to add a point to a route outside of the allowable index",
                    ooe);
            return false;
        }
        ...
        synchronized (this) {
            ...
            if (pmi != null) {
                ...
                if (pmic != null) {
                    ...
                    AttributeSet as = new AttributeSet();
                    AttributeSetUtils.putAll(as, point.getMetaData(), true);
                    pmic.putAll(as);
                }
    ...
    protected CotEvent toCot() {
        ...
        AccessUtils.setAccessDefault(event, this);
     ...
     * @return a GeoPointMetadata equivalent to the provided one, but with the latitude clamped
     */
    ...
    /**
     * Cache the average point for an editable polyline until the points are changed.
     */
    private GeoPoint cachedAvgPoint = GeoPoint.UNKNOWN_POINT;
