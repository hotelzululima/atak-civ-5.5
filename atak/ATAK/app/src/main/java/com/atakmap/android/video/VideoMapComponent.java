...
import com.atakmap.android.video.export.VideoExportMarshal;
...
import gov.tak.api.video.cot.VideoDetailHandler;
...
public class VideoMapComponent extends DropDownMapComponent {
    ...
    private ImportVideoAliasResolver aliasResolver;
    private VideoAliasImporter cotImporter;
    private VideoDetailHandler cotHandler;

    private final MapEventDispatcher.MapEventDispatchListener videoMarkerMonitor = new MapEventDispatcher.MapEventDispatchListener() {
        @Override
    ...
    public void onCreate(Context context, Intent intent, MapView view) {
        ...
        ImportExportMapComponent.getInstance().addImporterClass(
                this.aliasResolver = new ImportVideoAliasResolver(context));
            ...
            public void run() {
                ToolsPreferenceFragment.register(
                        new ToolsPreferenceFragment.ToolPreference(
                                context.getString(R.string.videoPreferences),
                                "",
                                "videoPreference",
                                context.getResources().getDrawable(
                                        R.drawable.nav_video),
                                new VideoPreferenceFragment()));
            }
        });
        ...
        CotDetailManager.getInstance().registerHandler(
                cotHandler = new VideoDetailHandler(manager));
        ...
        executor.submit(new Runnable() {
            @Override
            public void run() {
                synchronized (VideoMapComponent.this) {
                    if (!initDone) {
                        VideoDropDownReceiver.cleanupTmpDir(context);
                        try {
                            /* Initialize Gv2F **/
                            MediaProcessor.PGSCMediaInit(
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        ExporterManager.unregisterExporter(context.getString(R.string.video));
    ...
    public static void addVideoAlias(final String address,
        ...
        try {
            final ConnectionEntry ce = new ConnectionEntry(alias, address,
                    port, -1,
