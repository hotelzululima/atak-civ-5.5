...
public class LinkLineHandler
    ...
    public boolean processLink(String parentUid, MapItem parentItem,
        ...
        if (parentItem == null || childItem == null) {
            ...
            final _Link link = _getLink(parentUid, childUid);

            // enforce the a child can only have one parent rule
            final String currentParentUid = findObjectInMap(childUid);
            if (currentParentUid != null) {
                final _Link prevlink = _getLink(currentParentUid, childUid);
                _removeLink(prevlink);
    ...
    private String findObjectInMap(String newChildUid) {
        synchronized (_links) {
            for (Map.Entry<String, Map<String, _Link>> pair : _links
                    .entrySet()) {
    ...
    private boolean _addLink(final MapItem parentItem,
        ...
        if (parentItem instanceof PointMapItem
            ...
            link.assoc.addOnGroupChangedListener(link.mapEventListener);
    ...
    private _Link _getLink(String parentUid, String childUid) {
        _Link link = null;
        synchronized (_links) {
            Map<String, _Link> map = _links.get(parentUid);
            if (map != null) {
                link = map.get(childUid);
            }
        }
        return link;
    }

    private boolean _removeLink(_Link link) {
        if (link != null) {
            ...
            if (item1 != null && item2 != null) {
                ...
                synchronized (_links) {
                    Map<String, _Link> map = _links.get(item1.getUID());
                    if (map != null) {
                        map.remove(item2.getUID());
                        if (map.isEmpty())
                            _links.remove(item1.getUID());
                    }
                }
            }
            _linkGroup.removeItem(link.assoc);
