 ...
 * metadata.
 */
class VideoMapItemController implements Disposable {

    public static final String TAG = "VideoMapItemController";

    private static final long FUTURE_DATE_MILLIS = 3347274348000L;
    ...
    // in case fields 3 and 10 are not set or the 0104 equvelents - just use this.
    private final String genericUid = UUID.randomUUID().toString();

    private final CotModificationManager.ModificationAction modificationAction = new CotModificationManager.ModificationAction() {
        @Override
        public boolean checkAndPrompt(CotEvent event, Bundle bundle) {
            // do not accept changes from external CoT  // just return true
            return sensorMarker != null
                    && event.getUID().equals(sensorMarker.getUID());
        }
    };
    ...
    private void updateSensorPos(final VideoMetadata vmd) {
        ...
        if (mapView != null) {
            ...
            //For EG 0104.5 implementations, Concatenate Device Designation and Episode Number with the two values
            //separated by an underscore (“_”) character; for 0601.1implementations, concatenate Tags 10 and 3
            //separated by an underscore (“_”) character.
            final String uid;

            if (vmd.platformDesignator != null && vmd.missionId != null)
                uid = vmd.platformDesignator + "_" + vmd.missionId;
            else
                uid = genericUid;

            if (sensorMarker != null && (!sensorMarker.getUID().equals(uid))) {
                CotModificationManager.unregister(sensorMarker.getUID());
                mapView.getRootGroup().removeItem(sensorMarker);
                sensorMarker = null;
            }

            if (sensorMarker == null && showSensorMarker) {
                // block external CoT with the same UID //
                CotModificationManager.register(uid, modificationAction);
                MapItem item = mapView.getMapItem(uid);
                if (item instanceof Marker) {
                    sensorMarker = (Marker) item;
                    // in the case that a CoT marker already exists and has been
                    // utilized for the new Map Item, then set it so it does not
                    // stale out.   Once the video stops playing, the marker is
                    // removed from the map and the next CoT message will create
                    // a new marker.   The reason that the CotEvent Marker is used
                    // is so that the type and other metadata can be preserved.
                    // we are assuming that the CoTEvent has a much richer set of
                    // metadata than the marker generated from the KLV.
                    sensorMarker.setMetaLong("lastUpdateTime",
                            FUTURE_DATE_MILLIS);
                } else {
                    sensorMarker = new Marker(gp, uid);
                }
    ...
    synchronized private void createAssociation(final Marker sensor,
        ...
        frame.setMetaString("parent_type", sensor.getType());
