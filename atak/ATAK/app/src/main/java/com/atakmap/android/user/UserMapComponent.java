...
public class UserMapComponent extends AbstractMapComponent {
    ...
    private SaMarkerBroadcastReceiver selfDetailReceiver;

    @Override
    public void onCreate(Context context, Intent intent, MapView view) {
        _mapView = view;

        _mapView.getMapData().setMetaBoolean("disableTasking", true);
        String[] groupNames = {
                "Hostile",
                "Friendly",
                "Neutral",
        ...
        try {
            ...
            //programmatically created ones e.g. "Icons". See MapOverlayManager.addMarkersOverlay
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        if (selfDetailReceiver != null)
            selfDetailReceiver.dispose();
    ...
    private void _registerReceivers(Context context) {
        ...
        selfDetailReceiver = new SaMarkerBroadcastReceiver(_mapView);
        DocumentedIntentFilter saMarkerFilter = new DocumentedIntentFilter();
        saMarkerFilter.addAction(SaMarkerBroadcastReceiver.SA_ADD_DETAIL);
        AtakBroadcast.getInstance().registerSystemReceiver(selfDetailReceiver, saMarkerFilter);
                ...
                new MapEventDispatcher.MapEventDispatchListener() {
                    ...
                    public void onMapEvent(MapEvent event) {
                        ...
                        if (mi instanceof DrawingShape) {
                            ...
                            return;
                        } else if (mi instanceof EditablePolyline
                                && mi.getMetaBoolean("static_shape", false)) {
                            mi = ((EditablePolyline) mi).getShapeMarker();
                            if (mi != null) {
                                Intent i = new Intent(
                                        ToolManagerBroadcastReceiver.BEGIN_SUB_TOOL);
                                i.putExtra("tool",
    ...
    private void _registerIfTaskable(final Marker marker) {
        if (marker.hasMetaValue("tasks")) {
            ...
            _mapView.getMapData().getMetaBoolean("disableTasking", false);
                        ...
                        public void onItemRemoved(MapItem item,
                            ...
                            if (item.getUID()
                                ...
                                if (_taskMarkerList.isEmpty())
                                    _mapView.getMapData().getMetaBoolean(
                                            "disableTasking",
                                            true);
                            }
                        }
