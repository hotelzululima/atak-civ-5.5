...
public final class DoghouseViewModel {
    ...
    public boolean addRow(@Nullable Doghouse.DoghouseFields key) {
        ...
        if (allSuccessful) {
            ...
            _prefs.set(prefKey, key.toString());
    ...
    final Route route) {
        ...
        _prefs.set(key, Doghouse.DoghouseFields.BLANK.toString());
    }
    ...
    final Route route) {
        ...
        //Per customer doghouse turn point ID should start at 2 for the first doghouse
        ...
        if (doghouses != null) {
            ...
            for (int i = 0; i < route.getNumPoints() - 1; i++) {
                ...
                if (pmi instanceof Route.ControlPointMapItem) {
                    AttributeSet uids = pmi
                            .getMetaAttributeSet(Doghouse.EXTRA_UID);
                    if (uids != null) {
                        final String val = uids
                                .getStringAttribute(pmi.getUID());
                        if (val != null)
                            uid = val;
                    }
                } else {
    ...
    private void handlePointAdded(Route route) {
        for (int i = 0; i < route.getNumPoints() - 1; i++) {
            ...
            PointMapItem pmi = route.getPointMapItem(i);
            if (pmi instanceof Route.ControlPointMapItem) {
                AttributeSet uids = pmi.getMetaAttributeSet(Doghouse.EXTRA_UID);
                if (uids == null
                        || uids.getStringAttribute(pmi.getUID()) == null) {
    ...
    private void updateRelativeLocation(int index, Doghouse doghouse,
        ...
        final Doghouse.DoghouseLocation pref = Doghouse.DoghouseLocation
                .fromConstant(
                        _prefs.get(DoghouseReceiver.RELATIVE_LOCATION, 1));
