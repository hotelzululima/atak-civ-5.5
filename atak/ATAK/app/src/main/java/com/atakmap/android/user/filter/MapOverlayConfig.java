...
public class MapOverlayConfig {
    ...
    /**
     * Combines all configured filters
     */
    ...
    private String translate(String title) {
        if (title.startsWith("filter_team_")) {
            try {
                Class<?> c = com.atakmap.app.R.string.class;
                Field f = c.getField(title.replace("filter_team_", "color_"));

                int i = f.getInt(null);
                Context ctx = com.atakmap.android.maps.MapView.getMapView()
                        .getContext();
                if (ctx != null)
                    return String.format(ctx.getString(R.string.team_formatted),
                            ctx.getString(i));
            } catch (Exception e) {
                Log.e(TAG, "error, could not find id: " + title);
            }
        } else if (title.startsWith("filter_absolute_")) {
            try {
                Class<?> c = com.atakmap.app.R.string.class;
                Field f = c.getField(title.replace("filter_absolute_", ""));
        ...
        } else if (title.startsWith("filter_title_")) {
            try {
                Class<?> c = com.atakmap.app.R.string.class;
                Field f = null;
