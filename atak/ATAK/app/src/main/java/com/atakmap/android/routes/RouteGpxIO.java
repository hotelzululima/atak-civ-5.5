...
import com.atakmap.android.gpx.GpxRouteExtension;
...
public class RouteGpxIO {
    ...
    public static Gpx toGpx(Route route) {
        ...
        if (points.isEmpty()) {
            ...
            // Add custom extension values
            GpxRouteExtension extensions = new GpxRouteExtension();
            extensions.setLineColor(route.getColor());
            extensions.setLineWidth(route.getStrokeWeight());
            extensions.setLineStyle(route.getLineStyle());
            extensions.setRouteOrder(route.getRouteOrder());
            extensions.setRouteMethod(route.getRouteMethod());
            extensions.setRouteDirection(route.getRouteDirection());
            extensions.setRouteType(route.getRouteType());
            extensions.setPlanningMethod(route.getPlanningMethod());
            gpxroute.setRouteExtension(extensions);
    ...
    static List<Route> toRoute(MapView mapView, Gpx gpx,
        ...
        double curDistance;
        if (gpx.getRoutes() != null && !gpx.getRoutes().isEmpty()) {
            for (GpxRoute gpxroute : gpx.getRoutes()) {
                ...
                GpxRouteExtension extensions = gpxroute.getRouteExtension();
                route.setColor(extensions.getLineColor());
                route.setStrokeWeight(extensions.getLineWidth());
                route.setLineStyle(extensions.getLineStyle());
                route.setRouteOrder(extensions.getRouteOrder().text);
                route.setRouteMethod(extensions.getRouteMethod().text);
                route.setRouteDirection(extensions.getRouteDirection().text);
                route.setRouteType(extensions.getRouteType().text);
                route.setPlanningMethod(extensions.getPlanningMethod());
