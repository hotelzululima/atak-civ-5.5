...
import com.atakmap.android.toolbars.BullseyeMapItem;
...
public class CompassOverlayReceiver extends DropDownReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        List<MapItem> itemList = _mapView.getRootGroup().deepFindItems(
                "uid", uid);
        if (itemList.size() != 1) {
    ...
    private void showBullseye() {
        MapGroup bullseyeGroup = RangeAndBearingMapComponent.getGroup();
        ...
        if (hostileItem instanceof Marker) {
            ...
            if (hostileItem.hasMetaValue("bullseyeOverlay")) {
                ...
                if (mi instanceof BullseyeMapItem) {
                    mi.removeFromGroup();
                    hostileItem.removeMetaData("bullseyeUID");
                    hostileItem.removeMetaData("bullseyeOverlay");
                }
                return;
            }
            ...
            MapItem bearingCircleMI = MapGroup.findItemWithMetaString(
                    _mapView.getRootGroup(), "uid", UID_PREFIX
                            + hostileItem.getUID());
            if (bearingCircleMI != null) {
                _mapView.getRootGroup().removeItem(bearingCircleMI);
        ...
        if (hostileItem instanceof Marker) {
            MapItem mi = MapGroup.findItemWithMetaString(
                    _mapView.getRootGroup(), "uid", UID_PREFIX
                            + hostileItem.getUID());
            if (mi != null) {
                _mapView.getRootGroup().removeItem(mi);
                hostileItem.removeMetaData("hostileBearingCircle");
                removeListeners(hostileItem);
                return;
            }
            ...
            compassDial.setNorthReference(NorthReference.MAGNETIC);
            ...
            _mapView.getRootGroup().addItem(compassDial);
