...
class GLAngleOverlay2 extends GLShape2 implements
    ...
    final PointD tmpPoint = new PointD();
    double offsetAngle;
    ...
    protected void _projectVerts(final GLMapView ortho) {
        if (this.invalid) {
            ...
            centerMarker.setState(Marker.STATE_MODIFIER_MASK,
            ...
            GeoPoint center = new GeoPoint(
                    ...
                    ortho.getTerrainMeshElevation(centerSurface.getLatitude(),
                            centerSurface.getLongitude()));
            ...
            for (int i = 0; i < outline.length; i++)
                outline[i].set(ortho.getTerrainMeshElevation(
                        outline[i].getLatitude(), outline[i].getLongitude()));
            ...
            for (int i = 0; i < 4; i++) {
                ...
                GeoPoint cardinal = GeoCalculations.pointAtDistance(centerGP,
                        angle, radius);
                double alt = ortho.getTerrainMeshElevation(
                        cardinal.getLatitude(),
                        cardinal.getLongitude());
    ...
    public void draw(GLMapView ortho, int renderPass) {
        ...
        if (!bounds.intersects(ortho.currentPass.northBound,
                ortho.currentPass.westBound,
                ortho.currentPass.southBound, ortho.currentPass.eastBound))
            return;
    ...
    void drawText() {
        ...
        String azimuth;
        if (sw.getNorthReference() == NorthReference.TRUE)
            azimuth = "T";
        else if (sw.getNorthReference() == NorthReference.MAGNETIC)
            azimuth = "M";
        else
            azimuth = "G";
        ...
        for (int i = 0; i < labelIds.length; i++) {
            ...
            int index360 = !sw.isShowingEdgeToCenter() ? 0
                    : (labelIds.length / 2);
            if (i == index360) {
                text = sw.getBearingUnits() == Angle.MIL
                        ? AngleUtilities.formatNoUnitsNoDecimal(360,
                                Angle.DEGREE, Angle.MIL).replace(",", "")
                                + "mils"
                                + azimuth
    ...
    protected RectF getWidgetViewWithoutActionbarF() {
        ...
        float right = ((GLMapView) this.context).currentPass.focusx * 2;
