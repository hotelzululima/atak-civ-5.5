...
public class NavButtonDrawable extends Drawable implements Drawable.Callback {
    ...
    private static final String TAG = "NavButtonDrawable";
    ...
    /**
     * Set whether automatic shadow padding is enabled
    ...
    public void draw(@NonNull Canvas canvas) {
        try {
            // there is a case where the BitmapDrawable backing the NavButtonDrawable
            // can be disposed while this is still rendering to the canvas.   Catch all
            // runtime exceptions and log them for now.   Given that there is no logcat
            // and it has not been reproduced, this is the best attempt at making sure
            // ATAK does not crash.   Worst case scenario, the image button does not
            // render correctly but since it has been recycled, it does not seem like it
            // should even be showing.
            drawImpl(canvas);
        } catch (RuntimeException e) {
            Log.e(TAG,
                    "an error has occurred while rendering the NavButtonDrawable",
                    e);
        }
    }

    private void drawImpl(@NonNull Canvas canvas) {
