...
public class GLMarker2 extends GLPointMapItem2 implements
    ...
    private void updateLabelVisibility(GLMapView view) {
        ...
        final int labelHints = _labelManager.getHints(_labelID);
        ...
        if (labelHints != scrollingLabelHints)
            _labelManager.setHints(_labelID, scrollingLabelHints);
    }
    ...
    private void updateHitRect(GLMapView view) {
        ...
        if (_icon != null && _iconVisibility != Marker.ICON_GONE) {
            ...
            h = _icon.getHeight();
                ...
                Rectangle.contains(view.currentPass.left,
                        view.currentPass.bottom, view.currentPass.right,
                        view.currentPass.top, x, y)) {
    ...
    public void onIconChanged(Marker marker) {
        ...
        final int state = marker.getState() & Marker.ICON_STATE_MASK;
        this.context.queueEvent(new Runnable() {
            @Override
            public void run() {
                _releaseIcon();
    ...
    private void debugHitBox(GLMapView view) {
        ...
        GLES20FixedPipeline.glColor4f(1.f, 0.f, 0.f, 1.f);
        GLES20FixedPipeline.glLineWidth(2f);
        GLES20FixedPipeline
                .glEnableClientState(
                        GLES20FixedPipeline.GL_VERTEX_ARRAY);
    ...
    private void debugHitBox(GLMapView view, GLRenderBatch2 batch) {
        batch.batch(_hitRect.left, _hitRect.bottom, _hitRect.left, _hitRect.top,
                1f, 0f, 0f, 1f);
        batch.batch(_hitRect.left, _hitRect.top, _hitRect.right, _hitRect.top,
                1f, 0f, 0f, 1f);
        batch.batch(_hitRect.right, _hitRect.top, _hitRect.right,
                _hitRect.bottom, 1f, 0f, 0f, 1f);
        batch.batch(_hitRect.right, _hitRect.bottom, _hitRect.left,
                _hitRect.bottom, 1f, 0f, 0f, 1f);
    }
    ...
    private void updateDrawPosition(GLMapView ortho) {
        ...
        if (ortho.currentPass.drawTilt > 0d) {
            // can't move up half icon height because icons using anchor
            // to have have the same center will become offset with tilt.
            // choose a uniform icon height such that most icons will not be
            // overdrawn by the ground
            if (_icon != null && _iconVisibility != Marker.ICON_GONE) {
                final int ICON_HEIGHT = 48;

                _point.y += (ICON_HEIGHT / 2d
                        + (_icon.getAnchorY() - _icon.getHeight() / 2))
    ...
    private boolean shouldDrawLabel(GLMapView ortho) {
        ...
        if (subject.hasMetaValue("minRenderScale")
                && Globe.getMapScale(ortho.currentPass.scene.dpi,
                        ortho.currentPass.drawMapResolution) >= subject
                                .getMetaDouble(
                                        "minRenderScale",
