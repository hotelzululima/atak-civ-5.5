...
public class GroupContact extends Contact implements Search {
    ...
    public GroupContact(String uid, String callsign, List<Contact> contacts,
        ...
        this(uid, callsign, contacts, userCreated,
                getDefaultIconUri(MapView.getMapView().getContext()));
    }

    public GroupContact(String uid, String callsign, List<Contact> contacts,
            boolean userCreated, String iconUri) {
    ...
    public GroupContact(String uid, String callsign, boolean userCreated) {
        this(uid, callsign, new ArrayList<>(), userCreated);
    }
    ...
    public List<String> getUserUIDs(boolean recursive) {
        return Contacts.toUIDs(new ArrayList<>(getUsers(recursive)));
    }
    ...
    private void addContactNoSync(Contact c) {
        if (c != null && !c.getUid().isEmpty()) {
            ...
            for (Contact pContact = this; pContact != null; pContact = pContact
                    .getParent()) {
                if (c.getUid().equals(pContact.getUid())) {
                    Log.d(TAG,
                            "a group cannot exist within a decendent of itself: "
                                    + c.getName());
                    return;
                }
            }
    ...
    static String getDefaultIconUri(Context context) {
        String resourcePath;
        try {
            resourcePath = (context != null) ? context.getPackageName() : "";
        } catch (ExceptionInInitializerError e) {
            // swallow exception that will occur during unit testing
            // when MapView singleton will not be initialized
            resourcePath = "";
        }
        return "android.resource://" + resourcePath + "/"
