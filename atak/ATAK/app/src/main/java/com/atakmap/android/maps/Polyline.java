...
public class Polyline extends Shape {
    ...
    /**
     * A flag for the render flags to hint to the renderer that the item should be faded.
     */
    ...
    public Polyline(final long serialId, final MetaDataHolder2 metadata,
            final String uid) {
    ...
    public double getPerimeterOrLength() {
        synchronized (this) {
            ...
            while (iter.hasNext()) {
                current = iter.next();
                distance += GeoCalculations.distanceTo(last.get(),
                        current.get());
                last = current;
            }
    ...
    public void setLabels(final AttributeSet labels) {
        if (labels == null) {
            this.labels = null;
        } else {
            this.labels = new AttributeSet();
            this.labels.putAll(labels);
        }
        onLabelsChanged();
    }

    public AttributeSet getLabels() {
        return labels;
    }
    ...
    public Bundle preDrawCanvas(CapturePP cap) {
        ...
        AttributeSet labels = getLabels();
        if (labels == null || labels.getAttributeNames().isEmpty())
            return ret;
        PointF[] p = (PointF[]) ret.getSerializable("points");
        if (p == null || p.length < 2)
            return ret;
        boolean closed = (getStyle() & Polyline.STYLE_CLOSED_MASK) > 0;
        PointF[] labelPoints = new PointF[labels.getAttributeNames().size()];
        int i = -1;
        float width = cap.getWidth(), height = cap.getHeight();
        RectF clipRect = new RectF(0, 0, width, height);

        Set<String> labelNames = labels.getAttributeNames();
        for (String labelName : labelNames) {
            i++;
            AttributeSet l = labels.getAttributeSetAttribute(labelName);
            try {
                int indexObject = l.getIntAttribute("segment");
    ...
    public void drawCanvas(CapturePP cap, Bundle data) {
        ...
        if (labels != null && !labels.getAttributeNames().isEmpty()) {
            ...
            Set<String> labelNames = labels.getAttributeNames();
            for (String labelName : labelNames) {
                AttributeSet lMap = labels.getAttributeSetAttribute(labelName);

                l++;
                if (labelPoints[l] == null)
                    continue;
                try {
                    String text = String
                            .valueOf(lMap.getStringAttribute("text"));
    ...
    /**
     * A set of hints used during rendering to be applied to the polyline.
     * @return the current render hints mask
     */
    ...
    /**
     * Allow for rendering hints to be set on a per line basis.
     * @param renderHints the rendering hints to be used.
     */
    public void setRenderHints(int renderHints) {
        this.renderHints = renderHints;
        // trigger a rendering hint change, suffices to call onStrokeColorChange
