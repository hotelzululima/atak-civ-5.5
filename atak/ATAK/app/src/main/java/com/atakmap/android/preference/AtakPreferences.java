...
public class AtakPreferences {
    ...
    private static AtakPreferences _instance;

    private static final int MAX_SIZE = 64 * 1024;

    /**
     * Construct an ATAK core preference manager
     * @param appContext the application context used by the preference manager
     */
    public AtakPreferences(Context appContext) {
        if (appContext == null)
            appContext = MapView.getMapView().getContext();
    ...
    /**
     * Return the singleton preferences.
     * @param context the context which should be the map context
     * @return the singleton for the preference access
     * Not synchronized as it does not matter much it a few copies are made since they are all backed
     * by the same shared preference.
     */
    public static AtakPreferences getInstance(Context context) {
        if (_instance == null)
            _instance = new AtakPreferences(context);
    ...
    private Object get(String key, Object defVal) {
        try {
            if (defVal instanceof Integer)
                ...
                List<String> retval = getStringList(key);
    ...
    public float get(String key, float defVal) {
        return (Float) get(key, (Float) defVal);
    }
    ...
    /**
     * Retrieve a string array from the preferences, storage is internally defined and it is not
     * recommended to make use of this keys value otherwise.
     *
     * @param key Preference key
     * @return String array or null if not found or wrong preference type
     */
    public List<String> getStringList(String key) {
        try {
            Set<String> internalRepresentation = _prefs.getStringSet(key, null);
            if (internalRepresentation == null)
                return null;
            List<String> ret = new ArrayList<>(internalRepresentation);

            for (String s : internalRepresentation) {
                ret.set(Integer.parseInt(s.substring(0, 5)), s.substring(5));
            }
