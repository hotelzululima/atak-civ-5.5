...
public class GLArrow2 extends GLShape2 implements OnPointsChangedListener,
    ...
    public GLArrow2(MapRenderer surface, Arrow arrow) {
        ...
        updateText(arrow.getText(), arrow.getTextColor());
    ...
    /**
     * Computes the style based on the basic style that the subject has.
     * @param basicStyle one of Shape.BASIC_LINE_STYLE_SOLID, BASIC_LINE_STYLE_DASHED,
     *                   BASIC_LINE_STYLE_DOTTED,BASIC_LINE_STYLE_OUTLINED
     * @param strokeColor the color in ARGB
     * @param strokeWeight the weight to use for the stroke
     * @param xray if the style will be used for the xray component
     * @return the style
     */
    private static Style computeStyle(final int basicStyle,
            final int strokeColor,
            final float strokeWeight, final boolean xray) {
        Style s;
        ...
        if (basicStyle == Polyline.BASIC_LINE_STYLE_DASHED)
            s = new PatternStrokeStyle(1, (short) 0x3F3F,
                    ...
                    Color.alpha(xray ? strokeColor & xraymask : strokeColor)
                            / 255f,
                    strokeWeight);
            ...
            BasicStrokeStyle bg = new BasicStrokeStyle(
                    xray ? xraymask & strokeColor : 0xFF000000 & strokeColor,
                    strokeWeight + 2f);
            ...
            s = new BasicStrokeStyle(
                    xray ? strokeColor & xraymask : strokeColor, strokeWeight);
        }
        return s;
    }

    private void refreshStyle() {
        // would love to listen to changes much like GLPolyline but due to the fact that this needs
        // to be backported and cannot modify the mapping.
        final int basicStyle = _subject.getBasicLineStyle();

        final Style strokeStyle = computeStyle(basicStyle, strokeColor,
                strokeWeight, false);
        this.impl.setStyle(strokeStyle);

        final Style xStrokeStyle = computeStyle(basicStyle, strokeColor,
                strokeWeight, true);
        this.ximpl.setStyle(xStrokeStyle);
    }
    ...
    protected void _validateArrowhead(GLMapView ortho) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (_nadirClamp || _clampToGround) {
                ortho.scratch.geo.set(Double.NaN);
            } else if (ptAgl) {
        ...
        if (_nadirClamp || _clampToGround)
            pts[0] = new GeoPoint(pts[0].getLatitude(), pts[0].getLongitude(),
                    Double.NaN);

        // Get the tail and head in screen coordinates so we can calculate the on-screen angle
        for (int i = 0; i < 2; i++) {
            ortho.currentPass.scene.forward(pts[i], ortho.scratch.pointD);
    ...
    private void pointsChangedImpl(GLMapView ortho, GeoPoint[] p,
        ...
        for (int i = 0; i < numPts; i++) {
            ...
            final boolean ptAgl = (_pts[i]
                    .getAltitudeReference() == GeoPoint.AltitudeReference.AGL ||
                    Double.isNaN(_pts[i].getAltitude()) || _nadirClamp
                    || _clampToGround);
            _ptsAgl |= ptAgl;

            final double lat = _pts[i].getLatitude();
            final double lng = _pts[i].getLongitude();
            final double hae = (_nadirClamp || _clampToGround) ? Double.NaN
                    : getHae(ortho, _pts[i]);
    ...
    static GeoPoint closestSurfacePointToFocus(GLMapView ortho, float xmid,
        ...
        ortho.currentPass.scene.mapProjection.inverse(
                ortho.currentPass.scene.camera.target,
                ortho.scratch.geo);
        ...
        ortho.currentPass.scene.mapProjection.forward(ortho.scratch.geo,
        ...
        com.atakmap.math.Vector3D normal = new com.atakmap.math.Vector3D(
                (ortho.scratch.pointD.x
                        - ortho.currentPass.scene.camera.target.x),
                (ortho.scratch.pointD.y
                        - ortho.currentPass.scene.camera.target.y),
                (ortho.scratch.pointD.z
                        - ortho.currentPass.scene.camera.target.z));
        Plane focusPlane = new Plane(normal,
                ortho.currentPass.scene.camera.target);
        GeoPoint focus = GeoPoint.createMutable();
        if (ortho.currentPass.scene.inverse(new PointF(xmid, ymid), focus,
                focusPlane) == null)
            ortho.currentPass.scene.mapProjection
                    .inverse(ortho.currentPass.scene.camera.target, focus);

        if (ortho.currentPass.drawSrid == 4978) {
            ...
            ortho.currentPass.scene.mapProjection.forward(a,
                    ortho.scratch.pointD);
            final double ax = ortho.scratch.pointD.x
                    * ortho.currentPass.scene.displayModel.projectionXToNominalMeters;
            final double ay = ortho.scratch.pointD.y
                    * ortho.currentPass.scene.displayModel.projectionYToNominalMeters;
            final double az = ortho.scratch.pointD.z
                    * ortho.currentPass.scene.displayModel.projectionZToNominalMeters;
            ortho.currentPass.scene.mapProjection.forward(b,
                    ortho.scratch.pointD);
            ...
            ortho.currentPass.scene.mapProjection.forward(focus,
                    ortho.scratch.pointD);
            ...
            return ortho.currentPass.scene.mapProjection.inverse(
                    ortho.scratch.pointD,
                    null);
        }
    }
