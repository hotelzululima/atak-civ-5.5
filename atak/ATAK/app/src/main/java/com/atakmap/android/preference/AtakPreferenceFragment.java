...
public abstract class AtakPreferenceFragment extends PreferenceFragment {
    ...
    static AtakPreferences _mainControlPrefs; // ugg
    ...
    public Preference findPreference(final CharSequence key) {
        ...
        if (_mainControlPrefs == null)
            _mainControlPrefs = AtakPreferences.getInstance(getActivity());
    ...
    private void processPreference(final Preference p) {
        ...
        if (key != null) {
            ...
            if (_mainControlPrefs.get(disablePref, false)) {
            ...
            if (_mainControlPrefs.get(hidePref, false)) {
                try {
            ...
            if (dependsOnKey != null) {
                final String dependsHidePref = "hidePreferenceItem_"
                        + dependsOnKey;
                if (_mainControlPrefs.get(dependsHidePref, false)) {
    ...
    static public int getOrientation(Context context) {
        ...
        final boolean reverse = prefs.get("atakControlReverseOrientation",
                false);

        if (prefs.get("atakControlForcePortrait", false)) {
    ...
    static public void setSoftKeyIllumination(final Activity activity) {
        final AtakPreferences prefs = AtakPreferences.getInstance(activity);
        final boolean disable = prefs
                .get("atakDisableSoftkeyIllumination", true);
    ...
    static public void setOrientation(final Activity activity,
        ...
        AtakPreferences _controlPrefs = AtakPreferences.getInstance(activity);
        if (_controlPrefs.get("atakControlForcePortrait", false)) {
            desiredOrientation = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT;
        } else if (_controlPrefs.get("atakControlReverseOrientation",
                false)) {
    ...
    static public void checkScreenLock(Activity activity) {
        AtakPreferences _controlPrefs = AtakPreferences.getInstance(activity);
        boolean screenLock = _controlPrefs.get("atakScreenLock", false);
        if (screenLock) {
    ...
    static public void checkKeyGuard(Activity activity) {
        ...
        try {
            if (_controlPrefs.get("atakDisableKeyguard", false)) {
                kl.disableKeyguard();
            } else {
                kl.reenableKeyguard();
            }
    ...
    static public void setMainWindowActivity(final Activity activity) {
        _mainControlPrefs = AtakPreferences.getInstance(activity);
        ...
        _mainControlPrefs.registerListener(listener);
    ...
    /**
     * Ability to programmatically toggle on or off the state of the preference
     ...
     * @return an empty list if there is an error, otherwise returns a PreferenceSearchIndex object created
    ...
    protected static List<PreferenceSearchIndex> index(Context context,
        ...
        try {
            ...
            Log.d(TAG, "error indexing: " + clazz, e);
        }
        return retval;
    }
    ...
    /**
     * If the locale needs to be force set to US.   If the preference is false, then the
     * locale is restored back to the device locale settings.
     * @param activity the activity to modify the locale for
     */
    public static void setLocale(final Activity activity) {
        // during the orientation check make sure to if the language is forced
        // force it here
