...
public class GeoFenceAlerting {
    ...
    /**
     * Ability for plugins to listen for alerts.
     */
    public interface GeoFenceAlertListener {
        /**
         * Fired when a new alert occurs
    ...
    private ConcurrentLinkedQueue<GeoFenceAlertListener> geoFenceAlertListeners = new ConcurrentLinkedQueue<>();
     ...
     *
     * @return the Map of alerts
     */
    public synchronized Map<GeoFenceMonitor, List<Alert>> getAlerts() {
        if (_ongoingAlerts == null || _ongoingAlerts.isEmpty())
            return new HashMap<>();
    ...
    /**
     * Adds a listener for any geofence alert triggers
     * @param geoFenceAlertListener the geofence alert listener
     */
    public void addOnGeoFenceAlertListener(
            GeoFenceAlertListener geoFenceAlertListener) {
        if (!geoFenceAlertListeners.contains(geoFenceAlertListener))
            geoFenceAlertListeners.add(geoFenceAlertListener);
    }

    /**
     * Removes a listener if it has been previously registered
     * @param geoFenceAlertListener the geofence alert listener
     */
    public void removeOnGeoFenceAlertListener(
            GeoFenceAlertListener geoFenceAlertListener) {
        geoFenceAlertListeners.remove(geoFenceAlertListener);
    }

    private void fireOnGeoFenceAlert(Alert alert) {
        for (GeoFenceAlertListener geoFenceAlertListener : geoFenceAlertListeners) {
            try {
                geoFenceAlertListener.onAlert(alert);
            } catch (Exception e) {
                Log.e(TAG, "unknown error occurred while calling: "
                        + geoFenceAlertListener, e);
            }
        }
    }
