...
import com.atakmap.android.importfiles.resource.RemoteResourceResolver;
...
public class LayersMapComponent extends AbstractMapComponent
    ...
    private final RasterLayer2.OnSelectionChangedListener cacheSourcesUpdater = new RasterLayer2.OnSelectionChangedListener() {
        @Override
        public void onSelectionChanged(RasterLayer2 layer) {
            if (layer == mobileLayers2) {
                // the mobile selection has changed, dispatch the new source/sink for smart caching
                ...
                try {
                    ...
                    params.imageryTypes = Collections.singleton(selection);
                    params.limit = 1;
                    params.remoteLocalFlag = RasterDataStore.DatasetQueryParameters.RemoteLocalFlag.REMOTE;
                    result = mobileLayers2.getDataStore().queryDatasets(params);
                    if (!result.moveToNext())
                        return;
                    final DatasetDescriptor desc = result.get();
                    final String cache = desc.getExtraData("offlineCache");
                    if (cache == null)
                        return;
                    eventBus.post(new UpdateCacheSource(
                            new String[] {
                                    desc.getUri()
                    }, new String[] {
                            cache
    ...
    public void onCreate(final Context context, Intent intent, MapView view) {
        ...
        NATIVE_QUERY_PARAMS.datasetTypes.add("tiles");
        //}

        try {
            ImageryScanner.setDepth(prefs.get("imageryScannerDepth", 3));
        } catch (Exception e) {
            Log.e(TAG, "error: ", e);
        }
        ...
        // set any persisted relative scale before building the layers
        final float relativeScale = prefs.get("prefs_imagery_relative_scaling",
                1f);
        ImageryRelativeScaleControl ctrl = _mapView.getRenderer3().getControl(
                ImageryRelativeScaleControl.class);
        if (ctrl != null)
            ctrl.setRelativeScale(relativeScale);

        buildUpLayers(context, view, prefs.getSharedPrefs());

        prefs.registerListener(this);
        prefs.registerListener(controlPrefsChangedListener);

        // sync preferences for smart caching
        onSharedPreferenceChanged(prefs.getSharedPrefs(),
                "prefs_smart_cache_download_limit");
        onSharedPreferenceChanged(prefs.getSharedPrefs(),
                "prefs_enable_smart_cache");
    ...
    private void buildUpLayers(Context context, MapView view,
        ...
        this.mobileLayers2
                .addOnSelectionChangedListener(this.cacheSourcesUpdater);
        this.cacheSourcesUpdater.onSelectionChanged(this.mobileLayers2);

        final OnlineImageryExtension svc = mobileLayers2
                .getExtension(OnlineImageryExtension.class);
        if (svc != null)
            svc.addOnOfflineOnlyModeChangedListener(
                    new OnlineImageryExtension.OnOfflineOnlyModeChangedListener() {
                        @Override
                        public void onOfflineOnlyModeChanged(
                                OnlineImageryExtension ext,
                                boolean offlineOnly) {
                            // online/offline has been toggled, update smart caching appropriately
        ...
        this.mobileOutlines = new MobileOutlinesDataStore2(this.mobileLayers2,
                0, Adapters.adapt(new FeatureSetDatabase(null)));

        this.mobileOutlinesLayer = new FeatureLayer3("Mobile Layer Outlines",
                mobileOutlines);
        this.nativeOutlines = new OutlinesFeatureDataStore2(this.nativeLayers,
                0, false, Adapters.adapt(new FeatureSetDatabase(null)));
        ...
        _importResolvers.add(new RemoteResourceResolver(view));
    ...
    private void _initializeLayers() {
        ...
        _contentHandler = new Handler(
                _mapView.getContext().getMainLooper());
        ...
        loadLayerState(
                AtakPreferences.getInstance(_context).getSharedPrefs(),
                "native",
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        AtakPreferences.getInstance(context)
                .unregisterListener(controlPrefsChangedListener);
    ...
    private void showLastViewedLayer() {
        try {
            ...
            final String active = prefs.get("lastViewedLayer.active", null);
            final String layerName = prefs.get("lastViewedLayer.name", null);
            final boolean autoSelect = prefs.get("lastViewedLayer.autoselect",
                    false);
    ...
    private void _saveLastViewedLayer(Context context) {
        try {
            if (rasterLayers != null) {
                ...
                if (ls == null)
                    prefs.set("lastViewedLayer.name", null);
                else
                    prefs.set("lastViewedLayer.name", ls.getName());
                prefs.set("lastViewedLayer.active", active);
                prefs.set("lastViewedLayer.autoselect",
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
        ...
        if (key.equals("prefs_layer_outlines_default_show")) {
            Log.d(TAG, "showOutlines changed");

            setFeaturesVisible(mobileOutlines, null, sharedPreferences
                    .getBoolean("prefs_layer_outlines_default_show", false));
        }
        if (key.equals("prefs_smart_cache_download_limit")) {
            CachingService
                    .setSmartCacheDownloadLimit(Long.parseLong(sharedPreferences
                            .getString(key, String.valueOf(CachingService
                                    .getSmartCacheDownloadLimit()))));
        }
        if (key.equals("prefs_enable_smart_cache")) {
            boolean smartCacheEnabled = sharedPreferences
                    .getBoolean(key, CachingService.getSmartCacheEnabled());
            if (_mobileLayersAdapter2 != null) {
                smartCacheEnabled &= !_mobileLayersAdapter2.isOfflineOnly();
            }
            CachingService.setSmartCacheEnabled(smartCacheEnabled);
        }
    }
                        ...
                        while (result.moveToNext()) {
                            ...
                            if (f.getStyle() instanceof BasicStrokeStyle) {
                                ...
                                if (adoptParentVisibility)
                                    visible |= Utils.isFeatureVisible(
                                            layerOutlines, f.getId());
                ...
                while (result.moveToNext()) {
                    ...
                    if (f.getStyle() instanceof BasicStrokeStyle) {
                        colors.put(f.getName(), Pair.create(
                                ...
                                Utils.isFeatureVisible(layerOutlines,
                                        f.getId())));
                    }
                }
            } catch (DataStoreException ignored) {
                                ...
                                public void onClick(
                                    ...
                                    try {
                                        ...
                                        if (c.moveToNext()) {
                                            ...
                                            // fall back, walk directories; maybe not
                                            // needed
                                        }
                                    } finally {
                                        if (c != null)
