...
public class ImageGalleryFileAdapter extends ImageGalleryBaseAdapter
            ...
            public void run() {
                ...
                if (usingUIDs) {
                    ...
                    for (File f : files) {
                        ...
                        if (uid != null) {
                            MapItem mapItem = group.deepFindUID(uid);
    ...
    /**
     * Updates an existing GalleryFileItem objet inside the adapter
     *
     * @param oldUri          the old file uri
     * @param galleryFileItem The object to update in the list that matches the old uri String
     */
    public int updateGalleryItem(File oldUri, GalleryFileItem galleryFileItem) {
        GalleryFileItem gfi = null;
        for (int i = 0; i < allItems.size(); i++) {
            GalleryFileItem item = allItems.get(i);
            if (item.getFile().equals(oldUri)) {
                gfi = item;
                break;
            }
        }
        ...
        if (gfi != null) {
            int idx = allItems.indexOf(gfi);
            if (idx > -1) {
                allItems.set(idx, galleryFileItem);
                refreshImpl();
                return idx;
            }
        }
    ...
    public void customizeGalleryView(View gallery) {
        ...
        final AtakPreferences prefs = AtakPreferences
                .getInstance(getMapView().getContext());

        CheckBox showAll = gallery.findViewById(R.id.showAll_cb);

        if (usingUIDs && !markerAttachments) {
            boolean filterByFOV = prefs.get(FOVFilter.PREF, false);
    ...
    protected void displayFile(GalleryItem item) {
        ...
        List<gov.tak.api.importfiles.ImportResolver> resolvers = ImportFilesTask.GetKernelSorters(
                getMapView().getContext(), false);
        for (gov.tak.api.importfiles.ImportResolver r : resolvers) {
            if (r.match(f)) {
                ...
                ImportExportMapComponent.getInstance().cleanupResolvers(resolvers);
                return;
            }
        }
        ImportExportMapComponent.getInstance().cleanupResolvers(resolvers);
