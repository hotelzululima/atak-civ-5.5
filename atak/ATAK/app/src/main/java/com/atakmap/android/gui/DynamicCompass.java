...
public class DynamicCompass extends RelativeLayout
    ...
    private static final String PREFERENCES_KEY_COMPASS_HEADING_DISPLAY = "compass_heading_display";

    public enum HeadingDisplayType {
        NUMERIC,
        CARDINAL
    }
    ...
    private HeadingDisplayType headingDisplayType = HeadingDisplayType.NUMERIC;
    ...
    private void init() {
        ...
        prefs = AtakPreferences.getInstance(getContext());
        prefs.registerListener(this);
        headingDisplayType = getHeadingDisplayType();
        ...
        headingTxt.setHeadingDisplayType(headingDisplayType);
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
            String key) {
        if (PREFERENCES_KEY_COMPASS_HEADING_DISPLAY.equalsIgnoreCase(key)) {
            headingDisplayType = getHeadingDisplayType();
            headingTxt.setHeadingDisplayType(headingDisplayType);
            update();
        }
    ...
    private HeadingDisplayType getHeadingDisplayType() {
        final String displayType = prefs.get(
                PREFERENCES_KEY_COMPASS_HEADING_DISPLAY,
                HeadingDisplayType.NUMERIC.name());
        return displayType.equalsIgnoreCase(HeadingDisplayType.NUMERIC.name())
                ? HeadingDisplayType.NUMERIC
                : HeadingDisplayType.CARDINAL;
    }
    ...
    public void update() {
        ...
        if (gpsLock) {
            ...
            int heading = Math.round(this.heading);
            if (heading == 0)
                heading = 360;
    ...
    public static class HeadingText extends View {
        ...
        public void setHeadingDisplayType(
                HeadingDisplayType headingDisplayType) {
            this.headingDisplayType = headingDisplayType;
        ...
        private String getTextForHeading() {
            return headingDisplayType == HeadingDisplayType.NUMERIC
                    ? String.valueOf(headingDeg)
                    : DirectionType.getDirection(headingDeg).getAbbreviation();
        }
        ...
        protected void onDraw(@NonNull Canvas canvas) {
            ...
            String text = headingStr;

            if (units != null
                    && headingDisplayType == HeadingDisplayType.NUMERIC) {
