...
public class DownloadAndCacheBroadcastReceiver
    ...
    public void onRegionSelectFinished(Intent intent) {
        ...
        if (downloader.hasRegionShape())
            lmbr.getDownloadButton().setEnabled(true);
    }
        ...
        switch (status.code) {
            ...
            case DownloadJob.ERROR: {
                ...
                if (status.queuedDownloads <= 0
                    ...
                    if (errors)
                        lmbr.onDownloadWorkflowComplete(-1);
                    else if (canceled)
                        lmbr.onDownloadWorkflowComplete(1);
                    else
                        lmbr.onDownloadWorkflowComplete(0);
                }
                if (status.queuedDownloads <= 0)
