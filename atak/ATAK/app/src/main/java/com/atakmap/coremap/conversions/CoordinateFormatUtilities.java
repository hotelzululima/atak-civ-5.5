...
public class CoordinateFormatUtilities {
     ...
     * @param precise if precision is desired at the cost of speed
    ...
    public static String _convertToLatDegString(GeoPoint p, boolean precise) {
        ...
        if (latDegS.contains(CHAR_NEG)) {
            latDegS = latDegS.replace(CHAR_NEG, CHAR_S + CHAR_LRM_SPACE);
        } else {
            latDegS = CHAR_N + CHAR_LRM_SPACE + latDegS;
        }
        return latDegS + CHAR_DEG + CHAR_LRM_SPACE;
    }

    /**
     * @param p GeoPoint that describes a specific location
     * @param precise is precision is desired at the cost of speed
    ...
    public static String _convertToLngDegString(GeoPoint p, boolean precise) {
        ...
        if (lonDegS.contains(CHAR_NEG)) {
            lonDegS = lonDegS.replace(CHAR_NEG, CHAR_W + CHAR_LRM_SPACE);
        } else {
            lonDegS = CHAR_E + CHAR_LRM_SPACE + lonDegS;
        }
        return lonDegS + CHAR_DEG + CHAR_LRM_SPACE;
    }
     ...
     * @throws IllegalArgumentException if there is an issue converting a between
     * coordinate formats.
     */
        ...
        switch (entry) {
            case MGRS:
                ...
                if (coordinate.length == 4
                    ...
                    } else if (coordinate[2] instanceof Double
                ...
                if (coordinate.length == 3
                    ...
                    if (coordinate[1] instanceof String
                        ...
                        return _convertFromUTM((String) coordinate[0],
                                (String) coordinate[1],
                                (String) coordinate[2]);
                    } else if (coordinate[1] instanceof Double
    ...
    private static String _convertToDecDegString(final GeoPoint p)
        ...
        if (latS.contains(CHAR_NEG)) {
            latS = latS.replace(CHAR_NEG, CHAR_S + CHAR_LRM_SPACE);
        } else {
            latS = CHAR_N + CHAR_LRM_SPACE + latS;
        }
        if (lonS.contains(CHAR_NEG)) {
            lonS = lonS.replace(CHAR_NEG, CHAR_W + CHAR_LRM_SPACE);
        } else {
            lonS = CHAR_E + CHAR_LRM_SPACE + lonS;
        }

        latS += CHAR_DEG;
        lonS += CHAR_DEG;
        return latS + CHAR_LRM_SPACE + CHAR_LRM_SPACE + lonS;
    }
    ...
    private static GeoPoint _convertFromDecDeg(String coordinate)
        ...
        coord = coord.replace(CHAR_SPACE + CHAR_LRM, CHAR_SPACE);
        coord = coord.replace(CHAR_N + CHAR_LRM, CHAR_N);
        coord = coord.replace(CHAR_E + CHAR_LRM, CHAR_E);

        // Change the S W characters to be negative signs
        coord = coord.replace(CHAR_S + CHAR_LRM_SPACE, CHAR_NEG);
        coord = coord.replace(CHAR_W + CHAR_LRM_SPACE, CHAR_NEG);
        ...
        coord = coord.replace(CHAR_COMMA, CHAR_LRM_SPACE);
        coord = coord.replace(CHAR_COLON, CHAR_LRM_SPACE);
        coord = coord.replace(CHAR_SEMICOLON, CHAR_LRM_SPACE);

        coord = coord.replace(CHAR_LRM, CHAR_SPACE);
    ...
    private static String _convertToMGRSString(GeoPoint p)
        ...
        MGRSPoint mgrs = MGRSPoint.fromLatLng(Ellipsoid.WGS_84,
                ...
                null);
        return mgrs.getZoneDescriptor() + CHAR_LRM_SPACE
                + mgrs.getGridDescriptor()
                + CHAR_LRM_SPACE
                + mgrs.getEastingDescriptor() + CHAR_LRM_SPACE
    ...
    private static String _convertToUTMString(GeoPoint p)
        ...
        return utm.getZoneDescriptor()
                + CHAR_LRM_SPACE
                + utm.getEastingDescriptor() + CHAR_LRM_SPACE
    ...
    private static GeoPoint _convertFromDegMin(String coordinate)
        ...
        coord = coord.replace(CHAR_DEG + CHAR_LRM_SPACE, CHAR_COLON);
    ...
    private static String _convertToDegMinString(GeoPoint p, boolean precise) {
        ...
        return latDegS + CHAR_DEG + CHAR_LRM_SPACE + latMinS + CHAR_MIN
                + CHAR_LRM_SPACE + CHAR_LRM_SPACE + lonDegS + CHAR_DEG
                + CHAR_LRM_SPACE
                + lonMinS + CHAR_MIN;
    }
    ...
    private static GeoPoint _convertFromDegMinSec(String coordinate) {
        ...
        coord = coord.replace(CHAR_SPACE + CHAR_LRM, CHAR_SPACE);
        coord = coord.replace(CHAR_SEC + CHAR_LRM, CHAR_SEC);
        coord = coord.replace(CHAR_N + CHAR_LRM, CHAR_N);
        coord = coord.replace(CHAR_E + CHAR_LRM, CHAR_E);
        ...
        coord = coord.replace(CHAR_DEG + CHAR_LRM_SPACE, CHAR_COLON);
        coord = coord.replace(CHAR_DEG, CHAR_COLON);
        coord = coord.replace(CHAR_MIN + CHAR_LRM_SPACE, CHAR_COLON);
    ...
    private static String _convertToDegMinSecString(GeoPoint p,
        ...
        if (latDeg.contains(CHAR_NEG)) {
            latDeg = latDeg.replace(CHAR_NEG, CHAR_S + CHAR_LRM_SPACE);
        } else {
            latDeg = CHAR_N + CHAR_LRM_SPACE + latDeg;
        }
        if (lonDeg.contains(CHAR_NEG)) {
            lonDeg = lonDeg.replace(CHAR_NEG, CHAR_W + CHAR_LRM_SPACE);
        } else {
            lonDeg = CHAR_E + CHAR_LRM_SPACE + lonDeg;
        }

        return latDeg + CHAR_DEG + CHAR_LRM_SPACE + latMin + CHAR_MIN
                + CHAR_LRM_SPACE
                + latSec + CHAR_SEC
                + CHAR_LRM_SPACE + CHAR_LRM_SPACE + lonDeg + CHAR_DEG
                + CHAR_LRM_SPACE
                + lonMin + CHAR_MIN
                + CHAR_LRM_SPACE + lonSec + CHAR_SEC;
    }
    ...
    private static final String CHAR_LRM_SPACE = "\u200E ";
    private static final String CHAR_LRM = "\u200E"; // use left to right mark for BIDI text cases
