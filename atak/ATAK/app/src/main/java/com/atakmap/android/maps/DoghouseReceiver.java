...
public class DoghouseReceiver extends BroadcastReceiver implements
    ...
    public static final String TOGGLE_DOGHOUSE_VISIBLE_ACTION = "com.atakmap.android.maps.TOGGLE_DOGHOUSE_VISIBLE";
    ...
    private DoghouseReceiver(MapView mapView) {
        ...
        if (checkApplyDefaults(prefs.getSharedPrefs())) {
            applyDefaults(prefs.getSharedPrefs());
        }
    ...
    public void dispose() {
        ...
        AtakPreferences.getInstance(_mapView.getContext())
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action == null)
            return;
        if (action.equals(TOGGLE_DOGHOUSE_VISIBLE_ACTION)) {
            MapItem item = _mapView.getMapItem(intent.getStringExtra("uid"));
            if (item instanceof Route) {
                boolean b = item.getMetaBoolean(META_SHOW_DOGHOUSES, false);
                item.setMetaBoolean(META_SHOW_DOGHOUSES, !b);
                if (!b)
                    _viewModel.addDoghouses((Route) item);
                else
                    _viewModel.removeDoghouses((Route) item);
    ...
    public void onMapEvent(MapEvent event) {
        ...
        if (MapEvent.ITEM_ADDED.equals(event.getType())) {
            if (route.getRouteMethod() == Route.RouteMethod.Flying
                    && route.getMetaBoolean(META_SHOW_DOGHOUSES, false)) {
    ...
    final Route route) {
        if (route.getMetaBoolean(META_SHOW_DOGHOUSES, false)) {
            _viewModel.addDoghouses(route);
        }
    }
