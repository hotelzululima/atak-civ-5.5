...
public class VideoContentResolver extends FileContentResolver
    ...
    public FileContentHandler getHandler(String tool, String uri) {
        ...
        for (String ext : VideoFileWatcher.VIDEO_EXTS) {
            if (FileSystemUtils.checkExtension(f, ext)) {

                // we check to see if the xml file is actually a video alias later on
                if (ext.equals("xml"))
                    continue;

                if (FileSystemUtils.isFile(f)) {
                    return new VideoContentHandler(_mapView,
                            new ConnectionEntry(f));
                }
            }
        }
