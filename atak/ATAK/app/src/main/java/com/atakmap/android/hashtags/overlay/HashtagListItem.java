...
public class HashtagListItem extends AbstractHierarchyListItem2 implements
        View.OnClickListener, Visibility2, Search, Delete, Export {
    ...
    public Object toObjectOf(Class<?> target, ExportFilters filters)
        ...
        if (MissionPackageExportWrapper.class.equals(target)) {
            MissionPackageExportWrapper mpew = new MissionPackageExportWrapper();
            Set<HashtagContent> contents = HashtagManager.getInstance()
                    .findContents(_tag);
            for (HashtagContent content : contents) {
                if (content instanceof Exportable &&
                        ((Exportable) content).isSupported(
                                MissionPackageExportWrapper.class)) {
                    MissionPackageExportWrapper itemWrapper = (MissionPackageExportWrapper) ((Exportable) content)
                            .toObjectOf(target, filters);
                    if (itemWrapper != null
                            && !FileSystemUtils
                                    .isEmpty(itemWrapper.getFilepaths())) {
                        mpew.getFilepaths().addAll(itemWrapper.getFilepaths());
                    }
                    if (itemWrapper != null
                            && !FileSystemUtils
                                    .isEmpty(itemWrapper.getUIDs())) {
                        mpew.getUIDs().addAll(itemWrapper.getUIDs());
                    }
