...
public class GPXExportWrapper {
    ...
    public GPXExportWrapper(Gpx f) {
        ...
        if (f.getWaypoints() != null && !f.getWaypoints().isEmpty())
            _exports.addAll(f.getWaypoints());
        if (f.getTracks() != null && !f.getTracks().isEmpty())
            _exports.addAll(f.getTracks());
        if (f.getRoutes() != null && !f.getRoutes().isEmpty())
    ...
    public void add(GPXExportWrapper folder) {
        ...
        if (folder.getExports() != null && !folder.getExports().isEmpty())
            _exports.addAll(folder.getExports());
    }

    public boolean isEmpty() {
        return _exports == null || _exports.isEmpty();
    }
}
