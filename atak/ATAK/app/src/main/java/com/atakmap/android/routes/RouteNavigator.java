...
public class RouteNavigator {
    ...
    private final AtakPreferences navPrefs;
    ...
    RouteNavigator(final MapView mapView, final MapGroup navGroup,
        ...
        navPrefs = AtakPreferences.getInstance(mapView
                .getContext());
    ...
    public synchronized boolean startNavigating(final Route route,
        ...
        if (route.getNumPoints() < 2) {
            ...
            if (route.getNumPoints() == 1 && (m = route.getMarker(0)) != null) {
                ...
                Intent i = new Intent(BloodHoundTool.BLOOD_HOUND);
        ...
        navigating = true;
        navigate();
        mapView.addLayer(MapView.RenderStack.RASTER_OVERLAYS, billboardLayer);
        billboardLayer.setVisible(navPrefs.get(
                "route_billboard_enabled", true));
        Log.d(TAG, "navigating route: " + route);
        resetWaypointsReached(route);
        fireOnNavigationStarted();
        return true;
    }

    /**
     * Resets the reached status of any required way points in a route to an unreached state
     *
     * @param route Route of which to reset the status of any required way points
     */
    void resetWaypointsReached(Route route) {
        //remove the metadata boolean we set on points that were reached during the current nav session
        for (MapItem mi : route.getRequiredWaypoints()) {
            mi.removeMetaData(RouteRequiredWayPointHandler.REACHED_WAYPOINT);
        }
    }
    ...
    synchronized public void stopNavigating() {
        ...
        mapView.removeLayer(MapView.RenderStack.RASTER_OVERLAYS,
                billboardLayer);
