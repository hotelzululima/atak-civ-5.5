...
public class MissionPackageQueryResultListAdapter extends BaseAdapter {
    ...
    void reset(List<MissionPackageQueryResult> available,
        ...
        lastQuery = AtakPreferences.getInstance(_context)
                .get("mission-package-last-query." + serverConnectString, "");
        ...
        String newest = "";
        for (MissionPackageQueryResult result : _available) {
            if (newest.compareToIgnoreCase(
                    result.getSubmissionDateTime()) < 0)
                newest = result.getSubmissionDateTime();
        }
        AtakPreferences.getInstance(_context).set(
                "mission-package-last-query." + serverConnectString, newest);
    ...
    public View getView(int position, View row, ViewGroup parent) {
        ...
        if (lastQuery.compareToIgnoreCase(
                l.getSubmissionDateTime()) < 0) {
            h.nameText.setTypeface(Typeface.DEFAULT_BOLD);
            h.nameText.setTextColor(Color.GREEN);
        } else {
            h.nameText.setTypeface(Typeface.DEFAULT);
            h.nameText.setTextColor(Color.WHITE);
        }
