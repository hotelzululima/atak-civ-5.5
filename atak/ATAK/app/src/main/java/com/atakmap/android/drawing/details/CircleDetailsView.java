...
public class CircleDetailsView extends GenericDetailsView implements
    ...
    private Button _extrudeModeBtn;
    ...
    public boolean setItem(MapView mapView, MapItem circle) {
        if (_circle != null) {
            ...
            _circle.removeOnMetadataChangedListener("extrudeMode", this);
        }
        ...
        _circle.addOnMetadataChangedListener("extrudeMode", this);
        _init();
        return true;
    }
    ...
    private void _init() {
        ...
        findViewById(R.id.drawingCircleEditButton)
                .setEnabled(_circle.getMetaBoolean("editable", true));

        _extrudeModeBtn = findViewById(R.id.extrudeModeBtn);
        setupExtrudeModeButton(_extrudeModeBtn, (Shape) _item);
    ...
    public void refresh() {
        ...
        _extrudeModeBtn.setEnabled(!Double.isNaN(height) && height != 0d);

        CoTInfoView.refreshAuthorLayout(_mapView, _circle, _authorText,
                _productionTimeText, cotAuthorLayout, cotAuthorIconButton,
                cotAuthorPanButton);

        _circle.refresh();
        _circle.persist(MapView.getMapView().getMapEventDispatcher(), null,
                this.getClass());
    }
    ...
    private void _onCenterSelected() {
        ...
        CoordinateEntryCapability.getInstance(getContext()).showDialog(
                getContext().getString(R.string.circle_center_point),
                _circle.getCenter(),
                _circle.getMovable(), _mapView.getPoint(), _cFormat, false,
                new CoordinateEntryCapability.ResultCallback() {
                    ...
                    public void onResultCallback(String pane,
                        ...
                        GenericPointDetailsView.setAddress(point,
                                _circle.getAnchorItem(), _nameEdit);
                        refresh();
    ...
    protected void heightSelected(double height, Span u, double h) {
        ...
        // NOTE: if sphere extrusion, modifying the height will reset the radius
        if (!Double.isNaN(height) &&
                height > 0d &&
                _circle.getMetaString(DetailConstants.EXTRUDE_MODE, "")
                        .equals("sphere")) {

            _circle.setRadius(height);
        }
        refresh();
    }
    ...
    public void onRadiusChanged(DrawingCircle circle, double oldRadius) {
        // NOTE: if sphere extrusion, modifying the radius will reset the height
        if (!Double.isNaN(circle.getHeight()) &&
                circle.getHeight() > 0d &&
                _circle.getMetaString(DetailConstants.EXTRUDE_MODE, "")
                        .equals("sphere")) {

            _circle.setHeight(circle.getRadius());
        }
        refresh();
    }
    ...
    private void setupExtrudeModeButton(@NonNull
    final View button,
            @NonNull
            final Shape shape) {

        updateExtrudeModeButton(button, shape);
        button.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                promptExtrudeMode(shape, new Runnable() {
                    @Override
                    public void run() {
                        updateExtrudeModeButton(button, shape);
                        refresh();
    ...
    private void updateExtrudeModeButton(@NonNull
    final View button,
            @NonNull
            final Shape shape) {
        // visual names can be translated but the values should not be as they should always be in english
        final String[] extrudeModeNames = getResources()
                .getStringArray(R.array.circle_extrude_modes_names);
        final String[] extrudeModeValues = getResources()
                .getStringArray(R.array.circle_extrude_modes_values);
        final int s = indexOf(extrudeModeValues,
                shape.getMetaString("extrudeMode", extrudeModeValues[2]), 2);
        if (button instanceof TextView)
            ((TextView) button).setText(s < 0 ? "" : extrudeModeNames[s]);
        // Sub-classes may add support for other types of views
    }

    private void promptExtrudeMode(@NonNull
    final Shape shape,
            @Nullable
        ...
        AlertDialog.Builder b = new AlertDialog.Builder(getContext());
        b.setTitle(R.string.extrude_mode);
        b.setSingleChoiceItems(R.array.circle_extrude_modes_names,
                indexOf(extrudeModeValues,
                        shape.getMetaString("extrudeMode",
                                extrudeModeValues[2]),
                        2),
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog,
                        ...
                        if (extrudeModeIdx < 0)
                            return;
                        shape.setMetaString("extrudeMode",
                                extrudeModeValues[extrudeModeIdx]);
                        // NOTE: if sphere extrusion, the height and radius are sync'd
                        ...
                        if (!Double.isNaN(height) &&
                            ...
                            // sync the height to the current radius
                            _circle.setHeight(_circle.getRadius());
                        }
    ...
    private static <T> int indexOf(T[] arr, T val, int def) {
