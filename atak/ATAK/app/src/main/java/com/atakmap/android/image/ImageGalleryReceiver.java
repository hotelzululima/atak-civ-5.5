...
public class ImageGalleryReceiver extends DropDownReceiver implements
    ...
    public static final String UPDATE_ITEM = "com.atakmap.android.image.UPDATE_ITEM";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (extras == null) {
            Log.e(TAG, "malformed image gallery request: " + action);
            return;
        }
        ...
        switch (action) {
            ...
            //refresh an image given mapitem uid and new uri
            case UPDATE_ITEM: {
                String oldUri = extras.getString("oldUri", null);
                String newUri = extras.getString("newUri", null);
                String uid = extras.getString("uid", null);
                boolean show = extras.getBoolean("show", false);

                if (StringUtils.isEmpty(uid)) return;

                MapItem mi = mapView.getRootGroup().deepFindUID(uid);
                if (mi == null) return;

                File oldFile = new File(oldUri);
                File newFile = new File(newUri);

                if (adapter != null && adapter instanceof ImageGalleryFileAdapter) {
                    ImageGalleryFileAdapter igfa = (ImageGalleryFileAdapter) adapter;
                    GalleryFileItem newItem = new GalleryFileItem(mapView, newFile, mi);
                    int idx = igfa.updateGalleryItem(oldFile, newItem);
                    if (idx > -1 && show) {
                        //delay so the thread can run
                        mapView.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                igfa.displayImage(newItem);
                            }
                        }, 500);
                    }
                }
                    ...
                    for (String path : files) {
                        if (FileSystemUtils.isFile(path)) {
                            ...
                            if (ImageContainer.NITF_FilenameFilter.accept(
                                ...
                                File nitfXml = new File(f.getParent(),
                                        f.getName()
                                                + ".aux.xml");
    ...
    private void showGallery(List<File> files, List<String> uids,
        ...
        final MapItem mapItem = uid == null ? null
                : mapView.getRootGroup().deepFindUID(uid);
    ...
    private void showImportDialog() {
        ...
        d.addButton(R.drawable.nav_video, R.string.video);
    ...
    private void importLocalFile() {
        ...
        final AtakPreferences defaultPrefs = AtakPreferences.getInstance(
                mapView.getContext());
        ...
        public void onReceive(Context c, Intent intent) {
            ...
            if (PIC_CAPTURED.equals(intent.getAction())) {
                ...
                try {
                    //Attach the file
                    if (filePath == null) {
                        Log.w(TAG, "Cannot attach null file");
                        return;
                    }
            ...
            if (!PIC_CAPTURED.equals(intent.getAction())) {
                ...
                try {
                    if (filePath == null) {
                        Log.w(TAG, "Cannot import null file");
                        return;
                    }
                    File src = new File(
                            FileSystemUtils.validityScan(filePath));
