...
public class CotImporterManager extends AbstractCotEventImporter {
    ...
    public CotImporterManager(MapView mapView) {
        ...
        _importers = new CopyOnWriteArrayList<>();
        ...
        if (importer instanceof CotEventTypeImporter) {
            ...
            if (!types.isEmpty() && !typeImporter.isPrefixOnly()) {
                for (String type : types) {
                    List<CotEventTypeImporter> list = null;
                    synchronized (_typeImporters) {
                        // briefly lock around map access
                        list = _typeImporters
                                .get(type);
                        if (list == null) {
                            do {
                                list = _typeImporters.get(type);
                                if (list != null)
                                    break;
                                _typeImporters.put(type,
     ...
     * @param importer CoT importer
     */
    public void unregisterImporter(CotEventImporter importer) {
        if (importer instanceof CotEventTypeImporter) {
            CotEventTypeImporter typeImporter = (CotEventTypeImporter) importer;
            Set<String> types = typeImporter.getSupportedCotTypes();
            for (String type : types) {
                synchronized (_typeImporters) {
                    ...
                    if (list != null)
                        list.remove(importer);
                }

                // `_typeImporters` registration for type stays intact to
                // eliminate re-insert and its required lock acquisition
            }
        }
    ...
    public ImportResult importData(CotEvent event, Bundle extras) {
        ...
        List<? extends CotEventImporter> importers = null;
        synchronized (_typeImporters) {
            // synchronize typeImporters map use; importers Set is a Concurrent impl
            importers = _typeImporters.get(type);
        }
        if (importers == null)
