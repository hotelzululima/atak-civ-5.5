...
import com.atakmap.android.layers.overlay.FeatureCenterLabelListItem;
...
import com.atakmap.spatial.file.GeoJSONContentResolver;
...
public class WktMapComponent extends AbstractMapComponent implements
    ...
    private PersistentDataSourceFeatureDataStore3 spatialDb;
    ...
    private boolean labelsControlInitialized = false;
    ...
    private void initComponents(Context context, MapView view) {
        ...
        mapView.getRenderer3().addOnControlsChangedListener(this);
        ...
        onSharedPreferenceChanged(prefs.getSharedPrefs(),
                FeatureCenterLabelListItem.PREF_FILE_LABEL);
    }
    ...
    public void onSharedPreferenceChanged(SharedPreferences p, String key) {
        ...
        if (key.equals(FeatureCenterLabelListItem.PREF_FILE_LABEL)) {
            mapView.getRenderer3().visitControl(this.layer,
                    new Visitor<FeaturesLabelControl>() {
                        @Override
                        public void visit(FeaturesLabelControl c) {
                            syncLabelsControl(c);
                        }
    ...
    private synchronized void refreshPersistedComponents(MapView view) {
        ...
        view.post(new Runnable() {
            public void run() {
                ...
                if (detailsDropdownReceiver != null) {
                    AtakBroadcast.getInstance()
                            .unregisterReceiver(detailsDropdownReceiver);
                    ((FeaturesDetailsDropdownReceiver) detailsDropdownReceiver)
                            .dispose();
                    detailsDropdownReceiver = null;
                }
                if (featureEditDropdownReceiver != null) {
                    AtakBroadcast.getInstance()
                            .unregisterReceiver(featureEditDropdownReceiver);
                    ((FeaturesDetailsDropdownReceiver) featureEditDropdownReceiver)
                            .dispose();
                    featureEditDropdownReceiver = null;
                }
            }
        });
        ...
        this.spatialDb = new PersistentDataSourceFeatureDataStore3(
                SPATIAL_DB_DIR);
        ...
        // GeoJSON files
        addContentSource(context, new GeoJSONSpatialDb(this.spatialDb),
                new GeoJSONContentResolver(view, this.spatialDb), query);
        ...
        final FeatureDataStore2 fdb = spatialDb;
        view.post(new Runnable() {
            public void run() {
                detailsDropdownReceiver = new FeaturesDetailsDropdownReceiver(
                        view,
                        fdb);
                ...
                featureEditDropdownReceiver = new FeatureEditDropdownReceiver(
                        view,
                        fdb);
                ...
                AtakBroadcast.getInstance()
                        .registerReceiver(featureEditDropdownReceiver, f);
            }
        });
    ...
    private void backgroundInit(
            final PersistentDataSourceFeatureDataStore3 fdb) {
    ...
    public void onControlRegistered(Layer2 layer, MapControl ctrl) {
        if ((layer == this.layer) && (ctrl instanceof FeaturesLabelControl)) {
            syncLabelsControl((FeaturesLabelControl) ctrl);
        }
    }
    ...
    private void syncLabelsControl(FeaturesLabelControl ctrl) {
        boolean vis = AtakPreferences.getInstance(mapView.getContext())
                .get(FeatureCenterLabelListItem.PREF_FILE_LABEL, false);
        ctrl.setShapeCenterMarkersVisible(vis);
        // set up the defaults
        if (!labelsControlInitialized) {
            labelsControlInitialized = true;

            // values adopted from `GLMarkerFeature`
            ctrl.setDefaultLabelBackground(
                    Color.argb(153 /*=60%*/, 0, 0, 0),
                    true);
            ctrl.setDefaultLabelLevelOfDetail(
                    OSMUtils.mapnikTileLevel(
    ...
    static void refresh(PersistentDataSourceFeatureDataStore3 fdb) {
        try {
            fdb.acquireModifyLock(true);
            try {
                try (DataSourceDataStoreControl.FileCursor result = fdb
                        .queryFiles()) {
    ...
    private class WktImporter implements Importer {
        ...
        public ImportResult importData(Uri uri, String mime, Bundle bundle)
            ...
            synchronized (WktMapComponent.this) {
                try {
                    WktMapComponent.this.spatialDb.acquireModifyLock(true);
                    try {
                        retval = this.source.importData(uri, mime, bundle);
                    } finally {
                        WktMapComponent.this.spatialDb.releaseModifyLock();
                    }
                } catch (InterruptedException ignored) {
                }
            }
    ...
    ClearContentRegistry.ClearContentListener dataMgmtReceiver = new ClearContentRegistry.ClearContentListener() {
        ...
        public void onClearContent(boolean clearmaps) {
            ...
            DataSourceDataStoreControl.FileCursor result = null;
            try {
    ...
    // loosely based on https://booster.johnsonlee.io/en/guide/performance/webview-preloading.html#the-problems-of-webview
    private static void preloadWebView() {
        preloadWebViewExecutor.execute(new Runnable() {
            @Override
            public void run() {
                ...
                try {
                    ...
                    method.invoke(null);
                    Log.d(TAG, "preloaded webview in: "
    ...
    private static final Executor preloadWebViewExecutor = Executors
            .newSingleThreadExecutor(new NamedThreadFactory("preloadWebview"));
