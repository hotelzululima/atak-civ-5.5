 ...
 * Largely unmodified copy of https://github.com/justasm/DragLinearLayout
 */
...
public class DragLinearLayout extends LinearLayout {
    ...
    public interface OnViewSwapListener {
         ...
         */
        void onSwap(View firstView, int firstPosition, View secondView,
                int secondPosition);
    }
    ...
    private class DragItem {
        ...
        public synchronized void startDetectingOnPossibleDrag(final View view,
                final int position) {
            this.view = view;
        ...
        public synchronized void onDragStart() {
            if (view == null)
                return;
        ...
        public synchronized void setTotalOffset(int offset) {
            totalDragOffset = offset;
            updateTargetTop();
        }

        public synchronized void updateTargetTop() {
            if (view == null)
                return;
            ...
            if (getOrientation() == VERTICAL)
                ...
                targetTopOffset = startLeft - view.getLeft() + totalDragOffset;
        }

        public synchronized void onDragStop() {
            this.dragging = false;
        }

        public synchronized boolean settling() {
    ...
    public DragLinearLayout(Context context, AttributeSet attrs) {
        ...
        dragTopShadowDrawable = ContextCompat.getDrawable(context,
                R.drawable.ab_solid_shadow_holo_flipped);
        dragBottomShadowDrawable = ContextCompat.getDrawable(context,
                R.drawable.ab_solid_shadow_holo);
        dragShadowHeight = resources
                .getDimensionPixelSize(R.dimen.downwards_drop_shadow_height);

        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.DragLinearLayout, 0, 0);
        try {
            scrollSensitiveAreaHeight = a
                    .getDimensionPixelSize(
                            R.styleable.DragLinearLayout_scrollSensitiveHeight,
                            (int) (DEFAULT_SCROLL_SENSITIVE_AREA_HEIGHT_DP
                                    * resources.getDisplayMetrics().density
                                    + 0.5f));
            scrollSensitiveAreaWidth = a
                    .getDimensionPixelSize(
                            R.styleable.DragLinearLayout_scrollSensitiveWidth,
                            (int) (DEFAULT_SCROLL_SENSITIVE_AREA_HEIGHT_DP
                                    * resources.getDisplayMetrics().density
                                    + 0.5f));
        } finally {
        ...
        nominalDistanceScaled = (int) (NOMINAL_DISTANCE
    ...
    /**
     * Sets the width from upper / lower edge at which a container {@link android.widget.ScrollView},
     * if one is registered via {@link #setContainerScrollView(android.widget.ScrollView)},
     * is scrolled.
     */
    @SuppressWarnings("UnusedDeclaration")
    public void setScrollSensitiveWidth(int width) {
        this.scrollSensitiveAreaWidth = width;
    }
    ...
    private long getTranslateAnimationDuration(float distance) {
        return Math.min(MAX_SWITCH_DURATION, Math.max(MIN_SWITCH_DURATION,
                (long) (NOMINAL_SWITCH_DURATION * Math.abs(distance)
    ...
    private void onDragStop() {
        draggedItem.settleAnimation = ValueAnimator
                .ofFloat(draggedItem.totalDragOffset,
                        draggedItem.totalDragOffset
        ...
        draggedItem.settleAnimation
                .addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(
                        ...
                        draggedItem.setTotalOffset(
                                ((Float) animation.getAnimatedValue())
                                        .intValue());

                        final int shadowAlpha = (int) ((1
                                - animation.getAnimatedFraction()) * 255);
                        if (null != dragTopShadowDrawable)
                            dragTopShadowDrawable.setAlpha(shadowAlpha);
                        if (null != dragBottomShadowDrawable)
    ...
    private void onDrag(final int offset) {
        ...
        int currentTop;
        if (getOrientation() == VERTICAL)
            currentTop = draggedItem.startTop + draggedItem.totalDragOffset;
        else
            currentTop = draggedItem.startLeft + draggedItem.totalDragOffset;
        ...
        if (getOrientation() == VERTICAL)
            isBelow = (belowView != null) &&
                    (currentTop + draggedItem.height > belowView.getTop()
                            + belowView.getHeight() / 2);
        else
            isBelow = (belowView != null) &&
                    (currentTop + draggedItem.width > belowView.getLeft()
                            + belowView.getWidth() / 2);

        if (getOrientation() == VERTICAL)
            isAbove = (aboveView != null) &&
                    (currentTop < aboveView.getTop()
                            + aboveView.getHeight() / 2);
        else
            isAbove = (aboveView != null) &&
                    (currentTop < aboveView.getLeft()
                            + aboveView.getWidth() / 2);
        ...
        if (isBelow || isAbove) {
            ...
            final float switchViewStartY = getOrientation() == VERTICAL
                    ? switchView.getY()
                    : switchView.getX();

            if (null != swapListener) {
                swapListener.onSwap(draggedItem.view, draggedItem.position,
                        switchView, switchPosition);
            }
            ...
            final ViewTreeObserver switchViewObserver = switchView
                    .getViewTreeObserver();
            switchViewObserver.addOnPreDrawListener(new OnPreDrawListener() {
                ...
                public boolean onPreDraw() {
                    ...
                    final int switchViewTop;
                    if (getOrientation() == VERTICAL) {
                        switchViewTop = switchView.getTop();
                        propertyName = "y";
                    } else {
                        switchViewTop = switchView.getLeft();
                        propertyName = "x";
                    }

                    final ObjectAnimator switchAnimator = ObjectAnimator
                            .ofFloat(switchView, propertyName,
                                    switchViewStartY, switchViewTop)
                            .setDuration(getTranslateAnimationDuration(
                                    switchViewTop - switchViewStartY));
                    switchAnimator.addListener(new AnimatorListenerAdapter() {
                        @Override
                        public void onAnimationStart(Animator animation) {
                            draggableChildren.get(
                                    originalPosition).swapAnimation = switchAnimator;
                        }

                        @Override
                        public void onAnimationEnd(Animator animation) {
                            draggableChildren
                                    .get(originalPosition).swapAnimation = null;
                        }
                    });
            ...
            final ViewTreeObserver observer = draggedItem.view
                    .getViewTreeObserver();
        ...
        if (null != containerScrollView) {
            if (getOrientation() == VERTICAL) {
                ...
                if (absTop < scrollSensitiveAreaHeight) {
                    delta = (int) (-MAX_DRAG_SCROLL_SPEED
                            * smootherStep(scrollSensitiveAreaHeight, 0,
                                    absTop));
                } else if (absTop > height - scrollSensitiveAreaHeight) {
                    delta = (int) (MAX_DRAG_SCROLL_SPEED * smootherStep(
                            height - scrollSensitiveAreaHeight, height,
                            absTop));
                } else {
                    delta = 0;
                }
                ...
                dragUpdater = new Runnable() {
                    ...
                    public void run() {
                        if (draggedItem.dragging
                                && startScrollY != containerScrollView
                                        .getScrollY()) {
                ...
                final int absTop = getWidth() - startScrollX + currentTop;
                final int width = containerScrollView.getWidth();

                final int delta;

                if (absTop < scrollSensitiveAreaWidth) {
                    delta = (int) (-MAX_DRAG_SCROLL_SPEED
                            * smootherStep(scrollSensitiveAreaWidth, 0,
                                    absTop));
                } else if (absTop > width - scrollSensitiveAreaWidth) {
                    delta = (int) (MAX_DRAG_SCROLL_SPEED * smootherStep(
                            width - scrollSensitiveAreaWidth, width, absTop));
                } else {
                    delta = 0;
                }
                ...
                dragUpdater = new Runnable() {
                    ...
                    public void run() {
                        if (draggedItem.dragging
                                && startScrollX != containerScrollView
                                        .getScrollX()) {
                            onDrag(draggedItem.totalDragOffset + delta);
                        }
                    }
    ...
    protected void dispatchDraw(@NonNull Canvas canvas) {
        ...
        if (draggedItem.detecting
                && (draggedItem.dragging || draggedItem.settling())) {
            canvas.save();
            if (getOrientation() == VERTICAL)
            ...
            dragBottomShadowDrawable.setBounds(left, bottom, right,
                    bottom + dragShadowHeight);
            dragBottomShadowDrawable.draw(canvas);

            if (null != dragTopShadowDrawable) {
                dragTopShadowDrawable.setBounds(left, top - dragShadowHeight,
                        right, top);
    ...
    public boolean onInterceptTouchEvent(final MotionEvent event) {
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                downY = (getOrientation() == VERTICAL) ? (int) event.getY(0)
                        : (int) event.getX(0);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                final float y = (getOrientation() == VERTICAL)
                        ? event.getY(pointerIndex)
                        : event.getX(pointerIndex);
                final float dy = y - downY;
                if (Math.abs(dy) > slop / 2f) {
                    startDrag();
                    return true;
                }
                return false;
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                int lastEventY = (getOrientation() == VERTICAL)
    ...
    private BitmapDrawable getDragDrawable(View view) {
        ...
        drawable.setBounds(new Rect(left, top, left + view.getWidth(),
                top + view.getHeight()));
