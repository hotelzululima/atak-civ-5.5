...
public class ToolbarBroadcastReceiver extends BroadcastReceiver
    ...
    protected final HashMap<String, IToolbarExtension2> toolbars = new HashMap<>();
    ...
    @Override
    public void dispose() {
        for (IToolbarExtension2 toolbarComponent : toolbars.values()) {
    ...
    public void registerToolbarComponent(String toolbarIdentifier,
            IToolbarExtension2 toolbarComponent) {
    ...
    public void unregisterToolbarComponent(String toolbarIdentifier) {
        IToolbarExtension2 toolbarComponent = toolbars
                .remove(toolbarIdentifier);
    ...
    protected void setToolbar(String toolbarIdentifier) {
        ...
        IToolbarExtension2 ext = toolbars.get(toolbarIdentifier);
        IToolbarExtension2 active = getActiveToolbar();
    ...
    protected IToolbarExtension2 getActiveToolbar() {
        if (activeToolbar == null || activeToolbar.contentEquals("")) {
            return null;
        } else {
            IToolbarExtension2 ite = toolbars.get(activeToolbar);
            if (ite != null)
    ...
    private void closeToolbar(IToolbarExtension2 ext) {
        ActionBarView abv = ext.getToolbarView();
        if (abv == null)
            return;
    ...
    private void openToolbar(String toolbar) {
        IToolbarExtension2 tbInterface = toolbars.get(toolbar);
        ...
        switch (abv.getPosition()) {
            case ActionBarView.TOP_LEFT: {
                ...
                if (childView == null) {
                    ...
                    childView = new NavButtonChildView(context, icon);
                }
                final View anchor = takButton != null ? takButton : menuButton;
                childView.layoutForToolbarView(anchor, abv);

                childView.setScaleX(NavView.getInstance().getUserIconScale());
                childView.setScaleY(NavView.getInstance().getUserIconScale());
