...
public class VehicleMapComponent extends AbstractMapComponent {
    ...
    // introduced to limit the of spam during map movement where accuracy is not as crucial
    private final LimitingThread lt = new LimitingThread(
            "vehicle-display-thread", new Runnable() {
                @Override
                public void run() {
                    double res = _mapView.getMapResolution();
