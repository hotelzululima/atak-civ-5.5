...
public class DragWidgetHelper implements
    ...
    public PointF getUpperLeft() {
        float ulxp = prefs.get(preferenceKey + ".upperLeftX.portrait",
                mw.getPointX());
        float ulyp = prefs.get(preferenceKey + ".upperLeftY.portrait",
                mw.getPointY());

        float ulxl = prefs.get(preferenceKey + ".upperLeftX.landscape",
                mw.getPointX());
        float ulyl = prefs.get(preferenceKey + ".upperLeftY.landscape",
                mw.getPointY());
    ...
    public boolean onMapWidgetMove(MapWidget widget, MotionEvent event) {
        ...
        if (dragging) {
            ...
            if (mapView.isPortrait()) {
                prefs.set(preferenceKey + ".upperLeftX.portrait", x);
                prefs.set(preferenceKey + ".upperLeftY.portrait", y);
            } else {
                prefs.set(preferenceKey + ".upperLeftX.landscape", x);
                prefs.set(preferenceKey + ".upperLeftY.landscape", y);
            }
