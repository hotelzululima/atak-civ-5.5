...
import com.atakmap.android.fires.SearchHelper;
...
public class DangerCloseCalculator implements Runnable,
    ...
    private static final int MAX_CLOSEST_FRIENDLY = 92600;
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
        ...
        if (key.equals("expandedDangerClose")) {
            expandedDangerClose = _prefs.get(key, false);
        }
    }
    ...
    public static synchronized List<DangerCloseAlert> find(
        ...
        // find all map items within the specified range
        final Collection<MapItem> items = SearchHelper.deepFindItems(mapView,
                target.getPoint(), range + MAX_CLOSEST_FRIENDLY);

        // check to see if the resulting map items are of a specific type and determine if they
        // are considered closest friendly
        for (MapItem item : items) {
            if (item instanceof PointMapItem) {
                PointMapItem mapItem = (PointMapItem) item;
                ...
                if (localCustomFilter != null) {
                    ...
                    isCorrectTypePrefix &= airDefense == type
                            .startsWith("a-f-A");
                    ...
                    if (isCorrectTypePrefix) {
                        ...
                        if (d <= range) {
                            ...
                            final double b = target.getPoint().bearingTo(
                                    mapItem.getPoint());
                            list.add(new DangerCloseAlert(target, mapItem, d,
                                    b));
                        }
