...
public class StateSaver extends AbstractMapComponent {
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        vacuumThread.execute(new Runnable() {
            @Override
            public void run() {
                synchronized (dbWriteLock) {
    ...
    private void checkStateSaverDatabaseNoSync() {
        if (stateSaverDatabase == null) {
            ...
            stateSaverDatabase = ATAKDatabaseHelper
                    .getPrefetchDb(STATE_SAVER_DB_FILE2);
            if (stateSaverDatabase != null) {
    ...
    private final ExecutorService vacuumThread = Executors
            .newSingleThreadExecutor(new NamedThreadFactory("vacuumThread"));
