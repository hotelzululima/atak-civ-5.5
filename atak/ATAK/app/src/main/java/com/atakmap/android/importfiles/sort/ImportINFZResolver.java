 ...
 */
public class ImportINFZResolver extends gov.tak.api.importfiles.ImportResolver {

    private static final String TAG = "ImportINFZSort";

    private final static String INFMATCH = ",";

    public ImportINFZResolver(Context context) {
        super(".infz",
                FileSystemUtils.getItem(FileSystemProductProvider.LOCAL_REPO_PATH),
                context.getString(R.string.app_mgmt_product_repo),
                ResUtils.getDrawable(context, R.drawable.ic_menu_plugins));
    }
     ...
     *
     * @param zip the zip file examined for the product.inf
