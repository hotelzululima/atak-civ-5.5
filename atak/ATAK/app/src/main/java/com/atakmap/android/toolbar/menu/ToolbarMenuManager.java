...
public class ToolbarMenuManager extends BroadcastReceiver {
    ...
    public void register(IMenu parent, MenuButton menu) {
        if (registeredMenuButtons.get(parent) == null) {
            registeredMenuButtons.put(parent, new ArrayList<>());
        }
        List<MenuButton> menuButtons = registeredMenuButtons.get(parent);
        if (menuButtons != null)
            menuButtons.add(menu);
    }

    public void showMenu(String identifier, int x, int y) {
        Pair<PopupWindow, IMenu> menuPair = registeredMenus.get(identifier);
        if (menuPair == null)
            return;

        PopupWindow newActivePopUpWindow = menuPair.first;
        ...
        if (menuPair.second.getPreferredWidth() == -1)
            this.activePopUpWindow
                    .setWidth(menuPair.second.getView()
                            .getLayoutParams().width);
        else
            this.activePopUpWindow
                    .setWidth(menuPair.second
                            .getPreferredWidth());

        if (menuPair.second.getPreferredHeight() == -1)
            this.activePopUpWindow
                    .setHeight(menuPair.second.getView()
                            .getLayoutParams().height);
        else
            this.activePopUpWindow
                    .setHeight(menuPair.second
                            .getPreferredHeight());
