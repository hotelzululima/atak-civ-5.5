...
public class SpiButtonTool extends SpecialPointButtonTool implements
     ...
     * @param index the index of the spi in use {1, 2, 3}
     */
    ...
    public SpiButtonTool(MapView mapView, ImageButton button, int index) {
        ...
        _prefs = AtakPreferences.getInstance(_mapView
                .getContext());
        _prefs.registerListener(_prefsChangedListener);
        updateTimeout();
    }
    ...
    private void updateTimeout() {
        try {
            _updateTimeout = Integer
                    .parseInt(_prefs.get("spiUpdateDelay", "5"));
        } catch (Exception e) {
            _updateTimeout = 0;
        }
        if (_updateTimeout <= 0) {
            _updateTimeout = 5;
            _prefs.set("spiUpdateDelay", "5");
    ...
    private String generateName() {
        final String callsign = _mapView.getSelfMarker().getMetaString(
                "callsign",
                null);
        return callsign + "."
    ...
    private final BroadcastReceiver br = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (uid != null && SPI_OFF.equals(action)
                    && uid.equals(_marker.getUID())) {
                off();
            }
        }
    };
