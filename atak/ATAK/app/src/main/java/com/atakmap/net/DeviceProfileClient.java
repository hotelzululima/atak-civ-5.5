 ...
 */
public class DeviceProfileClient implements Disposable {
    ...
    /**
     * Maintain ref to callbacks so they do not get garbage collected prior to processing
     */
    ...
    public boolean callbackReceived(DeviceProfileRequest request,
            boolean bSuccess) {
        ...
        if (ret != null) {
            Log.d(TAG, "callbackReceived: " + request);
            return true;
        } else {
            Log.d(TAG, "callbackReceived not found: " + request);
            return false;
        }
    }
    ...
    public boolean getProfile(Context context, final String server,
        ...
        return getProfile(context, new DeviceProfileRequest(
                server, serverConnectString, username, password, onEnrollment,
                onConnection,
     ...
     * @param serverConnectString the server connect string used for the certificates
     * @param tool the specific tool
     * @param syncSecago    -1 to get all profile data. Positive value to get profile data published
     *                      within this amount of time
     * @param callback
     * @return true if request sent
    ...
    public boolean getProfile(Context context, final String server,
            ...
            DeviceProfileCallback callback) {
        return getProfile(context, server, serverConnectString, null, null,
                tool, null, syncSecago,
                true, null, callback);
    }
    ...
    public boolean getProfile(Context context, final String server,
        ...
        return getProfile(context, new DeviceProfileRequest(
                server, serverConnectString, username, password, tool, filename,
                null,
     ...
     * @return boolean if the call is successful
     */
    public boolean getProfile(Context context, final String server,
            final String serverConnectString,
            String username, String password,
     ...
     * @param password the password
     * @param tool the specific tool
     * @param filenames the files to be obtained
    ...
    public boolean getProfile(Context context, final String server,
        ...
        return getProfile(context, new DeviceProfileRequest(
                server, serverConnectString, username, password, tool,
                filenames, ifModifiedSince,
     ...
     * @param serverConnectString the connect string to be used
     * @param tool the tool to retrieve the profile for
     */
    public boolean getProfile(Context context, final String server,
            final String serverConnectString, final String tool) {
    ...
    public boolean getProfile(Context context, final String tool) {
        ...
        final TAKServer[] servers = inst.getServers();
        ...
        for (TAKServer server : servers) {
            ...
            getProfile(context, ncs.getHost(), ncs.toString(), tool);
            bRet = true;
        }
