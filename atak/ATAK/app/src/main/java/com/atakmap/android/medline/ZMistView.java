...
public class ZMistView extends LinearLayout {
    ...
    private void setUpClickListeners() {
        ...
        addButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                zmist.loadData((AttributeSet) null);
                zmist.saveData();
        ...
        if (_marker != null) {
            ...
            if (zMistsAttrs != null) {
                List<String> keys = new ArrayList<>(
                        zMistsAttrs.getAttributeNames());
                //need to sort the keys
                Collections.sort(keys);
                for (String key : keys) {
                    AttributeSet zMistData = zMistsAttrs
    ...
    private void remove(int id) {
        ...
        zMistsAttrs.removeAttribute(title);
        //decrement any of the id numbers after the removed item
        while (id <= zMistsAttrs.getAttributeNames().size()) {
            id++;
            String oldTitle = _context.getString(R.string.zmist) + id;
            if (zMistsAttrs.containsAttribute(oldTitle)) {
                AttributeSet zMist = zMistsAttrs
                        .getAttributeSetAttribute(oldTitle);
                if (zMist != null) {
                    int newId = id - 1;
                    String newTitle = _context.getString(R.string.zmist)
                            + newId;
                    zMistsAttrs.setAttribute(newTitle, zMist);
                    zMistsAttrs.removeAttribute(oldTitle);
                }
            }
        }
