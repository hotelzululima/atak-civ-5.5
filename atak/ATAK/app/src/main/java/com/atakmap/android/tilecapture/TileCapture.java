...
public class TileCapture extends DatasetTileReader {
    ...
    public static TileCapture create(ImageDatasetDescriptor info) {
        ...
        try {
            ...
            if (r instanceof MobileTileReader) {
                final TileMatrix tiles = ((MobileTileReader) r).getTiles();
                ...
                Point x0 = TileMatrix.Util.getTileIndex(tiles, zoom0.level,
                        bounds.minX, (bounds.minY + bounds.maxY) / 2d);
                Point x1 = TileMatrix.Util.getTileIndex(tiles, zoom0.level,
                        bounds.maxX - zoom0.pixelSizeX,
                        (bounds.minY + bounds.maxY) / 2d);
                Point y0 = TileMatrix.Util.getTileIndex(tiles, zoom0.level,
                        (bounds.minX + bounds.maxX) / 2d, bounds.maxY);
                Point y1 = TileMatrix.Util.getTileIndex(tiles, zoom0.level,
                        (bounds.minX + bounds.maxX) / 2d,
                        bounds.minY + zoom0.pixelSizeY);

                final int ntx = (x1.x - x0.x) + 1;
                final int nty = (y1.y - y0.y) + 1;

                final long width = ((long) ntx
                        * zoom0.tileWidth) << (long) (zoomLevels[zoomLevels.length
                                - 1].level - zoom0.level);
                final long height = ((long) nty
                        * zoom0.tileHeight) << (long) (zoomLevels[zoomLevels.length
                                - 1].level - zoom0.level);

                tc._imprecise = new DefaultDatasetProjection2(
                        srid, width, height,
                        ul, ur, lr, ll);
                tc._gsd = zoomLevels[zoomLevels.length - 1].resolution;
            } else {
    ...
    public static DatasetTileReader createTileReader(
        ...
        try {
            // disable proxying in favor of pull-through caching
            final TileClientSpi.Options clientOpts = new TileClientSpi.Options();
            clientOpts.proxy = false;

            // XXX - should use `TileContainer` for offline case, but
            //       downstream usage is resulting in bad level (and tile
            //       index) calculations. Continue using legacy mechanism until
            //       adequate time can be invested in a thorough review and
            //       rewrite

            // try to create a tile matrix
            final TileMatrix tiles = info.isRemote() ? TileClientFactory.create(
                    info.getUri(),
                    info.getExtraData("offlineCache"),
                    clientOpts) : null;
            if (tiles != null) {
                return new MobileTileReader(tiles);
