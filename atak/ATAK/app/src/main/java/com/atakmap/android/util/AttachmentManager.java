...
public class AttachmentManager {
    ...
    private static final Map<String, List<File>> attachmentCache = new ConcurrentHashMap<>();
        ...
        fObserver = new FileObserver(
            ...
            public void onEvent(int event, String path) {
                if (event == FileObserver.CREATE
                    ...
                    notifyAttachmentChange(mapItem.getUID());
     ...
     * @return Map of map items with attachments and their associated files
     */
    public static Map<MapItem, File[]> findAttachmentItems2(
            MapGroup rootGroup) {
        Map<MapItem, File[]> ret = new HashMap<>();
        ...
        for (File f : subDirs) {
            List<File> files = getAttachments(f.getName());
            MapItem item = rootGroup.deepFindUID(f.getName());
            if (!files.isEmpty() && item != null) {
                ret.put(item, files.toArray(new File[0]));
            }
        }
        return ret;
    }
     ...
     * @return list of file attachments which can be manipulated
     */
    public static List<File> getAttachments(final String uid) {
        if (uid == null)
            return new ArrayList<>();

        List<File> ret = attachmentCache.get(uid);
        if (ret == null) {
            ret = new ArrayList<>();
            ...
            attachmentCache.put(uid, ret);
        }
        return new ArrayList<>(ret);
    }

    /**
     * When changing the attachments associated with a map item, it is important to
     * notify that the attachments have changed for a specific map item.
     * @param uid the uid for the map item where an attachment has changed.
     */
    public static void notifyAttachmentChange(String uid) {
        attachmentCache.remove(uid);
    }
    ...
    public static void viewItem(MapView mapView, MapItem mapItem,
        ...
        if (ImageDropDownReceiver.ImageFileFilter.accept(file.getParentFile(),
            ...
            for (String supportedExt : ImportFilesTask
