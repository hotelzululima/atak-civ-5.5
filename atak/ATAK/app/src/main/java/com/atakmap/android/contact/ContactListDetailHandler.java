...
public class ContactListDetailHandler
    ...
    public void toMarkerMetadata(final Marker marker, final CotEvent event,
        ...
        String altphoneNumber = detail.getAttribute("altphone");

        if (phoneNumber == null) {
            // only the alternative is specified
        ...
        if (!FileSystemUtils.isEmpty(altphoneNumber)) {
            //phoneNumber = altphoneNumber;
            marker.setMetaString("altphoneNumber", altphoneNumber);
        } else {
            marker.removeMetaData("altphoneNumber");
        }
        ...
        if (group != null) {
            ...
            //mark as non sendable so details send submenu is hidden since this is a contact a-f marker
            marker.setMetaBoolean("sendable", false);
        } else {
        ...
        if (!FileSystemUtils.isEmpty(connectionEndpoint) && callsign != null) {
            ...
            if (existingContact == null && ncs != null) {
                ...
                if (existingContact instanceof IndividualContact) {
                    ...
                    // only called when the team is changed
                    if (team != null
                        ...
                        existingContact.setExtras(extras);
                        contacts.contactUpdated(existingContact);
                    }
            ...
            if (existingContact instanceof IndividualContact) {
                ...
                if (!FileSystemUtils.isEmpty(altphoneNumber)) {
                    castedContact.addConnector(new AltTelephoneConnector(
                            altphoneNumber));
                } else {
                    castedContact
                            .removeConnector(
                                    AltTelephoneConnector.CONNECTOR_TYPE);
                }
