...
public abstract class AbstractHierarchyListItem2 implements
    ...
    public void sortItems(List<HierarchyListItem> items) {
        ...
        if (sort instanceof ComparatorSort) {
            ...
            try {
                ...
                // if the HierarchyListItems are being updated during the sort, this could cause
                // a failure during the sorting.   Protect against it and continue on until the
                // next refresh occurs which will leave the list in a partially sorted state which
                // would be transient.
            }
        }
    }
    ...
    /** @deprecated use {@link #buildQueryParams2(HierarchyListFilter)} */
    @Deprecated
    ...
    protected static FeatureDataStore.FeatureQueryParameters buildQueryParams(
            HierarchyListFilter filter) {
        return Adapters.adapt(buildQueryParams2(filter), null);
    }

    protected static FeatureDataStore2.FeatureQueryParameters buildQueryParams2(
            HierarchyListFilter filter) {
        ...
        if (filter != null) {
            ...
            params.spatialFilter = GeometryFactory.fromEnvelope(new Envelope(
                    fov.westBound, fov.southBound, 0d,
                    fov.eastBound, fov.northBound, 0d));
