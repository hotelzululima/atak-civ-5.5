...
public class ContourLinesOverlay extends AbstractHierarchyListItem
    ...
    //constant strings
    private final String isMajorLineKey = "isMajorLine";

    private boolean visible; //this is the boolean flag to determine if we are in range to generate contours or not
    private String workingPath;
    private PersistentDataSourceFeatureDataStore3 contourDataStore;
    ...
    private boolean _linesVisible = true; //tracker for if the lines are visible or not

    public ContourLinesOverlay(MapView mapView) {
        this.mapView = mapView;
        prefs = AtakPreferences.getInstance(
                mapView.getContext());
        ...
        if (files != null) {
            synchronized (this) {
                if (contourDataStore == null)
                    contourDataStore = new PersistentDataSourceFeatureDataStore3(
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                if (cursor.getType().toLowerCase(LocaleUtil.getCurrent())
                    ...
                    try {
                        //DTED'#'
                        cell.dtedRank = Character.getNumericValue(cursor
                                .getType()
                        ...
                        final List<DtedCell> l = cells.get(cellKey);
                        if (l != null)
                            l.add(cell);
                    } catch (Exception ex) {
                        // ignored
    ...
    private void AddDataStoreFiles() {
        ...
        try (DataSourceDataStoreControl.FileCursor result = contourDataStore
                .queryFiles()) {
            try {
                contourDataStore.acquireModifyLock(true);
                try {
                    ...
                    contourDataStore.releaseModifyLock();
                }
    ...
    private void buildContours() {
        SpatialCalculator calculator = null;
        ...
        try {
            ...
            params.spatialFilter = GeometryFactory.fromEnvelope(new Envelope(
                    ul.getLongitude(), lr.getLatitude(), 0d,
                    lr.getLongitude(), ul.getLatitude(), 0d));
            ...
            double conversionFactor = ((prefs
                    .get(CONTOUR_PREFERENCE_UNIT_KEY, "m")
                    .equals("m")) ? 1
                ...
                if (set.getAttributes() != null) {
                    if (set.getAttributes()
                        ...
                        if (set.getGeometry() instanceof LineString) {
                            ...
                            polyLine.setMetaBoolean("nevercot", true);
                            ...
                            if (lineString.getNumPoints() > 1) {
                                ...
                                for (int i = 0; i < lineString
                                        .getNumPoints(); i++) {
                                    pts.add(new GeoPoint(lineString.getY(i),
                                            lineString.getX(i), ele));
                                }
                            }
                            ...
                            if (simplified != null && simplified.size() > 2) {
                                polyLine.setPoints(
                                        simplified.toArray(new GeoPoint[0]));
                            } else {
                                continue;
                            }
                            polyLine.setStrokeColor(
                                    prefs.get(CONTOUR_PREFERENCE_LINE_COLOR_KEY,
                                            Color.WHITE));
                            polyLine.setClickable(false);
                            polyLine.setEditable(false);
                            polyLine.setMovable(false);

                            //check if line is marked as major or minor and store a key for it in its metadata to be evaluated later on
                            if (checkMajorLine(ele * conversionFactor)) {
                                //store whether or not this line is a minor or major
                                polyLine.setMetaBoolean(isMajorLineKey, true);
                                polyLine.setStrokeWeight(strokeWidth);
                            } else {
                                //store whether or not this line is a minor or major
                                polyLine.setMetaBoolean(isMajorLineKey, false);
                            ...
                            polyLine.setLineLabel((int) Math.round(
                                    ...
                                    + " "
                                    + prefs.get(CONTOUR_PREFERENCE_UNIT_KEY,
                                            "m"));
                            polyLine.setTitle(
                                    "Contour Line " + polyLine.getLineLabel());
                            polyLine.toggleMetaData("labels_on", true);
            ...
            //hide/reshow the checkboxes for filtering our minor lines
            //depending on the interval and if we actually got ay minor lines we need to update it here regardless
            updateMinorCb(showingMinor);
        }
    }

    private synchronized void setContourVisible(boolean isVisible) {
        for (MapItem mapItem : getMapGroup().getItems()) {
            Polyline p = (Polyline) mapItem;
            p.toggleMetaData("labels_on", isVisible);
            if (!p.getMetaBoolean(isMajorLineKey, true)) {
                //set visibility of minor marked lines if the lines are visible and the minor lines checkbox state is true
                p.setVisible(isVisible && prefs
                        .get(CONTOUR_PREFERENCE_MINOR_VISIBLE_KEY, true));
            } else {
    ...
    private synchronized void setMinorLineVisibility() {
        for (MapItem mapItem : getMapGroup().getItems()) {
            Polyline p = (Polyline) mapItem;
            if (!p.getMetaBoolean(isMajorLineKey, true)) {
                //set visibility of minor marked lines if the lines are visible and the minor lines checkbox state is true
                p.setVisible(_linesVisible && prefs
                        .get(CONTOUR_PREFERENCE_MINOR_VISIBLE_KEY, true));
            }
        }
    }
    ...
    private synchronized void updateLabelUnits() {
        for (MapItem mapItem : getMapGroup().getItems()) {
            Polyline p = (Polyline) mapItem;
            String newUnit = prefs.get(CONTOUR_PREFERENCE_UNIT_KEY, "m");
    ...
    private synchronized void updatePolylineWidth() {
        for (MapItem mapItem : getMapGroup().getItems()) {
            int newWidth = prefs.get(
                    ContourLinesOverlay.CONTOUR_PREFERENCE_MAJOR_WIDTH_KEY, 4);
            Polyline p = (Polyline) mapItem;
            boolean isMajorLine = p.getMetaBoolean(isMajorLineKey, true);
            if (isMajorLine) {
    ...
    /**
     * disposal call which will skip clearing the polylines
     */
    @Override
    public void dispose() {
        setCancelled();
    ...
    private synchronized void clearLines() {
        final MapGroup mg = getMapGroup();
        if (mg == null)
            return;
        for (MapItem mapItem : mg.getItems())
            mapItem.removeFromGroup();
    }
    ...
    // introduced to limit the of spam during map movement.
    private final LimitingThread lt = new LimitingThread(
            "contour-line-visibility", new Runnable() {
                private boolean zoomPrevState;

                @Override
                public void run() {
                    boolean zoom = checkZoom();
                    if (zoomPrevState != zoom) {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
        ...
        switch (key) {
            ...
            case CONTOUR_PREFERENCE_CONTOUR_VISIBLE_KEY:
                _linesVisible = sharedPreferences.getBoolean(key, true);
                setContourVisible(_linesVisible);
                break;
            case CONTOUR_PREFERENCE_MINOR_VISIBLE_KEY:
                setMinorLineVisibility();
                break;
    ...
    class ContourRunnable implements Runnable {
        ...
        @Override
        public void run() {
            double interval = prefs.get(CONTOUR_PREFERENCE_INTERVAL_KEY, 20);
            if (prefs.get(CONTOUR_PREFERENCE_UNIT_KEY, "m")
                    .equalsIgnoreCase("ft"))
