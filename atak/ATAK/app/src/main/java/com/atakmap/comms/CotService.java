...
public class CotService implements OnSharedPreferenceChangeListener,
    ...
    public CotService(final Context context) {
        ...
        try {
            multicastTTL = Integer.parseInt(prefs.get("multicastTTL",
                    "64"));
        ...
        try {
            udpNoDataTimeout = Integer.parseInt(prefs.get(
                    "udpNoDataTimeout",
                    "30"));
        } catch (NumberFormatException e) {
            udpNoDataTimeout = 30;
        ...
        try {
            tcpConnectTimeout = Integer.parseInt(prefs.get(
        ...
        if (!_loadSavedInputs()
                && prefs.get("createDefaultCoTInputs", true)) {
            _addDefaultInputs();
        }
    ...
    private boolean _loadSavedInputs() {
        ...
        synchronized (propertyFileLock) {
            ...
            for (File config : inputs) {
                ...
                String connectString = props
                        .getProperty(
                                com.atakmap.comms.TAKServer.CONNECT_STRING_KEY,
                                null);
                if (connectString != null) {
                    ...
                    inputsToLoad.add(Pair.create(connectString, input));
                }
            }
        }
        Thread t = new Thread(new Runnable() {
            public void run() {
                for (Pair<String, Bundle> input : inputsToLoad)
        ...
        if (!allLoaded) {
            // maintain synchronous behavior per legacy if not all input config files were loaded
            try {
                t.join();
    ...
    public void addInput(final String connectString, final Bundle input) {
        ...
        switch (connectParts[2]) {
            case "udp":
                input.putString(TAKServer.CONNECT_STRING_KEY, connectString);
    ...
    public void addOutput(String connectString, Bundle output) {
        ...
        if (connectParts[2].equals("udp")) {
            output.putString(TAKServer.CONNECT_STRING_KEY, connectString);
    ...
    public void reconnectStreams() {
        ...
        if (streams != null) {
            for (Bundle sb : streams) {
                String connStr = sb.getString(TAKServer.CONNECT_STRING_KEY);
                if (connStr != null) {
                    addStreamingImpl(connStr, sb);
                }
            }
        }
    }

    private boolean validateExclusiveConnection(String connectString) {
        // return true if we don't have an exclusive connection
        if (!prefs.contains("exclusiveConnection")) {
            return true;
        } else {
            String exclusiveConnection = prefs.get("exclusiveConnection", "");
            // return true if this is the exclusive connection
            if (exclusiveConnection.equals(connectString)) {
                return true;
            } else {
                // return true if the exclusive connection is not enabled
                Bundle b = CommsMapComponent.getInstance().getAllPortsBundle();
                Bundle[] streams = (Bundle[]) b.getParcelableArray("streams");
                if (streams != null) {
                    for (Bundle sb : streams) {
                        ...
                        if (connStr != null) {
                            String[] connParts = connStr.split(":");
                            if (connParts.length != 3)
                                throw new IllegalArgumentException(
                                        "Connect string not comprised of ip:port:protocol ("
                                                + Arrays.toString(connParts)
                                                + ")");

                            if (connParts[0] != null && connParts[0]
                                    .equals(exclusiveConnection)) {
                                return !sb.getBoolean(TAKServer.ENABLED_KEY,
                                        true);
                            }
                        }
                    }
    ...
    public void addStreaming(String connectString, Bundle output) {
        ...
        addStreamingImpl(connectString, output);

        // if were adding the exclusive connection, reconnectStreams to disable other connections
        String exclusiveConnection = prefs.get("exclusiveConnection", null);
        if (exclusiveConnection != null) {
            ...
            if (exclusiveConnection.equals(connectParts[0])) {
                reconnectStreams();
            }
        }
    }
     ...
     */
    private void addStreamingImpl(String connectString, Bundle output) {
        ...
        if (!validateExclusiveConnection(connectParts[0])) {
            output.putBoolean(TAKServer.ENABLED_KEY, false);
        }
        ...
        CotServiceRemote.Proto proto = CotServiceRemote.Proto.tcp;

        if (connectParts[2].equals(CotServiceRemote.Proto.ssl.toString())) {
            proto = CotServiceRemote.Proto.ssl;
        } else if (connectParts[2]
                .equals(CotServiceRemote.Proto.quic.toString())) {
            proto = CotServiceRemote.Proto.quic;
        ...
        if (proto == CotServiceRemote.Proto.ssl
                || proto == CotServiceRemote.Proto.quic) {
            ...
            // don't pull in default certs if the connection uses enrollment. enrollment
            ...
            if (!hadError && (caCertCredentials == null
                    ...
                    || caCertCredentials.password.isEmpty())) {
                NotificationUtil
                        .getInstance()
                        .postNotification(
                                31346,
            ...
            if (!hadError && (clientCertCredentials == null
                    ...
                    || clientCertCredentials.password.isEmpty())) {
                NotificationUtil.getInstance().postNotification(
                        31346,
                        NotificationUtil.GeneralIcon.NETWORK_ERROR.getID(),
                        NotificationUtil.RED,
        ...
        output.putString(TAKServer.CONNECT_STRING_KEY, connectString);
        output.putBoolean(TAKServer.ISSTREAM_KEY, true);
        CommsMapComponent.getInstance().addStreaming(connectString, output,
                hadError, proto, connectParts[0], port, clientCert, trustStore,
    ...
    public void addStreaming(com.atakmap.comms.TAKServer server) {
    ...
    private void validateCert(String hostname, int port, Bundle b) {
        ...
        boolean skipCertificateValidationForHost = prefs.get(
                "skipCertificateValidationForHost-" + hostname.toLowerCase() + "-" + port, false);
        if (skipCertificateValidationForHost) {
            Log.d(TAG, "Skipping certificate validation for: " + hostname);
            return;
        }
        ...
        String errHint = "TLS certificate";
        for (int i = 0; i < 2; ++i) {
            if (i == 1) {
                // Now check trust
                errHint = "Truststore";
                validity = TLSUtils.validateTrust(hostname, port);
            }
            if (validity == null) {
                Log.w(TAG, errHint + " invalid for: " + hostname);
                TLSUtils.promptCertificateError(context, hostname,
                        "Unable to validate " + errHint, true);
                //leave enabled so red dot server notification will be displayed
                //b.putBoolean("enabled", false);
                TAKServer.appendString(b, TAKServer.ERROR_KEY,
                        "Invalid " + errHint);
            } else if (!validity.isValid()) {
                Log.w(TAG, errHint + " invalid for: " + hostname + ", "
                        + validity);
                TLSUtils.promptCertificateError(context, hostname,
                        errHint + " invalid", true);
                //leave enabled so red dot server notification will be displayed
                //b.putBoolean("enabled", false);
                TAKServer.appendString(b, TAKServer.ERROR_KEY,
                        "Invalid " + errHint);
                ...
                Log.w(TAG,
                        errHint + " expires very soon for: " + hostname + ", "
                                + validity);
                TAKServer.appendString(b, TAKServer.ERROR_KEY,
                        errHint + " expires in " + validity.daysRemaining()
                                + " days");
                TLSUtils.promptCertificateError(context, hostname,
                        errHint + " expires in " + validity.daysRemaining()
                ...
                Log.w(TAG, errHint + " expires soon for: " + hostname + ", "
                        + validity);
                TAKServer.appendString(b, TAKServer.ERROR_KEY,
                        errHint + " expires in " + validity.daysRemaining()
                                + " days");
                ...
                Log.d(TAG, errHint + " is valid for: " + hostname + ", "
                        + validity);
                continue;
            }
            // Had at least one error above, just bail
            break;
        }
    }
    ...
    public void setCredentialsForStream(String connectString, String user,
            String pass) {
        TAKServer port = CommsMapComponent.getInstance().removeStreaming(
                connectString);
        if (port != null) {
            CommsMapComponent.getInstance().removeStreaming(connectString);
            Bundle portBundle = port.getData();
            portBundle.putString(TAKServer.USERNAME_KEY, user);
            portBundle.putString(TAKServer.PASSWORD_KEY, pass);
    ...
    public void setUseAuthForStream(String connectString, boolean useAuth) {
        ...
        if (port != null) {
            CommsMapComponent.getInstance().removeStreaming(connectString);
            Bundle portBundle = port.getData();
            portBundle.putBoolean(TAKServer.USEAUTH_KEY, useAuth);
            addStreaming(connectString, portBundle);
        }
    }
    ...
    private boolean _loadSavedOutputs() {
        ...
        for (File config : outputs) {
            ...
            String desc = props.getProperty("description", "");
            String connectString = props
                    .getProperty(com.atakmap.comms.TAKServer.CONNECT_STRING_KEY,
                            null);
            if (connectString != null) {
                boolean isEnabled = !props.getProperty("enabled", "1")
                        .equals("0");
    ...
    private void upgradeLegacyConfiguration(String prefsName) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            String connectString = prefs
                    .getString(
                            com.atakmap.comms.TAKServer.CONNECT_STRING_KEY + i,
                            "");
            ...
            boolean enrollUseTrust = prefs.getBoolean(
                    "enrollUseTrust" + i, true);

            long expiration = -1;
            try {
                expiration = prefs.getLong(
                        com.atakmap.comms.TAKServer.EXPIRATION_KEY + i, -1);
            } catch (Exception e) {
                Log.d(TAG,
            ...
            input.putBoolean("enrollUseTrust",
                    enrollUseTrust);
            input.putLong(com.atakmap.comms.TAKServer.EXPIRATION_KEY,
                    expiration);
            ...
            // bring it over to english just in case it is set to the current locale
            if (!FileSystemUtils.isEmpty(cacheCreds)) {
                ...
                if (cacheCreds
                    ...
                    cacheCreds = ResourceUtil.getString(context,
                            R.string.cache_creds_both, Locale.US);
                else if (cacheCreds.equals(
                        context.getString(R.string.cache_creds_username)))
                    cacheCreds = ResourceUtil.getString(context,
            ...
            if (cacheCreds != null && (cacheCreds.equalsIgnoreCase(ResourceUtil
                    .getString(context, R.string.cache_creds_both, Locale.US))
                    ||
                    cacheCreds.equalsIgnoreCase(ResourceUtil
                            .getString(context, R.string.cache_creds_username,
                                    Locale.US)))) {
            ...
            if (cacheCreds != null && cacheCreds.equalsIgnoreCase(ResourceUtil
                    .getString(context, R.string.cache_creds_both,
                            Locale.US))) {
    ...
    private boolean _loadSavedStreams() {
        ...
        for (Properties props : cotStreamProperties) {
            ...
            boolean enrollUseTrust = !props.getProperty(
                    "enrollUseTrust", "1").equals("0");
            ...
            input.putBoolean("enrollForCertificateWithTrust",
                    enrollForCertificateWithTrust);
            input.putBoolean("enrollUseTrust", enrollUseTrust);
    ...
    private void _saveInputOutput(String prefsName, String connectString,
        ...
        synchronized (propertyFileLock) {
            ...
            props.setProperty(com.atakmap.comms.TAKServer.CONNECT_STRING_KEY,
                    connectString);
            ...
            props.setProperty(com.atakmap.comms.TAKServer.EXPIRATION_KEY,
                    Long.toString(data.getLong(
                            com.atakmap.comms.TAKServer.EXPIRATION_KEY, -1)));
            ...
            if (!FileSystemUtils.isEmpty(cacheCreds)) {
                ...
                String cacheUsername = (cacheCreds
                        .equalsIgnoreCase(ResourceUtil.getString(context,
                                R.string.cache_creds_both, Locale.US))
                        || cacheCreds
                                .equalsIgnoreCase(
                ...
                AtakAuthenticationDatabase.saveCredentials(
                        ...
                        data.getLong(com.atakmap.comms.TAKServer.EXPIRATION_KEY,
                                -1));
            ...
            props.setProperty("enrollForCertificateWithTrust",
                    data.getBoolean("enrollForCertificateWithTrust", false)
                            ? "1"
                            : "0");
            props.setProperty("enrollUseTrust",
                    data.getBoolean("enrollUseTrust", true)
                            ? "1"
                            : "0");
    ...
    private void _removeSavedInputOutput(String prefsName,
        ...
        AtakAuthenticationDatabase.delete(
                AtakAuthenticationCredentials.TYPE_COT_SERVICE, server);

        final File configFile = getConnectionConfig(context, prefsName,
                connectString);
        IOProviderFactory.delete(configFile, IOProvider.SECURE_DELETE);

        if (prefs.get("exclusiveConnection", "")
                .equals(server)) {
            prefs.remove("exclusiveConnection");
        }
    }
