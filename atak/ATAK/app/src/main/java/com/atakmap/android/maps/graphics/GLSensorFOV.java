...
public class GLSensorFOV extends GLShape2 implements OnMetricsChangedListener,
    ...
    protected float _rangeLineStrokeWeight = 1f;
    protected int _rangeLineStrokeColor = Color.BLACK;
    ...
    public GLSensorFOV(MapRenderer surface, SensorFOV subject) {
        ...
        _setMetrics(subject.getAzimuth(), subject.getFOV(),
                ...
                subject.getRangeLines(), subject.getElevation());
        refreshStyle();
    }
    ...
    private Style getRangeLineStyle() {
        return new BasicStrokeStyle(_rangeLineStrokeColor,
                _rangeLineStrokeWeight);
    ...
    private void refreshStyle() {
        runOnGLThread(new Runnable() {
            ...
            public void run() {
                ...
                final Style rls = getRangeLineStyle();
                if (rangeLines != null) {
    ...
    private GLBatchLineString constructRangeLine(GLMapView view,
        ...
        for (int i = 0; i <= numSlices; i++) {
            final GeoPoint gp = GeoCalculations.pointAtDistance(_point,
                    ...
                    distance, _elevation);
            ls.addPoint(gp.getLongitude(), gp.getLatitude(), gp.getAltitude());
        }
    ...
    public void onMetricsChanged(SensorFOV fov) {
        final float elevation = fov.getElevation();
        ...
        _rangeLineStrokeWeight = (float) fov.getRangeLineStrokeWeight();
        _rangeLineStrokeColor = fov.getRangeLineStrokeColor();

        runOnGLThread(new Runnable() {
            @Override
            public void run() {
                _setMetrics(azimuth, f, extent, labels, labelL, labelR,
                        rangeLines, elevation);
            }
        });
    }
    ...
    private void _setMetrics(float azimuth, float fov, float extent,
            boolean labels,
            String labelL, String labelR, float rangeLines, float elevation) {
    ...
    private void updatePolygon() {
        ...
        ls.addPoint(_point.getLongitude(), _point.getLatitude(), _point.getAltitude());
        ...
        if (isDrawLabels() && this.subject.getVisible()
            ...
            lsLabels.addPoint(_point.getLongitude(), _point.getLatitude(), _point.getAltitude());
            final GeoPoint end = GeoCalculations.pointAtDistance(_point,
                    _azimuth - (_fov / 2.0f), _extent, _elevation);

            lsLabels.addPoint(end.getLongitude(), end.getLatitude(), _point.getAltitude());
        ...
        if (isDrawLabels() && this.subject.getVisible()
            ...
            final GeoPoint end = GeoCalculations.pointAtDistance(_point,
                    _azimuth + (_fov / 2.0f), _extent, _elevation);
            lsLabels.addPoint(end.getLongitude(), end.getLatitude(), end.getAltitude());
