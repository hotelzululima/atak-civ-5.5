...
public class GeoPackageImporter implements Importer, Disposable {
    ...
    private static final gov.tak.api.importfiles.ImportResolver kernelImportResolver =
            new gov.tak.api.importfiles.GeoPackageImportResolver("GeoPackage Feature Layer",
                    FileSystemUtils.getRoot(), ResUtils.getDrawable(MapView.getMapView().getContext(), R.drawable.gpkg));
    ...
    /**
     * @deprecated will return a {@link gov.tak.api.importfiles.ImportResolver}
     */
    @Deprecated
    ...
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    public static gov.tak.api.importfiles.ImportResolver getKernelImportResolver() {
        return kernelImportResolver;
    }
            ...
            if (IOProviderFactory.exists(overlayDir)) {
                ...
                if (overlayFiles != null) {
                    for (File file : overlayFiles) {
                        if (kernelImportResolver.match(file))
                            importFile(file);
                    }
                }
            } else {
    ...
    public synchronized ImportResult importData(Uri uri,
        ...
        if (!isDisposed
            ...
            try {
                cursor = importDB.queryCatalog(file);
        ...
        public boolean match(File file) {
            do {
                ...
                if(!GeoPackage.isGeoPackage(file))
                    break;
                // open and check for one or more features tables
                GeoPackage gpkg = null;
                try {
                    gpkg = new GeoPackage(file, true);
                    boolean hasFeatures = false;
                    for(GeoPackage.ContentsRow content : gpkg.getPackageContents())
                        hasFeatures |= (content.data_type == GeoPackage.TableType.FEATURES);
                    if(!hasFeatures)
                        break;
                } catch(Throwable t) {
                    break;
    ...
    private boolean importFile(File packageFile) {
        ...
        try {
            ...
            GeoPackageFeatureDataStore2 dataStore = new GeoPackageFeatureDataStore2(
                    packageFile);
