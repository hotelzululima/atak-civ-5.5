...
public class CompassArrowMapComponent extends AbstractMapComponent implements
    ...
    public void onCreate(Context context, Intent intent, MapView view) {
        ...
        final String status_mapmode_heading = _prefs
                .get("status_mapmode_heading_value", null);
        ...
        lockedHeading = _prefs.get("status_mapmode_heading_locked", false);

        final String status_mapmode = _prefs.get("status_mapmode", null);
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        if (mapTouch.isFreeForm3DEnabled()
                && !_prefs.get("status_3d_enabled", false))
            view.getMapController().tiltTo(0, false);

        // record the heading value at the time the app is being shut down.
        _prefs.set("status_mapmode_heading_value",
                Double.toString(mapView.getMapRotation()));
    ...
    protected final BroadcastReceiver orientationReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            switch (action) {
                ...
                case LOCK_TILT:
                    boolean def = !_prefs.get("status_tilt_enabled", true);
                    _prefs.set("status_tilt_enabled", def);
                    if (tlwc != null) {
                        // now that the preference has been set, now determine the lock state
                        tlwc.setLocked(
                                !_prefs.get("status_tilt_enabled", true));
                    }
                    break;
            ...
            _prefs.set("status_mapmode_heading_value",
                    Double.toString(lockedHeadingValue));

            _prefs.set("status_mapmode_heading_locked", lockedHeading);

            _prefs.set("status_mapmode", Integer.toString(mapMode.getValue()));
    ...
    public void onMapProjectionChanged(AtakMapView view) {
        ...
        boolean globeModeEnabled = _prefs.get("atakGlobeModeEnabled", true);
    ...
    public void onTrackChanged(Marker marker) {
        ...
        if (Float.isNaN(heading))
            heading = (float) mapView.getMapData().getMetaDouble(
                    "deviceAzimuth",
                    0d);
    ...
    protected synchronized void toggleWidget(boolean show) {
        ...
        enable3DControls(show && _prefs.get("status_3d_enabled", false));
        isVisible = show;
    }
