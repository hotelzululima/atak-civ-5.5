...
public class DynamicRangeAndBearingEndpoint extends Marker implements
    ...
    public DynamicRangeAndBearingEndpoint(MapView mapView,
        ...
        setEditing(true); // hint to always render interactively
        ...
        if (item == this
            ...
            if (type.equals(MapEvent.ITEM_DRAG_STARTED))
                if (MetricsApi.shouldRecordMetric()) {
                    ...
                    b.putString("action", "drag_started");
                    MetricsUtils.record(
                            MetricsUtils.CATEGORY_MAPITEM,
                            MetricsUtils.EVENT_MAPITEM_CHANGED,
                            "DynamicRangeAndBearingEndPoint", this, b);
                }
            ...
            if (getParent() != null && getParent().isLocked()) {
                ...
                gp = GeoPointMetaData
                        .wrap(GeoCalculations.pointAtDistance(anchorPoint,
                                bearing, range));
                try {
                    gp = ATAKUtilities.getElevationMetadata(
                            gp.get().getLatitude(), gp.get().getLongitude());
                } catch (Exception e) {
