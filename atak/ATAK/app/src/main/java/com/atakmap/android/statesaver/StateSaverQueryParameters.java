...
public class StateSaverQueryParameters {
    ...
    public static class Order {
        ...
        public static final class Default extends Order {
            ...
            public String toString() {
                return StateSaver.COLUMN_QUERY_ORDER;
            }
        }
        ...
        public static final class Time extends Order {
            ...
            @Override
            public String toString() {
                return StateSaver.COLUMN_LAST_UPDATE;
            }
        }
    ...
    CursorIface executeQuery(DatabaseIface db) {
        ...
        if (!MathUtils.hasBits(this.ignoredProperties,
            ...
            sql.append(", ").append(StateSaver.COLUMN_EVENT);

        sql.append(" FROM ").append(StateSaver.TABLE_COTEVENTS);

        // Filtering
        StringBuilder whereClause = new StringBuilder();
        appendWhere(StateSaver.COLUMN_TYPE, this.types, whereClause, args);
        appendWhere(StateSaver.COLUMN_UID, this.uids, whereClause, args);

        if (this.minimumTimestamp >= 0 || this.maximumTimestamp >= 0) {
            if (whereClause.length() > 0)
                whereClause.append(" AND ");
            whereClause.append(StateSaver.COLUMN_LAST_UPDATE);
            if (this.minimumTimestamp >= 0 && this.maximumTimestamp >= 0) {
        ...
        if (this.order != null && !this.order.isEmpty()) {
            ...
            sql.append(StateSaver.COLUMN_QUERY_ORDER).append(" ASC");
