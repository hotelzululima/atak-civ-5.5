...
public class MetricReportMapComponent extends AbstractMapComponent
    ...
    public void onCreate(final Context context, Intent intent,
        ...
        this.prefs = AtakPreferences.getInstance(view.getContext());
        this._realtime_metrics = prefs.get("realtime_metrics", false);
        this._hideLocation = prefs.get("dispatchLocationHidden", false);
        ...
        AtakBroadcast.getInstance().registerReceiver(
                _exportCrashLogRec = new BroadcastReceiver() {
                    ...
                    public void onReceive(Context context,
                            Intent intent) {
                        if (prefs.get("collect_metrics", false)) {
    ...
    private synchronized void startMetricsCollection() {
        ...
        recordingThread.execute(new Runnable() {
            @Override
            public void run() {
                setupLog();
                beginLog();
    ...
    private void writeCotEventDetails(final CotEvent ce) {
        if (!prefs.get("generate_detail_pool", false))
            return;
        ...
        for (int i = 0; i < cd.childCount(); i++) {
            ...
            int roll = prefs.get("metrics.detailscount." + name, 0);
            roll++;
            ...
            prefs.set("metrics.detailscount." + name, (roll % 250));
        }
    }
    ...
    private void writeCotEvent(final CotEvent ce) {
        if (!prefs.get("generate_full_pool", false))
            return;
    ...
    private CotEvent getStatsEvent(CotDetail stats) {
        ...
        final MetaDataHolder2 mapData = view.getMapData();
        ...
        final String selfType = mapData.getMetaString("deviceType", null);
        ...
        if (LocationManager.getInstance()
            ...
            if (ATAKUtilities.findSelf(view) != null)
    ...
    private void beginLog() {
        sessionid = prefs.get("core_sessionid", "unknown");
    ...
    private final CotServiceRemote.ConnectionListener cl = new CotServiceRemote.ConnectionListener() {
        @Override
    ...
    private final CotServiceRemote.OutputsChangedListener _outputsChangedListener = new CotServiceRemote.OutputsChangedListener() {
        @Override
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (i != null) {
                ...
                record("intent", i.getExtras(), null,
                        prop("action", i.getAction()), prop("stacktrace",
                                intent.getStringArrayExtra("stacktrace")));
                return;
            }
    ...
    private void reapLogs() {
        int numOfDays = prefs.get("metricNumberOfDays", 30);
    ...
    private void setupLog() {
        try {
            ...
            fos = new BufferedOutputStream(
                    IOProviderFactory.getOutputStream(logFile));
    ...
    private void writeToFos(byte[] bytes) {
        if (!collecting)
            return;
        ...
        recordingThread.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // force flush every 3 seconds
                    long time = SystemClock.elapsedRealtime();
                    if (time - lastFlush > 3000) {
                        fos.flush();
                        lastFlush = time;
                    }
                    error = false;
                    ...
                    Log.w(TAG, "Could not record: " + e.getMessage() + ", "
                            + new String(bytes));
                    // if an error has occurred, try to resolve one time before unable to record
                    if (!error) {
                        error = true;
                        setupLog();
                    }
    ...
    private final Executor recordingThread = Executors.newSingleThreadExecutor(
            new NamedThreadFactory("MetricRecorderThread"));

    private long lastFlush = -1;

    private Prop prop(final String key, final String[] arr) {
        return new Prop(key,
                (arr == null) ? "not enabled" : Arrays.toString(arr));
    }
