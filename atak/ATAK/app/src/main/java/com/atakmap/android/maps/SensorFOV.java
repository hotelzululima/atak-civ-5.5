...
public class SensorFOV extends Shape implements MapItem.OnGroupChangedListener,
    ...
    private double rangeLineStrokeWeight = 1d;
    private int rangeLineStrokeColor = Color.BLACK;
    ...
    public SensorFOV(final long serialId, final MetaDataHolder2 metadata,
            final String uid) {
    ...
    public GeoPoint[] getPoints() {
        ...
        return new GeoPoint[] {
                _point.get(),
                GeoCalculations.pointAtDistance(_point.get(), _azimuth - (_fov / 2.0f), _extent,
                         _elevation),
                GeoCalculations.pointAtDistance(_point.get(), _azimuth + (_fov / 2.0f), _extent,
                         _elevation)
        };
    ...
    public void setMetrics(float azimuth, float fov, float extent) {
        this.setMetrics(azimuth, fov, extent, isShowLabels(), getRangeLines());
    }

    public void setMetrics(float azimuth, float fov, float extent,
                           boolean bLabels, float rangeLines) {
        setMetrics(azimuth, fov, extent, bLabels, rangeLines, 0.0f);
    }
     ...
     * @param azimuth  the azimuth is in degrees true
     * @param fov field of view in angular degrees
     * @param extent in meters bounded between Integer.MAX_VALUE and Integer.MIN_VALUE.   If the value
     ...
     */
    public void setMetrics(float azimuth, float fov, float extent,
            boolean bLabels, float rangeLines, float elevation) {
    ...
    /**
     * Returns the elevation of the sensor field of view in degrees (also called the inclination)
     * @return the elevation / inclination
     */
    ...
    public void setRangeLineStrokeWeight(double strokeWeight) {
        if (Double.compare(strokeWeight, rangeLineStrokeWeight) != 0) {
            rangeLineStrokeWeight = strokeWeight;
            onMetricsChanged();
            onFillColorChanged();
        }
    }

    public double getRangeLineStrokeWeight() {
        return rangeLineStrokeWeight;
    }

    public void setRangeLineStrokeColor(int strokeColor) {
        if (strokeColor != rangeLineStrokeColor) {
            rangeLineStrokeColor = strokeColor;
            onMetricsChanged();
            onFillColorChanged();
        }
    }

    public int getRangeLineStrokeColor() {
        return rangeLineStrokeColor;
    }
    ...
    public void setPoint(GeoPointMetaData point) {
        ...
        arrow.setPoint1(_point);
        arrow.setPoint2(GeoPointMetaData
                .wrap(GeoCalculations.pointAtDistance(_point.get(), _azimuth,
                        _extent, _elevation)));
        onPointsChanged();
    }
    ...
    public GeoBounds getBounds(MutableGeoBounds bounds) {
        ...
        if (_point != null) {
            ...
            points.add(GeoCalculations.pointAtDistance(
                    _point.get(), _azimuth, _extent, _elevation));
            ...
            for (int deg = 0; deg < 360; deg += 90) {
                if (angleWithin(deg, minDeg, maxDeg))
                    points.add(GeoCalculations.pointAtDistance(
                            _point.get(), deg, _extent, _elevation));
            }
        }
    ...
    public Bundle preDrawCanvas(CapturePP capture) {
        ...
        if (_point != null && _point.get().isValid()) {
            PointF top = capture.forward(
                    GeoCalculations.pointAtDistance(_point.get(), 0, _extent, _elevation));
            PointF right = capture.forward(
                    GeoCalculations.pointAtDistance(_point.get(), 90, _extent, _elevation));
            PointF bottom = capture.forward(GeoCalculations
                    .pointAtDistance(_point.get(), 180, _extent, _elevation));
            PointF left = capture.forward(GeoCalculations
                    .pointAtDistance(_point.get(), 270, _extent, _elevation));
            ...
            if (isDrawLabels()) {
                ...
                GeoPoint rgp1 = GeoCalculations.pointAtDistance(_point.get(),
                        _azimuth + _fov / 2, _extent, _elevation);
     ...
     */
    private static boolean angleWithin(double deg, double minDeg,
            double maxDeg) {
