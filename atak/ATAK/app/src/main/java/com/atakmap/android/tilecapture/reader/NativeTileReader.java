...
public class NativeTileReader extends DatasetTileReader {
    ...
    public NativeTileReader(ImageDatasetDescriptor info) {
        ...
        double pixelSize = info.getSpatialReferenceID() == 4326
                ? (180.0 / _tileWidth) / (double) (1 << _levelOffset)
                : OSMUtils.mapnikTileResolution(_levelOffset);

        TileMatrix.ZoomLevel minZoom = new TileMatrix.ZoomLevel();
        minZoom.level = _levelOffset;
        minZoom.pixelSizeX = minZoom.pixelSizeY = pixelSize;
        minZoom.resolution = OSMUtils.mapnikTileResolution(_levelOffset);
