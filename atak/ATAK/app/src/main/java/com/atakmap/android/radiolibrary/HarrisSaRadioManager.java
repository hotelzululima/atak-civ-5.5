 ...
 */
class HarrisSaRadioManager implements Disposable {
    /**
     * Tag for logging
     */
    ...
    HarrisSaRadioManager(Context context) {
        ...
        String deviceName = sharedPreferences.get(PREFERENCE_KEY, null);
        ...
        if (devicesList != null) {
            for (File device : devicesList) {
                try {
                    ...
                    if (uevent.exists()) {
                        String contents = FileSystemUtils.copyStreamToString(
                                uevent,
                            ...
                            if (radio != null) {
                                ...
                                if (subDirFiles != null) {
                                    for (File subDir : subDirFiles) {
                                        File ttyAcmFile = new File(subDir,
                                                "tty");
                                            ...
                                            if (ttyAcmFileList != null) {
                                                for (File ttyAcmPort : ttyAcmFileList) {
                                                    ...
                                                    name = name.substring(name
                                                            ...
                                                            + "ttyACM"
                                                                    .length());
                                                    int port = Parsers.parseInt(name, -1);
                                                    if (port != -1) {
                                                        radio.addTtyAcmPort(
                                                                port);
                                                    }
                    ...
                    Log.e(TAG, "could not open: " + device + "/uevent", e);
                }
            }
    ...
    public void setSelectedRadio(HarrisRadio selectedRadio) {
        ...
        if (selectedRadio != null && selectedRadio.getUsbDevice() != null) {
            sharedPreferences.set(PREFERENCE_KEY,
                    selectedRadio.getUsbDevice().getDeviceName());
        }
    }
}
