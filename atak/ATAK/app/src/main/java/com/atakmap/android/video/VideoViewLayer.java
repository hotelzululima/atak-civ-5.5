 ...
 */
public class VideoViewLayer implements Disposable {
     ...
     * @param connectionEntry the connection entry representing the video
     */
    public void snapshot(ConnectionEntry connectionEntry, File file) {
    }
     ...
     * 
     * @return the copy of the connection entry currently being used.
     */
    ...
    /**
     * Requests the next available frame from the video consumer.
     * Invoke only on the main UI thread.
     * @param callback callback to be invoked when the request completes
     * @throws IllegalStateException if invoked when the layer is not initialized
     */
    public void requestBitmapFrame(BitmapFrameRequestCallback callback)
            throws IllegalStateException {
        if (vddr == null)
            throw new IllegalStateException("Layer not currently initialized");
        vddr.requestBitmapFrame(callback);
    }
}
