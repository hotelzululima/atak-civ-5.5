...
public class RouteMapReceiver extends BroadcastReceiver implements
    ...
    private final AtakPreferences _navPrefs;
    ...
    public RouteMapReceiver(final MapView mapView,
        ...
        _context = context;
        _navPrefs = AtakPreferences.getInstance(mapView
                .getContext());
        _navPrefs.registerListener(this);
    ...
    public void dispose() {
        ...
        _navPrefs.unregisterListener(this);
    }
    ...
    public SharedPreferences getPreferences() {
        return _navPrefs.getSharedPrefs();
    }
    ...
    public Route getNewRoute(final String uid) {
        ...
        final double userSuppliedStroke = getPreferences()
                .getFloat("routeLastUserThickness", -1f);
        if (userSuppliedStroke > 0d) {
            route.setStrokeWeight(userSuppliedStroke);
        }
        ...
        switch (action) {
            ...
            case START_NAV: {
                ...
                if (_navigator.isNavigating()) {
                    ...
                    if (found != null) {
                        {
                            for (String e : found.getAttributeNames()) {
                                Log.e(TAG,
                                        "Key: " + e
                                                + " = "
                                                + found.get(e));
                    ...
                    // Start up nav. Will setup the reroute button if appropriate
            ...
            case "com.atakmap.android.maps.TOOLSELECTOR_READY":
                ...
                PendingIntent act = PendingIntent.getBroadcast(_context,
                        _navigator.hashCode(),
                        myLocationIntent, PendingIntentHelper.adaptFlags(0));
    ...
    private void showRerouteDialog(final RoutePlannerManager rpm) {
        ...
        dlg.setListener(new Dialog.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                final MarkerIconWidget rrb = rerouteButton;
                if (rrb == null)
                    return;

                if (which == Dialog.BUTTON_NEGATIVE) {
                    rrb.setIcon(rerouteInactiveIcon);
                    rrb.setState(REROUTE_BUTTON_OFF);
                    _navPrefs.set(
                            RerouteDialog.PREF_IS_REROUTE_ACTIVE_KEY,
                            false);
                    dialog.dismiss();
                } else if (which == Dialog.BUTTON_POSITIVE) {
                    rrb.setIcon(rerouteActiveIcon);
                    rrb.setState(REROUTE_BUTTON_ON);
                    _navPrefs.set(
                            RerouteDialog.PREF_IS_REROUTE_ACTIVE_KEY,
                            true);
                    dialog.dismiss();
    ...
    void editCues(final Route route, final PointMapItem pmi,
        ...
        b.setPositiveButton(R.string.ok, new OnClickListener() {
            ...
            public void onClick(DialogInterface d, int w) {
                ...
                if (MetricsApi.shouldRecordMetric() && newCue != null) {
                    final Bundle b = new Bundle();
                    b.putString("point", cpUid);
                    b.putString("cue", newCue.getTextCue());
        ...
        b.setNegativeButton(R.string.cancel, new OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                MetricsUtils.record(MetricsUtils.CATEGORY_MAPITEM,
                        MetricsUtils.EVENT_MAPITEM_CHANGED, "RouteMapReceiver",
                        route, "cue_changed");
            }
        });
        b.show();
     ...
     */
    public void showRouteDetails(Route route, RoutePlannerInterface2 autoPlan,
            boolean edit) {
            ...
            public void run() {
                _plannerView = (RoutePlannerView) LayoutInflater.from(
                        _mapView.getContext()).inflate(R.layout.route_planner,
                                _mapView, false);
                _plannerView.init(_mapView, route, RouteMapReceiver.this,
                        autoPlan);
                if (edit) {
                    _plannerView.startEdit();
                    _mapView.post(new Runnable() {
        ...
        synchronized (routes_) {
            for (Route r : routes_) {
                if (state && (FileSystemUtils.isEmpty(excludedUID)
                    ...
                    r.setRenderHints(Polyline.RENDER_FLAG_FADE_ITEM);
                    r.hideLabels(true);
                    if (stopTouch)
     ...
     */
    @Deprecated
    @DeprecatedApi(forRemoval = true, removeAt = "5.8", since = "5.5")
    public void mirrorImage(Route route, String newUID) {
        mirrorImage(route, newUID, true);
    }

    /**
     * Constructs a complete mirror image of the route without doing much.
     * @param route     The route to mirror
     * @param newUID    UID of the new mirrored route
     * @param keepCues  true to keep the cues of the original route; false otherwise
     */
    public void mirrorImage(Route route, String newUID, boolean keepCues) {
        ...
        r.setStrokeWeight(route.getStrokeWeight());
        r.setStrokeStyle(route.getStrokeStyle());
    ...
    public void createReversedRoute(Route route, String newUID,
            RoutePlannerInterface2 rpi, boolean includeCP,
            boolean showDetails) {
    ...
    private class ImportRouteTask extends AsyncTask<Void, Void, String> {
        ...
        protected String importKml() {
            ...
            Route route = RouteKmlIO.toRoute(_mapView, kml, _routeGroup,
                    _waypointGroup, _navPrefs.getSharedPrefs());
            if (route == null) {
                Log.w(TAG,
        ...
        protected void onPostExecute(String error) {
            ...
            } else if (_importRoutes == null || _importRoutes.isEmpty()) {
     ...
     */
    private synchronized void setupRerouteButton() {
        ...
        if (routePlannerManager.getRoutePlanners2().isEmpty()) {
            removeRerouteButton();
            return;
        }
        ...
        rerouteButton.addOnPressListener(new MapWidget.OnPressListener() {
            ...
            public void onMapWidgetPress(MapWidget widget, MotionEvent event) {
                ...
                if (routePlannerManager.getReroutePlanners2().isEmpty()) {
                    // No reroute capable route planners - hide the button
    ...
    private synchronized void removeRerouteButton() {
        if (rerouteButton != null)
            getRightLayoutWidget().removeWidget(rerouteButton);
        rerouteButton = null;
        _navPrefs.set(RerouteDialog.PREF_IS_REROUTE_ACTIVE_KEY,
                false);
    }
    ...
    public static void promptPlanRoute(final MapView _mapView,
        ...
        final List<RoutePlannerInterface2> routePlanners = new ArrayList<>();
        ...
        if (!routePlanners.isEmpty()) {
            createReplanDialog(_mapView, origin, dest, _route, byWayOf, routePlanners);
        }
    }


    static private void createReplanDialog(final MapView _mapView,
                                           final PointMapItem origin,
                                           final PointMapItem dest,
                                           final Route _route,
                                           final List<GeoPoint> byWayOf,
                                           final List<RoutePlannerInterface2> routePlanners) {

        final Context _context = _mapView.getContext();
        final View v = LayoutInflater.from(_context).inflate(R.layout.reroute_dialog, null);
        final TextView startPoint = v.findViewById(R.id.startpoint);
        final TextView endPoint = v.findViewById(R.id.endpoint);
        final Spinner planSpinner = v.findViewById(R.id.planspinner);
        final Button openRouteAround = v.findViewById(R.id.btn_open_route_around);
        final CheckBox routeAroundGeofence = v.findViewById(R.id.chk_route_around_geo_fences);
        final CheckBox routeAroundRegions = v.findViewById(R.id.chk_route_regions);
        final AtakPreferences _prefs = AtakPreferences.getInstance(_context);
        routeAroundRegions.setChecked(_prefs.get(
                RouteAroundRegionManagerView.OPT_AVOID_ROUTE_AROUND_REGIONS,
                false));
        routeAroundGeofence.setChecked(_prefs.get(
        ...
        for (int i = 0; i < routePlanners.size(); i++) {
            RoutePlannerInterface2 entry = routePlanners
                    .get(i);
            plannerNames[i] = entry.getDescriptiveName();
        }

        AlertDialog.Builder adb = new AlertDialog.Builder(_context);
        adb.setTitle(R.string.reroute_planner);
        adb.setView(v);

        startPoint.setText(ATAKUtilities.getDisplayName(origin));
        endPoint.setText(ATAKUtilities.getDisplayName(dest));
        ...
        if (lastUsedPlanner != null) {
            for (int i = 0; i < plannerNames.length; ++i) {
                if (plannerNames[i].equals(lastUsedPlanner))
                    planSpinner.setSelection(i);
            }
        }


        adb.setPositiveButton(R.string.reroute, new OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                final RoutePlannerInterface2 planner = routePlanners
                        .get(planSpinner.getSelectedItemPosition());
                Log.d(TAG, "Planner \"" + planner.getDescriptiveName() + "\" was selected");

                _prefs.set("lastUsedPlanner", planSpinner.getSelectedItem());

                _prefs.set(RouteAroundRegionManagerView.OPT_AVOID_ROUTE_AROUND_REGIONS,
                        routeAroundRegions.isChecked());

                _prefs.set(RouteAroundRegionManagerView.OPT_AVOID_GEOFENCES,
                        routeAroundGeofence.isChecked());
        ...
        RouteAroundRegionManager _routeAroundManager = RouteAroundRegionManager.getInstance();
        RouteAroundRegionManagerView regionManagerView = new RouteAroundRegionManagerView(_mapView,
                new RouteAroundRegionViewModel(_routeAroundManager));

        openRouteAround.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                regionManagerView.setParentParentDialog(ad);
                dialog.show();
            }
        });
        ad.show();
    ...
    static final Comparator<RoutePlannerInterface2> RPI_COMP = new Comparator<RoutePlannerInterface2>() {
        @Override
        public int compare(RoutePlannerInterface2 lhs,
                RoutePlannerInterface2 rhs) {
            String lName = lhs.getDescriptiveName();
            String rName = rhs.getDescriptiveName();
            if (lName == null)
                return 1;
            else if (rName == null)
                return -1;
            ...
            String lKey = lhs.getUniqueIdenfier(),
                    rKey = rhs.getUniqueIdenfier();
            if (lKey == null)
                return 1;
            else if (rKey == null)
                return -1;
