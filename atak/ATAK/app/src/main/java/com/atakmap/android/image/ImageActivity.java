...
public class ImageActivity {
    ...
    private boolean _useGeoTakCam = false;
    ...
    public ImageActivity(MapView mapView, File outImage, String uid,
        ...
        _output = outImage;
        _tmpFile = createTempFile(outImage);
        _uid = uid;
    ...
    private static File createTempFile(File file) {
        ...
        File cacheDir = FileSystemUtils.getItem("tmp");
        File result = null;
        try {
    ...
    public void useTakGeoCam(final boolean useGeoTakCam) {
        this._useGeoTakCam = useGeoTakCam;
    }
    ...
    public void start() {
        ...
        if (_useGeoTakCam) {
            ...
            if (_broadcast.hasExtras()) {
                for (ActionBroadcastExtraStringData dataItem : _broadcast
                        .getExtras()) {
                    i.putExtra(dataItem.getKey(), dataItem.getValue());
                }
            }
        } else {
        ...
        if (requestCode == CAMERA_REQUEST
            ...
            if (_multiCapture && data != null && data.hasExtra("pathList")) {
                ...
                if (photo != null) {
                    ...
                    Log.d(TAG,
                            "bitmap unable to be retrieved from the extra, could not save");
                }
            } else {
                Log.w(TAG,
    ...
    private void insertExifLocationData(File f) {
        ...
        List<String> tags = StickyHashtags.getInstance().getEnabledTags();
