...
public class ImageFileContainer
    ...
    public String getCurrentImageUID() {
        ...
        if (f != null) {
            final File pFile = f.getParentFile();
            if (pFile != null)
    ...
    public boolean setCurrentImageByUID(String imageUID) {
        ...
        if (imageUID != null) {
            ...
            for (File f : files) {
                final File pFile = f.getParentFile();
                if (pFile != null) {
                    if (pFile.getName().equals(imageUID)) {
                        imageIndex = index;
                        foundImage = true;
                        break;
                    }
    ...
    synchronized protected void refreshView() {
        ...
        final GifImageView imageView = layout
                    ...
                    if (bmpFile != null) {
                        ...
                        ((Activity) context).runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                boolean error = false;
                                if (FileSystemUtils.checkExtension(
                                        imageFile.getName(), ".gif") &&
                                        FileSystemUtils.getFileSize(
                                                getCurrentImageFile()) < ANIMATED_GIF_MAX_SIZE) {

                                    try {
                                        final byte[] data = FileSystemUtils
                                                .read(getCurrentImageFile());
                                        if (FileSystemUtils.isEmpty(data))
                                            error = true;
                                        else
                                            imageView.setGif(data);
                                    } catch (Exception e) {
                                        imageView.requestState(
                                                GifImageView.GifImageViewState.NoGif); //remove gif image if present
                                        error = true;
                                    }
                                } else {
                                    imageView.requestState(
                                ...
                                imageError.setVisibility(
                                        (bmp == null || error) ? View.VISIBLE
                                                : View.GONE);
                                imageView.setVisibility(
                                        (bmp == null || error) ? View.GONE
                                                : View.VISIBLE);
    ...
    private final int ANIMATED_GIF_MAX_SIZE = 1024 * 1024 * 16;
}
