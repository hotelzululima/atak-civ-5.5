...
public class PreferenceSearchDialog
    ...
    private final static List<PreferenceSearchIndex> masterlist = new ArrayList<>();
    private static volatile boolean init = false;

    /**
     * Allow for preinitialization of the Search Dialog
     * @param context the context to use for looking up (this is the ATAK context)
     */
    private synchronized static void preinit(Context context) {
        if (init)
            return;
        init = true;
        ...
        add(masterlist, BasicDisplayPrefFragment.index(context));
        ...
        add(masterlist, ContactPreferenceFragment.index(context));
        Log.d(TAG, "preference search indexing completed in " +
    ...
    public PreferenceSearchDialog(final Context context) {
        ...
        /**
        for (PreferenceSearchIndex psi: masterlist) {
            if (psi != null && psi.getKey() != null && psi.getSummary() != null) {
                Log.d(TAG, "'" + psi.getKey() + "', '" + psi.getSummary().replaceAll("'", "") + "'");
            }
        }
         **/
    }
    ...
    private static List<PreferenceSearchIndex> loadIndex() {
    ...
    private static void add(Map<String, PreferenceSearchIndex> list,
    ...
    private static void add(List<PreferenceSearchIndex> list,
            List<PreferenceSearchIndex> toAdd) {
        if (list == null || toAdd == null) {
            Log.w(TAG, "Index add failed");
            return;
