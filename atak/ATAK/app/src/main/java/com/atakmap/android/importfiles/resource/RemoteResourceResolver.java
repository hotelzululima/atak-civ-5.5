...
public class RemoteResourceResolver extends gov.tak.api.importfiles.ImportResolver {
    ...
    public RemoteResourceResolver(MapView mapView) {
        super(".kml", FileSystemUtils.getItem(FileSystemUtils.OVERLAYS_DIRECTORY),
                mapView.getContext()
                        .getString(R.string.importmgr_remote_resource),
                ResUtils.getDrawable(mapView.getContext(), R.drawable.ic_kml_network));
        _mapView = mapView;
    ...
    public boolean match(File file) {
        ...
        boolean isKMZ = FileSystemUtils.checkExtension(file, "kmz");
        if (!(isKML || isXML))
            return false;

        if (isKMZ) {
            return processNetworkKMZ(file, true);
        }
    ...
    public boolean beginImport(File file, EnumSet<SortFlags> flags) {
        try (FileInputStream fis = IOProviderFactory.getInputStream(file)) {
            ...
            if (FileSystemUtils.checkExtension(file, "kmz")) {
                return processNetworkKMZ(file, false);
            } else if (FileSystemUtils.checkExtension(file, "kml")) {
    ...
    /**
     * Process the network KMZ.
     * @param file
     * @param checkOnly if this is true, only return true if the kmz does indeed contain
     *                  a kml with a network link file.   If false, then actually process
     *                  the kml file in the kmz with the network link and return true if
     *                  the processing was successful.
     * @return true if the file does contain a kml with a network link OR true if the file
     * was processed successfully when checkOnly is false
     */
    private boolean processNetworkKMZ(File file, boolean checkOnly) {
        ZipFile zipFile = null;
        try {
            zipFile = new ZipFile(file);
            List<? extends ZipEntry> arZipEnt = Collections
                    ...
                    try {
                        ...
                        if (isNetworkKML(zis)) {
                            if (checkOnly) {
                                ...
                                KMLUtil.parseNetworkLinks(zis,
                                        new FeatureHandler<NetworkLink>() {
                                            @Override
                                            public boolean process(
                                                    NetworkLink nl) {
                                                RemoteResource res = createRemoteResource(
                                                        nl);
                                                if (res != null)
                                                    resources.add(res);
                                                return false;
