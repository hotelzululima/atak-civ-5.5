...
public class AngleOverlayShape extends AutoSizeAngleOverlayShape
    ...
    /**
     * Use a simple spoke view for bullseye display othewise will be used to display the
     * compass rose.
     * @param simpleSpokeView
     */
    ...
    /**
     * The state of the angle overlay shape
     * @return true if it is a bullseye or false if it is a compass rose.
     */
    ...
    /**
     * The radius in meters for the angle overlay shape
    ...
    /**
     * Set the desired span for display of the radius in when drawing
     */
    public void setRadiusUnits(Span units) {
        _units = units;
    }
    ...
    private String getAngleLabel(int deg) {
        ...
        if (deg == 0)
            deg = 360;
        String unit = (getBearingUnits() == Angle.MIL ? "mils"
                : Angle.DEGREE_SYMBOL)
                + _azimuth.getAbbrev();
        return (getBearingUnits() == Angle.MIL
                ? AngleUtilities.formatNoUnitsNoDecimal(
                        deg, Angle.DEGREE, Angle.MIL).replace(",", "")
                : String.valueOf(deg))
                + (deg == 360 ? unit : "");
    }
}
