...
public class DrawingEllipseImporter extends DrawingImporter {
    ...
    protected ImportResult importMapItem(MapItem existing, CotEvent event,
        ...
        GeoPointMetaData center = PrecisionLocationHandler
        ...
        List<Ellipse> prevRings = ellipse.getEllipses();
        ...
        for (CotDetail d : children) {
            ...
            final String ringUid = ellipse.getUID() + "." + rings.size();

            Ellipse e = null;
            for (Ellipse pr : prevRings) {
                if (pr.getUID().equals(ringUid)) {
                    e = pr;
                    break;
                }
            }
        ...
        if (shape2 != null) {
            ellipse.setMetaBoolean("editable",
                    Parsers.parseBoolean(shape2.getAttribute("editable"),
                            true));
            ellipse.setCenterPointVisible(
