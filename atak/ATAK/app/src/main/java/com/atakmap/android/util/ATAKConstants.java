...
public class ATAKConstants {
    ...
    private static String majorminorsubmajor = "";
    ...
    public synchronized static void init(final Context context) {
        ...
        if (FileSystemUtils.isEmpty(versionName)) {
            ...
            try {
                ...
                // check.
                String flavorApi = SystemComponentLoader
                        .getFlavorAPI(context);
                String coreApi = AtakPluginRegistry.getPluginApiVersion(context,
                        context.getPackageName(), false);

                if (!FileSystemUtils.isEmpty(flavorApi)) {

                    flavorApi = flavorApi.substring(flavorApi.lastIndexOf('.'));
                    coreApi = coreApi.substring(0, coreApi.lastIndexOf('.'));
                    flavorApi = coreApi + flavorApi;
                ...
                majorminorsubmajor = AtakPluginRegistry.getPluginApiVersion(context,
                        context.getPackageName(), true);
                majorminorsubmajor =
                        majorminorsubmajor.substring(0, majorminorsubmajor.lastIndexOf('.'));
    ...
    /**
     * Return the major.minor.submajor version code for core
     */
    public static String getAbiVersion() {
        return majorminorsubmajor;
    }
    ...
    public static void displayAbout(final Context context,
        ...
        AtakPreferences _controlPrefs = AtakPreferences.getInstance(context);
    ...
    public static Map<String, String> getGeneralInformation(Context context) {
        ...
        retVal.put("tak.uid",
                prefs.get("bestDeviceUID", "unknown"));
    ...
    /**
     * Compares the input in the form of #.#.# and #.#.# to determine if the
     * left is greater than the right
     * @param left a version code strictly in the form X.Y.Z where X,Y,Z are integers
     * @param right a version code strictly in the form X.Y.Z where X,Y.Z are integers
     */
    public static boolean isGreaterThan(String left, String right) {
        final String[] la = left.split("\\.");
        final String[] ra = right.split("\\.");
        if (la.length != 3 && ra.length != 3)
            throw new IllegalArgumentException("input not in the form X.Y.Z: l=" +
                    left + " r=" + right);

        for (int i = 0; i < 3; ++i) {
            // if the actual api is greater than the forced api, just return
            // the actual api
            if (Integer.parseInt(la[i]) > Integer.parseInt(ra[i]))
                return true;
        }
        return false;
    }
