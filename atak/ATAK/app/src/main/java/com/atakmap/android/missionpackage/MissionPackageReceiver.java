...
public class MissionPackageReceiver extends BroadcastReceiver implements
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        // Compress data package to ZIP and optionally send
            ...
            final String[] contactUIDs = intent.getStringArrayExtra("sendTo");
            if (extras == null) {
                Log.e(TAG, "missing package manifest");
                return;
            }
            ...
            if (overlay == null)
                return;
            if (extras == null) {
                Log.e(TAG, "missing package manifest uid");
                return;
            }
            ...
            if (bSave) {
                ...
                final MissionPackageBaseTask.Callback fCb = cb;
                final MissionPackageBaseTask.Callback fOverlay = overlay;

                _component.getFileIO().save(group.getManifest(), new MissionPackageBaseTask.Callback() {
                    @Override
                    public void onMissionPackageTaskComplete(MissionPackageBaseTask task, boolean success) {

                        if (fOverlay != fCb)
                            fOverlay.onMissionPackageTaskComplete(task, success);
                        if (fCb != null)
                            fCb.onMissionPackageTaskComplete(task, success);
                    }
                });
            } else {
    ...
    public int getWebServerPort() {
        return MissionPackagePreferenceListener.getInt(_prefs.getSharedPrefs(),
    ...
    public void fileUpdateCallback(File file, OPERATION op,
        ...
        if (file.toString().startsWith(
                FileSystemUtils.getItem("tools/datapackage/transfer") + "/")) {
            Log.d(TAG, "ignoring transfer file: " + file.getName());
            return;
        }
        ...
        try {
            switch (op) {
                case ADD: {
                    ...
                    // only process VALID fully formed mission packages
                    MissionPackageExtractor mpe = new MissionPackageExtractor();
                    MissionPackageManifest mpm = mpe.getManifest(file);

                    if (mpm == null || !mpm.isValid()) {
                        Log.d(TAG,
                                "encountered a zip file that is not a datapackage: "
                                        + file.getName());

                        // if this is a plain old zip file and is not any other sorter - needs to be checked here
                        // in case a person copies the zip file into the datapackage directory.
                        ImportMissionPackageSort sorter = new ImportMissionPackageSort.ImportMissionV1PackageSort(
                                _context, true,
                                true, false);

                        String pFile = file.getParent();
                        if (pFile != null && pFile.equals(FileSystemUtils
                                .getItem("tools/datapackage").toString()) &&
                                sorter.match(file)) {
                            Log.d(TAG,
                                    "the file was placed in the datapackage directory, so treat it as such: "
                                            + file.getName());
                            new ExtractMissionPackageTask(file, this, null)
                                    .execute();
                        }
                        return;
                    } else
