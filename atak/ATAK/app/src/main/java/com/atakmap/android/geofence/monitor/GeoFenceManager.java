...
public class GeoFenceManager implements GeoFenceComponent.GeoFenceListener,
        ...
        MapItem.OnGroupChangedListener, Disposable {
    ...
    // caching `MonitoredTypes.values()` is micro-optimization observed to save ~20% time to
    // process item for membership in _global tracked lists_
    private final static GeoFence.MonitoredTypes[] _monitoredTypes = GeoFence.MonitoredTypes
            .values();
    ...
    /**
     * Deferred add events - wait until the key UID is added to the map
     */
    ...
    public GeoFenceManager(GeoFenceComponent component, MapView view) {
        synchronized (GeoFenceManager.class) {
            if (_instance != null)
                throw new IllegalStateException(
                        "GeoFenceManager already exists, it can be obtained by the GeoFenceComponent");
            _instance = this;
        }
        ...
        _deferredAdds = new ConcurrentHashMap<>();
    ...
    private void initialize() {
        ...
        MapGroup.deepMapItems(baseGroup, new MapGroup.MapItemsCallback() {
            ...
            public boolean onItemFunction(MapItem item) {
                ...
                } else if (MapItem.EMPTY_TYPE.equals(item.getType())) {
                    item.addOnTypeChangedListener(onTypeChangedListener);
                }
        ...
        _monitorTimer.schedule(new TimerTask() {
            ...
            public void run() {
                try {
                    ...
                    Log.e(TAG, "error running monitor", ise);
                }
            }
        }, 0, MONITOR_RATE);
    ...
    public void onMapEvent(final MapEvent event) {
        ...
        geofenceMonitor.execute(new Runnable() {
            @Override
            public void run() {
                if (type.equals(MapEvent.ITEM_ADDED)) {
                    Pair<GeoFence, MapItem> reAdd = _deferredAdds
                            .remove(item.getUID());
                    if (reAdd != null)
                    ...
                    if (!(item instanceof PointMapItem))
                        return;
                    // the expectation here is that any removal is done off of the UI thread
                    removeItem_sync(item);
                }
            }
        });
    }
    ...
    private void addItem(final MapItem item) {
        ...
        if (MapItem.EMPTY_TYPE.equals(item.getType())) {
            item.addOnTypeChangedListener(onTypeChangedListener);
            return;
        }
        addItem_sync(item);
    ...
    public static class Metrics {
        public static AtomicLong globalTrackedLists = new AtomicLong(0L);
        public static AtomicLong monitors = new AtomicLong(0L);
        public static AtomicLong debug = new AtomicLong(0L);
    }

    private synchronized void addItem_sync(final MapItem item) {
        PointMapItem pointMapItem = (PointMapItem) item;

        //try(MarkerImporter.NanoTimer nt = new MarkerImporter.NanoTimer(Metrics.globalTrackedLists))
        {
            //add to global tracked lists
            for (GeoFence.MonitoredTypes tt : _monitoredTypes) {
        ...
        //try(MarkerImporter.NanoTimer nt = new MarkerImporter.NanoTimer(Metrics.monitors))
        {
            for (GeoFenceMonitor monitor : _monitors.values()) {
                ...
                if (monitor.getFence()
                    ...
                    if (CircleGeoFenceMonitor.QuickCheck(
                            monitor.getCenter().get(),
        ...
        //try(MarkerImporter.NanoTimer nt = new MarkerImporter.NanoTimer(Metrics.debug))
        {
        ...
        b.setNeutralButton(R.string.monitor,
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface d, int i) {
                        ...
                        beginMonitoring(new ArrayList<>(), monitor,
                                false);
                    }
                });
    ...
    private final ExecutorService geofenceMonitor = Executors
            .newSingleThreadExecutor(new NamedThreadFactory(
                    "geofence-monitor"));

    private final MapItem.OnTypeChangedListener onTypeChangedListener = new MapItem.OnTypeChangedListener() {
        @Override
        public void onTypeChanged(MapItem item) {
            if (item != null && !FileSystemUtils.isEmpty(item.getType())) {
                Log.d(TAG, "marker came without a type originally: " + item);
                addItem(item);
