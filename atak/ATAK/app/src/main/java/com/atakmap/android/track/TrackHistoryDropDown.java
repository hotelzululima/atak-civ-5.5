...
public class TrackHistoryDropDown extends DropDownReceiver implements
    ...
    private synchronized View getTrackDetailsView(TrackDetails track,
        ...
        if (bReQuery || track == null)
            new GetTrackDetailsTask(getMapView(),
                    ...
                    _prefs.get("elevProfileInterpolateAlt", true))
                            .execute();
    ...
    public void onClick(View v) {
        ...
        // Toolbar - Start new track
            ...
            promptNewTrack(_prefs.getSharedPrefs(), getMapView(), false);
        }
    ...
    public void onReceive(Context c, Intent intent) {
        ...
        switch (action) {
            case TRACK_HISTORY:
                int trackDbId = -1;
                final String metaTrackDbId = intent.getStringExtra(
                        CrumbDatabase.META_TRACK_DBID);

                if (metaTrackDbId != null) {
                    trackDbId = Parsers.parseInt(metaTrackDbId, -1);
                }
            ...
            case TRACK_SEARCH:
                ...
                if (uid != null) {
                    ...
                    if (item != null) {
                        final String callsign = (item == getMapView()
                                .getSelfMarker())
                                        ? getMapView().getDeviceCallsign()
                                        : item.getTitle();
                        trackUser = new TrackUser(callsign, uid, 0);
                    }
                }
                showTrackSearchView(trackUser, false);
                break;
            case TRACKSEXPORTED: {
            ...
            case SERVERTRACKSEXPORTED: {
                ...
                final String cotType = _prefs.get("locationUnitType",
                        _context.getString(
                                R.string.default_cot_type));
                final String groupName = _prefs.get("locationTeam", "Cyan");
                final String groupRole = _prefs.get("atakRoleType",
                        "Team Member");
                ...
                String connectString = intent.getStringExtra("connectString");
                if (!FileSystemUtils.isEmpty(baseUrl)
                        && !FileSystemUtils.isEmpty(connectString)) {
                    ...
                    postUserTracks(baseUrl,
                            ...
                            callbackAction, params, connectString);
                    return;
                }
    ...
    private void postUserTracks(String baseUrl, String callsign, String uid,
        ...
        PostUserTracksRequest request = new PostUserTracksRequest(baseUrl,
                ...
                groupRole, filePath, callbackAction, params, connectString);
    ...
    public void searchRemoteTracks(String baseUrl, String callsign, String uid,
            long searchStart, long searchEnd, String serverNCS) {
        if (FileSystemUtils.isEmpty(baseUrl)) {
            onError(_context.getString(
                    R.string.no_server_selected));
            return;
        ...
        QueryUserTracksRequest request = new QueryUserTracksRequest(baseUrl,
                curNotificationId++,
                callsign, uid, searchStart, searchEnd, serverNCS);
     ...
     */
    public com.atakmap.comms.TAKServer[] getServers() {
        List<TAKServer> ret = new ArrayList<>();
        TAKServer[] servers = _serverListener.getServers();
        if (servers != null) {
            for (TAKServer c : servers) {
                if (c.isConnected())
                    ret.add(c);
            }
        }
        return ret.toArray(new TAKServer[0]);
    }
