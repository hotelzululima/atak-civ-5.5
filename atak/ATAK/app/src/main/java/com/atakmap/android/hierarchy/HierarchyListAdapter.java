...
public class HierarchyListAdapter extends BaseAdapter implements
    ...
    private AtomicBoolean blockRefresh = new AtomicBoolean(false);
    ...
    protected final Stack<HierarchyListItem> prevListStack = new Stack<>();

    // The back-stack of scroll positions so the user doesn't lose their place
    // after navigating into a hierarchy of lists
    protected final Stack<Integer> prevListScroll = new Stack<>();
    ...
    public HierarchyListAdapter(Context context, MapView mapView,
        ...
        this.filterFOV = this.prefs.get(FOVFilter.PREF, false);
        this.rangeSystem = Integer.parseInt(prefs.get(
                "rab_rng_units_pref", String.valueOf(1)));
        this.bearingUnits = Angle.findFromValue(Integer.parseInt(
                prefs.get("rab_brg_units_pref", String.valueOf(0))));
        this.northRef = NorthReference.findFromValue(Integer.parseInt(
                prefs.get("rab_north_ref_pref", String.valueOf(
                        NorthReference.MAGNETIC.getValue()))));
        this.coordFmt = CoordinateFormat.find(prefs.get(
    ...
    void registerListener() {
        if (!this.active) {
            ...
            this.prefs.registerListener(this);
    ...
    void unregisterListener() {
        if (this.active) {
            ...
            this.prefs.unregisterListener(this);
            this.active = false;
    ...
    /**
     * Check whether a touch or press is active.  Checking this prevents
     * touches from being lost when updating the data table
     *
     * @return True if press is active
     */
    boolean isPressActive() {
        final ListView listView = view._getHierarchyList();
        return receiver.isTouchActive()
                || (listView != null && listView.isPressed());
    }
    ...
    public void filterFOV(boolean filterOn) {
        if (this.filterFOV != filterOn) {
            ...
            this.prefs.set(FOVFilter.PREF, filterOn);
    ...
    public View getView(int position, View row, ViewGroup parent) {
        ...
        h.vizButton.setVisibility(View.INVISIBLE);
        ...
        // Item has a location
            ...
            h.elevation.setText(AltitudeUtilities.format(point,
                    this.prefs.getSharedPrefs()));
    ...
    public void blockRefresh(boolean block) {
        if (this.blockRefresh.compareAndSet(!block, block)) {
            /*if (!block) {
    ...
    public boolean isRootList() {
        return this.prevListStack.isEmpty();
    }
    ...
    public void popList(final HierarchyListItem toList) {
        ((Activity) this.context).runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (prevListStack.isEmpty() && view != null) {
