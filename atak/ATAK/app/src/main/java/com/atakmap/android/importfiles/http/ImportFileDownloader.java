...
public class ImportFileDownloader extends NetworkLinkDownloader {
    ...
    public void ndl(final RemoteResourceRequest r) {
        Thread t = new Thread("RemoteResourceRequestDownload") {
            public void run() {
                ...
                try {
                    ...
                    NetConnectString netConnectString = null;
    ...
    public void onRequestFinished(Request request, Bundle resultData) {
        ...
        if (request
            ...
            if (initialRequest.getResource().getType()
                    .equals("INTERNAL_TRANSIENT")) {
                Log.d(TAG, "direct import - do not treat as a remote resource: "
                        + downloadedFile.getAbsolutePath());
                ImportFileTask importFileTask = new ImportFileTask(_context,
                        null);
                if (initialRequest.showNotifications()) {
                    NotificationUtil.getInstance()
                            .clearNotification(getNotificationId());
                }
                importFileTask.execute(downloadedFile.getAbsolutePath());
            } else {
