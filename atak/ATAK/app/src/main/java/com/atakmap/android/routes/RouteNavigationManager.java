...
public final class RouteNavigationManager implements Disposable {
    ...
    public RouteNavigationManager(final Route route) {
        ...
        arrivedAtPoints = Collections.newSetFromMap(
                new ConcurrentHashMap<>(wayPointCount));
    ...
    private void onLocationChanged(GeoPoint newLocation) {
        ...
        if (closePoints != null) {
            ...
            for (Pair<Integer, Boolean> closePoint : closePoints) {
                ...
                if (newPoint) {
                    ...
                    PointMapItem pmi = route.getPointMapItem(closePoint.first);
                    //if the waypoint reached is a required waypoint mark that we hit it
                    if (pmi.getMetaBoolean(
                            RouteRequiredWayPointHandler.IS_WAY_POINT_REQUIRED,
                            false)) {
                        pmi.setMetaBoolean(
                                RouteRequiredWayPointHandler.REACHED_WAYPOINT,
                                true);
                    }
                    fireOnArrivedAtPoint(pmi);
                }
            }
        }
        ...
        if (trigger != null) {
            ...
            if (newTrigger) {
                ...
                Queue<Integer> fired = triggerPointsFired.get(trigger.first);
                if (fired != null) {
                    fired.add(trigger.second);
                } else {
