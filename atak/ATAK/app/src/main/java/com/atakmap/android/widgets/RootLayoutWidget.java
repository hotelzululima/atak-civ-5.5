...
public class RootLayoutWidget extends LayoutWidget implements IRootLayoutWidget,
    ...
    public RootLayoutWidget(final MapView mapView) {
        ...
        _prefs.set(PREF_CURVED_SCREEN, _prefs.get(
                PREF_CURVED_SCREEN, defValue));
        onSharedPreferenceChanged(_prefs.getSharedPrefs(), PREF_CURVED_SCREEN);
    ...
    public List<Rect> getOccupiedBounds(boolean includeWidgets) {
        List<Rect> ret;
        synchronized (this) {
            ret = new ArrayList<>(_views.size() + _layouts.length);
            for (View v : _views) {
    ...
    private synchronized void addView(View v) {
        if (v != null) {
            _views.add(v);
    ...
    private synchronized void removeView(View v) {
        if (v != null) {
            _views.remove(v);
    ...
    private final Executor executor = Executors
            .newSingleThreadExecutor(new NamedThreadFactory("onchange-thread"));
}
