    ...
    protected GeoPointMetaData findPoint(MapEvent event) {
        ...
        if (mi != null && !type.startsWith("item_drg")) {
            if (mi instanceof PointMapItem) {
                ...
                point = GeoPointMetaData.wrap(mi.getClickPoint());
                // if the item has a recorded touch, pass through the hit-type for downstream
                // processing
                if (mi.hasMetaValue("last_touch")
                        && mi.hasMetaValue("hit_type")) {
                    try {
                        point.setMetaValue("hit_type",
                                HitTestResult.Type.valueOf(
                                        mi.getMetaString("hit_type", null)
                                                .toUpperCase(LocaleUtil
                                                        .getCurrent())));
                    } catch (Exception e) {
