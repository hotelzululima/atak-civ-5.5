...
public final class FilterMapOverlay extends AbstractMapOverlay2
    ...
    private GridSlicerOverlay gridSlicerOverlay;

    private final ReadWriteLock itemsRWLock = new ReadWriteLock();
    ...
    View getHeaderView() {
        if (isGridSlicerValid())
            return gridSlicerOverlay.getHeaderView();
        else
            return null;
    }
    ...
    View getCustomLayout() {
       if (isGridSlicerValid())
           return gridSlicerOverlay.getCustomLayout();
       else
           return null;
    }


    private boolean isGridSlicerValid() {
        // spoiler alert - grid slicer is not valid for routes or nav which is a special
        // cased type of overlay manager
        if (getIdentifier().equals("routes") || getIdentifier().equals("nav"))
            return false;
        else
            return true;
    }
    ...
    public static class ListModelImpl extends AbstractHierarchyListItem2
        ...
        public View getHeaderView() {
            return _overlay.getHeaderView();
        }

        @Override
        public View getCustomLayout() {
            return _overlay.getCustomLayout();
        }
            ...
            for (HierarchyListItem item : children) {
                ...
                if (title != null
                    ...
                    Set<HierarchyListItem> childretval = ((ListModelImpl) item)
                            .find(terms);
                    if (childretval != null && !childretval.isEmpty())
        ...
        private Folder toKml(ExportFilters filters)
            ...
            f.setFeatureList(new ArrayList<>());
            List<HierarchyListItem> children = getChildren();
            for (HierarchyListItem item : children)
    ...
    private void init(FiltersConfig colorFilters) {
        ...
        synchronized (this.overlays) {
            ...
            this.gridSlicerOverlay = GridSlicerOverlay.getInstance();
        }
    }
    ...
    private boolean onPrefChanged(String key) {
        ...
        List<FilterMapOverlay> overlays = getOverlaysUnmodifiable();
        for (FilterMapOverlay child : overlays)
    ...
    private boolean addItem(MapItem item, boolean updateVis) {
        ...
        if (accept(item)) {
            ...
            MapItemHierarchyListItem listItem = new MapItemHierarchyListItem(
                    mapView, item);
            itemsRWLock.acquireWrite();
            try {
                this.items.put(item, listItem);
            } finally {
                itemsRWLock.releaseWrite();
            }
        }
        ...
        if (!changed) {
            List<FilterMapOverlay> children = getOverlaysUnmodifiable();
    ...
    private boolean preAccept(MapItem item) {
        ...
        if (item.hasMetaValue("filterOverlayId") && this.config.isExclusive()) {
            ...
            //printDebug("preAccept:alreadyAccepted", item);
            return false;
        }
        ...
        if (item instanceof Marker) {
            ...
            if (UserIcon.IsValidIconsetPath(path, false,
                ...
                //printDebug("preAccept:notAcceptingUserIcon", item);
                return false;
            }
        }
        ...
        if (this.config != null &&
            ...
            //printDebug("preAccept:didNotPassLitmusTest: " + config.getLitmusFilterString(), item);
            return false;
        }
    ...
    private boolean removeItem(MapItem item) {
        ...
        try {
            if (this.items.containsKey(item)) {
                // if this filter was the one that set the overlay id, remove it otherwise leave
                // it in place
                if (getIdentifier()
                        .equals(item.getMetaString("filterOverlayId", null)))
    ...
    public synchronized static MapGroup.MapItemsCallback getRejectFilter(
        ...
        try {
            ...
            Log.e(TAG, "Unexpected IO error loading filters.",
                    e);
            return null;
        }
    }
    ...
    public static synchronized Collection<FilterMapOverlay> createDefaultFilterOverlays(
        ...
        for (MapOverlayConfig config : overlayFilters.getOverlays()) {
            retval.add(new FilterMapOverlay(null, mapView, config,
    ...
    public List<MapItem> getItems() {
        itemsRWLock.acquireRead();
        try {
            return new ArrayList<>(this.items.keySet());
        } finally {
            itemsRWLock.releaseRead();
        }
    }
    ...
    public List<FilterMapOverlay> getOverlaysUnmodifiable() {
    ...
    private void printDebug(String action, MapItem item) {
        Log.d(TAG + toString(), action + ": " +
                item.toString() + " filterOverlayId=" +
                item.getMetaString("filterOverlayId", null) +
                " addToObjList: " + item.getMetaBoolean("addToObjList", true));
    }
}
