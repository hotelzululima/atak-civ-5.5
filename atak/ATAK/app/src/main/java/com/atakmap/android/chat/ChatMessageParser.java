...
public class ChatMessageParser {
    ...
    public void parseCotEvent(final CotEvent event) {
        ...
        if (event.getAccess() != null)
            message.setAccess(event.getAccess());

        if (event.getCaveat() != null)
            message.setCaveat(event.getCaveat());

        if (event.getReleasableTo() != null)
            message.setReleasableTo(event.getReleasableTo());
        ...
        if (chatChild != null) {
            ...
            if (conv == null) {
                ...
                String myTadilJ = _mapView.getMapData()
                        .getMetaString("tadiljId", null);
        ...
        final String messageContents = getMessageContents(detail);

        message.setMessage(messageContents);

        message.setCotEvent(event);
        ...
        if (serverdestination != null) {
            ...
            for (String dest : dests) {
                if (dest.startsWith(CotServiceRemote.Proto.udp.toString())) {
                    ...
                    String[] components = dest.split(":"); //Have to split for backwards compatibility with 2.2
