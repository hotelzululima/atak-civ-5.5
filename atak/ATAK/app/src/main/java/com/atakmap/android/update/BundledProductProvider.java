...
public class BundledProductProvider extends BaseProductProvider {
    ...
    protected ProductRepository load() {
        ...
        String prevProvider = prefs.get("bundledProductProvider", "");
        String provider = "bundleProvider-" + BuildConfig.FLAVOR;
        prefs.set("bundledProductProvider", provider);

        boolean reload = !prevProvider.equals(provider);
        return load(reload);
    }
