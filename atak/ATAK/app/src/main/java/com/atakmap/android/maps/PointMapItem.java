...
public abstract class PointMapItem extends MapItem {
    ...
    protected static final EventSet pointMapItemEventSet;

    private static final EventSet.Parameters1<OnPointChangedListener, PointMapItem> onPointChanged;

    static {
        EventSet.Builder builder = new EventSet.Builder(mapItemEventSet,
                "PointMapItem");
        onPointChanged = builder.addEvent(
                new EventSet.Invoker1<OnPointChangedListener, PointMapItem>() {
                    @Override
                    public void invoke(OnPointChangedListener l,
                            PointMapItem mapItem) {
                        l.onPointChanged(mapItem);
                    }
                });
        pointMapItemEventSet = builder.build();
    }

    @Override
    protected EventSet.Listeners createEventListeners() {
        return pointMapItemEventSet.createListeners();
    }
    ...
    protected PointMapItem(final long serialId, final MetaDataHolder2 metadata,
            final String uid) {
    ...
    /**
     * Add a point property listener.   If the listener is already added,
     * no action is performed.
     *
     * @param listener the listener
     */
    public void addOnPointChangedListener(OnPointChangedListener listener) {
        eventListeners.addUniqueListener(onPointChanged, listener);
    }
    ...
    public void removeOnPointChangedListener(OnPointChangedListener listener) {
        eventListeners.removeListener(onPointChanged, listener);
    }
     ...
     * @param point the point for the point map item
     */
    ...
    protected void onPointChanged() {
        onPointChanged.invoke(eventListeners, this);
    }
    ...
    public synchronized void dispose() {
        super.dispose();
        // clearing of propertyListeners happens in MapItem
    }
