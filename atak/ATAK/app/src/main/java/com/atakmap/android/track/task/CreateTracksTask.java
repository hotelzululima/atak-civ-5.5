...
public class CreateTracksTask extends AsyncTask<Void, Integer, Integer> {
    ...
    public CreateTracksTask(MapView mapView, String title, String callsign,
        ...
        _titlePrefix = prefs.get("track_prefix", "track_");
        _items = new ArrayList<>();
        _items.add(new Item(title,
                BreadcrumbReceiver.getNextColor(prefs.getSharedPrefs()),
                callsign, uid, stitch));
    }
    ...
    public CreateTracksTask(MapView mapView, Collection<PointMapItem> items,
        ...
        int trackColor = Integer.parseInt(prefs
                .get("track_history_default_color", String.valueOf(defColor)));
        int c = -1;
        if (prefs.get("toggle_rotate_track_colors", true)) {
            int lastColorUsed = prefs.get("track_last_color", defColor);
        ...
        prefs.set("track_last_color", trackColor);
    }
