...
public class DrawingEllipseEditTool extends ButtonTool implements Undoable,
    ...
    public boolean onToolBegin(Bundle extras) {
        ...
        final GeoBounds bounds = _ellipse.getBounds(null);
        if (!bounds.intersects(_mapView.getBounds()))
            ATAKUtilities.scaleToFit(_ellipse);
    ...
    public void onConflict(final SortedSet<MapItem> hitItems) {
        Set<MapItem> remove = new HashSet<>();

        for (MapItem item : hitItems) {
            if (item instanceof Ellipse)
                remove.add(item);
        }
        for (MapItem item : remove) {
            hitItems.remove(item);
        }
    }
        ...
        if (anchor != _center) {
            ...
            if (!userAlt) {
                ...
                if (_ellipse.getAltitudeMode() == AltitudeMode.ClampToGround) {
                    ATAKUtilities.getElevation(center.getLatitude(),
                            center.getLongitude(), gpmd);
                } else
