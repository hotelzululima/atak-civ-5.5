...
import com.atakmap.android.importfiles.callbacks.ImportResolverCallbackRegistry;
...
public class ImportExportMapComponent extends AbstractMapComponent implements
    ...
    /**
     * In 5.8, many of the {@link ImportResolver}s that fired this intent will be removed; see
     * {@link gov.tak.api.importfiles.ImportResolver} and {@link ImportResolverCallbackRegistry}
     */
    ...
    /**
     * List of dynamically added import resolvers
     */
    private List<gov.tak.api.importfiles.ImportResolver> _kernelImportResolvers;

    private ImportReceiver _importReceiver;
    private ImportResolverCallbackRegistry _resolverCallbackRegistry;
    ...
    /**
     * List of kernel import listeners
     */
    private final List<gov.tak.api.importfiles.ImportResolver.ImportListener> _kernelImportListeners = new ArrayList<>();
    ...
    private gov.tak.api.importfiles.ImportResolver.FileSortedListener _importFileSortedListener;
    private gov.tak.api.importfiles.ImportResolver.ImportListener _importListener;

    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    private Map<ImportResolver, gov.tak.api.importfiles.ImportResolver> _resolverMap;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (action) {
                ...
                case USER_HANDLE_IMPORT_FILE_ACTION:
                    ...
                    boolean bShowNotificationsDuringImport = intent
                            .getBooleanExtra(
                                    "showNotificationsDuringImport", false);
                    ...
                    if (bShowNotificationsDuringImport)
                        importTask.addFlag(
        ...
        public void onReceive(Context context, Intent intent) {
            if (IMPORT_COT.equals(intent.getAction())) {
                ...
                if (event != null && event.isValid()) {
                    ...
                    CotMapComponent.getParallelInternalDispatcher()
                            .dispatchFrom(event,
    ...
    private final BroadcastReceiver ftpUploadReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (FTP_UPLOAD_FILE_ACTION.equals(intent.getAction())) {
                ...
                if (filepath == null) {
                    Log.w(TAG, "Cannot FTP update a null file");
                    return;
                }
    ...
    private final BroadcastReceiver setExportLogServerReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (SET_EXPORT_LOG_SERVER.equals(intent.getAction())) {
                ...
                final TAKServer[] servers = _serverListener
                        .getServers();
                ...
                if (servers != null) {
                    ...
                    if (servers.length > 1) {
                                ...
                                new ServerListDialog.Callback() {
                                    ...
                                    public void onSelected(
                                        ...
                                        String server = takServer
                                                .getConnectString();
                                        _prefs.set("autoUploadLogServer",
                                                server);
                                        if (logFile != null) {
                        ...
                        _prefs.set("autoUploadLogServer", server);
                        if (logFile != null) {
                            _errorLogsClient.sendLogsToServer(
    ...
    public void onCreate(Context context, Intent intent, MapView mapView) {
        ...
        _resolverCallbackRegistry = ImportResolverCallbackRegistry.getInstance();
        ...
        _prefs = AtakPreferences.getInstance(mapView
                .getContext());
        _dropDown = new ImportManagerDropdown(mapView, this,
        ...
        _ftp = new FtpUploader(context, _prefs.getSharedPrefs());
        ...
        this.registerReceiver(context, _importReceiver = new ImportReceiver(), filter);
        ...
        // set up a generic file downloader
        this.registerReceiver(context, qrCodeReceiver,
                new DocumentedIntentFilter(ATAKActivity.ONSTART_URI));
        ...
        boolean enableAutoUploadLogs = _prefs.get(
                "enableAutoUploadLogs", false);
        String prefsAutoUploadLogServer = _prefs.get(
                "autoUploadLogServer", "");
        ...
        if (!_isSoftReset) {
            ...
            _kernelImportResolvers = new ArrayList<>();
        }

        // Send methods used by export
        _senders.add(new TAKContactSender(mapView));
        // Only register a TAKServerSender if our configured Comms supports one.
        ...
        _importFileSortedListener = new ImportFileSortedListener();
        _importListener = new NotifyingImportListener();
        _resolverMap = new HashMap<>();

        // prime the default signatures
        TxtImportSignatures.getInstance();
    ...
    protected void onDestroyImpl(Context context, MapView view) {
        ...
        _senders.clear();
        cleanupResolvers(_kernelImportResolvers);
        if (!_isSoftReset) {
            _importerResolvers.clear();
            _kernelImportResolvers.clear();
        }
        ...
        if (_importReceiver != null) {
            _importReceiver = null;
        }
    }
     ...
     *
     * @param resolver
     * @deprecated use {@link #addImporterClass(gov.tak.api.importfiles.ImportResolver)}
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    public synchronized void addImporterClass(ImportResolver resolver) {
        ...
        gov.tak.api.importfiles.ImportResolver kernelResolver = new ImportResolverWrapper(resolver);
        _resolverMap.put(resolver, kernelResolver);
        addImporterClass(kernelResolver);
    }

    /**
     * @deprecated use {@link #removeImporterClass(gov.tak.api.importfiles.ImportResolver)}
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    public synchronized void removeImporterClass(ImportResolver resolver) {
        ...
        if (_resolverMap.containsKey(resolver)) {
            removeImporterClass(_resolverMap.get(resolver));
            _resolverMap.remove(resolver);
        }
    }
     ...
     * @return
     * @deprecated in 5.8, the return type will change to a {@link Collection} of {@link gov.tak.api.importfiles.ImportResolver}s
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    public synchronized Collection<ImportResolver> getImporterResolvers() {
        ...
        return Collections
                .unmodifiableCollection(new ArrayList<>(_importerResolvers));
    }
     ...
     * @param resolver
     */
    public synchronized void addImporterClass(gov.tak.api.importfiles.ImportResolver resolver) {
        if (resolver == null) {
            Log.w(TAG, "Invalid importer resolver");
            return;
        }

        if (!_kernelImportResolvers.contains(resolver)) {
            _kernelImportResolvers.add(resolver);
            Log.d(TAG, "Adding import resolver class: "
                    + resolver.getClass().getName());
            ImportFilesTask.registerExtension(resolver.getExt());
            initImportResolver(resolver);
        }
    }

    public synchronized void removeImporterClass(gov.tak.api.importfiles.ImportResolver resolver) {
        if (resolver == null) {
            Log.w(TAG, "Invalid importer resolver");
            return;
        }
        ...
        if (_kernelImportResolvers.contains(resolver)) {
            _kernelImportResolvers.remove(resolver);
            Log.d(TAG, "Removing import resolver class: "
                    + resolver.getClass().getName());
            ImportFilesTask.unregisterExtension(resolver.getExt());
            _resolverCallbackRegistry.unregisterResolver(resolver);
        }
    }
     ...
     * Get list of <code>ImportResolver</code>
     * @return
     * @deprecated will be renamed {@code getImporterClass}
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    public synchronized Collection<gov.tak.api.importfiles.ImportResolver> getKernelImporterResolvers() {
        if (FileSystemUtils.isEmpty(_kernelImportResolvers)) {
            return new ArrayList<>();
        }

        return Collections
                .unmodifiableCollection(new ArrayList<>(_kernelImportResolvers));
    }

    /**
     * @deprecated use {@link #addImportListener(gov.tak.api.importfiles.ImportResolver.ImportListener)}
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    ...
    /**
     * @deprecated use {@link #removeImportListener(gov.tak.api.importfiles.ImportResolver.ImportListener)}
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    ...
    /**
     * @deprecated signature will change to return a {@link gov.tak.api.importfiles.ImportResolver.ImportListener}
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    ...
    public synchronized void addImportListener(gov.tak.api.importfiles.ImportResolver.ImportListener l) {
        if (!_kernelImportListeners.contains(l)) {
            _kernelImportListeners.add(l);
        }
    }

    public synchronized void removeImportListener(gov.tak.api.importfiles.ImportResolver.ImportListener l) {
        _kernelImportListeners.remove(l);
    }

    /**
     * @deprecated will be renamed getImportListeners when {@link #getImportListeners()} is removed
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    public synchronized List<gov.tak.api.importfiles.ImportResolver.ImportListener> getKernelImportListeners() {
        return new ArrayList<>(_kernelImportListeners);
    }

    /**
     * Registers ATAK-required callbacks for kernel-provided {@link ImportResolver}s. Plugins
     * should pass their own {@link ImportResolver}s through here.
     * </br></br>
     * {@link #cleanupResolvers(List)} them when done.
     */
    public void initImportResolver(gov.tak.api.importfiles.ImportResolver resolver) {
        resolver.addFileSortedListener(_importFileSortedListener);
        resolver.addFileSortedListener(_importReceiver);
        resolver.addFileImportedListener(_importListener);
        _resolverCallbackRegistry.registerResolver(resolver);
    }

    public void cleanupResolvers(List<gov.tak.api.importfiles.ImportResolver> resolvers) {
        for (gov.tak.api.importfiles.ImportResolver resolver : resolvers) {
            resolver.removeFileSortedListener(_importFileSortedListener);
            resolver.removeFileSortedListener(_importReceiver);
            resolver.removeFileImportedListener(_importListener);
            _resolverCallbackRegistry.unregisterResolver(resolver);
        }
    }
    ...
    private class ServerListener extends CotStreamListener {
        ...
        private final boolean enableAutoUploadLogs;
        private final String prefsAutoUploadLogServer;
    ...
    /**
     * Responsible for processing URI "com.atakmap.app/import keys and values in the form of
     * repeating query parameters as an encoded url
     * tak://com.atakmap.app/import?url=http%3A%2F%2Fwebaddress.com%2Ffile%2Ffile.zip
     * or
     * tak://com.atakmap.app/import?url=https%3A%2F%2Fdownload.osgeo.org%2Fgeotiff%2Fsamples%2Fmisc%2Ftjpeg.tif
     * @param u
     */
    private final BroadcastReceiver qrCodeReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            final String action = intent.getAction();
            if (action == null)
            ...
            if (action.equals(ATAKActivity.ONSTART_URI)) {
                final Uri u = intent.getParcelableExtra("uri");
                if (u != null && "com.atakmap.app/import"
                        .equals(u.getHost() + u.getPath())) {
                    AlertDialog.Builder ad = new AlertDialog.Builder(
                            _mapView.getContext());
                    ad.setTitle(R.string.import_string);
                    ad.setCancelable(false);
                    String importUrlString = u.getQueryParameter("url");
                    if (importUrlString == null)
                        return;

                    Uri importUrl = Uri.parse(importUrlString);

                    ad.setMessage(context.getString(R.string.import_string) +
                            " " + importUrl);
    ...
    private void beginImport(Uri uri) {
        ...
        rr.setUrl(uri.toString());
        rr.setType("INTERNAL_TRANSIENT");
        String name = uri.getPath();
        if (name != null)
            rr.setName(new File(name).getName());
        _downloader.download(rr, true);
    }

    /**
     * Remove when {@link #addImporterClass(ImportResolver)} is phased out
     */
    @Deprecated
    @DeprecatedApi(since = "5.5", forRemoval = true, removeAt = "5.8")
    private static final class ImportResolverWrapper extends gov.tak.api.importfiles.ImportResolver {
        private final ImportResolver _impl;
        private Method onFileSortedMethod = null;
        private static final String TAG = "ImportResolverWrapper";

        public ImportResolverWrapper(ImportResolver impl) {
            super(impl.getExt(),
                    impl.getDestinationPath(new File("ignore")).getParentFile(),
                    impl.getDisplayableName(),
                    gov.tak.platform.marshal.MarshalManager.marshal(impl.getIcon(), android.graphics.drawable.Drawable.class, Drawable.class));

            _impl = impl;

            try {
                Method m = _impl.getClass().getDeclaredMethod(
                        "onFileSorted", File.class, File.class, Set.class);
                onFileSortedMethod = m;
                onFileSortedMethod.setAccessible(true);
                Log.d(TAG, "performed direct wrap of the onFileSortedMethod for: " + _impl.getClass());
            } catch (Exception e) {
                for (Method m : _impl.getClass().getDeclaredMethods()) {
                    // harder than it has to be because of possible obfuscation
                    // after looking through all the mapping files for the last 3
                    // releases, it seems as though onFileSorted is not obfuscated, but
                    // just in case.
                    Class<?>[] parameters = m.getParameterTypes();
                    if (parameters.length == 3) {
                        if (parameters[0].equals(File.class) &&
                                parameters[1].equals(File.class) &&
                                parameters[2].equals(Set.class)) {

                            onFileSortedMethod = m;
                            onFileSortedMethod.setAccessible(true);
                            Log.d(TAG, "performed indirect wrap of the onFileSortedMethod for: " + _impl.getClass());
                            break;
                        }
                    }
                }
        ...
        public boolean getFileSorted() {
            return _impl.getFileSorted();
        }

        @Override
        public boolean match(final File file) {
            return _impl.match(file);
        }

        @Override
        protected void onFileSorted(final File src, final File dst,
                                    EnumSet<SortFlags> sortedFlags) {
            if (onFileSortedMethod != null) {
                try {
                    onFileSortedMethod.invoke(_impl, src, dst, convert(sortedFlags));
                } catch (InvocationTargetException | IllegalAccessException e) {
                    Log.e(TAG, "unable to execute onFileSortedMethod on: " + _impl.getClass(), e);
                }
            }
        }
        ...
        public boolean beginImport(File file, EnumSet<SortFlags> sortFlags) {
            return _impl.beginImport(file, convert(sortFlags));
        }

        @Override
        public void finalizeImport() {
            _impl.finalizeImport();
        }

        @Override
        public File getDestinationPath(File file) {
            return _impl.getDestinationPath(file);
        }
        ...
        public boolean directoriesSupported() {
            return _impl.directoriesSupported();
        }

        @Override
        public String getDisplayableName() {
            return _impl.getDisplayableName();
        }

        @Override
        public Drawable getIcon() {
            return gov.tak.platform.marshal.MarshalManager.marshal(_impl.getIcon(), android.graphics.drawable.Drawable.class, Drawable.class);
        }

        @Override
        public Pair<String, String> getContentMIME() {
            return _impl.getContentMIME();
        }

        private Set<ImportResolver.SortFlags> convert(EnumSet<SortFlags> sortFlags) {
            Set<ImportResolver.SortFlags> convertedFlags = new HashSet<>();
            for (SortFlags sortFlag : sortFlags) {
                String name = sortFlag.name();
                convertedFlags.add(ImportResolver.SortFlags.valueOf(name));
            }
