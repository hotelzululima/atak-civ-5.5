...
public class TargetBubbleReceiver extends BroadcastReceiver implements
    ...
    public static final String buttonClicked = "CONFIRM_OR_CANCEL";
    ...
    public TargetBubbleReceiver(MapView mapView) {
        ...
        _instance = this;
        sp = AtakPreferences.getInstance(_mapView.getContext());
        _cFormat = CoordinateFormat.find(sp.get(
                "coord_display_pref",
        ...
        confirmWidget = new MarkerDrawableWidget() {
            @Override
            public void orientationChanged() {
                final boolean isPortrait = isPortraitLayout(
                        _mapView.getContext());
                final float widthFactor = isPortrait ? 1.0f : 0.5f;
                final float width = _mapView.getWidth() * widthFactor;
                final float height = retSize;

                this.setSize(width, height);
                if (isPortrait) {
                    this.setPoint(0, _mapView.getHeight() - 2 * height);
                } else {
                    this.setPoint(0, _mapView.getHeight() - height);
                }
            }
        };
        confirmWidget.setDrawable(
                mapView.getResources().getDrawable(R.drawable.confirm_widget));
        cancelWidget = new MarkerDrawableWidget() {
            @Override
            public void orientationChanged() {
                final boolean isPortrait = isPortraitLayout(
                        _mapView.getContext());
                ...
                if (AtakActionBarListData.getOrientation(
                    ...
                    this.setPoint(width, _mapView.getHeight() - height);
                }
            }
        };
        cancelWidget.setDrawable(
                mapView.getResources().getDrawable(R.drawable.cancel_widget));

        confirmWidget.addOnClickListener(this);
        cancelWidget.addOnClickListener(this);
    ...
    public synchronized void setExitReticleVisible(boolean state) {
        if (state) {
            _rootLayoutWidget.removeWidget(confirmWidget);
            _rootLayoutWidget.addWidget(confirmWidget);
            _rootLayoutWidget.removeWidget(cancelWidget);
            _rootLayoutWidget.addWidget(cancelWidget);
        } else {
            _rootLayoutWidget.removeWidget(confirmWidget);
            _rootLayoutWidget.removeWidget(cancelWidget);
        }
    ...
    public void onMapWidgetClick(MapWidget widget, MotionEvent event) {
        if (widget == cancelWidget && _editPoint != null) {
    ...
    public void onReceive(Context cxt, Intent intent) {
        ...
        if (action.equals(FINE_ADJUST) ||
                action.equals("com.atakmap.android.maps.FOCUS") ||
                action.equals("com.atakmap.android.maps.UNFOCUS")) {

            if (action.equals("com.atakmap.android.maps.FOCUS")
    ...
    private void _generalEntry(Intent intent) {
        ...
        CoordinateEntryCapability.getInstance(getContext()).showDialog(
                ...
                _editPoint.getGeoPointMetaData(),
                _editPoint.getMovable(), _mapView.getPoint(), _cFormat, false,
    ...
    private PointMapItem findPoint(String uid) {
        if (uid != null) {
            ...
            MapItem s = ATAKUtilities.findAssocShape(item);
            if (s instanceof DrawingRectangle || s instanceof DrawingEllipse)
                item = s;
            ...
            if (item instanceof PointMapItem) {
                ...
                DrawingRectangle rectangle = (DrawingRectangle) item;
                return new RectanglePointMapItem(rectangle,
                        GeoPointMetaData.wrap(
                                rectangle.getClickPoint()));
            } else if (item instanceof DrawingEllipse) {
                DrawingEllipse ellipse = (DrawingEllipse) item;
                return new EllipsePointMapItem(ellipse, GeoPointMetaData.wrap(
                        ellipse.getClickPoint()));
    ...
    private GeoPointMetaData createPoint(final double lat, final double lon) {
        ...
        ATAKUtilities.getElevation(lat, lon, gpm);
        return gpm;
    }

    private void _setMapDataTargetPoint() {
        MetaDataHolder2 bundle = _mapView.getMapData();

        GeoPointMetaData bubblePoint = createPoint(_bubble.getLatitude(),
                _bubble.getLongitude());
        bundle.setMetaString("targetPoint", bubblePoint.get().toString());
    ...
    protected MapTargetBubble onTargetBubbleShow(MapView mapView,
        ...
        confirmWidget.orientationChanged();
        cancelWidget.orientationChanged();
        ...
        if (intent.hasExtra("point")) {
            ...
            if (point == null) // there was a failure somewhere
            {
                ...
                StringBuilder output = new StringBuilder(
                        action == null ? "malformed action" : action);
                output.append("\r\n");
    ...
    private final GestureDetector.OnGestureListener _gestureListener = new GestureDetector.OnGestureListener() {
        ...
        public boolean onFling(@Nullable MotionEvent e1,
                @NonNull MotionEvent e2, float velocityX,
                float velocityY) {
            return false;
        }
    ...
    static boolean isPortraitLayout(Context ctx) {
        return AtakActionBarListData.getOrientation(
                ctx) == AtakActionBarMenuData.Orientation.portrait;
    }
    ...
    private static class ShapePointMapItem extends MetaPointMapItem {
        ...
        ShapePointMapItem(EditablePolyline shape, int index, boolean line) {
            ...
            this.index = index;
            this.line = line;
            setEditable(shape.getEditable());
            setMovable(shape.getMovable() &&
                    (shape.getEditable()
                            || shape.getMetaBoolean("static_shape", false)));
        }
    ...
    private class RectanglePointMapItem extends MetaPointMapItem {

        private final Rectangle rect;

        RectanglePointMapItem(Rectangle circle, GeoPointMetaData point) {
            super(circle, point);
            this.rect = circle;
        }
        ...
        @Override
        public void setPoint(GeoPointMetaData point) {
            GeoPointMetaData oldPoint = rect.getCenter();
            rect.move(oldPoint, point);
            rect.persist(_mapView.getMapEventDispatcher(), null, getClass());
        }
    }

    private class EllipsePointMapItem extends MetaPointMapItem {

        private final DrawingEllipse ellipse;

        EllipsePointMapItem(DrawingEllipse ellipse, GeoPointMetaData point) {
            super(ellipse, point);
        ...
        public void setPoint(GeoPointMetaData point) {
            ...
            PointMapItem pmi = ellipse.getAnchorItem();
            pmi.setPoint(point);
            ellipse.persist(_mapView.getMapEventDispatcher(), null, getClass());
        }
    }
}
