...
public abstract class GeoFenceMonitor {
    ...
    protected synchronized boolean checkDismissed(PointMapItem item) {
        if (!_uidsToIgnore.isEmpty() && _uidsToIgnore.contains(item.getUID())) {
            Log.d(TAG,
    ...
    public static class Factory {
        ...
        public static GeoFenceMonitor Create(MapView view, MapGroup group,
            ...
            if (shape instanceof DrawingCircle) {
                ...
                EllipseGeoFenceMonitor monitor = new EllipseGeoFenceMonitor(
                        view, spatialCalc, fence, (DrawingEllipse) shape);
                if (!monitor.isValid()) {
                    Log.w(TAG, "Invalid EllipseGeoFenceMonitor");
                    return monitor;
                }

                Log.d(TAG,
                        "creating EllipseGeoFenceMonitor: "
                                + monitor);
                return monitor;
