...
class RouteElevationPresenter implements ChartSelectionListener,
        ...
        if (RouteElevationBroadcastReceiver.getInstance().isDropDownOpen()) {
            ...
            if (_route instanceof FloatingPointRoute) {
                ...
                if (indicies.length < 2 || index < indicies[0]
                    ...
                    _marker.draw(point.get(), AltitudeMode.Absolute);
                ...
                _marker.draw(gp, AltitudeMode.Absolute);
            } else {
                // Generic route
                AltitudeMode altMode = _route.getAltitudeMode();
                _marker.draw(point.get(), altMode);
            }
        }
    ...
    public void updateChart(Route route, RouteData data) {
        ...
        int altFmt = Integer.parseInt(_prefs.get("alt_unit_pref",
                String.valueOf(Span.ENGLISH)));

        String altDisplayPref = _prefs.get("alt_display_pref",
                "MSL");
        ...
        double startHeight = Double.NaN;
        double endHeight = Double.NaN;
        // XXX - less ew
        // if the route is a R&B line, obtain start/end

        _mapView.post(new Runnable() {
            @Override
            public void run() {
                // XXX - hackish - this whole bit needs to be redone so that Route ElevationPresenter knows
                // about the side screen.
                if (_view != null) { 
                    final ViewGroup vg = ((ViewGroup) _view.getParent());
                    if (vg != null) {
                        vg.findViewById(R.id.showLineOfSight_cb).setVisibility(
                                route.getMetaBoolean("isRangeAndBearing", false)
                                        ? View.VISIBLE
                                        : View.GONE);
                    }
                 }
        ...
        if (route.getMetaBoolean("isRangeAndBearing", false)) {
            startHeight = route.getPoint(0).get().getAltitude();
            endHeight = route.getPoint(route.getNumPoints() - 1).get()
                    .getAltitude();

            // normalize heights with data series
            if (altDisplayPref.equals("MSL")) {
                GeoPoint start = GeoPoint.createMutable();
                start.set(yValues[0].get());
                start.set(startHeight);
                startHeight = EGM96.getMSL(start);

                GeoPoint end = GeoPoint.createMutable();
                end.set(yValues[yValues.length - 1].get());
                end.set(endHeight);
                endHeight = EGM96.getMSL(end);
            }

            if (altFmt == Span.ENGLISH) {
                // convert meters to ft
                startHeight *= ConversionFactors.METERS_TO_FEET;
                endHeight *= ConversionFactors.METERS_TO_FEET;
            }
        }
        refreshChart(dataset, imageDataset, hValues, startHeight, endHeight);
    }
    ...
    private void refreshChart(final XYMultipleSeriesDataset dataset,
        ...
        _view.post(new Runnable() {
            ...
            public void run() {
                ...
                chart.setHighLow(startHeight, endHeight);
