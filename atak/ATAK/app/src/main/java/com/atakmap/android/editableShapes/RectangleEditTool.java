...
public class RectangleEditTool extends ButtonTool implements Undoable,
    ...
    public boolean onToolBegin(Bundle extras) {
        ...
        final GeoBounds bounds = _rectangle.getBounds(null);
        if (!bounds.intersects(_mapView.getBounds()))
            ATAKUtilities.scaleToFit(_rectangle);
    ...
    public class ManualRectangleEditReceiver extends BroadcastReceiver {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            double lat = Parsers.parseDouble(pointLat, Double.NaN);
            double lon = Parsers.parseDouble(pointLon, Double.NaN);
            double alt = Parsers.parseDouble(pointAlt, Double.NaN);
            if (Double.isNaN(lat) || Double.isNaN(lon))
                return;

            dropPoint = new GeoPoint(lat, lon, alt);
    ...
    public void onMoved(Rectangle r, GeoPointMetaData[] oldPoints,
        ...
        if (center != null) {
            EditAction act = new Rectangle.MovePointAction(
                    center, center.getGeoPointMetaData(), _rectangle,
                    oldPoints);
            run(act);
        }
    }
