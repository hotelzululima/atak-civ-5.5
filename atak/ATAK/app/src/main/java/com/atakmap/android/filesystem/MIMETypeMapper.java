...
public class MIMETypeMapper implements ContentTypeMapper {
    ...
    public static Intent getOpenIntent(Context context, File file) {
        ...
        if (!FileSystemUtils.isFile(file)) {
            Log.d(TAG, "cannot open a non file: " + file);
            return null;
        }
        ...
        if (FileSystemUtils.isEmpty(mime)) {
            Log.d(TAG, "cannot open a file with an empty mime type: " + file);
            return null;
        }

        // fix up for testing purposes
        if (mime.equals(
        ...
        try {
            ...
            if (resolveInfo == null) {
                Log.d(TAG,
                        "cannot resolve information for the following intent: "
                                + intent + " " + file);
                return null;
            }
            return intent;
    ...
    public static void openFile(final File file, final Context context) {
        ...
        if (intent == null) {
            ...
            Log.w(TAG, String.format(
                    "Failed to resolve Activity for MIME type: %s", mime));
