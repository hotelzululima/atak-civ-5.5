...
public class BattlePositionLayoutHandler implements View.OnClickListener {
    ...
    private BPHAGridView _3x3 = null;
    ...
    synchronized private void createView(LayoutInflater inflater) {
        ...
        _3x3 = _RootView.findViewById(R.id.gridView2x3);
        ...
        ((View) _3x3.getParent()).setOnClickListener(this);
        ...
        int col = 2;
        try {
            AtakPreferences sp = AtakPreferences
                    .getInstance(_RootView.getContext());
            row = sp.get("bpha_custom_row", 2);
            col = sp.get("bpha_custom_col", 2);
        ...
        customOKButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ((View) _3x3.getParent()).setSelected(false);
        ...
        _RowsET.addTextChangedListener(new AfterTextChangedWatcher() {
            ...
            public void afterTextChanged(Editable editable) {
                String rows = _RowsET.getText().toString();
                if (rows.isEmpty())
                    return;

                int r = Parsers.parseInt(rows, 1);
                if (r < 1) {
                    _ColumnsET.setText("");
                    return;
                }
                ...
                try {
                    AtakPreferences sp = AtakPreferences.getInstance(
                            _RootView.getContext());
                    sp.set("bpha_custom_row", _BPHA.getRows());
        ...
        _ColumnsET.addTextChangedListener(new AfterTextChangedWatcher() {
            @Override
            public void afterTextChanged(Editable editable) {
                String columns = _ColumnsET.getText().toString();
                if (columns.isEmpty())
                    return;

                int c = Parsers.parseInt(columns, 1);
                ...
                try {
                    AtakPreferences sp = AtakPreferences.getInstance(
                            _RootView.getContext());
                    sp.set("bpha_custom_col", _BPHA.getColumns());
                } catch (Exception ignored) {
                }
            }
        });
    ...
    private void setSelected() {
        ...
        } else if (_BPHA.getRows() == 3 && _BPHA.getColumns() == 3) {
            ((View) _2x2.getParent()).setSelected(false);
            ((View) _3x3.getParent()).setSelected(true);
            ((View) _3x2.getParent()).setSelected(false);
            _Custom.setSelected(false);
