...
public class AddEditAlias {
    ...
    public void addEditConnection(final ConnectionEntry currentCE) {
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (p == Protocol.RTSP ||
                        ...
                        p == Protocol.HTTP ||
                        p == Protocol.HTTPS) {
                    if (aliasVal.startsWith(p.toURL()))
                        aliasVal = aliasVal.substring(p.toURL().length());
                ...
                if (p == Protocol.SRT && path.length() > 1) {
                    if (path.charAt(0) == '/') {
                ...
                if (p == Protocol.RAW) {
                    ...
                    Toast.makeText(
                            ...
                            String.format(LocaleUtil.getCurrent(),
                                    context.getString(
                                            R.string.problem_with_video_alias),
                                    host),
        ...
        ArrayAdapter<NetworkDeviceStringWrapper> adapter = getNetworkDeviceAdapter(context);
        ...
        if (preferredNetworkSpinner.getAdapter() != null && currentCE != null) {
            final SpinnerAdapter spAdapter = preferredNetworkSpinner.getAdapter();
            for (int i = 0; i < spAdapter.getCount(); ++i) {
                final NetworkDeviceStringWrapper ndsw = (NetworkDeviceStringWrapper) spAdapter.getItem(i);
                if (ndsw.nd != null) {
                    final String address = NetworkManagerLite
        ...
        if (currentCE != null) {
            ...
            if (currentCE.getProtocol().equals(Protocol.RAW)) {
                ...
                if (currentCE.getPath() != null
                        && !currentCE.getPath().trim().isEmpty())
                ...
                if (path.length() > 1) {
                    ...
                    if (currentCE.getProtocol().equals(Protocol.SRT) && path.startsWith("?")) {
                        // skip as ? is a valid start for an alias
    ...
    @NonNull
    public static ArrayAdapter<NetworkDeviceStringWrapper>
                getNetworkDeviceAdapter(Context context) {
        ...
        List<NetworkDeviceStringWrapper> networkList = new ArrayList<>();
    ...
    public static class NetworkDeviceStringWrapper {
        public final NetworkDevice nd;
