...
public final class GLSegmentFloatingLabel {
        ...
        for (int x = 0; x < pts.length; x++) {
            ...
            if (!_clampToGround && zpos > 1f) {
                ...
                if (x0 != x1) {
                    ...
                    final double losdx = ortho.currentPass.scene.camera.location.x
                            - ortho.currentPass.scene.camera.target.x;
                    final double losdy = ortho.currentPass.scene.camera.location.y
                            - ortho.currentPass.scene.camera.target.y;
                    final double losdz = ortho.currentPass.scene.camera.location.z
                            - ortho.currentPass.scene.camera.target.z;

                    if (near == null)
                        near = new Plane(new Vector3D(losdx, losdy, losdz),
                                new PointD(
                                        ortho.currentPass.scene.camera.location.x
                                                - (losdx / MathUtils.distance(
                                                        losdx,
                                                        losdy, losdz, 0d, 0d,
                                                        0d))
                                                        * ortho.currentPass.scene.camera.nearMeters,
                                        ortho.currentPass.scene.camera.location.y
                                                - (losdy
                                                        / MathUtils.distance(
                                                                losdx,
                                                                losdy, losdz,
                                                                ...
                                                                0d))
                                                        * ortho.currentPass.scene.camera.nearMeters,
                                        ortho.currentPass.scene.camera.location.z
                                                - (losdz
                                                        / MathUtils.distance(
                                                                losdx,
                                                                losdy, losdz,
                    ...
                    ortho.currentPass.scene.mapProjection.forward(pts[x0],
                    ...
                    ortho.currentPass.scene.mapProjection.forward(pts[x1],
                            ortho.scratch.pointD);
                    ...
                    if (isect != null) {
                        ortho.currentPass.scene.forward.transform(isect, isect);
                        xpos = (float) isect.x;
                        ypos = (float) isect.y;
                        zpos = (float) isect.z;
                    }
        ...
        if (xmax - xmin < _textWidth
            ...
            if (ptLen % 2 == 0) {
                ...
                if (_clampToGround) {
                    ...
                    ortho.currentPass.scene.forward(textLoc,
                            ortho.scratch.pointD);
                    ...
                    ortho.scratch.geo.set(GeoCalculations.pointAtDistance(
                            ...
                            _textWidth / 2d * ortho.currentPass.scene.gsd));
    ...
    public void draw(GLMapView view) {
        ...
        // cut off, shift it
        final float l = view.currentPass.left + _insetLeft;
        final float r = view.currentPass.right - _insetRight;
        final float b = view.currentPass.bottom + _insetBottom;
        final float t = view.currentPass.top - _insetTop;
        if (Rectangle.contains(view.currentPass.left, view.currentPass.bottom,
                view.currentPass.right, view.currentPass.top,
