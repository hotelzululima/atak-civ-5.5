...
public class SpotMapPalletFragment extends Fragment {
    ...
    private void _onTypeCheckedChanged(CompoundButton buttonView,
        ...
        if (MetricsApi.shouldRecordMetric()) {
            ...
            b.putString(MetricsUtils.FIELD_MAPITEM_TYPE, "b-m-p-s-m");
            b.putInt("color", _curColor);
            MetricsUtils.record(MetricsUtils.CATEGORY_TOOL,
                    MetricsUtils.EVENT_CLICKED,
                    "SpotMapPalletFragment",
                    "SpotMapPalletFragment",
                    b);
        }
    ...
    private void saveCustomColors(Integer c1, Integer c2, Integer c3) {
        ...
        if (c1 == null) {
            _prefs.remove("spotmap.customcolor.1");
        } else {
            _prefs.set("spotmap.customcolor.1", c1);
        }

        if (c2 == null) {
            _prefs.remove("spotmap.customcolor.2");
        } else {
            _prefs.set("spotmap.customcolor.2", c2);
        }

        if (c3 == null) {
            _prefs.remove("spotmap.customcolor.3");
        } else {
            _prefs.set("spotmap.customcolor.3", c3);
        }
    }
    ...
    private void loadCustomColors(boolean buttonChecked) {
        ...
        if (_prefs.contains("spotmap.customcolor.1")) {
            recentCustomColors
                    .add(_prefs.get("spotmap.customcolor.1", Color.WHITE));
        }
        if (_prefs.contains("spotmap.customcolor.2")) {
            recentCustomColors
                    .addLast(_prefs.get("spotmap.customcolor.2", Color.WHITE));
        }
        if (_prefs.contains("spotmap.customcolor.3")) {
            recentCustomColors
                    .addLast(_prefs.get("spotmap.customcolor.3", Color.WHITE));
        }
