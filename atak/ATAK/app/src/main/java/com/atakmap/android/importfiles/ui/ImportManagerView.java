...
public class ImportManagerView extends BroadcastReceiver implements
    ...
    private AtakPreferences defaultPrefs;

    public ImportManagerView(MapView mapView) {
        _mapView = mapView;
        _context = mapView.getContext();
        defaultPrefs = AtakPreferences.getInstance(
                _mapView.getContext());
        ...
        b.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (selectedFiles.isEmpty()) {
                    ...
                    defaultPrefs.set("lastDirectory",
                            importView.getCurrentPath());

                    promptImportAction(selectedFiles);
    ...
    private void promptImportAction(final List<File> selectedFiles) {

        if (selectedFiles.isEmpty()) {
            return;
        }

        final int importStrategy = defaultPrefs.get("import_strategy_default",
                -1);

        switch (importStrategy) {
            case ImportFileTask.FlagImportInPlace:
            case ImportFileTask.FlagCopyFile:
                importAction(selectedFiles, importStrategy);
            default: {
                final MapView mapView = MapView.getMapView();
                final Context context = mapView.getContext();

                TileButtonDialog d = new TileButtonDialog(mapView);
                d.addButton(context.getDrawable(R.drawable.copy_sort),
                        context.getString(R.string.import_copy));
                d.addButton(context.getDrawable(R.drawable.move_sort),
                        context.getString(R.string.import_move));
                d.addButton(context.getDrawable(R.drawable.inplace_sort),
                        context.getString(R.string.import_use_in_place));
                d.setTitle(
                        context.getString(R.string.suggested_import_strategy));
                d.setMessage(
                        context.getString(R.string.import_strategy_summary));

                final CheckBox cb = new CheckBox(context);
                cb.setText(R.string.remember_this_choice);
                //d.setCustomView(cb);
                ...
                d.setOnClickListener(new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        int importStrategy = ImportFileTask.FlagImportInPlace;
                        if (which == 0)
                            importStrategy = ImportFileTask.FlagCopyFile;
                        else if (which == 1)
                            importStrategy = 0; // do not set based on comments from ImportFileTask Flags

                        if (cb.isChecked()) {
                            defaultPrefs.set("import_strategy_default",
                                    importStrategy);
                        }
    ...
    private void importAction(List<File> selectedFiles, int importStrategy) {
