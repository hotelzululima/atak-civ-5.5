...
final class FilteredContactsDatabase {
    ...
    private static class DBColumn {
        @FortifyFinding(finding = "Password Management: Hardcoded Password", rational = "This is only a database key and not a password")
        public String key;
        public String type;
    ...
    private void initDatabase() {
        ...
        DatabaseIface newFilteredContactsDb = ATAKDatabaseHelper
                .getPrefetchDb(FILTEREDCONTACTS_DB_FILE);
    ...
    public FilteredContactsDatabase() {
        if (!IOProviderFactory.exists(FILTEREDCONTACTS_DB_FILE.getParentFile()))
            ...
            if (!IOProviderFactory
                ...
                File pFile = FILTEREDCONTACTS_DB_FILE.getParentFile();
                Log.e(TAG, "Failed to make Directory at " +
                        (pFile == null ? "null parent"
    ...
    private void onUpgrade(DatabaseIface db, int oldVersion, int newVersion) {
        ...
        switch (oldVersion) {
            ...
            default:
                db.execute("DROP TABLE IF EXISTS " + TABLE_FILTEREDCONTACTS,
                        null);
                onCreate(db);
        }
    }
