...
public class NavigationCue {
    ...
    private static String flipText(String oldText) {
        String newText;
        if (Pattern.compile("Stay|Turn Right",
                Pattern.CASE_INSENSITIVE).matcher(oldText).find())
            newText = oldText.replaceFirst("(?i)Right", "Left");
        else if (Pattern.compile("Stay|Turn Left",
                Pattern.CASE_INSENSITIVE).matcher(oldText).find())
            newText = oldText.replaceFirst("(?i)Left", "Right");
        else // leave text unmodified
            newText = oldText;
