...
public abstract class ImageContainer implements OnTouchListener,
        ...
        View.OnLayoutChangeListener, Disposable {
    //==================================
    //
    //  PUBLIC INTERFACE
    //
    ...
    synchronized public View getView() {
        ...
        if (AtakPreferences.getInstance(
                view.getContext()).get("dexControls", false)) {
    ...
    protected String getFormattedLocation(GeoPoint loc) {
        ...
        CoordinateFormat coordFormat = CoordinateFormat.find(prefs.get(
                "coord_display_pref",
                defaultCoordFormat));
        return CoordinateFormatUtilities.formatToString(loc, coordFormat)
                + "\n" + AltitudeUtilities.format(loc, prefs.getSharedPrefs());
    }
