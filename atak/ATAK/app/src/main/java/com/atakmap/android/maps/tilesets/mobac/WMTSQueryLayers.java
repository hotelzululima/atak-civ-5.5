...
public class WMTSQueryLayers extends QueryLayers {
    ...
    // During discovery a dimension can be specified and applied to the ResourceURL
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    if (parser.getName().equalsIgnoreCase("Layer")) {
                        try {
                            ...
                            Log.e(TAG, "skipping malformed layer");
                        }
        ...
        do {
            ...
            switch (eventType) {
                case XmlPullParser.START_TAG:
                    ...
                    } else if (inTag.equalsIgnoreCase("ResourceURL")) {
                        template = parser.getAttributeValue("", "template");
                        if (template != null && template.contains("{TileMatrix}") &&
                                template.contains("{TileCol}") && template.contains("{TileRow}")) {
                    ...
                    } else if (inTag.equalsIgnoreCase("Dimension")) {
                        dimensions.add(parseDimension(parser));
    ...
    private void parseTileMatrixSet(XmlPullParser parser) throws IOException,
        ...
        do {
            ...
            switch (eventType) {
                ...
                case XmlPullParser.TEXT:
                    ...
                    } else if (inTag.equalsIgnoreCase("SupportedCRS") || inTag.equalsIgnoreCase("crs")) {
                        tileMatrixCRS = parser.getText();
                    }
                    break;
                case XmlPullParser.END_DOCUMENT:
                    throw new IOException("Unexpected end of document");
        ...
        if (tileMatrixCRS != null) {
            for (TileMatrix tm : tileMatricies) {
                tm.crs = tileMatrixCRS;
            }
        }
    ...
    private Dimension parseDimension(XmlPullParser parser) throws IOException,
            XmlPullParserException {
        Stack<String> tagStack = new Stack<>();
        String tagName = parser.getName();
        tagStack.push(tagName);
            ...
            switch (eventType) {
                ...
                case XmlPullParser.TEXT:
                    ...
                    if (inTag.equalsIgnoreCase("Identifier")) {
                         ...
                         returnDimension.identifier = parser.getText();
                    } else if (inTag.equalsIgnoreCase("Default")) {
                        returnDimension.defaultValue = parser.getText();
                    }
                    break;
                case XmlPullParser.END_DOCUMENT:
                    throw new IOException("Unexpected end of document");
    ...
    private static class WMTSWebMapLayer extends WebMapLayer {
        ...
        private String findCommonPrefix(String tileMatrixSet) {
            ...
            String sharedPrefix = "";
            if (matricies != null && !matricies.isEmpty()) {
        ...
        public File writeMobacXML(Style style) throws IOException {
            ...
            } else if (!tileMatrixSetLinks.isEmpty()) {
            ...
            List<TileMatrix> matrices = matrixMap.get(tileMatrixSet);
            int numMatrixLevels = matrices != null ? matrices.size() : 0;
            ...
            // use the template otherwise generate a template when none was provided
            if (resourceUrl != null) {
                // obviously broken if the style or its contents, tileMatrixSet or the format
                // is not set but the resource url is referencing it.
                if (style != null && style.getName() != null)
                    resourceUrl = resourceUrl.replace("{style}", style.getName());

                if (tileMatrixSet != null)
                    resourceUrl = resourceUrl.replace("{TileMatrixSet}", tileMatrixSet);

                if (format != null) {
                    // format is currently only set in 2 cases, png or jpg
                    if (format.equalsIgnoreCase("image/png")) {
                        resourceUrl = resourceUrl.replace("{format}", "png");
                    } else {
                        resourceUrl = resourceUrl.replace("{format}", "jpg");
                    }
                }

                for (Dimension dimension: ((WMTSQueryLayers) queryLayer).dimensions) {
                    if (dimension != null && dimension.identifier != null && dimension.defaultValue != null) {
                        resourceUrl = resourceUrl.replace("{" + dimension.identifier + "}", dimension.defaultValue);
                    }
                }

                if (queryLayer.baseURL.getProtocol().equalsIgnoreCase("https")) {
                    resourceUrl = resourceUrl.replace("http://", "https://");
                }

                url = resourceUrl;
            } else if (protocol.equals(GetTileProtocol.KVP)) {
            ...
            try (Writer w = IOProviderFactory.getFileWriter(f);
                ...
                out.println(
                        "    <url>" + WMSQueryLayers.escape(url) + "</url>");
                ...
                List<TileMatrix> tmList = matrixMap.get(tileMatrixSet);
                if (tmList != null && !tmList.isEmpty()) {
                    TileMatrix tm = tmList.get(0);
                    if (tm != null && tm.crs != null) {
                        String crs = tm.crs;
                        if (crs.startsWith("urn:ogc:def:crs:")) {
                            String[] arr = crs.split(":", -1);
                            if (arr.length > 6) {
                                if (!arr[4].isEmpty() && !arr[6].isEmpty())
                                    crs = arr[4] + ":" + arr[6];
                            }
                        }
                        out.println("    <coordinatesystem>" + crs + "</coordinatesystem>");
                    }
                }
                out.println("</customMapSource>");
            }
