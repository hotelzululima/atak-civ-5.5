...
/**
 * Attempt to load system level changes that can be made to ATAK by installing one or more APKs.
...
public class SystemComponentLoader {
    ...
    private static boolean check(final ApplicationInfo info,
        ...
        try {
            ...
            final String pluginApiVersion = getCoreApi(getAPI(info)) + ".CIV";
            final String coreAtakApiVersion = getAPI(
            ...
            // version
            if (AtakPluginRegistry.compareVersions(pluginApiVersion,
                    "4.10.0.CIV") >= 0 &&
                    AtakPluginRegistry.compareVersions(pluginApiVersion,
                            coreAtakApiVersion) <= 0) {
                // success
            } else {
                Log.d(TAG, "api does not match, needed: " + pluginApiVersion
                        + " but the system plugin had: " + coreAtakApiVersion);
                return false;
            }
     ...
     *    <a href="https://thehackernews.com/2014/07/android-fake-id-vulnerability-allows_29.html">Fake ID</a>
     *    <a href="https://www.blackhat.com/docs/us-14/materials/us-14-Forristal-Android-FakeID-Vulnerability-Walkthrough.pdf">Vulnerability Walkthough</a>
    ...
    private static boolean verifySignature(final Context context,
        ...
        try {
            ...
            if (SystemPluginValidator.checkAppTransparencySignature(context,
                    pkgname)) {
                return true;
            }
        } catch (Exception e) {
    ...
    private static List<ApplicationInfo> applicationCache = null;
    ...
    private static ApplicationInfo getApplicationInfo(final PackageManager pm,
        ...
        for (ApplicationInfo ai : applicationCache) {
    ...
    /**
     *
     * Attempt to load the Comms system component.
     *
     * @param activity The Activity contect used to load the system component
     */
    public synchronized static void initializeComms(final Activity activity) {
        if (commsComponent != null)
            return;
        ...
        try {
            long start = SystemClock.elapsedRealtime();
            info = getApplicationInfo(pm, "com.atakmap.app.flavor.comms.");
            PackageInfo pInfo = pm.getPackageInfo(info.packageName,
                    PackageManager.GET_META_DATA);
            Log.d(TAG, "loading comms "
                    + (SystemClock.elapsedRealtime() - start) + "ms");
            commsName = info.loadLabel(pm) + " " + pInfo.versionName;
            commsCrashInfo = infoToJson(info.packageName, activity);
        } catch (Exception e) {
            Log.d(TAG, "no comms system component found, using default");
            return;
        }
        ...
        try {
            ...
            final Class<?> c = pluginClassLoader
                    .loadClass("com.atakmap.android.CommsComponent");

            if (!ensureAbstractImpl(AbstractSystemComponent.class, c))
                return;

            commsComponent = (AbstractSystemComponent) c.newInstance();
            commsComponent.setPluginContext(pluginContext);
            commsComponent.setAppContext(activity);
        } catch (Throwable e) {
            Log.d(TAG, "error loading the comms system component", e);
            commsComponent = null;
        }
    }
     ...
     * @return the array of the system components
     */
    ...
    /**
     * Returns the component name if a custom encryption module is installed.
     */
    ...
    /**
     * @eturn the component name if a custom comms module is installed.
     */
    public static String getCommsComponentName() {
        return commsName;
    }
    ...
    public static String getCrashLogInfo() {
        List<String> crashInfo = new ArrayList<>(Arrays.asList(flavorCrashInfo,
                encryptionCrashInfo, raptorApiCrashInfo, commsCrashInfo));
